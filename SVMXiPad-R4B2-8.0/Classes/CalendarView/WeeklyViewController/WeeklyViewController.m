...
- (void) setupEvents
{
 ...
 //   NSLog(@"%@ %@", startDate, endDate);
    ...
    appDelegate.wsInterface.eventArray = [appDelegate.calDataBase GetEventsFromDBWithStartDate:startDate
                                                                        endDate:endDate];
    NSLog(@"%@", appDelegate.wsInterface.eventArray);
    ...
    if ([touch view] == eventView.view)
    {
        ...
        else
        {
            ...
            if(ContinueRescheduling == TRUE)
            {
                ...
                appDelegate.wsInterface.eventArray = [appDelegate.calDataBase GetEventsFromDBWithStartDate:_startDate  endDate:_endDate];
              /*  while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, 1, FALSE))
                {
                    NSLog(@"WeeklyViewController touchesEnded in while loop");
                    if ( appDelegate.wsInterface.didRescheduleEvent == TRUE )
