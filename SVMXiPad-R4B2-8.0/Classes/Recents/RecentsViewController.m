...
- (void) showSFMWithProcessId:(NSString *)processId recordId:(NSString *)recordId objectName:(NSString *)objectName
{
    ...
    NSMutableDictionary * page_layoutInfo = [appDelegate.databaseInterface  queryProcessInfo:processId object_name:objectName];
    
    if(page_layoutInfo == nil)
    {
        UIAlertView * no_page_Layout = [[UIAlertView alloc] initWithTitle:@"" message:@"No Page Layout is present for the process" delegate:nil cancelButtonTitle:@"Cancel" otherButtonTitles:nil, nil];
    ...
    BOOL flag = [appDelegate.databaseInterface EntryCriteriaForRecordFortableName:objectName record_id:recordId expression:expression_id];
