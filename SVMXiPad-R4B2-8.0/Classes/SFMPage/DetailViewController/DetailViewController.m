...
-(void)fillSFMdictForOfflineforProcess:(NSString *) processId forRecord:(NSString *)recordId
{
   ...
   // databaseIntefaceSfm * database = [[databaseIntefaceSfm alloc] init];
    ...
    {
        UIAlertView * no_page_Layout = [[UIAlertView alloc] initWithTitle:@"" message:@"No Page Layout is present is present for the process" delegate:nil cancelButtonTitle:@"Cancel" otherButtonTitles:nil, nil];
    ...
    if([process_type isEqualToString:@"VIEWRECORD"] )
    {
        ...
        if(!Entry_criteria)
        {
            UIAlertView * enty_criteris = [[UIAlertView alloc] initWithTitle:@"" message:@" Record does Not meet  an Entry Criteria, Please Switch to some Other Process" delegate:nil cancelButtonTitle:@"Cancel" otherButtonTitles:nil, nil];
            [enty_criteris show];
    ...
    else if ([process_type isEqualToString:@"SOURCETOTARGET"] || [process_type isEqualToString:@"SOURCETOTARGETONLYCHILDROWS"])
    {
        ...
        for(int j= 0; j < [details count]; j++)
        {
            ...
            NSMutableDictionary * process_components = [appDelegate.databaseInterface getProcessComponentsForComponentType:TARGETCHILD process_id:processId layoutId:detail_layout_id objectName:detailObjectName];//[appDelegate.databaseInterface  getValueMappingForlayoutId:detail_layout_id process_id:process_id objectName:detailObjectName];
            
            NSString * source_child_object_name = [process_components objectForKey:SOURCE_OBJECT_NAME];
            
            NSString * parent_column_name = [process_components objectForKey:PARENT_COLUMN_NAME];//[appDelegate.databaseInterface getParentColumnNameFormChildInfoTable:SFChildRelationShip childApiName:source_child_object_name parentApiName:source_parent_object_name];
            ...
            NSMutableArray * object_mapping_array = [appDelegate.databaseInterface getObjectMappingForMappingId:process_components source_record_id:appDelegate.sfmPageController.sourceRecordId field_name:parent_column_name];
...
- (void) showTroubleshooting
{
    ...
    troubleshooting.productId = @"01t70000002xWaeAAE";
...
- (void) accessoryTapped:(id)sender
{
    if(appDelegate.isWorkinginOffline)
    {
        ...
        NSMutableDictionary * process_components = [appDelegate.databaseInterface getProcessComponentsForComponentType:TARGETCHILD process_id:process_id layoutId:layout_id objectName:detailObjectName];//[appDelegate.databaseInterface  getValueMappingForlayoutId:layout_id process_id:process_id objectName:detailObjectName];
...
-(void)SaveRecordIntoPlist:(NSString *)record_id objectName:(NSString *) object_name;
{
    ...
    if([process_id length] == 0  || process_id == nil)
    {
        UIAlertView * alert1 = [[UIAlertView alloc] initWithTitle:@"" message:@"No View Process is associated with the Process" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alert1 show];
        [alert1 release];
        return;
    }
