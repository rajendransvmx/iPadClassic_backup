...
- (void) viewDidLoad
{
    ...
    BOOL status1 = [Reachability connectivityStatus];
    NSLog(@"%d", status1);
    if (status1 == NO)
    {
        UIAlertView * _alert = [[UIAlertView alloc] initWithTitle:@"ServiceMax" message:@"Map is displayed only when online connectivity is available" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [_alert show];
        [_alert release];
...
- (IBAction) viewJobDetail
{
    ...
    {
        didFindProcess = YES;
        appDelegate.sfmPageController.processId = @"1CustIPAD-012";
    }
...
{    
    ...
    //radha
    NSString * colorCode = [appDelegate.database getColorCodeForPriority:[dict objectForKey:WHATID]];
