...
{
    NSString * _query = [[NSString stringWithFormat:@"SELECT Id, Name from Document WHERE Keywords LIKE '%%%@%%'", productName] retain];
...
- (void) setSignImageData:(NSData *)imageData
{
    ...
    NSString * fileName = [appDelegate.sfmPageController.detailView.currentRecordId substringToIndex:15];
    ...
    NSLog(@"%@", appDelegate.sfmPageController.detailView.currentRecordId);
    [obj setFieldValue:appDelegate.sfmPageController.detailView.currentRecordId field:@"ParentId"];
