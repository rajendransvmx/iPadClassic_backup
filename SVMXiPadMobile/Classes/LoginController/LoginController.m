...
-(IBAction)callLogin:(id)sender
{
    ...
    if ( status == NO )
    {
        BOOL retVal = [appDelegate.database isUsernamePresent:txtUsernameLandscape.text password:txtPasswordLandscape.text];
        if ( retVal == YES )
        {
            appDelegate.wsInterface.tagsDictionary     =  [appDelegate.wsInterface getDefaultTags];
            appDelegate.wsInterface.createProcessArray =  [appDelegate.database getProcessFromDatabase];
            
            //for create process 
            NSMutableArray * createprocessArray = [appDelegate.databaseInterface getAllTheProcesses:@"C"];
            ...
            [appDelegate.database startQueryConfiguration];
