...
- (void) setupTasksForDate:(NSString *)date
{
    ...
    _tasks = [appDelegate.database didGetTaskFromDB:date];
    NSLog(@"%@", _tasks);
    if (taskView == nil)
        taskView = [[TaskViewController alloc] initWithNibName:@"TaskViewController" bundle:nil];
...
- (void) refresh
{
    ...
    NSString *endDate = [weekBoundaries objectAtIndex:1];
    startDate = [iOSInterfaceObject getGMTFromLocalTime:appDelegate.wsInterface.startDate];
    endDate = [iOSInterfaceObject getGMTFromLocalTime:appDelegate.wsInterface.endDate];
    ...
    //Abinash & radha 24-10-11    
    startDate = [startDate stringByReplacingCharactersInRange:NSMakeRange(11, 5) withString:@"18:29"];
    endDate = [endDate stringByReplacingCharactersInRange:NSMakeRange(11, 5) withString:@"18:30"];    
    
    appDelegate.wsInterface.eventArray = [appDelegate.database GetEventsFromDBWithStartDate:startDate  endDate:endDate];
    [self reloadCalendar];
...
- (void) setDate:(NSUInteger)date
{
    ...
    NSString *startDate = [iOSInterfaceObject getGMTFromLocalTime:_startDate];
    NSString *endDate = [iOSInterfaceObject getGMTFromLocalTime:_endDate];
    startDate = [startDate stringByReplacingOccurrencesOfString:@"T" withString:@" "];
    startDate = [startDate stringByReplacingOccurrencesOfString:@"Z" withString:@""];
    endDate = [endDate stringByReplacingOccurrencesOfString:@"T" withString:@" "];
    endDate = [endDate stringByReplacingOccurrencesOfString:@"Z" withString:@""];
    ...
    //Abinash & radha 24-10-11
 //   startDate = [startDate stringByReplacingOccurrencesOfString:@"13" withString:@"19"];
  //  endDate = [endDate stringByReplacingOccurrencesOfString:@"12" withString:@"18"];
    
    startDate = [startDate stringByReplacingCharactersInRange:NSMakeRange(11, 5) withString:@"18:29"];
    endDate = [endDate stringByReplacingCharactersInRange:NSMakeRange(11, 5) withString:@"18:30"];
    
    appDelegate.wsInterface.eventArray = [appDelegate.database GetEventsFromDBWithStartDate:startDate endDate:endDate];   //Shrinivas 
...
- (void) setEventsView:(NSString *)_date
{
    ...
    //    NSLog(@"eventDate = %@", eventDate);
        ...
        NSString * colourCode = [appDelegate.database getColorCodeForPriority:[dict objectForKey:WHATID]];
            ...
            else
            {
                ...
                if(Continue_rescheduling)
                {
                    ...
                    if ([updatestartDateTime length] > 0 && [updateendDateTime length] > 0)
                    {
                        ...
                        [appDelegate.database updateMovedEventWithStartTime:updatestartDateTime EndDate:updateendDateTime RecordID:eventView.eventId];
                        [activity stopAnimating];
...
- (void) showSFMWithDayEvent:(NSDictionary *)event
{
    ...
    else
    {
        ...
        appDelegate.sfmPageController.processId = @"1CustIPAD-012"; //(processId != nil)?processId:[event objectForKey:PROCESSID];
   ...
   /* WSInterface *wsinterface = [[WSInterface alloc] init];
    wsinterface.delegate = appDelegate.sfmPageController.detailView; 
    [wsinterface  calldidrecivepageLayout];*/
