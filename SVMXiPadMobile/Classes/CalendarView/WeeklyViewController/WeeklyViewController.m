...
- (void) setupEvents
{
 ...
 //   NSLog(@"%@ %@", startDate, endDate);
    ...
    appDelegate.wsInterface.eventArray = [appDelegate.database GetEventsFromDBWithStartDate:startDate
                                                                        endDate:endDate];
    NSLog(@"%@", appDelegate.wsInterface.eventArray);
        ...
        else
        {
            ...
            if(ContinueRescheduling == TRUE)
            {
                ...
                if ([updatestartDateTime length] > 0 && [updateendDateTime length] > 0)
                {
                    ...
                    NSMutableArray * weekBound = [calendar getWeekBoundaries:[updatestartDateTime substringToIndex:10]];
                ...
                //Shrinivas 
                NSString *_startDate = [iOSInterfaceObject getGMTFromLocalTime:appDelegate.wsInterface.startDate];
                NSString *_endDate = [iOSInterfaceObject getGMTFromLocalTime:appDelegate.wsInterface.endDate];
                _startDate = [_startDate stringByReplacingOccurrencesOfString:@"T" withString:@" "];
                _startDate = [_startDate stringByReplacingOccurrencesOfString:@"Z" withString:@""];
                _endDate = [_endDate stringByReplacingOccurrencesOfString:@"T" withString:@" "];
                _endDate = [_endDate stringByReplacingOccurrencesOfString:@"Z" withString:@""];
                
                //Abinash & radha 24-10-11
                startDate = [startDate stringByReplacingCharactersInRange:NSMakeRange(11, 5) withString:@"18:29"];
                endDate = [endDate stringByReplacingCharactersInRange:NSMakeRange(11, 5) withString:@"18:30"];
                
                appDelegate.wsInterface.eventArray = [appDelegate.database GetEventsFromDBWithStartDate:_startDate  endDate:_endDate];
              /*  while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, 1, FALSE))
                {
                    NSLog(@"WeeklyViewController touchesEnded in while loop");
                    if ( appDelegate.wsInterface.didRescheduleEvent == TRUE )
