        ...
        case SFMEditPageMasterSectionTypeAttachment:
            if ([self isAttachmentEnabled]) {
                if (indexPath.row == 0 ) {
                    ...
                    title = kAttachmentImagesAndVideos;
                }
            }
            break;
        default:
...
- (id)attachmentViewControllerForIndexPath:(NSIndexPath *)indexPath
{
    id viewController = nil;
    if ([[self rowTitleForIndexPath:indexPath] isEqualToString:[[TagManager sharedInstance] tagByName:kTagDocuments]])
    {
        ...
        ((DocumentsViewController*)viewController).isViewMode = NO;
        ((DocumentsViewController*)viewController).recordId = self.sfmPage.recordId;
        ((DocumentsViewController*)viewController).parentId = [SFMPageHelper getSfIdForLocalId:self.sfmPage.recordId objectName:self.sfmPage.objectName];
        ((DocumentsViewController*)viewController).parentSFObjectName = self.sfmPage.objectName;
        ((DocumentsViewController*)viewController).parentObjectName = [SFMPageHelper getObjectLabelForObjectName:self.sfmPage.objectName];
    }
    else if ([[self rowTitleForIndexPath:indexPath] isEqualToString:kAttachmentImagesAndVideos])
    {
        ...
        ((ImagesVideosViewController*)viewController).isViewMode = NO;
        ((ImagesVideosViewController*)viewController).parentId = [SFMPageHelper getSfIdForLocalId:self.sfmPage.recordId objectName:self.sfmPage.objectName];
        ((ImagesVideosViewController*)viewController).parentSFObjectName = self.sfmPage.objectName;
        ((ImagesVideosViewController*)viewController).parentObjectName = [SFMPageHelper getObjectLabelForObjectName:self.sfmPage.objectName];
    }
    return viewController;
}
...
- (void)selectMasterTableViewCellWithIndexPath:(NSIndexPath*)indexPath
{
    ...
    if ([[self.masterTableView indexPathsForSelectedRows] count] > 0) {
        selectedIndexPath = [[self.masterTableView indexPathsForSelectedRows] objectAtIndex:0];
    }
    
    if (![selectedIndexPath isEqual:indexPath]) {
        [self.masterTableView selectRowAtIndexPath:indexPath animated:NO scrollPosition:0];
...
{
    return [self.sfmPage isAttachmentEnabled];
}
