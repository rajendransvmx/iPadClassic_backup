...
- (void) didLogin:(ZKLoginResult *)lr error:(NSError *)error context:(id)context
{
    ...
    SMLog(@"%@", appDelegate.loginResult);
    //Fix for defect #002869
    BOOL passwordExpired = [lr passwordExpired];
    if(passwordExpired)
    {
        
        NSDictionary * defaultTags = [appDelegate.wsInterface getDefaultTags];
        NSString * alert_ok = [defaultTags objectForKey:ALERT_ERROR_OK];
        NSString *title = @"Salesforce User Password Got Expired";
        NSString *message = @"Your Salesforce Password has expired.Please contact System Administrator";
 ...
 //   NSString * serviceMax = [defaultTags objectForKey:ALERT_ERROR_TITLE];
    ...
    if ([error userInfo] == nil)
        description = @"Error connecting to ServiceMax. Internet connection not available!"; 
    else
        description = [[error userInfo] objectForKey:@"faultstring"];
  
    if (lr == nil)
    {
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Connection Error" message:description delegate:nil cancelButtonTitle:alert_ok otherButtonTitles:nil];
        [alert show];
        [alert release];
        [activity stopAnimating];
    ...
    appDelegate.locationPingTimerValue = 1800; //30 min default Value
    appDelegate.isLocationPingEnabled = NO;
    ...
    {
        SMLog(@"LoginViewController didLogin in while loop");
        if (!appDelegate.isInternetConnectionAvailable)
        {
            [activity stopAnimating];
            [appDelegate displayNoInternetAvailable];
...
- (void) registerCoreLocationUpdates
{
...
- (void) initDebriefData:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int i = 0; i < [array count]; i++)
	{
        ...
        NSString *technicianId = [[[obj fields] objectForKey:@"Id"] retain];
        NSString * _query = [NSString stringWithFormat:@"Select Id,SVMXC__Active__c,SVMXC__Street__c, SVMXC__City__c, SVMXC__State__c, SVMXC__Zip__c, SVMXC__Country__c FROM SVMXC__Service_Group_Members__c WHERE Id = '%@'", technicianId];

        /*
        NSString * _query = [NSString stringWithFormat:@"Select SVMXC__Active__c,SVMXC__Street__c, SVMXC__City__c, SVMXC__State__c, SVMXC__Zip__c, SVMXC__Country__c FROM SVMXC__Service_Group_Members__c WHERE Id = '%@'", appDelegate.appTechnicianId];*/
...
- (void) didQueryTechnician:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        if(![[dict objectForKey:@"SVMXC__Active__c"] boolValue])
        {
            appDelegate.technicianAddress = @"";
            return;
        }
        didQueryActiveTechnician = YES;
        appDelegate.appTechnicianId = [dict objectForKey:@"Id"];
...
- (BOOL) checkVersion
{
    ...
    else
    {
        ...
        NSString * version_app  = [NSString stringWithFormat:@"%@ %@", version, [[NSBundle mainBundle] objectForInfoDictionaryKey:(NSString *)kCFBundleVersionKey]];
        NSString * message  = [NSString stringWithFormat:@"iPad %@ requires ServiceMax version %.5f or higher.",version_app ,APPVERSION];
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"Invalid ServiceMax Version" message:message  delegate:self cancelButtonTitle:ALERT_ERROR_OK_DEFAULT otherButtonTitles:nil, nil];
        [alertView show];
        [alertView release];
...
- (void) showHomeScreenviewController
{
    if (homeScreenView == nil)
    {
        ...
        [self presentModalViewController:homeScreenView animated:YES];
...
- (void)viewDidLoad
{
    ...
    NSString * _password = [SFHFKeychainUtils getPasswordForUsername:@"password" andServiceName:KEYCHAIN_SERVICE_NAME error:&error];
    SMLog(@"%@", _password);
...
- (void) didGetSettingsInfo:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
        if(settingInfoId) [settingInfoId release];
        if(settingsInfoArray) [settingsInfoArray release];
...
- (void) didGetSettingsValue:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
        ...
        if(settingsValueArray) [settingsValueArray release];
...
- (void) didGetLocationPingSettingsValue:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    if ([[result records] count] > 0)
    {
        ...
        for (int i = 0; i < [settingsInfoArrayForLocationPing count]; i++)
        {
            for (int j = 0; j < [settingsValueArrayForLocationPing count]; j++)
            {
                ...
                if ([settingsValueSettingId isEqualToString:settingsInfoSettingId])
                {
                    ...
                    {
                        if([[[settingsInfoArrayForLocationPing objectAtIndex:i] 
                            objectForKey:@"SVMXC__SettingID__c"] isEqualToString:@"SET001"])
                        {
                            NSString *frequency =[[settingsValueArrayForLocationPing objectAtIndex:j] objectForKey:@"SVMXC__Internal_Value__c"] ; 
                            long settingValue = [frequency intValue];
                            if(settingValue >0)
                            {
                                appDelegate.locationPingTimerValue = settingValue * 60; //converting to sec
                            }

                        }
                        if([[[settingsInfoArrayForLocationPing objectAtIndex:i] 
                             objectForKey:@"SVMXC__SettingID__c"] isEqualToString:@"SET002"])
                        {
                            NSString *LocationPingStatus = [[settingsValueArrayForLocationPing objectAtIndex:j] objectForKey:@"SVMXC__Internal_Value__c"];
                            appDelegate.isLocationPingEnabled = [LocationPingStatus boolValue];
                        }
        ...
        SMLog(@"Frequency = %d and isLocationPing Enabled = %d",appDelegate.locationPingTimerValue,appDelegate.isLocationPingEnabled);
    }
}
// #################################################################################################
@end
