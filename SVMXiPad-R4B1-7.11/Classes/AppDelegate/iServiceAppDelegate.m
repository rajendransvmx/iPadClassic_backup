...
@synthesize didUpdateTechnicianLocation;
@synthesize locationManager;
@synthesize locationPingTimerValue;
@synthesize isLocationPingEnabled;
@synthesize didQueryTechnician;
@synthesize didQueryActiveTechnician;
...
- (void) updateInterfaceWithReachability: (Reachability*)curReach
{
    ...
    switch (netStatus)
    {
        ...
        case ReachableViaWiFi:
            ...
            internetConnectionStatus = YES;
            [self PostInternetNotificationAvailable];
            SMLog(@"WiFi WWAN Reachable");
            break;
        default:
            isInternetConnectionAvailable = YES;
            internetConnectionStatus = YES;
            [self PostInternetNotificationAvailable];
            SMLog(@"Default Reachable");
    }
}
...
- (void) getUpdatedTechnicianId
{
    NSString * _query = [NSString stringWithFormat:@"SELECT Id, SVMXC__Service_Group__c, SVMXC__Inventory_Location__c  FROM SVMXC__Service_Group_Members__c WHERE SVMXC__Active__c = TRUE and SVMXC__Salesforce_User__c = '%@' limit 1", loggedInUserId];
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0, FALSE)) 
    {
        ...
        if (didUpdateTechnicianLocation)
            break;
        if([[NSDate date] timeIntervalSinceDate:curTime] > 2.5 )
            break;
    }
...
- (void) initDebriefDataLocationPing:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if([array count] == 0)
    {
        SMLog(@"No Active Technician Found");
        appTechnicianId = @"";
        appServiceTeamId = @"";
        return;
    }
...
-(void)didUpdateToLocation:(CLLocation*)location
{
    ...
    UIApplication *application = [UIApplication sharedApplication];;
     ...
     dispatch_async(dispatch_get_main_queue(), ^{
         ...
         {
             [self getUpdatedTechnicianId];
             if(!didUpdateTechnicianLocation)
                 break;
             if(appTechnicianId == nil || ([appTechnicianId length] == 0))
                 break;
             ...
             ZKSObject * obj = [[ZKSObject alloc] initWithType:@"SVMXC__Service_Group_Members__c"];
             [obj setFieldValue:latitude field:@"SVMXC__Latitude__c"];
             [obj setFieldValue:longitude field:@"SVMXC__Longitude__c"];
             [obj setFieldValue:appTechnicianId field:@"Id"];
             NSArray * array = [[NSArray alloc] initWithObjects:obj, nil];
             [[ZKServerSwitchboard switchboard] update:array target:self selector:@selector(didUpdateTechnician:error:context:) context:nil];
             NSDate *curTime = [NSDate date];
             while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0, FALSE)) 
             {
                 if (!self.isInternetConnectionAvailable)
...
- (void) didUpdateTechnician:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    didUpdateTechnicianLocation = YES;
    if(error != nil)
    {
        SMLog(@" Updating Technician Address Failed with Error %@",[[error userInfo] description]);
        return;        
    }
    SMLog(@" Updating Technician Address Successful");
}
@end
