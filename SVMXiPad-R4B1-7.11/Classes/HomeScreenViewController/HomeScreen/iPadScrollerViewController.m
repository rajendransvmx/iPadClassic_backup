...
- (void) showMap
{
    ...
    else
    {
        ...
        for (int i = 0; i < [appDelegate.workOrderEventArray count]; i++)
        {
            ...
            {
                SMLog(@"iPadScrollerViewController showMap in while loop");
...
- (void)viewDidLoad
{
    ...
    if(appDelegate.isLocationPingEnabled)
    {
           ...
           fromLocation:(CLLocation *)oldLocation
{
    ...
    if (userDefaults) {
        ...
        if (!([newLocationTimestamp timeIntervalSinceDate:lastLocationUpdateTiemstamp] < appDelegate.locationPingTimerValue)) {
