...
- (void)viewDidLoad
{
    ...
    if ( objName!=nil && [objName length] >0  )
    {
        ...
        {
            SMLog(@"DetailViewController viewdidLoad in while loop");
            ...
            SMLog(@"Receiving RecordTypeId Info ..");
        }
    }
}
...
- (void)splitViewController:(UISplitViewController *)svc willShowViewController:(UIViewController *)aViewController invalidatingBarButtonItem:(UIBarButtonItem *)barButtonItem
{
    ...
    //[tableView reloadData];//Fix for Keyborad Retracting
...
- (void) action:(id)sender
{
    ...
    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"EDIT"]) 
    {
        ...
        NSString * HideQickSave =[appDelegate.SFMPage objectForKey:gHideQuickSave];
        NSString * HideSave = [appDelegate.SFMPage objectForKey:gHideSave];
        ...
        if([HideQickSave isEqualToString:@""])
        {
            keys = [NSMutableArray arrayWithObjects:gBUTTON_TITLE,gBUTTON_EVENTS, nil];
...
- (void) didInvokeWebService:(NSString *)targetCall
{
    ...
    SMLog(@"Web Service Method - %@", targetCall);
...
- (UITableViewCell *) SFMEditCellForTable:(UITableView *)_tableView AtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (selectedSection == SHOW_HEADER_ROW || selectedSection == SHOWALL_HEADERS)
	{
        ...
        for (int j = 0; j < [field_columns count]; j++)
		{
            ...
            {
                SMLog(@"Dict Value = %@",dict);
                //SMLog(@" Value = %@",value);
                SMLog(@"key Value = %@",keyValue);
...
- (void)didDescribeSObjectLayout:(ZKDescribeLayoutResult *)result error:(NSError *)error context:(id)context
{
    ...
    for(ZKRecordTypeMapping *recordType in recordTypeMappings)
    {
        ...
        SMLog(@"Layout = %@",[recordType layoutId]);
        SMLog(@"Name = %@",[recordType name]);
        SMLog(@"RecordTypeId = %@",[recordType recordTypeId]);
        ...
        {
            SMLog(@"Picklist Name = %@",[pickList picklistName]);
            ...
            for(ZKPicklistEntry *pickListValue in pickListValueArray)
            {
                ...
                SMLog(@"label  = %@ : value  = %@ Default = %d",[pickListValue label],[pickListValue value],[pickListValue defaultValue]);   
    ...
    if(RecordTypePickList != nil)
        SMLog(@"Record Type Dict = %@",RecordTypePickList);
...
//        isCellNew = YES;
	...
	if (selectedSection == SHOW_HEADER_ROW || selectedSection == SHOWALL_HEADERS)
	{
        ...
        if(SLA_FLAG)
        {
           ...
           actual_resolution =  [[header_dict objectForKey:@"hdr_Data"] 
            ...
            SMLog(@"Actual Resolution = %@",actual_resolution);
            SMLog(@"Actual Restoration = %@",actual_restoration);
            SMLog(@"SLA Clock Paused = %d",isSLAClockPaused);
            if (row == 0)
            {
                for (int i = 0; i < 2; i++)
                {
            ...
            else
            {
                ...
                SMLog(@"Rest = %@ and Reso = %@",restorationTimerValue,resolutionTimerValue);
                if (!restorationTimer)
                ...
                if(actual_restoration!=nil)
                {
                    ...
                    SMLog(@"Actual Restoration = %@",actual_restoration);
                    SMLog(@"Restoration Customer = %@",restoration_customer);
        ...
        for (int j = 0; j < [field_columns count]; j++)
		{
            ...
            else
            {
                ...
                NSLog(@"Key = %@",[[field_columns objectAtIndex:j] objectForKey:gFIELD_VALUE_KEY]);
                NSLog(@"Value = %@",[[field_columns objectAtIndex:j] objectForKey:gFIELD_VALUE_VALUE]);
	...
	else if (selectedSection == SHOW_LINES_ROW || selectedSection == SHOWALL_LINES)
	{
		...
		else //display the column values for the row
		{
			...
			NSMutableArray * detail_values = [[detail objectForKey:gDETAILS_VALUES_ARRAY] objectAtIndex:row-1];
            NSString * record_id  =  [[detail objectForKey:gDETAIL_VALUES_RECORD_ID] objectAtIndex:row-1];
			...
			for (int j = 0; j < columns && j < [detail_values count] && j< [detail_fields count]; j++)
			{
                CGRect frame =  CGRectMake(j*field_width, 6, field_width-4,control_height-6);
                ...
                /*if([field_data_type isEqualToString:@"reference"])
                {
                    NSString * key = [[detail_values objectAtIndex:j] objectForKey:gVALUE_FIELD_VALUE_KEY];
                    NSString * related_to_table_name = [[detail_fields objectAtIndex:j] objectForKey:gFIELD_RELATED_OBJECT_NAME];
                    NSString * api_name = [[detail_fields objectAtIndex:j] objectForKey:gFIELD_API_NAME];
                    
                    CusLabel * custLabel = [[CusLabel alloc] initWithFrame:frame];
                    custLabel.backgroundColor = [UIColor clearColor];
                    
                    custLabel.text = value;
                    custLabel.textColor = [appDelegate colorForHex:@"2d5d83"];
                    custLabel.font = [UIFont boldSystemFontOfSize:custLabel.font.pointSize];
                    custLabel.userInteractionEnabled = TRUE;
                    custLabel.id_ = key;
                    custLabel.refered_to_table_name = related_to_table_name;
                ...
                //if the record is reference record then made it blue color
            ...
            UITapGestureRecognizer * cell_tap; cell_tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(celltapRecognizer:)];
            [cell_tap setNumberOfTapsRequired:2];
            NSInteger cellCount = 10000;
            int section_ = index +1;
            cell.tag = (cellCount * section_ )+ row; 
            
            [cell addGestureRecognizer:cell_tap];
    ...
    else if(selectedSection == SHOW_ADDITIONALINFO_ROW || selectedSection == SHOW_ALL_ADDITIONALINFO)
    {
        ...
        if (indexPath.row  == 0) //display the column titles
		{
            ...
            NSArray * array = [[[NSArray alloc] initWithObjects:@"Created Date",@"Problem Description", nil] autorelease];
            // If fields are not present - ERROR CONDITION - then columns should not be added
...
-(void)celltapRecognizer:(id)sender
{
    ...
    if ([tap.view isKindOfClass:[UITableViewCell class]])    
    {
        ...
        NSString * record_id    =  [array objectAtIndex:row-1];
        NSString * objectName_  =  [detail objectForKey:gDETAIL_OBJECT_NAME];;
        NSString * newProcessId = @"";
        ...
        if([newProcessId length] != 0 && newProcessId != nil && [record_id length] != 0 && record_id  != nil )
        {
            ...
            appDelegate.oldRecordId = currentRecordId;
            appDelegate.oldProcessId = currentProcessId;
            [self didSubmitProcess:newProcessId forRecord:record_id];
            
            return;
        }
        
        
        /*UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"No view Process is available for this object" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
        [alertView show];*/
...
//        oldBackgroundView = [[cell.contentView subviews] objectAtIndex:0];
    ...
    if ([control_type isEqualToString:@"reference"] && [fieldAPIName isEqualToString:@"RecordTypeId"])
    {
        ...
        if([refObjSearchId isEqualToString:@""])
        {
            ...
            while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0, FALSE)) 
            {
                SMLog(@"SFMEditDetailCellForTable in while loop");
                if (!appDelegate.isInternetConnectionAvailable)
                {
                    [activity stopAnimating];
                    appDelegate.wsInterface.sfm_response = FALSE;
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
  ...
  /*  NSMutableArray * details = [appDelegate.SFMPage objectForKey:gDETAILS];
    ...
    NSMutableArray * array  =  [detail objectForKey:gDETAIL_VALUES_RECORD_ID];
    NSString * record_id    =  [array objectAtIndex:_indexPath.row-1];
    NSString * objectName_  =  [detail objectForKey:gDETAIL_OBJECT_NAME];;
    NSString * newProcessId = @"";
...
-(void)swipeRecognizer:(UITapGestureRecognizer *)sender
{
    ...
    if([tap.view isKindOfClass:[CusLabel  class]])
    {
    ...
    if ([tap.view isKindOfClass:[UILabel  class]])    
    {
        ...
        {
            UIAlertView * alert_view = [[UIAlertView alloc] initWithTitle:@"" message:@"No View Process Avaible for the refernce " delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [alert_view  show];
            SMLog(@"swiped");
            ...
            for(int i=0; i <[header_sections count] ;i++)
            {
                ...
                for(int j= 0;j<[sectionFileds count]; j++)
                {
                    ...
                    if([filed_api_name isEqualToString:fieldApi_name])
                    {
                        ...
                        if([control_type isEqualToString:@"boolean"])
                        {
                            if([dict_value isEqualToString:@"True"] || [dict_value isEqualToString:@"true"])
                            {
                                return 1;
                            }
                            else
    ...
    else
    {
        ...
        for (int i = 0; i < [detail_values count]; i++)
        {
            ...
            if([fieldApi_name isEqualToString:value_Field_API])
            {
                if([controlType isEqualToString: @"picklist"])
                {
                    int pickListValue = 9999999;
                    NSMutableArray * details = [appDelegate.SFMPage objectForKey:gDETAILS];
                    NSMutableDictionary * detail = [details objectAtIndex:indexPath.section];
                    ZKDescribeSObject * descObj = nil;
                    for (int i = 0; i < [appDelegate.describeObjectsArray count]; i++)
                    {
                        if([[[appDelegate.describeObjectsArray objectAtIndex:i] name] isEqualToString:[detail objectForKey:@"detail_object_name"]])
                        {
                            descObj = [appDelegate.describeObjectsArray objectAtIndex:i];
                            break;
                        }
                    }
                    if(descObj == nil)
        ...
        for (int i = 0; i < [detail_values count]; i++)
        {
                ...
                [value_Field_API isEqualToString:@"RecordTypeId"])
            {
                for (int i = 0; i < [appDelegate.describeObjectsArray count]; i++)
                {
                    ...
                    SMLog(@"RecordTypeControllerr --- %@", controller_type);
                    if (descField == nil)
                        continue;
    ...
    else
    {
        ...
        if(pickLists != nil )
        {
            ...
            for(int j=0; j< [pickLists count]; j++)
            {
                ...
                SMLog(@"PickList Name = %@",pickListName);
                if([pickListName isEqualToString:api_name])
                {
                    ...
                    {
                        SMLog(@"PickList Value %d = %@",k,[[pickListArray objectAtIndex:k] objectForKey:@"value"]);
               ...
               // NSString * dict_value = [filed_info objectForKey:gFIELD_VALUE_VALUE];
                ...
                if([control_type isEqualToString:@"picklist"])
                {
                    for (int i = 0; i < [appDelegate.describeObjectsArray count]; i++)
                    {
                        ...
                        NSString * controller_type = [descField controllerName];
                        SMLog(@"Controllerr --- %@", controller_type);
                        if (descField == nil)
                            continue;
...
-(void)doubleTapOnCusLabel:(id)cusLabel
{
    if ([cusLabel isKindOfClass:[UILabel  class]])    
    {
        ...
        if([newProcessId length] != 0 && newProcessId != nil && [record_id length] != 0 && record_id  != nil )
        {
            ...
            appDelegate.oldProcessId = currentProcessId;
            if(isInEditDetail)
                [parentReference didSubmitProcess:newProcessId forRecord:record_id];
            else
                [self didSubmitProcess:newProcessId forRecord:record_id];
        }
        else
