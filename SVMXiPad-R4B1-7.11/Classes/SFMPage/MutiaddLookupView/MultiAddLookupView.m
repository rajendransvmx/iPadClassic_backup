...
- (void)viewDidLoad
{
    ...
    selectedObjDetails = [[NSMutableDictionary alloc] initWithCapacity:0];
    [super viewDidLoad];
...
- (void)viewDidUnload
{
    [self setSelectedObjDetails:nil];
    [searchBar release];
    searchBar = nil;
    [_tableView release];
    _tableView = nil;
...
{
    NSArray *indexKeys = [selectedObjDetails allKeys];
    for (id object in indexKeys )
    {
        [self selectObjectValue:[selectedObjDetails objectForKey:object]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for(id object in indexKeys)
    {
        if([object integerValue] == indexPath.row)
        {
            found = YES;
            break;
        }
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(found)
    {
        [selectedObjDetails removeObjectForKey:[NSString stringWithFormat:@"%d",indexPath.row]];
        cell.accessoryType = UITableViewCellAccessoryNone;
    }
    else
    {
         NSArray * _array = [[lookupData objectForKey:@"DATA"] objectAtIndex:indexPath.row];
        [selectedObjDetails setObject:_array forKey:[NSString stringWithFormat:@"%d",indexPath.row]];
        cell.accessoryType = UITableViewCellAccessoryCheckmark;
    }
