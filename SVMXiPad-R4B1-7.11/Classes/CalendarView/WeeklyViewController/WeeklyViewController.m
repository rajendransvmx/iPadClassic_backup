...
- (void) populateWeekView
{
    ...
    for ( int i = 0; i < [appDelegate.wsInterface.eventArray count]; i++ )
    {
       ...
       if(currentSliderPositionIndex+1 > [weeksArray count])
            currentSliderPositionIndex = 0;//Fix For Calendar Crash 
        ...
        for (int v = 0; v < [appDelegate.wsInterface.viewLayoutsArray count]; v++)
        {
            ...
            if ([object_label isEqualToString:objectAPIName])
            {
                ...
                SMLog(@"[populateWeekView] Event Process Id =%@",events.processId);
                break;
            }
        }
...
//        
//        //Taking the time and date of an event
//        NSDate * eventDateTime = nil; // [dict objectForKey:ACTIVITYDATE];
//        
////        eventDateTime = [dict objectForKey:ACTIVITYDTIME];
//          
//        NSString * dateString = nil; // [self dateToStringConversion:eventDateTime];
//          
////        eventDateTime = [dict objectForKey:STARTDATETIME];
//        
//        dateString = [self dateToStringConversion:eventDateTime];
            ...
            else
            {
                ...
                if(ContinueRescheduling == TRUE)
                {
                    ...
                    if ([updatestartDateTime length] > 0 && [updateendDateTime length] > 0)
                    {
                        ...
                        {
                            SMLog(@"[StartDateTime] WeeklyViewController touchesEnded in while loop");
                            if (!appDelegate.isInternetConnectionAvailable)
                            {
                                [activity stopAnimating];
                    ...
                    while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, 1, FALSE))
                    {
                        SMLog(@"[StartDate] WeeklyViewController touchesEnded in while loop");
                        if (!appDelegate.isInternetConnectionAvailable)
                        {
                            [activity stopAnimating];
                            appDelegate.wsInterface.didRescheduleEvent = FALSE;
