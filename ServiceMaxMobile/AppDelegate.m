...
- (void)bindUserTrunkLocationToClientInfo
{
    ...
    for (NSString *valueString in SVMXDEV_client.clientInfo)
    {
    ...
    if ([dataBase getTechnicianLocationId] == nil)
    {
        if (previousTrunkLocation != nil)
        {
            ...
            [SVMXDEV_client.clientInfo removeObject:previousTrunkLocation];
        }
    }
    else
    {
        ...
        if (previousTrunkLocation == nil)
        {
            [SVMXDEV_client.clientInfo addObject:trunkLocation];
        }
...
//create new object
- (INTF_WebServicesDefServiceSvc_SVMXDEVlient  *) getSVMXDEVlientObject
{
    // Vipindas - Adv.lookup search
    [self bindUserTrunkLocationToClientInfo];
    //ADD SVMXDEVlient
    return SVMXDEV_client;
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    SVMXDEV_client = [[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init];
    
    NSDictionary *clientInfoDict = [self getClientInfoDict];
    SVMXDEV_client.clientType = [clientInfoDict objectForKey:deviceType];//@"iPad";
    [SVMXDEV_client.clientInfo addObject:[clientInfoDict objectForKey:osVersion]];
    [SVMXDEV_client.clientInfo addObject:[clientInfoDict objectForKey:applicationVersion]];
    [SVMXDEV_client.clientInfo addObject:[clientInfoDict objectForKey:devVersion]];
	...
	if ( accessToken != nil )
	{
		...
		else
		{
            ...
            NSLog(@"Database   user version : %d",  [appDelegate.dataBase getCurrentDatabaseUserVersion]);
            
            [appDelegate.dataBase alterTableSchemaIfNeeded];
            
            NSLog(@"Database   user version after alteration : %d",  [appDelegate.dataBase getCurrentDatabaseUserVersion]);
...
- (void)installCoreLibrary
{
    ...
    //18 march 2014
    [SSZipArchive unzipFileAtPath:[resourcePath stringByAppendingPathComponent:@"com.servicemax.client.sal.sfmconsole.model.zip"] toDestination:documentsDir];
}
...
-(void)performInitialSynchronization
{
    /** Pushpak 10734 */
    /**
     * Check whether remote access is revoked,
     * Fix for multiple functional error at login page due to check version.
     */
    ...
    if ([appDelegate isInternetConnectionAvailable] )
    {
        BOOL isSessionValidCheck = [[ZKServerSwitchboard switchboard] doCheckSession];
        if ([appDelegate isRemoteAccessRevoked] || isSessionValidCheck == NO) {
...
-(processInfo *) getViewProcessForObject:(NSString *)object_name record_id:(NSString *)recordId processId:(NSString *)LastprocessId_  isswitchProcess:(BOOL)isSwitchProcess
{
    ...
    @try{
        for (NSDictionary * dict_ in self.view_layout_array)
        {
            ...
            NSString * viewLayoutObjectName = [dict_ objectForKey:SVMXDEV_OBJECT_NAME];
            if ([viewLayoutObjectName isEqualToString:object_name])
            {
                process_id_temp = [dict_ objectForKey:SVMXDEV_ProcessID];
        ...
        NSString * finalExpressionId = nil; //008303 - Defect Fix
        ...
        for(int i = 0 ; i< [process_id_array count]; i++)
        {
            ...
            finalExpressionId = expression_id;
        ...
        if(process_exists)
        {
            ...
            pinfo.expressionId = finalExpressionId; //008303 - Defect Fix
        }
        else
        {
...
-(void) getCreateProcessArray:(NSMutableArray *)processes_array
{
    ...
    @try{
        for(int i = 0 ;i<[processes_array count]; i++)
        {
            ...
            NSString * str = [dictionary objectForKey:SVMXDEV_OBJECT_NAME];
        ...
        for(int i=0 ;i< [_objectNames_array count]; i++)
        {
            ...
            for(int j=0;j<[processes_array count];j++)
            {
                ...
                NSString * str = [_dict objectForKey:SVMXDEV_OBJECT_NAME];
        ...
        for(int i=0 ;i< [_objectNames_array count]; i++)
        {
            ...
            for (int k = 0; k <[createobjects count]; k++)
            {
                ...
                NSString * key1 = [dict1 objectForKey:@"SVMXDEV__Name__c"];
                for (int r = k + 1; r < [createobjects count]; r++)
                {
                    NSDictionary * dict2 = [createobjects objectAtIndex:r];
                    NSString * key2 = [dict2 objectForKey:@"SVMXDEV__Name__c"];
...
- (void) setSyncStatus2
{
    ...
    if (_SyncStatus == SYNC_RED)
    {
        ...
        [SyncProgress setAccessibilityIdentifier:kAccSyncIndicatorRed];
    ...
    else if (_SyncStatus == SYNC_GREEN)
    {
        ...
        [img stretchableImageWithLeftCapWidth:10 topCapHeight:10];
        animatedImageView.isAccessibilityElement = NO;
        [SyncProgress setAccessibilityIdentifier:kAccSyncIndicatorGreen];
	...
	else if (_SyncStatus == SYNC_ORANGE)
	{
        ...
        animatedImageView.isAccessibilityElement = NO;
        [SyncProgress setAccessibilityIdentifier:kAccSyncIndicatorOrange];
...
- (void)findOrCreateAppSupportDirectory
{
    ...
    rootPath = [rootPath stringByAppendingPathComponent:@"SVMXDEV"]; // Custom sub directory for our app use
...
- (void) updateNextSyncTimeIfSyncFails:(NSDate *)syncStarted syncCompleted:(NSDate *)syncCompleted
{
    /** 
     * Pushpak 10734
     * Restarting data sync if its failed due to invalid session id error
     */
    if (appDelegate.didDataSyncFailWithInvalidSession)
    {
        appDelegate.didAutoRestartedDataSyncFailedWithInvalidSession = YES;
        appDelegate.didDataSyncFailWithInvalidSession = NO;
        [appDelegate.wsInterface DoIncrementalDataSync];
        return;
    }
    /**
     * resetting flag because data sync didn't fail because of invalid session 
     */
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex  // after animation
{
	...
	else
	{
        ...
        if (buttonIndex == 0)
        {
            ...
            {
                [self postNotificationToRemoveDataPurgeProgress];
            }
        }
...
- (void) postNotificationToRemoveDataPurgeProgress
{
...
/**
 * Pushpak
 * Modification as required for defect 10734
 */
- (BOOL)isRemoteAccessRevoked
{
    BOOL hasRemoteAccessRevoked = NO;
    NSUserDefaults *defaults= [NSUserDefaults standardUserDefaults];
    if([[[defaults dictionaryRepresentation] allKeys] containsObject:@"RemoteAccessRevoked"]){
        /**
         * Check whether remote access is revoked,
         * if yes then return
         */
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"RemoteAccessRevoked"]) {
            hasRemoteAccessRevoked = YES;
            dispatch_async(dispatch_get_main_queue(), ^{
            
                /** Setting sync idicator red to indicate error. */
                [self  setSyncStatus:SYNC_RED];
                
            });
        }
    }
    return hasRemoteAccessRevoked;
}

// Vipindas field merge feature
/**
 * @name  isFieldMergeEnabled
 *
 ...
 *
 * \par
 *  Module: ServiceMax iPad Client
 Submodule: IPAD018
 Setting ID: SET016
 Setting Name: Advanced conflict resolution during data synchronization.
 Setting Description: When set to true, during data synchronization between client and server only modified field values are pushed to server. Synchronization conflicts are raised only when the same field is modified by both client and server. Otherwise field merge will be performed
 *  Setting Type : Group
 *  Default Value: false
 *  Datatype: Boolean
 *
...
- (BOOL)isFieldMergeEnabled
{
    BOOL fieldMergeEnabled = NO;
    
    if ((appDelegate.settingsDict != nil) && ([appDelegate.settingsDict objectForKey:@"IPAD018_SET016"] != nil))
    {
        fieldMergeEnabled = [[appDelegate.settingsDict objectForKey:@"IPAD018_SET016"] boolValue];
    }
    
     NSLog(@" Field Level Merge Enabled :%@", [appDelegate.settingsDict objectForKey:@"IPAD018_SET016"]);
    
   return fieldMergeEnabled;
   // return YES;
}


- (BOOL)shouldSaveChanges:(BOOL)foundChanges
{
...
@synthesize process_exists, process_id, expressionId; //008303 - Defect Fix
