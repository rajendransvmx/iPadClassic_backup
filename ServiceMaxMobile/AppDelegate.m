...
- (void)serializeDatabaseConnection
{
    ...
    {
        SMLog(@"[OPMX] - 1 Use sqlite on multiple threads, using the same connection");
    }
    else
    {
        SMLog(@"[OPMX] - 1 Single connection single thread - configResult : %d", configResult);
    }
}
...
- (NSDictionary *)getClientInfoDict {
    ...
    SMLog(@"model : %@ systemVersion : %@ appversion %@",type,systemOSVersion,appVersion);
...
-(void)initWithDBName:(NSString *)name type:(NSString *)type
{
    ...
    else
    {
        ...
        SMLog(@" AppDelegate : initWithDBName : Reset DB Config");
...
-(processInfo *) getViewProcessForObject:(NSString *)object_name record_id:(NSString *)recordId processId:(NSString *)LastprocessId_  isswitchProcess:(BOOL)isSwitchProcess
{
    ...
    @try{
        ...
        SMLog(@"Exception Name AppDelegate :insertvaluesToPicklist %@",exp.name);
        SMLog(@"Exception Reason AppDelegate :insertvaluesToPicklist %@",exp.reason);
		...
		else
		{
			...
			if (isSessionInavalid == YES)
			{
				...
				while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, FALSE))
				{
					...
					SMLog(@"AppDelegate.m : doOnlineIfRequired: ZKS Check For Session");
#endif
...
- (BOOL) pingServer
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        SMLog(@"AppDelegate.m : pingServer: ZKS Check For Session");
#endif
...
- (BOOL) showloginScreen
{
    ...
    if([appDelegate.syncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : showLoginScreen: Check For Data Sync Thread Status");
#endif
    ...
    if ([appDelegate.metaSyncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : showLoginScreen: Check For Meta Sync Thread Status");
#endif
    ...
    if ([appDelegate.event_thread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : showLoginScreen: Check For Event Sync Thread Status");
#endif
    ...
    if (metaSyncRunning)
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : showLoginScreen: Check For Meta Sync Thread Status2");
#endif
	...
	if (appDelegate.eventSyncRunning)
	{
		...
		{
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : showLoginScreen: Check For Event Sync Thread Status2");
#endif
...
#ifdef kPrintLogsDuringWebServiceCall
        SMLog(@"AppDelegate.m : getDPpicklistInfo: Dependent Picklist");
#endif
...
-(void) getCreateProcessArray:(NSMutableArray *)processes_array
{
    ...
    @try{
        ...
        SMLog(@"Exception Name AppDelegate :getCreateProcessArray %@",exp.name);
        SMLog(@"Exception Reason AppDelegate :getCreateProcessArray %@",exp.reason);
...
- (void) updateInstalledPackageVersion
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        SMLog(@"AppDelegate.m : updateInstalledPackageVersion: Check for installed pkg version ");
#endif
...
-(void)invalidateAllTimers
{
    if([appDelegate.syncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : invalidateAllTimers: Data Sync thread status");
#endif
    ...
    if ([appDelegate.metaSyncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : invalidateAllTimers: Meta Sync thread status");
#endif
    ...
    if ([appDelegate.event_thread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : invalidateAllTimers: Event Sync thread status");
#endif
    ...
    if (metaSyncRunning)
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : invalidateAllTimers: Meta Sync thread status 2");
#endif
    ...
    if (appDelegate.eventSyncRunning)
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(@"AppDelegate.m : invalidateAllTimers: Event Sync thread status2");
#endif
...
- (BOOL)addSkipBackupAttributeToItemAtURL:(NSURL *)URL
{
    ...
    if(!success){
        SMLog(@"Error excluding %@ from backup %@", [URL lastPathComponent], error);
    }
    return success;
}
...
- (void) printIfError:(NSString *)err ForQuery:(NSString *)query type:(SQL_QUERY)type
{
    ...
    {
        SMLog(@"Exception Name AppDelegate :printIfError %@",exp.name);
        SMLog(@"Exception Reason AppDelegate :printIfError %@",exp.reason);
        [self CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
-(void)CustomizeAletView:(NSError*)error alertType:(ALERT_VIEW_ERROR)type Dict:(NSMutableDictionary*)errorDict exception:(NSException *)exp
{
    ...
    {
        SMLog(@"Exception Name AppDelegate :CustomizeAletView %@",exp.name);
        SMLog(@"Exception Reason AppDelegate :CustomizeAletView %@",exp.reason);
    }
}
...
-(void) sendingEmail:(UIAlertView*)alertview
{
    ...
    {
        SMLog(@"Exception Name AppDelegate :sendingEmail %@",exp.name);
        SMLog(@"Exception Reason AppDelegate :sendingEmail %@",exp.reason);
    }
}
...
-(void)copyToClipboard:(UIAlertView*)alertview
{
    ...
    {
        SMLog(@"Exception Name AppDelegate :copyToClipboard %@",exp.name);
        SMLog(@"Exception Reason AppDelegate :copyToClipboard %@",exp.reason);
    }
}
