...
- (void)bindUserTrunkLocationToClientInfo
{
    ...
    for (NSString *valueString in SPR14_client.clientInfo)
    {
    ...
    if ([dataBase getTechnicianLocationId] == nil)
    {
        if (previousTrunkLocation != nil)
        {
            ...
            [SPR14_client.clientInfo removeObject:previousTrunkLocation];
        }
    }
    else
    {
        ...
        if (previousTrunkLocation == nil)
        {
            [SPR14_client.clientInfo addObject:trunkLocation];
        }
...
//create new object
- (INTF_WebServicesDefServiceSvc_SPR14lient  *) getSPR14lientObject
{
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    SPR14_client = [[INTF_WebServicesDefServiceSvc_SPR14lient alloc] init];
    
    NSDictionary *clientInfoDict = [self getClientInfoDict];
    SPR14_client.clientType = [clientInfoDict objectForKey:deviceType];//@"iPad";
    [SPR14_client.clientInfo addObject:[clientInfoDict objectForKey:osVersion]];
    [SPR14_client.clientInfo addObject:[clientInfoDict objectForKey:applicationVersion]];
    [SPR14_client.clientInfo addObject:[clientInfoDict objectForKey:devVersion]];
...
-(processInfo *) getViewProcessForObject:(NSString *)object_name record_id:(NSString *)recordId processId:(NSString *)LastprocessId_  isswitchProcess:(BOOL)isSwitchProcess
{
    ...
    @try{
        for (NSDictionary * dict_ in self.view_layout_array)
        {
            ...
            NSString * viewLayoutObjectName = [dict_ objectForKey:SPR14_OBJECT_NAME];
            if ([viewLayoutObjectName isEqualToString:object_name])
            {
                process_id_temp = [dict_ objectForKey:SPR14_ProcessID];
...
-(void) getCreateProcessArray:(NSMutableArray *)processes_array
{
    ...
    @try{
        for(int i = 0 ;i<[processes_array count]; i++)
        {
            ...
            NSString * str = [dictionary objectForKey:SPR14_OBJECT_NAME];
        ...
        for(int i=0 ;i< [_objectNames_array count]; i++)
        {
            ...
            for(int j=0;j<[processes_array count];j++)
            {
                ...
                NSString * str = [_dict objectForKey:SPR14_OBJECT_NAME];
        ...
        for(int i=0 ;i< [_objectNames_array count]; i++)
        {
            ...
            for (int k = 0; k <[createobjects count]; k++)
            {
                ...
                NSString * key1 = [dict1 objectForKey:@"SPR14__Name__c"];
                for (int r = k + 1; r < [createobjects count]; r++)
                {
                    NSDictionary * dict2 = [createobjects objectAtIndex:r];
                    NSString * key2 = [dict2 objectForKey:@"SPR14__Name__c"];
...
- (void)findOrCreateAppSupportDirectory
{
    ...
    rootPath = [rootPath stringByAppendingPathComponent:@"SPR14"]; // Custom sub directory for our app use
