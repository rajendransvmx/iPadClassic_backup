...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
	...
	if ( accessToken != nil )
	{
		...
		//Fix for defect #7078
		NSLog(@"%@", [userDefaults valueForKey:@"UserFullName"]);
...
-(BOOL)checkVersion
{
    ...
    while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
    {
        ...
        NSLog (@"iserviceAppdelegate checkVersion in while loop");
...
- (BOOL) isInternetConnectionAvailable
{
    ...
    NSLog(@"[%f] %@",[[NSDate date] timeIntervalSinceDate:date],internetStatus);
    return status;
}
...
-(processInfo *) getViewProcessForObject:(NSString *)object_name record_id:(NSString *)recordId processId:(NSString *)LastprocessId_  isswitchProcess:(BOOL)isSwitchProcess
{
    ...
    @try{
        ...
        NSLog(@"Exception Name AppDelegate :insertvaluesToPicklist %@",exp.name);
        NSLog(@"Exception Reason AppDelegate :insertvaluesToPicklist %@",exp.reason);
		...
		else
		{
			...
			if (isSessionInavalid == YES)
			{
				...
				while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, FALSE))
				{
					...
					NSLog(@"AppDelegate.m : doOnlineIfRequired: ZKS Check For Session");
#endif
...
- (BOOL) pingServer
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"AppDelegate.m : pingServer: ZKS Check For Session");
#endif
...
- (BOOL) showloginScreen
{
    ...
    if([appDelegate.syncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : showLoginScreen: Check For Data Sync Thread Status");
#endif
    ...
    if ([appDelegate.metaSyncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : showLoginScreen: Check For Meta Sync Thread Status");
#endif
    ...
    if ([appDelegate.event_thread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : showLoginScreen: Check For Event Sync Thread Status");
#endif
    ...
    if (metaSyncRunning)
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : showLoginScreen: Check For Meta Sync Thread Status2");
#endif
	...
	if (appDelegate.eventSyncRunning)
	{
		...
		{
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : showLoginScreen: Check For Event Sync Thread Status2");
#endif
...
-(void)callDataSync
{
    ...
    NSLog(@"Return No Datasync triggred1");//7344
    ...
    if(retVal == TRUE)
    {
		...
		NSLog(@"Return No Datasync triggred2");//7344
        return;
    }
    ...
    if (![appDelegate isInternetConnectionAvailable])
    {
		...
		NSLog(@"Return No Datasync triggred3");//7344
        return;
    }
    ...
    if (syncThread != nil)
    {
        ...
        else
        {
			...
			NSLog(@"Return No Datasync triggred4");//7344
...
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"AppDelegate.m : getDPpicklistInfo: Dependent Picklist");
#endif
...
-(void) getCreateProcessArray:(NSMutableArray *)processes_array
{
    ...
    @try{
        ...
        NSLog(@"Exception Name AppDelegate :getCreateProcessArray %@",exp.name);
        NSLog(@"Exception Reason AppDelegate :getCreateProcessArray %@",exp.reason);
...
- (NSString *) serverPackageVersion
{
    ...
    if (userDefaults)
    {
        ...
        NSLog(@"Pkg Version = %@",packageVersion);
    }
...
- (void) updateInstalledPackageVersion
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"AppDelegate.m : updateInstalledPackageVersion: Check for installed pkg version ");
#endif
...
-(void)invalidateAllTimers
{
    if([appDelegate.syncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : invalidateAllTimers: Data Sync thread status");
#endif
    ...
    if ([appDelegate.metaSyncThread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : invalidateAllTimers: Meta Sync thread status");
#endif
    ...
    if ([appDelegate.event_thread isExecuting])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : invalidateAllTimers: Event Sync thread status");
#endif
    ...
    if (metaSyncRunning)
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : invalidateAllTimers: Meta Sync thread status 2");
#endif
    ...
    if (appDelegate.eventSyncRunning)
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"AppDelegate.m : invalidateAllTimers: Event Sync thread status2");
#endif
...
-(void) sendingEmail:(UIAlertView*)alertview
{
    ...
    {
        NSLog(@"Exception Name AppDelegate :sendingEmail %@",exp.name);
        NSLog(@"Exception Reason AppDelegate :sendingEmail %@",exp.reason);
    }
}
...
-(void)copyToClipboard:(UIAlertView*)alertview
{
    ...
    {
        NSLog(@"Exception Name AppDelegate :copyToClipboard %@",exp.name);
        NSLog(@"Exception Reason AppDelegate :copyToClipboard %@",exp.reason);
    }
}
...
-(void)setAgrressiveSync_flag
{
    ...
    if (syncThread != nil)
    {
        if ([syncThread isFinished] == YES)
        {
            NSLog(@"setAgrressiveSync_flag:thread finished its work");
        ...
        else
        {
            ...
            Enable_aggresssiveSync = FALSE;
            NSLog(@"setAgrressiveSync_flag:thread is not finished its work");
        }
