...
#import "Globals.h" // Vipindas field merge feature
...
#define kCheckSessionTimeStamp @"checkSessionTimeStamp"
...
@synthesize get_update_last_sync_time; // Damodar : Defect 8593
@synthesize delegate;
...
- (void) dealloc
{
    ...
    jsonParserForDataSync = nil;
    


    if(get_update_last_sync_time)
    {
        [get_update_last_sync_time release];
        get_update_last_sync_time = nil;
    }
...
-(void)optimizedAggressiveSync:(NSMutableDictionary *)sync_record  method_name:(NSString *)sfmMethodName class_name:(NSString *)webServiceClass event_type:(NSString *)Event_type event_name:(NSString *)event_name request_id:(NSString *)request_id
{
    ...
    NSMutableArray * customSync_records = [[NSMutableArray alloc] initWithCapacity:0];
    ...
    for(NSString * object_type in object_types)
    {
        ...
        for(NSString * object_name in allobjects)
        {
            ...
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEV_valueMAp = [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEV_valueMAp.key = object_type;
            SVMXDEV_valueMAp.value = object_name;
            ...
            for(int i = 0 ; i< [fields_array count]; i++)
            {
                ...
                {
                    [SVMXDEV_valueMAp.values addObject:field];
                }
            }
            ...
            for(NSString  * single_operation in all_operations)
            {
                ...
                for(NSDictionary * info_dict in record_info_dict_array)
                {
                    ...
                    if(![operation_type isEqualToString:DELETE])
                    {
                        ...
                        BOOL replace_success = [self replaceReferenceLocalIdWithSfidForRecord:each_record isChild:ischild referenceTo:referenceTo parentColoumnName:parent_column_name ObjectName:object_name recordLocalId:local_id recordSfId:sf_id];
                        if(!replace_success)
                        {
                            send_request = FALSE;
                            [customSync_records addObject:local_id];
                            //sahana
                            //dont break from here,there might be unsyched records upon which normal sync records are dependent
                            //if outer loop runs completely then we have coverded all records which are part of sync.
                        }
                    }
                    else
                    {
            ...
            if(additional_valuemap == nil && ![object_type isEqualToString:MASTER] && update_operation == nil && Delete_operation == nil  && Insert_operation == nil)
            {
                ...
                [SVMXDEV_valueMAp addValueMap:additional_valuemap];
                [additional_valuemap release]; additional_valuemap = nil;
            ...
            if(update_operation != nil)
            {
                ...
                [SVMXDEV_valueMAp addValueMap:update_operation];
                [update_operation release]; update_operation = nil;
            }
            if(Delete_operation!= nil)
            {
                [SVMXDEV_valueMAp addValueMap:Delete_operation];
                [Delete_operation release]; Delete_operation = nil;
            }
            else
            {
				//Radha #6951
...
//                }
            ...
            if(Insert_operation != nil)
            {
                ...
                [SVMXDEV_valueMAp addValueMap:Insert_operation];
                [Insert_operation release]; Insert_operation = nil;
            }
            
            [sfmRequest addValueMap:SVMXDEV_valueMAp];
            [referenceTo release]; referenceTo = nil;
        }
    }
     ...
     if(![object_types containsObject:DETAIL])
     {
        ...
        for(NSString * child_object in FinalChild)
        {
            ...
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEV_valueMAp = [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEV_valueMAp.key = DETAIL;
            SVMXDEV_valueMAp.value = child_object;
    ...
    //INTF_WebServicesDefServiceSvc_SVMXDEVlient  * SVMXDEV_client = [[[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init]  autorelease];
    //SVMXDEV_client.clientType = @"iPad";
    //[SVMXDEV_client.clientInfo addObject:@"OS:iPadOS"];
    //[SVMXDEV_client.clientInfo addObject:@"R4B2"];
    //[client_listMap.valueList addObject:SVMXDEV_client];
    ...
    if(!send_request)
    {
        [customSync_unsynched_records addObjectsFromArray:customSync_records];
        [custom_unsync_reqIds addObject:request_id];
        [customSync_records release]; customSync_records = nil;
...
-(void)clearCustomSyncStatus;
{
    [self.customSync_unsynched_records removeAllObjects];
    [self.custom_unsync_reqIds removeAllObjects];
}
-(void)customAggressiveSync
{
	...
	if(self.customSync_unsynched_records == nil)
    {
        NSMutableArray * tempArray = [[NSMutableArray alloc] initWithCapacity:0];
        self.customSync_unsynched_records = tempArray;
        [tempArray release]; tempArray = nil;
    }
    
    if(self.custom_unsync_reqIds == nil)
    {
        NSMutableArray * tempArry = [[NSMutableArray alloc] initWithCapacity:0];
        self.custom_unsync_reqIds = tempArry;
        [tempArry release]; tempArry = nil;
    }
...
//			[appDelegate setCurrentSyncStatusProgress:CUSTOMSYNC_STARTS optimizedSynstate:0];
            ...
            [monitor monitorSMMessageWithName:@"[WSInterface customAggressiveSync]"
            ...
            if(![self.custom_unsync_reqIds containsObject:request_id])
            {
                //Radha - Signaturecapture starts
...
//		[appDelegate setCurrentSyncStatusProgress:CUSTOMSYNC_END optimizedSynstate:0];
    ...
    NSMutableArray * final_custom_rec_set = [[NSMutableArray alloc] initWithCapacity:0];
    NSArray * existing_headerLocal_ids = [appDelegate.databaseInterface getallmasterRecordsForCustomAggressiveSync];
    for(NSString * requestdId in existing_headerLocal_ids)
    {
        if(![self.custom_unsync_reqIds containsObject:requestdId])
        {
            [final_custom_rec_set addObject:requestdId];
        }
        
    }
    if([final_custom_rec_set count] > 0)//if([existing_headerLocal_ids count] > 0)
    {
        [self customAggressiveSync];
    }
    
    [final_custom_rec_set release]; final_custom_rec_set = nil;
...
//			[appDelegate setCurrentSyncStatusProgress:CUSTOMSYNC_REQDATA optimizedSynstate:0];
            SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
            [monitor monitorSMMessageWithName:@"[WSInterface specialOverrideSync]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Start"
                                 timeInterval:10.0];
...
-(NSString *)get_SYNCHISTORYTime_ForKey:(NSString *)forkey
{
    ...
    [dict release]; dict= nil; //AvoidMemoryLeak-Sahana
    return value;
}
...
-(NSString *)getSyncTimeStampWithTheIntervalof15days:(NSString *)time
{
    ...
    @try{
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getSyncTimeStampWithTheIntervalof15days %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getSyncTimeStampWithTheIntervalof15days %@",exp.reason);
    }
    if([time isEqualToString:START_TIME])
    {
...
@try{
    ...
    [dict writeToFile:plistPath_SYNHIST atomically:YES];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :setSyncReqId %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :setSyncReqId %@",exp.reason);
    }
}
...
-(BOOL)getSyncStatusForRequestId
{
    ...
    {   
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getSyncStatusForRequestId %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getSyncStatusForRequestId %@",exp.reason);
    }
...
@try{
    ...
    if([sync_type isEqualToString:INSERT] && [operation_type isEqualToString:REQUEST])
    {
        for(NSString *  str in allkeys)
        {
            SMLog(kLogLevelVerbose,@"%@" ,current_gmt_time);
    ...
    else if([sync_type isEqualToString:GET_UPDATE_DOWNLOAD_CRITERIA] && [operation_type isEqualToString:RESPONSE])
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.reason);
    }  
    [dict writeToFile:plistPath_SYNHIST atomically:YES];
    [dict release]; dict =  nil;                   //sahana30April
...
-(void)DoSpecialIncrementalSync
{
    ...
    if(Insert_requestId == nil || [Insert_requestId isEqualToString:@""] )                                     
        Insert_requestId = [AppDelegate GetUUID];     
    ...
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface DoSpecialIncrementalSync:PUT_INSERT]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
    ...
    while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
    {
        ...
        SMLog(kLogLevelVerbose,@"WSinterface.m : DoSplIncSync: PUT_INSERT");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTINSERT optimizedSynstate:0];
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface DoSpecialIncrementalSync:PUT_DELETE]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        SMLog(kLogLevelVerbose,@"WSinterface.m : DoSplIncSync: PUT_DELETE");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTDELETE optimizedSynstate:0];
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface DoSpecialIncrementalSync:PUT_UPDATE]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        SMLog(kLogLevelVerbose,@"WSinterface.m : DoSplIncSync: PUT_UPDATE");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTUPDATE optimizedSynstate:0];
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface DoSpecialIncrementalSync:TX_FETCH]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:60.0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        SMLog(kLogLevelVerbose,@"WSinterface.m : DoSplIncSync: TX_FETCH");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTRECORDS optimizedSynstate:0];
    ...
    {
        [monitor monitorSMMessageWithName:@"[WSInterface DoSpecialIncrementalSync:CLEAN_UP]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Start"
                                 timeInterval:10.0];
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(kLogLevelVerbose,@"WSinterface.m : DoSplIncSync: CLEAN_UP");
#endif
...
//		[appDelegate setCurrentSyncStatusProgress:cSYNC_END optimizedSynstate:0];
    ...
    [appDelegate.attachmentDataBase updateParentSfidInAttachmentTrailer];
    [[AttachmentQueue sharedInstance] startQueue];
}

- (NSString *)lastModifiedFieldsFromTrailorTableById:(NSString *)sId
{
    // Vipindas field merge feature
    return [appDelegate.databaseInterface fieldsModifiedDataFromTrailorTempTableById:sId];
}
...
-(void)getAllRecordsForOperationTypeFromSYNCCONFLICT:(NSString *)operationType OverRideFlag:(NSString *)overrideFlag; 
{
    ...
    // Vipindas field merge feature
    NSArray * keys_all =  [NSArray arrayWithObjects:@"local_id" ,@"object_name" , @"record_type" ,@"sf_id", @"override_flag", @"fields_modified",nil];
    ...
    @try{
        ...
        for(int i = 0 ; i < [object_array count] ; i++)
        {
            ...
            // Lets fetch fields modification data from trailor table, this help us to create correct modification on Field merging.
            NSLog(@"WSI : SYNCCONFLICT dict  : %@ ", dict);
            NSLog(@"WSI : SYNCCONFLICT ids  : %@ - %@", local_id, sf_id);
            fields_modified = [self lastModifiedFieldsFromTrailorTableById:sf_id];
            
            NSLog(@"WSI : SYNCCONFLICT fields_modified  : %@", fields_modified);
            ...
            if(flag)
            {
                ...
                NSDictionary * dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:local_id,object_name,record_type,sf_id,override_flag, fields_modified, nil] forKeys:keys_all];
            ...
            NSLog(@"WSI : SYNCCONFLICT appDelegate.dataSync_dict  : %@", appDelegate.dataSync_dict);
        }
    }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.reason);
...
- (NSString *) getClientVersionString
{
    ...
    else
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getClientVersionString %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getClientVersionString %@",exp.reason);
    }
    return finalString2;
}
...
-(void) PutAllTheRecordsForIds
{
    ...
    for(int i = 0 ; i < [allKeys count] ; i++)
    {
        ...
        if([object_api_name length] != 0 && ![object_api_name isEqualToString:@""])
        {
            ...
            [SVMXDEVmap release]; SVMXDEVmap = nil;
    ...
    SMLog(kLogLevelVerbose,@"  TX_FETCH Request Sent: %@", [NSDate date]);
    //[[ZKServerSwitchboard switchboard] doCheckSession];
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :PutAllTheRecordsForIds %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :PutAllTheRecordsForIds %@",exp.reason);
    }
    
    SMLog(kLogLevelVerbose,@" ------- PutAllTheRecordsForIds  Finished -----");
...
-(void)deleteALlCustomWsEntriesFromSyncHeap
{
    ...
    [custom_ws_records release]; custom_ws_records = nil;
}
...
-(void)DoIncrementalDataSync
{
    ...
    [[AttachmentQueue sharedInstance] stopQueue];
    SMLog(kLogLevelVerbose,@"[isync] DoIncrementalDataSync  - started");

    [[PerformanceAnalytics sharedInstance] stopPerformAnalysis];
    
    //VP-Optmz2
    //[appDelegate.dataBase cleanupDatabase]; //8639
    [[PerformanceAnalytics sharedInstance] startAnalyticsWithCode:@"PA-IC-221"
    ...
    SMLog(kLogLevelVerbose,@"%@, %d",appDelegate.currentServerUrl, [appDelegate.currentServerUrl length] );
    ...
    if(retVal == NO || [appDelegate.currentServerUrl Contains:@"null"] || ![appDelegate isInternetConnectionAvailable])//8639
    {
        ...
        /** Pushpak 10734 
         * Sync icon is set to green even if session in invalid
         * Fix : if session invalid then set the sync icon red.
         */
        if (retVal == NO)
        {
            [appDelegate setSyncStatus:SYNC_RED];
    ...
    //sahana Jul 8 for  S2T modification
    [self clearCustomSyncStatus];
    [self customAggressiveSync];
 ...
 /* Shravya - Advanced look up filters - User trunk location */
    SMLog(kLogLevelVerbose,@"User location update Incremental starts");
	...
	if (doOneCallSync)
	{
        ...
        BOOL advFlag = [self advCriteria:syncStarted];
	...
	else
	{
        ...
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:GetDelete]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: GET_DELETE");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:GETDELETE_DONE optimizedSynstate:oGETDELETE_DONE];
        ...
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:GET_DELETE_DOWNLOAD_CRITERIA]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: GET_DELETE_DWN_CRIT");
			#endif
        ...
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:CLEAN_UP_SELECT]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: CLEAN_UP_SELECT 1");
			#endif
	...
	//		[appDelegate setCurrentSyncStatusProgress:GETEDELETE_DC_DONE optimizedSynstate:0];
		...
		{
            [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:PUT_DELETE]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Start"
                                 timeInterval:10.0];
			...
			{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: PUT_DELETE");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:PUTDELETE_DONE optimizedSynstate:oPUTDELETE_DONE];
		...
		{
			SMLog(kLogLevelError,@"Adv Download Criteria Failed");
			if ([appDelegate isInternetConnectionAvailable])
			{
				//Defect 6774
				[appDelegate checkifConflictExistsForConnectionError];
        ...
        //Shravya-8282
        //Need to check if there is  conflict on any PUT_DELETE records
        //If conflict exist, get thier JSON and reinsert into resepctive table
        //release the dictionary
        [appDelegate.databaseInterface  reInsertDeletedRecordsOnConflict];
		...
		if([appDelegate.dataSync_dict count] >0)
		{
            [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:PUT_INSERT]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Start"
                                 timeInterval:10.0];
			...
			{
				#ifdef kPrintLogsDuringWebServiceCall
					SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: PUT_INSERT");
				#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:PUTINSERT_DONE optimizedSynstate:oPUTINSERT_DONE];
		...
		[self resetSyncLastindexAndObjectName];  //sahana
			...
			[[PerformanceAnalytics sharedInstance] completedPerformanceObservationForContext:@"GetDelete-VP"
																 andRecordCount:0];
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:GET_INSERT]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: GET_INSERT");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:GETINSERT_DONE optimizedSynstate:oGETINSERT_DONE];
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:GET_INSERT_DOWNLOAD_CRITERIA]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: GET_INSERT_DWN_CRIT");
			#endif
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: CLEAN_UP_SELECT 2");
			#endif
	...
	//		[appDelegate setCurrentSyncStatusProgress:GETINSERT_DC_DONE optimizedSynstate:0];
		didWriteSignature = NO;
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:SIG_BEFOREUPDATE]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
		{
			SMLog(kLogLevelVerbose,@"Signature to SFDC");
		...
		if([appDelegate.dataSync_dict count] >0)
		{
            [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:PUT_UPDATE]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Start"
                                 timeInterval:10.0];
			...
			{
				#ifdef kPrintLogsDuringWebServiceCall
					SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: PUT_UPDATE");
				#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:PUTUPDATE_DONE optimizedSynstate:oPUTUPDATE_DONE];
        ...
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:SIG_AFTERUPDATE]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		[self GetUpdate];
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:GET_UPDATE]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: GET_UPDATE");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:GETUPDATE_DONE optimizedSynstate:oGETUPDATE_DONE];
        [monitor monitorSMMessageWithName:@"[WSInterface DoIcnrementalDataSync:GET_UPDATE_DOWNLOAD_CRITERIA]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: GET_UPDATE_DWN_CRIT");
			#endif
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: CLN_UP_SELECT 3");
			#endif
    ...
    //8282
    //If one call sync and conflict on PUT_DELETE, then re insert the record
    //as it will be deleted in the getAllDataForIds function.
    if (doOneCallSync) {
        [appDelegate.databaseInterface reInsertDeletedRecordsOnConflict];
    }
...
/*******************************************************DOWNLOAD_CRITERIA_CHANGE*************************************************************************/
	...
	[appDelegate.databaseInterface deleteDownloadCriteriaObjects:deletedObjects];
        ...
        [deletedObjects release]; deletedObjects = nil;
	[self downloadcriteriaplist:dcobjects_incrementalSync];
/*******************************************************DOWNLOAD_CRITERIA_CHANGE*************************************************************************/
   

//Radha Purging
    if(![appDelegate.databaseInterface ContinueIncrementalDataSync] && !([appDelegate.databaseInterface getConflictsStatus]))
    {
...
//    }
	...
	if (optimizeSyncCalls.purgingEventIdArray != nil && [optimizeSyncCalls.purgingEventIdArray count] > 0)
	{
		...
		//[optimizeSyncCalls.purgingEventIdArray release]; //COMMENTED TO FIX A ERROR. TODO: VERIFY THE LEAK
		optimizeSyncCalls.purgingEventIdArray = nil;
	}
...
//        normal sync
	...
	if(![appDelegate.databaseInterface ContinueIncrementalDataSync])
	{
		...
		else if(appDelegate.shouldScheduleTimer)
		{
            ...
            //#Defect 8524
            NSComparisonResult result3 = [scheduledTimerdate compare:syncCompleted];
    ...
    else
    {
		...
		if (!doOneCallSync)
		{
            [monitor monitorSMMessageWithName:@"[WSInterface CLEAN_UP]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Start"
                                 timeInterval:10.0];
			...
			{
				#ifdef kPrintLogsDuringWebServiceCall
					SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: CLEAN_UP");
				#endif
    ...
    for(int j = 0; j < [alleventsTime count] ; j++)
    {
        ...
        if( [current_dateTime compare:timeStamp] == NSOrderedDescending)
        {
            ...
            if(!conflictExist)
            {
                ...
                SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
                [monitor monitorSMMessageWithName:@"[WSInterface DoIncrementalDataSync]"
                                     withUserName:appDelegate.currentUserName
                                         logLevel:kPerformanceLevelWarning
                                       logContext:@"Start"
                ...
                while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
                {
                    SMLog(kLogLevelVerbose,@"pagelevel events");
...
//	[appDelegate setCurrentSyncStatusProgress:SYNC_END optimizedSynstate:oSYNC_END];
    ...
    else
    {
        ...
        if(!conflict_exists && ![appDelegate.databaseInterface ContinueIncrementalDataSync])
        {
            [plistUtility clearPlist:EVENT_REFERENCE_PLIST];
        }
    }
    ...
    {
        appDelegate.dataSyncRunning = NO;
        // Defect 9957 fix - Vipin  EventSync - FEB 22
 ...
 [autoreleasePool release];
    ...
    SMLog(kLogLevelVerbose,@" [isync] DoIncrementalDataSync  - completed");
    /** Pushpak 10734 
     * Resetting restarted flag as sync is successfully completed
     */
    appDelegate.didAutoRestartedDataSyncFailedWithInvalidSession = NO;
...
- (BOOL) CustomSingleSyncCall:(NSDate *)syncStarted
{
	...
	didWriteSignature = NO;
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface CustomSingleSyncCall]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
	...
	SMLog(kLogLevelVerbose,@" ********************** START : SINGLE SYNC CALL ********************** %@",[NSDate date]);
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface ONCE_CALL_SYNC]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
	...
	{
		#ifdef kPrintLogsDuringWebServiceCall
			SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: CustomSingleSyncCall");
		#endif
	...
	SMLog(kLogLevelVerbose,@" ********************** END : SINGLE SYNC CALL ********************** %@",[NSDate date]);
	appDelegate.Incremental_sync_status = INCR_STARTS;
...
- (BOOL) getAllDataForIds:(NSDate *)syncStarted flag:(BOOL)doOnesync
{
	...
	if (!doOnesync)
	{
        ...
        SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
        [monitor monitorSMMessageWithName:@"[WSInterface getAllDataForIDs:TX_FETCH]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:10.0];
        ...
        {
			#ifdef kPrintLogsDuringWebServiceCall
				SMLog(kLogLevelVerbose,@"WSinterface.m : DOINC_DATA_SYNC: TX_FETCH");
			#endif
	...
	{
		SMLog(kLogLevelVerbose,@" ********************** START : TXFETCH SYNC CALL OPT ********************** %@",[NSDate date]);
		[appDelegate.databaseInterface deleteAll_GET_DELETES_And_PUT_DELETE_From_HeapAndObject_tables:GET_DELETE];
		[appDelegate.databaseInterface deleteAll_GET_DELETES_And_PUT_DELETE_From_HeapAndObject_tables:PUT_DELETE];
		...
		SMLog(kLogLevelVerbose,@" ********************** END : TXFETCH SYNC CALL OPT ********************** %@",[NSDate date]);
	}
...
- (void) doSignatureAndPDF:(BOOL)doOnesync
{
	...
	if (doOnesync)
	{
		...
		didWriteSignature = NO;
        [monitor monitorSMMessageWithName:@"[WSInterface doSignatureAndPDF:SIG_AFTERUPDATE]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:kWSExecutionDuration];
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface doSignatureAndPDF:SIG_AFTERUPDATE]"
                         withUserName:appDelegate.currentUserName
                           ...
                           logContext:@"Stop"
                         timeInterval:kWSExecutionDuration];
    [monitor monitorSMMessageWithName:@"[WSInterface doSignatureAndPDF:SIG_AFTERSYNC]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:kWSExecutionDuration];

    //WO signatures non uploading. in offline mode 6/11/13
    didWriteSignatures = NO;
	[appDelegate.calDataBase getAllLocalIdsForSignature:SIG_AFTERSYNC andSignType:@""];
	while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
	{
		SMLog(kLogLevelVerbose,@"Signature to SFDC- For OPDOC");
		if (didWriteSignature == YES)
			break;
		if (![appDelegate isInternetConnectionAvailable])
		{
	...
	//krishna sync
	didWriteOPDOC = NO;
    [monitor monitorSMMessageWithName:@"[WSInterface doSignatureAndPDF:syncOutPutDoc]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:kWSExecutionDuration];
	...
	while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
	{
		SMLog(kLogLevelVerbose,@"PDF to SFDC");
		if (didWritePDF == YES)
			break;
		if (![appDelegate isInternetConnectionAvailable])
		{
...
- (BOOL) checkForConflictsForId:(NSString *)record_id andObject_name:(NSString *)object_name
{
    ...
    SMLog(kLogLevelVerbose,@"isconflict %d andSfId %@",isConflictExists,sf_id);
...
- (void)requestForStaticResources
{
    ...
    [binding INTF_MetaSync_WSAsyncUsingParameters:metaSync
                       ...
                       AllowFieldTruncationHeader:allowFieldTruncationHeader
                                         delegate:self];
    SMLog(kLogLevelVerbose,@"Requested!!");
}
//Damodar OPDOC
...
- (void)submitHTMLDocuments:(NSArray*)docs withSignatures:(NSArray*)signatures
{
    ...
    @catch (NSException *exp)
    {
        SMLog(kLogLevelError,@"Exception Name WSInterface : Submit_Document %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface : Submit_Document %@",exp.reason);
    }
}
...
- (void)generatePDFfor:(NSArray*)docs withSignatures:(NSArray*)signatures
{
    ...
    @catch (NSException *exp)
    {
        SMLog(kLogLevelError,@"Exception Name WSInterface : GENERATE_PDF %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface : GENERATE_PDF %@",exp.reason);
    }
}
...
- (void)doGetPrice
{
    ...
    {
        SMLog(kLogLevelError,@"Some Objects Doesn't Have permission. Give the permission and the sync again for Get Price");
        return;
    }
    ...
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-1]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];

    [self dataSyncWithEventName:GET_PRICE_DATA eventType:SYNC requestId:requestId withData:nil lastIndex:@"0" responseTypeForLastIndexTwo:FALSE]; //Get price performance
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        SMLog(kLogLevelVerbose,@"iPadScrollerViewController.m : doGetPrice: Download GetPrice Data Sync");
        if (appDelegate.initial_sync_succes_or_failed == DATA_SYNC_FAILED)
        {
            SMLog(kLogLevelError,@"GetPrice Failed");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
            break;
        }
        if (![appDelegate isInternetConnectionAvailable])
        {
            SMLog(kLogLevelError,@"GetPrice Data Sync Failed due to Internet Lost");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
            break;
        }
        if(appDelegate.connection_error)
        {
			...
			[appDelegate checkifConflictExistsForConnectionError];
            appDelegate.connection_error=FALSE;
            SMLog(kLogLevelError,@"GetPrice Data Sync Failed due to Connection Error");
            [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-1]"
                                 withUserName:appDelegate.currentUserName
                                     logLevel:kPerformanceLevelWarning
                                   logContext:@"Stop"
        ...
        {
            SMLog(kLogLevelVerbose,@"GetPrice Data Sync Completed for Request ID = %@",requestId);
            break;
        }
    }
    [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-1]"
    ...
    // Second Call
    requestId = [[AppDelegate GetUUID] retain];
    appDelegate.initial_sync_status = SYNC_GP_DATA; //  Need to change it for Get Price
    appDelegate.Incremental_sync_status = INCR_STARTS;
    appDelegate.Sync_check_in = FALSE;
    [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-2]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];

    [self  dataSyncWithEventName:GET_PRICE_DATA eventType:SYNC requestId:requestId withData:nil lastIndex:@"1" responseTypeForLastIndexTwo:FALSE]; //Get price performance
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        SMLog(kLogLevelVerbose,@"iPadScrollerViewController.m : doGetPrice: Download GetPrice Data Sync");
        if (appDelegate.initial_sync_succes_or_failed == DATA_SYNC_FAILED)
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-3]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];

    [self dataSyncWithEventName:GET_PRICE_DATA eventType:SYNC requestId:requestId withData:data lastIndex:@"2" responseTypeForLastIndexTwo:TRUE];//Get price performance
    ...
    //D-00003729 - Price Calculation 2.0 (Radha) - START
    requestId = [[AppDelegate GetUUID] retain];
    appDelegate.initial_sync_status = SYNC_GP_DATA; //  Need to change it for Get Price
    appDelegate.Incremental_sync_status = INCR_STARTS;
    appDelegate.Sync_check_in = FALSE;
    
    [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-4]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
    ...
    [self dataSyncWithEventName:GET_PRICE_DATA eventType:SYNC requestId:requestId withData:nil lastIndex:@"3" responseTypeForLastIndexTwo:FALSE]; //Get price performance
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        SMLog(kLogLevelVerbose,@"iPadScrollerViewController.m : doGetPrice: Download GetPrice Data Sync");
        if (appDelegate.initial_sync_succes_or_failed == DATA_SYNC_FAILED)
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface doGetPrice:Call-4]"
                         withUserName:appDelegate.currentUserName
                           ...
                           logContext:@"Stop"
                         timeInterval:10.0];
    //D-00003729 - Price Calculation 2.0 (Radha) - END
...
- (BOOL)doAdvanceDownloadCriteria 
{
    ...
    appDelegate.Sync_check_in = FALSE;
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface ADV_DOWNLOAD_CRITERIA]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];

    [self dataSyncWithEventName:@"ADV_DOWNLOAD_CRITERIA" eventType:SYNC requestId:requestId withData:nil lastIndex:@"" responseTypeForLastIndexTwo:FALSE]; //Get price performance
    [requestId autorelease];
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        SMLog(kLogLevelVerbose,@"WSInterface.m :Download Advance Download Criteria ");
        if (appDelegate.initial_sync_succes_or_failed == DATA_SYNC_FAILED)
        {
            SMLog(kLogLevelError,@"Adv Download Criteria Failed");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
        ...
        if (![appDelegate isInternetConnectionAvailable])
        {
            SMLog(kLogLevelError,@"Adv Download Criteria Data Sync Failed due to Internet Lost");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
            return FALSE;
        }
        if(appDelegate.connection_error)
        {
            SMLog(kLogLevelError,@"Adv Download Criteria Data Sync Failed due to Connection Error");
            return FALSE;
        }
        if (appDelegate.Incremental_sync_status == SYNC_ADV_DWN_CRT_DONE)
        {
            SMLog(kLogLevelVerbose,@"Adv Download Criteria Data Sync Completed for Request ID = %@",requestId);
            break;
        }
    }
    [monitor monitorSMMessageWithName:@"[WSInterface ADV_DOWNLOAD_CRITERIA]"
...
-(void)generateRequestId
{
    ...
    if(Insert_requestId == nil || [Insert_requestId isEqualToString:@""])
        Insert_requestId = [AppDelegate GetUUID];     //GEnerate the request_Id
    else
    {
        SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
        [monitor monitorSMMessageWithName:@"[WSInterface generateRequestId:CLEAN_UP_SELECT]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:kWSExecutionDuration];
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(kLogLevelVerbose,@"WSinterface.m : generateRequestId: CLEAN_UP_SELECT");
#endif
...
-(void)getAllRecordsForOperationType:(NSString *)OpearationType
{
    ...
    // Vipindas field merge feature
    NSArray * keys_all = [NSArray arrayWithObjects:@"local_id",          @"parent_object_name",
                                                   @"parent_local_id",   @"time_stamp",
                                                   @"record_type",       @"fields_modified",    nil];
    @try
    {
        
     // Iterate over Data_trailor-temp table fetched records
    ...
    for(int i = 0 ; i < [object_array count] ; i++)
    {
        ...
        {
            // Vipindas field merge feature
            // Object mapping found in data-sync dictionary of global
            NSMutableArray * array = [appDelegate.dataSync_dict objectForKey:object_name];
            NSDictionary * dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:local_id, parent_object_name,parent_local_id, time_stamp, record_type, fields_modified, nil] forKeys:keys_all];
            [array addObject:dict];
        }
        else
        {
            // No Object mapping found in data-sync dictionary of global, lets create one!
            NSMutableArray  * array = [[NSMutableArray alloc] initWithCapacity:0];
            NSDictionary * dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:local_id,parent_object_name,parent_local_id,time_stamp,record_type, fields_modified, nil] forKeys:keys_all];
            [array addObject:dict];
            [appDelegate.dataSync_dict setValue:array forKey:object_name];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getAllRecordsForOperationType %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getAllRecordsForOperationType %@",exp.reason);
    }
...
@try{
    ...
    //ADD SVMXDEVlient : krishna 10.4.404 change
    INTF_WebServicesDefServiceSvc_SVMXDEVlient  * SVMXDEV_client =  [appDelegate getSVMXDEVlientObject];//[self getSVMXDEVlientObject];
    
    [sfmRequest addClientInfo:SVMXDEV_client];
    [datasync setRequest:sfmRequest];
    
    //    [[[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init] autorelease];
    //
    //    SVMXDEV_client.clientType = @"iPad";
    //    [SVMXDEV_client.clientInfo addObject:@"OS:iPadOS"];
    //    [SVMXDEV_client.clientInfo addObject:@"R4B2"];
...
//        [sfmRequest.valueMap addObject:valueMapforObject];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getOnDemandRecords %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getOnDemandRecords %@",exp.reason);
    }
}
-(void)GetInsert
{
    SMLog(kLogLevelVerbose,@" GetInsert started....");
...
@try{
    ...
    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_lastModified =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
    SVMXDEVMap_lastModified.key = @"LAST_SYNC_TIME";
    SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_INSERT_RESONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_INSERT_RESONSE_TIME];
    
    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_startTime =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
    SVMXDEVMap_startTime.key  = @"RANGE_START";
    SVMXDEVMap_startTime.value = [self getSyncTimeStampWithTheIntervalof15days:START_TIME] == nil ? @"" :[self getSyncTimeStampWithTheIntervalof15days:START_TIME];
    
    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_endTime =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
    SVMXDEVMap_endTime.key  = @"RANGE_END";
    SVMXDEVMap_endTime.value = [self getSyncTimeStampWithTheIntervalof15days:END_TIME]== nil ? @"" :[self getSyncTimeStampWithTheIntervalof15days:END_TIME];
    
    [sfmRequest.valueMap addObject:SVMXDEVMap_lastModified];
    [sfmRequest.valueMap addObject:SVMXDEVMap_startTime];
    [sfmRequest.valueMap addObject:SVMXDEVMap_endTime];

    //ADD SVMXDEVlient changed : kri 10.4.404
    //    INTF_WebServicesDefServiceSvc_SVMXDEVlient  * SVMXDEV_client = [[[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init]  autorelease];
    //
    //    SVMXDEV_client.clientType = @"iPad";
    //    [SVMXDEV_client.clientInfo addObject:@"OS:iPadOS"];
    ...
    SMLog(kLogLevelVerbose,@" GetInsert ends....");
}

-(void)GetUpdate
{
    SMLog(kLogLevelVerbose,@" GetUpdate started....");
    ...
    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_lastModified =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
    SVMXDEVMap_lastModified.key = @"LAST_SYNC_TIME";
    SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_UPDATE_RESONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_UPDATE_RESONSE_TIME];
    ...
    [sfmRequest.valueMap addObject:SVMXDEVMap_endTime];
        ...
        //    INTF_WebServicesDefServiceSvc_SVMXDEVlient  * SVMXDEV_client = [[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init];
        //
        //    SVMXDEV_client.clientType = @"iPad";
        //    [SVMXDEV_client.clientInfo addObject:@"OS:iPadOS"];
        //    [SVMXDEV_client.clientInfo addObject:@"R4B2"];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :GetUpdate %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :GetUpdate %@",exp.reason);
    }
    
    [[PerformanceAnalytics sharedInstance] completedPerformanceObservationForContext:@"GetUpdate"
                                                         andRecordCount:0];
    
    SMLog(kLogLevelVerbose,@" GetUpdate end....");
...
-(void)GetDelete
{
    ...
    SMLog(kLogLevelVerbose,@" GetDelete started....");
    ...
    SVMXDEVMap_lastModified.key = @"LAST_SYNC_TIME";
    SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_DELETE_RESPONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_DELETE_RESPONSE_TIME];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :GetDelete %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :GetDelete %@",exp.reason);
    }

    [[PerformanceAnalytics sharedInstance] completedPerformanceObservationForContext:@"GetDelete"
                                                         andRecordCount:0];
    
    SMLog(kLogLevelVerbose,@" GetDelete Ends...");
}

-(void)cleanUpForRequestId:(NSString *)requestId  forEventName:(NSString *)eventName
{
    SMLog(kLogLevelVerbose,@" cleanUpForRequestId : %@ - %@", requestId, eventName);
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :cleanUpForRequestId %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :cleanUpForRequestId %@",exp.reason);
    }
...
@try{
    ...
    SMLog(kLogLevelVerbose,@"GETDownloadCriteriaRecordsFor : %@ starts", event_name);
    ...
    {
        SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_DC_DELETE_RESPONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_DC_DELETE_RESPONSE_TIME];
    }
    else if ([event_name isEqualToString:GET_INSERT_DOWNLOAD_CRITERIA])
    {
        SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_DC_INSERT_RESPONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_DC_INSERT_RESPONSE_TIME];
    }
    else if ([event_name isEqualToString:GET_UPDATE_DOWNLOAD_CRITERIA])
    {
        SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_DC_UPDATE_RESPONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_DC_UPDATE_RESPONSE_TIME];
    }
    ...
    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_dc_objects =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init] autorelease];
    SVMXDEVMap_dc_objects.key  = DOWNLOAD_CRITERIA_OBJECTS;
    NSDictionary * dict = [self getdownloadCriteriaObjects];
   /* NSAutoreleasePool * autoreleasePool = [[NSAutoreleasePool alloc] init];
    SBJsonWriter * writer = [[[SBJsonWriter alloc] init] autorelease];
    NSString * jsonstring = [[writer  stringWithObject:dict] retain];
    SVMXDEVMap_dc_objects.value = jsonstring;
    [autoreleasePool release];*/
    ...
    for(NSString * object_name in allobjects)
    {
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_Object =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVMap_Object.key = object_name;
        SVMXDEVMap_Object.value = [dict objectForKey:object_name];
        [SVMXDEVMap_dc_objects.valueMap addObject:SVMXDEVMap_Object];
        [SVMXDEVMap_Object release]; SVMXDEVMap_Object = nil;
    }
    ...
    [sfmRequest.valueMap addObject:SVMXDEVMap_startTime];
    [sfmRequest.valueMap addObject:SVMXDEVMap_endTime];
    [sfmRequest.valueMap addObject:SVMXDEVMap_dc_objects];
...
//    [SVMXDEV_client.clientInfo addObject:@"R4B2"];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :GETDownloadCriteriaRecordsFor %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :GETDownloadCriteriaRecordsFor %@",exp.reason);
    }
    ...
    SMLog(kLogLevelVerbose,@"GETDownloadCriteriaRecordsFor : %@ ends...", event_name);
}
...
- (void) updateSyncTimeStampINSyncHistoryPlist:(BOOL)doOnesync
{
	...
	if (doOnesync)
	{
		for(NSString *  str in allkeys)
		{
            ...
            else if ([str isEqualToString:LAST_UPDATE_RESONSE_TIME]) // Damodar : Defect 8593
			{
				if ([self.get_update_last_sync_time length] > 0)
					[dict setObject:self.get_update_last_sync_time forKey:LAST_UPDATE_RESONSE_TIME];
			}
		}
        //#Defect 8524
        [dict writeToFile:plistPath_SYNHIST atomically:YES];
		if(optimizeSyncCalls.lastSyncTime !=  nil)
		{
            ...
            {
                [self setsyncHistoryForSyncType:UPDATE  requestOrResponse:RESPONSE request_id:@"" last_sync_time:optimizeSyncCalls.lastSyncTime];
            }
            
            if(!([self.get_update_last_sync_time length] > 0)) // Damodar : Defect 8593 :: Updating last update time with last sync time if get_update value is unavailable
            {
                [self setsyncHistoryForSyncType:UPDATE  requestOrResponse:RESPONSE request_id:@"" last_sync_time:optimizeSyncCalls.lastSyncTime];
            }
		}
...
-(void) Put:(NSString *)event_name
{
    ...
    SMLog(kLogLevelVerbose,@" PUT %@ started.... ", event_name);
...
@try{
    ...
    // Vipindas field merge feature
    databaseIntefaceSfm *databaseInterface =  appDelegate.databaseInterface;
   ...
   // event_name = @"PUT_INSERT";
    if([event_name isEqualToString:@"PUT_UPDATE"] && [event_type isEqualToString:@"SYNC"])//PUT_UPSATE
    {
      ...
      INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_lastModified =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init]  autorelease];
		...
		//Radha - #defect 8227
		SMLog(kLogLevelVerbose,@" rm put_insert_time = %@", put_insert_time);
        
        /* Removing put_insert_time - Vipindas
        ...
        for(int i = 0 ; i < [all_objects count]; i++)
        {
           ...
           // SVMXDEVmap.key = @"object_name" ;
            ...
            //// Vipindas field merge feature - Removing value for Fields key.
            //record_SVMXDEV.value = field_string;
            
            //SMLog(kLogLevelVerbose,@" rm >Fields</ = %@", field_string);

            
            BOOL value_map_count = 0;

            NSMutableArray *beforeSaveFields = [[NSMutableArray alloc] init];
            NSLog(@" PUT_UP info_dict  : %@", info_dict);
            
            // Iterate over data set - info_dict
            ...
            for (NSDictionary * dict in info_dict) 
            {
                ...
                {
                    // Ohh it is resolving conflict sync
                    if(appDelegate.speacialSyncIsGoingOn)
                    {
                        if([sf_id length] == 0)
                        {
                            // Wooo SF_ID is missing....
                            // We are skipping current record, lets move to next record in the list.
                            continue;
                        }
                    }
                    else
                    {
                        //check for id - SalesForce id - SF_ID
                        NSString * child_sf_id  = [databaseInterface getSfid_For_LocalId_From_Object_table:object_name
                                                                                                  local_id:local_id];
                ...
                // Ohh try to resolve conflict
                if(appDelegate.speacialSyncIsGoingOn)
                {
                    if([local_id length] == 0)
                    {
                        // Missing local_id ... lets fetch local_id by using record's SF_ID. when it will happen?
                        local_id =  [databaseInterface getLocalIdFromSFId:sf_id
                                                                tableName:object_name];
                    }
                }
                
                // Fetch Actual record from respective table as dictionary. key:ColumnName => value:Data
                
                NSMutableDictionary * each_record = [databaseInterface getRecordsForRecordId:local_id
                                                                               ForObjectName:object_name
                                                                                      fields:field_string];
                
                // Replace parent_column_name with Parent_sf_id
                BOOL  replaceSuccess = [self replaceReferenceLocalIdWithSfidForRecord:each_record
                                                                              isChild:isChild_object
                                                                          referenceTo:referenceTo
                                                                    parentColoumnName:parent_column_name
                ...
                if(!replaceSuccess)
                {
                    ...
                    for(NSString * fieldApiName in referenceFields)
                    {
                        if([record_fields containsObject:fieldApiName])
                        {
                            ...
                            {
                                // Go back and continue with next record
                                continue;
                            }
                            
                            // There is local_id exist, not meaningful lets remove them.
                ...
                if(!check_id_exists)
                {
                    ...
                    NSLog(@" fields_modified :- %@", fields_modified);
                    ...
                    {
                        SBJsonWriter * localJsonWriter = [[SBJsonWriter alloc] init];
                        json_record = [localJsonWriter stringWithObject:each_record];
                        [localJsonWriter release]; localJsonWriter = nil;
                    }
          ...
          /* // code to differentiate b/w the parent and child 
           ...
           if([Master_or_parent isEqualToString:MASTER])
            {
                SVMXDEVmap.key = @"Parent_Object";
            }
            else if([Master_or_parent isEqualToString:DETAIL])
            {
                SVMXDEVmap.key = @"Child_Object";
            }

            //assign key is whether master or detail
            SPR14map.value = object_name; //object  api name */
            // Vipindas field merge feature
            ...
            [record_SVMXDEV release]; record_SVMXDEV = nil;
            [iscallBack release]; iscallBack = nil;
            [SVMXDEVmap release];  SVMXDEVmap = nil;
   ...
   else if([event_name isEqualToString:@"PUT_INSERT"] && [event_type isEqualToString:@"SYNC"])
   {
        ...
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_lastModified =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init] autorelease];         //sahana30April
        SVMXDEVMap_lastModified.key = @"SYNC_TIME_STAMP";
        SVMXDEVMap_lastModified.value = [self get_SYNCHISTORYTime_ForKey:LAST_INSERT_RESONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_INSERT_RESONSE_TIME];
        
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_startTime =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init] autorelease];            //sahana30April
        SVMXDEVMap_startTime.key  = @"RANGE_START";
        SVMXDEVMap_startTime.value = [self getSyncTimeStampWithTheIntervalof15days:START_TIME] == nil ? @"" :[self getSyncTimeStampWithTheIntervalof15days:START_TIME];
        
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_endTime =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init] autorelease];               //sahana30April
        SVMXDEVMap_endTime.key  = @"RANGE_END";
        SVMXDEVMap_endTime.value = [self getSyncTimeStampWithTheIntervalof15days:END_TIME]== nil ? @"" :[self getSyncTimeStampWithTheIntervalof15days:END_TIME];
            ...
            for(int x = 0 ; x < [tempArray count] && count < CHUNCKING_LIMIT ;x++)
            {
                ...
                for(int i = 0 ; i < [all_objects count] && count < CHUNCKING_LIMIT ; i++)
                {
                    ...
                    if([tempMasterOrDetail isEqualToString:object_name])
                    {
                        ...
                        SVMXDEVmap.key = key ;
                        SVMXDEVmap.value = object_name; //object  api name 
                        ...
                        for (NSDictionary * dict in info_dict) 
                        {
                            ...
                            if(!recordExists)
                            {
                                //[appDelegate.databaseInterface DeleteDataTrailerTableAfterSync:local_id forObject:object_name sync_type:INSERT];
                                [appDelegate.databaseInterface DeleterecordFromDataTrailerTableForlocal_id:local_id];
                            ...
                            if([key isEqualToString:@"Child_Object"] && replaceSuccess)
                            {
                                ...
                                if(![finalSetOfrecords containsObject:parent_local_id] && [Parent_SF_ID_from_object_table length] == 0)
                                {
                                    ...
                                    if(!does_referenceRecordExists)
                                    {
                                      ...
                                      /*  [appDelegate.databaseInterface DeleterecordFromTable:object_name Forlocal_id:local_id];
                                        [appDelegate.databaseInterface DeleteDataTrailerTableAfterSync:local_id forObject:object_name sync_type:INSERT];
                                        [appDelegate.databaseInterface deleteRecordFromConflictTableForRecord:local_id operation:PUT_INSERT];
                                        [appDelegate.databaseInterface updateDataTrailer_RecordSentForlocalId:local_id operation_type:INSERT];*/
                                        // continue;
                                    ...
                                    else
                                    {
                                        ...
                                        if(conflict_exists_local_id || ParentrecordNotSent ||  [self.customSync_unsynched_records containsObject:parent_local_id])
                                        {
                                            [appDelegate.databaseInterface updateDataTrailer_RecordSentForlocalId:local_id operation_type:INSERT];
                                        }
                                        continue;
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :Put %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :Put %@",exp.reason);
    }
            ...
            if([referenceFieldId length] > 30)
            {
                ...
                if([reference_sf_id length] == 0)
                {
                    ...
                    else
                    {
                        ...
                        if(conflict_exists_local_id || ParentrecordNotSent || [self.customSync_unsynched_records containsObject:referenceFieldId])
                        {
...
#pragma mark - SVMX Ping
- (void) connectSVMX
{
    SMXPing *pingObj = [[SMXPing alloc] init];
    [pingObj connectSalesforce];
    [pingObj release]; pingObj = nil;
    return;
}
...
@try{
    ...
    [binding SVMX_GetSvmxVersionAsyncUsingParameters:getVersion
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getSvmxVersion %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getSvmxVersion %@",exp.reason);
    }
...
- (void) metaSyncWithEventName:(NSString *)eventName eventType:(NSString *)eventType values:(NSMutableArray *)values
{
    ...
    NSLog(@" MEMX Exception : name: %@  - type : %@", eventName, eventType);
 @try{   
    [INTF_WebServicesDefServiceSvc initialize];
    
    INTF_WebServicesDefServiceSvc_SessionHeader * session = [[INTF_WebServicesDefServiceSvc_SessionHeader alloc] init] ;
...
//    svmxMap = NULL;
     ...
     //krishna
     INTF_WebServicesDefServiceSvc_SVMXDEVlient * client = nil;//[[[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init] autorelease];
    ...
    else
    {
        ...
        client = [appDelegate getSVMXDEVlientObject];
        sfmRequest.value = @"";
        [sfmRequest.values addObjectsFromArray:values];
    }
    ...
    {
        INTF_WebServicesDefServiceSvc_SVMXDEVlient * client_iPad_Version = [[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init];
        ...
        [client_iPad_Version release]; client_iPad_Version = nil;
        [iPadVersion release]; iPadVersion = nil;
    }

    [metaSync setRequest:sfmRequest];  
     [sfmRequest release]; sfmRequest = nil;
    //[[ZKServerSwitchboard switchboard] doCheckSession];
    [binding INTF_MetaSync_WSAsyncUsingParameters:metaSync 
                                    SessionHeader:session 
                                      CallOptions:callOptions 
     ...
     [allowFieldTruncationHeader release];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :metaSyncWithEventName %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :metaSyncWithEventName %@",exp.reason);
    }
...
//009493 - Memory optimization : Krishna
//Autorelease removed
...
@try{
    ...
    //krishna
//    INTF_WebServicesDefServiceSvc_SVMXDEVlient * client = [[[INTF_WebServicesDefServiceSvc_SVMXDEVlient alloc] init] autorelease];
    ...
    [SVMXDEVMap_startTime release];
    [SVMXDEVMap_endTime release];
    //    client.clientType = @"iPad";
    //    [client.clientInfo addObject:@"OS:iPadOS"];
    //    [client.clientInfo addObject:@"R4B2"];
    ...
    //krishna 10.4.404
    INTF_WebServicesDefServiceSvc_SVMXDEVlient * client = [appDelegate getSVMXDEVlientObject];
//    [self addClientInfoForObject:client];
    ...
    {
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVProcessID =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVProcessID.key  = @"SearchProcessId";
        if([values objectAtIndex:0] != nil)
            SVMXDEVProcessID.value = [values objectAtIndex:0];
        else
            SVMXDEVProcessID.value = @"";
        [sfmRequest.valueMap addObject:SVMXDEVProcessID];
        [SVMXDEVProcessID release]; SVMXDEVProcessID = nil;
        
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVCrtieria =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVCrtieria.key  = @"SEARCH_OPERATOR";
        if([values objectAtIndex:3] != nil)
            SVMXDEVCrtieria.value = [values objectAtIndex:3];
        else
            SVMXDEVCrtieria.value = @"";
        [sfmRequest.valueMap addObject:SVMXDEVCrtieria];
        [SVMXDEVCrtieria release]; SVMXDEVCrtieria = nil;
        
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVUserFilterString =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVUserFilterString.key  = @"KeyWord";
        if([values objectAtIndex:4] != nil)
            SVMXDEVUserFilterString.value = [values objectAtIndex:4];
        else
            SVMXDEVUserFilterString.value = @"";
        [sfmRequest.valueMap addObject:SVMXDEVUserFilterString];
        [SVMXDEVUserFilterString release]; SVMXDEVUserFilterString = nil;
        ...
        for(int i=0; i<[objectList count]; i++)
        {
            ...
            SVMXDEVObject.key  = @"ObjectId";
            SVMXDEVObject.value = [objectList objectAtIndex:i];
            /*
            for(int j=0; j< [resultsArray count]; j++)
            {
                [SVMXDEVObject addValues:[resultsArray objectAtIndex:j]];
            }
             */
            [sfmRequest.valueMap addObject:SVMXDEVObject];            
            [SVMXDEVObject release]; SVMXDEVObject = nil;
        }
         
        NSString *limit = [values objectAtIndex:2];
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVLimitObject =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        
        SVMXDEVLimitObject.key  = @"RecordLimit";
        SVMXDEVLimitObject.value = limit;
        [sfmRequest.valueMap addObject:SVMXDEVLimitObject];
        [SVMXDEVLimitObject release]; SVMXDEVLimitObject = nil;
        
    }
    if([eventName isEqualToString:@"TECH_LOCATION_UPDATE"] && [eventType isEqualToString:SYNC])
    {
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVField =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVField.key=@"Fields";
        SVMXDEVField.value=@"";
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVLatitude =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVLatitude.key  = @"SVMXDEV__Latitude__c";
        if([values objectAtIndex:0] != nil)
            SVMXDEVLatitude.value = [values objectAtIndex:0];
        else
            SVMXDEVLatitude.value = @"";
        [SVMXDEVField.valueMap addObject:SVMXDEVLatitude];
        [SVMXDEVLatitude release];
        
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVLongitude =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVLongitude.key  = @"SVMXDEV__Longitude__c";
        if([values objectAtIndex:1] != nil)
            SVMXDEVLongitude.value = [values objectAtIndex:1];
        else
            SVMXDEVLongitude.value = @"";
        [SVMXDEVField.valueMap addObject:SVMXDEVLongitude];
        [SVMXDEVLongitude release];
        [sfmRequest.valueMap addObject:SVMXDEVField];
        [SVMXDEVField release];
    ...
    else if([eventName isEqualToString:@"LOCATION_HISTORY"] && [eventType isEqualToString:SYNC])
    {
        ...
        {
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVField =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVField.key=@"Record";
            SVMXDEVField.value=@"";
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVLocalID =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVLocalID.key  = @"localId";
            SVMXDEVLocalID.value = [dict objectForKey:@"localId"];
            [SVMXDEVField.valueMap addObject:SVMXDEVLocalID];
            [SVMXDEVLocalID release];

            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVLatitude =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVLatitude.key  = @"SVMXDEV__Latitude__c";
            NSString *latitude = [dict objectForKey:@"SVMXDEV__Latitude__c"];
            if(latitude != nil)
                SVMXDEVLatitude.value = latitude;
            else
                SVMXDEVLatitude.value = @"";
            ...
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVLongitude =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVLongitude.key  = @"SVMXDEV__Longitude__c";
            NSString *longitude = [dict objectForKey:@"SVMXDEV__Longitude__c"];
            if(longitude != nil)
                SVMXDEVLongitude.value = longitude;
            else
                SVMXDEVLongitude.value = @"";
            [SVMXDEVField.valueMap addObject:SVMXDEVLongitude];
            [SVMXDEVLongitude release];

            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVStatus =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVStatus.key  = @"SVMXDEV__Status__c";
            SVMXDEVStatus.value = [dict objectForKey:@"SVMXDEV__Status__c"];
            [SVMXDEVField.valueMap addObject:SVMXDEVStatus];
            [SVMXDEVStatus release];

            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVUser =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVUser.key  = @"SVMXDEV__User__c";
            SVMXDEVUser.value = [dict objectForKey:@"SVMXDEV__User__c"];
            [SVMXDEVField.valueMap addObject:SVMXDEVUser];
            [SVMXDEVUser release];
            
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVDeviceType =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVDeviceType.key  = @"SVMXDEV__Device_Type__c";
            SVMXDEVDeviceType.value = [dict objectForKey:@"SVMXDEV__Device_Type__c"];
            [SVMXDEVField.valueMap addObject:SVMXDEVDeviceType];
            [SVMXDEVDeviceType release];
            
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVAdditionalInfo =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVAdditionalInfo.key  = @"SVMXDEV__Additional_Info__c";
            SVMXDEVAdditionalInfo.value = [dict objectForKey:@"SVMXDEV__Additional_Info__c"];
            [SVMXDEVField.valueMap addObject:SVMXDEVAdditionalInfo];
            [SVMXDEVAdditionalInfo release];
            
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVTimeRecorded =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVTimeRecorded.key  = @"SVMXDEV__Time_Recorded__c";
            SVMXDEVTimeRecorded.value = [dict objectForKey:@"SVMXDEV__Time_Recorded__c"];
            [SVMXDEVField.valueMap addObject:SVMXDEVTimeRecorded];
            [SVMXDEVTimeRecorded release];
    ...
    [dataSync release];                     dataSync = nil;
    [allowFieldTruncationHeader release];   allowFieldTruncationHeader= nil;
    [debuggingHeader release];              debuggingHeader = nil;
    [callOptions release];                  callOptions = nil;
    [sessionHeader release];                sessionHeader = nil;
    
    }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name WSInterface :dataSyncWithEventName %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :dataSyncWithEventName %@",exp.reason);
    }
}
...
- (void) dataSyncWithEventName:(NSString *)eventName
                    ...
                    responseTypeForLastIndexTwo:(BOOL)iSDCResponseType
{
    @try{
        [INTF_WebServicesDefServiceSvc initialize];
        ...
        SMLog(kLogLevelVerbose,@"  Incremental DataSync request looping  starts: %@", [NSDate date]);
        if([eventName isEqualToString:GET_PRICE_DATA] && [eventType  isEqualToString:SYNC])
        {

            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_last_index =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXDEVMap_last_index.key  = @"LAST_INDEX";
            SVMXDEVMap_last_index.value = lastIndex;
            [sfmRequest.valueMap addObject:SVMXDEVMap_last_index];
            
            if(appDelegate.dataSyncRunning)
            {
                INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMapLastSyncTime =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
                SVMXDEVMapLastSyncTime.key  = @"LAST_SYNC_TIME";
                
                //8581
                if([appDelegate shouldDoOneCallSync])
                {
                    SVMXDEVMapLastSyncTime.value=[appDelegate.wsInterface get_SYNCHISTORYTime_ForKey:LAST_OSC_TIMESTAMP] == nil ?@"":[appDelegate.wsInterface get_SYNCHISTORYTime_ForKey:LAST_OSC_TIMESTAMP];
                }
                else
                {
                    SVMXDEVMapLastSyncTime.value = [self get_SYNCHISTORYTime_ForKey:LAST_DC_INSERT_RESPONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_DC_INSERT_RESPONSE_TIME];
                }
                [sfmRequest.valueMap addObject:SVMXDEVMapLastSyncTime];
                /*Mem Opt*/
                [SVMXDEVMapLastSyncTime release];
            }
            ...
            if(([lastIndex isEqualToString:@"1"]) || ([lastIndex isEqualToString:@"2"]))
            {
                ...
                if([data count] > 1)
                {
                    ...
                    if((records != nil) && ([records count] == 2))
                    {
                        ...
                        {
                            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMapPartialObject =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init] autorelease];
                            SVMXDEVMapPartialObject.key  = @"PARTIAL_EXECUTED_OBJECT";
                            SVMXDEVMapPartialObject.value = [partialObjectDict objectForKey:@"partialObject"];
                            NSString *criteria = [NSString stringWithFormat:@"sync_flag = 'false' AND object_name = '%@'",SVMXDEVMapPartialObject.value];
                            ...
                            {
                                [SVMXDEVMapPartialObject.values addObject:[recordID objectForKey:@"sf_id"]];
                            }

                            [sfmRequest.valueMap addObject:SVMXDEVMapPartialObject];
                        }
                    }
                    if([records count] > 1)
                    {
                ...
                if([lastIndex isEqualToString:@"2"])
                {
                    ...
                    if([activityType count])
                    {
                        ...
                        [sfmRequest.valueMap addObject:SVMXDEVObject];
                        [SVMXDEVObject release];
                    }
                    //9937
                    //Sending unique currencies of Work Order
                    NSString *tableName = @"SVMXDEV__Service_Order__c";
                    NSString *columnName = @"CurrencyIsoCode";
                    ...
                    SVMXDEVMapCurrency.key  = @"CurrencyISO";
                     NSArray *uniqueCurrency = [[appDelegate.dataBase getUniqueRecordsFromTable:tableName
                                                                                    forColumn:columnName
                                                                               filterCriteria:nil] retain];
                    [SVMXDEVMapCurrency.values addObjectsFromArray:uniqueCurrency];
                    [sfmRequest.valueMap addObject:SVMXDEVMapCurrency];
                    [uniqueCurrency release];
                    [SVMXDEVMapCurrency release];
                    
                    //Get price performance
                    
                    if (iSDCResponseType)
                    {
                        INTF_WebServicesDefServiceSvc_SVMXMap * getPriceNewKey =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
                        getPriceNewKey.key  = @"GET_PRICE_RESPONSE_TYPE";
                        getPriceNewKey.value = @"DOWNLOAD_CRITERIA_TYPE";
                        [sfmRequest.valueMap addObject:getPriceNewKey];
                        [getPriceNewKey release];
                    }
                    
                }
            }
            //D-00003729 - Price Calculation 2.0 (Radha)
            else if ([lastIndex isEqualToString:@"3"])
            {
                if (data != nil && [data count] > 0)
                {
                    INTF_WebServicesDefServiceSvc_SVMXMap * lastId = [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
                    lastId.key = @"LAST_ID";
                    lastId.value = [data objectAtIndex:0];
                    [sfmRequest addValueMap:lastId];
                    [lastId release];
                }
            }
            else
        ...
        else if([eventName isEqualToString:@"ADV_DOWNLOAD_CRITERIA"] && [eventType  isEqualToString:SYNC])
        {
            ...
            {
                SVMXDEVMap_lastModified.value =@"";
            }
            [sfmRequest.valueMap addObject:SVMXDEVMap_lastModified];
            [SVMXDEVMap_lastModified release];
            
                      
            if ([lastIndex length]>0)
            {
               INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMapPartialObject =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
                SVMXDEVMapPartialObject.key = @"PARTIAL_EXECUTED_OBJECT";
                SVMXDEVMapPartialObject.value =lastIndex; // Passing Object Name as LastIndex string
                NSString *criteria = [NSString stringWithFormat:@"sync_flag = 'false' AND object_name = '%@'",SVMXDEVMapPartialObject.value];
                NSArray *columns = [NSArray arrayWithObjects:@"sf_id", nil];
                ...
                [sfmRequest.valueMap addObject:SVMXDEVMapPartialObject];
                [SVMXDEVMapPartialObject release];
            }
        }
        
        SMLog(kLogLevelVerbose,@"  Incremental DataSync request looping  ends: %@", [NSDate date]);
...
//        [self addClientInfoForObject:client];
        ...
        SMLog(kLogLevelVerbose,@"  Incremental DataSync Request sent: %@", [NSDate date]);
...
- (void) getTags
{
    ...
    [binding INTF_Get_Tags_WSAsyncUsingParameters:getTags
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getTags %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getTags %@",exp.reason);
    }
}
...
- (void) getCreateProcesses
{
    ...
    [binding INTF_Get_StandaloneCreate_LayoutsAsyncUsingParameters:getCreateProcesses 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getCreateProcesses %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getCreateProcesses %@",exp.reason);
    }
}
...
- (void) getViewLayouts
{
    ...
    [binding INTF_Get_View_Layouts_WSAsyncUsingParameters:getViewLayouts
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getViewLayouts %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getViewLayouts %@",exp.reason);
    }
...
//                                          delegate:self];
//    }@catch (NSException *exp) {
//        SMLog(kLogLevelVerbose,@"Exception Name WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.name);
//        SMLog(kLogLevelVerbose,@"Exception Reason WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.reason);
//    }
//}
...
- (void) savePageLayout
{
    ...
    INTF_WebServicesDefServiceSvc_SVMXDEV__Page_Layout_Detail__c * detailPage = [[[INTF_WebServicesDefServiceSvc_SVMXDEV__Page_Layout_Detail__c alloc] init] autorelease];
    
    detailPage.SVMXDEV__Detail_Type__c = @"Field";
    detailPage.SVMXDEV__Field_API_Name__c = @"AccountId";
    detailPage.SVMXDEV__DataType__c
    = @"reference";
    detailPage.SVMXDEV__Related_Object_Name__c = @"Account";
    detailPage.SVMXDEV__Related_Object_Name_Field__c = @"Name";
    detailPage.SVMXDEV__Named_Search__c = @"a0VA0000001AEp0MAG";
    detailPage.SVMXDEV__Display_Row__c = [NSNumber numberWithInt:1];
    detailPage.SVMXDEV__Display_Column__c = [NSNumber numberWithInt:1];
    detailPage.SVMXDEV__Required__c = NO;
    detailPage.SVMXDEV__Readonly__c = NO;
    detailPage.SVMXDEV__Sequence__c = [NSNumber numberWithInt:1];
    detailPage.SVMXDEV__Override_Related_Lookup__c = NO;
    ...
    INTF_WebServicesDefServiceSvc_SVMXDEV__Page_Layout__c * objPageHeaderLayout = [[[INTF_WebServicesDefServiceSvc_SVMXDEV__Page_Layout__c alloc] init] autorelease];
    objPageHeaderLayout.SVMXDEV__Name__c = @"Created from the custom Webservice";
    objPageHeaderLayout.SVMXDEV__Object_Name__c = @"Case";
    objPageHeaderLayout.SVMXDEV__Page_Layout_ID__c = @"CustFromWEBService";
    objPageHeaderLayout.SVMXDEV__Type__c = @"Header";
    ...
    [binding INTF_SavePageLayout_WSAsyncUsingParameters:savePageLayout
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :savePageLayout %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :savePageLayout %@",exp.reason);
    }
...
- (void) getUpdateEventsForStartDate:(NSString *)_startDate EndDate:(NSString *)_endDate recordID:(NSString *)_recordID
{
    ...
    [binding INTF_Update_Events_WSAsyncUsingParameters:(INTF_WebServicesDefServiceSvc_INTF_Update_Events_WS *) getUpdateEvents
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getUpdateEventsForStartDate %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getUpdateEventsForStartDate %@",exp.reason);
    }
}
...
- (void) getLookUpFieldsWithKeyword:(NSString *)keyword forObject:(NSString *)objectName returnTo:(id)caller setting:(BOOL)idAvailable overrideRelatedLookup:(NSNumber *)Override_Related_Lookup lookupContext:(NSString *)Lookup_Context lookupQuery:(NSString *)Lookup_Query_Field
{
    ...
    if (idAvailable == false)
    {
        ...
        strMap.value = objectName; //@"SVMXDEV__Service_Order__c";
    }
    else
    {
        strMap.key = RECORDID;
...
- (NSMutableDictionary *) getLookUpFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    INTF_WebServicesDefServiceSvc_SVMXDEV__ServiceMax_Processes__c * namedSearchHdr = [namedSearchInfo namedSearchHdr];
    NSString * defaultLookupColumn = [namedSearchHdr SVMXDEVX1__Default_Lookup_Column__c];
    ...
    {
        INTF_WebServicesDefServiceSvc_SVMXDEV__ServiceMax_Config_Data__c * data = [fields objectAtIndex:j];
        NSString * family;
        NSNumber * sequence;
        NSString * type = data.SVMXDEV__Search_Object_Field_Type__c;
        
        if ([type isEqualToString:@"Result"])
        {
            family = data.SVMXDEV__Field_Name__c;
            sequence = data.SVMXDEV__Sequence__c;
...
-(void) SaveSFMData:(NSDictionary *)sfmpage
{
    ...
    [binding SFM_SaveTargetRecords_WSAsyncUsingParameters:saveTargetRecords
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :SaveSFMData %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :SaveSFMData %@",exp.reason);
    }
}
...
//    webServiceClass = [webServiceName stringByDeletingPathExtension];
    ...
    SMLog(kLogLevelVerbose,@"Server URL = %@",serverURL);
    ...
    [binding INTF_PREQ_GetPrice_WSAsyncUsingParameters:getThoonsEvent
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :callCustomSFMAction %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :callCustomSFMAction %@",exp.reason);
    }
}
...
{   
    ...
    else if([event_name isEqualToString:BEFORESAVE] || [event_name isEqualToString:AFTERSAVE])
    {
        ...
        [dict release];
       // SMLog(kLogLevelVerbose,@" count %d", [appDelegate.afterSavePageLevelEvents count]);
        self.getPrice = TRUE;
    }
	}@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name WSInterface :callSFMEvent %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :callSFMEvent %@",exp.reason);
    }
}
...
-(void) AddRecordForLines:(NSString*) process_id ForDetailLayoutId:(NSString*) layout_id
{
    ...
    [binding INTF_AddRecords_WSAsyncUsingParameters:AddRecordWS
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :AddRecordForLines %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :AddRecordForLines %@",exp.reason);
    }
}
...
-(NSMutableDictionary *)getAddRecordsFields:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    add_WS = TRUE;  
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getAddRecordsFields %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getAddRecordsFields %@",exp.reason);
    }
    return result_dict;
}
...
@try{
    ...
    for(NSString * object_name in allobjects)
    {
        INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEVMap_dcObject =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
        SVMXDEVMap_dcObject.key = object_name;
        SVMXDEVMap_dcObject.value = [dict objectForKey:object_name];
        [sfmRequest.valueMap addObject:SVMXDEVMap_dcObject];
        [SVMXDEVMap_dcObject release];
    }
...
//    [SVMXDEV_client.clientInfo addObject:@"R4B2"];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :checkdownloadcriteria %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :checkdownloadcriteria %@",exp.reason);
    }
}
...
- (void)deleteRecordIDsFromTrailerAndObjectTables:(NSDictionary *)safeToDeleteIDs
{
    ...
    for(NSString *objectName in objectNames)
    {
        if([appDelegate.dataBase isTabelExistInDB:objectName])
        {
            ...
            SMLog(kLogLevelVerbose,@" Query to Delete Records from sync_Records_Heap = %@",dataTrailerDeleteQuery);
            [appDelegate.dataBase executeQuery:dataTrailerDeleteQuery];
            NSString *objectDeleteQuery = [NSString stringWithFormat:@"DELETE FROM `%@` WHERE local_id in (%@) OR Id in (%@)",objectName,deleteQueryForObjectTable,deleteQuery];
...
- (void) operation:(INTF_WebServicesDefBindingOperation *)operation completedWithResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    SMLog(kLogLevelVerbose,@"____________  WSInterface OPERATION COMPLETED RESPONSE ____________");
    ...
    if (ret)
    {
        ...
        SMLog(kLogLevelError,@"%@", sFault.faultcode);
        SMLog(kLogLevelError,@"%@", sFault.faultstring);
        ...
        if(custom_sync_status == CUSTOM_SYNC_INITIATED)
        {
            ...
            NSArray * error_conflict =[[NSArray alloc] initWithObjects:cw_local_id,cw_sf_id,
                                       cw_operation_type, cw_record_type, cw_error_mesg,
                                       cw_error_type, cw_object_name,cw_custom_error_type,nil];
            
            NSArray *conflictValues =  [[NSArray alloc] initWithObjects:custom_sync_object_name, @"", @"", @"",
                                        soap_fault, @"ERROR",custom_sync_object_name,CUSTOM_SYNC_SOAP_FAULT, nil];
            
            NSDictionary * dict = [[NSDictionary alloc] initWithObjects:conflictValues forKeys:error_conflict];
            
            NSMutableArray * conflict_array = [[NSMutableArray alloc] initWithCapacity:0];
            [conflict_array addObject:dict];
            
            [appDelegate.databaseInterface insertCustomWebserviceResponsewithError:conflict_array
                                                                        class_name:cus_class_name
                                                                       method_name:cus_method_name
                                                              related_record_error:TRUE
                                                                        request_id:cus_sync_req_id];

            [soap_fault release];
            [error_conflict release];
            error_conflict = nil;
            [conflictValues release];
            conflictValues = nil;
            [dict release];
             dict = nil;
            [conflict_array release];
            conflict_array = nil;
        ...
        if (!tagsDictionary)
        {
            ...
            SMLog(kLogLevelVerbose,@"%@", tagsDictionary);
        }
        ...
        if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
        {
            if ([appDelegate.syncThread isExecuting])
                SMLog(kLogLevelVerbose,@"Data Sync");
            else
        ...
        myException = [NSException exceptionWithName:[NSString stringWithFormat:@"%@",sFault.faultcode]
    ...
    if ([response.error isKindOfClass:[NSURLErrorDomain class]])
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:kInternetConnectionChanged
...
//		[appDelegate setCurrentSyncStatusProgress:CUSTOMSYNC_GETDATA optimizedSynstate:0];
        ...
        if (([wsResponse.result.eventName isEqualToString:@"PROCESS"] || [wsResponse.result.eventName isEqualToString:@"IGNORE"]) && [wsResponse.result.eventType isEqualToString:SYNC])
        {
            ...
            {
                INTF_WebServicesDefServiceSvc_SVMXMap * SVMXDEV_map = [valueMap objectAtIndex:i];
                //level 1 MASTER/DETAIL
                NSString * record_type = SVMXDEV_map.key;
                NSString * object_name = SVMXDEV_map.value;
                ...
                NSArray * operation_SVMXDEV_map = SVMXDEV_map.valueMap;
                for(int j = 0; j < [operation_SVMXDEV_map count]; j++)
                {
                    //level 2 UPDATE/INSERT/DELETE
                    INTF_WebServicesDefServiceSvc_SVMXMap * each_operation = [operation_SVMXDEV_map objectAtIndex:j];
...
//            [record_dict release];
        ...
        SMLog(kLogLevelVerbose,@"This is INTF_WebServicesDefBinding_INTF_SyncOverRide_WS");
    }
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_MetaSync_WS class]])
    {
        ...
        {
            didOpSFMSearchComplete = TRUE;
           // SMLog(kLogLevelVerbose,@" MetaSync SFM_SEARCH received, processing starts: %@", [NSDate date]);
            ...
            @autoreleasepool
            {
                for (int  i = 0; i < [array count]; i+=2)
                {
                    ...
                    for(int k=0; k<[objectInfo count]; k++) //4 objects
                    {
                        ...
                        for(int l=0; l< [detailsArray count]; l+=2)// 2 maps // object map, config data
                        {
                            ...
                            for(int m=0; m<[objectInfoArray count]; m++)
                            {
                                ...
                                //11246 As per the response advance expression will be prehandled for escaping single quotes from server by prefixing \, which is and error in SQLite
                                if([objectInfo.key isEqualToString:@"SVMXDEV__Advance_Expression__c"]) {
                                    NSString *value = [objectInfo.value stringByReplacingOccurrencesOfString:@"\\'" withString:@"''"];
                                    [objectInfoDict setObject:value forKey:objectInfo.key];
                                }
                                else
                                {
                            ...
                            // SMLog(kLogLevelVerbose,@"Count = %d",[configMapArray count]);
                    ...
                    //   SMLog(kLogLevelVerbose,@"Object Data = %@",sfmObjectData);
                    [processInfoDict setObject:sfmObjectData forKey:@"Objects"];
                    [sfmProcessData addObject:processInfoDict];
                    [sfmObjectData release];
                    [processInfoDict release];
                }
                //   SMLog(kLogLevelVerbose,@" MetaSync SFM_SEARCH processing End: %@", [NSDate date]);
           ...
           // SMLog(kLogLevelVerbose,@"SFM Search Configuration = %@",sfmProcessData);
...
//            NSMutableDictionary *finalDict = [NSMutableDictionary dictionary];
        ...
        {            
            //SMLog(kLogLevelVerbose,@"  MetaSync SFM_METADATA received, processing starts: %@", [NSDate date]);
            ...
            else
            {
                ...
                @autoreleasepool
                {
                  [appDelegate.dataBase insertValuesToProcessTable:processDictionary];
                }
                
                NSMutableArray * pageId = [self getAllPageLauoutId];
                self.processDictionary = nil;
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_PAGEDATA])    
        {
            ...
            if(pageUiHistory ==nil)
                pageUiHistory = [[NSMutableArray alloc] init];
                ...
                for (int i = 0; i < [pageArray count]; i++)
                {
                    ...
                    for ( int j = 0; j < [details count]; j++)
                    {
                        ...
                        for (int k = 0; k < [pageDetail.fields count]; k++)
                        {
                            ...
                            /*Aparna: FORMFILL -> Added 'SVMXDEV__Field_Mapping__c'*/
                            /*Krishna : CONTEXTLOOKUP -> Added 'SVMXDEV__Context_Source_Object__c' */
                            ...
                            NSMutableArray * detailFieldObjects = [NSMutableArray arrayWithObjects:
                                                                   (detailUiField.fieldDetail.SVMXDEV__Field_API_Name__c != nil)?detailUiField.fieldDetail.SVMXDEV__Field_API_Name__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Display_Column__c != nil)?detailUiField.fieldDetail.SVMXDEV__Display_Column__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Display_Row__c != nil)?detailUiField.fieldDetail.SVMXDEV__Display_Row__c:@"",
                                                                   [NSNumber numberWithBool:detailUiField.fieldDetail.SVMXDEV__Readonly__c.boolValue],
                                                                   [NSNumber numberWithBool:detailUiField.fieldDetail.SVMXDEV__Required__c.boolValue],
                                                                   (detailUiField.fieldDetail.SVMXDEV__Lookup_Context__c != nil)?detailUiField.fieldDetail.SVMXDEV__Lookup_Context__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Lookup_Query_Field__c != nil)?detailUiField.fieldDetail.SVMXDEV__Lookup_Query_Field__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Sequence__c != nil)?detailUiField.fieldDetail.SVMXDEV__Sequence__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Named_Search__c != nil)?detailUiField.fieldDetail.SVMXDEV__Named_Search__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Related_Object_Name__c != nil)?detailUiField.fieldDetail.SVMXDEV__Related_Object_Name__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__DataType__c != nil)?detailUiField.fieldDetail.SVMXDEV__DataType__c:@"",
                                                                   @"",
                                                                   @"",
                                                                   @"",
                                                                   [NSNumber numberWithInt:detailUiField.fieldDetail.SVMXDEV__Override_Related_Lookup__c.boolValue],
                                                                   (detailUiField.fieldDetail.SVMXDEV__Field_Mapping__c != nil)?detailUiField.fieldDetail.SVMXDEV__Field_Mapping__c:@"",
                                                                   (detailUiField.fieldDetail.SVMXDEV__Context_Source_Object__c != nil)?detailUiField.fieldDetail.SVMXDEV__Context_Source_Object__c:@"",
                                                                   nil];
                        ...
                        NSString * detail_header_info = pageDetail.DetailLayout.SVMXDEV__Header_Reference_Field__c;
                        NSString * detail_object_name = pageDetail.DetailLayout.SVMXDEV__Object_Name__c;
                        NSString * detail_object_alias_name = pageDetail.DetailLayout.SVMXDEV__Name__c;
                        NSString * detail_multi_add_config = pageDetail.DetailLayout.SVMXDEV__Multi_Add_Configuration__c;
                        NSString * detail_multi_add_search_field = pageDetail.DetailLayout.SVMXDEV__Multi_Add_Search_Field__c;
                        NSString * detail_mutlti_add_search_object = pageDetail.DetailLayout.SVMXDEV__Multi_Add_Search_Object__c;
                        NSString * detail_page_layout_id = pageDetail.DetailLayout.SVMXDEV__Page_Layout_ID__c;
                        ...
                        NSMutableArray * detailObjects = [NSMutableArray arrayWithObjects:
                                                          ...
                                                          [NSNumber numberWithBool:pageDetail.DetailLayout.SVMXDEV__Allow_New_Lines__c.boolValue],
                                                          [NSNumber numberWithBool:pageDetail.DetailLayout.SVMXDEV__Allow_Delete_Lines__c.boolValue],
                                                          (pageDetail.noOfColumns != nil)?pageDetail.noOfColumns:@"",
                                                          (pageDetail.DetailLayout.SVMXDEV__Name__c != nil)?pageDetail.DetailLayout.SVMXDEV__Name__c:@"",
                                                          detailValuesId,
                                                          (detail_header_info != nil)?detail_header_info:@"",
                                                          (detail_object_name != nil)?detail_object_name:@"",
                                                          (pageDetail.DetailLayout.SVMXDEV__Sequence__c != nil) ? pageDetail.DetailLayout.SVMXDEV__Sequence__c:@"",
                                                          ...
                                                          nil];
                        [detailsValuesArray release];
                        [detailValuesId release];
                        [detail_deleted_rec release];
                        NSMutableDictionary * detailsDataDict = [NSMutableDictionary dictionaryWithObjects:detailObjects forKeys:detailKeys];
                    ...
                    for ( INTF_WebServicesDefServiceSvc_INTF_UIButton * button in header.buttons)
                    {
                        ...
                        {
                            INTF_WebServicesDefServiceSvc_SVMXDEV__SFM_Event__c * bEvent = [button.buttonEvents objectAtIndex:be];
                            ...
                            NSMutableArray * beObjects = [NSMutableArray arrayWithObjects:
                                                          (bEvent.SVMXDEV__Target_Call__c != nil)?bEvent.SVMXDEV__Target_Call__c:@"",
                                                          (bEvent.SVMXDEV__Event_Call_Type__c != nil)?bEvent.SVMXDEV__Event_Call_Type__c:@"",
                                                          (bEvent.SVMXDEV__Event_Type__c != nil)?bEvent.SVMXDEV__Event_Type__c:@"",
                                                          nil];
                            NSMutableDictionary * dict = [NSMutableDictionary dictionaryWithObjects:beObjects forKeys:beKeys];
                        ...
                        NSMutableArray * buttonObjects = [NSMutableArray arrayWithObjects:
                                                          (button.buttonDetail.SVMXDEV__Title__c != nil)?button.buttonDetail.SVMXDEV__Title__c:@"",
                                                          (buttonEventsArray != nil)?buttonEventsArray:[NSMutableArray array],
                                                          (button.enable != nil)?[NSNumber numberWithBool:button.enable.boolValue]:[NSNumber numberWithInt:1], nil];
                        [buttonEventsArray release];
                        ...
                        // SMLog(kLogLevelVerbose,@"buttons");
                    }
                    ...
                    for (int i = 0; i < [header.sections count]; i++)
                    {
                        ...
                        for (int j = 0; j < [fields count]; j++)
                        {
                            ...
                            NSMutableArray * hdrSectionFieldValues = [NSMutableArray arrayWithObjects:
                                                                      ...
                                                                      (uiField.fieldDetail.SVMXDEV__Field_API_Name__c != nil)?uiField.fieldDetail.SVMXDEV__Field_API_Name__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Display_Column__c != nil)?uiField.fieldDetail.SVMXDEV__Display_Column__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Display_Row__c != nil)?uiField.fieldDetail.SVMXDEV__Display_Row__c:@"",
                                                                      [NSNumber numberWithBool:uiField.fieldDetail.SVMXDEV__Readonly__c.boolValue],
                                                                      [NSNumber numberWithBool:uiField.fieldDetail.SVMXDEV__Required__c.boolValue],
                                                                      (uiField.fieldDetail.SVMXDEV__Lookup_Context__c != nil)?uiField.fieldDetail.SVMXDEV__Lookup_Context__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Lookup_Query_Field__c != nil)?uiField.fieldDetail.SVMXDEV__Lookup_Query_Field__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Sequence__c != nil)?uiField.fieldDetail.SVMXDEV__Sequence__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Named_Search__c != nil) ? uiField.fieldDetail.SVMXDEV__Named_Search__c :@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Related_Object_Name__c != nil)?uiField.fieldDetail.SVMXDEV__Related_Object_Name__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__DataType__c != nil)?uiField.fieldDetail.SVMXDEV__DataType__c:@"",
                                                                      @"",
                                                                      @"",
                                                                      @"",
                                                                      [NSNumber numberWithBool:uiField.fieldDetail.SVMXDEV__Use_For_SLA_Clock__c.boolValue],
                                                                      [NSNumber numberWithInt:uiField.fieldDetail.SVMXDEV__Override_Related_Lookup__c.boolValue],
                                                                      (uiField.fieldDetail.SVMXDEV__Field_Mapping__c != nil)?uiField.fieldDetail.SVMXDEV__Field_Mapping__c:@"",
                                                                      (uiField.fieldDetail.SVMXDEV__Context_Source_Object__c != nil)?uiField.fieldDetail.SVMXDEV__Context_Source_Object__c:@"",
                        ...
                        NSMutableArray * hdrSectionValues = [NSMutableArray arrayWithObjects:
                                                             (section.sectionDetail.SVMXDEV__No_Of_Columns__c != nil)?section.sectionDetail.SVMXDEV__No_Of_Columns__c:@"",
                                                             (section.sectionDetail.SVMXDEV__Title__c != nil)?section.sectionDetail.SVMXDEV__Title__c:@"",
                                                             (section.sectionDetail.SVMXDEV__Sequence__c != nil)?section.sectionDetail.SVMXDEV__Sequence__c:@"",
                                                             (hdrSectionFields != nil)?hdrSectionFields:[NSMutableArray array],
                                                             [NSNumber numberWithBool:section.sectionDetail.SVMXDEV__Use_For_SLA_Clock__c.boolValue],
                                                             nil];
                        ...
                        [hdrSectionFields release];
                        hdrSectionFields  = nil;
                    }
                    ...
                    {
                        INTF_WebServicesDefServiceSvc_SVMXDEV__SFM_Event__c * eventDetail = [pageLevelEvents objectAtIndex:i];
                        ...
                        [eventsDictionary setObject:((eventDetail.SVMXDEV__Event_Type__c != nil)?eventDetail.SVMXDEV__Event_Type__c:@"") forKey:gEVENT_TYPE];
                        [eventsDictionary setObject:((eventDetail.SVMXDEV__Target_Call__c != nil)?eventDetail.SVMXDEV__Target_Call__c:@"") forKey:gEVENT_TARGET_CALL];
                        [eventsDictionary setObject:((eventDetail.SVMXDEV__Event_Id__c != nil)?eventDetail.SVMXDEV__Event_Id__c:@"") forKey:gEVENT_ID];
                        [eventsDictionary setObject:((eventDetail.SVMXDEV__Page_Layout__c != nil)?eventDetail.SVMXDEV__Page_Layout__c:@"") forKey:gEVENT_LAYOUT_ID];
                    ...
                    NSMutableArray * hdrLayoutKeys = [NSMutableArray arrayWithObjects:
                                                      ...
                                                      gHEADER_ID,
                                                      gSVMXDEV__Resolution_Customer_By__c,
                                                      gSVMXDEV__Restoration_Customer_By__c,
                    ...
                    NSString * resolution = [hdrData objectForKey:gSVMXDEV__Resolution_Customer_By__c];
                    if ((resolution == nil) || [resolution isKindOfClass:[NSNull class]])
                        resolution = @"";
                    NSString * restoration = [hdrData objectForKey:gSVMXDEV__Restoration_Customer_By__c];
                    if ((restoration == nil) || [restoration isKindOfClass:[NSNull class]])
                        restoration = @"";
                    
                    NSMutableArray * hdrLayoutObjects = [NSMutableArray arrayWithObjects:
                                                         (header.headerLayout.SVMXDEV__Page_Layout_ID__c != nil)?header.headerLayout.SVMXDEV__Page_Layout_ID__c:@"",
                                                         (header.headerLayout.SVMXDEV__Name__c != nil)?header.headerLayout.SVMXDEV__Name__c
                                                                                        :@"",
                                                         (header.headerLayout.SVMXDEV__Object_Name__c != nil)?header.headerLayout.SVMXDEV__Object_Name__c:@"",
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Allow_New_Lines__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Allow_Delete_Lines__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__IsStandard__c.boolValue],
                                                         (header.headerLayout.SVMXDEV__Action_On_Zero_Lines__c != nil)?header.headerLayout.SVMXDEV__Action_On_Zero_Lines__c:@"",
                                                         hdrSections,
                                                         (hdrButtons != nil)?hdrButtons:[NSMutableArray array],
                                                         ...
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Enable_Attachments__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Enable_Chatter__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Enable_Troubleshooting__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Enable_Service_Report_View__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Enable_Service_Report_Generation__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Show_All_Sections_By_Default__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Show_Product_History__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Show_Account_History__c.boolValue],
                                                         @"",
                                                         @"",
                                                         resolution,
                                                         restoration,
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Hide_Quick_Save__c.boolValue],
                                                         [NSNumber numberWithBool:header.headerLayout.SVMXDEV__Hide_Save__c.boolValue],
                                                         hdrData,
                                                         sfmPageEvents,
                                                         nil];
                    [hdrSections release];
                    [sfmPageEvents release];
            ...
            else
            {
                ...
                @autoreleasepool
                {
                    [appDelegate.dataBase insertPageUIDetails:self.pageUiHistory];
                }
                
                self.pageUiHistory =nil;

                [self metaSyncWithEventName:SFM_OBJECT_DEFINITIONS eventType:SYNC values:self.pageLayoutIDs];
                self.pageLayoutIDs = nil;
            }
        }
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_PICKLIST_DEFINITIONS])
        {
            ...
            //SMLog(kLogLevelVerbose,@"  MetaSync SFM_PICKLIST_DEFINITIONS received, processing starts: %@", [NSDate date]);
            didGetPicklistValues = TRUE;
            NSMutableArray * arr;
            ...
            NSMutableArray * array = [wsResponse.result valueMap];
            if(self.picklistObject == nil)
                picklistObject = [[NSMutableArray alloc] init];
            if(self.picklistField == nil)
                picklistField = [[NSMutableArray alloc] init];
            if(self.picklistValues == nil)
                picklistValues = [[NSMutableArray alloc] init];
            ...
            @autoreleasepool
            {
                for (int i = 0; i < [array count]; i++)
                {
                    ...
                    [self.picklistObject addObject:dict];
                    ...
                    [self.picklistValues addObject:picklistDict];
                    
                    if (array1)
                        [array1 release];
                    arr = [NSMutableArray arrayWithArray:Fields];
                    picklistDict = [NSMutableDictionary dictionaryWithObject:arr forKey:objectkey];
                    [self.picklistField addObject:picklistDict];
                    if ([Fields count] > 0)
            ...
            else
            {
                ...
                @autoreleasepool
                {
                    [appDelegate.dataBase insertvaluesToPicklist:self.picklistObject
                                                          fields:self.picklistField
                                                           value:self.picklistValues];
                    
                    self.picklistObject = nil;
                    self.picklistField = nil;
                    self.picklistValues = nil;
                }
                ...
                //SMLog(kLogLevelVerbose,@"  MetaSync SFM_PICKLIST_DEFINITIONS received, processing ends: %@", [NSDate date]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_OBJECT_DEFINITIONS])
        {
           ...
           // SMLog(kLogLevelVerbose,@"  MetaSync SFM_OBJECT_DEFINITIONS received, processing starts: %@", [NSDate date]);
            int m = 0;
            NSMutableArray * arr = [[NSMutableArray alloc] initWithCapacity:0];
            ...
            @autoreleasepool
            {
                for (int i = 0; i < [array count]; i++)
                {
                        ...
                        for (int k = 0; k < [valueMapArray count]; k++)
                        {
                            ...
                            for (int g = 0 ; g < [mapValuesArray count]; g++)
                            {
                                ...
                                if ([detailArray count] > 0)
                                {
                                    ...
                                    [detailkey release]; detailkey = nil;
                                    [detailValue release]; detailValue = nil;
                    ...
                    [self.objectDefinitions insertObject:objectDict atIndex:m++];
                    if (arr)
                        [arr removeAllObjects];
                }
            ...
            NSMutableArray * objects = [NSMutableArray  arrayWithObjects:@"Task", @"Event",@"Attachment", @"User",nil];
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_BATCH_OBJECT_DEFINITIONS])
        {
            ...
            //SMLog(kLogLevelVerbose,@"  MetaSync SFM_BATCH_OBJECT_DEFINITIONS received, processing starts: %@", [NSDate date]);
            NSMutableArray * arr = [[NSMutableArray alloc] initWithCapacity:0];
            NSMutableArray * object_array = [[NSMutableArray alloc] initWithCapacity:0];
            NSMutableArray * array1;
            ...
            @autoreleasepool
            {
                for (int i = 0; i < [array count]; i++)
                {
                    ...
                    array1 = [NSMutableArray arrayWithArray:arr];
                    objectDict = [NSMutableDictionary dictionaryWithObject:array1 forKey:key]; 
                    [self.objectDefinitions addObject:objectDict];
                    if (arr)
                        [arr removeAllObjects];
                }
            }
            ...
            else 
            {
                ...
                @autoreleasepool
                {
                    [appDelegate.dataBase insertValuesInToOBjDefTableWithObject:self.object
                                                                     definition:self.objectDefinitions];
                    self.objectDefinitions = nil;
                }
                
	            //sahana Aug 16th
    	        [appDelegate.dataBase getRecordTypeValuesForObject:pickListObj];
                
                // SMLog(kLogLevelVerbose,@"  MetaSync SFM_OBJECT_DEFINITIONS received, processing ends: %@", [NSDate date]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFW_METADATA])
        {
           ...
           // SMLog(kLogLevelVerbose,@"  MetaSync SFW_METADATA received, processing starts: %@", [NSDate date]);
      ...
      else if ([wsResponse.result.eventName isEqualToString:MOBILE_DEVICE_TAGS])
      {
         ...
         // SMLog(kLogLevelVerbose,@"  MetaSync MOBILE_DEVICE_TAGS processing starts: %@", [NSDate date]);
        ...
        else if([wsResponse.result.eventName isEqualToString:@"CODE_SNIPPET"])
        {
            ...
            @autoreleasepool
            {
                ...
                for (int i = 0; i < [valueMaps count]; i++)
                {
                    ...
                    SMLog(kLogLevelVerbose,@"%@",code_snippet);
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_DataSync_WS class]])
    {
        ...
        if([wsResponse.result.eventType isEqualToString:SYNC])
        {
            ...
            {
                NSArray * array = [wsResponse.result valueMap];
                BOOL iSDCResponseType = FALSE; //Get price performance
                WSResponseParser *obj = [[WSResponseParser classForEventName:eventName
                                                                   eventType:SYNC] retain];
                obj.dataBase = appDelegate.dataBase;
                obj.dataBaseInterface = appDelegate.databaseInterface;
                ...
                if(callBack)
                {
                    ...
                    else if([lastIndex isEqualToString:@"2"])
                    {
                        ...
                        [data insertObject:activityType atIndex:0];
                        //Get price performance
                        if ([obj responseTypeDCForLastIndexTwo]) {
                    ...
                    {
                        NSString * lastId = [obj getRequiredData:@"LAST_ID"];
                        
                        if (lastId != nil && [lastId length] > 0)
                        {
                            [data insertObject:lastId atIndex:0];
                        }
                    }
                    ...
                    [appDelegate.wsInterface dataSyncWithEventName:GET_PRICE_DATA
                                                         ...
                                                         requestId:requestId
                                                          withData:data
                                                         lastIndex:lastIndex
                                                        responseTypeForLastIndexTwo:iSDCResponseType]; //Get price performance
                    [data release];
                    [requestId release];
                    // Mem_leak_fix - Vipindas 9493 Jan 18
                }
            ...
            else if([eventName isEqualToString:@"ADV_DOWNLOAD_CRITERIA"])
            {
                ...
                if(callBack && [remainingObjects count]>0)
                {
                    ...
                    [appDelegate.wsInterface dataSyncWithEventName:@"ADV_DOWNLOAD_CRITERIA"
                                                        ...
                                                        responseTypeForLastIndexTwo:FALSE]; //Get price performance
                    [requestId release];
                }
                else
                {
		...
		if ([wsResponse.result.eventName isEqualToString:@"GET_INSERT_DC_OPTIMZED"] || [wsResponse.result.eventName isEqualToString:@"GET_UPDATE_DC_OPTIMZED"] || [wsResponse.result.eventName isEqualToString:@"GET_DELETE_DC_OPTIMZED"] || [wsResponse.result.eventName isEqualToString:@"ONE_CALL_SYNC"])
		{
			...
			SMLog(kLogLevelVerbose,@"************************* END Parsing - Once sync Call **********************************");
		}
		//Changes for optimized sync - one sync call
        ...
        {
            appDelegate.dataBase.isResponseWithIds = NO;
            appDelegate.dataBase.didSubmitHTML = (int)[wsResponse.result.success boolValue];
            if(appDelegate.dataBase.didSubmitHTML) {
                //2 because everytime two objects should be an array (signature and html)
                if([wsResponse.result.valueMap count] == 3) {
                    
                    appDelegate.dataBase.isResponseWithIds = YES;
                    for (INTF_WebServicesDefServiceSvc_SVMXMap *mapObject in wsResponse.result.valueMap) {
                        if ([mapObject.key isEqualToString:@"HTMLID"]) {
                            appDelegate.dataBase.requiredHtmlArray = [mapObject values];
                        }
                        else if([mapObject.key isEqualToString:@"SIGNATURE"]) {
                            appDelegate.dataBase.requiredSignatureArray = [mapObject values];
                        }
                        else if([mapObject.key isEqualToString:@"DELETE_ID"]) {
                            appDelegate.dataBase.signaturesNotInServer = [mapObject values];
                        }
                    }
                }
            }
        ...
        if([wsResponse.result.eventName isEqualToString:GENERATE_PDF] && [wsResponse.result.eventType isEqualToString:SYNC])
        {
            ...
            if(appDelegate.dataBase.didGeneratePDF) {
                if([wsResponse.result.valueMap count] == 2) {
                    appDelegate.dataBase.isResponseWithIds = YES;
                    for (INTF_WebServicesDefServiceSvc_SVMXMap *mapObject in wsResponse.result.valueMap) {
                        if ([mapObject.key isEqualToString:@"HTMLID"]) {
                            appDelegate.dataBase.requiredHtmlArray = [mapObject values];
        ...
        {
            [appDelegate.dataBase setDidUserGPSLocationUpdated:YES];
            SMLog(kLogLevelVerbose,@"Sent GPS Logs to Server");
            ...
            )
        {
            ...
            didOpComplete = YES;         
            SMLog(kLogLevelVerbose,@"SFM Search Results Got");
            ...
            @autoreleasepool
            {
                NSArray *fieldsForTableHeader = [NSArray array];
                
                SMLog(kLogLevelVerbose,@"%@",tableField);
                ...
                SMLog(kLogLevelVerbose,@"%@",filedForObjectID);
                NSMutableArray * array = [wsResponse.result valueMap];
                ...
                SMLog(kLogLevelVerbose,@"Results = %@",resultsArray);
               ...
               SBJsonParser *parser = [[SBJsonParser alloc] init];
                ...
                for(int counter =0;counter < [array count];counter++){
                    ...
                    NSString * key = (svmxMap.key != nil)?(svmxMap.key):@"";
                    SMLog(kLogLevelVerbose,@"%@",key);
                    if ([key isEqualToString:@"SVMXDEV__Site__c"]) {
                        ...
                        if (jsonValue != nil && jsonValue.length > 0) {
                            ...
                            if ([objectsArray count] > 0) {
                                ...
                                SMLog(kLogLevelVerbose,@"Name is %@",siteName);
                                ...
                                SMLog(kLogLevelVerbose,@"Id is %@",siteIdentifier);
        ...
        } else if ([wsResponse.result.eventName isEqualToString:EVENT_SYNC] || [wsResponse.result.eventName isEqualToString:DOWNLOAD_CREITERIA_SYNC])
        {
            ...
            SMLog(kLogLevelVerbose,@"  Incremental DataSync response recieved: %@", [NSDate date]);
           ...
           // NSString * event_name = wsResponse.result.eventName;
            ...
            SMLog(kLogLevelVerbose,@"  Incremental DataSync response parsing starts: %@", [NSDate date]);
            for (int i = 0; i < [array count]; i++)
            {
                INTF_WebServicesDefServiceSvc_SVMXMap * svmxMap = [array objectAtIndex:i];
                NSString * key = (svmxMap.key != nil)?(svmxMap.key):@"";
                ...
                else if ([key isEqualToString:@"Child_Object"] || [key isEqualToString:@"Parent_Object"])
                {
                    ...
                    for (int j = 0; j < [valueMap count]; j++)
                    {
                        ...
                        for (int k = 0; k < [values count]; k++)
                        {
                            ...
                            if(flag)
                            {
                                ...
                                NSArray * objects = [NSArray arrayWithObjects:@"", @"",sf_id,event_name_temp,record_type, nil];
                    ...
                    SMLog(kLogLevelVerbose,@"valueMap %d",[valueMap retainCount]);
                }
            }
            [keys_ release];
            
            SMLog(kLogLevelVerbose,@"array %d",[array retainCount]);
            
            SMLog(kLogLevelVerbose,@"  Incremental DataSync response parsing starts: %@", [NSDate date]);
            [appDelegate.databaseInterface insertRecordIdsIntosyncRecordHeap:record_dict];
           
            SMLog(kLogLevelVerbose,@"record_dict %d" , [record_dict retainCount]);
            [record_dict release]; 

            if(call_Back)
            {
                SMLog(kLogLevelVerbose,@"NxtCallDC");
            ...
            {
                SMLog(kLogLevelVerbose,@"NxtCallDC1");
                appDelegate.wsInterface.didOpComplete = TRUE;
                SMLog(kLogLevelVerbose,@"IComeOUTHere wsinterface");
            }
        ...
        else if ([wsResponse.result.eventName isEqualToString:@"TX_FETCH"])
        {
            ...
            NSMutableArray * array = [wsResponse.result valueMap];
            //9644
            NSMutableDictionary *safeToDeleteIDs = [[NSMutableDictionary alloc] init];
            // Mem_leak_fix - Vipindas 9493 Jan 18
            @autoreleasepool
            {
                ...
                for (int i = 0; i < [array count]; i++)
                {
                    ...
                    if(safeToDeleteKey && ([safeToDeleteKey caseInsensitiveCompare:@"SAFE_TO_DELETE"] == NSOrderedSame))
                    {
                        for (int j = 0; j < [valueMap count]; j++)
                        {
                            ...
                            NSLog(@"[SAFETODELETE] Object Name = %@ : Record ID : %@",objName,safeToDeleteRecordIDs);
            ...
            if(appDelegate.do_meta_data_sync == ALLOW_META_AND_DATA_SYNC)
            {
                ...
                SMLog(kLogLevelVerbose,@"insertAllRecordsToRespectiveTables - OUTER");
                [appDelegate.databaseInterface insertAllRecordsToRespectiveTables:record_dict andParser:jsonParserForDataSync];
                
                [[PerformanceAnalytics sharedInstance] observePerformanceForContext:@"insertAllRecordsToRespectiveTables - OUTER"
                                                                     andRecordCount:0];
                
                SMLog(kLogLevelVerbose,@"insertAllRecordsToRespectiveTables - OUTER-ended");
            ...
            //SMLog(kLogLevelVerbose,@"Insertion duration at %f and sum  %f",totaltimeInSec,totalTimeTakenForInsertion);
        ...
        {
            SMLog(kLogLevelVerbose,@"  GET_INSERT/PUT_INSERT Processing starts: %@", [NSDate date]);
            NSString * event_name = wsResponse.result.eventName;
            NSMutableDictionary * record_dict = [[NSMutableDictionary alloc] initWithCapacity:0];
        ...
        else if ( [wsResponse.result.eventName isEqualToString:@"PUT_UPDATE"] )//INSERT
        {
            ...
            NSString * event_name = wsResponse.result.eventName;
            NSMutableDictionary * record_dict = [[NSMutableDictionary alloc] initWithCapacity:0];
            NSMutableDictionary * conflict_dict = [[NSMutableDictionary alloc] initWithCapacity:0];
            NSMutableArray * array = [wsResponse.result valueMap];
                  ...
                  //  NSMutableDictionary * dcobjects = [[NSMutableDictionary alloc] initWithCapacity:0];
                    ...
                    for(NSString  * str in dc_objects)
                    {
                        ...
                        if([arr count] <= 2)
                        {
                            NSString * objectName =[arr objectAtIndex:0];
                            NSString * whereclause = [arr objectAtIndex:1];
                            SMLog(kLogLevelVerbose,@"class - %@" ,[dcobjects_incrementalSync class]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:@"DATA_ON_DEMAND"] || [wsResponse.result.eventName isEqualToString:@"GET_DATA"])
        {
                ...
                for(int i = 0 ;i< [array count]; i++)
                {
                    ...
                    for(int j = 0 ; j < [records count]; j++)
                    {
                        ...
                        else if([master_or_detail isEqualToString:@"Child_Record"])
                        {
                            ...
                            SMLog(kLogLevelVerbose,@"%@",master_or_detail);
                            ...
                            for (int counter = 0; counter < [valueMapGpArray count]; counter++) {
                                ...
                                SMLog(kLogLevelVerbose,@"GPTable name is %@",tableName);
                                NSArray *gpTableRecords = pbRelatedObjectMap.valueMap;
                                
                                SMLog(kLogLevelVerbose,@"Table name is %@ and count is %d",tableName,[gpTableRecords count]);
                                ...
                                for (int innerCounter = 0; innerCounter< [gpTableRecords count]; innerCounter++) {
                                    ...
                                    SMLog(kLogLevelVerbose,@"GPJSON is %@",gpJsonRec);
            ...
            SMLog(kLogLevelVerbose,@"record_dict %@",record_dict);
        ...
        SMLog(kLogLevelVerbose,@"DataSync End: %@", [NSDate date]);
    }
    ...
    if([operation isKindOfClass:[INTF_WebServicesDefBinding_SVMX_GetSvmxVersion class]])
    {
        ...
        @autoreleasepool
        {
            ...
            if([result count] > 0)
            {
                ...
                NSString *serverVersion = value.value;
                NSString *data = [serverVersion substringFromIndex:[serverVersion rangeOfString:@"."].location+1];
                for(int i=0;i<(5 -  [data length]);i++)
                {
                    serverVersion  = [serverVersion stringByAppendingString:@"0"];
                }
                appDelegate.SVMX_Version = serverVersion;
            }
            appDelegate.didGetVersion = TRUE;
    ...
    {
        SMLog(kLogLevelVerbose,@"%@",response);
    ...
    if([operation isKindOfClass:[INTF_WebServicesDefBinding_SFM_SaveTargetRecords_WS class]])
    {
        ...
        if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"STANDALONECREATE"])
        {
            ...
            while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
            {
                SMLog(kLogLevelVerbose,@"WSInterface operation in while loop");
                /*if (![appDelegate isInternetConnectionAvailable])
                {
                ...
                SMLog(kLogLevelVerbose,@"SaveResponse");
                if (didGetProcessId)
                {
                    didGetProcessId = FALSE;
                    break;
            ...
            SMLog(kLogLevelVerbose,@"%@", appDelegate.createObjectContext); 
        }
    ...
    if ( [operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_Get_Tags_WS class]] )
    {
        ...
        responseError = 0;
        //   SMLog(kLogLevelVerbose,@"%@", appDelegate.tagsDictionary);
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_Get_View_Layouts_WS class]])
    {
        ...
        SMLog(kLogLevelVerbose,@"%@", viewLayoutsArray);
    ...
    {
        SMLog(kLogLevelVerbose,@"Mapview");
    ...
    {
        SMLog(kLogLevelVerbose,@"Get Events Completed");
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_Get_Account_History_WS class]])
    {
        ...
        SMLog(kLogLevelVerbose,@"%@", accountHistory);
        didGetAccountHistory = YES;
    }
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_Get_Product_History_WS class]])
    {
        ...
        SMLog(kLogLevelVerbose,@"%@", productHistory);
        didGetProductHistory = YES;
    }
    ...
    {        
            ...
            for (int i = 0; i < [detailDataSetArray count]; i++)
            {
                ...
                for(int j= 0 ;j< [pageDataSet count];j++)
                {
                    ...
                    for (int k = 0; k < [bubbleInfo count]; k++)
                    {
                        ...
                        SMLog(kLogLevelVerbose,@"Field API Name = %@",field_api_name);
                        ...
                        {
                            SMLog(kLogLevelVerbose,@" key %@  loopValue %d",key,k);
                            if(key == nil)
                            {
                                recordNO = 99999;
                            }
                        ...
                        SMLog(kLogLevelVerbose,@" key %@  loopValue %@",key,value);
                    ...
                    SMLog(kLogLevelVerbose,@"   NUm  %d" ,recordNO);
                    ...
                    SMLog(kLogLevelVerbose,@" event_Record_id_set :%@", event_Record_id_set);
                            ...
                            if([DictaliasName isEqualToString:aliasName])
                            {
                                ...
                                for(int p = 0; p < [details_record_ids count];p++)
                                {
                                    ...
                                    if([rec_id isEqualToString:@""])
                                    {
                                        ...
                                        SMLog(kLogLevelVerbose,@"  Record_Id --> %d  rec_id --> %d",emptyRecId_count , recordNO);
                                        if (emptyRecId_count == recordNO)
                                        {
                                            ...
                                            SMLog(kLogLevelVerbose,@"%@", bubbleInfoDict);
                                            for(int q = 0 ; q < [allkeys count]; q++)
                                            {
                                                ...
                                                for(int q = 0; q < [detailValuesArray count]; q++)
                                                {
                                                    ...
                                                    SMLog(kLogLevelVerbose,@"%@ %@",bubbleInfoDictKey , api_name);
                                                ...
                                                if(flag)
                                                {
                                                    ...
                                                    if([strmap count] > 0)
                                                    {
                                                        ...
                                                        SMLog(kLogLevelVerbose,@"present  bubbleInfoDictKey %@ key %@ value %@", bubbleInfoDictKey,key,value);
                                                ...
                                                if(!flag)
                                                {
                                                    ...
                                                    if([strmap count] > 0)
                                                    {
                                                        ...
                                                        NSMutableArray * keys = [NSMutableArray arrayWithObjects:
                                                                                 ...
                                                                                 nil];
                                                        SMLog(kLogLevelVerbose,@"bubbleInfoDictKey %@ key %@ value %@", bubbleInfoDictKey,key,value);
                                ...
                                SMLog(kLogLevelVerbose,@"valuearray%@", detail_values_array);
                                SMLog(kLogLevelVerbose,@" record_id ---%@",details_record_ids);
                            }
                            ...
                            if([DictaliasName isEqualToString:aliasName])
                            {
                                ...
                                for(int p = 0; p < [details_record_ids count];p++)
                                {
                                    ...
                                    if((local_id != nil) && (rec_id != nil))
                                    {
                                        ...
                                        SMLog(kLogLevelVerbose,@"  Record_Id --> %@  rec_id --> %@",Record_Id , rec_id);
                                        if([local_id isEqualToString:rec_id])
                                        {
                                            ...
                                            for(int q = 0 ; q < [allkeys count]; q++)
                                            {
                                                ...
                                                for(int q = 0; q < [detailValuesArray count]; q++)
                                                {
                                                    ...
                                                    NSString * api_name = [keyValueDict objectForKey:gVALUE_FIELD_API_NAME];
                                                    //collect all keys from bubbleinfo dict
                                                    SMLog(kLogLevelVerbose,@"Idbubble info dict key %@ api_name %@ ", bubbleInfoDictKey , api_name);
                                                    if([bubbleInfoDictKey isEqualToString:api_name])
                                                    {
                                                        flag = TRUE;
                                                        break;
                                ...
                                if(!record_exist)
                                {
                                    ...
                                    if([detailValuesArray count] >  0)
                                    {
                                        ...
                                        // Mem_leak_fix - Vipindas 9493 Jan 18
                                        NSString * detail_local_id = [[AppDelegate GetUUID] retain];
                                        [detail_values_array addObject:detailValuesArray];
                                        [details_record_ids addObject:detail_local_id];
                                        [detail_local_id release];
                                        // [detail_sobject addObject:@""];
                                    }
                    ...
                    if([id_ length] != 0 && ![id_ isEqualToString:escape_string])
                    {
                        ...
                        for(int p = 0 ; p < [event_Record_id_set count]; p++)
                        {
                            ...
                            if([dict count] != 0)
                            {
                                ...
                                SMLog(kLogLevelVerbose,@"value %@  record id %@", value , id_);
                        ...
                        SMLog(kLogLevelVerbose,@"after deleting each item  %@", details_record_ids);
                    }
                }
                ...
                SMLog(kLogLevelVerbose,@"details record id --------%@",deleted_details_array);
                    ...
                    for(int k = 0 ; k < [detailValuesArray count]; k++)
                    {
                        ...
                        if([api_name isEqualToString:gDETAIL_SEQUENCENO_GETPRICE])
                        {
                            ...
                            if(apiNmae_exist)
                            {
                                if(flag)
                                {
                                    SMLog(kLogLevelVerbose,@"present");
                ...
                SMLog(kLogLevelVerbose,@"local_deletedRecord_array --------%@",local_deletedRecord_array);
                ...
                SMLog(kLogLevelVerbose,@"final  record_id array : %@ ",details_record_ids);
                SMLog(kLogLevelVerbose,@" event_Record_id_set :%@", event_Record_id_set);
        ...
        SMLog(kLogLevelVerbose,@"BEFORE HDRE_DATA %@", header_data);
            ...
            for(NSString * str in allkeys)
            {
                ...
                if(!flag)
                {
                    ...
                    if([strmap count] > 0)
                    {
                        ...
                        SMLog(kLogLevelVerbose,@"BEFOREEVENT %@  AFTEREVENT %@",temp_value,key);
                    }
        ...
        SMLog(kLogLevelVerbose,@"AFTER HDRE_DATA %@", header_data);
    ...
    }@catch (NSException *exp)
    {
        ...
        //Pushpak 10734
        /** Sometimes we get the session invalid id error in soap fault */
        NSString *faultString = nil;
        if ([response.bodyParts isKindOfClass:[SOAPFault class]]) {
            if ([response.bodyParts count]>0) {
                SOAPFault * sFaultString = [response.bodyParts objectAtIndex:0];
                faultString = sFaultString.faultstring;
            }
        }
        if ([exp.reason Contains:@"INVALID_SESSION_ID"] || [faultString Contains:@"INVALID_SESSION_ID"])
        {
            /** To avoid multiple revoke token alerts */
            if (![appDelegate isRemoteAccessRevoked])
            {
                if (![self handleInvalidSessionIdError])
                {
...
#pragma mark Hadle Invalid SessionId Error
/**
 * Pushpak 10734
 */
- (BOOL)handleInvalidSessionIdError
{
    BOOL didWeHandleError = NO;
    /**
     * Called on invalid session id error
     * We are trying to fetch new session id.
     * If we are successful, we trigger again the request that failed.
     * else we set a global flag indicating the remote access is revoked.
     * The user has to logout and login back to continue other activities.
     */
    /** variable to keep track the session status */
    BOOL isSessionValid = FALSE;
    
    /** Our refresh token from Keychain */
    NSString *refreshToken = [SFHFKeychainUtils getValueForIdentifier:KEYCHAIN_SERVICE];
    
    /** Lets try to fetch new access token (session) */
    isSessionValid = [appDelegate.oauthClient refreshAccessToken:refreshToken isInvokeByBackgroundProcess:NO];
    
    /** Handling the valid and invalid session id situation */
    if (isSessionValid)
    {
        /** Yuppy we have new access token */
        /** Currently handling only for data sync */
        if (appDelegate.dataSyncRunning)
        {
            /** Clear flags */
            [self settingFlags];
            /** Time to trigger data sync */
            appDelegate.didDataSyncFailWithInvalidSession = YES;
            didWeHandleError = YES;
            
            /**
             * Check if the restarted data sync failed with session invalid
             * Then break the loop and display the session invalid error
             */
            if (appDelegate.didAutoRestartedDataSyncFailedWithInvalidSession == YES)
            {
                appDelegate.didDataSyncFailWithInvalidSession = NO;
                didWeHandleError = NO;
            }
        }
    }
    ...
    {
        /**
         * We failed to fetch new access token.
         * Lets set global flag so that all other requests are cancelled until logout,login.
         * Lets set the sync status red so that it indicates user error.
         */
        /** Clear flags */
        //[self settingFlags];
...
-(NSMutableArray *)getIdsFromJsonString:(NSString *)jsonstrings
{
    ...
    @autoreleasepool
    {
        ...
        SBJsonParser * jsonParser_ = [[SBJsonParser alloc] init];
...
- (NSMutableArray *) collectPickListObject
{
    ...
    {
        NSDictionary * dict = [self.object objectAtIndex:i];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :collectPickListObject %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :collectPickListObject %@",exp.reason);
    }
    return result;
}
...
{
    // Mem_leak_fix - Vipindas 9493 Jan 18
    NSMutableArray * pageId = [[NSMutableArray alloc] initWithCapacity:0] ;
    @autoreleasepool {
        
        @try{
                NSArray * array = [self.processDictionary objectForKey:@"SFMProcess"];
            ...
            SMLog(kLogLevelVerbose,@"Exception Name WSInterface :getAllPageLauoutId %@",exp.name);
            SMLog(kLogLevelVerbose,@"Exception Reason WSInterface :getAllPageLauoutId %@",exp.reason);
        }
    }
    return [pageId autorelease];
}
...
//    {
    ...
    if( tempObjects.count != 0 )
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getRecordTypeDictForObjects %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getRecordTypeDictForObjects %@",exp.reason);
    }
    
    [appDelegate.dataBase insertValuesInToRTPicklistTableForObject:tempObjects2 Values:recordTypeDict];
    [recordTypeDict release];
    recordTypeDict = nil;
...
- (void) didDescribeSObjectLayoutForObject:(ZKDescribeLayoutResult *)result error:(NSError *)error context:(id)context
{
    ...
    [self getRecordTypeDictForObjects:tempObjects];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getAllProcessId %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getAllProcessId %@",exp.reason);
    }
}
...
- (NSMutableArray *) getTasksFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for (int i = 0; i < [bodyParts count]; i++)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getTasksFromResponse %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getTasksFromResponse %@",exp.reason);
    }
    return _tasks;
}
...
- (NSMutableArray *) getProductHistoryFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    NSMutableArray * keys = [NSMutableArray arrayWithObjects:
                      @"CreatedDate",
                      @"SVMXDEV__Problem_Description__c",
                   //   @"Id",
                   //   @"Name",
                   //   @"OwnerId",
                   //   @"SVMXDEV__Top_Level__c",
                      nil];
    ...
    for (int i = 0; i < [bodyParts count]; i++)
    {
        ...
        {
            INTF_WebServicesDefServiceSvc_SVMXDEV__Service_Order__c * serviceOrder = [historyInfo objectAtIndex:j];
            NSMutableArray * objects = [NSMutableArray arrayWithObjects:
                                        (serviceOrder.CreatedDate != nil)?serviceOrder.CreatedDate:@"",
                                        (serviceOrder.SVMXDEV__Problem_Description__c != nil)?serviceOrder.SVMXDEV__Problem_Description__c:@"",
                                   ...
                                   //     (serviceOrder.SVMXDEV__Top_Level__c != nil)?serviceOrder.SVMXDEV__Top_Level__c:@"",
                                        nil];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getProductHistoryFromResponse %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getProductHistoryFromResponse %@",exp.reason);
    }
...
- (NSMutableArray *) getAccountHistoryFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    {
        INTF_WebServicesDefServiceSvc_SVMXDEV__Service_Order__c * serviceOrder = [historyInfo objectAtIndex:i];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getAccountHistoryFromResponse %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getAccountHistoryFromResponse %@",exp.reason);
...
- (void) describeObjectFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    INTF_WebServicesDefServiceSvc_INTF_NamedSearchInfo * namedSearchInfo = [namedSearch objectAtIndex:0];
    INTF_WebServicesDefServiceSvc_SVMXDEV__ServiceMax_Processes__c * namedSearchHdr = [namedSearchInfo namedSearchHdr];
    NSString * sourceObject = [namedSearchHdr SVMXDEV__Source_Object_Name__c];
...
- (NSMutableDictionary *) getDescribeObjects:(NSMutableDictionary *)bodyParts
{
    ...
    INTF_WebServicesDefServiceSvc_SVMXDEV__Page_Layout__c * headerLayout = nil; 
    ...
    if (headerLayout != nil)
        objectName = headerLayout.SVMXDEV__Object_Name__c;
    else
        objectName = @"";
    ...
    for (int h = 0; h < [hdrSections count]; h++)
    {
        ...
        for (int f = 0; f < [fields count]; f++)
        {
            ...
            INTF_WebServicesDefServiceSvc_SVMXDEV__Page_Layout_Detail__c * fieldDetail = field.fieldDetail;
            if ([fieldDetail.SVMXDEV__DataType__c isEqualToString:@"reference"])
                [describeObjectsArray addObject:fieldDetail.SVMXDEV__Related_Object_Name__c];
        }
    }
    ...
    for (int d = 0; d < [dtl count]; d++)
    {
        ...
        NSString * objName = pageDetail.DetailLayout.SVMXDEV__Object_Name__c;
        BOOL flag = YES;
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getDescribeObjects %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getDescribeObjects %@",exp.reason);
    }
...
- (void) didDescribeSObject:(ZKDescribeSObject *)describeObject error:(NSError *)error context:(id)context
{
    ...
    SMLog(kLogLevelVerbose,@"%@", lookupDetails);
    ...
    if ([lookupCaller respondsToSelector:@selector(setLookupData:)])
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :didDescribeSObject %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :didDescribeSObject %@",exp.reason);
    }
}
...
- (NSMutableDictionary *) GetHeaderSectionForSequenceNumber:(NSInteger)sequence
{
	...
	for (int i=0;i<[sections count];i++)
	{
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :GetHeaderSectionForSequenceNumber %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :GetHeaderSectionForSequenceNumber %@",exp.reason);
...
- (void) getDictionaryFromPageLayout:(NSMutableDictionary *)bodyParts withDescribedObjects:(NSMutableArray *)describeObjects
{
  ...
  @try{
    ...
    for (int itr = 0; itr < [describeObjects count]; itr++)
    {
        ...
        if ([[sObj name] isEqualToString:hdr.headerLayout.SVMXDEV__Object_Name__c])
            break;
    }
    ...
    for (int i = 0; i < [hdr.sections count]; i++)
    {
        ...
        for (int j = 0; j < [fields count]; j++)
        {
            ...
            NSMutableArray * hdrSectionFieldValues = [NSMutableArray arrayWithObjects:
                                               ...
                                               ([[sObj fieldWithName:uiField.fieldDetail.SVMXDEV__Field_API_Name__c] label] != nil)?[[sObj fieldWithName:uiField.fieldDetail.SVMXDEV__Field_API_Name__c] label]:@"",
    ...
    NSMutableArray * hdrLayoutObjects = [NSMutableArray arrayWithObjects:
                                         (hdr.headerLayout.SVMXDEV__Object_Name__c != nil)?hdr.headerLayout.SVMXDEV__Object_Name__c:@"",
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Allow_New_Lines__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Allow_Delete_Lines__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__IsStandard__c.boolValue],
                                         (hdr.headerLayout.SVMXDEV__Action_On_Zero_Lines__c != nil)?hdr.headerLayout.SVMXDEV__Action_On_Zero_Lines__c:@"",
                                         hdrSections,
                                         hdrButtons,
                                         sfmPageEvents,
                                         ...
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Enable_Attachments__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Enable_Chatter__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Enable_Troubleshooting__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Enable_Service_Report_View__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Enable_Service_Report_Generation__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Show_All_Sections_By_Default__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Show_Product_History__c.boolValue],
                                         [NSNumber numberWithBool:hdr.headerLayout.SVMXDEV__Show_Account_History__c.boolValue],
    ...
    for (int d = 0; d < [dtl count]; d++)
    {
        ...
        {
            sObj = [describeObjects objectAtIndex:itr];
            if ([[sObj name] isEqualToString:pageDetail.DetailLayout.SVMXDEV__Object_Name__c])
                break;   
        }
        ...
        for (int f = 0; f < [pageDetail.fields count]; f++)
        {
            ...
            NSMutableArray * detailFieldObjects = [NSMutableArray arrayWithObjects:
                                            (field.fieldDetail.SVMXDEV__Field_API_Name__c != nil)?field.fieldDetail.SVMXDEV__Field_API_Name__c:@"",
                                            (field.fieldDetail.SVMXDEV__Display_Column__c != nil)?field.fieldDetail.SVMXDEV__Display_Column__c:@"",
                                            (field.fieldDetail.SVMXDEV__Display_Row__c != nil)?field.fieldDetail.SVMXDEV__Display_Row__c:@"",
                                            [NSNumber numberWithBool:field.fieldDetail.SVMXDEV__Readonly__c.boolValue],
                                            [NSNumber numberWithBool:field.fieldDetail.SVMXDEV__Required__c.boolValue],
                                            (field.fieldDetail.SVMXDEV__Lookup_Context__c != nil)?field.fieldDetail.SVMXDEV__Lookup_Context__c:@"",
                                            (field.fieldDetail.SVMXDEV__Lookup_Query_Field__c != nil)?field.fieldDetail.SVMXDEV__Lookup_Query_Field__c:@"",
                                            (field.fieldDetail.SVMXDEV__Sequence__c != nil)?field.fieldDetail.SVMXDEV__Sequence__c:@"",
                                            (field.fieldDetail.SVMXDEV__Named_Search__c != nil) ? field.fieldDetail.SVMXDEV__Named_Search__c:@"",
                                            (field.fieldDetail.SVMXDEV__Related_Object_Name__c != nil)?field.fieldDetail.SVMXDEV__Related_Object_Name__c:@"",
                                            (field.fieldDetail.SVMXDEV__DataType__c != nil)?field.fieldDetail.SVMXDEV__DataType__c:@"",
                                            ([[sObj fieldWithName:field.fieldDetail.SVMXDEV__Field_API_Name__c] label] != nil)?[[sObj fieldWithName:field.fieldDetail.SVMXDEV__Field_API_Name__c] label]:@"",
                                            ...
                                            [NSNumber numberWithInt:field.fieldDetail.SVMXDEV__Override_Related_Lookup__c.boolValue],
                                            nil];
    ...
    SMLog(kLogLevelVerbose,@"%@", pageLayout);
    ...
    if ([sfmPageObjectName isEqualToString:@"SVMXDEVX1__SERVICE_ORDER__C"])
    {
        ...
        SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
        [monitor monitorSMMessageWithName:@"[WSInterface getProductAndAccountHistory]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:kWSExecutionDuration];
        ...
        while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
        {
            ...
            SMLog(kLogLevelVerbose,@"WSInterface getDictionaryFromPageLayout in while loop");
    ...
    if ([self.delegate respondsToSelector:@selector(didReceivePageLayout:withDescribeObjects:)])
      ...
      SMLog(kLogLevelError,@"Exception Name WSInterface :getDictionaryFromPageLayout %@",exp.name);
      SMLog(kLogLevelError,@"Exception Reason WSInterface :getDictionaryFromPageLayout %@",exp.reason);
  }
}
...
- (INTF_WebServicesDefServiceSvc_INTF_TargetRecord *) getTargetRecordsFromSFMPage:(NSDictionary *)sfmpage
{
    ...
    for (int i = 0; i < [details count]; i++) //parts, labor, expense for instance
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getTargetRecordsFromSFMPage %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getTargetRecordsFromSFMPage %@",exp.reason);
    }
    return targetRecord;
}
...
- (BOOL)getPriceInformationForWorkOrderId:(NSString *)sfmId {
    ...
    @try {
        ...
        SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
        [monitor monitorSMMessageWithName:@"[WSInterface getPriceInformationForWorkOrderId]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:kWSExecutionDuration];
        ...
        while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
        {
            SMLog(kLogLevelVerbose,@"getPriceInformationForWorkOrderId: Download  PRICE _INFO for %@",sfmId);
          
            if (![appDelegate isInternetConnectionAvailable])
            {
                SMLog(kLogLevelError,@"getPriceInformationForWorkOrderId: Download  PRICE _INFO Failed due to Internet Lost");
                returnValue = NO;
                break;
            }
            if(appDelegate.connection_error)
            {
				//Defect 6774
				[appDelegate checkifConflictExistsForConnectionError];
                SMLog(kLogLevelError,@"getPriceInformationForWorkOrderId: Download  PRICE _INFO Failed due to Connection Error");
                returnValue = NO;
                break;
            }
            ...
            {
                returnValue = YES;
                SMLog(kLogLevelError,@"getPriceInformationForWorkOrderId: Download  PRICE _INFO Completed successfully");
                break;
            }
        }
        [monitor monitorSMMessageWithName:@"[WSInterface getPriceInformationForWorkOrderId]"
...
- (void)sendRequestToGetData:(NSString *)workOrderId {
    ...
    @try{
        ...
        valueMapforObject.key=@"Object_Name";
        valueMapforObject.value=@"SVMXDEV__Service_Order__c";
...
- (void)parseAndStoreTheResponse:(NSArray *)valueMapArray {
    ...
    for(int i = 0 ;i< [valueMapArray count]; i++)
    {
        ...
        if([tagName isEqualToString:@"PRICING_DATA"]){
            ...
            /* For all other tables , insert the data */
            SMLog(kLogLevelVerbose,@"%@",tagName);
...
- (NSMutableDictionary *) getTagsdisplay:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for ( int i = 0; i < [response.bodyParts count]; i++ )
    {
        ...
        if ( ret )
        {
            SMLog(kLogLevelError,@"ERROR: IN THE RESPONSE RECEIVED");
            break;
        }
        else
        {
    ...
    SMLog(kLogLevelVerbose,@"%d", [response.bodyParts count]);
    ...
    SMLog(kLogLevelVerbose,@"%@", _tagsDictionary);
    // Samman
    _tagsDictionary = [self fillEmptyTags:_tagsDictionary];
    }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name WSInterface :getTagsdisplay %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getTagsdisplay %@",exp.reason);
    }
    return _tagsDictionary;
}
...
-(NSMutableDictionary *) fillEmptyTags:(NSMutableDictionary *)_tagsDictionary
{
    ...
    @autoreleasepool
    {
        @try{
            ...
            SMLog(kLogLevelError,@"Exception Name WSInterface :fillEmptyTags %@",exp.name);
            SMLog(kLogLevelError,@"Exception Reason WSInterface :fillEmptyTags %@",exp.reason);
        }
    }
...
- (void)reloadTagsDictionary
{
    ...
    NSMutableDictionary *tagsFromDatabase = [[appDelegate.dataBase getTagsDictionary] retain];
    NSMutableDictionary *refilledTags = [[self fillEmptyTags:tagsFromDatabase] retain];
    self.tagsDictionary = refilledTags;
    [refilledTags release];
    [tagsFromDatabase release];
}
...
{    
    ...
    for ( int i = 0; i < [_eventArray count]; i++ ) 
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getEventdisplay %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getEventdisplay %@",exp.reason);
    }
...
- (NSArray *) getCreateProcessesDictionaryArray:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    {
        INTF_WebServicesDefServiceSvc_SVMXDEV__ServiceMax_Processes__c * create_process = [create_processes objectAtIndex:i];
        ...
        NSArray * createInfoObjects = [NSArray arrayWithObjects:
                                     (create_process.SVMXDEV__Name__c != nil)?create_process.SVMXDEV__Name__c:@"",
                                     (create_process.SVMXDEV__ProcessID__c != nil)?create_process.SVMXDEV__ProcessID__c:@"",
                                     (create_process.SVMXDEV__Description__c != nil) ? create_process.SVMXDEV__Description__c:@"",
                                     (create_process.SVMXDEV__Source_Object_Name__c != nil)? create_process.SVMXDEV__Source_Object_Name__c:@"",
                                     nil];
    ...
    {
        NSDictionary * dict = [array objectAtIndex:i];
        NSString * str = [dict objectForKey:SVMXDEV_OBJECT_NAME];  
    ...
    SMLog(kLogLevelVerbose,@ "appdelegate---objectNames_array %@",objectNames_array);
    ...
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface getCreateProcessesDictionaryArray]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:kWSExecutionDuration];
    ...
    {
        SMLog(kLogLevelVerbose,@"WSInterface getCreateProcessDictionaryArray in while loop");
    ...
    SMLog(kLogLevelVerbose,@"appdelegate---objectNames_array %@",appDelegate.objectLabel_array);
    ...
    appDelegate.objectNames_array = [objectNames_array retain];
     //  SMLog(kLogLevelVerbose,@"viewLayouts = %@", dict);
    SMLog(kLogLevelVerbose,@"viewLayouts= %@", array);
    //SMLog(kLogLevelVerbose,@"%@" , objectNames_array);
    //SMLog(kLogLevelVerbose,@"%@" ,section_for_createObjects);
    SMLog(kLogLevelVerbose,@ "appdelegate--- %@",appDelegate.StandAloneCreateProcess);
    SMLog(kLogLevelVerbose,@"apdelegate-----%@",appDelegate.objectNames_array);
    SMLog(kLogLevelVerbose,@"%@", appDelegate.objectLabel_array);
    ...
    SMLog(kLogLevelVerbose,@"%@", appDelegate.objectLabelName_array);
    ...
    appDelegate.StandAloneCreateProcess = [section_for_createObjects retain];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getCreateProcessesDictionaryArray %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getCreateProcessesDictionaryArray %@",exp.reason);
    }
    return array;
}
...
- (NSArray *) getViewLayoutArray:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for (int i = 0; i < [viewLayoutsInfo count]; i++)
    {
        ...
        {
            INTF_WebServicesDefServiceSvc_SVMXDEV__ServiceMax_Processes__c * viewInfo = [layoutsInfo objectAtIndex:j];
            NSArray * viewInfokeys = [NSArray arrayWithObjects:
                                      VIEW_OBJECTNAME,
                                      VIEW_SVMXDEV_Name, 
                                      VIEW_SVMXDEV_ProcessID, 
                                      nil];
            
            NSArray * viewInfoObjects = [NSArray arrayWithObjects:
                                         (objectName != nil)?objectName:@"",
                                         (viewInfo.SVMXDEV__Name__c != nil)?viewInfo.SVMXDEV__Name__c:@"",
                                         (viewInfo.SVMXDEV__ProcessID__c != nil)?viewInfo.SVMXDEV__ProcessID__c:@"",
                                         nil];
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getViewLayoutArray %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getViewLayoutArray %@",exp.reason);
    }
    SMLog(kLogLevelVerbose,@"%@", array);
    return array;
}
...
-(void)didGetNameFields:(NSMutableArray *) describeObjects error:(NSError *) error context:(id)context;
{
    ...
    didGetObjectName = TRUE;
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :didGetNameFields %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :didGetNameFields %@",exp.reason);
    }
...
-(NSMutableDictionary *) getSaveTargetRecords:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    appDelegate.newRecordId = resultId;
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getSaveTargetRecords %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getSaveTargetRecords %@",exp.reason);
    }
...
- (void) getNameFieldForCreateProcess:(NSString *)ID
{
    ...
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface getNameFieldForCreateProcess]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:kWSExecutionDuration];
    ...
    while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, FALSE))
    {
        ...
        SMLog(kLogLevelVerbose,@"WSInterface getNameFieldForCreateProcess in while loop");
        if (![appDelegate isInternetConnectionAvailable])
        {
            didGetProcessId = TRUE;
    ...
    [monitor monitorSMMessageWithName:@"[WSInterface getNameFieldForCreateProcess]"
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :getNameFieldForCreateProcess %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :getNameFieldForCreateProcess %@",exp.reason);
    }
}
...
- (void) didGetNameField:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int j = 0; j < [appDelegate.wsInterface.viewLayoutsArray count]; j++)
    {
        ...
        {
            [appDelegate.createObjectContext setValue:[viewLayoutDict objectForKey:@"SVMXDEV__ProcessID__c"] forKey:PROCESSID];
            appDelegate.newProcessId = [viewLayoutDict objectForKey:@"SVMXDEV__ProcessID__c"];
            break;
        }
    }
    ...
    didGetNameField = TRUE;
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :didGetNameField %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :didGetNameField %@",exp.reason);
...
- (void) saveDictionaryToPList:(NSMutableDictionary *)dictionary 
{
    ...
    if(plistData)
    {
        ...
        if (count > VALUE) 
        {
            ...
            [array removeObjectAtIndex:count - 1];
        }
...
//        [appDelegate.recentObject addObjectsFromArray:array];
    ...
    {
        SMLog(kLogLevelVerbose,@"%@",error);
        [error release];
    }
    }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name WSInterface :saveDictionaryToPList %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :saveDictionaryToPList %@",exp.reason);
...
- (NSMutableDictionary *) getWorkOrderDetails:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    SMLog(kLogLevelVerbose,@"%@", dictionary);
    didGetWorkOder = TRUE;
    return dictionary;
}
...
- (void)getUserTrunkLocationRequest {
    ...
    if (package <  minVersion) {
        [appDelegate.dataBase removeUserTechnicianLocation];
         SMLog(kLogLevelVerbose,@"Lookup filters is not supported in %@",currentServerPkgVersion);
        return;
    }
    SMLog(kLogLevelVerbose,@"%@",currentServerPkgVersion);
    
    appDelegate.connection_error = NO;
    appDelegate.wsInterface.didOpComplete = NO;
    [Utility setUserTrunkRequestStatus:@"false"];
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[WSInterface getUserTrunkLocationRequest]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:10.0];
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        SMLog(kLogLevelVerbose,@"User trunk request");
        
        if (![appDelegate isInternetConnectionAvailable])
        {
            SMLog(kLogLevelVerbose,@"User trunk request: Failed due to Internet Lost");
        ...
        if(appDelegate.connection_error)
        {
             SMLog(kLogLevelVerbose,@"User trunk request: Failed due to Connection Error");
            
            break;
        }
        NSString *currentStaus =  [Utility getUserTrunkRequestStatus];
        if ([currentStaus isEqualToString:@"true"])
        {   SMLog(kLogLevelVerbose,@"User trunk request Completed successfully");
            break;
        }
    }
    [monitor monitorSMMessageWithName:@"[WSInterface getUserTrunkLocationRequest]"
...
- (BOOL)doCheckSession
{
    ...
    {
        BOOL isSessionValid = FALSE;
        /** Checking whether remote access revoked error is triggered. */
        if ([appDelegate isRemoteAccessRevoked])
        {
        ...
        NSUserDefaults * userdefaults = [NSUserDefaults standardUserDefaults];
        NSDate *prevCheckSessionTimeStamp = [userdefaults objectForKey:kCheckSessionTimeStamp];
        NSDate *currentTimeStamp = [NSDate date];
        
        if(!prevCheckSessionTimeStamp || ([currentTimeStamp timeIntervalSinceDate:prevCheckSessionTimeStamp] > 300))
        {
            NSString *refreshToken = [SFHFKeychainUtils getValueForIdentifier:KEYCHAIN_SERVICE];
            ...
            if (isSessionValid)
            {
                [userdefaults setObject:[NSDate date] forKey:kCheckSessionTimeStamp];
            }
        }
        else
        {
...
// Background calls calling check session
- (BOOL)doCheckSessionForBackgroundCalls
{
    /** Pushpak 10734 */
    @synchronized(self)
    {
        ...
        if(!prevCheckSessionTimeStamp || ([currentTimeStamp timeIntervalSinceDate:prevCheckSessionTimeStamp] > 300))
        {
            ...
            if ( refreshToken && [appDelegate isInternetConnectionAvailable] )
            {
                isSessionValid = [appDelegate.oauthClient refreshAccessToken:refreshToken isInvokeByBackgroundProcess:YES];
            }
            if (isSessionValid)
            {
                [userdefaults setObject:[NSDate date] forKey:kCheckSessionTimeStamp];
...
- (void)sessionDidResume:(ZKLoginResult *)loginResult error:(NSError *)error
{
    ...
    if (error)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WSInterface :sessionDidResume %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WSInterface :sessionDidResume %@",exp.reason);
