...
-(NSString *)getSyncTimeStampWithTheIntervalof15days:(NSString *)time
{
    ...
    @try{
        ...
        NSLog(@"Exception Name WSInterface :getSyncTimeStampWithTheIntervalof15days %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getSyncTimeStampWithTheIntervalof15days %@",exp.reason);
    }
    if([time isEqualToString:START_TIME])
    {
...
@try{
    ...
    [dict writeToFile:plistPath_SYNHIST atomically:YES];
        ...
        NSLog(@"Exception Name WSInterface :setSyncReqId %@",exp.name);
        NSLog(@"Exception Reason WSInterface :setSyncReqId %@",exp.reason);
    }
}
...
-(BOOL)getSyncStatusForRequestId
{
    ...
    {   
        ...
        NSLog(@"Exception Name WSInterface :getSyncStatusForRequestId %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getSyncStatusForRequestId %@",exp.reason);
    }
...
@try{
    ...
    else if([sync_type isEqualToString:GET_UPDATE_DOWNLOAD_CRITERIA] && [operation_type isEqualToString:RESPONSE])
    {
        ...
        NSLog(@"Exception Name WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getAllRecordsForOperationTypeFromSYNCCONFLICT %@",exp.reason);
    }  
    [dict writeToFile:plistPath_SYNHIST atomically:YES];
...
-(void)DoSpecialIncrementalSync
{
    ...
    while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
    {
        ...
        NSLog(@"WSinterface.m : DoSplIncSync: PUT_INSERT");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTINSERT optimizedSynstate:0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"WSinterface.m : DoSplIncSync: PUT_DELETE");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTDELETE optimizedSynstate:0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"WSinterface.m : DoSplIncSync: PUT_UPDATE");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTUPDATE optimizedSynstate:0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"WSinterface.m : DoSplIncSync: TX_FETCH");
#endif
...
//	[appDelegate setCurrentSyncStatusProgress:cSYNC_PUTRECORDS optimizedSynstate:0];
    ...
    {        
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"WSinterface.m : DoSplIncSync: CLEAN_UP");
#endif
...
- (NSString *) getClientVersionString
{
    ...
    else
    {
        ...
        NSLog(@"Exception Name WSInterface :getClientVersionString %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getClientVersionString %@",exp.reason);
    }
    return finalString2;
}
...
-(void) PutAllTheRecordsForIds
{
    ...
    NSLog(@"  TX_FETCH Request Sent: %@", [NSDate date]);
    //[[ZKServerSwitchboard switchboard] doCheckSession];
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :PutAllTheRecordsForIds %@",exp.name);
        NSLog(@"Exception Reason WSInterface :PutAllTheRecordsForIds %@",exp.reason);
    }
...
-(void)DoIncrementalDataSync
{
 ...
 /* Shravya - Advanced look up filters - User trunk location */
	...
	else
	{
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: GET_DELETE");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:GETDELETE_DONE optimizedSynstate:oGETDELETE_DONE];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: GET_DELETE_DWN_CRIT");
			#endif
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: CLEAN_UP_SELECT 1");
			#endif
	...
	//		[appDelegate setCurrentSyncStatusProgress:GETEDELETE_DC_DONE optimizedSynstate:0];
		...
		if([appDelegate.dataSync_dict count] >0)
		{
			...
			{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: PUT_DELETE");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:PUTDELETE_DONE optimizedSynstate:oPUTDELETE_DONE];
		...
		{
			NSLog(@"Adv Download Criteria Failed");
			if ([appDelegate isInternetConnectionAvailable])
			{
				//Defect 6774
				[appDelegate checkifConflictExistsForConnectionError];
		...
		if([appDelegate.dataSync_dict count] >0)
		{
			...
			{
				#ifdef kPrintLogsDuringWebServiceCall
					NSLog(@"WSinterface.m : DOINC_DATA_SYNC: PUT_INSERT");
				#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:PUTINSERT_DONE optimizedSynstate:oPUTINSERT_DONE];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: GET_INSERT");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:GETINSERT_DONE optimizedSynstate:oGETINSERT_DONE];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: GET_INSERT_DWN_CRIT");
			#endif
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: CLEAN_UP_SELECT 2");
			#endif
	...
	//		[appDelegate setCurrentSyncStatusProgress:GETINSERT_DC_DONE optimizedSynstate:0];
		...
		if([appDelegate.dataSync_dict count] >0)
		{
			...
			{
				#ifdef kPrintLogsDuringWebServiceCall
					NSLog(@"WSinterface.m : DOINC_DATA_SYNC: PUT_UPDATE");
				#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:PUTUPDATE_DONE optimizedSynstate:oPUTUPDATE_DONE];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: GET_UPDATE");
			#endif
	...
	//	[appDelegate setCurrentSyncStatusProgress:GETUPDATE_DONE optimizedSynstate:oGETUPDATE_DONE];
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: GET_UPDATE_DWN_CRIT");
			#endif
		...
		{
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: CLN_UP_SELECT 3");
			#endif
...
//        normal sync
    ...
    else
    {
		...
		if (!doOneCallSync)
		{
			...
			{
				#ifdef kPrintLogsDuringWebServiceCall
					NSLog(@"WSinterface.m : DOINC_DATA_SYNC: CLEAN_UP");
				#endif
...
- (BOOL) CustomSingleSyncCall:(NSDate *)syncStarted
{
	...
	{
		#ifdef kPrintLogsDuringWebServiceCall
			NSLog(@"WSinterface.m : DOINC_DATA_SYNC: CustomSingleSyncCall");
		#endif
...
- (BOOL) getAllDataForIds:(NSDate *)syncStarted flag:(BOOL)doOnesync
{
	...
	if (!doOnesync)
	{
        ...
        {
			#ifdef kPrintLogsDuringWebServiceCall
				NSLog(@"WSinterface.m : DOINC_DATA_SYNC: TX_FETCH");
			#endif
...
- (void)submitHTMLDocuments:(NSArray*)docs withSignatures:(NSArray*)signatures
{
    ...
    @catch (NSException *exp)
    {
        NSLog(@"Exception Name WSInterface : Submit_Document %@",exp.name);
        NSLog(@"Exception Reason WSInterface : Submit_Document %@",exp.reason);
    }
}
...
- (void)generatePDFfor:(NSArray*)docs withSignatures:(NSArray*)signatures
{
    ...
    @catch (NSException *exp)
    {
        NSLog(@"Exception Name WSInterface : GENERATE_PDF %@",exp.name);
        NSLog(@"Exception Reason WSInterface : GENERATE_PDF %@",exp.reason);
    }
}
...
- (void)doGetPrice
{
    ...
    {
        NSLog(@"Some Objects Doesn't Have permission. Give the permission and the sync again for Get Price");
        return;
    }
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        NSLog(@"iPadScrollerViewController.m : doGetPrice: Download GetPrice Data Sync");
        if (appDelegate.initial_sync_succes_or_failed == DATA_SYNC_FAILED)
        {
            NSLog(@"GetPrice Failed");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
            break;
        }
        if (![appDelegate isInternetConnectionAvailable])
        {
            NSLog(@"GetPrice Data Sync Failed due to Internet Lost");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
            break;
        }
        if(appDelegate.connection_error)
        {
			...
			[appDelegate checkifConflictExistsForConnectionError];
            appDelegate.connection_error=FALSE;
            NSLog(@"GetPrice Data Sync Failed due to Connection Error");
            return;
        }
        if (appDelegate.Incremental_sync_status == GET_PRICE_DONE)
        {
            NSLog(@"GetPrice Data Sync Completed for Request ID = %@",requestId);
            break;
        }
    }
...
- (BOOL)doAdvanceDownloadCriteria 
{
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        NSLog(@"WSInterface.m :Download Advance Download Criteria ");
        if (appDelegate.initial_sync_succes_or_failed == DATA_SYNC_FAILED)
        {
            NSLog(@"Adv Download Criteria Failed");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
        ...
        if (![appDelegate isInternetConnectionAvailable])
        {
            NSLog(@"Adv Download Criteria Data Sync Failed due to Internet Lost");
            appDelegate.initial_sync_succes_or_failed = DATA_SYNC_FAILED;
            return FALSE;
        }
        if(appDelegate.connection_error)
        {
            NSLog(@"Adv Download Criteria Data Sync Failed due to Connection Error");
            return FALSE;
        }
        if (appDelegate.Incremental_sync_status == SYNC_ADV_DWN_CRT_DONE)
        {
            NSLog(@"Adv Download Criteria Data Sync Completed for Request ID = %@",requestId);
            break;
        }
    }
...
-(void)generateRequestId
{
    ...
    else
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"WSinterface.m : generateRequestId: CLEAN_UP_SELECT");
#endif
...
-(void)getAllRecordsForOperationType:(NSString *)OpearationType
{
    ...
    for(int i = 0 ; i < [object_array count] ; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getAllRecordsForOperationType %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getAllRecordsForOperationType %@",exp.reason);
    }
...
//        [sfmRequest.valueMap addObject:valueMapforObject];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync
        ...
        NSLog(@"Exception Name WSInterface :getOnDemandRecords %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getOnDemandRecords %@",exp.reason);
    }
}
-(void)GetInsert
{
...
-(void)GetUpdate
{
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :GetUpdate %@",exp.name);
        NSLog(@"Exception Reason WSInterface :GetUpdate %@",exp.reason);
    }
...
-(void)GetDelete
{
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :GetDelete %@",exp.name);
        NSLog(@"Exception Reason WSInterface :GetDelete %@",exp.reason);
    }
...
-(void)cleanUpForRequestId:(NSString *)requestId  forEventName:(NSString *)eventName
{
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :cleanUpForRequestId %@",exp.name);
        NSLog(@"Exception Reason WSInterface :cleanUpForRequestId %@",exp.reason);
    }
...
//    [svmxc_client.clientInfo addObject:@"R4B2"];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :GETDownloadCriteriaRecordsFor %@",exp.name);
        NSLog(@"Exception Reason WSInterface :GETDownloadCriteriaRecordsFor %@",exp.reason);
    }
...
@try{
   ...
   // event_name = @"PUT_INSERT";
    if([event_name isEqualToString:@"PUT_UPDATE"] && [event_type isEqualToString:@"SYNC"])//PUT_UPSATE
    {
      ...
      INTF_WebServicesDefServiceSvc_SVMXMap * SVMXCMap_lastModified =  [[[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init]  autorelease];
		...
		//Radha - #defect 8227
		NSLog(@"put_insert_time = %@", put_insert_time);
   ...
   else if([event_name isEqualToString:@"PUT_INSERT"] && [event_type isEqualToString:@"SYNC"])
   {
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :Put %@",exp.name);
        NSLog(@"Exception Reason WSInterface :Put %@",exp.reason);
    }
...
@try{
    ...
    [binding SVMX_GetSvmxVersionAsyncUsingParameters:getVersion
        ...
        NSLog(@"Exception Name WSInterface :getSvmxVersion %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getSvmxVersion %@",exp.reason);
    }
...
//    svmxMap = NULL;
    ...
    [binding INTF_MetaSync_WSAsyncUsingParameters:metaSync 
        ...
        NSLog(@"Exception Name WSInterface :metaSyncWithEventName %@",exp.name);
        NSLog(@"Exception Reason WSInterface :metaSyncWithEventName %@",exp.reason);
    }
                     ...
                     lastIndex:(NSString *)lastIndex
{
    @try{
        ...
        NSLog(@"  Incremental DataSync request looping  starts: %@", [NSDate date]);
        ...
        NSLog(@"  Incremental DataSync request looping  ends: %@", [NSDate date]);
...
//        [self addClientInfoForObject:client];
        ...
        NSLog(@"  Incremental DataSync Request sent: %@", [NSDate date]);
...
- (void) getTags
{
    ...
    [binding INTF_Get_Tags_WSAsyncUsingParameters:getTags
        ...
        NSLog(@"Exception Name WSInterface :getTags %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getTags %@",exp.reason);
    }
}
...
- (void) getCreateProcesses
{
    ...
    [binding INTF_Get_StandaloneCreate_LayoutsAsyncUsingParameters:getCreateProcesses 
        ...
        NSLog(@"Exception Name WSInterface :getCreateProcesses %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getCreateProcesses %@",exp.reason);
    }
}
...
- (void) getViewLayouts
{
    ...
    [binding INTF_Get_View_Layouts_WSAsyncUsingParameters:getViewLayouts
        ...
        NSLog(@"Exception Name WSInterface :getViewLayouts %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getViewLayouts %@",exp.reason);
    }
...
- (void) savePageLayout
{
    ...
    [binding INTF_SavePageLayout_WSAsyncUsingParameters:savePageLayout
        ...
        NSLog(@"Exception Name WSInterface :savePageLayout %@",exp.name);
        NSLog(@"Exception Reason WSInterface :savePageLayout %@",exp.reason);
    }
...
- (void) getUpdateEventsForStartDate:(NSString *)_startDate EndDate:(NSString *)_endDate recordID:(NSString *)_recordID
{
    ...
    [binding INTF_Update_Events_WSAsyncUsingParameters:(INTF_WebServicesDefServiceSvc_INTF_Update_Events_WS *) getUpdateEvents
        ...
        NSLog(@"Exception Name WSInterface :getUpdateEventsForStartDate %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getUpdateEventsForStartDate %@",exp.reason);
    }
}
...
-(void) SaveSFMData:(NSDictionary *)sfmpage
{
    ...
    [binding SFM_SaveTargetRecords_WSAsyncUsingParameters:saveTargetRecords
        ...
        NSLog(@"Exception Name WSInterface :SaveSFMData %@",exp.name);
        NSLog(@"Exception Reason WSInterface :SaveSFMData %@",exp.reason);
    }
}
...
//    webServiceClass = [webServiceName stringByDeletingPathExtension];
    ...
    [binding INTF_PREQ_GetPrice_WSAsyncUsingParameters:getThoonsEvent
        ...
        NSLog(@"Exception Name WSInterface :callCustomSFMAction %@",exp.name);
        NSLog(@"Exception Reason WSInterface :callCustomSFMAction %@",exp.reason);
    }
}
...
{   
    ...
    else if([event_name isEqualToString:BEFORESAVE] || [event_name isEqualToString:AFTERSAVE])
    {
       ...
       // NSLog(@" count %d", [appDelegate.afterSavePageLevelEvents count]);
        ...
        NSLog(@"Exception Name WSInterface :callSFMEvent %@",exp.name);
        NSLog(@"Exception Reason WSInterface :callSFMEvent %@",exp.reason);
    }
}
...
-(void) AddRecordForLines:(NSString*) process_id ForDetailLayoutId:(NSString*) layout_id
{
    ...
    [binding INTF_AddRecords_WSAsyncUsingParameters:AddRecordWS
        ...
        NSLog(@"Exception Name WSInterface :AddRecordForLines %@",exp.name);
        NSLog(@"Exception Reason WSInterface :AddRecordForLines %@",exp.reason);
    }
}
...
-(NSMutableDictionary *)getAddRecordsFields:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    add_WS = TRUE;  
        ...
        NSLog(@"Exception Name WSInterface :getAddRecordsFields %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getAddRecordsFields %@",exp.reason);
    }
    return result_dict;
}
...
//    [svmxc_client.clientInfo addObject:@"R4B2"];
    ...
    [binding INTF_DataSync_WSAsyncUsingParameters:datasync 
        ...
        NSLog(@"Exception Name WSInterface :checkdownloadcriteria %@",exp.name);
        NSLog(@"Exception Reason WSInterface :checkdownloadcriteria %@",exp.reason);
    }
}
...
- (void) operation:(INTF_WebServicesDefBindingOperation *)operation completedWithResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    NSLog(@"____________  WSInterface OPERATION COMPLETED RESPONSE ____________");
...
//            [record_dict release];
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_MetaSync_WS class]])
    {
        ...
        {            
            //NSLog(@"  MetaSync SFM_METADATA received, processing starts: %@", [NSDate date]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_PICKLIST_DEFINITIONS])
        {
            ...
            //NSLog(@"  MetaSync SFM_PICKLIST_DEFINITIONS received, processing starts: %@", [NSDate date]);
            didGetPicklistValues = TRUE;
            NSMutableArray * arr;
            ...
            else
            {
                ...
                //NSLog(@"  MetaSync SFM_PICKLIST_DEFINITIONS received, processing ends: %@", [NSDate date]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_OBJECT_DEFINITIONS])
        {
           ...
           // NSLog(@"  MetaSync SFM_OBJECT_DEFINITIONS received, processing starts: %@", [NSDate date]);
            int m = 0;
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFM_BATCH_OBJECT_DEFINITIONS])
        {
            ...
            //NSLog(@"  MetaSync SFM_BATCH_OBJECT_DEFINITIONS received, processing starts: %@", [NSDate date]);
            NSMutableArray * arr = [[[NSMutableArray alloc] initWithCapacity:0] retain];
            NSMutableArray * object_array = [[[NSMutableArray alloc] initWithCapacity:0] retain];
            NSMutableArray * array1;
            ...
            else 
            {
                ...
                // NSLog(@"  MetaSync SFM_OBJECT_DEFINITIONS received, processing ends: %@", [NSDate date]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:SFW_METADATA])
        {
           ...
           // NSLog(@"  MetaSync SFW_METADATA received, processing starts: %@", [NSDate date]);
      ...
      else if ([wsResponse.result.eventName isEqualToString:MOBILE_DEVICE_TAGS])
      {
         ...
         // NSLog(@"  MetaSync MOBILE_DEVICE_TAGS processing starts: %@", [NSDate date]);
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_DataSync_WS class]])
    {
        ...
        } else if ([wsResponse.result.eventName isEqualToString:EVENT_SYNC] || [wsResponse.result.eventName isEqualToString:DOWNLOAD_CREITERIA_SYNC])
        {
            ...
            NSLog(@"  Incremental DataSync response recieved: %@", [NSDate date]);
           ...
           // NSString * event_name = wsResponse.result.eventName;
            ...
            NSLog(@"  Incremental DataSync response parsing starts: %@", [NSDate date]);
            for (int i = 0; i < [array count]; i++)
            {
                INTF_WebServicesDefServiceSvc_SVMXMap * svmxMap = [array objectAtIndex:i];
                NSString * key = (svmxMap.key != nil)?(svmxMap.key):@"";
        ...
        else if ([wsResponse.result.eventName isEqualToString:@"TX_FETCH"])
        {
            ...
            NSLog(@"  TX_FETCH Response recived: %@", [NSDate date]);
            NSLog(@"  TX_FETCH Processing starts: %@", [NSDate date]);
            NSMutableDictionary * record_dict = [[NSMutableDictionary alloc] initWithCapacity:0];
            NSMutableArray * array = [wsResponse.result valueMap];
            ...
            [keys_ release];
            NSLog(@"  TX_FETCH Processing ends: %@", [NSDate date]);
        ...
        {
            NSLog(@"  GET_INSERT/PUT_INSERT Processing starts: %@", [NSDate date]);
            NSString * event_name = wsResponse.result.eventName;
            NSMutableDictionary * record_dict = [[NSMutableDictionary alloc] initWithCapacity:0];
            ...
            NSLog(@"  GET_INSERT/PUT_INSERT Processing ends: %@", [NSDate date]);
            [appDelegate.databaseInterface insertRecordIdsIntosyncRecordHeap:record_dict];
                    ...
                    else if([master_or_detail isEqualToString:@"Child_Record"])
                    {
                       ...
                       if( [all_keys containsObject:DETAIL])
                       {
                        ...
                        for (int counter = 0; counter < [valueMapGpArray count]; counter++) {
                            ...
                            NSLog(@"GPTable name is %@",tableName);
                            NSArray *gpTableRecords = pbRelatedObjectMap.valueMap;
                            
                            NSLog(@"Table name is %@ and count is %d",tableName,[gpTableRecords count]);
    ...
    }@catch (NSException *exp)
    {
        ...
        [self settingFlags];
        NSLog(@"Exception Name WSInterface :operation:completesWithResponse %@",exp.name);
        NSLog(@"Exception Reason WSInterface :operation:completesWithResponse %@",exp.reason);
    }
}
...
-(NSString *)getIdFromJsonString:(NSString *)jsonString
{
    ...
    for(NSString * field in all_json_keys)
    {
        ...
        NSLog(@"Exception Name WSInterface :getIdFromJsonString %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getIdFromJsonString %@",exp.reason);
    }
    @finally{
        [autoReleasePool drain];
    }
...
- (NSMutableArray *) collectPickListObject
{
    ...
    for (int i = 0; i < [object count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :collectPickListObject %@",exp.name);
        NSLog(@"Exception Reason WSInterface :collectPickListObject %@",exp.reason);
    }
    return result;
}
...
- (NSMutableArray *) getAllPageLauoutId
{
    ...
    for (int i = 0; i < [array count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getAllPageLauoutId %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getAllPageLauoutId %@",exp.reason);
    }
    return pageId;
}
...
- (NSMutableArray *) getAllProcessId
{
    ...
    for (int i = 0; i < [sfm_processComp count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getAllProcessId %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getAllProcessId %@",exp.reason);
    }
    return processId;
}
...
//    {
    ...
    if( tempObjects.count != 0 )
    {
        ...
        NSLog(@"Exception Name WSInterface :getRecordTypeDictForObjects %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getRecordTypeDictForObjects %@",exp.reason);
    }
...
- (NSMutableArray *) getTasksFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for (int i = 0; i < [bodyParts count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getTasksFromResponse %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getTasksFromResponse %@",exp.reason);
    }
    return _tasks;
}
...
- (NSMutableArray *) getProductHistoryFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for (int i = 0; i < [bodyParts count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getProductHistoryFromResponse %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getProductHistoryFromResponse %@",exp.reason);
    }
...
- (NSMutableArray *) getAccountHistoryFromResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for (int i = 0; i < [historyInfo count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getAccountHistoryFromResponse %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getAccountHistoryFromResponse %@",exp.reason);
...
- (NSMutableDictionary *) getDescribeObjects:(NSMutableDictionary *)bodyParts
{
    ...
    for (int d = 0; d < [dtl count]; d++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getDescribeObjects %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getDescribeObjects %@",exp.reason);
    }
...
- (void) didDescribeSObject:(ZKDescribeSObject *)describeObject error:(NSError *)error context:(id)context
{
    ...
    if ([lookupCaller respondsToSelector:@selector(setLookupData:)])
        ...
        NSLog(@"Exception Name WSInterface :didDescribeSObject %@",exp.name);
        NSLog(@"Exception Reason WSInterface :didDescribeSObject %@",exp.reason);
    }
}
...
- (NSMutableDictionary *) GetHeaderSectionForSequenceNumber:(NSInteger)sequence
{
	...
	for (int i=0;i<[sections count];i++)
	{
        ...
        NSLog(@"Exception Name WSInterface :GetHeaderSectionForSequenceNumber %@",exp.name);
        NSLog(@"Exception Reason WSInterface :GetHeaderSectionForSequenceNumber %@",exp.reason);
...
- (void) getDictionaryFromPageLayout:(NSMutableDictionary *)bodyParts withDescribedObjects:(NSMutableArray *)describeObjects
{
  ...
  @try{
    ...
    if ([self.delegate respondsToSelector:@selector(didReceivePageLayout:withDescribeObjects:)])
      ...
      NSLog(@"Exception Name WSInterface :getDictionaryFromPageLayout %@",exp.name);
      NSLog(@"Exception Reason WSInterface :getDictionaryFromPageLayout %@",exp.reason);
  }
}
...
- (INTF_WebServicesDefServiceSvc_INTF_TargetRecord *) getTargetRecordsFromSFMPage:(NSDictionary *)sfmpage
{
    ...
    for (int i = 0; i < [details count]; i++) //parts, labor, expense for instance
    {
        ...
        NSLog(@"Exception Name WSInterface :getTargetRecordsFromSFMPage %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getTargetRecordsFromSFMPage %@",exp.reason);
    }
    return targetRecord;
}
...
- (BOOL)getPriceInformationForWorkOrderId:(NSString *)sfmId {
    ...
    @try {
        ...
        while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
        {
            NSLog(@"getPriceInformationForWorkOrderId: Download  PRICE _INFO for %@",sfmId);
          
            if (![appDelegate isInternetConnectionAvailable])
            {
                NSLog(@"getPriceInformationForWorkOrderId: Download  PRICE _INFO Failed due to Internet Lost");
                returnValue = NO;
                break;
            }
            if(appDelegate.connection_error)
            {
				//Defect 6774
				[appDelegate checkifConflictExistsForConnectionError];
                NSLog(@"getPriceInformationForWorkOrderId: Download  PRICE _INFO Failed due to Connection Error");
                returnValue = NO;
                break;
            }
            ...
            {
                returnValue = YES;
                NSLog(@"getPriceInformationForWorkOrderId: Download  PRICE _INFO Completed successfully");
                break;
            }
        }
...
- (NSMutableDictionary *) getTagsdisplay:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    _tagsDictionary = [self fillEmptyTags:_tagsDictionary];
        ...
        NSLog(@"Exception Name WSInterface :getTagsdisplay %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getTagsdisplay %@",exp.reason);
    }
    return _tagsDictionary;
}
...
-(NSMutableDictionary *) fillEmptyTags:(NSMutableDictionary *)_tagsDictionary
{
    ...
    for(int j = 0 ; j < [default_keys count];j++)
    {
        ...
        NSLog(@"Exception Name WSInterface :fillEmptyTags %@",exp.name);
        NSLog(@"Exception Reason WSInterface :fillEmptyTags %@",exp.reason);
    }
    return _tagsDictionary;
}
...
{    
    ...
    for ( int i = 0; i < [_eventArray count]; i++ ) 
    {
        ...
        NSLog(@"Exception Name WSInterface :getEventdisplay %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getEventdisplay %@",exp.reason);
    }
    NSLog(@"arr = %@", arr);
...
- (NSArray *) getCreateProcessesDictionaryArray:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    appDelegate.StandAloneCreateProcess = [section_for_createObjects retain];
        ...
        NSLog(@"Exception Name WSInterface :getCreateProcessesDictionaryArray %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getCreateProcessesDictionaryArray %@",exp.reason);
    }
    return array;
}
...
- (NSArray *) getViewLayoutArray:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    for (int i = 0; i < [viewLayoutsInfo count]; i++)
    {
        ...
        NSLog(@"Exception Name WSInterface :getViewLayoutArray %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getViewLayoutArray %@",exp.reason);
    }
    NSLog(@"%@", array);
    return array;
}
...
-(void)didGetNameFields:(NSMutableArray *) describeObjects error:(NSError *) error context:(id)context;
{
    ...
    didGetObjectName = TRUE;
        ...
        NSLog(@"Exception Name WSInterface :didGetNameFields %@",exp.name);
        NSLog(@"Exception Reason WSInterface :didGetNameFields %@",exp.reason);
    }
...
-(NSMutableDictionary *) getSaveTargetRecords:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    appDelegate.newRecordId = resultId;
        ...
        NSLog(@"Exception Name WSInterface :getSaveTargetRecords %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getSaveTargetRecords %@",exp.reason);
    }
...
- (void) getNameFieldForCreateProcess:(NSString *)ID
{
    ...
    while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, FALSE))
    {
        ...
        NSLog(@"Exception Name WSInterface :getNameFieldForCreateProcess %@",exp.name);
        NSLog(@"Exception Reason WSInterface :getNameFieldForCreateProcess %@",exp.reason);
    }
}
...
- (void) didGetNameField:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    didGetNameField = TRUE;
        ...
        NSLog(@"Exception Name WSInterface :didGetNameField %@",exp.name);
        NSLog(@"Exception Reason WSInterface :didGetNameField %@",exp.reason);
...
//        [appDelegate.recentObject addObjectsFromArray:array];
    ...
    else 
    {
        ...
        NSLog(@"Exception Name WSInterface :saveDictionaryToPList %@",exp.name);
        NSLog(@"Exception Reason WSInterface :saveDictionaryToPList %@",exp.reason);
...
- (void)getUserTrunkLocationRequest {
    ...
    if (package <  minVersion) {
        [appDelegate.dataBase removeUserTechnicianLocation];
         NSLog(@"Lookup filters is not supported in %@",currentServerPkgVersion);
        return;
    }
    NSLog(@"%@",currentServerPkgVersion);
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        NSLog(@"User trunk request");
        
        if (![appDelegate isInternetConnectionAvailable])
        {
            NSLog(@"User trunk request: Failed due to Internet Lost");
        ...
        if(appDelegate.connection_error)
        {
             NSLog(@"User trunk request: Failed due to Connection Error");
            
            break;
        }
        NSString *currentStaus =  [Utility getUserTrunkRequestStatus];
        if ([currentStaus isEqualToString:@"true"])
        {   NSLog(@"User trunk request Completed successfully");
            break;
        }
    }
}
...
- (void)sessionDidResume:(ZKLoginResult *)loginResult error:(NSError *)error
{
    ...
    if (error)
    {
        ...
        NSLog(@"Exception Name WSInterface :sessionDidResume %@",exp.name);
        NSLog(@"Exception Reason WSInterface :sessionDidResume %@",exp.reason);
