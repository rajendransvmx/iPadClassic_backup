...
@synthesize isCustomWebServiceCallInProgress;// Vipindas 10128 defect fix
...
//    }
    ...
    //11946
    else if ([optimizeSyncCalls.purgingEventIdArray count] == 0) {
        NSString *eventId = [appDelegate.dataBase getAllEventsIdsForloggedInUser];
        if ([eventId length] > 0) {
            [appDelegate.dataBase purgeAllEventsRelatedToLoggedInUser:eventId];
        }
		optimizeSyncCalls.purgingEventIdArray = nil;
    }
...
@try{
   ...
   /* NSAutoreleasePool * autoreleasePool = [[NSAutoreleasePool alloc] init];
    ...
    for(NSString * object_name in allobjects)
    {
        ...
        [SVMXCMap_Object release]; SVMXCMap_Object = nil;
    }
          ...
          /* // code to differentiate b/w the parent and child 
           ...
           if([Master_or_parent isEqualToString:MASTER])
            ...
            if ([beforeSaveFields count] > 0)
            {
                ...
                [record_SVMXC.valueMap addObject:beforeSaveMap];
            ...
            [record_SVMXC release]; record_SVMXC = nil;
...
//    svmxMap = NULL;
    ...
    {
        INTF_WebServicesDefServiceSvc_SVMXClient * client_iPad_Version = [[INTF_WebServicesDefServiceSvc_SVMXClient alloc] init];
...
//    INTF_WebServicesDefServiceSvc_SVMXClient * client = [[[INTF_WebServicesDefServiceSvc_SVMXClient alloc] init] autorelease];
    ...
    [SVMXCMap_startTime release];
    [SVMXCMap_endTime release];
    //    client.clientType = @"iPad";
    //    [client.clientInfo addObject:@"OS:iPadOS"];
    //    [client.clientInfo addObject:@"R4B2"];
...
//    [self addClientInfoForObject:client];
    ...
    if([eventName isEqualToString:@"SFM_SEARCH"] && [eventType isEqualToString:@"SEARCH_RESULTS"])
    {
        ...
        [SVMXCProcessID release]; SVMXCProcessID = nil;
        ...
        [SVMXCCrtieria release]; SVMXCCrtieria = nil;
        ...
        [SVMXCUserFilterString release]; SVMXCUserFilterString = nil;
        ...
        for(int i=0; i<[objectList count]; i++)
        {
            ...
            [SVMXCObject release]; SVMXCObject = nil;
        }
        ...
        [SVMXCLimitObject release]; SVMXCLimitObject = nil;
                    ...
                    responseTypeForLastIndexTwo:(BOOL)iSDCResponseType
{
    @try{
        ...
        if([eventName isEqualToString:GET_PRICE_DATA] && [eventType  isEqualToString:SYNC])
        {
            ...
            INTF_WebServicesDefServiceSvc_SVMXMap * SVMXCMap_last_index =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
            SVMXCMap_last_index.key  = @"LAST_INDEX";
            ...
            {
                INTF_WebServicesDefServiceSvc_SVMXMap * SVMXCMapLastSyncTime =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
                SVMXCMapLastSyncTime.key  = @"LAST_SYNC_TIME";
                
                //8581
                if([appDelegate shouldDoOneCallSync])
                {
                    SVMXCMapLastSyncTime.value=[appDelegate.wsInterface get_SYNCHISTORYTime_ForKey:LAST_OSC_TIMESTAMP] == nil ?@"":[appDelegate.wsInterface get_SYNCHISTORYTime_ForKey:LAST_OSC_TIMESTAMP];
                }
                else
                {
                    SVMXCMapLastSyncTime.value = [self get_SYNCHISTORYTime_ForKey:LAST_DC_INSERT_RESPONSE_TIME] == nil ?@"":[self get_SYNCHISTORYTime_ForKey:LAST_DC_INSERT_RESPONSE_TIME];
                }
                [sfmRequest.valueMap addObject:SVMXCMapLastSyncTime];
                /*Mem Opt*/
                [SVMXCMapLastSyncTime release];
            }
            ...
            if(([lastIndex isEqualToString:@"1"]) || ([lastIndex isEqualToString:@"2"]))
            {
                ...
                if([lastIndex isEqualToString:@"2"])
                {
                    ...
                    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXCMapCurrency =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
                    ...
                    [SVMXCMapCurrency.values addObjectsFromArray:uniqueCurrency];
                    [sfmRequest.valueMap addObject:SVMXCMapCurrency];
                    [uniqueCurrency release];
                    [SVMXCMapCurrency release];
...
//Pushpak 10128
- (BOOL)didCustomWebServiceFailForOperation:(INTF_WebServicesDefBindingOperation *)operation
                               withResponse:(INTF_WebServicesDefBindingResponse *)response
{
    BOOL didWebServiceFail = false;
    if (self.isCustomWebServiceCallInProgress && [operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_PREQ_GetPrice_WS class]]) {
        
        if (response.error != nil)
        {
            didWebServiceFail = true;
            self.getPrice = YES;
            SMLog(kLogLevelError,@"%@",response.error.domain);
            SMLog(kLogLevelError,@"%@",response.error.description);

            NSDictionary *errorDict = @{@"ExpName":response.error.domain,
                                        @"ExpReason":response.error.description,
                                        @"userInfo":@{@"userInfo": @""}};
            
            [appDelegate CustomizeAletView:nil alertType:SOAP_ERROR Dict:(NSMutableDictionary *)errorDict exception:nil];
        }
        else
        {
            if ([[response.bodyParts objectAtIndex:0] isKindOfClass:[SOAPFault class]])
            {
                ...
                NSDictionary *errorDict = @{@"ExpName":sFault.faultcode,
                                            @"ExpReason":sFault.faultstring,
                                            @"userInfo":@{@"userInfo": @""}};
...
- (void) operation:(INTF_WebServicesDefBindingOperation *)operation completedWithResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    if(appDelegate.dataSyncRunning)
    {
        BOOL didWebServiceFail = [self didCustomWebServiceFailForOperation:operation
                                     withResponse:response];
        if (didWebServiceFail) {
            
            //We are returning as we don't want the data sync to fail.
            return;
        }
    }
                    ...
                    for(int k=0; k<[objectInfo count]; k++) //4 objects
                    {
                        ...
                        for(int l=0; l< [detailsArray count]; l+=2)// 2 maps // object map, config data
                        {
                            ...
                            for(int m=0; m<[objectInfoArray count]; m++)
                            {
                                ...
                                if([objectInfo.key isEqualToString:@"SVMXC__Advance_Expression__c"]) {
                ...
                for (int i = 0; i < [pageArray count]; i++)
                {
                    ...
                    for ( int j = 0; j < [details count]; j++)
                    {
                        ...
                        for (int k = 0; k < [pageDetail.fields count]; k++)
                        {
                            ...
                            /*Krishna : CONTEXTLOOKUP -> Added 'SVMXC__Context_Source_Object__c' */
                            ...
                            NSMutableArray * detailFieldObjects = [NSMutableArray arrayWithObjects:
                                                                   ...
                                                                   (detailUiField.fieldDetail.SVMXC__Context_Source_Object__c != nil)?detailUiField.fieldDetail.SVMXC__Context_Source_Object__c:@"",
                                                                   nil];
                    ...
                    for (int i = 0; i < [header.sections count]; i++)
                    {
                        ...
                        for (int j = 0; j < [fields count]; j++)
                        {
                            ...
                            NSMutableArray * hdrSectionFieldValues = [NSMutableArray arrayWithObjects:
                                                                      ...
                                                                      (uiField.fieldDetail.SVMXC__Context_Source_Object__c != nil)?uiField.fieldDetail.SVMXC__Context_Source_Object__c:@"",
