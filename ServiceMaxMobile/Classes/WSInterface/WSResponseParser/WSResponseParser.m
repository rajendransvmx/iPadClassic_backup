...
{
    SMLog(kLogLevelWarning,@"None of the sub classes parsed the response %@",result);
    return FALSE;
}
        ...
        for (NSDictionary *dict in array)
        {
            ...
            if (returnValue ==  SQLITE_OK)
            {
                if (synchronized_sqlite3_step(statement) != SQLITE_DONE)
                {
                    ...
                    SMLog(kLogLevelError,@"Failure insertRecords - query => %@", query);
                } else
                {
            ...
            {
                SMLog(kLogLevelError,@" Failure prepared insertRecords  -  %d \n%s \nquery: %@", returnValue, sqlite3_errmsg(appDelegate.db), query );
            }
...
- (void) execStatementOnDataBase:(NSString *)queryStatement
{
    ...
    if (result != SQLITE_OK)
    {
        SMLog(kLogLevelError,@"Failed to Execute the Query %@. Error No = %d Error Message = %s",queryStatement,result,err);
    }
...
- (void) createTable:(NSString *) tableName
{
    ...
    SMLog(kLogLevelVerbose,@"Creating New Table Query = %@",queryStatement);
