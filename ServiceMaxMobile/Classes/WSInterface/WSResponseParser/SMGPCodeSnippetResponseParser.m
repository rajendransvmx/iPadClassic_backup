...
- (BOOL) parseResponse:(NSArray *)result
{
    for(int i=0; i<[result count]; i++)
    {
        ...
        codeSnippet = [json_dict objectForKey:@"SVMXDEV__Code_Snippet__c"];
        referencedCodeSnippet = [json_dict objectForKey:@"SVMXDEV__Referenced_Code_Snippet__c"];

        if(codeSnippet != nil)
        {
            if(referencedCodeSnippet != nil)
            {
                data = [json_dict objectForKey:@"SVMXDEV__Data__c"];
                ...
                NSArray *keys = [NSArray arrayWithObjects:@"local_id",@"Id",@"SVMXDEV__Code_Snippet__c",@"Name",@"SVMXDEV__Referenced_Code_Snippet__c", nil];
                
                NSString  *queryStatement = [NSString stringWithFormat:@"INSERT INTO SVMXDEV__Code_Snippet_Manifest__c (%@) VALUES (%@)",[keys componentsJoinedByString:@","],values];
        ...
        {
            data = [json_dict objectForKey:@"SVMXDEV__Data__c"];
            name = [json_dict objectForKey:@"SVMXDEV__Name__c"];
            type = [json_dict objectForKey:@"SVMXDEV__Type__c"];
            snippetId = [json_dict objectForKey:@"SVMXDEV__SnippetId__c"];
            ID = [json_dict objectForKey:@"Id"];
            snippetName = [json_dict objectForKey:@"Name"];
            // Mem_leak_fix - Vipindas 9493 Jan 18
            localId = [[self getUUID] retain];
            ...
            NSArray *keys = [NSArray arrayWithObjects:@"SVMXDEV__Data__c",@"SVMXDEV__Name__c",@"SVMXDEV__Type__c",@"SVMXDEV__SnippetId__c",@"local_id",@"id",@"Name", nil];
            
            NSString  *queryStatement = [NSString stringWithFormat:@"INSERT INTO SVMXDEV__Code_Snippet__c (%@) VALUES (%@)",[keys componentsJoinedByString:@","],values];
            [self execStatementOnDataBase:queryStatement];
            [localId release];
        }
        [iPool drain];
