...
//		[appDelegate displayNoInternetAvailable];
	...
	if ( [[request.URL absoluteString] Contains:@"frontdoor.jsp"] )
	{
		...
		SMLog(@"%@", [[request.URL absoluteString] pathComponents]);
		appDelegate.isUserOnAuthenticationPage = FALSE;
	}
...
- (void)extractAccessCodeFromCallbackURL:(NSURL *)callbackURL;
{
	...
	{
		SMLog(@"END USER DENIED AUTHORIZATION...");		
...
- (BOOL)refreshAccessToken:(NSString *)refresh_Token;
{
	...
	SMLog(@"RESPONSE WITH NEW ACCESS TOKENS : %@", dict);
...
/*- (void)revokeExistingToken:(NSString *)refresh_token
{
	...
	SMLog(@"%@ REVOKE STATUS : %d", data, [response statusCode]);
	
	if ( [response statusCode] == 200)
	{
		SMLog(@"Sucessfully Revoked the Tokens..");
		...
		SMLog(@"Failed Revoking the Tokens...");
...
- (BOOL)revokeExistingToken:(NSString *)refresh_token
{
    ...
    //Fix 9282: (Anish)Upgrading the Sum 13 app to Sum 13 sp, inconsistently revokes the access token and does not let the user to logout.
    //Sometimes the refresh token is reset during OAuth and this sets the refresh token to nil. A nil refresh token cannot be revoked.
...
{	
	...
	{
		SMLog(@"Cookie domain: %@", [cookie domain]);
		if([[cookie domain] Contains:@".salesforce.com"])
		{
			SMLog(@"FOUND AND DELETED COOKIE");
