...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString * row_tittle = [dict objectForKey:SVMXDEV_Name];
    ...
    NSString * object_description = [dict objectForKey:SVMXDEV_Description];
    CGFloat height ;
    CGSize size;
    ...
    description.text =  [dict objectForKey:SVMXDEV_Description];
    ...
    [button setAccessibilityIdentifier:kAccSFMDisclosure];
    ...
    [bgImage release];
        ...
        SMLog(kLogLevelError,@"Exception Name CreateObjectDetail :cellForRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason CreateObjectDetail :cellForRowAtIndexPath %@",exp.reason);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @try{
    NSDictionary * dict = [[appDelegate.StandAloneCreateProcess objectAtIndex:selectedRootViewRow] objectAtIndex:indexPath.row];
    NSString * processTitle = [dict objectForKey:SVMXDEV_Name];
    processId = [dict objectForKey:SVMXDEV_ProcessID];
    SMLog(kLogLevelVerbose,@"%@", processId);
    
    //sahana offline
    NSString * object_name = [dict objectForKey:SVMXDEV_OBJECT_NAME];
   ...
   [delegate showSFMCreateObjectWithProcessID:processId processTitle:processTitle object_name:object_name];
        ...
        SMLog(kLogLevelError,@"Exception Name CreateObjectDetail :didSelectRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason CreateObjectDetail :didSelectRowAtIndexPath %@",exp.reason);
          [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
//    activity.frame = CGRectMake(self.view.frame.size.height/2, self.view.frame.size.width/2, activity.frame.size.width, activity.frame.size.height);
    ...
    [delegate showSFMCreateObjectWithProcessID:processId processTitle:processTitle object_name:object_name];
        ...
        SMLog(kLogLevelError,@"Exception Name CreateObjectDetail :accessoryButtonTappedForRowWithIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason CreateObjectDetail :accessoryButtonTappedForRowWithIndexPath %@",exp.reason);
         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
