...
//        backgroundView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, tableView.frame.size.width, cell.frame.size.height)] autorelease];
	...
	NSLog(@"Exception Name CreateObject :cellForRowAtIndexPath %@",exp.name);
	NSLog(@"Exception Reason CreateObject :cellForRowAtIndexPath %@",exp.reason);
...
//            processId = [dict objectForKey:SVMXC_ProcessID];
    ...
    [self showSFMCreateObjectWithProcessID:processId processTitle:processTitle];
        ...
        NSLog(@"Exception Name CreateObject :accessoryButtonTappedForRowWithIndexPath %@",exp.name);
        NSLog(@"Exception Reason CreateObject :accessoryButtonTappedForRowWithIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
