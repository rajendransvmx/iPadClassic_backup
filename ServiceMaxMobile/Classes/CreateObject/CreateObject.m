...
- (void)viewDidLoad
{
	...
	splitView.view.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);    
    /*ios7_support shravya-navbar*/
    if (![Utility notIOS7]) {
...
//        backgroundView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, tableView.frame.size.width, cell.frame.size.height)] autorelease];
	...
	SMLog(kLogLevelError,@"Exception Name CreateObject :cellForRowAtIndexPath %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason CreateObject :cellForRowAtIndexPath %@",exp.reason);
...
//            processId = [dict objectForKey:SVMXDEV_ProcessID];
    ...
    [self showSFMCreateObjectWithProcessID:processId processTitle:processTitle];
        ...
        SMLog(kLogLevelError,@"Exception Name CreateObject :accessoryButtonTappedForRowWithIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason CreateObject :accessoryButtonTappedForRowWithIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
