...
#import "AccessibilityCalendarControlConstants.h"
...
{
    /*Accessibility Changes*/
    dateLabel.isAccessibilityElement = YES;
    [dateLabel setAccessibilityIdentifier:kAccDateLabel];
    
    dayLabel.isAccessibilityElement = YES;
    [dayLabel setAccessibilityIdentifier:kAccDayLabel];
    
    prevMonthBtn.isAccessibilityElement = YES;
    [prevMonthBtn setAccessibilityIdentifier:kAccPrevMonth];
    
    nextMonthBtn.isAccessibilityElement = YES;
    [nextMonthBtn setAccessibilityIdentifier:kAccNextMonth];
    
    label.isAccessibilityElement = YES;
    [label setAccessibilityIdentifier:kAccMonthLabel];
    
    calMonday.isAccessibilityElement = YES;
    [calMonday setAccessibilityIdentifier:kAccMondayLabel];
    
    calTuesday.isAccessibilityElement = YES;
    [calTuesday setAccessibilityIdentifier:kAccTuesdayLabel];
    
    calWednesday.isAccessibilityElement = YES;
    [calWednesday setAccessibilityIdentifier:kAccWedLabel];
    
    calThursday.isAccessibilityElement = YES;
    [calThursday setAccessibilityIdentifier:kAccThursdayLabel];
    
    calFriday.isAccessibilityElement = YES;
    [calFriday setAccessibilityIdentifier:kAccFridayLabel];
    
    calSaturday.isAccessibilityElement = YES;
    [calSaturday setAccessibilityIdentifier:kAccSatLabel];
    
    calSunday.isAccessibilityElement = YES;
    [calSunday setAccessibilityIdentifier:kAccSundayLabel];
...
- (void) PopulateDateFromDay:(NSInteger) weekday totalDays:(NSInteger) numDays
{
    ...
    */
    ...
    for (int i = 0; i < weekday-1; i++)
	{
        ...
        /*Accessibility Changes*/
        NSString * identifier = [[NSString alloc] initWithFormat:@"%@%@", kAccDate,[dates objectAtIndex:i]];
	...
	{
        // // SMLog(kLogLevelVerbose,@"Date inserted = %@", [NSString stringWithFormat:@"%d", dayCount]);
	...
	SMLog(kLogLevelError,@"Exception Name CalendarController :PopulateDateFromDay %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason CalendarController :PopulateDateFromDay %@",exp.reason);
                                 ...
                                 isPrevMonth:(BOOL) prevMonth
{
	...
	SMLog(kLogLevelError,@"Exception Name CalendarController :getRemainingDaysInMonth %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason CalendarController :getRemainingDaysInMonth %@",exp.reason);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (IBAction) DateClicked:(id)sender
{
    ...
    SMLog(kLogLevelVerbose,@"Tag = %d WeekDay = %d",[dateButton tag],newWeekday);
    [newDay release];
    ...
    // SMLog(kLogLevelVerbose,@"%d", selDate);
	...
	SMLog(kLogLevelError,@"Exception Name CalendarController :DateClicked %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason CalendarController :DateClicked %@",exp.reason);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (NSMutableArray *) getWeeksArray
{
	...
	SMLog(kLogLevelError,@"Exception Name CalendarController :getWeeksArray %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason CalendarController :getWeeksArray %@",exp.reason);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (NSDictionary *) getWeekDetails
{
    ...
    @catch(NSException *exp)
    {
        SMLog(kLogLevelError,@"Exception Name CalendarController :getWeekDetails %@",exp.name);
        SMLog(kLogLevelVerbose,@"Exception Reason CalendarController :getWeekDetails %@",exp.reason);
         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    return dict;
}
...
- (NSArray *) getWeekBoundaries:(NSString *)date
{
	...
	SMLog(kLogLevelError,@"Exception Name CalendarConroller :getWeekBoundaries %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason CalendarConroller :getWeekBoundaries %@",exp.reason);
	[appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void)dealloc
{
    ...
    [calSunday release];
    [calSunday release];
    [prevMonthBtn release];
    [nextMonthBtn release];
    [super dealloc];
}
