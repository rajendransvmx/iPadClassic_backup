...
{
	NSString *query = [NSString stringWithFormat:@"SELECT Id, SVMXDEV__Expense_Type__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Expenses' AND SVMXDEV__Service_Order__c = '%@' AND RecordTypeId = '%@'", appDelegate.currentWorkOrderId, appDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
...
- (void) getExpenses:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		SMLog(kLogLevelVerbose,@"%@", [[obj fields] objectForKey:@"SVMXDEV__Expense_Type__c"]);
		[md setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:[[obj fields] objectForKey:@"SVMXDEV__Expense_Type__c"]];
		NSString *key, *value;
		key = [[obj fields] objectForKey:@"SVMXDEV__Expense_Type__c"];
		value = [[obj fields] objectForKey:@"SVMXDEV__Work_Description__c"];
		if ( key != nil )
