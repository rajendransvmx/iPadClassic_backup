...
- (void) createMainPage
{
    ...
    else
    {
        ...
        {
            NSDictionary * company = [self getObjectForKey:@"SPR14__Company__c"];
        ...
        {
            _account = [NSArray arrayWithObjects:
                        [reportEssentialsDict objectForKey:@"SPR14__Street__c"], 
                        [reportEssentialsDict objectForKey:@"SPR14__City__c"], 
                        [reportEssentialsDict objectForKey:@"SPR14__State__c"], 
                        [reportEssentialsDict objectForKey:@"SPR14__Country__c"], 
                        [reportEssentialsDict objectForKey:@"SPR14__Zip__c"], nil];
        }
        else if ([appDelegate.addressType isEqualToString:@"Contact Address"])
        {
            NSDictionary * contact = [self getObjectForKey:@"SPR14__Contact__c"];
    ...
    _description = [reportEssentialsDict objectForKey:@"SPR14__Problem_Description__c"];
    if (![_description isKindOfClass:[NSString class]])
        _description = @"";
    if (srShowProblemDescription)
        [self setDescription:_description];

    _workPerformed = [reportEssentialsDict objectForKey:@"SPR14__Work_Performed__c"];
       ...
       //     Abinash 28th december
            ...
            linePrice *= (1 - ([[[_parts objectAtIndex:i] objectForKey:@"Discount"] floatValue]/100));
            {
                ...
                if (self.shouldShowBillablePrice) {
                    linePriceFinal =  [[[_parts objectAtIndex:i] objectForKey:SPR14__Billable_Line_Price__c] floatValue];
                ...
                if (self.shouldShowBillableQty) {
                    quantity = [[_parts objectAtIndex:i] objectForKey:SPR14__Billable_Quantity__c];
                }
                else {
...

    {
        ...
        for (int j = 0; j < [_labor count]; j++)
        {
            ...
            tLabor = [labor objectForKey:SPR14__Activity_Type__c];
            tHours = [labor objectForKey:SPR14__Actual_Quantity2__c];
            if (![tHours isKindOfClass:[NSString class]])
                tHours = @"0.0";
            tRate = [labor objectForKey:SPR14__Actual_Price2__c];
            ...
            /* 6773 */
            if (self.shouldShowBillablePrice) {
                double someValue = [[labor objectForKey:SPR14__Billable_Line_Price__c] doubleValue];
                tLinePrice = [NSString stringWithFormat:@"%.2f", someValue];
            }
            
            /* 6773 */
            if (self.shouldShowBillableQty) {
                tHours = [labor objectForKey:SPR14__Billable_Quantity__c];
                if (![tHours isKindOfClass:[NSString class]])
                    tHours = @"0.0";
            }
...

    {
        ...
        for (int k = 0; k < [_expenses count]; k++)
        {
            ...
            NSString * expenseType = [expenses objectForKey:@"SPR14__Expense_Type__c"];
            if (![expenseType isKindOfClass:[NSString class]])
                expenseType = @"";
            
            NSString * actualPrice = [expenses objectForKey:@"SPR14__Actual_Price2__c"];
            if (![actualPrice isKindOfClass:[NSString class]])
                actualPrice = @"0.0";
            NSString * expenseQty = [expenses objectForKey:@"SPR14__Actual_Quantity2__c"];
            ...
            /* 6773 */
            if (self.shouldShowBillablePrice) {
                double someDoubleValue =  [[expenses objectForKey:SPR14__Billable_Line_Price__c] doubleValue];
        ...
        for (int t = 0; t < [travelArray count]; t++)
        {
            ...
            float linePrice = [[[travelArray objectAtIndex:t] objectForKey:@"SPR14__Actual_Price2__c"] floatValue];
            linePrice *= [[[travelArray objectAtIndex:t] objectForKey:@"SPR14__Actual_Quantity2__c"] floatValue];
            ...
            if (self.shouldShowBillablePrice) {
                linePrice = [[obj objectForKey:SPR14__Billable_Line_Price__c] floatValue];
            }
           
            NSString *quantity = nil;
            
            if (self.shouldShowBillableQty) {
                quantity = [obj objectForKey:SPR14__Billable_Quantity__c];
            }
            else {
                quantity = [[travelArray objectAtIndex:t] objectForKey:@"SPR14__Actual_Quantity2__c"];
            }
            
            NSString *unitPrice = [[travelArray objectAtIndex:t] objectForKey:@"SPR14__Actual_Price2__c"];
            if (unitPrice == nil) {
                unitPrice = @"0.0";
            }
