...
- (void)viewDidLoad
{
    ...
    {
        SMLog(kLogLevelVerbose,@"%@",appDelegate.serviceReport);
    ...
    for (NSString * key in allKeys)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name PDFCreater :viewDidLoad %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason PDFCreater :viewDidLoad %@",exp.reason);
    }
    if (iOSObject == nil)
    ...
    SMLog(kLogLevelVerbose,@"%@", newFilePath);
#ifdef DEBUG
    // ######################################### //
...
- (void) setAccessibilityIdentifierForInstanceVariable
{
    ...
    [webView setAccessibilityIdentifier:kAccPdfWebview];
    
    [backButton setIsAccessibilityElement:YES];
    [backButton setAccessibilityIdentifier:kAccBackButton];
    
    [sendMailButton setIsAccessibilityElement:YES];
    [sendMailButton setAccessibilityIdentifier:kAccEmail];
    
    [helpButton setIsAccessibilityElement:YES];
    [helpButton setAccessibilityIdentifier:kAccHelpButton];
    
    [displayUser setIsAccessibilityElement:YES];
...
- (void) didInternetConnectionChange:(NSNotification *)notification
{
    ...
    if (isReachable)
    {
        SMLog(kLogLevelVerbose,@"PDFCreator Internet Reachable");
    }
    else
    {
        SMLog(kLogLevelWarning,@"PDFCreator Internet Not Reachable");
...
//    NSString * _query = [NSString stringWithFormat:@"SELECT Id FROM Attachment WHERE Name = '%@' and OwnerId = '%@'", pdf, appDelegate.current_userId];
    ...
    [monitor monitorSMMessageWithName:@"[PDFCreator removeAllPDF]"
   ...
   while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES)) 
    {
        ...
        SMLog(kLogLevelVerbose,@"PDFCreator.m : removeAllPDF: ZKS call");
#endif
...
- (void) didGetPDFList:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        didremoveallPdf = FALSE;
        SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
        [monitor monitorSMMessageWithName:@"[PDFCreator didGetPDFList]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
                               logContext:@"Start"
                             timeInterval:kWSExecutionDuration];
       ...
       while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES)) 
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(kLogLevelVerbose,@"PDFCreator.m : didGetPDFList: ZKS call");
#endif
    ...
    {
        SMLog(kLogLevelVerbose,@"2");
        didremoveallPdf = FALSE;
        [self didRemoveAllPDF:nil error:nil context:nil];
       while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES)) 
        {
#ifdef kPrintLogsDuringWebServiceCall
            SMLog(kLogLevelVerbose,@"PDFCreator.m : didGetPDFList: ZKS call 2");
#endif
        ...
        SMLog(kLogLevelError,@"Exception Name PDFCreator :didGetPDFList %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason PDFCreator :didGetPDFList %@",exp.reason);
    }
...
- (void) attachOPDOC:(NSString *)opdoc andDocName:(NSString *)docName forSFId:(NSString *)sf_id andProcessId:(NSString *)processId {
    ...
    //9007
    [dict setObject:docName forKey:@"Name"];
...
{
    SMLog(kLogLevelVerbose,@"Attach PDF successful");
    SMLog(kLogLevelVerbose,@"%@", [error description]);
    SMLog(kLogLevelVerbose,@"result %@",result);
    ...
    //9007
    NSString *docName = [dictionary objectForKey:@"Name"];
    
    //send delegate to the view controller. so that it can disable buttons.
    
    //krishna opdoc
    if(self.delegate && [self.delegate respondsToSelector:@selector(opDocumentAttached:withError:forSFID:andProcessID:forDocName:)] ) {
        //9007
        [self.delegate opDocumentAttached:attachedResult withError:error forSFID:sfID andProcessID:processId forDocName:docName];
    }
...
- (void) createMainPage
{
    ...
    else
    {
        ...
        {
            NSDictionary * company = [self getObjectForKey:@"SVMXDEV__Company__c"];
        ...
        {
            _account = [NSArray arrayWithObjects:
                        [reportEssentialsDict objectForKey:@"SVMXDEV__Street__c"], 
                        [reportEssentialsDict objectForKey:@"SVMXDEV__City__c"], 
                        [reportEssentialsDict objectForKey:@"SVMXDEV__State__c"], 
                        [reportEssentialsDict objectForKey:@"SVMXDEV__Country__c"], 
                        [reportEssentialsDict objectForKey:@"SVMXDEV__Zip__c"], nil];
        }
        else if ([appDelegate.addressType isEqualToString:@"Contact Address"])
        {
            NSDictionary * contact = [self getObjectForKey:@"SVMXDEV__Contact__c"];
    ...
    _description = [reportEssentialsDict objectForKey:@"SVMXDEV__Problem_Description__c"];
    if (![_description isKindOfClass:[NSString class]])
        _description = @"";
    if (srShowProblemDescription)
        [self setDescription:_description];

    _workPerformed = [reportEssentialsDict objectForKey:@"SVMXDEV__Work_Performed__c"];
...

    {
        ...
        {
            SMLog(kLogLevelVerbose,@"%@", [_parts objectAtIndex:i]);
       ...
       //     Abinash 28th december
            ...
            linePrice *= (1 - ([[[_parts objectAtIndex:i] objectForKey:@"Discount"] floatValue]/100));
            {
                ...
                if (self.shouldShowBillablePrice) {
                    linePriceFinal =  [[[_parts objectAtIndex:i] objectForKey:SVMXDEV__Billable_Line_Price__c] floatValue];
                ...
                if (self.shouldShowBillableQty) {
                    quantity = [[_parts objectAtIndex:i] objectForKey:SVMXDEV__Billable_Quantity__c];
                }
                else {
...

    {
        ...
        for (int j = 0; j < [_labor count]; j++)
        {
            ...
            tLabor = [labor objectForKey:SVMXDEV__Activity_Type__c];
            tHours = [labor objectForKey:SVMXDEV__Actual_Quantity2__c];
            if (![tHours isKindOfClass:[NSString class]])
                tHours = @"0.0";
            tRate = [labor objectForKey:SVMXDEV__Actual_Price2__c];
            ...
            /* 6773 */
            if (self.shouldShowBillablePrice) {
                double someValue = [[labor objectForKey:SVMXDEV__Billable_Line_Price__c] doubleValue];
                tLinePrice = [NSString stringWithFormat:@"%.2f", someValue];
            }
            
            /* 6773 */
            if (self.shouldShowBillableQty) {
                tHours = [labor objectForKey:SVMXDEV__Billable_Quantity__c];
                if (![tHours isKindOfClass:[NSString class]])
                    tHours = @"0.0";
            }
...

    {
        ...
        for (int k = 0; k < [_expenses count]; k++)
        {
            ...
            NSString * expenseType = [expenses objectForKey:@"SVMXDEV__Expense_Type__c"];
            if (![expenseType isKindOfClass:[NSString class]])
                expenseType = @"";
            
            NSString * actualPrice = [expenses objectForKey:@"SVMXDEV__Actual_Price2__c"];
            if (![actualPrice isKindOfClass:[NSString class]])
                actualPrice = @"0.0";
            NSString * expenseQty = [expenses objectForKey:@"SVMXDEV__Actual_Quantity2__c"];
            ...
            /* 6773 */
            if (self.shouldShowBillablePrice) {
                double someDoubleValue =  [[expenses objectForKey:SVMXDEV__Billable_Line_Price__c] doubleValue];
        ...
        SMLog(kLogLevelVerbose,@"travel array %d",[travelArray count]);
        if([travelArray count] >0) {
            [self setTravelImage];
        }
        for (int t = 0; t < [travelArray count]; t++)
        {
            SMLog(kLogLevelVerbose,@"%@", [travelArray objectAtIndex:t]);
        
            float linePrice = [[[travelArray objectAtIndex:t] objectForKey:@"SVMXDEV__Actual_Price2__c"] floatValue];
            linePrice *= [[[travelArray objectAtIndex:t] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] floatValue];
            ...
            if (self.shouldShowBillablePrice) {
                linePrice = [[obj objectForKey:SVMXDEV__Billable_Line_Price__c] floatValue];
            }
           
            NSString *quantity = nil;
            
            if (self.shouldShowBillableQty) {
                quantity = [obj objectForKey:SVMXDEV__Billable_Quantity__c];
            }
            else {
                quantity = [[travelArray objectAtIndex:t] objectForKey:@"SVMXDEV__Actual_Quantity2__c"];
            }
            
            NSString *unitPrice = [[travelArray objectAtIndex:t] objectForKey:@"SVMXDEV__Actual_Price2__c"];
            if (unitPrice == nil) {
                unitPrice = @"0.0";
            }
 ...
 @catch (NSException *exp) {
	SMLog(kLogLevelError,@"Exception Name PDFCreator :createMainPage %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PDFCreator :createMainPage %@",exp.reason);
    }
...
- (NSDictionary *) getObjectForKey:(NSString *)key
{
	...
	SMLog(kLogLevelError,@"Exception Name PDFCreator :getObjectForKey %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PDFCreator :getObjectForKey %@",exp.reason);
    }
...
- (NSString*) getValueFromDisplayValue:(NSString *)displayValue
{
	...
	SMLog(kLogLevelError,@"Exception Name PDFCreator :getValueFromDisplayValue %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PDFCreator :getValueFromDisplayValue %@",exp.reason);
    }
...
- (void) createCustomFields
{
    ...
    if(isWorkinginOFFline)
    {
        ...
        for (int i = 0; i < [customFields count]; i++)
        {
            for (int j = 0; j < [appDelegate.WorkDescription  count]; j++)
            {
                ...
                {
                   // SMLog(kLogLevelVerbose,@"%@ :: %@ (%@) -> %@", [field name], [field label], [field type], [reportEssentialsDict objectForKey:[field name]]);
                ...
                if ([[customFields objectAtIndex:i] isEqualToString:[dict objectForKey:@"api_name"]])
                {
                   // SMLog(kLogLevelVerbose,@"%@ :: %@ (%@) -> %@", [field name], [field label], [field type], [reportEssentialsDict objectForKey:[field name]]);
                    ...
                    else if ([[dict objectForKey:@"type"] isEqualToString:@"longtext"] || [[dict objectForKey:@"type"] isEqualToString:@"textarea"])
                    {
                        ...
                        //On iOS 7, the text gets truncated when they have \r or \n in them. The contentview is not resized even though the textView is resized.
                        if (![Utility notIOS7])
                        {
                            NSArray *arrayView=[wp subviews];
                            for (UIView *view in arrayView)
                            {
                                view.frame=CGRectMake(0, 0, pageRect.size.width - startingPoint.x - customTextSize.width, textsize.height+30);
                            }
                        ...
                        SMLog(kLogLevelVerbose,@"%f, %f", currentPoint.x, currentPoint.y);
    ...
    SMLog(kLogLevelError,@"Exception Name PDFCreator :createCustomFields %@",exp.name);
    SMLog(kLogLevelError,@"Exception Reason PDFCreator :createCustomFields %@",exp.reason);
    }
...
- (NSString *) getLogoFromDatabase
{
	...
	SMLog(kLogLevelError,@"Exception Name PDFCreator :getLogoFromDatabase %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PDFCreator :getLogoFromDatabase %@",exp.reason);
    }
...
- (void) setSignature
{
	...
	SMLog(kLogLevelError,@"Exception Name PDFCreator :setSignature %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PDFCreator :setSignature %@",exp.reason);
    }
...
//    SMLog(kLogLevelVerbose,@"%S", (const wchar_t *)workOrderNumberText);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (![Utility notIOS7])
    {
        ...
        for (UIView *view in arrayView)
        {
            view.frame=CGRectMake(0, 0, 230, textsize.height+40);
        }
    ...
    lastHeight = imageRect.size;
        ...
        SMLog(kLogLevelError,@"Exception Name PDFCreator :setAccount %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason PDFCreator :setAccount %@",exp.reason);
...
- (void) setDescription:(NSString *)description;
{
    ...
    if (![Utility notIOS7])
    {
        ...
        for (UIView *view in arrayView)
        {
            view.frame=CGRectMake(0, 0, 600, textsize.height+30);
        }
...
- (void) setWorkPerformed:(NSString *)workPerformed;
{
    ...
    SMLog(kLogLevelVerbose,@"Y = %f",currentPoint.y);
    if(yCoOrdinate < 0)
    {
        [self PDFPageEnd];
        [self PDFPageBegin];        
    ...
    SMLog(kLogLevelVerbose,@"Y = %f",yCoOrdinate);
