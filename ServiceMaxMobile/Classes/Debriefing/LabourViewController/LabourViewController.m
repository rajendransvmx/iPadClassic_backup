...
- (void)viewDidLoad
{
    ...
    */
	...
	NSLog(@"Exception Name LabourViewController :viewDidLoad %@",exp.name);
	NSLog(@"Exception Reason LabourViewController :viewDidLoad %@",exp.reason);
...
- (void) InitLaborData
{    
	// NSString *query = [NSString stringWithFormat:@"SELECT Id, SVMXC__Activity_Type__c, SVMXC__Actual_Quantity2__c, SVMXC__Actual_Price2__c, SVMXC__Work_Description__c FROM SVMXC__Service_Order_Line__c WHERE SVMXC__Line_Type__c = 'Labor' AND SVMXC__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SVMXC__Actual_Quantity2__c > 0 AND SVMXC__Actual_Price2__c > 0", AppDelegate.currentWorkOrderId, AppDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
    
    //pavaman 1st Jan 2011 - We are not saving Actual Price. So why are we filtering on non-zero Actual Price?
	//samman's version: //NSString *query = [NSString stringWithFormat:@"SELECT Id, SVMXC__Activity_Type__c, SVMXC__Actual_Quantity2__c, SVMXC__Actual_Price2__c, SVMXC__Work_Description__c FROM SVMXC__Service_Order_Line__c WHERE SVMXC__Line_Type__c = 'Labor' AND SVMXC__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SVMXC__Actual_Quantity2__c > 0 AND SVMXC__Actual_Price2__c > 0", AppDelegate.currentWorkOrderId, AppDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
	NSString * query = [NSString stringWithFormat:@"SELECT Id, SVMXC__Activity_Type__c, SVMXC__Actual_Quantity2__c, SVMXC__Actual_Price2__c, SVMXC__Work_Description__c FROM SVMXC__Service_Order_Line__c WHERE SVMXC__Line_Type__c = 'Labor' AND SVMXC__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SVMXC__Actual_Quantity2__c > 0 AND SVMXC__Actual_Price2__c >= 0", appDelegate.currentWorkOrderId, appDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
...
- (void) getExistingLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	{        
		...
		value = [[[obj fields] objectForKey:@"SVMXC__Actual_Price2__c"] isKindOfClass:[NSString class]]?[[obj fields] objectForKey:@"SVMXC__Actual_Price2__c"]:@"";
		if ( key != nil )
...
//pavaman 21st Jan 2011 - Always clear the LabourValuesDictionary so that old ones are completely removed
	...
	*/
    ...
    if (![appDelegate.workOrderCurrency isEqualToString:@""])
        query = [NSString stringWithFormat:@"SELECT SVMXC__Billable_Cost2__c FROM SVMXC__Service_Group_Costs__c	WHERE SVMXC__Group_Member__c = '%@' AND SVMXC__Cost_Category__c = 'Straight' AND CurrencyIsoCode = '%@' LIMIT 1", appDelegate.appTechnicianId, workOrderCurrency];
    else
	...
	NSLog(@"Exception Name LabourViewController :getExistingLabor %@",exp.name);
	NSLog(@"Exception Reason LabourViewController :getExistingLabor %@",exp.reason);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void) getPriceForLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	if( array != nil && ![array count] )
	{
        ...
        NSString * workOrderCurrency = [appDelegate.workOrderCurrency stringByReplacingOccurrencesOfString:@" " withString:@""];
		if (![appDelegate.workOrderCurrency isEqualToString:@""])
			query = [NSString stringWithFormat:@"SELECT SVMXC__Billable_Cost2__c FROM SVMXC__Service_Group_Costs__c WHERE SVMXC__Service_Group__c = '%@' AND SVMXC__Cost_Category__c = 'Straight' AND CurrencyIsoCode = '%@' LIMIT 1", appDelegate.appServiceTeamId, workOrderCurrency];
		else
			query = [NSString stringWithFormat:@"SELECT SVMXC__Billable_Cost2__c FROM SVMXC__Service_Group_Costs__c WHERE SVMXC__Service_Group__c = '%@' AND SVMXC__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appServiceTeamId];
	...
	NSLog(@"Exception Name LabourViewController :getPriceForLabor %@",exp.name);
	NSLog(@"Exception Reason LabourViewController :getPriceForLabor %@",exp.reason);
	  [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
@try{
    ...
    */
	...
	dataloaded = YES;
        ...
        NSLog(@"Exception Name LabourViewController :getPriceForServiceTeam %@",exp.name);
        NSLog(@"Exception Reason LabourViewController :getPriceForServiceTeam %@",exp.reason);
       [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
