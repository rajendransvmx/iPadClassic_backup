...
- (void) InitLaborData
{    
	// NSString *query = [NSString stringWithFormat:@"SELECT Id, SVMXDEV__Activity_Type__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Labor' AND SVMXDEV__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SVMXDEV__Actual_Quantity2__c > 0 AND SVMXDEV__Actual_Price2__c > 0", AppDelegate.currentWorkOrderId, AppDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
    
    //pavaman 1st Jan 2011 - We are not saving Actual Price. So why are we filtering on non-zero Actual Price?
	//samman's version: //NSString *query = [NSString stringWithFormat:@"SELECT Id, SVMXDEV__Activity_Type__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Labor' AND SVMXDEV__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SVMXDEV__Actual_Quantity2__c > 0 AND SVMXDEV__Actual_Price2__c > 0", AppDelegate.currentWorkOrderId, AppDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
	NSString * query = [NSString stringWithFormat:@"SELECT Id, SVMXDEV__Activity_Type__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Labor' AND SVMXDEV__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SVMXDEV__Actual_Quantity2__c > 0 AND SVMXDEV__Actual_Price2__c >= 0", appDelegate.currentWorkOrderId, appDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
...
- (void) getExistingLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	{        
        ...
        NSString *key, *value;
		key = [[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"];
		...
		SMLog(kLogLevelVerbose,@"%@", [[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"]);
		[md setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:[[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"]];
        
		value = [[[obj fields] objectForKey:@"SVMXDEV__Work_Description__c"] isKindOfClass:[NSString class]]?[[obj fields] objectForKey:@"SVMXDEV__Work_Description__c"]:@"";
		if ( key != nil )
		...
		value = [[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] isKindOfClass:[NSString class]]?[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"]:@"";
		if ( key != nil )
...
//pavaman 21st Jan 2011 - Always clear the LabourValuesDictionary so that old ones are completely removed
	...
	*/
    ...
    if (![appDelegate.workOrderCurrency isEqualToString:@""])
        query = [NSString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c	WHERE SVMXDEV__Group_Member__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight' AND CurrencyIsoCode = '%@' LIMIT 1", appDelegate.appTechnicianId, workOrderCurrency];
    else
        query = [NSString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c	WHERE SVMXDEV__Group_Member__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appTechnicianId]; // a0oA0000000UXEsIAO
...
- (void) getPriceForLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	if( array != nil && ![array count] )
	{
        ...
        NSString * workOrderCurrency = [appDelegate.workOrderCurrency stringByReplacingOccurrencesOfString:@" " withString:@""];
		if (![appDelegate.workOrderCurrency isEqualToString:@""])
			query = [NSString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c WHERE SVMXDEV__Service_Group__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight' AND CurrencyIsoCode = '%@' LIMIT 1", appDelegate.appServiceTeamId, workOrderCurrency];
		else
			query = [NSString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c WHERE SVMXDEV__Service_Group__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appServiceTeamId];
