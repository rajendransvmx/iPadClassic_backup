...
//pavaman 26th Feb 2011 - adding discount field to the query string
	NSString *query = [NSString stringWithFormat:@"SELECT Id, SPR14__Product__c, SPR14__Product__r.Name, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c, SPR14__Discount__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Parts' AND SPR14__Service_Order__c = '%@' AND RecordTypeId = '%@' AND SPR14__Actual_Quantity2__c > 0 AND SPR14__Actual_Price2__c >= 0", appDelegate.currentWorkOrderId, appDelegate.usageConsumptionRecordId]; // a0oA0000000UXEsIAO
...
- (void) getParts:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		//SMLog(kLogLevelVerbose,@"SPR14__Product__r.Name = %@", [[[[obj fields] objectForKey:@"SPR14__Product__r"] fields] objectForKey:@"Name"] );
		SMLog(kLogLevelVerbose,@"SPR14__Product__c = %@", [[obj fields] objectForKey:@"SPR14__Product__c"] );
        NSMutableDictionary *Part = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
        
        ZKSObject * obj2 = [[obj fields] objectForKey:@"SPR14__Product__r"];
        NSDictionary * SPR14__Product__r = [obj2 isKindOfClass:[NSNull class]]?nil:[obj2 fields];
        if (SPR14__Product__r != nil)
        {
            NSString * partName = [[SPR14__Product__r objectForKey:@"Name"] isKindOfClass:[NSString class]]?[SPR14__Product__r objectForKey:@"Name"]:@"";
            [Part setObject:partName forKey:@"Name"];
        }
        
        NSString * numPartsUsed = [dict objectForKey:@"SPR14__Actual_Quantity2__c"];
        ...
        NSString * description = [dict objectForKey:@"SPR14__Work_Description__c"];
        ...
        if ([Part objectForKey:KEY_COSTPERPART] == nil)
            [Part setObject:[[obj fields] objectForKey:@"SPR14__Actual_Price2__c"] forKey:KEY_COSTPERPART];
        
        if ([Part objectForKey:KEY_PRODUCTID] == nil)
            [Part setObject:[[obj fields] objectForKey:@"SPR14__Product__c"] forKey:KEY_PRODUCTID];
		
		//pavaman 26th Feb 2011 - adding discount field
        NSString * discount = [dict objectForKey:@"SPR14__Discount__c"];
...
- (void) getProductPrices:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    NSString * _query = [NSString stringWithFormat:@"SELECT Id, Name, ProductCode, Family, SPR14__Product_Line__c FROM Product2 WHERE Id IN %@", appDelegate.productIdList];
