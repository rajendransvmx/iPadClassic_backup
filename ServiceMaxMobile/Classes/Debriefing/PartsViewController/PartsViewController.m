...
- (void)viewDidLoad
{
	...
	SMLog(kLogLevelError,@"Exception Name PartsViewController :viewDidLoad %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PartsViewController :viewDidLoad %@",exp.reason);
    }
...
- (void) getParts:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		//SMLog(kLogLevelVerbose,@"SVMXDEV__Product__r.Name = %@", [[[[obj fields] objectForKey:@"SVMXDEV__Product__r"] fields] objectForKey:@"Name"] );
		SMLog(kLogLevelVerbose,@"SVMXDEV__Product__c = %@", [[obj fields] objectForKey:@"SVMXDEV__Product__c"] );
        NSMutableDictionary *Part = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
        
        ZKSObject * obj2 = [[obj fields] objectForKey:@"SVMXDEV__Product__r"];
        NSDictionary * SVMXDEV__Product__r = [obj2 isKindOfClass:[NSNull class]]?nil:[obj2 fields];
        if (SVMXDEV__Product__r != nil)
        {
            NSString * partName = [[SVMXDEV__Product__r objectForKey:@"Name"] isKindOfClass:[NSString class]]?[SVMXDEV__Product__r objectForKey:@"Name"]:@"";
            [Part setObject:partName forKey:@"Name"];
        }
        
        NSString * numPartsUsed = [dict objectForKey:@"SVMXDEV__Actual_Quantity2__c"];
        ...
        NSString * description = [dict objectForKey:@"SVMXDEV__Work_Description__c"];
        ...
        if ([Part objectForKey:KEY_COSTPERPART] == nil)
            [Part setObject:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:KEY_COSTPERPART];
        
        if ([Part objectForKey:KEY_PRODUCTID] == nil)
            [Part setObject:[[obj fields] objectForKey:@"SVMXDEV__Product__c"] forKey:KEY_PRODUCTID];
		
		//pavaman 26th Feb 2011 - adding discount field
        NSString * discount = [dict objectForKey:@"SVMXDEV__Discount__c"];
	...
	SMLog(kLogLevelError,@"Exception Name PartsViewController :getParts %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason PartsViewController :getParts %@",exp.reason);
	}
...
- (void) getPriceBookId:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		// Check the query. use dictionary value extraction technique, for e.g.
		SMLog(kLogLevelVerbose,@"Pricebook Id = %@", [[obj fields] objectForKey:@"Id"]);
...
- (void) getProductPrices:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    NSString * _query = [NSString stringWithFormat:@"SELECT Id, Name, ProductCode, Family, SVMXDEV__Product_Line__c FROM Product2 WHERE Id IN %@", appDelegate.productIdList];
...
- (void) didGetProductDetails:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int i = 0; i < [partsArray count]; i++)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name PartsViewController :didGetProductDetails %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason PartsViewController :didGetProductDetails %@",exp.reason);
    }
	@finally {
    appDelegate.Parts = [Parts retain];
    [PartsTable reloadData];
