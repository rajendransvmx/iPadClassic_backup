...
- (void)viewDidLoad
{
	...
	NSLog(@"Exception Name PartsViewController :viewDidLoad %@",exp.name);
	NSLog(@"Exception Reason PartsViewController :viewDidLoad %@",exp.reason);
    }
...
- (void) getParts:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	NSLog(@"Exception Name PartsViewController :getParts %@",exp.name);
	NSLog(@"Exception Reason PartsViewController :getParts %@",exp.reason);
	}
...
- (void) didGetProductDetails:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int i = 0; i < [partsArray count]; i++)
    {
        ...
        NSLog(@"Exception Name PartsViewController :didGetProductDetails %@",exp.name);
        NSLog(@"Exception Reason PartsViewController :didGetProductDetails %@",exp.reason);
    }
	@finally {
    appDelegate.Parts = [Parts retain];
    [PartsTable reloadData];
