...
- (void)viewDidLoad
{
    ...
    workPerformedView.text = [[workOrderDetails objectForKey:SVMXDEVWORKPERFORMED] isKindOfClass:[NSString class]]?[workOrderDetails objectForKey:SVMXDEVWORKPERFORMED]:@"";
...
- (IBAction) createPDF;
{
   /* if (![appDelegate isInternetConnectionAvailable])
...

    {
        ...
        pdfCreator._workPerformed = [[workOrderDetails objectForKey:SVMXDEVWORKPERFORMED] isKindOfClass:[NSString class]]?[workOrderDetails objectForKey:SVMXDEVWORKPERFORMED]:@"";
...
- (void) setTotalCost
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            NSString * actualPrice = [dict valueForKey:SVMXDEV__Actual_Price2__c];
            if (![actualPrice isKindOfClass:[NSString class]])
                actualPrice = @"0.0";
            
            NSString * actualQuantity = [dict valueForKey:SVMXDEV__Actual_Quantity2__c];
            if (![actualQuantity isKindOfClass:[NSString class]])
                actualQuantity = @"0.0";

            double cost = [actualPrice floatValue] * [actualQuantity floatValue];
             if(self.shouldShowBillablePrice){
                 double billableValue = [[dict valueForKey:SVMXDEV__Billable_Line_Price__c] doubleValue];
                 totalCost += billableValue;
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        NSString * actualPrice = [dict objectForKey:SVMXDEV__Actual_Price2__c];
        if (![actualPrice isKindOfClass:[NSString class]])
            actualPrice = @"0.0";
        
        NSString * actualQuantity = [dict objectForKey:SVMXDEV__Actual_Quantity2__c];
        if (![actualQuantity isKindOfClass:[NSString class]])
            actualQuantity = @"0.0";
...
{    
	...
	switch (indexPath.section)
	{
        case 0:
        {
			NSString * wfText = [[workOrderDetails objectForKey:SVMXDEVWORKPERFORMED] isKindOfClass:[NSString class]]?[workOrderDetails objectForKey:SVMXDEVWORKPERFORMED]:@"";
            
            if ([reportEssentials count] > 0)
            {
                NSDictionary * dict = [reportEssentials objectAtIndex:0];
                wfText = [[dict objectForKey:SVMXDEVWORKPERFORMED] isKindOfClass:[NSString class]]?[dict objectForKey:SVMXDEVWORKPERFORMED]:@"";
            }
		...
		case 2:
		{
			...
			labourcell.Labour.text = [[dict valueForKey:@"SVMXDEV__Activity_Type__c"] isKindOfClass:[NSString class]]?[dict valueForKey:@"SVMXDEV__Activity_Type__c"]:@"";
            NSString * laborRateStr = [dict valueForKey:@"SVMXDEV__Actual_Price2__c"];
            ...
            NSString * laborRateHours = [dict valueForKey:@"SVMXDEV__Actual_Quantity2__c"];
            if (![laborRateHours isKindOfClass:[NSString class]])
                laborRateHours = @"";
            
            /* 6773 */
            if (self.shouldShowBillableQty) {
                NSString *someValue =  [dict valueForKey:SVMXDEV__Billable_Quantity__c];
		...
		//	float cost = [laborRateStr intValue] * [laborRateHours floatValue];
            ...
            /* 6773 */
            if (self.shouldShowBillablePrice) {
                double billableCost =  [[dict valueForKey:SVMXDEV__Billable_Line_Price__c] doubleValue];
		...
		case 3:
		{
            ...
            NSString * expenseType = [dict objectForKey:@"SVMXDEV__Expense_Type__c"];
            NSString * expenseQty = [dict valueForKey:@"SVMXDEV__Actual_Quantity2__c"];
            ...
            NSString * linePrice = [dict objectForKey:@"SVMXDEV__Actual_Price2__c"];
        ...
        case 4:
        {
            ...
            if(self.shouldShowBillableQty){
                travelcell.Qty.text = [[dict valueForKey:SVMXDEV__Billable_Quantity__c] isKindOfClass:[NSString class]]?[dict valueForKey:SVMXDEV__Billable_Quantity__c]:@"";
            }
            else {
                travelcell.Qty.text = [[dict valueForKey:SVMXDEV__Actual_Quantity2__c] isKindOfClass:[NSString class]]?[dict valueForKey:SVMXDEV__Actual_Quantity2__c]:@"";
            }
			
            float costPerPart = 0.0;
          
            costPerPart = [[dict objectForKey:SVMXDEV__Actual_Price2__c] floatValue];
            travelcell.UnitPrice.text = [self getFormattedCost:costPerPart];
            
            
            NSString * keyPartsUsedStr = [dict valueForKey:SVMXDEV__Actual_Quantity2__c];
            if (![keyPartsUsedStr isKindOfClass:[NSString class]])
                keyPartsUsedStr = @"";
            
            NSString * billablePrice = [dict valueForKey:SVMXDEV__Billable_Line_Price__c];
