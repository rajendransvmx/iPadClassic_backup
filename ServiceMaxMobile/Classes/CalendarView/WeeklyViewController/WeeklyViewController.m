...
- (void)contiuePopulateWeekView {
    ...
    @try{
        ...
        NSLog(@"Exception Name WeeklyViewController :populateWeekView %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :populateWeekView %@",exp.reason);
    }
	@finally {
    [activity stopAnimating];
	}
...
- (NSArray *) getWeekStartEndDatesAtOptionalIndex:(NSString *)optionalIndex
{
	...
	NSLog(@"Exception Name WeeklyViewController :getWeekStartEndDatesAtOptionalIndex %@",exp.name);
	NSLog(@"Exception Reason WeeklyViewController :getWeekStartEndDatesAtOptionalIndex %@",exp.reason);
    }
...
- (void) setDays
{
	...
	NSLog(@"Exception Name WeeklyViewController :setDays %@",exp.name);
	NSLog(@"Exception Reason WeeklyViewController :setDays %@",exp.reason);
    }
...
- (void) setDaysAtSliderLocationIndex:(NSUInteger)index
{
   ...
   sunday.text = [array objectAtIndex:0];
    ...
    [self setupEvents];
        ...
        NSLog(@"Exception Name WeeklyViewController :setDaysAtSliderLocationIndex %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :setDaysAtSliderLocationIndex %@",exp.reason);
    }
}
...
- (NSString *) getFirstLastFromWeek:(NSMutableArray *)array
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        NSLog(@"Exception Name WeeklyViewController :getFirstLastFromWeek %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :getFirstLastFromWeek %@",exp.reason);
    }
...
- (void) setupEvents
{
 ...
 //   NSLog(@"%@ %@", startDate, endDate);
    ...
    //[appDelegate.wsInterface getEventsForStartDate:startDate EndDate:endDate];
        ...
        NSLog(@"Exception Name WeeklyViewController :setupEvents %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :setupEvents %@",exp.reason);
    }
...
- (void) clearWeekView
{
    ...
    [weeklyEventPositionArray removeAllObjects];
        ...
        NSLog(@"Exception Name WeeklyViewController :clearWeekView %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :clearWeekView %@",exp.reason);
    }
...
- (void) didQueryWorkOrder:(NSDictionary *)dictionary
{
	...
	didLoadWeekData = TRUE;
            ...
            NSLog(@"Exception Name WeeklyViewController :didQueryWorkOrder %@",exp.name);
            NSLog(@"Exception Reason WeeklyViewController :didQueryWorkOrder %@",exp.reason);
    }
...
- (void) movedEvent:(WeeklyViewEvent *)event
{
    ...
    updatestartDateTime = [updatestartDateTime stringByReplacingOccurrencesOfString:@"Z" withString:@""];
        ...
        NSLog(@"Exception Name WeeklyViewController :movedEvent %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :movedEvent %@",exp.reason);
    }
     @finally {
    [self enableUI];
	 }
...
//            else
    ...
    if ([touch view] == calEvent.view)
    {
        ...
        NSLog(@"Exception Name WeeklyViewController :touchesEnded %@",exp.name);
        NSLog(@"Exception Reason WeeklyViewController :touchesEnded %@",exp.reason);
    }
