...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    SMLog(@"VIEW WILL APPEAR IN WEEKVIEW");
...
- (void)contiuePopulateWeekView {
    ...
    @synchronized(self){
    SMLog(@"populateWeekView %d", [appDelegate.wsInterface.eventArray count]);
    @try{
...
{
    SMLog(@"Swipe Recorgnized weeklyview");
}
    ...
    if ([touch view] == calEvent.view)
    {
        ...
        else
        {
            ...
            if(ContinueRescheduling == TRUE)
            {
                ...
                {
                    SMLog(@"Start Time = %@ and End Time = %@",updatestartDateTime,updateendDateTime);
                    //Shrinivas 
                   ...
                   NSString * local_id = [appDelegate.databaseInterface getLocalIdFromSFId:calEvent.eventId tableName:@"Event"];
                    SMLog(@"Local Id = %@ For Event Id = %@",local_id,calEvent.eventId);
                    //sahana 26/Feb
