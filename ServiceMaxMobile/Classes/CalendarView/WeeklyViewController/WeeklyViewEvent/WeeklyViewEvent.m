...
- (void)viewDidLoad
{
    ...
    companyName.text = [workOrderDetail objectForKey:SVMXDEVACCOUNTNAME];
}
...
{
    SMLog(kLogLevelVerbose,@"%@",event);
    SMLog(kLogLevelVerbose,@"%@", workOrderName.text);
...
- (void) moveTo:(CGRect)location
{
   ...
   /* if(![appDelegate isInternetConnectionAvailable])
    ...
    { 
        SMLog(kLogLevelVerbose,@"WeeklyViewEvent moveTo in while loop");
...
- (CGRect) bringEventDownFromLocation:(CGRect)location
{
    ...
    // SMLog(kLogLevelVerbose,@"%f", self.view.frame.size.height);
...
- (CGRect) getIntersectedEventWithLocation:(CGPoint)point
{
   ...
   @try
    ...
    for (int i = 0; i < [eventPositions count]; i++)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WeeklyViewEvent :getIntersectedEventWithLocation %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WeeklyViewEvent :getIntersectedEventWithLocation %@",exp.reason);
...
- (CGRect) getRectForLocation:(CGPoint)point
{
    ...
    // SMLog(kLogLevelVerbose,@"Rect = %d, %d, %d, %d", rect.origin.x, rect.origin.y, rect.size.width, rect.size.height);
    // SMLog(kLogLevelVerbose,@"(X, Y) = %d, %d", x, y);
...
- (BOOL) isEventInRect:(CGRect)_rect
{
    ...
    for (int i = 0; i < [eventPositions count]; i++)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WeeklyViewEvent :isEventInRect %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WeeklyViewEvent :isEventInRect %@",exp.reason);
...
- (BOOL) canMoveToLocation
{
    ...
    for (int i = 0; i < [eventPositions count]; i++)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name WeeklyViewEvent :canMoveToLocation %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason WeeklyViewEvent :canMoveToLocation %@",exp.reason);

    }   
    // SMLog(kLogLevelVerbose,@"Can Move = %@", flag?@"YES":@"NO");
    return flag;
}
