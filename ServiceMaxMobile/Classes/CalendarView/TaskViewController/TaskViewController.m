...
- (void) addNewTask:(NSMutableArray*)task;
{
    ...
    SMLog(kLogLevelVerbose,@"%@", task);
    SMLog(kLogLevelVerbose,@"%@", resultId);
    SMLog(kLogLevelVerbose,@"%@", [task objectAtIndex:1]);
...
- (void) didCreateObjects:(ZKQueryResult *)result error:(NSError *)error context:(id)context;
{
    ...
    SMLog(kLogLevelVerbose,@"%@", [arr objectAtIndex:0]);
