...
{
    SMLog(kLogLevelVerbose,@"%@ %@", name, _name);
    label.text = name;
...
//        else
    ...
    SMLog(kLogLevelVerbose,@"%@ %@", label.text, subject.text);
}
...
- (void) setEvent:(NSString *)event Time:(NSString *)_time Duration:(CGFloat)_duration Color:(UIColor *)color
{
    ...
    duration = _duration;
    label.text = event;
    SMLog(kLogLevelVerbose,@"label = %@  duration = %f, time = %f", label.text, duration, time );
...
- (void) moveTo:(CGRect)location
{
   ...
   /* if (![appDelegate isInternetConnectionAvailable])
    ...
    {
        SMLog(kLogLevelVerbose,@"EventViewController moveTo in while loop");
