...
- (void) didQueryTasksForDate:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    [self performSelector:@selector(didGetAllTasks) withObject:nil afterDelay:0.3];
        ...
        NSLog(@"Exception Name ModalViewController :didQueryTasksForDate %@",exp.name);
        NSLog(@"Exception Reason ModalViewController :didQueryTasksForDate %@",exp.reason);
...
- (void) movedEvent:(EventViewController *)event
{
    ...
    updatestartDateTime = [updatestartDateTime stringByReplacingOccurrencesOfString:@"Z" withString:@""];
        ...
        NSLog(@"Exception Name ModalViewController :movedEvent %@",exp.name);
        NSLog(@"Exception Reason ModalViewController :movedEvent %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];;
    }
} 
...
- (IBAction) ShowMap
{
	...
	NSLog(@"Exception Name ModalViewController :ShowMap %@",exp.name);
	NSLog(@"Exception Reason ModalViewController :ShowMap %@",exp.reason);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void) setDate:(NSUInteger)date
{
    ...
    else
    {
        ...
        NSLog(@"Exception Name ModalViewController :setDate %@",exp.name);
        NSLog(@"Exception Reason ModalViewController :setDate %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
- (void) setEventsView:(NSString *)_date
{
    ...
    for ( int i = 0; i < [appDelegate.wsInterface.eventArray count]; i++ )
    {
        ...
        NSLog(@"Exception Name ModalViewController :setEventsView %@",exp.name);
        NSLog(@"Exception Reason ModalViewController :setEventsView %@",exp.reason);
         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    @finally
    {
...
- (void) showSFMForWeek:(NSDictionary *)event
{
	...
	NSLog(@"Exception Name ModalViewController :showSFMForWeek %@",exp.name);
	NSLog(@"Exception Reason ModalViewController :showSFMForWeek %@",exp.reason);
	[appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void) showSFMWithDayEvent:(NSDictionary *)event
{
    ...
    else
    {
        ...
        NSLog(@"Exception Name ModalViewController :showSFMWithDayEvent %@",exp.name);
        NSLog(@"Exception Reason ModalViewController :showSFMWithDayEvent %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    @finally {
    [activity stopAnimating];
