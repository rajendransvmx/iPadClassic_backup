...
- (void) insertValuesintoSFMProcessTable:(NSMutableArray *) processData
{
    ...
    for (int k=0; k< [ processData count]; k++)
    {
        ...
        if (synchronized_sqlite3_exec(appDelegate.db, [queryStatement UTF8String], NULL, NULL, &err) != SQLITE_OK)
        {
			...
			NSLog(@"METHOD: insertValuesintoSFMProcessTable");
...
- (NSMutableArray *) getResults:(NSString *)object withConfigData:(NSDictionary *)dataForObject
{
    ...
    @try
    {
        ...
        NSLog(@"Exception Name Database :getResults %@",exp.name);
        NSLog(@"Exception Reason Database :getResults %@",exp.reason);
...
-(NSMutableString*)getJoinFields:(NSDictionary*)dict
{
    ...
    @try
    {
       for (int i=0; i<[TableArray count]; i++)
       {
        ...
        NSLog(@"Exception Name Database :getJoinFields %@",exp.name);
        NSLog(@"Exception Reason Database :getJoinFields %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
@try
    ...
    if([criteriaString isEqualToString:[appDelegate.wsInterface.tagsDictionary objectForKey:SFM_CRITERIA_STARTS_WITH]])
    {
        ...
        NSLog(@"Exception Name Database :getSearchCriteriaStringFromUserData %@",exp.name);
        NSLog(@"Exception Reason Database :getSearchCriteriaStringFromUserData %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
//                objectValueWithoutBrace=[objectValueWithoutBrace stringByReplacingOccurrencesOfString:@"False"withString:@"0"];
        ...
        NSLog(@"Exception Name Database :getupdatedToken %@",exp.name);
        NSLog(@"Exception Reason Database :getupdatedToken %@",exp.reason);
         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    return resultDict;
}
...
-(NSMutableDictionary*) occurenceOfBraces:(NSString*)token
{
    ...
    else
    {
        ...
        NSLog(@"Exception Name Database :occurenceOfBraces %@",exp.name);
        NSLog(@"Exception Reason Database :occurenceOfBraces %@",exp.reason);
       [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    return bracesCount;
}
...
-(NSString *)getvalueforReference:(NSDictionary*) tableArray value:(NSString*)value
{
    ...
    if([strType isEqualToString:@"reference"])
    {
        ...
        NSLog(@"Exception Reason Database :getvalueforReference %@ \n reason :%@",exp.name, exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
//                    if(![arrayQueryField containsObject:[NSString stringWithFormat:@"'%@'.%@",object ,ApifieldName]]) //commented - keerti - 7275
            ...
            else
            {
                ...
                NSLog(@"Display Field skipped as no table named %@ ",[NSString stringWithFormat:@"%@.%@",ApiTableName ,ApifieldName]);
            }
        }
...
//                if(![arrayQueryField containsObject:[NSString stringWithFormat:@"'%@'.%@",ApiTableName ,ApifieldName]]) //commented - keerti - 7275
            ...
            {
                NSLog(@"Search Field %@ skipped as no table named ",[NSString stringWithFormat:@"%@.%@",ApiTableName ,ApifieldName]);
            }
        }
...
//                if(![arrayQueryField containsObject:[NSString stringWithFormat:@"'%@'.%@",ApiTableName ,ApifieldName]]) //commented - keerti - 7275
            ...
            {
                NSLog(@"Sort Field %@ skipped as no table named ",[NSString stringWithFormat:@"%@.%@",ApiTableName ,ApifieldName]);
            }
        }
        NSString *queryStatement;
        //for adding search field in the where clause
...
//                                NSDictionary *object = [displayArray objectAtIndex:j-2];
       ...
       /* if(SQLITE_OK != sqlite3_step(statement))
        ...
        NSLog( @"result %@",results);
         ...
         NSLog(@"Exception Name Database :getResultsForSFM name : %@ \n reason : %@",exp.name, exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
//                    objectValue=[objectValue stringByReplacingOccurrencesOfString:@"False"withString:@"0"];
        ...
        NSLog(@"Exception Name Database :getupdatedTokenForSFM %@",exp.name);
        NSLog(@"Exception Reason Database :getupdatedTokenForSFM %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
-(NSMutableString*)getJoinFieldsForSFM:(NSDictionary*)dict
{
    ...
    @try
    {
        ...
        NSLog(@"Exception Name Database :getJoinFieldsForSFM %@",exp.name);
        NSLog(@"Exception Reason Database :getJoinFieldsForSFM %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    return [joinFields autorelease];
}
...
- (void) deleteRecordFromUserGPSTable:(NSString *) localId
{
    ...
    if(localId != nil)
    {
        ...
        {
            NSLog(@"%@", queryStatementGPSLog);
			NSLog(@"METHOD:deleteRecordFromUserGPSTable");
...
- (void) deleteSequenceofTable:(NSString *)tableName 
{
    ...
    if([tblname isEqualToString:tableName])
    {
        ...
        {
            NSLog(@"%@", queryStatement);
			NSLog(@"METHOD:deleteSequenceofTable");
			NSLog(@"ERROR IN DELETE %s", err);
...
-(void) insertrecordIntoUserGPSLog:(NSDictionary *)locationInfo
{
   ...
   char *err;
    ...
    NSString *id_value = [AppDelegate GetUUID];
    ...
    if(synchronized_sqlite3_exec(appDelegate.db, [sql UTF8String], NULL, NULL, &err) != SQLITE_OK)
    {
        ...
        NSLog(@"%@", sql);
		NSLog(@"METHOD: insertrecordIntoUserGPSLog");
        NSLog(@"ERROR IN INSERTING %s", err);
    ...
    if(isInsertedIntoLocationTable)
    {
        ...
        NSLog(@"Exception Name Database :insertrecordIntoUserGPSLog %@",exp.name);
        NSLog(@"Exception Reason Database :insertrecordIntoUserGPSLog %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
{   
    ...
    while (row_count >=[limitLocationRecords intValue])
    {
        ...
        if(strId != nil)
        {
            ...
            {
                NSLog(@"%@", queryStatementGPSLog);
				NSLog(@"METHOD:purgeLocationPingTable");
				NSLog(@"ERROR IN DELETE %s", err);
				[appDelegate printIfError:nil ForQuery:queryStatementGPSLog type:DELETEQUERY];
...
- (void)updateUserGPSLocation
{
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        ...
        NSLog(@"Datebase.m : updateUserGPSLocation: LocationHistory");
#endif
...
- (void)updateTechnicianLocation
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"Datebase.m : updateTechinicianLocation: TECH_LOCATION_UPDATE");
#endif
        ...
        for (int i = 0; i < [processArray count]; i++)
        {
            ...
            if (ret != SQLITE_OK)
            {
                if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
                    [MyPopoverDelegate performSelector:@selector(throwException)];
                NSLog(@"%@", queryStatement);
				NSLog(@"METHOD:insertValuesToProcessTable");
				NSLog(@"ERROR IN INSERTING %s", err);
                /*
				[appDelegate printIfError:nil ForQuery:queryStatement type:INSERTQUERY];
                 */
...
{
    didRcvAttachment = NO;
    NSString * _query = [NSString stringWithFormat:@"SELECT Body,Name, Id FROM Attachment Where Id = '%@'", att_id];
...
- (void)deleteRequiredPdfForAttachement:(NSString *)attachementId {
	...
	if (synchronized_sqlite3_exec(appDelegate.db, [query UTF8String], NULL, NULL, &err) != SQLITE_OK)
	{
        ...
        NSLog(@"METHOD:deleteEventNotRelatedToLoggedInUser");
		NSLog(@"ERROR IN DELETE %s", err);
...
- (void) didGetAttachmentWithResult:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        if (bodyString != nil && [bodyString length] > 0)
        {
            ...
            {
                NSLog(@" ************ Update Attachments query ************ %@", updateQuery);
                NSLog(@"METHOD: Update Attachments table with body of attachment" );
                NSLog(@"ERROR IN UPDATING %s", err);
            ...
            {
                NSLog(@" ************ Update Attachments query ************ %@ \n DIDNOT WORK!!!! ***************** ", updateQuery);
            }
        }
    }
    didRcvAttachment = YES;
...
-(void)UpdateDocumentTemplateId:(NSString *)docTemplateId forProcessId:(NSString *)processId
{
    ...
    if(synchronized_sqlite3_exec(appDelegate.db, [update_statement UTF8String],NULL, NULL, &err) != SQLITE_OK)
    {
		...
		NSLog(@"METHOD:UpdateDocumentTemplateId" );
		NSLog(@"ERROR IN UPDATING %s", err);
...
- (void)requestForStaticResources
{
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        NSLog(@"Static resources library!!");
        if (appDelegate.did_fetch_static_resource_ids == YES)
        {
            NSLog(@"Static resources - SUCCESS!!");
            break;
        }
        if (![appDelegate isInternetConnectionAvailable])
        {
            NSLog(@"Static resources - FAILED due to internet connection unavailability!!");
            return;
        }
        if(appDelegate.connection_error)
        {
            NSLog(@"Static resources - FAILED due to conn error!!");
            return;
        }
    }
...
- (void) didRetrieveImageWithResult:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        if (synchronized_sqlite3_exec(appDelegate.db, [query UTF8String], NULL, NULL, &err) != SQLITE_OK)
        {
            NSLog(@"Document Image data NOT updated to database %s",err);
        }
...
{
    NSLog(@"  insertvaluesToPicklist Processing starts: %@", [NSDate date]);
    ...
    NSLog(@"  insertvaluesToPicklist Processing ends: %@", [NSDate date]);
}
...
- (void) insertValuesInToRTPicklistTableForObject:(id)objects Values:(NSMutableDictionary *)recordTypeDict
{
    ...
    NSLog(@"  insertValuesInToRTPicklistTableForObject Processing starts: %@", [NSDate date]);
...
- (void) insertValuesInToExpressionTables:(NSMutableDictionary *)processDictionary
{
    ...
    if (result == YES)
    {
        ...
        else
        {
            ...
            NSLog(@" SFExpression Prepared statement %@", bulkQueryStmt);
        }
        synchronized_sqlite3_finalize(bulkStmt);
        [self endTransaction];
...
- (void) insertValuesInToTagsTable:(NSMutableDictionary *)tagsDictionary
{
    ...
    NSLog(@"  MetaSync insertValuesInToTagsTable starts: %@", [NSDate date]);
    int id_value = 0;
    ...
    NSLog(@"  MetaSync insertValuesInToTagsTable ends: %@", [NSDate date]);
...
{
    NSLog(@"  MetaSync insertValuesInToSettingsTable processing starts: %@", [NSDate date]);
    int id_value = 0;
    ...
    NSLog(@"  MetaSync insertValuesInToSettingsTable processing ends: %@", [NSDate date]);
    //Radha - 24/March
...
{
    NSLog(@"  MetaSync insertValuesInToSFWizardsTable: processing starts: %@", [NSDate date]);
    int id_value = 0;
    ...
    NSLog(@"  MetaSync insertValuesInToSFWizardsTable: processing ends: %@", [NSDate date]);
    synchronized_sqlite3_finalize(bulkStmt);
    // Vipin-db-optmz
    [self endTransaction];
                ...
                for (int counter = 0; counter < [filterArray count]; counter++) {
                   ...
                   int success =  sqlite3_step(insertStatement);
                    if (success == SQLITE_DONE || success == SQLITE_OK) {
                        NSLog(@"Successfully inserted the Filter ");
                    }
                    else {
                        NSLog(@"Error while inserting the Filter");
                    }
    ...
    if (result == YES)
    {
        ...
        if (ret_value == SQLITE_OK)
        {
            for (int i = 0; i < [filterComponents count]; i++)
            {
                ...
                if (success == SQLITE_DONE || success == SQLITE_OK)
                {
                    NSLog(@"SFEXpressionComponent insertion successful");
                }
                else {
                    NSLog(@"SFEXpressionComponent insertion failed..");
                }
                sqlite3_reset(bulkStmt);
            }
        }
    ...
    if (!flag)
    {
        ...
        if (synchronized_sqlite3_exec(appDelegate.db, [queryStatement UTF8String], NULL, NULL, &err) != SQLITE_OK)
        {
            if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
                [MyPopoverDelegate performSelector:@selector(throwException)];
			NSLog(@"%@", queryStatement);
			NSLog(@"METHOD: insertUsernameToUserTable");
			NSLog(@"ERROR IN INSERTING %s", err);
			[appDelegate printIfError:nil ForQuery:queryStatement type:INSERTQUERY];
        }
    }
    ...
    if(!flag)
    {
        ...
        NSString *queryStatementUpdate = [NSString stringWithFormat:@"update User SET Id='%@',Name='%@' Where Username='%@' ",UserId ,name,userName];
            ...
            if (synchronized_sqlite3_exec(appDelegate.db, [queryStatementUpdate UTF8String], NULL, NULL, &err) != SQLITE_OK)
            {
                if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
                    [MyPopoverDelegate performSelector:@selector(throwException)];
                NSLog(@"%@", queryStatementUpdate);
			NSLog(@"METHOD:updateUserTable " );
			NSLog(@"ERROR IN UPDATING %s", err);
    ...
    if(!flag)
    {
        ...
        if (synchronized_sqlite3_exec(appDelegate.db, [queryStatementUpdate UTF8String], NULL, NULL, &err) != SQLITE_OK)
        {
            if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
                [MyPopoverDelegate performSelector:@selector(throwException)];
            NSLog(@"%@", queryStatementUpdate);
			NSLog(@"METHOD:updateUserLanguage" );
			NSLog(@"ERROR IN UPDATING %s", err);
			[appDelegate printIfError:nil ForQuery:queryStatementUpdate type:UPDATEQUERY];
        }
                ...
                if (retVal)
                {
                    ...
                    {  
                        if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
                            [MyPopoverDelegate performSelector:@selector(throwException)];
                        NSLog(@"%@", query);
						NSLog(@"METHOD: insertDataInToTables");
						NSLog(@"ERROR IN INSERTING %s", err);
                        /*
        ...
        if (reult == SQLITE_OK)
        {
            ...
            if (sqlite3_step(bulkStmt) != SQLITE_DONE)
            {
                ...
                if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
                    [MyPopoverDelegate performSelector:@selector(throwException)];
                NSLog(@"%@", query);
                NSLog(@"METHOD: addvaluesToLookUpFieldTable");
                NSLog(@"ERROR IN INSERTING %s", err);
            }
                        ...
                        if (synchronized_sqlite3_prepare_v2(appDelegate.db, [queryStatement2 UTF8String], -1, &stmt_id, NULL) == SQLITE_OK)
                        {
                            while (synchronized_sqlite3_step(stmt_id) == SQLITE_ROW)
                            {
                                ...
                                if (synchronized_sqlite3_exec(appDelegate.db, [queryStatement3 UTF8String], NULL, NULL, &err) != SQLITE_OK)
                                {
									...
									NSLog(@"METHOD:updateChildSfIdWithParentLocalId" );
									NSLog(@"ERROR IN UPDATING %s", err);
                                    /*
...
- (void) generatePDFSettings
{
	...
	NSLog(@"GeneratePDfSeetings = %@", _query);
...
- (void) didGetSubModuleInfo:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
		...
		NSLog(@"didGetSubModuleInfo = %@", _query);
...
- (void) didGetSettingsInfo:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
       ...
       settingsInfoArray = [[NSMutableArray alloc] initWithCapacity:0];
		...
		NSLog(@"didGetSettingsInfo = %@", _query);
...
- (void) didGetActiveGlobalProfile:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
       ...
       NSString * ActiveGloProInfoId = [[[obj fields] objectForKey:@"Id"] retain];
		...
		NSLog(@"didGetActiveGlobalProfile = %@", _query);
...
{
    NSLog(@"  MetaSync insertSettingsIntoTable processing starts: %@", [NSDate date]);
    NSString * field_string = @"";
...
- (void) doMetaSync
{
    ...
    NSString * retVal = [self callMetaSync];
     ...
     if([appDelegate enableGPS_SFMSearch])
     {
         ...
         {
#ifdef kPrintLogsDuringWebServiceCall
             NSLog(@"Datebase.m : doMetaSync: SFM_SEARCH");
#endif
             if (![appDelegate isInternetConnectionAvailable])
             {
             if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
...
//	[appDelegate setCurrentSyncStatusProgress:METASYNC_SEARCH optimizedSynstate:0];
    if([appDelegate doesServerSupportsModule:kMinPkgForGetPriceModule])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"iPadScrollerViewController.m : doMetaSync: Get Price Objects");
#endif
        ...
        NSLog(@"MetaSync Get Price PRICE_CALC_OBJECTS End: %@", [NSDate date]);
        ...
        if([[appDelegate.wsInterface getValueFromUserDefaultsForKey:@"doesGetPriceRequired"] boolValue])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"iPadScrollerViewController.m : doMetaSync: Get Price Code Snippet");
#endif
            ...
            NSLog(@"MetaSync Get Price PRICE_CALC_CODE_SNIPPET End: %@", [NSDate date]);
        }
    }
...
- (NSString *) callMetaSync
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"Datebase.m : callMetaSync: SFM_METADATA");
#endif
                            ...
                            Action:(NSString*)Action
{
    ...
    {
        NSLog(@"%@", queryStatement);
		NSLog(@"METHOD:purgingDataOnSyncSettings");
		NSLog(@"ERROR IN DELETE %s", err);
		[appDelegate printIfError:nil ForQuery:queryStatement type:DELETEQUERY];
...
- (BOOL) startEventSync
{
    NSLog(@"User location update starts");
    ...
    appDelegate.connection_error = FALSE;
    BOOL retVal = TRUE;
    NSLog(@"  DataSync WS Start: %@", [NSDate date]);
    appDelegate.wsInterface.didOpComplete = FALSE;
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"Datebase.m : startEventSync: EVENT_SYNC");
#endif
        if (![appDelegate isInternetConnectionAvailable])
            break;
...
//	[appDelegate setCurrentSyncStatusProgress:eEVENTSYNC_GETID optimizedSynstate:0];
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"Datebase.m : startEventSync: TX_FETCH");
#endif
...
- (void) insertMetaSyncDue:(NSString *)description
{
    ...
    else 
    {
        ...
        {
            NSLog(@"%@", query);
			NSLog(@"METHOD: insertMetaSyncDue");
			NSLog(@"ERROR IN INSERTING %s", err);
			[appDelegate printIfError:nil ForQuery:query type:INSERTQUERY];
        }
    }
...
- (BOOL) getImageForServiceReportLogo
{
	...
	NSLog(@"getImageForServiceReportLogo = %@", _query);
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"Datebase.m : getImageForServiceReportLogo: ZKs ServiceReportLogo");
#endif
...
//        [fileManager createFileAtPath:filePath contents:data attributes:nil];
        ...
        {
            NSLog(@"%@", query);
			NSLog(@"METHOD: didGetServiceReportLogo");
			NSLog(@"ERROR IN INSERTING %s", err);
			[appDelegate printIfError:nil ForQuery:query type:INSERTQUERY];
        }
    }
...
-(void)getRecordTypeValuesForObject:(NSArray *)allObjects
{
	...
	NSLog(@"getRecordTypeValuesForObject = %@", _query);
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"Datebase.m : getRecordTypeValuesForObject: ZKs get record type");
#endif
...
-(void)UpdateSFRecordTypeForId:(NSString *)_id value:(NSString *)valueField
{
    ...
    {
        NSLog(@"%@", update_statement);
		NSLog(@"METHOD:UpdateSFRecordTypeForId" );
		NSLog(@"ERROR IN UPDATING %s", err);
		[appDelegate printIfError:nil ForQuery:update_statement type:UPDATEQUERY];
    }
...
- (void) purgeEventsNotRelatedToLoggedInUser:(NSMutableString *)eventId
{
    ...
    {
		NSLog(@"Failed to delete");
        NSLog(@"purgeEventsNotRelatedToLoggedInUser");
		NSLog(@"ERROR IN DELETE %s", err);
    }
...
- (NSString *)expressionErrorMessageById:(NSString *)expressionId
{
    ...
    NSLog(@"  expressionErrorMessageById  Query : %@", queryString);
