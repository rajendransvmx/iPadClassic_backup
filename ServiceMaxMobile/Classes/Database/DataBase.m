   ...
   // [self createTable:query1];
    ...
    for (NSString * objectName in object_names)
    {
        /*if ([objectName isEqualToString:@"Case"])
            objectName = @"'Case'";*/
        //11501
        if ([objectName isEqualToString:@"SVMXC__SVMX_Job_Logs__c"])
        {
...
//11946
- (NSString *)getAllEventsIdsForloggedInUser
{
    NSString * ownerId = [self getLoggedInUserId:appDelegate.currentUserName];
    
    NSMutableString * eventId = nil;
    
    if ([ownerId length] > 0) {
        NSString *selectQuery=[NSString stringWithFormat:@"SELECT Id FROM 'Event' where OwnerId = '%@'", ownerId];
        ...
        if ( synchronized_sqlite3_prepare_v2(appDelegate.db, [selectQuery UTF8String], -1, &statement, nil ) == SQLITE_OK )
        {
            while( synchronized_sqlite3_step(statement) != SQLITE_DONE )
            {
                ...
                if ((sfId !=nil) && strlen(sfId))
                {
                    NSString *Id = [NSString stringWithUTF8String:sfId];
                    [array addObject:Id];
                }
            }
        }
        ...
        if ([array count] > 0) {
            ...
            eventId = [NSMutableString stringWithFormat:@"'%@'", value];
        }
    }
    return eventId;
}
...
//11946
- (void)purgeAllEventsRelatedToLoggedInUser:(NSString *)eventId
{
	NSString * queryStatement = nil;
	
	queryStatement = [NSString stringWithFormat:@"DELETE from Event WHERE  Id IN (%@)", eventId];
    ...
    [self deleteUserEventsRelatedRecordsFromSyncTable:eventId];
}

//11946
- (void)deleteUserEventsRelatedRecordsFromSyncTable:(NSString *)ids
{
    [self purgeEventRelatedLoggedInUserRecordFromSyncTables:SFDATATRAILER_TEMP data:ids];
    [self purgeEventRelatedLoggedInUserRecordFromSyncTables:SFDATATRAILER data:ids];
    [self purgeEventRelatedLoggedInUserRecordFromSyncTables:SYNC_ERROR_CONFLICT data:ids];
}

//11946
- (void)purgeEventRelatedLoggedInUserRecordFromSyncTables:(NSString *)tableName data:(NSString *)ids
{
    NSString * deleteQuery = [[NSString alloc] initWithFormat:@"DELETE FROM '%@' WHERE sf_id  IN (%@) AND object_name = 'Event'", tableName, ids];
