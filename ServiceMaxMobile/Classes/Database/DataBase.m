...
- (int)beginTransaction
{
    if (self.dbTransactionCount > 0)
    {
        ...
        SMLog(kLogLevelVerbose,@"[TXN] + db_txn_count - %d", self.dbTransactionCount);
    ...
    {
        SMLog(kLogLevelVerbose,@"[TXN] + db_txn begin failed : %d : %@", executionResult, [NSString stringWithUTF8String:err]);
    }
    else
    {
        // Lets record as first transaction
        self.dbTransactionCount = 1;
        SMLog(kLogLevelVerbose,@"[TXN] + db_txn_count - %d - begin", self.dbTransactionCount);
    }
...
- (int)endTransaction
{
    ...
    if (executionResult != SQLITE_OK )
    {
        SMLog(kLogLevelError,@"[TXN] - db_txn end failed : %d : %@", executionResult, [NSString stringWithUTF8String:err]);
    }
    else
    {
        // Lets reset to Zero since we closed all transaction safely.
        self.dbTransactionCount = 0;
        SMLog(kLogLevelVerbose,@"[TXN] - db_txn_count - %d - commit", self.dbTransactionCount);
    }
...
- (void)cleanupDatabase
{
    ...
    if ( executionResult == SQLITE_OK)
    {
        SMLog(kLogLevelVerbose,@"Vacuumed DataBase");
        SMLog(kLogLevelVerbose,@"[DB] Vacuumed DataBase");
    }
    else
    {
...
- (NSString *)evaluateLiteral:(NSString *)literal forControlType:(NSString *)controlType
{
    ...
    if ([controlType isEqualToString:@"date"] || [controlType isEqualToString:@"datetime"])
    {
        ...
        {
            //9122
            evaluatedLiteral = [self formatLiteralForDateOnly:[Utility today:0 andJusDate:YES]];
        }
        else if([literal caseInsensitiveCompare:MACRO_TOMMOROW]== NSOrderedSame)
        {
            //9122
            evaluatedLiteral = [self formatLiteralForDateOnly:[Utility today:1 andJusDate:YES]];
        }
...
{
    NSString *query = [NSString stringWithFormat:@"CREATE TABLE SFAttachmentTrailer ('local_id' INTEGER PRIMARY KEY  NOT NULL  DEFAULT (0) ,'priority' INTEGER,'attachment_id' VARCHAR,'parent_localid' VARCHAR,'parent_sfid' VARCHAR ,'file_name' VARCHAR,type 'VARCHAR','size' INTEGER,'status' VARCHAR,'action' VARCHAR,'per_progress' FLOAT)"];

    [self createTable:query];
}
- (void) createTableForAttachmentErrors
{
    NSString *query = [NSString stringWithFormat:@"CREATE TABLE AttachmentError ('local_id' INTEGER PRIMARY KEY  NOT NULL  DEFAULT (0) ,'attachment_id' VARCHAR,'error_message' VARCHAR,'file_name' VARCHAR,'Sync_Flag' VARCHAR ,'type' VARCHAR,'parent_localid' VARCHAR,'status' VARCHAR)"];
...
- (NSData *) getAttachmentDataForFile:(NSString *)fileName
{
    ...
    {
        SMLog(@"Unable to execute the Statement = %@",query);
    }
...
//    NSString *selectQuery=[NSString stringWithFormat:@"SELECT local_id,Name,ContentType,BodyLength,Description,LastModifiedDate FROM Attachment  where (ParentId='%@' or ParentId='%@') ",sf_id,parentId];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [selectQuery UTF8String], -1, &statement, NULL) == SQLITE_OK)
    {
        while(synchronized_sqlite3_step(statement) == SQLITE_ROW)
        {
            ...
            if (size >0)
            {
                bodyLength =(1.0 *size)/1048576; //size in MB
            }
...
{
    NSString * queryStatement = [NSString stringWithFormat:@"DELETE FROM SFAttachmentTrailer where local_id='%@'",attid];
    char * err;
    if (synchronized_sqlite3_exec(appDelegate.db, [queryStatement UTF8String], NULL, NULL, &err) != SQLITE_OK)
    {
...
-(NSDictionary*)getAttachmentLocalIdInfoFromDB:(NSString*)localId
{
    ...
    synchronized_sqlite3_finalize(stmt);
    NSString * keyPrefix = [ParentId substringWithRange:NSMakeRange(0, 3)];
