...
- (int)beginTransaction
{
    ...
    {
        SMLog(@"[TXN] + db_txn begin failed : %d : %@", executionResult, [NSString stringWithUTF8String:err]);
    }
    else
    {
        // Lets record as first transaction
        self.dbTransactionCount = 1;
        SMLog(@"[TXN] + db_txn_count - %d - begin", self.dbTransactionCount);
    }
...
- (int)endTransaction
{
    ...
    if (executionResult != SQLITE_OK )
    {
        SMLog(@"[TXN] - db_txn end failed : %d : %@", executionResult, [NSString stringWithUTF8String:err]);
    }
    else
    {
        // Lets reset to Zero since we closed all transaction safely.
        self.dbTransactionCount = 0;
        SMLog(@"[TXN] - db_txn_count - %d - commit", self.dbTransactionCount);
    }
...
-(NSString *) CreateRandomString:(NSString*)objectName
{
    ...
    if([objectName length]>0 || objectName != nil)
    {
        ...
        SMLog(@"arrayofAlias %@",arrayofAlias);
    }
                   ...
                   andIndexName:(NSString *)indexName
{
    ...
    if (result)
    {
        ...
        SMLog(@" [indx] failure : index query \n %@ ", query);
    }
...
- (void) didGetAttachmentWithResult:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        if (bodyString != nil && [bodyString length] > 0)
        {
            ...
            {
                SMLog(@" ************ Update Attachments query ************ %@", updateQuery);
                SMLog(@"METHOD: Update Attachments table with body of attachment" );
                SMLog(@"ERROR IN UPDATING %s", err);
...
-(void)UpdateDocumentTemplateId:(NSString *)docTemplateId forProcessId:(NSString *)processId
{
    ...
    if(synchronized_sqlite3_exec(appDelegate.db, [update_statement UTF8String],NULL, NULL, &err) != SQLITE_OK)
    {
		...
		SMLog(@"METHOD:UpdateDocumentTemplateId" );
		SMLog(@"ERROR IN UPDATING %s", err);
...
- (void)requestForStaticResources
{
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        SMLog(@"Static resources library!!");
        if (appDelegate.did_fetch_static_resource_ids == YES)
        {
            SMLog(@"Static resources - SUCCESS!!");
            break;
        }
        if (![appDelegate isInternetConnectionAvailable])
        {
            SMLog(@"Static resources - FAILED due to internet connection unavailability!!");
            return;
        }
        if(appDelegate.connection_error)
        {
            SMLog(@"Static resources - FAILED due to conn error!!");
            return;
        }
    }
...
- (void) didRetrieveImageWithResult:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        if (synchronized_sqlite3_exec(appDelegate.db, [query UTF8String], NULL, NULL, &err) != SQLITE_OK)
        {
            SMLog(@"Document Image data NOT updated to database %s",err);
        }
...
- (BOOL) createTable:(NSString *)statement
{
    ...
    {
        isSuccess = NO;
        SMLog(@" ERROR IN createTable \n  stetement: %@ \n %s", statement, err);
        SMLog(@" ERROR in detail  : %@",[NSString stringWithUTF8String:sqlite3_errmsg(appDelegate.db)]);
        if ([MyPopoverDelegate respondsToSelector:@selector(throwException)])
        {
            [MyPopoverDelegate performSelector:@selector(throwException)];
        }
...
- (void) insertDataInToTables:(NSMutableArray *)fieldValueArray
{
    ...
    // Vipin-db-optmz -rm
   // SMLog(@" [OBS] insertDataInToTables  --- starting ...");
...
- (void) generatePDFSettings
{
	...
	SMLog(@"GeneratePDfSeetings = %@", _query);
...
- (void) didGetSubModuleInfo:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
		...
		SMLog(@"didGetSubModuleInfo = %@", _query);
...
- (void) didGetSettingsInfo:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
       ...
       settingsInfoArray = [[NSMutableArray alloc] initWithCapacity:0];
		...
		SMLog(@"didGetSettingsInfo = %@", _query);
...
- (void) didGetActiveGlobalProfile:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([[result records] count] > 0)
    {
       ...
       NSString * ActiveGloProInfoId = [[[obj fields] objectForKey:@"Id"] retain];
		...
		SMLog(@"didGetActiveGlobalProfile = %@", _query);
...
-(void)insertSettingsIntoTable:(NSMutableArray*)array:(NSString*)TableName
{
    ...
    for (NSDictionary * dict in array) 
    {
       ...
       // SMLog(@" [OBSERV] insertSettingsIntoTable :Query  => \n %@", query);
...
-(void)StartIncrementalmetasync
{
    ...
    SMLog(@"  StartIncrementalmetasync  .... st");
...
{  
    ...
    [self endTransaction];
     ...
     SMLog(@"  createBackUpDb  .... releasing main DB");
    // Vipind-db-optmz - 3
    ...
    SMLog(@"  createBackUpDb  .... dettaching  kAttachementNameSFM");
...
- (void) copyTempsqlToSfm
{
   ...
   // [self createTable:query1];
    ...
    {
        SMLog(@"Unable to create attachment for copying temp sql to SFM");
        return;
    }
...
- (void)resetConfigurationForDataBase:(sqlite3 *)database
{
    if (self.dbTransactionCount > 0)
    {
        SMLog(@"Skipping db configuration reset since %d live transaction exist", dbTransactionCount);
        return;
    }
...
-(void)openDB:(NSString *)name type:(NSString *)type sqlite:(sqlite3 *)database
{
    ...
    if (tempDb != nil)
    {
        SMLog(@"[DBOPX] temp database exist.. skipping initialisation of DB");
        return;
    }
    ...
    {
        SMLog(@"[OPMX] Use sqlite on multiple threads, using the same connection");
    }
    else
    {
        SMLog(@"[OPMX] Single connection single thread");
    }
     */
...
- (BOOL) createTemporaryTable:(NSString *)statement
{
    ...
    {
        SMLog(@"[MON] Created createTemporaryTable");
    }
    return YES;
}
...
{
    SMLog(@"  ----- populateDatabaseFromBackUp ----- ");
    ...
    //SMLog(@" [MON]  populateDatabaseFromBackUp : ATTACH DATABASE %@", query1);
    ...
    if (! hasCreatedAttachment)
    {
        SMLog(@"Unable to create attachment for populating database from backup");
        return;
    }
    ...
    for (NSString * tableName in objects)
    {
        ...
        [self endTransaction];
        SMLog(@" Populate database  step 1 end"); 
        [field_type release];
        field_type=nil;
    }
    //Radha 2012june08
   ...
   SMLog(@" Populate database  step 2 start");
   NSArray * tempArray = [self createTempTableForSummaryAndTroubleShooting];
   SMLog(@" Populate database  step 2 end");
   SMLog(@" Populate database  step 3 start");
   [self beginTransaction];
    ...
    {  //008573
        if ([table isEqualToString:@"SFSignatureData"] && [self hasOldSchema:@"SFSignatureData"]) {
            ...
            SMLog(@"FIRST TIME CONFIG SYNC AFTER UPGRADE FROM 11.1.0.0");
    ...
    SMLog(@" Populate database  step 3 end");
...
{
    SMLog(@" DEL deleteDatabase - %@",databaseName);
                            ...
                            Action:(NSString*)Action
{
    ...
    SMLog(@" DEL purgingDataOnSyncSettings - %@",queryStatement);
}
#pragma mark -End
...
- (void) copyMetaTableInToSfm:(NSMutableArray *)metaTable
{
   ...
   // SMLog(@" [MON]  copyMetaTableInToSfm : ATTACH DATABASE %@", query1);
    
    if (! hasCreatedAttachment)
    {
        SMLog(@"Unable to create attachment for copying meta table into SFM table");
        return;
    }
...
- (BOOL) getImageForServiceReportLogo
{
	...
	SMLog(@"getImageForServiceReportLogo = %@", _query);
...
-(void)getRecordTypeValuesForObject:(NSArray *)allObjects
{
	...
	SMLog(@"getRecordTypeValuesForObject = %@", _query);
...
- (int)totalNumberOfOperationCountForDatabase:(sqlite3 *)database
{
    ...
    SMLog(@"[DBOPX] DB  totalChanges %d", totalChanges);
...
- (NSNumber *)dbMemoryUsage
{
    ...
    SMLog(@"[DBOPX] DB usedBytes %lld", usedBytes);
...
- (int)releaseHeapMemoryForDatabase:(sqlite3*)database
{
    if (database != nil)
    {
        ...
        SMLog(@"[DB] db : freedMemoryInbytes    %d ", freedMemoryInbytes);
        SMLog(@"[DB] db : releasedMemoryInbytes %d ", releasedMemoryInbytes);
    }
    else
    {
        SMLog(@"[DB] releaseHeapMemoryForDatabase  - bad momery pointer");
    }
    return 1;
}
...
- (BOOL)attachDatabase:(sqlite3*)database byName:(NSString *)attachmentName andPath:(NSString *)path
{
    ...
    SMLog(@" Database ATTACH now ");
    BOOL isSuccess = YES;
    
    if (database == nil)
    {
        //Skipping database attachment operation since db is Null pointer
        SMLog(@"[DB] Skipping database attachment operation since db is Null pointer!");
        isSuccess = NO;
    ...
    else if ( (! [Util isValidString:path]) || (! [Util isValidString:attachmentName]) )
    {
        ...
        SMLog(@"[DB] Skipping database attachment operation since invalid file path or database name!");
        isSuccess = NO;
    }
    else
    {
        ...
        SMLog(@" attachementQuery  : %@", attachementQuery);
        char * err;
        ...
        if (result != SQLITE_OK)
        {
            ...
            // Vipin 29-July-2013
            SMLog(@" ERROR IN ATTACH DATABASE \n error code : %d,  statement: %@ \n  error message : %s \n detail : %@",
                  result,
                  attachementQuery,
                  err,
...
{
    SMLog(@" Database dettaching now ");
    if (database == nil)
    {
        //Skipping database detachment operation since db is Null pointer
        SMLog(@"[DB] Skipping database detachment operation since db is Null pointer!");
        return NO;
    }
    ...
    if (result != SQLITE_OK)
    {
        ...
        // Vipin 22-July-2013
        SMLog(@" ERROR IN DETACH DATABASE error code : %d, \n  statement: %@ \n  error message : %s \n detail : %@",
              result,
              detachQuery,
              err,
              [NSString stringWithUTF8String:sqlite3_errmsg(database)]);
...
- (BOOL)closeDatabase:(sqlite3*)database
{
    ...
    if (SQLITE_OK != executionResult)
    {
        ...
        SMLog(@" Could not able to close database connection - error code : %d,  message : %@", executionResult, [NSString stringWithUTF8String:sqlite3_errmsg(database)]);
    }
...
- (sqlite3 *)getTempDatabase {
    ...
    if (tempDb == nil)
    {
        SMLog(@"[DBOPX] Temp DB initialising...");
        [self openDB:TEMPDATABASENAME type:DATABASETYPE1 sqlite:nil];
    }
