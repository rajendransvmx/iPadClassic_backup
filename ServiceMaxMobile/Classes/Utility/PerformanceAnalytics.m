      ...
      && ([self.dbOperationCounterDictionary count] > 0))
  {
      ...
      for (NSString *dbName in dbNames)
      {
          ...
          if ( totalReading > 1)
          {
              ...
              SMLog(@" %@   cur : %d  pre : %d  diff : %d", dbName, [currentReading intValue],  [previousReading intValue], diff);
          }
      }
  }
   
  SMLog(@" totalOperations : %lld ", totalOperations);
...
- (void)displayCurrentStatics
{
    ...
    if (self.dbMemoryRecords != nil)
    {
        ...
        for (NSString *key in keys)
        {
            ...
            SMLog(@" %@ : %lld", key, [number longLongValue]);
...
- (void)registerOperationCount:(int)count forDatabase:(NSString *)connectionName
{
    ...
    SMLog(@"[DBOPX] Reg Opcount for %@  - %d", connectionName, count);
