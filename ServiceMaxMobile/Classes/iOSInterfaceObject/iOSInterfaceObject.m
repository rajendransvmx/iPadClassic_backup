...
//	[obj setFieldValue:appDelegate.current_userId field:@"OwnerId"];
    ...
    {
        NSLog(@"check for finalized : %@",fileName);
   ...
   while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    ...
    if (attachNewSignature)
    {
        ...
        if(isOpdoc) {
         query = [NSString stringWithFormat:@"DELETE FROM SFSignatureData WHERE signature_name = '%@'", [fileName stringByDeletingPathExtension]];
        }
        else {
