...
- (void) getUserNameFromId:(NSArray *)userIdArray
{
    ...
    //Defect - 012038
    NSString *idSeparetedByComas = @"";
    
    if ([userIdArray count] > 1)
    {
        NSString *baseString = [userIdArray componentsJoinedByString:@"','"];
        idSeparetedByComas = [NSString stringWithFormat:@"'%@'", baseString];
    }
    else
    {
        idSeparetedByComas = [NSString stringWithFormat:@"'%@'", [userIdArray objectAtIndex:0]];
    }
    
    NSMutableString * _query = [[NSMutableString stringWithFormat:@"SELECT  Username, Id, Name, Email, FullPhotoUrl, SmallPhotoUrl FROM User WHERE Id in (%@)", idSeparetedByComas] retain];
...
- (BOOL) isSignatureForOPDOC:(NSString *)sfId andUniqueId:(NSString *)uniqueId {
    ...
    NSString *sqlQuery = [NSString  stringWithFormat:@"select COUNT(*) from SFSignatureData where sig_Id = '%@' and signature_name = '%@'  and sign_type = 'OPDOC'",sfId,uniqueId];
...
- (NSString *)nameForFile:(NSString *)sfid andSignatureId:(NSString *)sigUniqueId {
    ...
    NSString * sigId = @"";
    NSString * selectQuery = [NSString stringWithFormat:@"Select WorkOrderNumber from SFSignatureData Where sig_Id = '%@' and sign_type = 'OPDOC' and signature_name = '%@'", sfid,sigUniqueId];
    sqlite3_stmt * stmt;
...
//	[obj setFieldValue:appDelegate.current_userId field:@"OwnerId"];
    ...
    monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[iOSInterfaceObject setSignedImageData]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
