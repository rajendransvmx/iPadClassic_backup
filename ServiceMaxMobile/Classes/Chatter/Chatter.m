...
- (void) keyboardWillShow:(NSNotification *)notification
{
    ...
    isKeyboardShowing = YES;
        ...
        NSLog(@"Exception Name Chatter :keyboardWillShow %@",exp.name);
        NSLog(@"Exception Reason Chatter :keyboardWillShow %@",exp.reason);
...
- (void) didGetUserNameFromId:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    {
        NSLog(@"Exception Name Chatter :didGetUserNameFromId %@",exp.name);
        NSLog(@"Exception Reason Chatter :didGetUserNameFromId %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
@try{
    ...
    else
    {
        ...
        NSLog(@"Exception Name Chatter :connection %@",exp.name);
        NSLog(@"Exception Reason Chatter :connection %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
//    }
//     }@catch (NSException *exp) {
//        NSLog(@"Exception Name Chatter :didGetImagesForIds %@",exp.name);
//        NSLog(@"Exception Reason Chatter :didGetImagesForIds %@",exp.reason);
//         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
//    }
//    if (chatterArray != nil)
//        [self loadChatter];
     ...
     @try
        {
            ...
            NSLog(@"Exception Name Chatter :getUserNameFromArray %@",exp.name);
            NSLog(@"Exception Reason Chatter :getUserNameFromArray %@",exp.reason);
            [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
        }
    }
    return @"";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @try
    {
        ...
        NSLog(@"Exception Name Chatter :cellForRowAtIndexPath %@",exp.name);
        NSLog(@"Exception Reason Chatter :cellForRowAtIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    if (cell == nil)
    NSLog(@"%@", cell);
...
- (NSString *) getFullPhotoUrlFromArray:userArray WithId:usrString
{
    for (int i = 0; i < [userArray count]; i++)
    {
        @try
        {
            ...
            NSLog(@"Exception Name Chatter :getFullPhotoUrlFromArray %@",exp.name);
            NSLog(@"Exception Reason Chatter :getFullPhotoUrlFromArray %@",exp.reason);
            [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
        }
    }
    return @"";
