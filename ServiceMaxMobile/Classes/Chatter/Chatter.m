...
#import "AccessibilityChatterConstants.h"
...
- (void)viewDidLoad
{
    ...
    { 
        ...
        if ( data != NULL )
        {
            ...
            for (id postObj in chatterArray) {
                if (![postObj isKindOfClass:[NSDictionary class]])
                    [chatterArray removeObject:postObj];
            }
...
- (void) setAccessibilityIdentifierForInstanceVariable
{
    ...
    [shareButton setAccessibilityIdentifier:kAccShare];
...
- (void) didInternetConnectionChange:(NSNotification *)notification
{
    ...
    if (isReachable)
    {
        SMLog(kLogLevelVerbose,@"Chatter Internet Reachable");
    }
    else
    {
        SMLog(kLogLevelWarning,@"Chatter Internet Not Reachable");
        if (didRunOperation)
        {
...
- (void) keyboardWillShow:(NSNotification *)notification
{
    ...
    {
        frame = CGRectMake(chatTable.frame.origin.x, chatTable.frame.origin.y, chatTable.frame.size.width, chatTable.frame.size.height - boundRect.size.height - 15);
    }
    else {
        frame = CGRectMake(chatTable.frame.origin.x, chatTable.frame.origin.y, chatTable.frame.size.width, chatTable.frame.size.height - boundRect.size.width - 15);
    }
    ...
    isKeyboardShowing = YES;
        ...
        SMLog(kLogLevelError,@"Exception Name Chatter :keyboardWillShow %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason Chatter :keyboardWillShow %@",exp.reason);
...
- (void) didQueryChatterForProductId:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (![usrStringArray containsObject:usrString]) {
            [usrStringArray addObject:usrString];
        }
        
        ZKQueryResult *feeds = [[obj fields] objectForKey:FEEDCOMMENTFIELD];
        
        if ((feeds != nil) && ![feeds isKindOfClass:[NSNull class]])
        {
            for (ZKSObject * feedComments in [feeds records]) {
                NSString *commentedUserId = [[[feedComments fields] objectForKey:POSTCREATEDBYID] isKindOfClass:[NSString class]]?[[feedComments fields] objectForKey:POSTCREATEDBYID]:@"";
                
                if (![usrStringArray containsObject:commentedUserId]) {
                    [usrStringArray addObject:commentedUserId];
                }
            }
        }        
    }
...
- (void) didGetUserNameFromId:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        SMLog(kLogLevelVerbose,@"%dth Object \r %@", i, [obj fields]);
    ...
    SMLog(kLogLevelVerbose,@"%@  %d", chatterArray, [chatterArray count]);
    [appDelegate.calDataBase insertChatterDetailsIntoDBForWithId:productId andChatterDetails:chatterArray];
    
    [self loadChatter];
}@catch (NSException *exp)
    {
        SMLog(kLogLevelError,@"Exception Name Chatter :didGetUserNameFromId %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason Chatter :didGetUserNameFromId %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
@try{
    ...
    SMLog(kLogLevelVerbose,@"didReceiveData %@", userName);
    ...
    else
    {
        ...
        SMLog(kLogLevelError,@"Exception Name Chatter :connection %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason Chatter :connection %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    SMLog(kLogLevelVerbose,@"connectionDidFinishLoading %@", ((ChatterURLConnection*)connection).userName);
    ...
    SMLog(kLogLevelVerbose,@"%@", filePath);
...
//    }
//     }@catch (NSException *exp) {
//        SMLog(kLogLevelVerbose,@"Exception Name Chatter :didGetImagesForIds %@",exp.name);
//        SMLog(kLogLevelVerbose,@"Exception Reason Chatter :didGetImagesForIds %@",exp.reason);
//         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
//    }
//    if (chatterArray != nil)
//        [self loadChatter];
     ...
     @try
        {
            ...
            SMLog(kLogLevelError,@"Exception Name Chatter :getUserNameFromArray %@",exp.name);
            SMLog(kLogLevelError,@"Exception Reason Chatter :getUserNameFromArray %@",exp.reason);
            [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
        }
    }
    return @"";
...
- (void) loadChatter
{
    ...
    if (chatterArray != nil)
    {
        ...
        SMLog(kLogLevelVerbose,@"%@", chatterArrayForTable);
...
- (IBAction) Done
{
    ...
    SMLog(kLogLevelVerbose,@"%@", userNameImageList);
    NSError * error = nil;
...
- (ChatterCell *) createCustomCellWithId:(NSString *) cellIdentifier
{
    ...
    NSLog(@"CustonCell = %@",NSStringFromCGRect(customCell.frame));
...
{
    NSDictionary *postDict = [chatterArrayForTable objectAtIndex:indexPath.row];
    NSString *postTypeString = [postDict objectForKey:POSTTYPE];
    
    if ([postTypeString isEqualToString:TYPEFEED])
    {
        return 102.0f;
    }
    else if ([postTypeString isEqualToString:TYPECOMMENT])
    {
        return 102.0f;
    }
    else if ([postTypeString isEqualToString:TYPECOMMENTPOST])
    {
        return 51.0f;
    }
    else//TYPECHATSEPERATOR
    {
        return 37.0f;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @try
    {
        ...
        if ([postType isEqualToString:TYPEFEED])
        {
            ...
            NSLog(@"TYPEFEED= %@", NSStringFromCGRect(cell.frame));
            ...
            SMLog(kLogLevelVerbose,@"%@", cell.CreatedById);
            ...
            /*Accessibility Changes*/
            cell.commentButton.isAccessibilityElement = YES;
            [cell.commentButton setAccessibilityIdentifier:kAccPost];
            
            if ( ![appDelegate isInternetConnectionAvailable] )
            {
                SMLog(kLogLevelVerbose,@"%@", [postDict objectForKey:@"Username"]);
                
                image = [imageCache getImage:[NSString stringWithFormat:@"%@.png", [postDict objectForKey:@"Username"]]];
                
                if (image == nil)
                {
                    NSData *imageData = [appDelegate.calDataBase getImageDataForUserName:[postDict objectForKey:@"Username"]];
                    SMLog(kLogLevelVerbose,@"%d", [imageData length]);
                    
                    NSArray * paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
                    NSString * documentsDirectoryPath = [appDelegate getAppCustomSubDirectory]; // [paths objectAtIndex:0];
                    NSString * filePath = [documentsDirectoryPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@%@", [postDict objectForKey:@"Username"], @".png"]];
                    NSFileManager * fileManager = [NSFileManager defaultManager];
        ...
        if ([postType isEqualToString:TYPECOMMENT])
        {
            ...
            NSLog(@"TYPECOMMENT= %@", NSStringFromCGRect(cell.frame));
            [cell resetImages];
        ...
        if ([postType isEqualToString:TYPECOMMENTPOST])
        {
            ...
            NSLog(@"TYPECOMMENTPOST= %@", NSStringFromCGRect(cell.frame));
            cell.delegate = self;
            cell.feedCommentId = [postDict objectForKey:PRODUCT2FEEDID];
            // SMLog(kLogLevelVerbose,@"%@", [postDict objectForKey:PRODUCT2FEEDID]);
        }
        ...
        SMLog(kLogLevelError,@"Exception Name Chatter :cellForRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason Chatter :cellForRowAtIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    if (cell == nil)
    SMLog(kLogLevelVerbose,@"%@", cell);
    return cell;
}
...
- (NSString *) getFullPhotoUrlFromArray:userArray WithId:usrString
{
    for (int i = 0; i < [userArray count]; i++)
    {
        @try
        {
            ...
            SMLog(kLogLevelError,@"Exception Name Chatter :getFullPhotoUrlFromArray %@",exp.name);
            SMLog(kLogLevelError,@"Exception Reason Chatter :getFullPhotoUrlFromArray %@",exp.reason);
            [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
        }
    }
    return @"";
