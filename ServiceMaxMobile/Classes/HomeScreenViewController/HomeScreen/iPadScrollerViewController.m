...
#import "AccessibilityHomeScreenConstants.h"
...
- (void) showTasks
{
    ...
    if ([Utility notIOS7]) {
        ...
        appDelegate.modalCalendar = [[[ModalViewController alloc] initWithNibName:@"ModalViewControlleriOS7" bundle:nil] autorelease];
    }
...
-(void)logout
{
    ...
    //Shravya- commented the below code as it was behaving same for both ios6/7 on compilation from xcode 5
    // Has to be tested once merged into summer 13 main branch
	...
	for (GridViewCell *cell in homeIcons)
	{
        ...
        for(int i=0;i<[itemViews count];i++)
        {
            ...
            for (int i=0;i<[itemViewArray count];i++)
            {
                ...
                {
                    SMLog(kLogLevelVerbose,@"itemView.titleLable.text :%@ ",[[itemViewArray objectAtIndex:i] titleLable].text);
                    if ([[[itemViewArray objectAtIndex:i] titleLable].text isEqualToString:[appDelegate.wsInterface.tagsDictionary valueForKey:ipad_logout_label]])
                    {
    ...
    if ( [appDelegate showloginScreen] )
	{
		...
		[appDelegate.oauthClient.webview removeFromSuperview];//Fix for Defect# 007179
...
//            appDelegate.wsInterface.tagsDictionary = [temp_dict retain];
            ...
            [self restartAttachmentProcessing];
        }
    }
    ...
    /*Radha UIAutomation Accessibility*/
...
- (void) setAccessibilityIdentifierForInstanceVariable
{
    ...
    [backgroundView setAccessibilityIdentifier:kAccHomeBacgroundView];
    
    animateImage.isAccessibilityElement = YES;
    [animateImage setAccessibilityIdentifier:kAccServiceMaxLogo];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    /*Shrinivas : For Automation : Setting acceesibility to customer logo*/
    if (appDelegate.serviceReportLogo){
        [customerLogoImageView setIsAccessibilityElement:TRUE];
        [customerLogoImageView setAccessibilityIdentifier:kAccCustomerLogo]; /*Accessibility Changes*/
    }
    else{
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if(appDelegate.IsLogedIn == ISLOGEDIN_TRUE)
    {
        ...
        if(appDelegate.do_meta_data_sync == ALLOW_META_AND_DATA_SYNC)
        {
            ...
            if (serverSupportPurge) //010024 Defect fix
            {
            ...
            [appDelegate.dataBase cleanupDatabase];
            [self doTxFetch];
            //[appDelegate.dataBase cleanupDatabase]; // Commented for MEM Optimization
...
-(void)InitsyncSetting
{
	...
	//Radha Defect Fix 5542
	//[appDelegate updateNextDataSyncTimeToBeDisplayed:current_dateTime];//PB-TIME-FIX
...
- (void) scheduleLocationPingService
{
    ...
    if([enableLocationService boolValue])
    {
        ...
        {   // Vipindas - Fix Defect #10033
            // We have to reset the delegate, delegate may removed while stop listening to location update
            self.locationManager.delegate = self;
        }
        
        //Shubha Fix for 14136 :iOS8 changes
...
-(void)doMetaSync
{
    ...
    //011782
    [appDelegate.databaseInterface clearObjectMetaDataRelatedCache];
...
//    NSArray *arrayTemp = [[NSArray alloc] initWithObjects:@"HomeCalendar",@"HomeSFMSearch",@"HomeCreateNew",@"HomeMap",@"HomeRecents",@"HomeTasks",@"HomeSync",@"HomeHelp",@"HomeLogout", nil];
    ...
    /*Accessibility Changes*/
    NSArray *arrayTemp = [[NSArray alloc] initWithObjects:kAccCalendar, kAccSFMSerach, kAccCreateNew, kAccMap, kAccRecents, kAccTasks, kAccSyncScreen, kAccHelp, kAccLogout, nil];
    ...
    if (![appDelegate enableGPS_SFMSearch])
    {
        SMLog(kLogLevelVerbose,@"No SFMSearch to display");
...
#pragma mark -
#pragma mark Restart the downloads/uploads
- (void)restartAttachmentProcessing {
    
    [self performSelector:@selector(restartPendingAttachmentProcessing) withObject:nil afterDelay:4.0];
}

- (void)restartPendingAttachmentProcessing
{
    //NSLog(@"Hey I am lookig for any Attachment Job is pending. Looks Good Right ? ");
    [[SMAttachmentRequestManager sharedInstance] restartAllPendingAttachmentRequest];
}
...
{
    NSTimeInterval interval = [Utility getTimerIntervalForDataPurge];
    
    if (interval > 0)
    {
        NSLog(@"%@", [NSNumber numberWithDouble:interval]);
        ...
        [self updateNextDPTime:[Utility getDateTimeForNextDataPurge:[NSDate date]]];
    }
}


-(void)updateNextDPTime:(NSDate *)date
{
    [[SMDataPurgeManager sharedInstance] updateNextDataPurgeTime:date];
}
