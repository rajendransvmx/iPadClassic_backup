...
- (void) showMap
{
    ...
    else
    {
        ...
        NSLog(@"Exception Name iPadScrollerViewController :showMap %@",exp.name);
        NSLog(@"Exception Reason iPadScrollerViewController :showMap %@",exp.reason);
	...
	for (GridViewCell *cell in homeIcons)
	{
        ...
        for(int i=0;i<[itemViews count];i++)
        {
            ...
            for (int i=0;i<[itemViewArray count];i++)
            {
                ...
                {
                    NSLog(@"itemView.titleLable.text :%@ ",[[itemViewArray objectAtIndex:i] titleLable].text);
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if(appDelegate.IsLogedIn == ISLOGEDIN_TRUE)
    {
        ...
        if(appDelegate.do_meta_data_sync == ALLOW_META_AND_DATA_SYNC)
        {
            ...
            NSLog(@" ------- Adv Download Criteria Started -----");
			...
			NSLog(@" ------- Adv Download Criteria End -----");
...
-(void)clearuserinfoPlist
{
	...
	NSLog(@"Exception Name iPadScrollerViewController :clearuserinfoPlist %@",exp.name);
	NSLog(@"Exception Reason iPadScrollerViewController :clearuserinfoPlist %@",exp.reason);
	[appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
-(void)doMetaSync
{
    ...
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
    {
        ...
        NSLog(@"iPadScrollerViewController.m : doMetaSync: Inital Sync");
#endif
    ...
    if([appDelegate enableGPS_SFMSearch])
    {
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"iPadScrollerViewController.m : doMetaSync: Sfm Search");
#endif
    ...
    NSLog(@"  MetaSync WS End: %@", [NSDate date]);
...
-(void)doDataSync
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"iPadScrollerViewController.m : doDataSync: EventSync");
#endif
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"iPadScrollerViewController.m : doDataSync: Download Criteria Sync");
#endif
      ...
      NSLog(@"  DataSync WS End: %@", [NSDate date]);
    NSLog(@"  Incremental DataSync WS Start: %@", [NSDate date]);
...
-(void)doTxFetch
{
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"iPadScrollerViewController.m : doTxFetch: Cleanup Select");
#endif
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"iPadScrollerViewController.m : doTxFetch: Put Tx Fetch");
#endif
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"iPadScrollerViewController.m : doTxFetch: Unknown Check");
#endif
    ...
    NSLog(@"  Incremental DataSync WS End: %@", [NSDate date]);
    
    NSLog(@"  Update Sync Records Start: %@", [NSDate date]);
    ...
    {
#ifdef kPrintLogsDuringWebServiceCall
        NSLog(@"iPadScrollerViewController.m : doTxFetch: Unknow Check 2");
#endif
...
-(void)doAfterSyncSetttings
{
    ...
    NSLog(@"  Update Sync Records End: %@", [NSDate date]);
    //remove recents
    NSFileManager * fileManager = [NSFileManager defaultManager];
