	...
	for (GridViewCell *cell in homeIcons)
	{
        ...
        for(int i=0;i<[itemViews count];i++)
        {
            ...
            for (int i=0;i<[itemViewArray count];i++)
            {
                ...
                {
                    SMLog(@"itemView.titleLable.text :%@ ",[[itemViewArray objectAtIndex:i] titleLable].text);
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if(appDelegate.IsLogedIn == ISLOGEDIN_TRUE)
    {
        ...
        if(appDelegate.do_meta_data_sync == ALLOW_META_AND_DATA_SYNC)
        {
            ...
            // Drop All Data sync
            SMLog(@" ------- doDataSync Started -  removing all indexes -----");
			...
			SMLog(@" ------- doTxFetch  Started -----");
            SMLog(@" ------- doTxFetch Started -  creating indexes now -----");
            ...
            SMLog(@" ------- doAfterSyncSetttings  Started -----");
            ...
            [[PerformanceAnalytics sharedInstance] observePerformanceForContext:@"doAfterSyncSetttings"
                                                                 andRecordCount:0];
            SMLog(@" ------- doAfterSyncSetttings  Finished -----");
...
-(void)doMetaSync
{
	...
	SMLog(@"testing for exception thrown :%@",exp);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
	}
}
...
-(void)doTxFetch
{
    ...
    SMLog(@"-------  updatesfmIdsOfMasterToLocalIds  started -------");
    ...
    [[PerformanceAnalytics sharedInstance] observePerformanceForContext:@"updatesfmIdsOfMasterToLocalIds"
                                                         andRecordCount:0];
    SMLog(@"-------  updatesfmIdsOfMasterToLocalIds  Finished -------");
...
- (void) RefreshIcons
{
	...
	{
        //Shravya - 7933
        SMLog(@"Reloading Main Menu table");
