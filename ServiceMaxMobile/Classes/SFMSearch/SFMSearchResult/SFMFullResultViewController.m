...
#import "AccessibilitySearchFullResultViewConstants.h"
...
//    SMLog(kLogLevelVerbose,@"Display Values = %@",data);
...
//            [detailButton setAccessibilityIdentifier:@"DownloadDetailButton"];
            /*Accessibility changes*/
            detailButton.isAccessibilityElement = YES;
            [detailButton setAccessibilityIdentifier:kAccFullDetailResult];
        }
...
//            [detailButton setAccessibilityIdentifier:@"DisclosureDetailButton"];
            ...
            /*Accessibility changes*/
            detailButton.isAccessibilityElement = YES;
            [detailButton setAccessibilityIdentifier:kAccFullResultDisclosure];
            if (conflict)
            {
    ...
    if(!recordExists)
    {
        ...
        [createEvent setBackgroundColor:[UIColor clearColor]];
    }
...
- (void) setAccessibilityIdentifierForInstanceVariable
{
    ...
    [actionButton setAccessibilityIdentifier:kAccClose];
    
    onlineImageView.isAccessibilityElement = YES;
    [onlineImageView setAccessibilityIdentifier:kAccFullresultOnlineRecord];
    
    createEvent.isAccessibilityElement = YES;
    [createEvent setAccessibilityIdentifier:kAccAddEvent];
}
...
@try
    ...
    [objName release];
        ...
        SMLog(kLogLevelError,@"Exception Name SFMFullResultViewController :accessoryButtonTapped %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFMFullResultViewController :accessoryButtonTapped %@",exp.reason);
    }
}
- (void)viewDidUnload
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        {
            //10026 The field disappears from the SFM Search display window when user scroll on the page.
            //tableview view hierarchy is changed in iOS7
            if ([Utility notIOS7]) {
                [txtView removeFromSuperview];
            }
            else if([txtView isKindOfClass:[UIScrollView class]] )
            {
                [[txtView subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
            }
        }
    }
...
-(void)presentProgressBar:(NSString *)object_name sf_id:(NSString *)sf_id  reocrd_name:(NSString *)record_name
{
   ...
   //[appDelegate invalidateAllTimers]; /*Shravya-8226*/
    ...
    if(flag)
    {
        ...
        [monitor monitorSMMessageWithName:@"[SFMFullResultViewController presentProgressBar]"
        ...
        while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
        {
            ...
            SMLog(kLogLevelVerbose,@"SFMFullResultViewController.m : presentProgressBar: DOD");
#endif
