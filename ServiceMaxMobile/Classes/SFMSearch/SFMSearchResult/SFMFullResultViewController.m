...
@try
    ...
    [objName release];
        ...
        NSLog(@"Exception Name SFMFullResultViewController :accessoryButtonTapped %@",exp.name);
        NSLog(@"Exception Reason SFMFullResultViewController :accessoryButtonTapped %@",exp.reason);
    }
}
- (void)viewDidUnload
{
   ...
   //[appDelegate invalidateAllTimers]; /*Shravya-8226*/
    ...
    if(flag)
    {
        ...
        while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO))
        {
            ...
            NSLog(@"SFMFullResultViewController.m : presentProgressBar: DOD");
#endif
