...
#import "AccessibilitySearchDetailViewConstants.h"
...
@synthesize mainView;
@synthesize conflict;
@synthesize onlineresultsObjectDict;
...
//	syncBarButton.width =26;
    ...
    CustomToolBar* toolbar = [[[CustomToolBar alloc] initWithFrame:CGRectMake(600, 0, 90, 44)] autorelease];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        @try{
        NSString *objectID =[[tableDataArray  objectAtIndex:section] objectForKey:@"ObjectId"];
        NSMutableArray *array=[onlineresultsObjectDict objectForKey:objectID];
        //9713: Get the details from the offilne dictionary, if not available in online dictionary.
        if (array != nil) {
            cellArray =array;
        }
            /*
        ...
        SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :numberOfRowsInSection %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :numberOfRowsInSection %@",exp.reason);
        }
    }
    else {
        noOfRows = [cellArray count];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @try
    {
        ...
        {
            SMLog(kLogLevelVerbose,@"Array Count = %d",[cellArray count]);
        ...
        if([self.masterView.searchFilterSwitch isOn])
        {
           ...
           //9713
            ...
            SMLog(kLogLevelVerbose,@"%@",array);
                ...
                NSString * sf_id = [[cellArray objectAtIndex:indexPath.row] objectForKey:@"Id" ];
                ...
                if(![self isRecordPresentInOfflineResults:offlineRecords record:[[cellArray objectAtIndex:indexPath.row ] objectForKey:@"Id"]])
                {
                    UIImage *onlineImage = [UIImage imageNamed:@"OnlineRecord.png"];
                    /*Accessibility Changes*/
                    onlineImage.isAccessibilityElement = YES;
                    [onlineImage setAccessibilityIdentifier:kAccOnlineRecord];
...
//                        [control setAccessibilityIdentifier:@"FullResultDetailButton"];
                        /*Accessibility Changes*/
                        [control setAccessibilityIdentifier:kAccFullResultDetailButton];
                    ...
                    else if(processAvailbleForRecord && !recordExists)
                    {
                        ...
                        /*Accessibility Changes*/
                        button.isAccessibilityElement = YES;
                        [button setAccessibilityIdentifier:kAccSearchDisclosure];
                        if (conflict)
                        {
                ...
                {
                    if(processAvailbleForRecord && !recordExists)
                    {
                        [button setBackgroundImage:[UIImage imageNamed:@"SFM-Screen-Disclosure-Button.png"]
                                          forState:UIControlStateNormal];
                        [button addTarget:self action:@selector(accessoryButtonTapped:)  forControlEvents:UIControlEventTouchUpInside];
        ...
        SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :cellForRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :cellForRowAtIndexPath %@",exp.reason);
    }
...
{
//    SMLog(kLogLevelVerbose,@"Sections Data = %d",[tableDataArray count]);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(lastSelectedIndexPath && (lastSelectedIndexPath.row == indexPath.row) && (lastSelectedIndexPath.section == indexPath.section))
    {
        SMLog(kLogLevelVerbose,@"Selected the same cell");
    }
    else
    {
...
//            SMLog(kLogLevelVerbose,@"Tag1 = %d",[txtFieldLabel1 tag]);
//            SMLog(kLogLevelVerbose,@"Tag2 = %d",[txtFieldLabel2 tag]);
//            SMLog(kLogLevelVerbose,@"Tag3 = %d",[txtFieldLabel3 tag]);
...
//        {
        ...
        if([[onlineDataDict objectForKey:objectID] containsObject:fullDataDict])
           {
...
//        }
    ...
    else 
    {
        ...
        resultController.isOnlineRecord = NO;
        resultController.objectName = objectName;
//        SMLog(kLogLevelVerbose,@"Data = %@",fullDataDict);
    }
     ...
     [self.mainView presentViewController:resultController animated:YES completion:nil];
		...
		SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :didSelectRowAtIndexPath %@",exp.name);
		SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :didSelectRowAtIndexPath %@",exp.reason);
       }
    @finally {
...
{
//    SMLog(kLogLevelVerbose,@"Dismiss SFM Result Detail View Controller");
...
- (NSArray *) constructTableHeader : (NSArray *)data
{
    ...
    {
        [header addObject:[NSString stringWithFormat:@"%@.%@",[obj objectForKey:@"SVMXDEV__Object_Name2__c"],[obj objectForKey:@"SVMXDEV__Field_Name__c"]]];
    }
	}@catch (NSException *exp) {
	SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :constructTableHeader %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :constructTableHeader %@",exp.reason);
    }
...
- (NSMutableArray *)getResultsForObject:(NSString *)object withConfigData:(NSDictionary *)dataForObject
{
    ...
    /*ios7_support shravya ios6 */
    if ([Utility isStringEmpty:self.masterView.searchLimitString.text]) {
        self.masterView.searchLimitString.text = @"";
    }
    
    /*ios7_support shravya ios6*/
    if ([Utility isStringEmpty:self.masterView.searchCriteria.text]) {
        self.masterView.searchCriteria.text = @"";
    }
    
    /*ios7_support shravya ios6*/
    if ([Utility isStringEmpty:self.masterView.searchString.text]) {
        self.masterView.searchString.text = @"";
    }
    ...
    {
        result= [[appDelegate.dataBase getResults:object withConfigData:dataForObject] retain];
    }
    else
    {
        result = [[appDelegate.dataBase getResultsForSFM:object withConfigData:dataForObject] retain];
    }
    @try
    {
        ...
        {   
            ...
            {
                NSString *key = [NSString stringWithFormat:@"%@.%@",[displaydict objectForKey:@"SVMXDEV__Object_Name2__c"], [displaydict objectForKey:@"SVMXDEV__Field_Name__c"]];
            ...
            SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :getResultsForObject %@",exp.name);
            SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :getResultsForObject %@",exp.reason);
...
- (void) updateResultArray:(int) index
{
    if([resultArray count] > 0)
    {
        ...
        if(index == -1)
            ...
            tableDataArray = [[NSMutableArray arrayWithObjects:[resultArray objectAtIndex:index],nil] retain];
       ...
       forAllObjects:(BOOL) makeOnlineSearch
{
    ...
    {
        dispatch_queue_t  sfmSearchQueue = dispatch_queue_create("com.servicemaxmobile.sfmsearchqueue", 0);
		dispatch_sync(sfmSearchQueue, ^{
        if([resultArray count] == [[[self.masterView searchLimitString] text] intValue])
        {
            SMLog(kLogLevelVerbose,@"Offline DB has more or equal number records specified by the limit");
            return;
        }
        ...
        [monitor monitorSMMessageWithName:@"[SFMResultDetailViewController showObject]"
        ...
        while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
        {
            ...
            SMLog(kLogLevelVerbose,@"SFMResultDetailViewController.m : showObject: SFM Search");
#endif
            ...
            SMLog(kLogLevelVerbose,@"Retreiving Online Reccords");
        }
        [monitor monitorSMMessageWithName:@"[SFMResultDetailViewController showObject]"
                             withUserName:appDelegate.currentUserName
                                 logLevel:kPerformanceLevelWarning
        ...
        if(onlineDataDict)
            [onlineDataDict release];
        //Fixed memory leak while fixing the defect 9713
        if(onlineresultsObjectDict){
            [onlineresultsObjectDict release];
            onlineresultsObjectDict = nil;
        }

        onlineDataDict = [[NSMutableDictionary alloc] init];
        onlineresultsObjectDict=[[NSMutableDictionary alloc]init];
        for(NSDictionary *dict in appDelegate.onlineDataArray)
        {
            NSString *objectID = [dict objectForKey:@"SearchObjectId"];
            NSMutableArray *objectArray = [[onlineDataDict objectForKey:objectID] retain];
            if(! [[onlineresultsObjectDict allKeys] containsObject:objectID])
            {
                NSMutableArray *arry=[[NSMutableArray alloc]init];
                [arry addObject:dict];
                [onlineresultsObjectDict setObject:arry forKey:objectID];
                //Fixed memory leak while fixing the defect 9713
                [arry release];
            ...
            if(!objectArray)
                objectArray = [[NSMutableArray alloc] init];
            NSArray *offlineRecords = [self getOfflineRecordsForObjectID:objectID];
            if(([[onlineresultsObjectDict objectForKey:objectID ] count]) == [[[self.masterView searchLimitString] text] intValue])
            {
                continue;
            }
        ...
        SMLog(kLogLevelVerbose,@"onlineresultsObjectDict %@",onlineresultsObjectDict);
    });

	//Enhancement change for showing the number of records.
//	[mainView.resultmasterView reloadTableWithOnlineData:onlineDataDict]; Kirti
        dispatch_async(dispatch_get_main_queue(), ^{
            [mainView.resultmasterView reloadTableWithOnlineData:onlineDataDict overallResult:onlineresultsObjectDict];
        });
        dispatch_release(sfmSearchQueue);
    }
    else
    {
        dispatch_async(dispatch_get_main_queue(), ^{
	...
	SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :showObjects %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :showObjects %@",exp.reason);
    }
    }
}
- (NSArray *) getOfflineRecordsForObjectID:(NSString *) objectID
{
	...
	SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :getOfflineRecordsForObjectID %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :getOfflineRecordsForObjectID %@",exp.reason);
    }
...
- (BOOL) isRecordPresentInOfflineResults:(NSArray *) offlineRecords record:(NSString *) onlineRecordId
{
	...
	SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :isRecordPresentInOfflineResults %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :isRecordPresentInOfflineResults %@",exp.reason);
    }
...
//[appDelegate.sfmPageController release];
        ...
        SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :onDemandDataFecthing %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :onDemandDataFecthing %@",exp.reason);
    }
...
- (void) accessoryButtonTapped:(id)sender
{
    ...
    if (ownerCell != nil)
    {
        /* Now we will retrieve the index path of the cell which contains the section and the row of the cell */
        ownerCellIndexPath = [self.detailTable indexPathForCell:ownerCell];
//        SMLog(kLogLevelVerbose,@"Accessory in index path is tapped. Index path = %d", ownerCellIndexPath.row);
    }
       if(lastSelectedIndexPath != nil)
    {
        UITableViewCell *lastSelectedCell = [detailTable cellForRowAtIndexPath:lastSelectedIndexPath];
    ...
    NSDictionary *dict = [tableDataArray objectAtIndex:ownerCellIndexPath.section];
        ...
        if([self.masterView.searchFilterSwitch isOn])
        {
            dataDict =[array objectAtIndex:ownerCellIndexPath.row];
            if (dataDict == nil)
            {
                dataDict=[[dict objectForKey:@"Values"] objectAtIndex:ownerCellIndexPath.row];
            }
        }
        else
        {
...
//[appDelegate.sfmPageController release];
    ...
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, selectStatement,-1, &labelstmt, nil) == SQLITE_OK )
    {
        ...
        {
            field1 = (char *) synchronized_sqlite3_column_text(labelstmt,0);
//            SMLog(kLogLevelVerbose,@"%s",field1);
            if(field1)
                localId = [NSString stringWithFormat:@"%s", field1];
            else
                localId = @"";
    ...
    for (int v = 0; v < [appDelegate.view_layout_array count]; v++)
    {
        ...
        {
            processId = ([dict objectForKey:VIEW_SVMXDEV_ProcessID]!=nil)?[dict objectForKey:VIEW_SVMXDEV_ProcessID]:@"";
            break;
        }
    }
	...
	// //appDelegate.sfmPageController = nil;
        ...
        SMLog(kLogLevelError,@"Exception Name SFMResultDetailViewController :accessoryButtonTapped %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFMResultDetailViewController :accessoryButtonTapped %@",exp.reason);
