...
#import "AccesibilitySearchResultMasterViewConstants.h"
...
- (void) reachabilityChanged: (NSNotification* )notification 
{
    ...
    {
        SMLog(kLogLevelVerbose,@"Internet is Available");
        searchFilterSwitch.enabled=TRUE;
    }
    else
    {
        SMLog(kLogLevelWarning,@"Internet is Not Available");
       [searchFilterSwitch setOn:NO];
        searchFilterSwitch.enabled=FALSE;
        //9713
        resultDetailView.onlineresultsObjectDict = nil;
        //010287: SFM search count display zero when we tap on ‘GO’ in offline mode
...
{
    [super viewDidLoad];
    sfmSearchMainQueue = dispatch_queue_create("com.servicemaxmobile.sfmsearchmain", 0);
    // Do any additional setup after loading the view from its nib.
    SMLog(kLogLevelVerbose,@"Data = %@",searchData);
    appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
...
{
    searchCriteria.isAccessibilityElement = YES;
    [searchCriteria setAccessibilityIdentifier:kAccSearchCriteria];
    
    searchString.isAccessibilityElement = YES;
    [searchString setAccessibilityIdentifier:kAccSearchString];
    
    searchLimitString.isAccessibilityElement = YES;
    [searchLimitString setAccessibilityIdentifier:kAccSearchLimit];
    
    searchFilterSwitch.isAccessibilityElement  = YES;
    [searchFilterSwitch setAccessibilityIdentifier:kAccSearchFilterSwitch];
    
    actionButton.isAccessibilityElement = YES;
    [actionButton setAccessibilityIdentifier:kAccSearchOnline];
}
 ...
 didFinishPickingMediaWithInfo: (NSDictionary*) info
{
    ...
    [self performSelector:@selector(refineSearch:)];
        ...
        SMLog(kLogLevelError,@"Exception Name SFMResultMasterViewController :imagePickerController %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFMResultMasterViewController :imagePickerController %@",exp.reason);
    }
}
...
- (void) dealloc
{
    dispatch_release(sfmSearchMainQueue);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	SMLog(kLogLevelError,@"Exception Name SFMResultMasterViewController :cellForRowAtIndexPath %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFMResultMasterViewController :cellForRowAtIndexPath %@",exp.reason);
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SMLog(kLogLevelVerbose,@"Last Selected Index Path Row = %d",lastSelectedIndexPath.row);
...
- (UIView *) tableView:(UITableView *)_tableView viewForHeaderInSection:(NSInteger)section
{
   ...
   label.textColor = [UIColor whiteColor];
    ...
    /*Accessibility changes*/
    header_button.isAccessibilityElement = YES;
    [header_button setAccessibilityIdentifier:kAccShowllAllResults];
...
- (IBAction)refineSearch:(id)sender
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_sync(sfmSearchMainQueue, ^{
            [resultDetailView showObjects:tableArray forAllObjects:YES];
        });
...
- (void) reloadTableWithOnlineData:(NSMutableDictionary *)_onlineDict
{
	...
	SMLog(kLogLevelVerbose,@"%@", onlineRecordDict);
	[self.searchMasterTable reloadData];
}
