...
#import "SMDataPurgeManager.h"  //Data Purge - Radha
#import "Utility.h"  //10312

/*Accessibility changes*/
#import "AccessibilitySyncPopoverConstants.h"
...
@synthesize syncConfigurationFailed;
 /*********** Data Purge - Radha *************/
@synthesize button, button1, button2, button3, button4, button5;
...
- (void)loadView
{
    ...
    BOOL supportDataPurge = [appDelegate doesServerSupportsModule:kMinPkgForDataPurge];
    
    if (supportDataPurge)
    {
        UILabel *label5 = [[[UILabel alloc] initWithFrame:CGRectMake(10, 0, 200, 50)] autorelease];
        label5.backgroundColor = [UIColor clearColor];
        // label5.text =  [appDelegate.wsInterface.tagsDictionary objectForKey:Data_Purge];
        label5.text =  [Utility getValueForTagFromTagDict:Data_Purge];
        button5 = [UIButton buttonWithType:UIButtonTypeCustom];
        button5.frame = CGRectMake(0, 295, 214, 59);
        [button5 addSubview:label5];
        [button5 setImage:buttonBkground forState:UIControlStateNormal];
        [button5 addTarget:self action:@selector(dataPurge)forControlEvents:UIControlEventTouchUpInside];
    }
...
- (void) setAccessibilityIdentifierForInstanceVariable
{
    ...
    [button setAccessibilityIdentifier:kAccSyncData];
    
    button1.isAccessibilityElement = YES;
    [button1 setAccessibilityIdentifier:kAccSyncConfig];
    
    button2.isAccessibilityElement = YES;
    [button2 setAccessibilityIdentifier:kAccSyncEvent];
    
    button3.isAccessibilityElement = YES;
    [button3 setAccessibilityIdentifier:kAccRestApp];
    
    button4.isAccessibilityElement = YES;
    [button4 setAccessibilityIdentifier:kAccPushLogs];
    
    if ([appDelegate doesServerSupportsModule:kMinPkgForDataPurge])
    {
        button5.isAccessibilityElement = YES;
        [button5 setAccessibilityIdentifier:kAccDataPurge];
    }
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSArray *syncButtons = nil;
    NSArray *syncButtonTitleKeys = nil;
    
    //11012
    if ([appDelegate doesServerSupportsModule:kMinPkgForDataPurge])
    {
        syncButtons = [[NSArray alloc]initWithObjects:button, button1, button2,button3,button4,button5,nil];
        syncButtonTitleKeys = [[NSArray alloc] initWithObjects:sync_data_sync, sync_meta_data_configuration,sync_events,SYNC_RESETAPPLICATION,Push_Logs,Data_Purge, nil];
    }
    else
    {
        syncButtons = [[NSArray alloc]initWithObjects:button, button1, button2,button3,button4,nil];
        syncButtonTitleKeys = [[NSArray alloc] initWithObjects:sync_data_sync, sync_meta_data_configuration,sync_events,SYNC_RESETAPPLICATION,Push_Logs,nil];
    }
...
- (void)dataPurge
{
    SMLog(kLogLevelVerbose, @"Data Purge");
    [delegate dismisspopover];
    if (![appDelegate isInternetConnectionAvailable])
    {
        [delegate dismisspopover];
    ...
    [[SMDataPurgeManager sharedInstance] startMannualPurging]; //9884 - Defect Fix
}

- (void) updateUI
{
    [delegate presentProgessView];
}
...
//	[delegate disableControls];
    ...
    @catch (NSException * exception) {
        ...
        /** pushpak 10734 */
        /** overriding session invalid id message so commented*/
...
//	[appDelegate setCurrentSyncStatusProgress:METSSYNC_END optimizedSynstate:0];
    ...
    else
    {
        ...
        if ([appDelegate doesServerSupportsModule:kMinPkgForDataPurge])
        {
            ...
            [self performSelectorOnMainThread:@selector(rescheduleDataPurgeTimer) withObject:nil waitUntilDone:YES];
            //To clear the data purge view after completion of config sync
            [delegate resetPurgeViewIfExists]; //9889 Defect Fix
        }
...
{
    [[SMDataPurgeManager sharedInstance] clearDueIfConfigSyncSuccess]; //9862 - Defect Fix
    ...
    if (interval > 0)
    {
        ...
        NSDate * date = [Utility getDateTimeForNextDataPurge:[NSDate date]];
