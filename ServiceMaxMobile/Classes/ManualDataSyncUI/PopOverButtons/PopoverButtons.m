...
//        appDelegate.dataSyncRunning = YES;
        ...
        if ([appDelegate.metaSyncThread isExecuting])
        {
            while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
            {
                ...
                NSLog(@"popoverButtons.m : synchronise: check for meta sync thread");
#endif
        ...
        if ([appDelegate.event_thread isExecuting])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : synchronise: check for event sync thread");
#endif
		...
		if ([manualEventThread isExecuting])
		{
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : synchronise: check for Event sync thread 2");
#endif
    ...
    {        
        ...
        if([appDelegate.syncThread isExecuting])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : synchronise: check for data sync thread");
#endif
        ...
        if ([appDelegate.metaSyncThread isExecuting])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : synchronise: check for meta sync thread 2");
#endif
        ...
        {
#ifdef kPrintLogsDuringWebServiceCall
            NSLog(@"popoverButtons.m : synchronise: check for special inc sync");
#endif
    ...
    @try {
        ...
        if([appDelegate.syncThread isExecuting])
        {
           while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, kRunLoopTimeInterval, YES))
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : startSyncConfiguration: check for Data sync thread");
#endif
        ...
        if ([appDelegate.event_thread isExecuting])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : startSyncConfiguration: check for Event sync thread");
#endif
        ...
        if ([appDelegate.special_incremental_thread isExecuting])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : startSyncConfiguration: check for special incremental thread thread");
#endif
...
- (void) updateMetsSyncStatus:(NSString*)Status
{
	...
	NSLog(@"Exception Name PopoverButtons :updateMetsSyncStatus %@",exp.name);
	NSLog(@"Exception Reason PopoverButtons :updateMetsSyncStatus %@",exp.reason);
    }
...
- (void) startSyncEvents
{
    ...
    @try {
        ...
        if ([appDelegate.metaSyncThread isExecuting])
        {
            ...
            {
#ifdef kPrintLogsDuringWebServiceCall
                NSLog(@"popoverButtons.m : startSyncEvents: check for Meta sync thread");
#endif
