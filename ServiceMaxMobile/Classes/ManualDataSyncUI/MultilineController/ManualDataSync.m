...
- (void) viewDidLoad
{
    ...
    /*Radha - Data Purge*/
    dataSyncDetail.detailDelagate = self;
...
#pragma - END delegate ManualDetail

#pragma mark - Data Purge

- (void) addTranspertViewAndPresentProgressBar
{
    [self.view addSubview:transperentView];
    [self.view bringSubviewToFront:transperentView];
    
    progressBar = [[SMProgressBar alloc] init];
  
    progressBar.view.frame = CGRectMake(CGRectGetMidX(self.view.bounds)-212,CGRectGetMidY(self.view.bounds)-84, progressBar.view.frame.size.width, progressBar.view.frame.size.height);
    progressBar.progressBarDelegate = self;
    [self.view addSubview:progressBar.view];
    [self.view bringSubviewToFront:progressBar.view];
...
{
    NSMutableDictionary * dict = [[[SMDataPurgeManager sharedInstance] getProgressBarDetails] retain];
    [progressBar updateProgressBarAndpercentage:dict];
    [dict release];
}
...
-(void)removeProgresBar
{
    if (progressBar)
    {
        [progressBar.view removeFromSuperview];
        [progressBar release];
        progressBar = nil;
...
{
    progressBar.cancel.enabled = NO;
}
#pragma END - Data Purge

- (void)dealloc {
    [transperentView release];
