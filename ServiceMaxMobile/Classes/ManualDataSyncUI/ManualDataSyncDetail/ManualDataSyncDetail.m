...
#import "AccessibilitySyncScreenConstants.h"
...
- (void) Layout
{
    ...
    [titleLabel setAccessibilityIdentifier:kAccSyncTitle];
    ...
    [button setAccessibilityIdentifier:kAccSynchronize];
    ...
    /*Accessibility changes*/
    statusButton.isAccessibilityElement = YES;
    [statusButton setAccessibilityIdentifier:kAccStatus];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if(selectedSection == 0)
    {
        ...
        /** Pushpak 10734 */
        else if ([appDelegate isRemoteAccessRevoked])
        {
            return 1;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(selectedSection == 0)
    {
        if ( HeaderSelected == 1 )
        {
            ...
            /** pushpak defect 10734 */
            else if ([appDelegate isRemoteAccessRevoked])
            {
                cell.backgroundColor = [appDelegate colorForHex:@"#87AFC7"];
                NSString *remoteAccessRevokedErrorDescription = [NSString stringWithFormat:@"%@ :\n%@",[appDelegate.wsInterface.tagsDictionary valueForKey:alert_connection_error],[appDelegate.wsInterface.tagsDictionary valueForKey:remote_access_error]];
                UITextView *errorTextView = [[UITextView alloc]initWithFrame:CGRectMake(5, 0, 700, 70)];
                errorTextView.font = [UIFont systemFontOfSize:19.0];
                errorTextView.text = remoteAccessRevokedErrorDescription;
                errorTextView.editable = NO;
                errorTextView.backgroundColor = [UIColor clearColor];
                [cell.contentView addSubview:errorTextView];
...
- (void)showSyncDueStatusView
{
    ...
    /*Accessibility changes*/
    syncDueView.isAccessibilityElement = YES;
    [syncDueView setAccessibilityIdentifier:kAccConfigDue];
...
//    textViewFrame.size.height = 50;
    ...
    /*Accessibility changes*/
    dataPurgeDueView.isAccessibilityElement = NO;
    [dataPurgeDueView setAccessibilityIdentifier:kAccDataPurgeDue];
...
- (void)addButtonsToDPDueView;
{
    ...
    [continueButton setAccessibilityIdentifier:kAccPurgeContinue];
    
    removeButton.isAccessibilityElement = YES;
    [removeButton setAccessibilityIdentifier:kAccPurgeRemove];
