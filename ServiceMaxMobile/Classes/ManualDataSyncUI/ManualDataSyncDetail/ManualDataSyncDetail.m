...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    for(int i=0; i < [objectsArray count]; i++)
    {
        ...
        NSLog(@"Exception Name ManualDataSyncDetail :viewWillAppear %@",exp.name);
        NSLog(@"Exception Reason ManualDataSyncDetail :viewWillAppear %@",exp.reason);
 ...
 - (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (HeaderSelected == 1)
    {
        ...
        NSLog(@"Exception Name ManualDataSync :accessoryButtonTappedForRowWithIndexPath %@",exp.name);
        NSLog(@"Exception Reason ManualDataSync :accessoryButtonTappedForRowWithIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
- (void) DisclosureButtonTapped:(id) sender
{
    ...
    else
    {
        ...
        else if (HeaderSelected == 1)
        {
            ...
            NSLog(@"Exception Name ManualDataSync :DisclosureButtonTapped %@",exp.name);
            NSLog(@"Exception Reason ManualDataSync :DisclosureButtonTapped %@",exp.reason);
            [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
        }
    }
 }
		...
		if (buttonIndex == 0)
		{
			...
			else
			{
                ...
                @catch (NSException *e)
                {
                    ...
                    NSLog(@"ManualDataSyncDetail :didDismissWithButtonIndex - Exception Name  %@",e.name);
                    NSLog(@"ManualDataSyncDetail :didDismissWithButtonIndex - Exception Reason  %@",e.reason);
                }
