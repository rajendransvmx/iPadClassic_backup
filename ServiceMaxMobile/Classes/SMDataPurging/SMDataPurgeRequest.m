...
- (void)makeDataSyncRequest
{
    ...
    INTF_WebServicesDefServiceSvc_SVMXClient * client = [[appDelegate getSVMXClientObject]retain];
...
- (INTF_WebServicesDefServiceSvc_SVMXMap *) getLaborMap:(NSArray *)value
{
    ...
    if([activityType count])
    {
        ...
        return [SVMXCObject autorelease];
    }
    [activityType release];
    return nil;
}
...
- (INTF_WebServicesDefServiceSvc_SVMXMap *) getCurrencyMap
{
    ...
    return [SVMXCMapCurrency autorelease];
    
}
- (INTF_WebServicesDefServiceSvc_SVMXMap *) getPriceBookIdMap:(NSArray *)value
{
    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXCMapUniqueCurrency =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
    SVMXCMapUniqueCurrency.key  = @"PRICEBOOK_ID";
    [SVMXCMapUniqueCurrency.values addObjectsFromArray:value];
    
    return [SVMXCMapUniqueCurrency autorelease];
}
    ...
    if([self.index isEqualToString:@"2"])
    {
        ...
        if(([priceBookIds count] + [customPriceBookIds count]) > 0)
        {
            ...
            for(NSDictionary *priceBookDict in priceBookIds)
            {
                ...
                {
                    INTF_WebServicesDefServiceSvc_SVMXMap * SVMXCMapUniqueCurrency =  [[self getPriceBookIdMap:uniqueCurrencyArray] retain];
                    SVMXCMapUniqueCurrency.value = priceBookId;
                    [mapCurrency.valueMap addObject:SVMXCMapUniqueCurrency];
                    currencyPresent = YES;
                    [SVMXCMapUniqueCurrency release];
                }
