...
- (void)makeDataSyncRequest
{
    ...
    INTF_WebServicesDefServiceSvc_SPR14lient * client = [[appDelegate getSPR14lientObject]retain];
...
- (INTF_WebServicesDefServiceSvc_SVMXMap *) getLaborMap:(NSArray *)value
{
    ...
    if([activityType count])
    {
        ...
        return [SPR14Object autorelease];
    }
    [activityType release];
    return nil;
}
...
- (INTF_WebServicesDefServiceSvc_SVMXMap *) getCurrencyMap
{
    ...
    return [SPR14MapCurrency autorelease];
    
}
- (INTF_WebServicesDefServiceSvc_SVMXMap *) getPriceBookIdMap:(NSArray *)value
{
    INTF_WebServicesDefServiceSvc_SVMXMap * SPR14MapUniqueCurrency =  [[INTF_WebServicesDefServiceSvc_SVMXMap alloc] init];
    SPR14MapUniqueCurrency.key  = @"PRICEBOOK_ID";
    [SPR14MapUniqueCurrency.values addObjectsFromArray:value];
    
    return [SPR14MapUniqueCurrency autorelease];
}
    ...
    if([self.index isEqualToString:@"2"])
    {
        ...
        if(([priceBookIds count] + [customPriceBookIds count]) > 0)
        {
            ...
            for(NSDictionary *priceBookDict in priceBookIds)
            {
                ...
                {
                    INTF_WebServicesDefServiceSvc_SVMXMap * SPR14MapUniqueCurrency =  [[self getPriceBookIdMap:uniqueCurrencyArray] retain];
                    SPR14MapUniqueCurrency.value = priceBookId;
                    [mapCurrency.valueMap addObject:SPR14MapUniqueCurrency];
                    currencyPresent = YES;
                    [SPR14MapUniqueCurrency release];
                }
