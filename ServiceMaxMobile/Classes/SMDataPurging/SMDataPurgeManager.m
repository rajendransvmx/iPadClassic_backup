    ...
    @autoreleasepool
    {
        for (NSString * whatId in eventIds)
        {
            ...
            if (purgeModel != nil)
            {
                ...
                [self fillEventRelatedChildLine:purgeModel.purgeableReferenceObjects parentId:whatId parentName:objectName]; //9936 Defect Fix
            }
        }
    }
    [eventIds release];
    ...
    @autoreleasepool
    {
        for (SMObjectRelationModel * relationModel in childObjects) //9936 Defect Fix
        {
            if (relationModel != nil)
            {
                ...
                {
                    NSMutableArray * childIds = [[SMDataPurgeHelper getAllChildIdsForObject:relationModel.childName parentId:Id parentColumn:parentObject] retain];
