...
- (void)backView {
    ...
    NSString *documentDirectoryPath = [appDelegate getAppCustomSubDirectory]; // [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSFileManager *filemanager = [NSFileManager defaultManager];
    
//    NSString *htmlFileName = [[documentDirectoryPath stringByAppendingPathComponent:signName] stringByAppendingPathExtension:@"html"];
    
    // Check whether html file exists with prefix signname
    BOOL htmlFileExists = NO;
    ...
    {
        if([Utility containsString:signName inString:itm] && (![[itm pathExtension] isEqualToString:@"png"]))
        {
            htmlFileExists = YES;
            break;
        }
