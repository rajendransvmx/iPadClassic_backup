    ...
    if (ret == SQLITE_OK)
    {
        ...
        while (ret == SQLITE_ROW)
        {
           ...
           // retVal = [self isWorkOrder:whatId1];
			...
			retVal = [self isWorkOrderOrCase:whatId1 objectName:@"SVMXDEV__Service_Order__c"];
            ...
            if ( retVal == YES && (!([whatId1 isEqualToString:@""]) || whatId1 != nil) )
            {
                ...
                queryStatement1 = [NSMutableString stringWithFormat:@"Select Name,SVMXDEV__City__c,SVMXDEV__Zip__c,SVMXDEV__Street__c,SVMXDEV__State__c,SVMXDEV__Country__c From SVMXDEV__Service_Order__c where Id = '%@'",whatId1];
...
- (BOOL) isWorkOrderOrCaseForColorCode:(NSString *)whatId
{
    ...
    synchronized_sqlite3_finalize(stmt);
    if ([objectName isEqualToString:@"Case"] ||[objectName isEqualToString:@"SVMXDEV__Service_Order__c"] )
    {
        return YES;
    }
...
- (NSString *)getColorCodeForPriority:(NSString *)whatId objectname:(NSString *)objectName
{
    ...
    if ( retVal == YES )
    {
		...
		if ([objectName isEqualToString:@"Case"])
			...
			queryStatement = [NSString stringWithFormat:@"SELECT SVMXDEV__Priority__c from '%@' where Id = '%@'", objectName, whatId];
        const char * selectStatement = [queryStatement UTF8String];
...
-(NSString*)getNameField:(NSString *)SVMXDEV__Product__c
{
    NSMutableString *queryStatement = [[NSMutableString alloc]initWithCapacity:0];
    NSString * Name1 = @"";
    sqlite3_stmt *statement1;
//    defect 007237
    SVMXDEV__Product__c=[SVMXDEV__Product__c stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
    
    queryStatement = [NSString stringWithFormat:@"SELECT Name from Product2 where Id = '%@'", SVMXDEV__Product__c];
    const  char * selectStatement = [queryStatement UTF8String];
...
-(NSMutableArray*)queryForExpenses:(NSString *)currentRecordId
{
    ...
    NSString * sfid =[appDelegate.databaseInterface getSfid_For_LocalId_From_Object_table:@"SVMXDEV__Service_Order__c" local_id:currentRecordId];
    
    if ((sfid != nil) && [sfid length] > 0)
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Expense_Type__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c,SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Expenses' AND (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@')  AND (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    }
    else
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Expense_Type__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c,SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Expenses' AND SVMXDEV__Service_Order__c = '%@' AND (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    }
    ...
    NSArray * keys = [NSArray arrayWithObjects:
                      _ID,
                      SVMXDEV__EXPENSE_TYPE__C,
                      SVMXDEV__ACTUAL_QUANTITY2__C,
                      SVMXDEV__ACTUAL_PRICE2__C,
                      SVMXDEV__WORK_DESCRIPTION__C,
                      SVMXDEV_BILLABLE_QUANTITY,
                      SVMXDEV_BILLABLE_PRICE,
                      nil];
    @try{
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement1, nil) == SQLITE_OK )
    {
        while(synchronized_sqlite3_step(statement1) == SQLITE_ROW)
        {
            ...
            char *_SVMXDEV__Expense_Type__c = (char *) synchronized_sqlite3_column_text(statement1,1);
            NSString * SVMXDEV__Expense_Type__c = @"";
            if ((_SVMXDEV__Expense_Type__c != nil) && strlen(_SVMXDEV__Expense_Type__c))
            {         
                SVMXDEV__Expense_Type__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Expense_Type__c];
            }  
            char *_SVMXDEV__Actual_Quantity2__c = (char *) synchronized_sqlite3_column_text(statement1,2);
            NSString * SVMXDEV__Actual_Quantity2__c_ = @"";    
            if ((_SVMXDEV__Actual_Quantity2__c != nil) && strlen(_SVMXDEV__Actual_Quantity2__c))
            {         
                SVMXDEV__Actual_Quantity2__c_ = [[NSString alloc] initWithUTF8String:_SVMXDEV__Actual_Quantity2__c];
            }
            char *_SVMXDEV__Actual_Price2__c = (char *) synchronized_sqlite3_column_text(statement1,3);
            NSString * SVMXDEV__Actual_Price2__c_ = @"";    
            if ((_SVMXDEV__Actual_Price2__c != nil) && strlen(_SVMXDEV__Actual_Price2__c))
            {         
                SVMXDEV__Actual_Price2__c_ = [[NSString alloc] initWithUTF8String:_SVMXDEV__Actual_Price2__c];
            }   
            char *_SVMXDEV__Work_Description_c = (char *) synchronized_sqlite3_column_text(statement1,4);
            NSString * SVMXDEV__Work_Description_c = @"";    
            if ((_SVMXDEV__Work_Description_c != nil) && strlen(_SVMXDEV__Work_Description_c))
            {         
                SVMXDEV__Work_Description_c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Work_Description_c];
            }
            ...
            NSMutableArray * objects = [[NSMutableArray arrayWithObjects:Id,SVMXDEV__Expense_Type__c, SVMXDEV__Actual_Quantity2__c_,SVMXDEV__Actual_Price2__c_,SVMXDEV__Work_Description_c,billableQty,billablePrice,nil] retain];
...
- (NSMutableArray *) queryForParts:(NSString *)currentRecordId
{
    ...
    if ((sfid != nil) && [sfid length] > 0)
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Product__c,SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c, SVMXDEV__Discount__c, Name , SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Parts' AND (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@')  AND SVMXDEV__Actual_Quantity2__c > 0 AND SVMXDEV__Actual_Price2__c >= 0 AND (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    }
    else
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Product__c,SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c, SVMXDEV__Discount__c, Name , SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Parts' AND SVMXDEV__Service_Order__c = '%@' AND SVMXDEV__Actual_Quantity2__c > 0 AND SVMXDEV__Actual_Price2__c >= 0 AND (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    }
    ...
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement1, nil) == SQLITE_OK )
    {
        ...
        while(synchronized_sqlite3_step(statement1) == SQLITE_ROW)
        {
            NSMutableArray * keys = [NSMutableArray arrayWithObjects:
                                     _ID,
                                     SVMXDEV__PRODUCT__C,
                                     SVMXDEV__PRODUCT2__NAME,
                                     SVMXDEV__ACTUAL_QUANTITY2__C,
                                     SVMXDEV__ACTUAL_PRICE2__C,
                                     SVMXDEV__WORK_DESCRIPTION__C,
                                     SVMXDEV__DISCOUNT__C,
                                     SVMXDEV_BILLABLE_QUANTITY,
                                     SVMXDEV_BILLABLE_PRICE,
                                     nil];
            ...
            char *_SVMXDEV__Product__c = (char *) synchronized_sqlite3_column_text(statement1,1);
            NSString * SVMXDEV__Product__c = @"";
            if ((_SVMXDEV__Product__c != nil) && strlen(_SVMXDEV__Product__c))
            {         
                SVMXDEV__Product__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Product__c];
                nameField = [self getNameField:SVMXDEV__Product__c];
				
				//Shrinivas   --- Change for service Report  -- 22/06/2012
				if ( [nameField isEqualToString:@""])
				{
					nameField = [self getProductNameFromDbWithID:SVMXDEV__Product__c];
				}
            } 
            ...
            char *_SVMXDEV__Discount__c = (char *) synchronized_sqlite3_column_text(statement1,5);
            NSString * SVMXDEV__Discount__c = @"";    
            if ((_SVMXDEV__Discount__c != nil) && strlen(_SVMXDEV__Discount__c))
            {         
                SVMXDEV__Discount__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Discount__c];
            } 
            
            
            /* Fetching billable quantitye 6773*/
            char *tempValue = (char *) synchronized_sqlite3_column_text(statement1,7);
            NSString * _SVMXDEV__Billable_Quantity__c = @"0.0";
            if ((tempValue != nil) && strlen(tempValue))
            {
                _SVMXDEV__Billable_Quantity__c = [[NSString alloc] initWithUTF8String:tempValue];
                if (_SVMXDEV__Billable_Quantity__c == nil) {
                    _SVMXDEV__Billable_Quantity__c = @"0.0";
                }
            }
            
            tempValue = nil;
            tempValue = (char *) synchronized_sqlite3_column_text(statement1,8);
            NSString * _SVMXDEV__Billable_Line_Price__c = @"0.0";
            if ((tempValue != nil) && strlen(tempValue))
            {
                _SVMXDEV__Billable_Line_Price__c = [[NSString alloc] initWithUTF8String:tempValue];
                if (_SVMXDEV__Billable_Line_Price__c == nil) {
                    _SVMXDEV__Billable_Line_Price__c = @"0.0";
                }
            }
            ...
            NSMutableArray * objects = [[NSMutableArray arrayWithObjects:Id,SVMXDEV__Product__c,nameField,SVMXDEV__Actual_Quantity2__c_,SVMXDEV__Actual_Price2__c_,SVMXDEV__Work_Description_c,SVMXDEV__Discount__c,_SVMXDEV__Billable_Quantity__c,_SVMXDEV__Billable_Line_Price__c,nil] retain];
            ...
            NSString * numPartsUsed = [dictionary objectForKey:gSVMXDEV__Actual_Quantity2__c];
            ...
            NSString * description = [dictionary objectForKey:gSVMXDEV__Work_Description__c];
            if ([description isKindOfClass:[NSString class]])
                description = [NSString stringWithFormat:@"%@", description];
            
            NSString * costPerPart = @"";
            costPerPart = [dictionary objectForKey:gSVMXDEV__Actual_Price2__c];
            
            NSString * keyProduct = @"";
            keyProduct = [dictionary objectForKey:gSVMXDEV__Product__c];
            
            NSString * discount = [dictionary objectForKey:gSVMXDEV__Discount__c];
            if ([discount isKindOfClass:[NSString class]])
                discount = [NSString stringWithFormat:@"%@", discount];
            
            NSString  *keyBillableQty = [dictionary objectForKey:SVMXDEV_BILLABLE_QUANTITY];
            
            NSString  *keyBillablePrice = [dictionary objectForKey:SVMXDEV_BILLABLE_PRICE];
...
{ 
    ...
    @try{
        ...
        if ((sfid != nil) && [sfid length] > 0)
        {
            queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Activity_Type__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c,SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Labor' AND (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@' ) AND  (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    ...
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement1, nil) == SQLITE_OK )
    {
        while(synchronized_sqlite3_step(statement1) == SQLITE_ROW)
        {
            ...
            char *_SVMXDEV__Activity_Type__c = (char *) synchronized_sqlite3_column_text(statement1,1);
            NSString * SVMXDEV__Activity_Type__c_ = @"";
            if ((_SVMXDEV__Activity_Type__c != nil) && strlen(_SVMXDEV__Activity_Type__c))
            {         
                SVMXDEV__Activity_Type__c_ = [[NSString alloc] initWithUTF8String:_SVMXDEV__Activity_Type__c];
            }  
            char *_SVMXDEV__Actual_Quantity2__c = (char *) synchronized_sqlite3_column_text(statement1,2);
            NSString * SVMXDEV__Actual_Quantity2__c_ = @"";    
            if ((_SVMXDEV__Actual_Quantity2__c != nil) && strlen(_SVMXDEV__Actual_Quantity2__c))
            ...
            NSMutableArray * objects = [[NSMutableArray arrayWithObjects:Id,SVMXDEV__Activity_Type__c_,SVMXDEV__Actual_Quantity2__c_,SVMXDEV__Actual_Price2__c_,SVMXDEV__Work_Description_c,billableQty,billablePrice,nil] retain];
            ...
            if ([[dictionary objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:CALIBRATION])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_CALIBRATION];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_CALIBRATION]
                ;
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_PRICE ] forKey:BILL_RATE_CALIBRATION];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_QUANTITY] forKey:BILL_QTY_CALIBRATION];
                
            }
            if ([[dictionary objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:CLEANUP])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_CLEANUP];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_CLEANUP];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_PRICE ] forKey:BILL_RATE_CLEANUP];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_QUANTITY] forKey:BILL_QTY_CLEANUP];

                
            }
            if ([[dictionary objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:INSTALLATION])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_INSTALLATION];
                [LabourValuesDictionary setValue:[dictionary  objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_INSTALLATION];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_PRICE ] forKey:BILL_RATE_INSTALLATION];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_QUANTITY] forKey:BILL_QTY_INSTALLATION];
            }
            if ([[dictionary objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:REPAIR])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_REPAIR];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_REPAIR];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_PRICE ] forKey:BILL_RATE_REPAIR];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_QUANTITY] forKey:BILL_QTY_REPAIR];
            }
            if ([[dictionary objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:SERVICE])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_SERVICE];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_SERVICE];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_PRICE ] forKey:BILL_RATE_SERVICE];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SVMXDEV_BILLABLE_QUANTITY] forKey:BILL_QTY_SERVICE];
            }
    ...
    queryStatement6 = [NSMutableString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c  WHERE SVMXDEV__Group_Member__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight'", appDelegate.appTechnicianId];
    const char * _query6 = [queryStatement6 UTF8String];
    NSArray * keys6 = [NSArray arrayWithObjects:
                       SVMXDEV__BILLABLE_COST2__C,
                       nil];
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query6,-1, &statement6, nil) == SQLITE_OK )
    {
        ...
        {
            char *_SVMXDEV__Billable_Cost2__c = (char *) synchronized_sqlite3_column_text(statement6,0);
            NSString * SVMXDEV__Billable_Cost2__c = @"";    
            if ((_SVMXDEV__Billable_Cost2__c != nil) && strlen(_SVMXDEV__Billable_Cost2__c))
            {         
                SVMXDEV__Billable_Cost2__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Billable_Cost2__c];
                
            }
            NSMutableArray * objects6 = [[NSMutableArray arrayWithObjects:SVMXDEV__Billable_Cost2__c,nil] retain];
    ...
    if( array != nil && ![array count] )
	{
        ...
        queryStatement7 = [NSMutableString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c WHERE SVMXDEV__Service_Group__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight'", appDelegate.appServiceTeamId];
        ...
        if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query7,-1, &statement7, nil) == SQLITE_OK )
        {
            ...
            {
                char *_SVMXDEV__Billable_Cost2__c = (char *) synchronized_sqlite3_column_text(statement7,0);
                NSString * SVMXDEV__Billable_Cost2__c = @"";    
                if ((_SVMXDEV__Billable_Cost2__c != nil) && strlen(_SVMXDEV__Billable_Cost2__c))
                {         
                    SVMXDEV__Billable_Cost2__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Billable_Cost2__c];
                ...
                NSMutableArray * objects7 = [[NSMutableArray arrayWithObjects:SVMXDEV__Billable_Cost2__c,nil] retain];
        ...
        for (int i = 0; i < [array count]; i++)
        {
            ...
            rate = [[obj8 objectForKey:@"SVMXDEV__Billable_Cost2__c"] retain];
	...
	else
	{
        ...
        rate = [[obj7 objectForKey:@"SVMXDEV__Billable_Cost2__c"] retain];
...
- (NSMutableArray *) queryForTravel:(NSString *)currentRecordId
{
    ...
    if ((sfid != nil) && [sfid length] > 0)
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c, SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Travel' AND (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@' )AND (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    }
    else
    {
       queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Work_Description__c, SVMXDEV__Billable_Quantity__c, SVMXDEV__Billable_Line_Price__c FROM SVMXDEV__Service_Order_Line__c WHERE SVMXDEV__Line_Type__c = 'Travel' AND SVMXDEV__Service_Order__c = '%@' AND (SVMXDEV__Is_Billable__c = 'true' or  SVMXDEV__Is_Billable__c = 'True' or SVMXDEV__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    ...
    @try{
        ...
        if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement, nil) == SQLITE_OK )
        {
            while(synchronized_sqlite3_step(statement) == SQLITE_ROW)
            {
                ...
                char *_SVMXDEV__Actual_Quantity2__c = (char *) synchronized_sqlite3_column_text(statement,1);
                NSString * SVMXDEV__Actual_Quantity2__c_ = @"";
                if ((_SVMXDEV__Actual_Quantity2__c != nil) && strlen(_SVMXDEV__Actual_Quantity2__c))
                {
                    SVMXDEV__Actual_Quantity2__c_ = [[[NSString alloc] initWithUTF8String:_SVMXDEV__Actual_Quantity2__c] autorelease];
                    if (SVMXDEV__Actual_Quantity2__c_ == nil) {
                        SVMXDEV__Actual_Quantity2__c_ = @"0.0";
                    }
                }
                
                char *_SVMXDEV__Actual_Price2__c = (char *) synchronized_sqlite3_column_text(statement,2);
                NSString * SVMXDEV__Actual_Price2__c_ = @"";
                if ((_SVMXDEV__Actual_Price2__c != nil) && strlen(_SVMXDEV__Actual_Price2__c))
                {
                    SVMXDEV__Actual_Price2__c_ = [[[NSString alloc] initWithUTF8String:_SVMXDEV__Actual_Price2__c] autorelease];
                    if (SVMXDEV__Actual_Price2__c_ == nil) {
                        SVMXDEV__Actual_Price2__c_ = @"0.0";
                    }
                }
                char *_SVMXDEV__Work_Description_c = (char *) synchronized_sqlite3_column_text(statement,3);
                NSString * SVMXDEV__Work_Description_c = @"";
                if ((_SVMXDEV__Work_Description_c != nil) && strlen(_SVMXDEV__Work_Description_c))
                {
                    SVMXDEV__Work_Description_c = [[[NSString alloc] initWithUTF8String:_SVMXDEV__Work_Description_c] autorelease];
                    if (SVMXDEV__Work_Description_c == nil) {
                        SVMXDEV__Work_Description_c = @"";
                    }
                }
                
                char *_SVMXDEV__Billable_Quantity__c = (char *) synchronized_sqlite3_column_text(statement,4);
                NSString * billableQty = @"0.0";
                if ((_SVMXDEV__Billable_Quantity__c != nil) && strlen(_SVMXDEV__Billable_Quantity__c))
                {
                    billableQty = [[[NSString alloc] initWithUTF8String:_SVMXDEV__Billable_Quantity__c] autorelease];
                    if (billableQty == nil) {
                        billableQty = @"0.0";
                    }
                }
                ...
                char *_SVMXDEV__billable_line_price__c = (char *) synchronized_sqlite3_column_text(statement,5);
                NSString *billableLinePrice = @"0.0";
                if ((_SVMXDEV__billable_line_price__c != nil) && strlen(_SVMXDEV__billable_line_price__c))
                {
                    billableLinePrice = [[[NSString alloc] initWithUTF8String:_SVMXDEV__billable_line_price__c] autorelease];
                ...
                NSMutableArray * objects = [NSMutableArray arrayWithObjects:Id,SVMXDEV__Actual_Quantity2__c_,SVMXDEV__Actual_Price2__c_,SVMXDEV__Work_Description_c,billableQty,billableLinePrice,nil];
...
- (void) startQueryConfiguration
{
    @autoreleasepool
    {
        ...
        queryStatement2 = [NSMutableString stringWithFormat:@"SELECT Id, SVMXDEV__SubmoduleID__c, SVMXDEV__SettingID__c, SVMXDEV__Setting_Unique_ID__c, SVMXDEV__Settings_Name__c, SVMXDEV__Data_Type__c, SVMXDEV__Values__c, SVMXDEV__Default_Value__c, SVMXDEV__Setting_Type__c, SVMXDEV__Search_Order__c, SVMXDEV__IsPrivate__c, SVMXDEV__Active__c, SVMXDEV__Description__c, SVMXDEV__IsStandard__c, SVMXDEV__Submodule__c FROM SettingsInfo"];
        ...
        NSArray * keys2 = [NSArray arrayWithObjects:
                           _ID,
                           SVMXDEV__SUBMODULEID__C,
                           SVMXDEV__SETTINGID__C,
                           SVMXDEV__SETTING_UNIQUE_ID__C,
                           SVMXDEV__SETTINGS_NAME__C,
                           SVMXDEV__DATA_TYPE__C,
                           SVMXDEV__VALUES__C,
                           SVMXDEV__DEFAULT_VALUE__C,
                           SVMXDEV__SETTING_TYPE__C,
                           SVMXDEV__SEARCH_ORDER__C,
                           SVMXDEV__ISPRIVATE__C,
                           SVMXDEV__ACTIVE__C,
                           SVMXDEV__DESCRIPTION__C,
                           SVMXDEV__ISSTANDARD__C,
                           SVMXDEV__SUBMODULE__C,
                           nil];
        ...
        @try{
            ...
            if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query2,-1, &statement2, nil) == SQLITE_OK )
            {
                while(synchronized_sqlite3_step(statement2) == SQLITE_ROW)
                {
                    ...
                    char *_SVMXDEV__SubmoduleID__c = (char *) synchronized_sqlite3_column_text(statement2,1);
                    NSString * SVMXDEV__SubmoduleID__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__SubmoduleID__c != nil) && strlen(_SVMXDEV__SubmoduleID__c))
                    {
                        [SVMXDEV__SubmoduleID__c release];
                        SVMXDEV__SubmoduleID__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__SubmoduleID__c];
                    }
                    
                    char *_SVMXDEV__SettingID__c = (char *) synchronized_sqlite3_column_text(statement2,2);
                    NSString * SVMXDEV__SettingID__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__SettingID__c != nil) && strlen(_SVMXDEV__SettingID__c))
                    {
                        [SVMXDEV__SettingID__c release];
                        SVMXDEV__SettingID__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__SettingID__c];
                    }
                    
                    char *_SVMXDEV__Setting_Unique_ID = (char *) synchronized_sqlite3_column_text(statement2,3);
                    NSString * SVMXDEV__Setting_Unique_ID = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Setting_Unique_ID != nil) && strlen(_SVMXDEV__Setting_Unique_ID))
                    {
                        [SVMXDEV__Setting_Unique_ID release];
                        SVMXDEV__Setting_Unique_ID = [[NSString alloc] initWithUTF8String:_SVMXDEV__Setting_Unique_ID];
                    }
                    
                    char *_SVMXDEV__Settings_Name__c = (char *) synchronized_sqlite3_column_text(statement2,4);
                    NSString * SVMXDEV__Settings_Name__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Settings_Name__c != nil) && strlen(_SVMXDEV__Settings_Name__c))
                    {
                        [SVMXDEV__Settings_Name__c release];
                        SVMXDEV__Settings_Name__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Settings_Name__c];
                    }
                    
                    char *_SVMXDEV__Data_Type__c = (char *) synchronized_sqlite3_column_text(statement2,5);
                    NSString * SVMXDEV__Data_Type__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Data_Type__c != nil) && strlen(_SVMXDEV__Data_Type__c))
                    {
                        SVMXDEV__Data_Type__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Data_Type__c];
                    }
                    
                    char *_SVMXDEV__Values__c = (char *) synchronized_sqlite3_column_text(statement2,6);
                    NSString * SVMXDEV__Values__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Values__c != nil) && strlen(_SVMXDEV__Values__c))
                    {
                        [SVMXDEV__Values__c release];
                        SVMXDEV__Values__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Values__c];
                    }
                    
                    char *_SVMXDEV__Default_Value__c = (char *) synchronized_sqlite3_column_text(statement2,7);
                    NSString * SVMXDEV__Default_Value__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Default_Value__c != nil) && strlen(_SVMXDEV__Default_Value__c))
                    {
                        [SVMXDEV__Default_Value__c release];
                        SVMXDEV__Default_Value__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Default_Value__c];
                    }
                    
                    char *_SVMXDEV__Setting_Type__c = (char *) synchronized_sqlite3_column_text(statement2,8);
                    NSString * SVMXDEV__Setting_Type__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Setting_Type__c != nil) && strlen(_SVMXDEV__Setting_Type__c))
                    {
                        [SVMXDEV__Setting_Type__c release];
                        SVMXDEV__Setting_Type__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Setting_Type__c];
                    }
                    
                    char *_SVMXDEV__Search_Order__c = (char *) synchronized_sqlite3_column_text(statement2,9);
                    NSString * SVMXDEV__Search_Order__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Search_Order__c != nil) && strlen(_SVMXDEV__Search_Order__c))
                    {
                        [SVMXDEV__Search_Order__c release];
                        SVMXDEV__Search_Order__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Search_Order__c];
                    }
                    
                    char *_SVMXDEV__IsPrivate__c = (char *) synchronized_sqlite3_column_text(statement2,10);
                    NSString * SVMXDEV__IsPrivate__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__IsPrivate__c != nil) && strlen(_SVMXDEV__IsPrivate__c))
                    {
                        [SVMXDEV__IsPrivate__c release];
                        SVMXDEV__IsPrivate__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__IsPrivate__c];
                    }
                    
                    char *_SVMXDEV__Active__c = (char *) synchronized_sqlite3_column_text(statement2,11);
                    NSString * SVMXDEV__Active__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Active__c != nil) && strlen(_SVMXDEV__Active__c))
                    {
                        [SVMXDEV__Active__c release];
                        SVMXDEV__Active__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Active__c];
                    }
                    
                    char *_SVMXDEV__Description__c = (char *) synchronized_sqlite3_column_text(statement2,12);
                    NSString * SVMXDEV__Description__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Description__c != nil) && strlen(_SVMXDEV__Description__c))
                    {
                        [SVMXDEV__Description__c release];
                        SVMXDEV__Description__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Description__c];
                    }
                    
                    char *_SVMXDEV__IsStandard__c = (char *) synchronized_sqlite3_column_text(statement2,13);
                    NSString * SVMXDEV__IsStandard__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__IsStandard__c != nil) && strlen(_SVMXDEV__IsStandard__c))
                    {
                        [SVMXDEV__IsStandard__c release];
                        SVMXDEV__IsStandard__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__IsStandard__c];
                    }
                    
                    char *_SVMXDEV__Submodule__c = (char *) synchronized_sqlite3_column_text(statement2,14);
                    NSString * SVMXDEV__Submodule__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Submodule__c != nil) && strlen(_SVMXDEV__Submodule__c))
                    {
                        [SVMXDEV__Submodule__c release];
                        SVMXDEV__Submodule__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Submodule__c];
                    }
                    
                    NSMutableArray * objects2 = [[NSMutableArray arrayWithObjects:Id,SVMXDEV__SubmoduleID__c,SVMXDEV__SettingID__c,SVMXDEV__Setting_Unique_ID,SVMXDEV__Settings_Name__c,SVMXDEV__Data_Type__c,SVMXDEV__Values__c,SVMXDEV__Default_Value__c,SVMXDEV__Setting_Type__c,SVMXDEV__Search_Order__c,SVMXDEV__IsPrivate__c,SVMXDEV__Active__c,SVMXDEV__Description__c,SVMXDEV__IsStandard__c,SVMXDEV__Submodule__c,nil] retain];
            ...
            //queryStatement4 = [NSMutableString stringWithFormat:@"Select Id, SVMXDEV__Setting_Configuration_Profile__c, SVMXDEV__Setting_ID__c, SVMXDEV__Internal_Value__c, SVMXDEV__Display_Value__c, SVMXDEV__Active__c, SVMXDEV__IsDefault__c FROM SettingsValue"];
            NSMutableString *queryStatement4  = [[NSMutableString alloc] initWithFormat:@"Select Id, SVMXDEV__Setting_Configuration_Profile__c, SVMXDEV__Setting_ID__c, SVMXDEV__Internal_Value__c, SVMXDEV__Display_Value__c, SVMXDEV__Active__c, SVMXDEV__IsDefault__c FROM SettingsValue"];
            ...
            NSArray * keys4 = [NSArray arrayWithObjects:
                               _ID,
                               SVMXDEV__SETTING_CONFIGURATION_PROFILE__C,
                               SVMXDEV__SETTING_ID__C,
                               SVMXDEV__INTERNAL_VALUE__C,
                               SVMXDEV__DISPLAY_VALUE__C,
                               SVMXDEV__ACTIVE__C,
                               SVMXDEV__ISDEFAULT__C,
                               nil];
            if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query4,-1, &statement4, nil) == SQLITE_OK )
            {
                while(synchronized_sqlite3_step(statement4) == SQLITE_ROW)
                {
                    ...
                    char *_SVMXDEV__Setting_Configuration_Profile__c = (char *) synchronized_sqlite3_column_text(statement4,1);
                    NSString * SVMXDEV__Setting_Configuration_Profile__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Setting_Configuration_Profile__c != nil) && strlen(_SVMXDEV__Setting_Configuration_Profile__c))
                    {
                        [SVMXDEV__Setting_Configuration_Profile__c release];
                        SVMXDEV__Setting_Configuration_Profile__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Setting_Configuration_Profile__c];
                    }
                    char *_SVMXDEV__Setting_ID__c = (char *) synchronized_sqlite3_column_text(statement4,2);
                    NSString * SVMXDEV__Setting_ID__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Setting_ID__c != nil) && strlen(_SVMXDEV__Setting_ID__c))
                    {
                        [SVMXDEV__Setting_ID__c release];
                        SVMXDEV__Setting_ID__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Setting_ID__c];
                    }
                    char *_SVMXDEV__Internal_Value__c = (char *) synchronized_sqlite3_column_text(statement4,3);
                    NSString * SVMXDEV__Internal_Value__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Internal_Value__c!= nil) && strlen(_SVMXDEV__Internal_Value__c))
                    {
                        [SVMXDEV__Internal_Value__c release];
                        SVMXDEV__Internal_Value__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Internal_Value__c];
                    }
                    char *_SVMXDEV__Display_Value__c = (char *) synchronized_sqlite3_column_text(statement4,4);
                    NSString * SVMXDEV__Display_Value__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Display_Value__c != nil) && strlen(_SVMXDEV__Display_Value__c))
                    {
                        [SVMXDEV__Display_Value__c release];
                        SVMXDEV__Display_Value__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Display_Value__c];
                    }
                    char *_SVMXDEV__Active__c = (char *) synchronized_sqlite3_column_text(statement4,5);
                    NSString * SVMXDEV__Active__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__Active__c != nil) && strlen(_SVMXDEV__Active__c))
                    {
                        [SVMXDEV__Active__c release];
                    ...
                    char *_SVMXDEV__IsDefault__c = (char *) synchronized_sqlite3_column_text(statement4,6);
                    NSString * SVMXDEV__IsDefault__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SVMXDEV__IsDefault__c != nil) && strlen(_SVMXDEV__IsDefault__c))
                    {
                        [SVMXDEV__IsDefault__c release];
                        SVMXDEV__IsDefault__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__IsDefault__c];
                    }
                    NSMutableArray * objects4 = [[NSMutableArray arrayWithObjects:Id,SVMXDEV__Setting_Configuration_Profile__c,SVMXDEV__Setting_ID__c,SVMXDEV__Internal_Value__c,SVMXDEV__Display_Value__c,SVMXDEV__Active__c,SVMXDEV__IsDefault__c,nil] retain];
            ...
            if ([GetSettingsValueArray count] > 0)
            {
                ...
                for (int i = 0; i < [GetSettingsValueArray count]; i++)
                {
                    ...
                    {
                        NSDictionary * dict = [NSDictionary dictionaryWithObject:[obj6 objectForKey:@"SVMXDEV__Internal_Value__c"] forKey:[obj6 objectForKey:@"SVMXDEV__Display_Value__c"]];
                ...
                for (int i = 0; i < [settingsInfoArray count]; i++)
                {
                    for (int j = 0; j < [settingsValueArray count]; j++)
                    {
                        ...
                        NSString * settingsValueSettingId = [[settingsValueArray objectAtIndex:j] objectForKey:@"SVMXDEV__Setting_ID__c"];
                        if ([settingsValueSettingId isEqualToString:settingsInfoSettingId])
                        {
                            ...
                            if ([[[settingsInfoArray objectAtIndex:i] objectForKey:@"SVMXDEV__Setting_Unique_ID__c"] Contains:@"IPAD004"])
                            {
                                NSString * subModuleSettingKey = [[settingsInfoArray objectAtIndex:i] objectForKey:@"SVMXDEV__Setting_Unique_ID__c"];
                                ...
                                if (intNumVal == 23)
                                {
                                    appDelegate.signatureCaptureUpload = [[[settingsValueArray objectAtIndex:j] objectForKey:@"SVMXDEV__Internal_Value__c"] boolValue];
                                }
                                if (intNumVal >= 11 && intNumVal <= 20)
                                {
                                    NSString * queryField = [[settingsValueArray objectAtIndex:j] objectForKey:@"SVMXDEV__Internal_Value__c"];
                                    ...
                                    if ([arr count] >= 2)
                                    {
                                        ...
                                        NSString * displayValue = [[settingsValueArray objectAtIndex:j] objectForKey:@"SVMXDEV__Display_Value__c"];
                                        
                                        NSString * describeQuery = [NSString stringWithFormat:@"SELECT reference_to FROM SFReferenceTo WHERE object_api_name='SVMXDEV__Service_Order__c' AND field_api_name='%@'", displayValue];
                                        ...
                                        {
                                            NSString * val = [NSString stringWithFormat:@"SVMXDEV__Service_Order__c.%@", displayValue];
                                        ...
                                        // WHERE CLAUSE
                                        NSString * where = [NSString stringWithFormat:@"%@.Id = SVMXDEV__Service_Order__c.%@", api_name, displayValue];
                                    ...
                                    {
                                        NSString * actualQuery = [NSString stringWithFormat:@"SVMXDEV__Service_Order__c.%@", queryField];
                                        [selectClause setValue:actualQuery forKey:actualQuery];
                                    }
                                }
                            }
                            
                            if ([[[settingsInfoArray objectAtIndex:i] objectForKey:@"SVMXDEV__Settings_Name__c"] Contains:@"Address Type"])
                            {
                                NSString * queryField = [[settingsValueArray objectAtIndex:j] objectForKey:@"SVMXDEV__Internal_Value__c"];
                                if ([queryField isKindOfClass:[NSNull class]])
                                {
                                    NSArray * arr = [NSArray arrayWithObjects:@"SVMXDEV__Company__r.BillingCountry",@"SVMXDEV__Company__r.BillingPostalCode", @"SVMXDEV__Company__r.BillingState",@"SVMXDEV__Company__r.BillingCity",@" SVMXDEV__Company__r.BillingStreet", nil];
                                    
                                    [self getAllReferenceFields:arr];
                                    
                                    // [appDelegate.soqlQuery appendFormat:@", %@", @"SVMXDEV__Company__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__Company__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__Company__c"];  //Service report fix : Radha
                                    continue;
                                }
                                else if ([queryField isEqualToString:@"Account Bill To"]) // SVMXDEV__Company__r
                                {
                                    NSArray * arr = [NSArray arrayWithObjects: @"SVMXDEV__Company__r.BillingCountry",@"SVMXDEV__Company__r.BillingPostalCode",@"SVMXDEV__Company__r.BillingState", @"SVMXDEV__Company__r.BillingCity", @"SVMXDEV__Company__r.BillingStreet", nil];//Service report fix : Radha
                                ...
                                else if ([queryField isEqualToString:@"Account Ship To"]) // SVMXDEV__Company__r
                                {
                                    NSArray * arr = [NSArray arrayWithObjects:@"SVMXDEV__Company__r.ShippingCountry", @"SVMXDEV__Company__r.ShippingPostalCode", @"SVMXDEV__Company__r.ShippingState", @"SVMXDEV__Company__r.ShippingCity", @"SVMXDEV__Company__r.ShippingStreet", nil];
                                ...
                                else if ([queryField isEqualToString:@"Service Location"]) // SVMXDEV__Service_Order__c
                                {
                                    
                                    // [appDelegate.soqlQuery appendFormat:@", %@", @"SVMXDEV__Street__c, SVMXDEV__City__c, SVMXDEV__State__c, SVMXDEV__Zip__c, SVMXDEV__Country__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__Street__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__Street__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__City__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__City__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__State__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__State__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__Zip__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__Zip__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__Country__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__Country__c"];//Service report fix : Radha
                                    
                                }
                                else if ([queryField isEqualToString:@"Contact Address"]) // SVMXDEV__Contact__c
                                {
                                    NSArray * arr = [NSArray arrayWithObjects:@"SVMXDEV__Contact__r.MailingStreet", @"SVMXDEV__Contact__r.MailingState", @"SVMXDEV__Contact__r.MailingPostalCode", @"SVMXDEV__Contact__r.MailingCountry", @"SVMXDEV__Contact__r.MailingCity", nil];
                                    
                                    [self getAllReferenceFields:arr];
                                    
                                    // [appDelegate.soqlQuery appendFormat:@", %@", @"SVMXDEV__Contact__c"];
                                    [selectClause setValue:@"SVMXDEV__Service_Order__c.SVMXDEV__Contact__c" forKey:@"SVMXDEV__Service_Order__c.SVMXDEV__Contact__c"]; //Service report fix : Radha
                                }
                        ...
                        NSDictionary * valueDictionary = [settingsValueArray objectAtIndex:j]; // SVMXDEV__Setting_ID__c
                        
                        NSString * Id = [keyDictionary objectForKey:@"Id"];
                        NSString * settingId = [valueDictionary objectForKey:@"SVMXDEV__Setting_ID__c"];
                        ...
                        NSString * object = [valueDictionary objectForKey:@"SVMXDEV__Display_Value__c"];
                        NSString * key = [keyDictionary objectForKey:@"SVMXDEV__Setting_Unique_ID__c"];
                        ...
                        if ([[valueDictionary objectForKey:@"SVMXDEV__Display_Value__c"] isKindOfClass:[NSNull class]])
                            continue;
                        if ([[keyDictionary objectForKey:@"SVMXDEV__Setting_Unique_ID__c"] isKindOfClass:[NSNull class]])
                            continue;
                        
                        // Filter Service Report using IPAD004
                        if ([Id isEqualToString:settingId] && [[keyDictionary objectForKey:@"SVMXDEV__SubmoduleID__c"] isEqualToString:@"IPAD004"])
                        {
                ...
                [selectClause setValue:@"SVMXDEV__Service_Order__c.Name" forKey:@"SVMXDEV__Service_Order__c.Name"];
                [selectClause setValue:@"SVMXDEV__Problem_Description__c" forKey:@"SVMXDEV__Problem_Description__c"];
                [selectClause setValue:@"SVMXDEV__Work_Performed__c" forKey:@"SVMXDEV__Work_Performed__c"];
                ...
                if (checkFromTable == nil)
                {
                    ...
                    [whereClause addObject:@"Contact.Id = SVMXDEV__Service_Order__c.SVMXDEV__Contact__c"];
                }
                else
                {
                ...
                // FROM
                [appDelegate.soqlQuery appendFormat:@" FROM SVMXDEV__Service_Order__c "];
                ...
                NSArray * array = [NSArray arrayWithObjects:@"SVMXDEV__Contact__r.Name",@"SVMXDEV__Contact__r.Phone", nil];
            ...
            NSMutableArray *WorkOrderFields = [[NSMutableArray alloc]initWithCapacity:0];
            {
                ...
                //queryStatement9 = [NSMutableString stringWithFormat:@"SELECT api_name,label,type FROM SFObjectField WHERE object_api_name = 'SVMXDEV__Service_Order__c' "];
                
                // Vipin-memopt 16-1 9493
                NSMutableString *queryStatement9 = [[NSMutableString alloc] initWithFormat:@"SELECT api_name,label,type FROM SFObjectField WHERE object_api_name = 'SVMXDEV__Service_Order__c' "];
...
/*
 ...
 char *_SVMXDEV__Setting_Unique_ID = (char *) synchronized_sqlite3_column_text(statement2,3);
 NSString * SVMXDEV__Setting_Unique_ID = @"";
 if ((_SVMXDEV__Setting_Unique_ID != nil) && strlen(_SVMXDEV__Setting_Unique_ID))
 {
 SVMXDEV__Setting_Unique_ID = [[NSString alloc] initWithUTF8String:_SVMXDEV__Setting_Unique_ID];
 }
 ...
 char *_SVMXDEV__IsPrivate__c = (char *) synchronized_sqlite3_column_text(statement2,10);
 NSString * SVMXDEV__IsPrivate__c = @"";
 if ((_SVMXDEV__IsPrivate__c != nil) && strlen(_SVMXDEV__IsPrivate__c))
 {
 SVMXDEV__IsPrivate__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__IsPrivate__c];
 }
 char *_SVMXDEV__Active__c = (char *) synchronized_sqlite3_column_text(statement2,11);
 NSString * SVMXDEV__Active__c = @"";
 if ((_SVMXDEV__Active__c != nil) && strlen(_SVMXDEV__Active__c))
 {
 SVMXDEV__Active__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__Active__c];
 }
 ...
 char *_SVMXDEV__IsStandard__c = (char *) synchronized_sqlite3_column_text(statement2,13);
 NSString * SVMXDEV__IsStandard__c = @"";
 if ((_SVMXDEV__IsStandard__c != nil) && strlen(_SVMXDEV__IsStandard__c))
 {
 SVMXDEV__IsStandard__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__IsStandard__c];
 }
 ...
 char *_SVMXDEV__IsDefault__c = (char *) synchronized_sqlite3_column_text(statement4,6);
 NSString * SVMXDEV__IsDefault__c = @"";
 if ((_SVMXDEV__IsDefault__c != nil) && strlen(_SVMXDEV__IsDefault__c))
 {
 SVMXDEV__IsDefault__c = [[NSString alloc] initWithUTF8String:_SVMXDEV__IsDefault__c];
 }
...
-(NSMutableArray *)getReportEssentials:(NSString *) record_id
{
    ...
    NSString * final_query = [NSString stringWithFormat:@"%@ SVMXDEV__Service_Order__c.local_id = '%@'", clean_Query, record_id];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [final_query UTF8String],-1, &statement5, nil) == SQLITE_OK)
    {
        while(synchronized_sqlite3_step(statement5) == SQLITE_ROW)
        {
			...
			for(int k = 0 ; k < [all_fields count] ; k++)
            {
				...
				field_name = [field_name stringByReplacingOccurrencesOfString:@"SVMXDEV__Service_Order__c." withString:@""];
...
-(NSMutableArray *)getreferncetableFieldsForReportEsentials:(NSMutableArray *)array
{
    ...
    for(int k= 0 ; k<[allkeys count]; k++)
    {
        ...
        if([key isEqualToString:@"SVMXDEV__Company__c"])
        {
            table_name = @"Account";
        }
        else if([key isEqualToString:@"SVMXDEV__Component__c"])
        {
            table_name = @"SVMXDEV__Installed_Product__c";
        }
        else if([key isEqualToString:@"SVMXDEV__Contact__c"])
        {
            table_name = @"Contact";
        }
...
-(NSString *)getTableName:(NSString *)key
{
    ...
    NSString * query = @"";
    sqlite3_stmt * statement;
    query = [NSString stringWithFormat:@"Select reference_to From SFObjectField where api_name ='%@' and object_api_name ='SVMXDEV__Service_Order__c'",key];
...
- (BOOL)hasRecordMarkedForOverrideById:(NSString *)sfId andObjectName:(NSString *)objectName
{
    NSString * selectQuery = [NSString stringWithFormat:@"select count(*) from sync_error_conflict Where error_type = 'ERROR' AND sync_type = 'PUT_UPDATE' AND object_name = '%@' and sf_id = '%@'", objectName, sfId];
   
    SMLog(kLogLevelVerbose,@"%@" , selectQuery);
    
    sqlite3_stmt *statement;

    int ret = synchronized_sqlite3_prepare_v2(appDelegate.db, [selectQuery UTF8String], -1, &statement, NULL);
    ...
    if (ret == SQLITE_OK)
    {
        while(synchronized_sqlite3_step(statement) == SQLITE_ROW)
        {
            numberOfRecordFound = synchronized_sqlite3_column_int(statement, 0);
        }
    }
    else
    {
        SMLog(kLogLevelError,@"ERROR IN query preparation : hasRecordMarkedForOverrideById %s", sqlite3_errmsg(appDelegate.db));
    }
...
- (NSString *) getApi_NameWithReference:(NSString *)reference_to
{
	...
	NSString * selectQuery = [NSString stringWithFormat:@"Select DISTINCT api_name,label from SFObjectField where object_api_name = 'SVMXDEV__Service_Order__c' and reference_to = '%@'", reference_to];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, _selectQuery,-1, &statement, nil) == SQLITE_OK)
    {
        ...
        while(synchronized_sqlite3_step(statement) == SQLITE_ROW){
			...
			if ([api_name Contains:@"SVMXDEV"] && [reference_to Contains:label])
			{
...
{
	NSString * value = @"";
	NSString * selectQuery = [NSString stringWithFormat:@"Select %@ from SVMXDEV__Service_Order__c where local_id = '%@'", field_name, appDelegate.sfmPageController.recordId];
...
- (NSArray *)getPriceBook:(NSDictionary *)currentContext {
    ...
    @try {
       ...
       /* get the header and detail records*/
        ...
        NSString *workOrderProduct = [self getValueFOrKey:@"SVMXDEV__Product__c" FromArray:headerFieldArray];
        ...
        for(int counter = 0;counter < [detailRecords count];counter++){
            ...
            for(int counter = 0;counter < [records count];counter++){
                ...
                NSString *productId =  [self getValueFOrKey:@"SVMXDEV__Product__c" FromArray:detalFiledsArray];
                if (![Utility isStringEmpty:productId]) {
                        [productsArray addObject:productId];
                }
                
                NSString *activityType =  [self getValueFOrKey:@"SVMXDEV__Activity_Type__c" FromArray:detalFiledsArray];
        ...
        NSString *SVMXDEV__Warranty__c = [entitlementDict objectForKey:@"SVMXDEV__Warranty__c"];
        NSString *SVMXDEV__Service_Contract__c = [entitlementDict objectForKey:@"SVMXDEV__Service_Contract__c"];
        NSString *SVMXDEV__Entitled_By_Service__c =  [entitlementDict objectForKey:@"SVMXDEV__Entitled_By_Service__c"];
        if (![Utility isStringEmpty:SVMXDEV__Entitled_By_Service__c]) {
            idOfServiceOffering = SVMXDEV__Entitled_By_Service__c;
        }
        
        NSString *SVMXDEV__Entitled_Within_Threshold__c = [entitlementDict objectForKey:@"SVMXDEV__Entitled_Within_Threshold__c"];
        if ([SVMXDEV__Entitled_Within_Threshold__c isEqualToString:@"true"] ||[SVMXDEV__Entitled_Within_Threshold__c isEqualToString:@"True"] || [SVMXDEV__Entitled_Within_Threshold__c isEqualToString:@"1"]  ) {
            idServiceCovered = @"COVERED";
        }
        
       
        /* Wo comes under warranty then get warranty_id */
        if (![Utility isStringEmpty:SVMXDEV__Warranty__c]) {
            
            NSMutableDictionary *warrantyDictionary =  [self getRecordForSfId:SVMXDEV__Warranty__c andTableName:@"SVMXDEV__Warranty__c"];
            
            [warrantyDictionary setObject:[NSDictionary dictionaryWithObjectsAndKeys:@"SVMXDEV__Warranty__c",@"type",nil] forKey:@"attributes"];
        ...
        else if(![Utility isStringEmpty:SVMXDEV__Service_Contract__c]){
            
            /* else If wo comes under contract then get contract_id */
            NSMutableDictionary *serviceContractDictionary = [self getRecordForSfId:SVMXDEV__Service_Contract__c
                                                                       andTableName:@"SVMXDEV__Service_Contract__c"];
            
            /*Get Service Contract pricebook definition for Parts */
            NSString *SVMXDEV__Default_Parts_Price_Book__c =  [serviceContractDictionary objectForKey:@"SVMXDEV__Default_Parts_Price_Book__c"];
            if (![Utility isStringEmpty:SVMXDEV__Default_Parts_Price_Book__c]) {
                /* get the price book info */
                [partsPriceBookIdsArray addObject:SVMXDEV__Default_Parts_Price_Book__c];
                NSDictionary *tempDictionary =  [self preparePBEstimateId:SVMXDEV__Default_Parts_Price_Book__c andUsageValue:SVMXDEV__Default_Parts_Price_Book__c andKey:@"RECORDTYPEINFO_PARTS_CONTRACT" andRecordTypeId:recordTypeIds];
            ...
            NSString *SVMXDEV__Service_Pricebook__c =  [serviceContractDictionary objectForKey:@"SVMXDEV__Service_Pricebook__c"];
            if (![Utility isStringEmpty:SVMXDEV__Service_Pricebook__c]) {
                [labourPriceBookIdsArray addObject:SVMXDEV__Service_Pricebook__c];
                NSDictionary *tempDictionary =  [self preparePBLaourEstimateId:SVMXDEV__Service_Pricebook__c  andUsageValue:SVMXDEV__Service_Pricebook__c andKey:@"RECORDTYPEINFO_LABOR_CONTRACT" andRecordTypeId:recordTypeIds];
                if (tempDictionary != nil) {
                    [priceBookArray addObject:tempDictionary];
                }
            }
            ...
            /* Getting data for SVMXDEV__Pricing_Rule__,SVMXDEV__Parts_Pricing__c,SVMXDEV__Parts_Discount__c,,SVMXDEV__Labor_Pricing__c,SVMXDEV__Expense_Pricing__c,SVMXDEV__Travel_Policy__c,SVMXDEV__Mileage_Tiers__c,SVMXDEV__Zone_Pricing__c
             */

            
            NSAutoreleasePool *aPool = [[NSAutoreleasePool alloc] init];
            
            [self fillUpContractInformationInTheTargetArray:priceBookArray andContractId:SVMXDEV__Service_Contract__c andCurrency:currencyCode andNamedExpressionArray:namedExpressionArray andIdOffering:idOfServiceOffering  andCovered:idServiceCovered];
        ...
        /* Get IB Warranty*/
        //Check if targetRecordId is local Id
        //10760
        NSArray *allWarrantyArray = [self getIBWarrantyArray:appDelegate.sfmPageController.recordId andWorkOrderSfid:targetRecordId];
        ...
        [priceBookArray addObject: offlineDict];
        [offlineDict release];
        offlineDict = nil;
        ...
        /*Fix for JCI enhancement*/
        /*Adding one more value map to the request*/
        NSMutableArray * valueMapArray = [[NSMutableArray alloc] initWithCapacity:0];
        NSString * settingValue = [self getPricebookInformationForSettingId:@"WORD005_SET019"];
        if (settingValue != nil && [settingValue length] > 0)
        {
            NSDictionary * dict = [[NSDictionary alloc] initWithObjectsAndKeys:@"WORD005_SET019", @"key", settingValue, @"value", nil];
            [valueMapArray addObject:dict];
            [dict release];
        }
        
        NSDictionary * setttingDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"SETTINGS",@"key",valueMapArray, @"valueMap", nil];
        [priceBookArray addObject: setttingDict];
        [setttingDict release];
        setttingDict = nil;
        [valueMapArray release];
        valueMapArray = nil;
...
- (void)fillUpContractInformationInTheTargetArray:(NSMutableArray *)priceBookArray
                                    andContractId:(NSString *)SVMXDEV__Service_Contract__c
                                       ...
                                       andCovered:(NSString *)idServiceCovered
{
    ...
    [columnNames setObject:SVMXDEV__Service_Contract__c forKey:@"SVMXDEV__Service_Contract__c"];
    
    
    /* Get pricing rules for Contract*/
    NSArray *dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Pricing_Rule__c"];
    ...
    [columnNames setObject:@"1"  forKey:@"SVMXDEV__Active__c"];
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Parts_Pricing__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Parts_Discount__c"];
    ...
    [finalDict release];
    finalDict = nil;
    [columnNames removeObjectForKey:@"SVMXDEV__Active__c"];
    
    /* Get special labor pricing definition */
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Labor_Pricing__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Expense_Pricing__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Travel_Policy__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Mileage_Tiers__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Zone_Pricing__c"];
    ...
    if (idOfServiceOffering != nil) {
        [columnNames removeObjectForKey:@"SVMXDEV__Service_Contract__c"];
        [columnNames setObject:idOfServiceOffering forKey:@"Id"];
        dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SVMXDEV__Service_Contract_Services__c"];
...
- (NSDictionary *)getEntitlementHistoryForWorkorder:(NSString *)workOrderId {
    ...
    NSString *tableName = @"SVMXDEV__Entitlement_History__c";
    ...
    NSString *sqlQuery = [NSString  stringWithFormat:@"select %@ from %@ where SVMXDEV__Service_Order__c = '%@' and SVMXDEV__Inactive_Date__c = \"\"",allColumnNamesString,tableName,workOrderId];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sqlQuery UTF8String], -1, &selectStmt, nil) == SQLITE_OK  )
    {
        ...
        while(sqlite3_step(selectStmt) == SQLITE_ROW)
        {
            ...
            NSString *tempStringNew = [dataDictionary objectForKey:@"SVMXDEV__Entitled_Within_Threshold__c"];
            if ([Utility isItTrue:tempStringNew]) {
                tempStringNew = @"true";
            }else{
                tempStringNew = @"false";
            }
            [dataDictionary setObject:tempStringNew forKey:@"SVMXDEV__Entitled_Within_Threshold__c"];
            
            //Defect:9090 Shubha
            
            /* get the date and check whether it is valid */
            
            //            NSString *startDateString = [dataDictionary objectForKey:@"SPR14__Start_Date__c"];
            //            NSString *endDateString = [dataDictionary objectForKey:@"SPR14__End_Date__c"];
            //
...
- (NSArray *)getRecordWhereColumnNamesAndValues:(NSDictionary *)columnKeyAndValue  andTableName:(NSString *)tableName {
    ...
    for (NSString *columnKey in allKeys) {
        ...
        counter++;
        if ([columnKey isEqualToString:@"SVMXDEV__Active__c"]) {
            
            [someString appendFormat:@"( SVMXDEV__Active__c = '1' OR  SVMXDEV__Active__c = 'True' OR SVMXDEV__Active__c = 'true')"];
        }
        else {
...
- (NSArray *)getProductRecords:(NSArray *)productIdentifiers {
    ...
    NSString *sqlQuery = [NSString stringWithFormat:@"Select Id, SVMXDEV__Product_Line__c, Family From Product2 where Id IN %@",productIdentfierStr];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sqlQuery UTF8String], -1, &selectStmt, nil) == SQLITE_OK  )
    {
        ...
        while(sqlite3_step(selectStmt) == SQLITE_ROW)
        {
            ...
            if (tempCharString != nil) {
                tempString = [NSString stringWithUTF8String:tempCharString];
                [recordDictionary setObject:tempString forKey:@"SVMXDEV__Product_Line__c"];
            }
...
- (NSArray *)getPriceBookObjectsForLabourPriceBookIds:(NSArray *)priceBookIds OrPriceBookNames:(NSArray *)priceBookNames {
    ...
    NSString *isActiveString = @"( SVMXDEV__Active__c = '1' OR SVMXDEV__Active__c = 'true'  OR SVMXDEV__Active__c = 'True' )";
    NSString *sqlQuery = nil;
    
    if (priceBookIdString != nil && priceBookNameString != nil) {
        sqlQuery = [NSString stringWithFormat:@"SELECT Id, Name FROM SVMXDEV__Service_Pricebook__c WHERE (Id IN %@ OR Name IN %@) and %@",priceBookIdString,priceBookNameString,isActiveString];
    }
    else if (priceBookIdString != nil){
        
        sqlQuery = [NSString stringWithFormat:@"SELECT Id, Name FROM SVMXDEV__Service_Pricebook__c WHERE Id IN %@ and %@ ",priceBookIdString,isActiveString];
        
    }else if(priceBookNameString != nil){
        sqlQuery = [NSString stringWithFormat:@"SELECT Id, Name FROM SVMXDEV__Service_Pricebook__c WHERE Name IN %@ and %@ ",priceBookNameString,isActiveString];
    }
...
- (void)addNamedExpressionsFrom:(NSArray *)dataArray ToArray:(NSMutableArray *)namedExpressionArray {
    ...
    for (int counter = 0; counter < [dataArray count]; counter ++) {
        ...
        NSString  *nameExpressionId = [tempDictionary objectForKey:@"SVMXDEV__Named_Expression__c"];
...
- (NSArray *)getNamedExpressionsForIds:(NSArray *)namedExpressionArray {
    ...
    NSString *sqlQuery = [NSString stringWithFormat:@"Select SVMXDEV__Advance_Expression__c,Id from SVMXDEV__ServiceMax_Processes__c where %@ and recordTypeId = '%@'",concatenatedExpId,recordTypeRule];
    sqlite3_stmt *selectStmt = nil;
...
- (NSArray *) getExpressionComponentsForExpressionId:(NSString *)expressionId andExpression:(NSString *)expressionName andRecordId:(NSString *)recordTypeId{
    ...
    NSString *sqlQuery = [NSString stringWithFormat:@"Select SVMXDEV__Field_Name__c,SVMXDEV__Operator__c,SVMXDEV__Operand__c,SVMXDEV__Sequence__c,SVMXDEV__Expression_Type__c,SVMXDEV__Expression_Rule__c,Id from SVMXDEV__SERVICEMAX_CONFIG_DATA__C where SVMXDEV__Expression_Rule__c LIKE '%@%%' and recordTypeId = '%@'",expressionId,recordTypeId];
    sqlite3_stmt *selectStmt = nil;
        ...
        while(sqlite3_step(selectStmt) == SQLITE_ROW) {
            ...
            if (tempCharString != nil) {
                ...
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SVMXDEV__Field_Name__c"];
                }
            ...
            if (tempCharString != nil) {
                 someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SVMXDEV__Operator__c"];
                }
            ...
            if (tempCharString != nil) {
                 someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SVMXDEV__Operand__c"];
                }
            ...
            if (tempCharString != nil) {
                someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SVMXDEV__Sequence__c"];
                }
            ...
            if (tempCharString != nil) {
                someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SVMXDEV__Expression_Type__c"];
                }
            ...
            if (tempCharString != nil) {
                someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SVMXDEV__Expression_Rule__c"];
                }
            ...
            if ([recordDictionary count] > 0) {
                ...
                NSDictionary *tempDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"SVMXDEV__ServiceMax_Config_Data__c",@"type", nil];
                [recordDictionary setObject:tempDict forKey:@"attributes"];
                [tempDict release];
                tempDict = nil;
...
- (NSArray *)getPriceBookEntryForLabourArray:(NSArray *)labourArray andPriceBookIds:(NSArray *)priceBookIds andCurrency:(NSString *)currency {
    NSString *tableName = @"SVMXDEV__Service_Pricebook_Entry__c";
    NSString *priceBookIdString = [self getConcatenatedStringFromArray:priceBookIds withSingleQuotesAndBraces:YES];
    ...
    NSString *sqlQuery = nil;
    if (currency == nil) {
        sqlQuery = [[NSString alloc] initWithFormat:@"Select %@ from %@ where  SVMXDEV__Activity_Type__c IN %@ AND SVMXDEV__Price_Book__c IN %@",allColumnNamesString,tableName,labourString,priceBookIdString];
    }
    else {
        sqlQuery = [[NSString alloc] initWithFormat:@"Select %@ from %@ where SVMXDEV__Activity_Type__c IN %@ AND SVMXDEV__Price_Book__c IN %@ and CurrencyIsoCode = '%@'",allColumnNamesString,tableName,labourString,priceBookIdString,currency];
    }
...
- (NSArray *)getValidLabourPriceBookNames:(NSArray *)labourPbNames andLabourIdArray:(NSArray *)labourIdArray andCurrency:(NSString *)currency{
    ...
    NSString *tablename = @"SVMXDEV__Service_Pricebook__c";
    ...
    NSString *activeString = @"(SVMXDEV__Active__c = '1' OR SVMXDEV__Active__c = 'True' OR SVMXDEV__Active__c = 'true')";
...
- (NSArray *)getIBWarrantyArray:(NSString *)wordOrderLocalId  andWorkOrderSfid:(NSString *)workOrderSfid{
    ...
    NSDictionary *productServiceWorkdDetail = [self getProductServiceWorkDetail:wordOrderLocalId andWorkorderSfid:workOrderSfid];
    //[id , warantyID]
    ...
    if ([productServiceWorkdDetail count] > 0) {
        ...
        //10760
    NSDictionary *allWorkOrderIds =  [self getWorkDetailForProductServiceArray:[productServiceWorkdDetail allKeys] andWorkOrderIds:wordOrderLocalId andSfid:workOrderSfid];
        
        /* Get All warranties */
        NSDictionary *allWarranties = [self getAllRecordForSfIds:[productServiceWorkdDetail allValues] andTableName:@"SVMXDEV__Warranty__c"];
...
//10760
- (NSDictionary *)getProductServiceWorkDetail:(NSString *)wordOrderId andWorkorderSfid:(NSString *)sfid{
    ...
    {
        //10760
        NSString * query = [NSString stringWithFormat:@"SELECT local_id , SVMXDEV__Product_Warranty__c from 'SVMXDEV__Service_Order_Line__c' where RecordTypeId = '%@' and (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@')" , serviceRecordTypeId , wordOrderId,sfid];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
            {
                char * sfIdChar  = (char *)synchronized_sqlite3_column_text(stmt, 0);
                
                if(sfIdChar!= nil)
                    sfId = [NSString stringWithUTF8String:sfIdChar];
                
                
                char * warIdChar  = (char *)synchronized_sqlite3_column_text(stmt, 1);
                if(warIdChar!= nil)
                    warrantyId = [NSString stringWithUTF8String:warIdChar];
                
                
                
                
                if(warrantyId != nil && sfId != nil)
                {
    ...
    return [warrantyAndSfid autorelease];

}
//10760
- (NSDictionary *)getAllSfidsForLocalIds:(NSString *)localIdString {
    
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,local_id from 'SVMXDEV__Service_Order_Line__c' WHERE local_id in %@" , localIdString];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
        {
            ...
            char * localIdChar  = (char *)synchronized_sqlite3_column_text(stmt, 1);
            
            if(localIdChar!= nil)
                localId = [NSString stringWithUTF8String:localIdChar];
...
- (NSDictionary *)getWorkDetailForProductServiceArray:(NSArray *)productServiceIds
   ...
   NSString *idString =   [Utility getConcatenatedStringFromArray:productServiceIds withSingleQuotesAndBraces:YES];
    ...
    NSString *sfidString =   [Utility getConcatenatedStringFromArray:[sfidDict allKeys] withSingleQuotesAndBraces:YES];
    
    NSString * sfId = @"", *serviceId = nil;
    if(wordOrderId != nil || [wordOrderId length ] != 0)
    {
        //10760
        NSString * query = [NSString stringWithFormat:@"SELECT local_id, SVMXDEV__Work_Detail__c from 'SVMXDEV__Service_Order_Line__c' where ( SVMXDEV__Work_Detail__c IN %@ OR SVMXDEV__Work_Detail__c IN %@ ) and (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@')" , idString ,sfidString, wordOrderId,wosfid];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
            {
                ...
                char * warIdChar  = (char *)synchronized_sqlite3_column_text(stmt, 1);
                if(warIdChar!= nil)
                    serviceId = [NSString stringWithUTF8String:warIdChar];
    ...
    return [serviceIdsAndSfid autorelease];
...
- (NSMutableDictionary *)getAllRecordForSfIds:(NSArray *)allIds andTableName:(NSString *)tableName {
    ...
    return [allRecordsDictionary autorelease];
}
//price cal 2.0
#pragma mark -End
#pragma mark -
...
- (NSDictionary *)getCodeSnippetForId:(NSString *)codeSnippetId {
    ...
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,SVMXDEV__Data__c  from SVMXDEV__Code_Snippet__c  where Id = '%@' and SVMXDEV__Data__c <> \"\"" , codeSnippetId];
    sqlite3_stmt * stmt;
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
...
- (NSDictionary *)getCodeSnippetForName:(NSString *)codeSnippetName {
    ...
    NSString * fieldId = @"", *fieldData = nil;
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,SVMXDEV__Data__c  from SVMXDEV__Code_Snippet__c  where SVMXDEV__Name__c  = '%@' and SVMXDEV__Data__c <> \"\"" , codeSnippetName];
    sqlite3_stmt * stmt;
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (sqlite3_step(stmt)  == SQLITE_ROW)
...
- (NSString *)getCodeSnippetRefererenceForId:(NSString *)codeSnippetReference {
    ...
    NSString * SVMXDEV__Code_Snippet_Manifest__c = nil;
    NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV__Referenced_Code_Snippet__c  from SVMXDEV__Code_Snippet_Manifest__c  where SVMXDEV__Code_Snippet__c = '%@' and SVMXDEV__Referenced_Code_Snippet__c <> \"\"" ,codeSnippetReference];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (sqlite3_step(stmt)  == SQLITE_ROW)
        {
            ...
            if(temp_process_id!= nil) {
                SVMXDEV__Code_Snippet_Manifest__c = [NSString stringWithUTF8String:temp_process_id];
    ...
    return SVMXDEV__Code_Snippet_Manifest__c;
}
...
- (NSString *)getEntitlementStatus:(NSString *)recordIdentfier recordIdFromTable:(NSString *)tableName {
    ...
    NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV__Is_Entitlement_Performed__c FROM %@ where Id = '%@'",tableName,recordIdentfier];
    sqlite3_stmt * stmt;
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
...
- (BOOL)doesAllRecordsForGetPriceCalculationExist:(NSString *)recordId {
    ...
    if(someDictionary != nil && [someDictionary count] > 0 ){
        ...
        NSString *SVMXDEV__Warranty__c         = [someDictionary objectForKey:@"SVMXDEV__Warranty__c"];
        NSString *SVMXDEV__Service_Contract__c = [someDictionary objectForKey:@"SVMXDEV__Service_Contract__c"];
        NSInteger  numberOfRecordsOne = -1 , numberOfRecordsTwo = -1;
        if (![Utility isStringEmpty:SVMXDEV__Warranty__c]) {
            numberOfRecordsOne =  [self getRecordCountForSfId:SVMXDEV__Warranty__c andTableName:@"SVMXDEV__Warranty__c"];
        }
        else {
            numberOfRecordsOne = 1;
        }
        
        if (![Utility isStringEmpty:SVMXDEV__Service_Contract__c]) {
            numberOfRecordsTwo =  [self getRecordCountForSfId:SVMXDEV__Service_Contract__c andTableName:@"SVMXDEV__Service_Contract__c"];
        }
        else {
...
- (BOOL)checkIfBillablePriceExistForWorkOrderId:(NSString *)workOrderLocalId andFieldName:(NSString *)fieldName {
    ...
    NSString *sfmId = [self getSFIdForlocalId:workOrderLocalId andTableName:@"SVMXDEV__Service_Order__c"];
    
    /* Get all lines and check if one billable price exist and non empty  */
    
    NSString *sqlQuery = [NSString stringWithFormat:@"select %@ from SVMXDEV__Service_Order_Line__c where (SVMXDEV__Service_Order__c = '%@' OR SVMXDEV__Service_Order__c = '%@')", fieldName,workOrderLocalId,sfmId ];
    sqlite3_stmt * stmt;
