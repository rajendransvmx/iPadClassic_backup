    ...
    if (ret == SQLITE_OK)
    {
        ...
        while (ret == SQLITE_ROW)
        {
           ...
           // retVal = [self isWorkOrder:whatId1];
			...
			retVal = [self isWorkOrderOrCase:whatId1 objectName:@"SPR14__Service_Order__c"];
            ...
            if ( retVal == YES && (!([whatId1 isEqualToString:@""]) || whatId1 != nil) )
            {
                ...
                queryStatement1 = [NSMutableString stringWithFormat:@"Select Name,SPR14__City__c,SPR14__Zip__c,SPR14__Street__c,SPR14__State__c,SPR14__Country__c From SPR14__Service_Order__c where Id = '%@'",whatId1];
...
- (BOOL) isWorkOrderOrCaseForColorCode:(NSString *)whatId
{
    ...
    synchronized_sqlite3_finalize(stmt);
    if ([objectName isEqualToString:@"Case"] ||[objectName isEqualToString:@"SPR14__Service_Order__c"] )
    {
        return YES;
    }
...
- (NSString *)getColorCodeForPriority:(NSString *)whatId objectname:(NSString *)objectName
{
    ...
    if ( retVal == YES )
    {
		...
		if ([objectName isEqualToString:@"Case"])
			...
			queryStatement = [NSString stringWithFormat:@"SELECT SPR14__Priority__c from '%@' where Id = '%@'", objectName, whatId];
        const char * selectStatement = [queryStatement UTF8String];
...
-(NSString*)getNameField:(NSString *)SPR14__Product__c
{
    NSMutableString *queryStatement = [[NSMutableString alloc]initWithCapacity:0];
    NSString * Name1 = @"";
    sqlite3_stmt *statement1;
//    defect 007237
    SPR14__Product__c=[SPR14__Product__c stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
    
    queryStatement = [NSString stringWithFormat:@"SELECT Name from Product2 where Id = '%@'", SPR14__Product__c];
    const  char * selectStatement = [queryStatement UTF8String];
...
-(NSMutableArray*)queryForExpenses:(NSString *)currentRecordId
{
    ...
    NSString * sfid =[appDelegate.databaseInterface getSfid_For_LocalId_From_Object_table:@"SPR14__Service_Order__c" local_id:currentRecordId];
    
    if ((sfid != nil) && [sfid length] > 0)
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Expense_Type__c, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c,SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Expenses' AND (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@')  AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    }
    else
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Expense_Type__c, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c,SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Expenses' AND SPR14__Service_Order__c = '%@' AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    }
    ...
    NSArray * keys = [NSArray arrayWithObjects:
                      _ID,
                      SPR14__EXPENSE_TYPE__C,
                      SPR14__ACTUAL_QUANTITY2__C,
                      SPR14__ACTUAL_PRICE2__C,
                      SPR14__WORK_DESCRIPTION__C,
                      SPR14_BILLABLE_QUANTITY,
                      SPR14_BILLABLE_PRICE,
                      nil];
    @try{
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement1, nil) == SQLITE_OK )
    {
        while(synchronized_sqlite3_step(statement1) == SQLITE_ROW)
        {
            ...
            char *_SPR14__Expense_Type__c = (char *) synchronized_sqlite3_column_text(statement1,1);
            NSString * SPR14__Expense_Type__c = @"";
            if ((_SPR14__Expense_Type__c != nil) && strlen(_SPR14__Expense_Type__c))
            {         
                SPR14__Expense_Type__c = [[NSString alloc] initWithUTF8String:_SPR14__Expense_Type__c];
            }  
            char *_SPR14__Actual_Quantity2__c = (char *) synchronized_sqlite3_column_text(statement1,2);
            NSString * SPR14__Actual_Quantity2__c_ = @"";    
            if ((_SPR14__Actual_Quantity2__c != nil) && strlen(_SPR14__Actual_Quantity2__c))
            {         
                SPR14__Actual_Quantity2__c_ = [[NSString alloc] initWithUTF8String:_SPR14__Actual_Quantity2__c];
            }
            char *_SPR14__Actual_Price2__c = (char *) synchronized_sqlite3_column_text(statement1,3);
            NSString * SPR14__Actual_Price2__c_ = @"";    
            if ((_SPR14__Actual_Price2__c != nil) && strlen(_SPR14__Actual_Price2__c))
            {         
                SPR14__Actual_Price2__c_ = [[NSString alloc] initWithUTF8String:_SPR14__Actual_Price2__c];
            }   
            char *_SPR14__Work_Description_c = (char *) synchronized_sqlite3_column_text(statement1,4);
            NSString * SPR14__Work_Description_c = @"";    
            if ((_SPR14__Work_Description_c != nil) && strlen(_SPR14__Work_Description_c))
            {         
                SPR14__Work_Description_c = [[NSString alloc] initWithUTF8String:_SPR14__Work_Description_c];
            }
            ...
            NSMutableArray * objects = [[NSMutableArray arrayWithObjects:Id,SPR14__Expense_Type__c, SPR14__Actual_Quantity2__c_,SPR14__Actual_Price2__c_,SPR14__Work_Description_c,billableQty,billablePrice,nil] retain];
...
- (NSMutableArray *) queryForParts:(NSString *)currentRecordId
{
    ...
    if ((sfid != nil) && [sfid length] > 0)
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Product__c,SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c, SPR14__Discount__c, Name , SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Parts' AND (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@')  AND SPR14__Actual_Quantity2__c > 0 AND SPR14__Actual_Price2__c >= 0 AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    }
    else
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Product__c,SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c, SPR14__Discount__c, Name , SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Parts' AND SPR14__Service_Order__c = '%@' AND SPR14__Actual_Quantity2__c > 0 AND SPR14__Actual_Price2__c >= 0 AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    }
    ...
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement1, nil) == SQLITE_OK )
    {
        ...
        while(synchronized_sqlite3_step(statement1) == SQLITE_ROW)
        {
            ...
            char *_SPR14__Product__c = (char *) synchronized_sqlite3_column_text(statement1,1);
            NSString * SPR14__Product__c = @"";
            if ((_SPR14__Product__c != nil) && strlen(_SPR14__Product__c))
            {         
                SPR14__Product__c = [[NSString alloc] initWithUTF8String:_SPR14__Product__c];
                nameField = [self getNameField:SPR14__Product__c];
				
				//Shrinivas   --- Change for service Report  -- 22/06/2012
				if ( [nameField isEqualToString:@""])
				{
					nameField = [self getProductNameFromDbWithID:SPR14__Product__c];
				}
            } 
            ...
            char *_SPR14__Discount__c = (char *) synchronized_sqlite3_column_text(statement1,5);
            NSString * SPR14__Discount__c = @"";    
            if ((_SPR14__Discount__c != nil) && strlen(_SPR14__Discount__c))
            {         
                SPR14__Discount__c = [[NSString alloc] initWithUTF8String:_SPR14__Discount__c];
            } 
            
            
            /* Fetching billable quantitye 6773*/
            char *tempValue = (char *) synchronized_sqlite3_column_text(statement1,7);
            NSString * _SPR14__Billable_Quantity__c = @"0.0";
            if ((tempValue != nil) && strlen(tempValue))
            {
                _SPR14__Billable_Quantity__c = [[NSString alloc] initWithUTF8String:tempValue];
                if (_SPR14__Billable_Quantity__c == nil) {
                    _SPR14__Billable_Quantity__c = @"0.0";
                }
            }
            
            tempValue = nil;
            tempValue = (char *) synchronized_sqlite3_column_text(statement1,8);
            NSString * _SPR14__Billable_Line_Price__c = @"0.0";
            if ((tempValue != nil) && strlen(tempValue))
            {
                _SPR14__Billable_Line_Price__c = [[NSString alloc] initWithUTF8String:tempValue];
                if (_SPR14__Billable_Line_Price__c == nil) {
                    _SPR14__Billable_Line_Price__c = @"0.0";
                }
            }
            ...
            NSMutableArray * objects = [[NSMutableArray arrayWithObjects:Id,SPR14__Product__c,nameField,SPR14__Actual_Quantity2__c_,SPR14__Actual_Price2__c_,SPR14__Work_Description_c,SPR14__Discount__c,_SPR14__Billable_Quantity__c,_SPR14__Billable_Line_Price__c,nil] retain];
            ...
            NSString * numPartsUsed = [dictionary objectForKey:gSPR14__Actual_Quantity2__c];
            ...
            NSString * description = [dictionary objectForKey:gSPR14__Work_Description__c];
            if ([description isKindOfClass:[NSString class]])
                description = [NSString stringWithFormat:@"%@", description];
            
            NSString * costPerPart = @"";
            costPerPart = [dictionary objectForKey:gSPR14__Actual_Price2__c];
            
            NSString * keyProduct = @"";
            keyProduct = [dictionary objectForKey:gSPR14__Product__c];
            
            NSString * discount = [dictionary objectForKey:gSPR14__Discount__c];
            if ([discount isKindOfClass:[NSString class]])
                discount = [NSString stringWithFormat:@"%@", discount];
            
            NSString  *keyBillableQty = [dictionary objectForKey:SPR14_BILLABLE_QUANTITY];
            
            NSString  *keyBillablePrice = [dictionary objectForKey:SPR14_BILLABLE_PRICE];
...
{ 
    ...
    @try{
        ...
        if ((sfid != nil) && [sfid length] > 0)
        {
            queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Activity_Type__c, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c,SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Labor' AND (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@' ) AND  (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    ...
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement1, nil) == SQLITE_OK )
    {
        while(synchronized_sqlite3_step(statement1) == SQLITE_ROW)
        {
            ...
            char *_SPR14__Activity_Type__c = (char *) synchronized_sqlite3_column_text(statement1,1);
            NSString * SPR14__Activity_Type__c_ = @"";
            if ((_SPR14__Activity_Type__c != nil) && strlen(_SPR14__Activity_Type__c))
            {         
                SPR14__Activity_Type__c_ = [[NSString alloc] initWithUTF8String:_SPR14__Activity_Type__c];
            }  
            char *_SPR14__Actual_Quantity2__c = (char *) synchronized_sqlite3_column_text(statement1,2);
            NSString * SPR14__Actual_Quantity2__c_ = @"";    
            if ((_SPR14__Actual_Quantity2__c != nil) && strlen(_SPR14__Actual_Quantity2__c))
            ...
            NSMutableArray * objects = [[NSMutableArray arrayWithObjects:Id,SPR14__Activity_Type__c_,SPR14__Actual_Quantity2__c_,SPR14__Actual_Price2__c_,SPR14__Work_Description_c,billableQty,billablePrice,nil] retain];
            ...
            if ([[dictionary objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:CALIBRATION])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Price2__c"] forKey:RATE_CALIBRATION];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_CALIBRATION]
                ;
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_PRICE ] forKey:BILL_RATE_CALIBRATION];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_QUANTITY] forKey:BILL_QTY_CALIBRATION];
                
            }
            if ([[dictionary objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:CLEANUP])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Price2__c"] forKey:RATE_CLEANUP];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_CLEANUP];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_PRICE ] forKey:BILL_RATE_CLEANUP];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_QUANTITY] forKey:BILL_QTY_CLEANUP];

                
            }
            if ([[dictionary objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:INSTALLATION])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Price2__c"] forKey:RATE_INSTALLATION];
                [LabourValuesDictionary setValue:[dictionary  objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_INSTALLATION];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_PRICE ] forKey:BILL_RATE_INSTALLATION];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_QUANTITY] forKey:BILL_QTY_INSTALLATION];
            }
            if ([[dictionary objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:REPAIR])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Price2__c"] forKey:RATE_REPAIR];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_REPAIR];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_PRICE ] forKey:BILL_RATE_REPAIR];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_QUANTITY] forKey:BILL_QTY_REPAIR];
            }
            if ([[dictionary objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:SERVICE])
            {
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Price2__c"] forKey:RATE_SERVICE];
                [LabourValuesDictionary setValue:[dictionary objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_SERVICE];
                
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_PRICE ] forKey:BILL_RATE_SERVICE];
                [LabourValuesDictionary setValue:[dictionary objectForKey:SPR14_BILLABLE_QUANTITY] forKey:BILL_QTY_SERVICE];
            }
    ...
    queryStatement6 = [NSMutableString stringWithFormat:@"SELECT SPR14__Billable_Cost2__c FROM SPR14__Service_Group_Costs__c  WHERE SPR14__Group_Member__c = '%@' AND SPR14__Cost_Category__c = 'Straight'", appDelegate.appTechnicianId];
    const char * _query6 = [queryStatement6 UTF8String];
    NSArray * keys6 = [NSArray arrayWithObjects:
                       SPR14__BILLABLE_COST2__C,
                       nil];
    if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query6,-1, &statement6, nil) == SQLITE_OK )
    {
        ...
        {
            char *_SPR14__Billable_Cost2__c = (char *) synchronized_sqlite3_column_text(statement6,0);
            NSString * SPR14__Billable_Cost2__c = @"";    
            if ((_SPR14__Billable_Cost2__c != nil) && strlen(_SPR14__Billable_Cost2__c))
            {         
                SPR14__Billable_Cost2__c = [[NSString alloc] initWithUTF8String:_SPR14__Billable_Cost2__c];
                
            }
            NSMutableArray * objects6 = [[NSMutableArray arrayWithObjects:SPR14__Billable_Cost2__c,nil] retain];
    ...
    if( array != nil && ![array count] )
	{
        ...
        queryStatement7 = [NSMutableString stringWithFormat:@"SELECT SPR14__Billable_Cost2__c FROM SPR14__Service_Group_Costs__c WHERE SPR14__Service_Group__c = '%@' AND SPR14__Cost_Category__c = 'Straight'", appDelegate.appServiceTeamId];
        ...
        if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query7,-1, &statement7, nil) == SQLITE_OK )
        {
            ...
            {
                char *_SPR14__Billable_Cost2__c = (char *) synchronized_sqlite3_column_text(statement7,0);
                NSString * SPR14__Billable_Cost2__c = @"";    
                if ((_SPR14__Billable_Cost2__c != nil) && strlen(_SPR14__Billable_Cost2__c))
                {         
                    SPR14__Billable_Cost2__c = [[NSString alloc] initWithUTF8String:_SPR14__Billable_Cost2__c];
                ...
                NSMutableArray * objects7 = [[NSMutableArray arrayWithObjects:SPR14__Billable_Cost2__c,nil] retain];
        ...
        for (int i = 0; i < [array count]; i++)
        {
            ...
            rate = [[obj8 objectForKey:@"SPR14__Billable_Cost2__c"] retain];
	...
	else
	{
        ...
        rate = [[obj7 objectForKey:@"SPR14__Billable_Cost2__c"] retain];
...
- (NSMutableArray *) queryForTravel:(NSString *)currentRecordId
{
    ...
    if ((sfid != nil) && [sfid length] > 0)
    {
        queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c, SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Travel' AND (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@' )AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId, sfid];
    }
    else
    {
       queryStatement = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c, SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Travel' AND SPR14__Service_Order__c = '%@' AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    ...
    @try{
        ...
        if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement, nil) == SQLITE_OK )
        {
            while(synchronized_sqlite3_step(statement) == SQLITE_ROW)
            {
                ...
                char *_SPR14__Actual_Quantity2__c = (char *) synchronized_sqlite3_column_text(statement,1);
                NSString * SPR14__Actual_Quantity2__c_ = @"";
                if ((_SPR14__Actual_Quantity2__c != nil) && strlen(_SPR14__Actual_Quantity2__c))
                {
                    SPR14__Actual_Quantity2__c_ = [[[NSString alloc] initWithUTF8String:_SPR14__Actual_Quantity2__c] autorelease];
                    if (SPR14__Actual_Quantity2__c_ == nil) {
                        SPR14__Actual_Quantity2__c_ = @"0.0";
                    }
                }
                
                char *_SPR14__Actual_Price2__c = (char *) synchronized_sqlite3_column_text(statement,2);
                NSString * SPR14__Actual_Price2__c_ = @"";
                if ((_SPR14__Actual_Price2__c != nil) && strlen(_SPR14__Actual_Price2__c))
                {
                    SPR14__Actual_Price2__c_ = [[[NSString alloc] initWithUTF8String:_SPR14__Actual_Price2__c] autorelease];
                    if (SPR14__Actual_Price2__c_ == nil) {
                        SPR14__Actual_Price2__c_ = @"0.0";
                    }
                }
                char *_SPR14__Work_Description_c = (char *) synchronized_sqlite3_column_text(statement,3);
                NSString * SPR14__Work_Description_c = @"";
                if ((_SPR14__Work_Description_c != nil) && strlen(_SPR14__Work_Description_c))
                {
                    SPR14__Work_Description_c = [[[NSString alloc] initWithUTF8String:_SPR14__Work_Description_c] autorelease];
                    if (SPR14__Work_Description_c == nil) {
                        SPR14__Work_Description_c = @"";
                    }
                }
                
                char *_SPR14__Billable_Quantity__c = (char *) synchronized_sqlite3_column_text(statement,4);
                NSString * billableQty = @"0.0";
                if ((_SPR14__Billable_Quantity__c != nil) && strlen(_SPR14__Billable_Quantity__c))
                {
                    billableQty = [[[NSString alloc] initWithUTF8String:_SPR14__Billable_Quantity__c] autorelease];
                    if (billableQty == nil) {
                        billableQty = @"0.0";
                    }
                }
                ...
                char *_SPR14__billable_line_price__c = (char *) synchronized_sqlite3_column_text(statement,5);
                NSString *billableLinePrice = @"0.0";
                if ((_SPR14__billable_line_price__c != nil) && strlen(_SPR14__billable_line_price__c))
                {
                    billableLinePrice = [[[NSString alloc] initWithUTF8String:_SPR14__billable_line_price__c] autorelease];
                ...
                NSMutableArray * objects = [NSMutableArray arrayWithObjects:Id,SPR14__Actual_Quantity2__c_,SPR14__Actual_Price2__c_,SPR14__Work_Description_c,billableQty,billableLinePrice,nil];
...
- (void) startQueryConfiguration
{
    @autoreleasepool
    {
        ...
        queryStatement2 = [NSMutableString stringWithFormat:@"SELECT Id, SPR14__SubmoduleID__c, SPR14__SettingID__c, SPR14__Setting_Unique_ID__c, SPR14__Settings_Name__c, SPR14__Data_Type__c, SPR14__Values__c, SPR14__Default_Value__c, SPR14__Setting_Type__c, SPR14__Search_Order__c, SPR14__IsPrivate__c, SPR14__Active__c, SPR14__Description__c, SPR14__IsStandard__c, SPR14__Submodule__c FROM SettingsInfo"];
        ...
        NSArray * keys2 = [NSArray arrayWithObjects:
                           _ID,
                           SPR14__SUBMODULEID__C,
                           SPR14__SETTINGID__C,
                           SPR14__SETTING_UNIQUE_ID__C,
                           SPR14__SETTINGS_NAME__C,
                           SPR14__DATA_TYPE__C,
                           SPR14__VALUES__C,
                           SPR14__DEFAULT_VALUE__C,
                           SPR14__SETTING_TYPE__C,
                           SPR14__SEARCH_ORDER__C,
                           SPR14__ISPRIVATE__C,
                           SPR14__ACTIVE__C,
                           SPR14__DESCRIPTION__C,
                           SPR14__ISSTANDARD__C,
                           SPR14__SUBMODULE__C,
                           nil];
        ...
        @try{
            ...
            if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query2,-1, &statement2, nil) == SQLITE_OK )
            {
                while(synchronized_sqlite3_step(statement2) == SQLITE_ROW)
                {
                    ...
                    char *_SPR14__SubmoduleID__c = (char *) synchronized_sqlite3_column_text(statement2,1);
                    NSString * SPR14__SubmoduleID__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__SubmoduleID__c != nil) && strlen(_SPR14__SubmoduleID__c))
                    {
                        [SPR14__SubmoduleID__c release];
                        SPR14__SubmoduleID__c = [[NSString alloc] initWithUTF8String:_SPR14__SubmoduleID__c];
                    }
                    
                    char *_SPR14__SettingID__c = (char *) synchronized_sqlite3_column_text(statement2,2);
                    NSString * SPR14__SettingID__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__SettingID__c != nil) && strlen(_SPR14__SettingID__c))
                    {
                        [SPR14__SettingID__c release];
                        SPR14__SettingID__c = [[NSString alloc] initWithUTF8String:_SPR14__SettingID__c];
                    }
                    
                    char *_SPR14__Setting_Unique_ID = (char *) synchronized_sqlite3_column_text(statement2,3);
                    NSString * SPR14__Setting_Unique_ID = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Setting_Unique_ID != nil) && strlen(_SPR14__Setting_Unique_ID))
                    {
                        [SPR14__Setting_Unique_ID release];
                        SPR14__Setting_Unique_ID = [[NSString alloc] initWithUTF8String:_SPR14__Setting_Unique_ID];
                    }
                    
                    char *_SPR14__Settings_Name__c = (char *) synchronized_sqlite3_column_text(statement2,4);
                    NSString * SPR14__Settings_Name__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Settings_Name__c != nil) && strlen(_SPR14__Settings_Name__c))
                    {
                        [SPR14__Settings_Name__c release];
                        SPR14__Settings_Name__c = [[NSString alloc] initWithUTF8String:_SPR14__Settings_Name__c];
                    }
                    
                    char *_SPR14__Data_Type__c = (char *) synchronized_sqlite3_column_text(statement2,5);
                    NSString * SPR14__Data_Type__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Data_Type__c != nil) && strlen(_SPR14__Data_Type__c))
                    {
                        SPR14__Data_Type__c = [[NSString alloc] initWithUTF8String:_SPR14__Data_Type__c];
                    }
                    
                    char *_SPR14__Values__c = (char *) synchronized_sqlite3_column_text(statement2,6);
                    NSString * SPR14__Values__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Values__c != nil) && strlen(_SPR14__Values__c))
                    {
                        [SPR14__Values__c release];
                        SPR14__Values__c = [[NSString alloc] initWithUTF8String:_SPR14__Values__c];
                    }
                    
                    char *_SPR14__Default_Value__c = (char *) synchronized_sqlite3_column_text(statement2,7);
                    NSString * SPR14__Default_Value__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Default_Value__c != nil) && strlen(_SPR14__Default_Value__c))
                    {
                        [SPR14__Default_Value__c release];
                        SPR14__Default_Value__c = [[NSString alloc] initWithUTF8String:_SPR14__Default_Value__c];
                    }
                    
                    char *_SPR14__Setting_Type__c = (char *) synchronized_sqlite3_column_text(statement2,8);
                    NSString * SPR14__Setting_Type__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Setting_Type__c != nil) && strlen(_SPR14__Setting_Type__c))
                    {
                        [SPR14__Setting_Type__c release];
                        SPR14__Setting_Type__c = [[NSString alloc] initWithUTF8String:_SPR14__Setting_Type__c];
                    }
                    
                    char *_SPR14__Search_Order__c = (char *) synchronized_sqlite3_column_text(statement2,9);
                    NSString * SPR14__Search_Order__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Search_Order__c != nil) && strlen(_SPR14__Search_Order__c))
                    {
                        [SPR14__Search_Order__c release];
                        SPR14__Search_Order__c = [[NSString alloc] initWithUTF8String:_SPR14__Search_Order__c];
                    }
                    
                    char *_SPR14__IsPrivate__c = (char *) synchronized_sqlite3_column_text(statement2,10);
                    NSString * SPR14__IsPrivate__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__IsPrivate__c != nil) && strlen(_SPR14__IsPrivate__c))
                    {
                        [SPR14__IsPrivate__c release];
                        SPR14__IsPrivate__c = [[NSString alloc] initWithUTF8String:_SPR14__IsPrivate__c];
                    }
                    
                    char *_SPR14__Active__c = (char *) synchronized_sqlite3_column_text(statement2,11);
                    NSString * SPR14__Active__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Active__c != nil) && strlen(_SPR14__Active__c))
                    {
                        [SPR14__Active__c release];
                        SPR14__Active__c = [[NSString alloc] initWithUTF8String:_SPR14__Active__c];
                    }
                    
                    char *_SPR14__Description__c = (char *) synchronized_sqlite3_column_text(statement2,12);
                    NSString * SPR14__Description__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Description__c != nil) && strlen(_SPR14__Description__c))
                    {
                        [SPR14__Description__c release];
                        SPR14__Description__c = [[NSString alloc] initWithUTF8String:_SPR14__Description__c];
                    }
                    
                    char *_SPR14__IsStandard__c = (char *) synchronized_sqlite3_column_text(statement2,13);
                    NSString * SPR14__IsStandard__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__IsStandard__c != nil) && strlen(_SPR14__IsStandard__c))
                    {
                        [SPR14__IsStandard__c release];
                        SPR14__IsStandard__c = [[NSString alloc] initWithUTF8String:_SPR14__IsStandard__c];
                    }
                    
                    char *_SPR14__Submodule__c = (char *) synchronized_sqlite3_column_text(statement2,14);
                    NSString * SPR14__Submodule__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Submodule__c != nil) && strlen(_SPR14__Submodule__c))
                    {
                        [SPR14__Submodule__c release];
                        SPR14__Submodule__c = [[NSString alloc] initWithUTF8String:_SPR14__Submodule__c];
                    }
                    
                    NSMutableArray * objects2 = [[NSMutableArray arrayWithObjects:Id,SPR14__SubmoduleID__c,SPR14__SettingID__c,SPR14__Setting_Unique_ID,SPR14__Settings_Name__c,SPR14__Data_Type__c,SPR14__Values__c,SPR14__Default_Value__c,SPR14__Setting_Type__c,SPR14__Search_Order__c,SPR14__IsPrivate__c,SPR14__Active__c,SPR14__Description__c,SPR14__IsStandard__c,SPR14__Submodule__c,nil] retain];
            ...
            //queryStatement4 = [NSMutableString stringWithFormat:@"Select Id, SPR14__Setting_Configuration_Profile__c, SPR14__Setting_ID__c, SPR14__Internal_Value__c, SPR14__Display_Value__c, SPR14__Active__c, SPR14__IsDefault__c FROM SettingsValue"];
            NSMutableString *queryStatement4  = [[NSMutableString alloc] initWithFormat:@"Select Id, SPR14__Setting_Configuration_Profile__c, SPR14__Setting_ID__c, SPR14__Internal_Value__c, SPR14__Display_Value__c, SPR14__Active__c, SPR14__IsDefault__c FROM SettingsValue"];
            ...
            NSArray * keys4 = [NSArray arrayWithObjects:
                               _ID,
                               SPR14__SETTING_CONFIGURATION_PROFILE__C,
                               SPR14__SETTING_ID__C,
                               SPR14__INTERNAL_VALUE__C,
                               SPR14__DISPLAY_VALUE__C,
                               SPR14__ACTIVE__C,
                               SPR14__ISDEFAULT__C,
                               nil];
            if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query4,-1, &statement4, nil) == SQLITE_OK )
            {
                while(synchronized_sqlite3_step(statement4) == SQLITE_ROW)
                {
                    ...
                    char *_SPR14__Setting_Configuration_Profile__c = (char *) synchronized_sqlite3_column_text(statement4,1);
                    NSString * SPR14__Setting_Configuration_Profile__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Setting_Configuration_Profile__c != nil) && strlen(_SPR14__Setting_Configuration_Profile__c))
                    {
                        [SPR14__Setting_Configuration_Profile__c release];
                        SPR14__Setting_Configuration_Profile__c = [[NSString alloc] initWithUTF8String:_SPR14__Setting_Configuration_Profile__c];
                    }
                    char *_SPR14__Setting_ID__c = (char *) synchronized_sqlite3_column_text(statement4,2);
                    NSString * SPR14__Setting_ID__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Setting_ID__c != nil) && strlen(_SPR14__Setting_ID__c))
                    {
                        [SPR14__Setting_ID__c release];
                        SPR14__Setting_ID__c = [[NSString alloc] initWithUTF8String:_SPR14__Setting_ID__c];
                    }
                    char *_SPR14__Internal_Value__c = (char *) synchronized_sqlite3_column_text(statement4,3);
                    NSString * SPR14__Internal_Value__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Internal_Value__c!= nil) && strlen(_SPR14__Internal_Value__c))
                    {
                        [SPR14__Internal_Value__c release];
                        SPR14__Internal_Value__c = [[NSString alloc] initWithUTF8String:_SPR14__Internal_Value__c];
                    }
                    char *_SPR14__Display_Value__c = (char *) synchronized_sqlite3_column_text(statement4,4);
                    NSString * SPR14__Display_Value__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Display_Value__c != nil) && strlen(_SPR14__Display_Value__c))
                    {
                        [SPR14__Display_Value__c release];
                        SPR14__Display_Value__c = [[NSString alloc] initWithUTF8String:_SPR14__Display_Value__c];
                    }
                    char *_SPR14__Active__c = (char *) synchronized_sqlite3_column_text(statement4,5);
                    NSString * SPR14__Active__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__Active__c != nil) && strlen(_SPR14__Active__c))
                    {
                        [SPR14__Active__c release];
                    ...
                    char *_SPR14__IsDefault__c = (char *) synchronized_sqlite3_column_text(statement4,6);
                    NSString * SPR14__IsDefault__c = [[NSString alloc] initWithString:kEmptyString];
                    if ((_SPR14__IsDefault__c != nil) && strlen(_SPR14__IsDefault__c))
                    {
                        [SPR14__IsDefault__c release];
                        SPR14__IsDefault__c = [[NSString alloc] initWithUTF8String:_SPR14__IsDefault__c];
                    }
                    NSMutableArray * objects4 = [[NSMutableArray arrayWithObjects:Id,SPR14__Setting_Configuration_Profile__c,SPR14__Setting_ID__c,SPR14__Internal_Value__c,SPR14__Display_Value__c,SPR14__Active__c,SPR14__IsDefault__c,nil] retain];
            ...
            if ([GetSettingsValueArray count] > 0)
            {
                ...
                for (int i = 0; i < [GetSettingsValueArray count]; i++)
                {
                    ...
                    {
                        NSDictionary * dict = [NSDictionary dictionaryWithObject:[obj6 objectForKey:@"SPR14__Internal_Value__c"] forKey:[obj6 objectForKey:@"SPR14__Display_Value__c"]];
                ...
                for (int i = 0; i < [settingsInfoArray count]; i++)
                {
                    for (int j = 0; j < [settingsValueArray count]; j++)
                    {
                        ...
                        NSString * settingsValueSettingId = [[settingsValueArray objectAtIndex:j] objectForKey:@"SPR14__Setting_ID__c"];
                        if ([settingsValueSettingId isEqualToString:settingsInfoSettingId])
                        {
                            ...
                            if ([[[settingsInfoArray objectAtIndex:i] objectForKey:@"SPR14__Setting_Unique_ID__c"] Contains:@"IPAD004"])
                            {
                                NSString * subModuleSettingKey = [[settingsInfoArray objectAtIndex:i] objectForKey:@"SPR14__Setting_Unique_ID__c"];
                                ...
                                if (intNumVal == 23)
                                {
                                    appDelegate.signatureCaptureUpload = [[[settingsValueArray objectAtIndex:j] objectForKey:@"SPR14__Internal_Value__c"] boolValue];
                                }
                                if (intNumVal >= 11 && intNumVal <= 20)
                                {
                                    NSString * queryField = [[settingsValueArray objectAtIndex:j] objectForKey:@"SPR14__Internal_Value__c"];
                                    ...
                                    if ([arr count] >= 2)
                                    {
                                        ...
                                        NSString * displayValue = [[settingsValueArray objectAtIndex:j] objectForKey:@"SPR14__Display_Value__c"];
                                        
                                        NSString * describeQuery = [NSString stringWithFormat:@"SELECT reference_to FROM SFReferenceTo WHERE object_api_name='SPR14__Service_Order__c' AND field_api_name='%@'", displayValue];
                                        ...
                                        {
                                            NSString * val = [NSString stringWithFormat:@"SPR14__Service_Order__c.%@", displayValue];
                                        ...
                                        // WHERE CLAUSE
                                        NSString * where = [NSString stringWithFormat:@"%@.Id = SPR14__Service_Order__c.%@", api_name, displayValue];
                                    ...
                                    {
                                        NSString * actualQuery = [NSString stringWithFormat:@"SPR14__Service_Order__c.%@", queryField];
                                        [selectClause setValue:actualQuery forKey:actualQuery];
                                    }
                                }
                            }
                            
                            if ([[[settingsInfoArray objectAtIndex:i] objectForKey:@"SPR14__Settings_Name__c"] Contains:@"Address Type"])
                            {
                                NSString * queryField = [[settingsValueArray objectAtIndex:j] objectForKey:@"SPR14__Internal_Value__c"];
                                if ([queryField isKindOfClass:[NSNull class]])
                                {
                                    NSArray * arr = [NSArray arrayWithObjects:@"SPR14__Company__r.BillingCountry",@"SPR14__Company__r.BillingPostalCode", @"SPR14__Company__r.BillingState",@"SPR14__Company__r.BillingCity",@" SPR14__Company__r.BillingStreet", nil];
                                    
                                    [self getAllReferenceFields:arr];
                                    
                                    // [appDelegate.soqlQuery appendFormat:@", %@", @"SPR14__Company__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__Company__c" forKey:@"SPR14__Service_Order__c.SPR14__Company__c"];  //Service report fix : Radha
                                    continue;
                                }
                                else if ([queryField isEqualToString:@"Account Bill To"]) // SPR14__Company__r
                                {
                                    NSArray * arr = [NSArray arrayWithObjects: @"SPR14__Company__r.BillingCountry",@"SPR14__Company__r.BillingPostalCode",@"SPR14__Company__r.BillingState", @"SPR14__Company__r.BillingCity", @"SPR14__Company__r.BillingStreet", nil];//Service report fix : Radha
                                ...
                                else if ([queryField isEqualToString:@"Account Ship To"]) // SPR14__Company__r
                                {
                                    NSArray * arr = [NSArray arrayWithObjects:@"SPR14__Company__r.ShippingCountry", @"SPR14__Company__r.ShippingPostalCode", @"SPR14__Company__r.ShippingState", @"SPR14__Company__r.ShippingCity", @"SPR14__Company__r.ShippingStreet", nil];
                                ...
                                else if ([queryField isEqualToString:@"Service Location"]) // SPR14__Service_Order__c
                                {
                                    
                                    // [appDelegate.soqlQuery appendFormat:@", %@", @"SPR14__Street__c, SPR14__City__c, SPR14__State__c, SPR14__Zip__c, SPR14__Country__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__Street__c" forKey:@"SPR14__Service_Order__c.SPR14__Street__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__City__c" forKey:@"SPR14__Service_Order__c.SPR14__City__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__State__c" forKey:@"SPR14__Service_Order__c.SPR14__State__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__Zip__c" forKey:@"SPR14__Service_Order__c.SPR14__Zip__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__Country__c" forKey:@"SPR14__Service_Order__c.SPR14__Country__c"];//Service report fix : Radha
                                    
                                }
                                else if ([queryField isEqualToString:@"Contact Address"]) // SPR14__Contact__c
                                {
                                    NSArray * arr = [NSArray arrayWithObjects:@"SPR14__Contact__r.MailingStreet", @"SPR14__Contact__r.MailingState", @"SPR14__Contact__r.MailingPostalCode", @"SPR14__Contact__r.MailingCountry", @"SPR14__Contact__r.MailingCity", nil];
                                    
                                    [self getAllReferenceFields:arr];
                                    
                                    // [appDelegate.soqlQuery appendFormat:@", %@", @"SPR14__Contact__c"];
                                    [selectClause setValue:@"SPR14__Service_Order__c.SPR14__Contact__c" forKey:@"SPR14__Service_Order__c.SPR14__Contact__c"]; //Service report fix : Radha
                                }
                        ...
                        NSDictionary * valueDictionary = [settingsValueArray objectAtIndex:j]; // SPR14__Setting_ID__c
                        
                        NSString * Id = [keyDictionary objectForKey:@"Id"];
                        NSString * settingId = [valueDictionary objectForKey:@"SPR14__Setting_ID__c"];
                        ...
                        NSString * object = [valueDictionary objectForKey:@"SPR14__Display_Value__c"];
                        NSString * key = [keyDictionary objectForKey:@"SPR14__Setting_Unique_ID__c"];
                        ...
                        if ([[valueDictionary objectForKey:@"SPR14__Display_Value__c"] isKindOfClass:[NSNull class]])
                            continue;
                        if ([[keyDictionary objectForKey:@"SPR14__Setting_Unique_ID__c"] isKindOfClass:[NSNull class]])
                            continue;
                        
                        // Filter Service Report using IPAD004
                        if ([Id isEqualToString:settingId] && [[keyDictionary objectForKey:@"SPR14__SubmoduleID__c"] isEqualToString:@"IPAD004"])
                        {
                ...
                [selectClause setValue:@"SPR14__Service_Order__c.Name" forKey:@"SPR14__Service_Order__c.Name"];
                [selectClause setValue:@"SPR14__Problem_Description__c" forKey:@"SPR14__Problem_Description__c"];
                [selectClause setValue:@"SPR14__Work_Performed__c" forKey:@"SPR14__Work_Performed__c"];
                ...
                if (checkFromTable == nil)
                {
                    ...
                    [whereClause addObject:@"Contact.Id = SPR14__Service_Order__c.SPR14__Contact__c"];
                }
                else
                {
                ...
                // FROM
                [appDelegate.soqlQuery appendFormat:@" FROM SPR14__Service_Order__c "];
                ...
                NSArray * array = [NSArray arrayWithObjects:@"SPR14__Contact__r.Name",@"SPR14__Contact__r.Phone", nil];
            ...
            NSMutableArray *WorkOrderFields = [[NSMutableArray alloc]initWithCapacity:0];
            {
                ...
                //queryStatement9 = [NSMutableString stringWithFormat:@"SELECT api_name,label,type FROM SFObjectField WHERE object_api_name = 'SPR14__Service_Order__c' "];
                
                // Vipin-memopt 16-1 9493
                NSMutableString *queryStatement9 = [[NSMutableString alloc] initWithFormat:@"SELECT api_name,label,type FROM SFObjectField WHERE object_api_name = 'SPR14__Service_Order__c' "];
...
/*
 ...
 char *_SPR14__Setting_Unique_ID = (char *) synchronized_sqlite3_column_text(statement2,3);
 NSString * SPR14__Setting_Unique_ID = @"";
 if ((_SPR14__Setting_Unique_ID != nil) && strlen(_SPR14__Setting_Unique_ID))
 {
 SPR14__Setting_Unique_ID = [[NSString alloc] initWithUTF8String:_SPR14__Setting_Unique_ID];
 }
 ...
 char *_SPR14__IsPrivate__c = (char *) synchronized_sqlite3_column_text(statement2,10);
 NSString * SPR14__IsPrivate__c = @"";
 if ((_SPR14__IsPrivate__c != nil) && strlen(_SPR14__IsPrivate__c))
 {
 SPR14__IsPrivate__c = [[NSString alloc] initWithUTF8String:_SPR14__IsPrivate__c];
 }
 char *_SPR14__Active__c = (char *) synchronized_sqlite3_column_text(statement2,11);
 NSString * SPR14__Active__c = @"";
 if ((_SPR14__Active__c != nil) && strlen(_SPR14__Active__c))
 {
 SPR14__Active__c = [[NSString alloc] initWithUTF8String:_SPR14__Active__c];
 }
...
-(NSMutableArray *)getReportEssentials:(NSString *) record_id
{
    ...
    NSString * final_query = [NSString stringWithFormat:@"%@ SPR14__Service_Order__c.local_id = '%@'", clean_Query, record_id];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [final_query UTF8String],-1, &statement5, nil) == SQLITE_OK)
    {
        while(synchronized_sqlite3_step(statement5) == SQLITE_ROW)
        {
			...
			for(int k = 0 ; k < [all_fields count] ; k++)
            {
				...
				field_name = [field_name stringByReplacingOccurrencesOfString:@"SPR14__Service_Order__c." withString:@""];
...
-(NSMutableArray *)getreferncetableFieldsForReportEsentials:(NSMutableArray *)array
{
    ...
    for(int k= 0 ; k<[allkeys count]; k++)
    {
        ...
        if([key isEqualToString:@"SPR14__Company__c"])
        {
            table_name = @"Account";
        }
        else if([key isEqualToString:@"SPR14__Component__c"])
        {
            table_name = @"SPR14__Installed_Product__c";
        }
        else if([key isEqualToString:@"SPR14__Contact__c"])
        {
            table_name = @"Contact";
        }
...
-(NSString *)getTableName:(NSString *)key
{
    ...
    NSString * query = @"";
    sqlite3_stmt * statement;
    query = [NSString stringWithFormat:@"Select reference_to From SFObjectField where api_name ='%@' and object_api_name ='SPR14__Service_Order__c'",key];
...
- (NSString *) getApi_NameWithReference:(NSString *)reference_to
{
	...
	NSString * selectQuery = [NSString stringWithFormat:@"Select DISTINCT api_name,label from SFObjectField where object_api_name = 'SPR14__Service_Order__c' and reference_to = '%@'", reference_to];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, _selectQuery,-1, &statement, nil) == SQLITE_OK)
    {
        ...
        while(synchronized_sqlite3_step(statement) == SQLITE_ROW){
			...
			if ([api_name Contains:@"SPR14"] && [reference_to Contains:label])
			{
...
{
	NSString * value = @"";
	NSString * selectQuery = [NSString stringWithFormat:@"Select %@ from SPR14__Service_Order__c where local_id = '%@'", field_name, appDelegate.sfmPageController.recordId];
...
- (NSArray *)getPriceBook:(NSDictionary *)currentContext {
    ...
    @try {
       ...
       /* get the header and detail records*/
        ...
        NSString *workOrderProduct = [self getValueFOrKey:@"SPR14__Product__c" FromArray:headerFieldArray];
        ...
        for(int counter = 0;counter < [detailRecords count];counter++){
            ...
            for(int counter = 0;counter < [records count];counter++){
                ...
                NSString *productId =  [self getValueFOrKey:@"SPR14__Product__c" FromArray:detalFiledsArray];
                if (![Utility isStringEmpty:productId]) {
                        [productsArray addObject:productId];
                }
                
                NSString *activityType =  [self getValueFOrKey:@"SPR14__Activity_Type__c" FromArray:detalFiledsArray];
        ...
        NSString *SPR14__Warranty__c = [entitlementDict objectForKey:@"SPR14__Warranty__c"];
        NSString *SPR14__Service_Contract__c = [entitlementDict objectForKey:@"SPR14__Service_Contract__c"];
        NSString *SPR14__Entitled_By_Service__c =  [entitlementDict objectForKey:@"SPR14__Entitled_By_Service__c"];
        if (![Utility isStringEmpty:SPR14__Entitled_By_Service__c]) {
            idOfServiceOffering = SPR14__Entitled_By_Service__c;
        }
        
        NSString *SPR14__Entitled_Within_Threshold__c = [entitlementDict objectForKey:@"SPR14__Entitled_Within_Threshold__c"];
        if ([SPR14__Entitled_Within_Threshold__c isEqualToString:@"true"] ||[SPR14__Entitled_Within_Threshold__c isEqualToString:@"True"] || [SPR14__Entitled_Within_Threshold__c isEqualToString:@"1"]  ) {
            idServiceCovered = @"COVERED";
        }
        
       
        /* Wo comes under warranty then get warranty_id */
        if (![Utility isStringEmpty:SPR14__Warranty__c]) {
            
            NSMutableDictionary *warrantyDictionary =  [self getRecordForSfId:SPR14__Warranty__c andTableName:@"SPR14__Warranty__c"];
            
            [warrantyDictionary setObject:[NSDictionary dictionaryWithObjectsAndKeys:@"SPR14__Warranty__c",@"type",nil] forKey:@"attributes"];
        ...
        else if(![Utility isStringEmpty:SPR14__Service_Contract__c]){
            
            /* else If wo comes under contract then get contract_id */
            NSMutableDictionary *serviceContractDictionary = [self getRecordForSfId:SPR14__Service_Contract__c
                                                                       andTableName:@"SPR14__Service_Contract__c"];
            
            /*Get Service Contract pricebook definition for Parts */
            NSString *SPR14__Default_Parts_Price_Book__c =  [serviceContractDictionary objectForKey:@"SPR14__Default_Parts_Price_Book__c"];
            if (![Utility isStringEmpty:SPR14__Default_Parts_Price_Book__c]) {
                /* get the price book info */
                [partsPriceBookIdsArray addObject:SPR14__Default_Parts_Price_Book__c];
                NSDictionary *tempDictionary =  [self preparePBEstimateId:SPR14__Default_Parts_Price_Book__c andUsageValue:SPR14__Default_Parts_Price_Book__c andKey:@"RECORDTYPEINFO_PARTS_CONTRACT" andRecordTypeId:recordTypeIds];
            ...
            NSString *SPR14__Service_Pricebook__c =  [serviceContractDictionary objectForKey:@"SPR14__Service_Pricebook__c"];
            if (![Utility isStringEmpty:SPR14__Service_Pricebook__c]) {
                [labourPriceBookIdsArray addObject:SPR14__Service_Pricebook__c];
                NSDictionary *tempDictionary =  [self preparePBLaourEstimateId:SPR14__Service_Pricebook__c  andUsageValue:SPR14__Service_Pricebook__c andKey:@"RECORDTYPEINFO_LABOR_CONTRACT" andRecordTypeId:recordTypeIds];
                if (tempDictionary != nil) {
                    [priceBookArray addObject:tempDictionary];
                }
            }
            ...
            [self fillUpContractInformationInTheTargetArray:priceBookArray andContractId:SPR14__Service_Contract__c andCurrency:currencyCode andNamedExpressionArray:namedExpressionArray andIdOffering:idOfServiceOffering  andCovered:idServiceCovered];
...
- (void)fillUpContractInformationInTheTargetArray:(NSMutableArray *)priceBookArray
                                    andContractId:(NSString *)SPR14__Service_Contract__c
                                       ...
                                       andCovered:(NSString *)idServiceCovered
{
    ...
    [columnNames setObject:SPR14__Service_Contract__c forKey:@"SPR14__Service_Contract__c"];
    
    
    /* Get pricing rules for Contract*/
    NSArray *dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Pricing_Rule__c"];
    ...
    [columnNames setObject:@"1"  forKey:@"SPR14__Active__c"];
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Parts_Pricing__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Parts_Discount__c"];
    ...
    [finalDict release];
    finalDict = nil;
    [columnNames removeObjectForKey:@"SPR14__Active__c"];
    
    /* Get special labor pricing definition */
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Labor_Pricing__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Expense_Pricing__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Travel_Policy__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Mileage_Tiers__c"];
    ...
    dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Zone_Pricing__c"];
    ...
    if (idOfServiceOffering != nil) {
        [columnNames removeObjectForKey:@"SPR14__Service_Contract__c"];
        [columnNames setObject:idOfServiceOffering forKey:@"Id"];
        dataArray =  [self getRecordWhereColumnNamesAndValues:columnNames andTableName:@"SPR14__Service_Contract_Services__c"];
...
- (NSDictionary *)getEntitlementHistoryForWorkorder:(NSString *)workOrderId {
    ...
    NSString *tableName = @"SPR14__Entitlement_History__c";
    ...
    NSString *sqlQuery = [NSString  stringWithFormat:@"select %@ from %@ where SPR14__Service_Order__c = '%@' and SPR14__Inactive_Date__c = \"\"",allColumnNamesString,tableName,workOrderId];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sqlQuery UTF8String], -1, &selectStmt, nil) == SQLITE_OK  )
    {
        ...
        while(sqlite3_step(selectStmt) == SQLITE_ROW)
        {
            ...
            NSString *tempStringNew = [dataDictionary objectForKey:@"SPR14__Entitled_Within_Threshold__c"];
            if ([Utility isItTrue:tempStringNew]) {
                tempStringNew = @"true";
            }else{
                tempStringNew = @"false";
            }
            [dataDictionary setObject:tempStringNew forKey:@"SPR14__Entitled_Within_Threshold__c"];
...
- (NSArray *)getRecordWhereColumnNamesAndValues:(NSDictionary *)columnKeyAndValue  andTableName:(NSString *)tableName {
    ...
    for (NSString *columnKey in allKeys) {
        ...
        counter++;
        if ([columnKey isEqualToString:@"SPR14__Active__c"]) {
            
            [someString appendFormat:@"( SPR14__Active__c = '1' OR  SPR14__Active__c = 'True' OR SPR14__Active__c = 'true')"];
        }
        else {
...
- (NSArray *)getProductRecords:(NSArray *)productIdentifiers {
    ...
    NSString *sqlQuery = [NSString stringWithFormat:@"Select Id, SPR14__Product_Line__c, Family From Product2 where Id IN %@",productIdentfierStr];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sqlQuery UTF8String], -1, &selectStmt, nil) == SQLITE_OK  )
    {
        ...
        while(sqlite3_step(selectStmt) == SQLITE_ROW)
        {
            ...
            if (tempCharString != nil) {
                tempString = [NSString stringWithUTF8String:tempCharString];
                [recordDictionary setObject:tempString forKey:@"SPR14__Product_Line__c"];
            }
...
- (NSArray *)getPriceBookObjectsForLabourPriceBookIds:(NSArray *)priceBookIds OrPriceBookNames:(NSArray *)priceBookNames {
    ...
    NSString *isActiveString = @"( SPR14__Active__c = '1' OR SPR14__Active__c = 'true'  OR SPR14__Active__c = 'True' )";
    NSString *sqlQuery = nil;
    
    if (priceBookIdString != nil && priceBookNameString != nil) {
        sqlQuery = [NSString stringWithFormat:@"SELECT Id, Name FROM SPR14__Service_Pricebook__c WHERE (Id IN %@ OR Name IN %@) and %@",priceBookIdString,priceBookNameString,isActiveString];
    }
    else if (priceBookIdString != nil){
        
        sqlQuery = [NSString stringWithFormat:@"SELECT Id, Name FROM SPR14__Service_Pricebook__c WHERE Id IN %@ and %@ ",priceBookIdString,isActiveString];
        
    }else if(priceBookNameString != nil){
        sqlQuery = [NSString stringWithFormat:@"SELECT Id, Name FROM SPR14__Service_Pricebook__c WHERE Name IN %@ and %@ ",priceBookNameString,isActiveString];
    }
...
- (void)addNamedExpressionsFrom:(NSArray *)dataArray ToArray:(NSMutableArray *)namedExpressionArray {
    ...
    for (int counter = 0; counter < [dataArray count]; counter ++) {
        ...
        NSString  *nameExpressionId = [tempDictionary objectForKey:@"SPR14__Named_Expression__c"];
...
- (NSArray *)getNamedExpressionsForIds:(NSArray *)namedExpressionArray {
    ...
    NSString *sqlQuery = [NSString stringWithFormat:@"Select SPR14__Advance_Expression__c,Id from SPR14__ServiceMax_Processes__c where %@ and recordTypeId = '%@'",concatenatedExpId,recordTypeRule];
    sqlite3_stmt *selectStmt = nil;
...
- (NSArray *) getExpressionComponentsForExpressionId:(NSString *)expressionId andExpression:(NSString *)expressionName andRecordId:(NSString *)recordTypeId{
    ...
    NSString *sqlQuery = [NSString stringWithFormat:@"Select SPR14__Field_Name__c,SPR14__Operator__c,SPR14__Operand__c,SPR14__Sequence__c,SPR14__Expression_Type__c,SPR14__Expression_Rule__c,Id from SPR14__SERVICEMAX_CONFIG_DATA__C where SPR14__Expression_Rule__c LIKE '%@%%' and recordTypeId = '%@'",expressionId,recordTypeId];
    sqlite3_stmt *selectStmt = nil;
        ...
        while(sqlite3_step(selectStmt) == SQLITE_ROW) {
            ...
            if (tempCharString != nil) {
                ...
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SPR14__Field_Name__c"];
                }
            ...
            if (tempCharString != nil) {
                 someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SPR14__Operator__c"];
                }
            ...
            if (tempCharString != nil) {
                 someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SPR14__Operand__c"];
                }
            ...
            if (tempCharString != nil) {
                someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SPR14__Sequence__c"];
                }
            ...
            if (tempCharString != nil) {
                someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SPR14__Expression_Type__c"];
                }
            ...
            if (tempCharString != nil) {
                someExpression = [NSString stringWithUTF8String:tempCharString];
                if (someExpression != nil) {
                    [recordDictionary setObject:someExpression forKey:@"SPR14__Expression_Rule__c"];
                }
            ...
            if ([recordDictionary count] > 0) {
                ...
                NSDictionary *tempDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"SPR14__ServiceMax_Config_Data__c",@"type", nil];
                [recordDictionary setObject:tempDict forKey:@"attributes"];
                [tempDict release];
                tempDict = nil;
...
- (NSArray *)getPriceBookEntryForLabourArray:(NSArray *)labourArray andPriceBookIds:(NSArray *)priceBookIds andCurrency:(NSString *)currency {
    NSString *tableName = @"SPR14__Service_Pricebook_Entry__c";
    NSString *priceBookIdString = [self getConcatenatedStringFromArray:priceBookIds withSingleQuotesAndBraces:YES];
    ...
    NSString *sqlQuery = nil;
    if (currency == nil) {
        sqlQuery = [[NSString alloc] initWithFormat:@"Select %@ from %@ where  SPR14__Activity_Type__c IN %@ AND SPR14__Price_Book__c IN %@",allColumnNamesString,tableName,labourString,priceBookIdString];
    }
    else {
        sqlQuery = [[NSString alloc] initWithFormat:@"Select %@ from %@ where SPR14__Activity_Type__c IN %@ AND SPR14__Price_Book__c IN %@ and CurrencyIsoCode = '%@'",allColumnNamesString,tableName,labourString,priceBookIdString,currency];
    }
...
- (NSArray *)getValidLabourPriceBookNames:(NSArray *)labourPbNames andLabourIdArray:(NSArray *)labourIdArray andCurrency:(NSString *)currency{
    ...
    NSString *tablename = @"SPR14__Service_Pricebook__c";
    ...
    NSString *activeString = @"(SPR14__Active__c = '1' OR SPR14__Active__c = 'True' OR SPR14__Active__c = 'true')";
...
- (NSArray *)getIBWarrantyArray:(NSString *)wordOrderLocalId  andWorkOrderSfid:(NSString *)workOrderSfid{
    ...
    NSDictionary *allWorkOrderIds =  [self getWorkDetailForProductServiceArray:[productServiceWorkdDetail allKeys] andWorkOrderIds:wordOrderLocalId andSfid:workOrderSfid];
        ...
        /* Get All warranties */
        NSDictionary *allWarranties = [self getAllRecordForSfIds:[productServiceWorkdDetail allValues] andTableName:@"SPR14__Warranty__c"];
...
- (NSDictionary *)getProductServiceWorkDetail:(NSString *)wordOrderId andWorkorderSfid:(NSString *)sfid{
    ...
    {
        //10760
        NSString * query = [NSString stringWithFormat:@"SELECT local_id , SPR14__Product_Warranty__c from 'SPR14__Service_Order_Line__c' where RecordTypeId = '%@' and (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@')" , serviceRecordTypeId , wordOrderId,sfid];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
...
- (NSDictionary *)getAllSfidsForLocalIds:(NSString *)localIdString {
    ...
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,local_id from 'SPR14__Service_Order_Line__c' WHERE local_id in %@" , localIdString];
...
- (NSDictionary *)getWorkDetailForProductServiceArray:(NSArray *)productServiceIds
   ...
   NSString *idString =   [Utility getConcatenatedStringFromArray:productServiceIds withSingleQuotesAndBraces:YES];
    ...
    {
        //10760
        NSString * query = [NSString stringWithFormat:@"SELECT local_id, SPR14__Work_Detail__c from 'SPR14__Service_Order_Line__c' where ( SPR14__Work_Detail__c IN %@ OR SPR14__Work_Detail__c IN %@ ) and (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@')" , idString ,sfidString, wordOrderId,wosfid];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
...
- (NSDictionary *)getCodeSnippetForId:(NSString *)codeSnippetId {
    ...
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,SPR14__Data__c  from SPR14__Code_Snippet__c  where Id = '%@' and SPR14__Data__c <> \"\"" , codeSnippetId];
    sqlite3_stmt * stmt;
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
...
- (NSDictionary *)getCodeSnippetForName:(NSString *)codeSnippetName {
    ...
    NSString * fieldId = @"", *fieldData = nil;
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,SPR14__Data__c  from SPR14__Code_Snippet__c  where SPR14__Name__c  = '%@' and SPR14__Data__c <> \"\"" , codeSnippetName];
    sqlite3_stmt * stmt;
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (sqlite3_step(stmt)  == SQLITE_ROW)
...
- (NSString *)getCodeSnippetRefererenceForId:(NSString *)codeSnippetReference {
    ...
    NSString * SPR14__Code_Snippet_Manifest__c = nil;
    NSString * query = [NSString stringWithFormat:@"SELECT SPR14__Referenced_Code_Snippet__c  from SPR14__Code_Snippet_Manifest__c  where SPR14__Code_Snippet__c = '%@' and SPR14__Referenced_Code_Snippet__c <> \"\"" ,codeSnippetReference];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (sqlite3_step(stmt)  == SQLITE_ROW)
        {
            ...
            if(temp_process_id!= nil) {
                SPR14__Code_Snippet_Manifest__c = [NSString stringWithUTF8String:temp_process_id];
    ...
    return SPR14__Code_Snippet_Manifest__c;
}
...
- (NSString *)getEntitlementStatus:(NSString *)recordIdentfier recordIdFromTable:(NSString *)tableName {
    ...
    NSString * query = [NSString stringWithFormat:@"SELECT SPR14__Is_Entitlement_Performed__c FROM %@ where Id = '%@'",tableName,recordIdentfier];
    sqlite3_stmt * stmt;
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
...
- (BOOL)doesAllRecordsForGetPriceCalculationExist:(NSString *)recordId {
    ...
    if(someDictionary != nil && [someDictionary count] > 0 ){
        ...
        NSString *SPR14__Warranty__c         = [someDictionary objectForKey:@"SPR14__Warranty__c"];
        NSString *SPR14__Service_Contract__c = [someDictionary objectForKey:@"SPR14__Service_Contract__c"];
        NSInteger  numberOfRecordsOne = -1 , numberOfRecordsTwo = -1;
        if (![Utility isStringEmpty:SPR14__Warranty__c]) {
            numberOfRecordsOne =  [self getRecordCountForSfId:SPR14__Warranty__c andTableName:@"SPR14__Warranty__c"];
        }
        else {
            numberOfRecordsOne = 1;
        }
        
        if (![Utility isStringEmpty:SPR14__Service_Contract__c]) {
            numberOfRecordsTwo =  [self getRecordCountForSfId:SPR14__Service_Contract__c andTableName:@"SPR14__Service_Contract__c"];
        }
        else {
...
- (BOOL)checkIfBillablePriceExistForWorkOrderId:(NSString *)workOrderLocalId andFieldName:(NSString *)fieldName {
    ...
    NSString *sfmId = [self getSFIdForlocalId:workOrderLocalId andTableName:@"SPR14__Service_Order__c"];
    
    /* Get all lines and check if one billable price exist and non empty  */
    
    NSString *sqlQuery = [NSString stringWithFormat:@"select %@ from SPR14__Service_Order_Line__c where (SPR14__Service_Order__c = '%@' OR SPR14__Service_Order__c = '%@')", fieldName,workOrderLocalId,sfmId ];
    sqlite3_stmt * stmt;
