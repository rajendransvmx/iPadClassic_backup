...
{ 
    ...
    sqlite3_stmt *statement1;
    @try{
    queryStatement = [NSString stringWithFormat:@"SELECT Id, SPR14__Activity_Type__c, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c,SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Labor' AND SPR14__Service_Order__c = '%@' AND  (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    const char * _query = [queryStatement UTF8String];
    NSArray * keys = [NSArray arrayWithObjects:
                      _ID,
...
- (NSMutableArray *) queryForTravel:(NSString *)currentRecordId
{
    ...
    queryStatement = [NSString stringWithFormat:@"SELECT Id, SPR14__Actual_Quantity2__c, SPR14__Actual_Price2__c, SPR14__Work_Description__c, SPR14__Billable_Quantity__c, SPR14__Billable_Line_Price__c FROM SPR14__Service_Order_Line__c WHERE SPR14__Line_Type__c = 'Travel' AND SPR14__Service_Order__c = '%@' AND (SPR14__Is_Billable__c = 'true' or  SPR14__Is_Billable__c = 'True' or SPR14__Is_Billable__c = '1') AND RecordTypeId   in   (select  record_type_id  from SFRecordType where record_type = 'Usage/Consumption' )", currentRecordId];
    @try{
        const char * _query = [queryStatement UTF8String];
        if ( synchronized_sqlite3_prepare_v2(appDelegate.db, _query,-1, &statement, nil) == SQLITE_OK )
        {
...
- (NSArray *)getPriceBook:(NSDictionary *)currentContext {
    ...
    @try {
       ...
       /* get the header and detail records*/
        ...
        NSArray *allWarrantyArray = [self getIBWarrantyArray:appDelegate.sfmPageController.recordId];
...
//Price cal 2.0
- (NSArray *)getIBWarrantyArray:(NSString *)wordOrderLocalId  {
    
    NSDictionary *productServiceWorkdDetail = [self getProductServiceWorkDetail:wordOrderLocalId];
    //[id , warantyID]
    
    if ([productServiceWorkdDetail count] > 0) {
        
        /* Get all work order ids which matches the condition */
       NSDictionary *allWorkOrderIds =  [self getWorkDetailForProductServiceArray:[productServiceWorkdDetail allKeys] andWorkOrderIds:wordOrderLocalId];
...
- (NSDictionary *)getProductServiceWorkDetail:(NSString *)wordOrderId {
    ...
    {
        NSString * query = [NSString stringWithFormat:@"SELECT Id , SPR14__Product_Warranty__c from 'SPR14__Service_Order_Line__c' where RecordTypeId = '%@' and SPR14__Service_Order__c = '%@'" , serviceRecordTypeId , wordOrderId];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
...
- (NSDictionary *)getWorkDetailForProductServiceArray:(NSArray *)productServiceIds
                                      andWorkOrderIds:(NSString *)wordOrderId {
   ...
   NSString *idString =   [Utility getConcatenatedStringFromArray:productServiceIds withSingleQuotesAndBraces:YES];
    ...
    {
        NSString * query = [NSString stringWithFormat:@"SELECT Id, SPR14__Work_Detail__c from 'SPR14__Service_Order_Line__c' where SPR14__Work_Detail__c IN %@ and SPR14__Service_Order__c = '%@'" , idString , wordOrderId];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
