...
- (NSArray *)getIBWarrantyArray:(NSString *)wordOrderLocalId  andWorkOrderSfid:(NSString *)workOrderSfid{
    ...
    NSDictionary *allWorkOrderIds =  [self getWorkDetailForProductServiceArray:[productServiceWorkdDetail allKeys] andWorkOrderIds:wordOrderLocalId andSfid:workOrderSfid];
        ...
        /* Get All warranties */
        NSDictionary *allWarranties = [self getAllRecordForSfIds:[productServiceWorkdDetail allValues] andTableName:@"SVMXC__Warranty__c"];
...
- (NSDictionary *)getProductServiceWorkDetail:(NSString *)wordOrderId andWorkorderSfid:(NSString *)sfid{
    ...
    {
        //10760
        NSString * query = [NSString stringWithFormat:@"SELECT local_id , SVMXC__Product_Warranty__c from 'SVMXC__Service_Order_Line__c' where RecordTypeId = '%@' and (SVMXC__Service_Order__c = '%@' OR SVMXC__Service_Order__c = '%@')" , serviceRecordTypeId , wordOrderId,sfid];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
...
- (NSDictionary *)getAllSfidsForLocalIds:(NSString *)localIdString {
    ...
    NSString * query = [NSString stringWithFormat:@"SELECT Id ,local_id from 'SVMXC__Service_Order_Line__c' WHERE local_id in %@" , localIdString];
...
- (NSDictionary *)getWorkDetailForProductServiceArray:(NSArray *)productServiceIds
   ...
   NSString *idString =   [Utility getConcatenatedStringFromArray:productServiceIds withSingleQuotesAndBraces:YES];
    ...
    {
        //10760
        NSString * query = [NSString stringWithFormat:@"SELECT local_id, SVMXC__Work_Detail__c from 'SVMXC__Service_Order_Line__c' where ( SVMXC__Work_Detail__c IN %@ OR SVMXC__Work_Detail__c IN %@ ) and (SVMXC__Service_Order__c = '%@' OR SVMXC__Service_Order__c = '%@')" , idString ,sfidString, wordOrderId,wosfid];
        sqlite3_stmt * stmt;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
