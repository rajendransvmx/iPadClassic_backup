...
- (id)initWithMapView:(MKMapView *)mapView
{
    ...
    //SMLog(kLogLevelVerbose,@"%f, %f", mapView.frame.size .width, mapView.frame.size.height);
	if (self != nil)
    {
...
- (void)drawRect:(CGRect)rect
{
	if(!self.hidden && self.routes != nil && self.routes.count > 0)
    {
		...
		for(int i = 0; i < self.routes.count; i++)
        {
            ...
            //SMLog(kLogLevelVerbose,@"%f, %f", route.coordinate.latitude, route.coordinate.longitude);
            //SMLog(kLogLevelVerbose,@"%f, %f", firstDestinationCoordinate.latitude, firstDestinationCoordinate.longitude);
            if (
