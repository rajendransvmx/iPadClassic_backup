...
- (void)loadView 
{
    ...
    //SMLog(kLogLevelVerbose,@"%@", workOrderArray);
}
...
- (void) showRecentVisits:(NSArray *)_recentVisitsArray
{
    ...
    for (int i = 0; i < [recentVisitsArray count]; i++)
    {
        ...
        coordinate.latitude = [[[[recentVisitsArray objectAtIndex:i] fields] objectForKey:SVMXDEVLATITUDE] doubleValue];
        coordinate.longitude = [[[[recentVisitsArray objectAtIndex:i] fields] objectForKey:SVMXDEVLONGITUDE] doubleValue];
...
- (void)directionsDidUpdateDirections:(UICGDirections *)directions 
{
	...
	//	NSInteger numberOfRoutes = [directions numberOfRoutes];
		...
		for (NSInteger index = 0; index < [wayPoints count]; index++)
        {
            ...
            //SMLog(kLogLevelVerbose,@"%@, %f, %f", address, coord.latitude, coord.longitude);
            // SAMMAN - BEGIN - Ensure duplicate addresses are not used
                       ...
                       context:(void *)context
{
    ...
    SMLog(kLogLevelVerbose,@"Call Custom Callout");
    if([action isEqualToString:GMAP_ANNOTATION_SELECTED])
    {
        BOOL annotationAppeared = [[change valueForKey:@"new"] boolValue];
        // do something
        // if (annotationAppeared)
            // SMLog(kLogLevelVerbose,@"MKAnnotationView Clicked = %d", annotationAppeared);
        
        CGPoint point = [routeMapView convertCoordinate:[object coordinate] toPointToView:self.view];
        // if (annotationAppeared)
            // SMLog(kLogLevelVerbose,@"x = %f, y = %f", point.x, point.y);
...
{
    SMLog(kLogLevelVerbose,@"Selected annotation");
}

- (void)deselectAnnotation:(id < MKAnnotation >)annotation animated:(BOOL)animated
{
    SMLog(kLogLevelVerbose,@"Deselected annotation");
}
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation {
	...
	if ([annotation isKindOfClass:[UICRouteAnnotation class]])
    {
		...
		if ([(UICRouteAnnotation *)annotation annotationType] == UICRouteAnnotationTypeStart)
        {
            ...
            /*Accessibility Changes*/
            [pinAnnotation setAccessibilityIdentifier:kAccHomeAnnotationPin];
            pinAnnotation.accessibilityValue = kAccHomePinColor;
        ...
        else 
        {
            ...
            pinAnnotation.isAccessibilityElement = YES;
            /*Accessibility Changes*/
            [pinAnnotation setAccessibilityIdentifier:kAccWOAnnotationPin];
            pinAnnotation.accessibilityValue = kAccWOPinColor;
    ...
    else if ([annotation isKindOfClass:[SFAnnotation class]])   // for City of San Francisco
    {
        ...
        if (!pinView)
        {
            ...
            annotationView.isAccessibilityElement = YES;
            /*Accessibility Changes*/
            [annotationView setAccessibilityIdentifier:kAccWOAnnotationPin];
            annotationView.accessibilityValue = kAccWOPinColor;
