...
- (void) customCallOutForAnnotation:(UICRouteAnnotation *)annotationObject AtPosition:(CGPoint)point;
{
    ...
    @catch (NSException *exp) {
        NSLog(@"Exception Name FirstDetailViewController :customCallOutForAnnotation %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :customCallOutForAnnotation %@",exp.reason);
...
//            str = [str stringByReplacingOccurrencesOfString:[appDelegate.wsInterface.tagsDictionary objectForKey:MAP_POPOVER_TITLE] withString:@""];
    ...
    [array addObject:homeDict];
        ...
        NSLog(@"Exception Name FirstDetailViewController :getWorkOrderArray %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :getWorkOrderArray %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];;;
    }
    return array;
}
...
- (void) viewDidLoad
{
    ...
    if ([appDelegate isInternetConnectionAvailable])
    {
		...
		NSLog(@"viewDidLoad, Mapview = %@", _query);
...
- (void) setupMapView
{
    ...
    {
        NSLog(@"Exception Name FirstDetailViewController :setupMapView %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :setupMapView %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];;
    }
}
...
- (void) setJobDetailsForWorkOrder:(NSDictionary *)workOrderDetails workOrderInfo:(NSDictionary *)workOrderInfo
{
    ...
    @catch (NSException *exp) {
        NSLog(@"Exception Name FirstDetailViewController :setJobDetailsForWorkOrder %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :setJobDetailsForWorkOrder %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void) queryImagesForAccount:(NSString *)companyId Contact:(NSString *)contactId
{
    ...
    if (contactId != nil)
    {
		...
		NSLog(@"queryImagesForAccount = %@", _query);
...
{    
    ...
    @catch (NSException *exp)
    {
        NSLog(@"Exception Name FirstDetailViewController :cellForRowAtIndexPath %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :cellForRowAtIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    return cell;
}
...
- (void)reverseGeocoder:(MKReverseGeocoder *)geocoder didFindPlacemark:(MKPlacemark *)placemark
{
    ...
    currentLocation = [[NSString stringWithFormat:@"%@, %@, %@, %@, %@, %@, %@",
        ...
        NSLog(@"Exception Name FirstDetailViewController :reverseGeocoder %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :reverseGeocoder %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
- (void) initDebriefData:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	NSLog(@"initDebriefData, Mapview = %@", _query);
...
- (void) didQueryTechnician:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        NSLog(@"Exception Name FirstDetailViewController :didQueryTechnician %@",exp.name);
        NSLog(@"Exception Reason FirstDetailViewController :didQueryTechnician %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    @finally {
