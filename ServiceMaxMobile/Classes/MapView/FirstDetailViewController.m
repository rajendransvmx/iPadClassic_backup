...
- (void) viewDidLoad
{
    ...
    if ([appDelegate isInternetConnectionAvailable])
    {
		...
		SMLog(@"viewDidLoad, Mapview = %@", _query);
...
- (void) queryImagesForAccount:(NSString *)companyId Contact:(NSString *)contactId
{
    ...
    if (contactId != nil)
    {
		...
		SMLog(@"queryImagesForAccount = %@", _query);
...
- (void) initDebriefData:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	SMLog(@"initDebriefData, Mapview = %@", _query);
...
{
    SMLog(@"FirstDetailViewController: Recieve [%@] notification in refreshMap",[notification name]);
