...
- (void) customCallOutForAnnotation:(UICRouteAnnotation *)annotationObject AtPosition:(CGPoint)point;
{
	@try{
        ...
        //SMLog(kLogLevelVerbose,@"%@", workOrder);
        ...
        //SMLog(kLogLevelVerbose,@"%f, %f", point.x, point.y);
        //SMLog(kLogLevelVerbose,@"%f, %f", popOver.contentViewController.view.frame.origin.x, popOver.contentViewController.view.frame.origin.y);
        [locationPop release];
	}
    @catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :customCallOutForAnnotation %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :customCallOutForAnnotation %@",exp.reason);
...
- (void) showDrivingDirectionsForAnnotationIndex:(NSUInteger)annotationIndex
{
    ...
    if ([Utility notIOS7]) {
        ...
        routeView.view.frame = CGRectMake(15, 387, 253, 361);
    }
...
- (NSArray *) getWorkOrderArray
{
    ...
    {        
        ...
        //SMLog(kLogLevelVerbose,@"adddress = %@", address);
...
//            str = [str stringByReplacingOccurrencesOfString:[appDelegate.wsInterface.tagsDictionary objectForKey:MAP_POPOVER_TITLE] withString:@""];
    ...
    [array addObject:homeDict];
        ...
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :getWorkOrderArray %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :getWorkOrderArray %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];;;
    }
    return array;
}
...
- (void) viewDidLoad
{
    ...
    //SMLog(kLogLevelVerbose,@"%@", eventsArray);
    ...
    if ([appDelegate isInternetConnectionAvailable])
    {
        ...
        NSString * _query = [NSString stringWithFormat:@"SELECT Id, SVMXDEV__Service_Group__c, SVMXDEV__Inventory_Location__c  FROM SVMXDEV__Service_Group_Members__c WHERE SVMXDEV__Salesforce_User__c = '%@'", appDelegate.loggedInUserId];
        SMLog(kLogLevelVerbose,@"%@", appDelegate.loggedInUserId);
		
		SMLog(kLogLevelVerbose,@"viewDidLoad, Mapview = %@", _query);
    ...
    [monitor monitorSMMessageWithName:@"[FirstDetailViewController viewDidLoad]"
    ...
    {
        //SMLog(kLogLevelVerbose,@"Mapview initDebrief in while loop");
        if (![appDelegate isInternetConnectionAvailable])
        {
...
- (void) setAccessibilityIdentifierForInstanceVariable
{
    ...
    jobDetails.isAccessibilityElement = YES;
    [jobDetails setAccessibilityIdentifier:kAccSFMDisclosure];
    
    displayUser.isAccessibilityElement = YES;
    [displayUser setAccessibilityIdentifier:kAccDisplayUser];
    
    contactName.isAccessibilityElement = YES;
    [contactName setAccessibilityIdentifier:kAccContactName];
    
    contactAddress.isAccessibilityElement = YES;
    [contactAddress setAccessibilityIdentifier:kAccContactAddress];
    
    contactEmail.isAccessibilityElement = YES;
    [contactEmail setAccessibilityIdentifier:kAccContactEmail];
    
    WODescription.isAccessibilityElement = YES;
    [WODescription setAccessibilityIdentifier:kAccWoDescription];
    
    WONumber.isAccessibilityElement = YES;
    [WONumber setAccessibilityIdentifier:kAccWoNumber];
    
    problemDescription.isAccessibilityElement = YES;
    [problemDescription setAccessibilityIdentifier:kAccProblemDescription];
    
    problemCode.isAccessibilityElement = YES;
    [problemCode setAccessibilityIdentifier:kAccProblemCode];
...
- (void) didInternetConnectionChange:(NSNotification *)notification
{
    ...
    if (isReachable)
    {
        SMLog(kLogLevelVerbose,@"Map Internet Reachable");
        [self setupMapView];
        [self setContactImage];
    }
    else
    {
        ...
        SMLog(kLogLevelWarning,@"Map Internet Not Reachable");
        if (didRunOperation)
        {
...
- (void) setupMapView
{
    ...
    SMLog(kLogLevelVerbose,@"%f, %f, %f, %f", mapView.bounds.origin.x,mapView.bounds.origin.y,mapView.bounds.size.width, mapView.bounds.size.height);
    ...
    //SMLog(kLogLevelVerbose,@"addressarray = %@", array);
    
    controller.wayPoints = array; 
    
    wayPoints = array;

    //SMLog(kLogLevelVerbose,@"%@", controller.wayPoints);
    ...
    {
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :setupMapView %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :setupMapView %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];;
    }
}
...
- (void) setJobDetailsForWorkOrder:(NSDictionary *)workOrderDetails workOrderInfo:(NSDictionary *)workOrderInfo
{
    ...
    contactName.text = [workOrderInfo objectForKey:SVMXDEV__CONTACT_NAME];
    contactEmail.text = [workOrderInfo objectForKey:SVMXDEV__CONTACT_EMAIL];
    contactAddress.text = [workOrderInfo objectForKey:SVMXDEV__CONTACT_PHONE];
    problemDescription.text = [workOrderInfo objectForKey:SVMXDEV__PROBLEM_DESCRIPTION];
    WODescription.text = [workOrderInfo objectForKey:SVMXDEV__ORDER_TYPE];
	}
    @catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :setJobDetailsForWorkOrder %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :setJobDetailsForWorkOrder %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
{
    NSString * contactId = [currentWorkOrderInfo objectForKey:SVMXDEV__CONTACT__C];
...
- (void) queryImagesForAccount:(NSString *)companyId Contact:(NSString *)contactId
{
    ...
    if (contactId != nil)
    {
		...
		SMLog(kLogLevelVerbose,@"queryImagesForAccount = %@", _query);
...
//    SMLog(kLogLevelVerbose,@"%@", woStartTiming);
    ...
    NSDate * startime = [dict objectForKey:STARTDATETIME]; //10346
        
    NSString * woStartTiming = [dateFormatter stringFromDate:startime];
    
    // END TIME
    NSDate * endtime = [dict objectForKey:ENDDATETIME];
    SMLog(kLogLevelVerbose,@"%@", endtime);
    
     // First convert the time to local
    NSString * woEndTiming = [dateFormatter stringFromDate:endtime];
    SMLog(kLogLevelVerbose,@"%@", woEndTiming);
    ...
    @catch (NSException *exp)
    {
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :cellForRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :cellForRowAtIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    {
        // SMLog(kLogLevelVerbose,@"latitude %+.6f, longitude %+.6f\n", newLocation.coordinate.latitude, newLocation.coordinate.longitude);
...
{
    // SMLog(kLogLevelVerbose,@"Reverse Geocoder error = %@", error.description);    
    //SMLog(kLogLevelVerbose,@"%@", placemark.addressDictionary);
}

- (void)reverseGeocoder:(MKReverseGeocoder *)geocoder didFindPlacemark:(MKPlacemark *)placemark
{
    // SMLog(kLogLevelVerbose,@"Address Dictionary = %@", [placemark addressDictionary]);
    //SMLog(kLogLevelVerbose,@"The geocoder has returned: %@", [placemark addressDictionary]);
    @try{
    ...
    currentLocation = [[NSString stringWithFormat:@"%@, %@, %@, %@, %@, %@, %@",
        ...
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :reverseGeocoder %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :reverseGeocoder %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
}
...
- (void) initDebriefData:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    NSArray * array = [result records];
    SMLog(kLogLevelVerbose,@"hello hi = %@",array);
        
    for (int i = 0; i < [array count]; i++)
	{
		ZKSObject * obj = [array objectAtIndex:i];
        
        SMLog(kLogLevelVerbose,@"SVMXDEV__Service_Group__c = %@", [[obj fields] objectForKey:@"SVMXDEV__Service_Group__c"]);
        if (appDelegate.appServiceTeamId != nil)
        {
            [appDelegate.appServiceTeamId release];
            appDelegate.appServiceTeamId = nil;
        }
		appDelegate.appServiceTeamId = [[[obj fields] objectForKey:@"SVMXDEV__Service_Group__c"] retain];
    ...
    NSString * _query = [NSString stringWithFormat:@"Select SVMXDEV__Street__c, SVMXDEV__City__c, SVMXDEV__State__c, SVMXDEV__Zip__c, SVMXDEV__Country__c FROM SVMXDEV__Service_Group_Members__c WHERE Id = '%@'", appDelegate.appTechnicianId];
    SMLog(kLogLevelVerbose,@"my query = %@",_query);
	
	SMLog(kLogLevelVerbose,@"initDebriefData, Mapview = %@", _query);
    SMXMonitor *monitor = [[[SMXMonitor alloc] init] autorelease];
    [monitor monitorSMMessageWithName:@"[FirstDetailViewController initDebrief]"
                         withUserName:appDelegate.currentUserName
                             logLevel:kPerformanceLevelWarning
                           logContext:@"Start"
                         timeInterval:kWSExecutionDuration];
...
- (void) didQueryTechnician:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        NSString * woStreet = [[dict objectForKey:@"SVMXDEV__Street__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SVMXDEV__Street__c"]:@"";
        NSString * woCity = [[dict objectForKey:@"SVMXDEV__City__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SVMXDEV__City__c"]:@"";
        NSString * woState = [[dict objectForKey:@"SVMXDEV__State__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SVMXDEV__State__c"]:@"";
        NSString * woZip = [[dict objectForKey:@"SVMXDEV__Zip__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SVMXDEV__Zip__c"]:@"";
        NSString * woCountry = [[dict objectForKey:@"SVMXDEV__Country__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SVMXDEV__Country__c"]:@"";
        ...
        SMLog(kLogLevelVerbose,@"Technician Address = %@", appDelegate.technicianAddress);
    }
    else
        appDelegate.technicianAddress = @"";  
    }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name FirstDetailViewController :didQueryTechnician %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason FirstDetailViewController :didQueryTechnician %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    @finally {
...
{
    SMLog(kLogLevelVerbose,@"FirstDetailViewController: Recieve [%@] notification in refreshMap",[notification name]);
...
- (void) updateWorkOrderInfo
{
    ...
    [aPool drain];
        ...
        @try{
            ...
            SMLog(kLogLevelError,@"Exception Name iPadScrollerViewController :showMap %@",exp.name);
            SMLog(kLogLevelError,@"Exception Reason iPadScrollerViewController :showMap %@",exp.reason);
            [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
        }
}
