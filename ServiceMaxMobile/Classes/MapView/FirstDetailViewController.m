...
- (void) viewDidLoad
{
    ...
    if ([appDelegate isInternetConnectionAvailable])
    {
        ...
        NSString * _query = [NSString stringWithFormat:@"SELECT Id, SPR14__Service_Group__c, SPR14__Inventory_Location__c  FROM SPR14__Service_Group_Members__c WHERE SPR14__Salesforce_User__c = '%@'", appDelegate.loggedInUserId];
...
- (void) setJobDetailsForWorkOrder:(NSDictionary *)workOrderDetails workOrderInfo:(NSDictionary *)workOrderInfo
{
    ...
    contactName.text = [workOrderInfo objectForKey:SPR14__CONTACT_NAME];
    contactEmail.text = [workOrderInfo objectForKey:SPR14__CONTACT_EMAIL];
    contactAddress.text = [workOrderInfo objectForKey:SPR14__CONTACT_PHONE];
    problemDescription.text = [workOrderInfo objectForKey:SPR14__PROBLEM_DESCRIPTION];
    WODescription.text = [workOrderInfo objectForKey:SPR14__ORDER_TYPE];
	}
    @catch (NSException *exp) {
...
{
    NSString * contactId = [currentWorkOrderInfo objectForKey:SPR14__CONTACT__C];
...
- (void) initDebriefData:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int i = 0; i < [array count]; i++)
	{
        ...
        SMLog(kLogLevelVerbose,@"SPR14__Service_Group__c = %@", [[obj fields] objectForKey:@"SPR14__Service_Group__c"]);
        if (appDelegate.appServiceTeamId != nil)
        {
            [appDelegate.appServiceTeamId release];
            appDelegate.appServiceTeamId = nil;
        }
		appDelegate.appServiceTeamId = [[[obj fields] objectForKey:@"SPR14__Service_Group__c"] retain];
    ...
    NSString * _query = [NSString stringWithFormat:@"Select SPR14__Street__c, SPR14__City__c, SPR14__State__c, SPR14__Zip__c, SPR14__Country__c FROM SPR14__Service_Group_Members__c WHERE Id = '%@'", appDelegate.appTechnicianId];
...
- (void) didQueryTechnician:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if ([array count] > 0)
    {
        ...
        NSString * woStreet = [[dict objectForKey:@"SPR14__Street__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SPR14__Street__c"]:@"";
        NSString * woCity = [[dict objectForKey:@"SPR14__City__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SPR14__City__c"]:@"";
        NSString * woState = [[dict objectForKey:@"SPR14__State__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SPR14__State__c"]:@"";
        NSString * woZip = [[dict objectForKey:@"SPR14__Zip__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SPR14__Zip__c"]:@"";
        NSString * woCountry = [[dict objectForKey:@"SPR14__Country__c"] isKindOfClass:[NSString class]]?[dict objectForKey:@"SPR14__Country__c"]:@"";
