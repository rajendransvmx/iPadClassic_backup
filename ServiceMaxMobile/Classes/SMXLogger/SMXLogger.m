    ...
    @synchronized(self)
    {
        ...
        if(insertLog)
        {
            ...
            if(format)
            {
                ...
                if(logContext)
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SPR14__SVMX_Job_Logs__c (SPR14__Log_Timestamp__c,SPR14__Log_level__c,SPR14__Log_Context__c,SPR14__Message__c,SPR14__Type__c,SPR14__Group_Id__c,SPR14__Profile_Id__c,SPR14__Log_Category__c,SPR14__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,logContext,messageToDB,@"iPad",groupID,profileID,logCategory,methodContext];
                }
                else
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SPR14__SVMX_Job_Logs__c (SPR14__Log_Timestamp__c,SPR14__Log_level__c,SPR14__Message__c,SPR14__Type__c,SPR14__Group_Id__c,SPR14__Profile_Id__c,SPR14__Log_Category__c,SPR14__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,messageToDB,@"iPad",groupID,profileID,logCategory,methodContext];
                }
            ...
            else
            {
                if(logContext)
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SPR14__SVMX_Job_Logs__c (SPR14__Log_Timestamp__c,SPR14__Log_level__c,SPR14__Log_Context__c,UserInfo__c,SPR14__Type__c,SPR14__Group_Id__c,SPR14__Profile_Id__c,SPR14__Log_Category__c,SPR14__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,logContext,@"iPad",groupID,profileID,logCategory,methodContext];
                }
                else
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SPR14__SVMX_Job_Logs__c (SPR14__Log_Timestamp__c,SPR14__Log_level__c,UserInfo__c,SPR14__Type__c,SPR14__Group_Id__c,SPR14__Profile_Id__c,SPR14__Log_Category__c,SPR14__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,@"iPad",groupID,profileID,logCategory,methodContext];
                }
           ...
           // synchronized_sqlite3_exec(appDelegate.db, [kDBEndTransaction UTF8String], NULL, NULL, &error);
            ...
            int existingLogsCount = [appDelegate.dataBase getRecordCountFromTable:@"SPR14__SVMX_Job_Logs__c"];
            
            if(existingLogsCount > kMaxNumberOfLogRecordsToKeep)
            {
                      int logsCountToDelete = existingLogsCount - kMaxNumberOfLogRecordsToKeep;
                     [appDelegate.dataBase deleteRecordFromTable:@"SPR14__SVMX_Job_Logs__c"
                                         numberOfRecordsToDelete:logsCountToDelete
                                                    orderByField:@"local_id"];
            }
            
           
            /*if([appDelegate.dataBase getRecordCountFromTable:@"SPR14__SVMX_Job_Logs__c"] > kMaxNumberOfLogRecordsToKeep)
            {
                int logsCountToDelete = [appDelegate.dataBase getRecordCountFromTable:@"SPR14__SVMX_Job_Logs__c"] - kMaxNumberOfLogRecordsToKeep;
                [appDelegate.dataBase deleteRecordFromTable:@"SPR14__SVMX_Job_Logs__c" numberOfRecordsToDelete:logsCountToDelete orderByField:@"local_id"];
            }
            */
        }
    }
...
- (void) createLoggerTable
{
    ...
    {
         NSString * query  = [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS SPR14__SVMX_Job_Logs__c (local_id INTEGER PRIMARY KEY  AUTOINCREMENT NOT NULL UNIQUE DEFAULT (0),SPR14__Log_Timestamp__c DATE,SPR14__Log_level__c int,SPR14__Log_Context__c TEXT,SPR14__Message__c TEXT,SPR14__Type__c TEXT,SPR14__Group_Id__c TEXT,SPR14__Profile_Id__c TEXT,SPR14__Log_Category__c TEXT,SPR14__Operation__c TEXT)"];
        char * err;
