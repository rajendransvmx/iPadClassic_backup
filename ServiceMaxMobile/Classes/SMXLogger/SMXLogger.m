    ...
    @synchronized(self)
    {
        ...
        if(insertLog)
        {
            ...
            if(format)
            {
                ...
                if(logContext)
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SVMXDEV__SVMX_Job_Logs__c (SVMXDEV__Log_Timestamp__c,SVMXDEV__Log_level__c,SVMXDEV__Log_Context__c,SVMXDEV__Message__c,SVMXDEV__Type__c,SVMXDEV__Group_Id__c,SVMXDEV__Profile_Id__c,SVMXDEV__Log_Category__c,SVMXDEV__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,logContext,messageToDB,@"iPad",groupID,profileID,logCategory,methodContext];
                }
                else
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SVMXDEV__SVMX_Job_Logs__c (SVMXDEV__Log_Timestamp__c,SVMXDEV__Log_level__c,SVMXDEV__Message__c,SVMXDEV__Type__c,SVMXDEV__Group_Id__c,SVMXDEV__Profile_Id__c,SVMXDEV__Log_Category__c,SVMXDEV__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,messageToDB,@"iPad",groupID,profileID,logCategory,methodContext];
                }
            ...
            else
            {
                if(logContext)
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SVMXDEV__SVMX_Job_Logs__c (SVMXDEV__Log_Timestamp__c,SVMXDEV__Log_level__c,SVMXDEV__Log_Context__c,UserInfo__c,SVMXDEV__Type__c,SVMXDEV__Group_Id__c,SVMXDEV__Profile_Id__c,SVMXDEV__Log_Category__c,SVMXDEV__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,logContext,@"iPad",groupID,profileID,logCategory,methodContext];
                }
                else
                {
                    insertStatement = [NSString stringWithFormat:@"INSERT INTO SVMXDEV__SVMX_Job_Logs__c (SVMXDEV__Log_Timestamp__c,SVMXDEV__Log_level__c,UserInfo__c,SVMXDEV__Type__c,SVMXDEV__Group_Id__c,SVMXDEV__Profile_Id__c,SVMXDEV__Log_Category__c,SVMXDEV__Operation__c) VALUES (\"%@\",\"%d\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",timeStamp,logLevel,@"iPad",groupID,profileID,logCategory,methodContext];
                }
           ...
           // synchronized_sqlite3_exec(appDelegate.db, [kDBEndTransaction UTF8String], NULL, NULL, &error);
            ...
            int existingLogsCount = [appDelegate.dataBase getRecordCountFromTable:@"SVMXDEV__SVMX_Job_Logs__c"];
            
            if(existingLogsCount > kMaxNumberOfLogRecordsToKeep)
            {
                      int logsCountToDelete = existingLogsCount - kMaxNumberOfLogRecordsToKeep;
                     [appDelegate.dataBase deleteRecordFromTable:@"SVMXDEV__SVMX_Job_Logs__c"
                                         numberOfRecordsToDelete:logsCountToDelete
                                                    orderByField:@"local_id"];
            }
            
           
            /*if([appDelegate.dataBase getRecordCountFromTable:@"SVMXDEV__SVMX_Job_Logs__c"] > kMaxNumberOfLogRecordsToKeep)
            {
                int logsCountToDelete = [appDelegate.dataBase getRecordCountFromTable:@"SVMXDEV__SVMX_Job_Logs__c"] - kMaxNumberOfLogRecordsToKeep;
                [appDelegate.dataBase deleteRecordFromTable:@"SVMXDEV__SVMX_Job_Logs__c" numberOfRecordsToDelete:logsCountToDelete orderByField:@"local_id"];
            }
            */
        }
    }
...
- (void) createLoggerTable
{
    ...
    {
         NSString * query  = [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS SVMXDEV__SVMX_Job_Logs__c (local_id INTEGER PRIMARY KEY  AUTOINCREMENT NOT NULL UNIQUE DEFAULT (0),SVMXDEV__Log_Timestamp__c DATE,SVMXDEV__Log_level__c int,SVMXDEV__Log_Context__c TEXT,SVMXDEV__Message__c TEXT,SVMXDEV__Type__c TEXT,SVMXDEV__Group_Id__c TEXT,SVMXDEV__Profile_Id__c TEXT,SVMXDEV__Log_Category__c TEXT,SVMXDEV__Operation__c TEXT)"];
        char * err;
