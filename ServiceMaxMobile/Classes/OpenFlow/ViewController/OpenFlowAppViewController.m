...
- (void) showSearch
{
    ...
    [self presentViewController:search animated:YES];
    [search release];
    */
}
- (void) showCalendar
...
- (void) showMap
{
	...
	SMLog(kLogLevelError,@"Exception Name OpenFlowAppViewController :showMap %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason OpenFlowAppViewController :showMap %@",exp.reason);
    }
...
{
    SMLog(kLogLevelVerbose,@"Selected item %@", [itemList objectAtIndex:index]);
    switch (index) {
        case 0:
            [self showTasks];
            break;
