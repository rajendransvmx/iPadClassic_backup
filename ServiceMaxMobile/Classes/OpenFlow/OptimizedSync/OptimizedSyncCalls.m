...
-(void)GetOptimizedDownloadCriteriaRecordsFor:(NSString *)event_name requestId:(NSString *)requestId;
{
	@try
	{
		...
		binding.logXMLInOut = [appDelegate enableLogs];
		NSLog(@"**************************** Start binding - One call sync *******************************");
		...
		NSLog(@"**************************** END binding - One call sync *******************************");
    }
	@catch (NSException *exp)
	{
        NSLog(@"Exception Name OptimizedSyncCalls :GETDownloadCriteriaRecordsFor %@",exp.name);
        NSLog(@"Exception Reason OptimizedSyncCalls :GETDownloadCriteriaRecordsFor %@",exp.reason);
    }
...
-(INTF_WebServicesDefServiceSvc_INTF_SFMRequest *) Put:(NSString *)event_name
{
	...
	@catch (NSException *exp)
	{
        NSLog(@"Exception Name OptimizedSyncCalls.m :Put %@",exp.name);
        NSLog(@"Exception Reason OptimizedSyncCalls.m :Put %@",exp.reason);
    }
...
-(void) tx_fetch
{
    @try
	{
        ...
        NSLog(@"Exception Name OptimizedSyncCalls :tx_fetch %@",exp.name);
        NSLog(@"Exception Reason OptimizedSyncCalls :tx_fetch %@",exp.reason);
    }
}
