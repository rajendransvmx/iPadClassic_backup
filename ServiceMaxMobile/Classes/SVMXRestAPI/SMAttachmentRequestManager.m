...
- (void)uploadResponseResult:(NSArray *)result withError:(NSError *)error andContext:(NSString *)context
{
    ...
    {
        NSArray *resultArray = [result valueForKey:@"id" ];

        NSString *sfId = [resultArray objectAtIndex:0];
     
//        NSLog(@" uploadResponseResult Susscees for localId : %@  - sfid : %@",context, sfId);
        if (nil != sfId)
        {
    ...
    [self completedActionService:context];
    
    // Delete record from Trailer table
    [appDelegate.attachmentDataBase deleteFromAttachmentTrailerTable:context];
}
