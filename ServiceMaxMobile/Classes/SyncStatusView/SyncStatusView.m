...
- (void)viewDidLoad
{
    ...
    pushLogStatus.text = [self setLastPushLog:plistDict]; //10312
...
- (NSString *) updateLastDataSynctime
{
    ...
    //10312
    NSDate * date = nil;
    date = [Utility getDateFromStringFromPlist:[dictionary objectForKey:DATASYNC_TIME_TOBE_DISPLAYED] userRedable:NO];
    lastSyncTime = [Utility getStringFromDateForDeviceTime:date];
...
-(NSString *) updateNextDataSyncTime
{
	...
	else
	{
        ...
        //10312
        NSDate * date = nil;
        date = [Utility getDateFromStringFromPlist:[dictionary objectForKey:NEXT_DATA_SYNC_TIME_DISPLAYED] userRedable:NO];
        nextSyncTime = [Utility getStringFromDateForDeviceTime:date];
	}
    return nextSyncTime;
...
- (NSString *) updateLastConfigsyncTime
{
    ...
    //10312
    NSDate * date = nil;
    date = [Utility getDateFromStringFromPlist:[dictionary objectForKey:LAST_INITIAL_META_SYNC_TIME] userRedable:NO];
    lastSyncTime = [Utility getStringFromDateForDeviceTime:date];
...
- (NSString *) updateNextConfigsyncTime
{
	...
	else
	{
        ...
        date = [Utility getDateFromStringFromPlist:[dictionary objectForKey:NEXT_META_SYNC_TIME] userRedable:NO];
...
//10312
- (NSString *) setLastPushLog:(NSDictionary *)plistDict
{
    NSString * pushLog = nil;
    NSDate * pushLogdate = nil;
    
    //This is to check when the app is upgrated
    if ([[plistDict objectForKey:PUSH_LOG_LABEL] isKindOfClass:[NSString class]])
    {
        pushLogdate = [Utility getDateFromStringFromPlist:[plistDict objectForKey:PUSH_LOG_LABEL] userRedable:YES];
    }
    else
    {
        pushLogdate = [plistDict objectForKey:PUSH_LOG_LABEL];
    }
    
    
    if (pushLogdate != nil)
    {
        pushLog = [Utility getStringFromDateForDeviceTime:pushLogdate];
        
    }
    if (pushLog == nil && [pushLog length] == 0)
    {
        pushLog = @"";
    }
    return pushLog;
