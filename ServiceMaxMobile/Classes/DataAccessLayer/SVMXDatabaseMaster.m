...
- (BOOL)openDatabase {
    ...
    NSString *documentsDirectory = [[[UIApplication sharedApplication] delegate] getAppCustomSubDirectory]; // [paths objectAtIndex:0];
    ...
    SMLog(kLogLevelVerbose,@"DALAYER: Database opened successfully ");
    return YES;
}
...
- (id)parseSOQLJsonStringFromDARequest:(DARequest *)requestObject {
    ...
    if ([requestObject.fieldsArray count] <= 0) {
        ...
        [objectFields addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"local_id",@"fieldName",@"string",@"fieldType", nil]];
        if ([objectFields count] > 0) {
...
- (id)describeObject:(DARequest *)request {
    ...
    @synchronized(self) {
        ...
        //11644 : Handled Precision Scale in OPDoc
        BOOL isScale = [appDelegate.dataBase isColumnPresentInTable:@"SFObjectField" columnName:@"scale"];
        NSArray *someArray = nil;
        
        //11644
        if(isScale) {
            sqlQuery = [NSString stringWithFormat:@"Select api_name, label ,type, reference_to, nillable, dependent_picklist, controler_field,scale from SFObjectField where object_api_name = '%@'",objectName];
            someArray = [[NSArray alloc] initWithObjects:@"name",@"label",@"dataType",@"referenceTo",@"nillable", @"dependentPicklist", @"controlerName",@"scale", nil];
        }
        else {
            ...
            someArray = [[NSArray alloc] initWithObjects:@"name",@"label",@"dataType",@"referenceTo",@"nillable", @"dependentPicklist", @"controlerName", nil];
        }
        selectStmt = nil;
        if(synchronized_sqlite3_prepare_v2(database, [sqlQuery UTF8String], -1, &selectStmt, nil) == SQLITE_OK)
        {
...
- (NSDictionary*)getFieldTypeForFieldName:(NSString *)fieldName Object:(NSString*)objectName {
    NSString *queryStatement1 = [NSMutableString stringWithFormat:@"SELECT type,reference_to FROM SFObjectField where object_api_name = '%@' and api_name = '%@' ",objectName,fieldName];
    sqlite3_stmt * labelstmt = nil;
    char *tempStr=nil;
    ...
    if ( synchronized_sqlite3_prepare_v2(database, [queryStatement1 UTF8String],-1, &labelstmt, nil) == SQLITE_OK )
    {
        if(synchronized_sqlite3_step(labelstmt) == SQLITE_ROW)
        {
            tempStr = (char *) sqlite3_column_text(labelstmt,0);
            if((tempStr != nil)&& strlen(tempStr))
                fieldType = [NSString stringWithUTF8String:tempStr];
            else
                fieldType = nil;
            
            tempStr = nil;
            
            tempStr = (char *) sqlite3_column_text(labelstmt,1);
            if((tempStr != nil)&& strlen(tempStr))
                referenceTable = [NSString stringWithUTF8String:tempStr];
            else
                referenceTable = nil;
