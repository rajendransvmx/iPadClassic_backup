...
- (void) syncLogsToServer
{
    @try
    {
        ...
        NSDictionary *typeDict = [NSDictionary dictionaryWithObject:@"SVMXDEV__SVMX_Job_Logs__c" forKey:@"type"];
...
{
    NSArray *columns = [NSArray arrayWithObjects:@"SVMXDEV__Log_Timestamp__c",@"SVMXDEV__Log_level__c",@"SVMXDEV__Log_Context__c",@"SVMXDEV__Message__c",@"SVMXDEV__Type__c",@"SVMXDEV__Profile_Id__c",@"SVMXDEV__Group_Id__c",@"SVMXDEV__Log_Category__c",@"SVMXDEV__Operation__c", nil];
    NSArray *logsArray = [appDelegate.dataBase getAllRecordsFromTable:@"SVMXDEV__SVMX_Job_Logs__c"
                                                           forColumns:columns
...
- (void) operation:(INTF_WebServicesDefBindingOperation *)operation completedWithResponse:(INTF_WebServicesDefBindingResponse *)response
{
    ...
    @try {
        ...
        if(status)
        {
            ...
            NSString *jobLogTableName = @"SVMXDEV__SVMX_Job_Logs__c";
            AppDelegate * appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
