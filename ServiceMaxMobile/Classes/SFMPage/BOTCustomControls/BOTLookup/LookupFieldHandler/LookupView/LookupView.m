...
@synthesize contextLookupFilter;
    ...
    if (advancedFilterView == nil) {
        ...
        if ([Utility notIOS7]) {
            ...
            filterView = [[UIScrollView alloc] initWithFrame:CGRectMake(0,
                                                                        44,
                                                                        self.view.frame.size.width,
                                                                        self.view.frame.size.height - 20)]; //Defect Fix - 8577
        }
        ...
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,
                                                                               ...
                                                                               advancedFilterView.frame.size.height)]; //Defect Fix - 8577
...
- (void)viewDidLoad
{
    ...
    if( ( (self.advancedFilters != nil) &&  ([self.advancedFilters count] > 0) ) || (self.contextLookupFilter.lookupContext != nil && ![self.contextLookupFilter.lookupContext isEqualToString:@""] && self.contextLookupFilter.allowOverride))
    {
...
- (void) setLookupData:(NSDictionary *)lookupDictionary
{
    ...
    if(appDelegate.isWorkinginOffline)
    {
        ...
        SMLog(kLogLevelError,@"Exception Name LookupView :setLookupData %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason LookupView :setLookupData %@",exp.reason);
...
{    
    ...
    @try{
        ...
        SMLog(kLogLevelError,@"Exception Name LookupView :accessoryButtonTappedForRowWithIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason LookupView :accessoryButtonTappedForRowWithIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void)switchValueChanged:(id)sender
{
    if ([sender isKindOfClass:[UISwitch class]])
    {
        ...
        SMLog(kLogLevelVerbose,@" filterSwitch  switchRow  : %d ->  %@", switchRow, (filterSwitch.on) ? @"ON" : @"OFF");
...
//krishna CONTEXTFILTER
- (void)switchValueChangedForContextLookUp:(id)sender {
    
    if ([sender isKindOfClass:[UISwitch class]])
    {
        UISwitch *filterSwitch = (UISwitch *) sender;
        SMLog(kLogLevelVerbose,@" filterSwitch for Context Row  : ->  %@", (filterSwitch.on) ? @"ON" : @"OFF");
        if (self.contextLookupFilter.lookupContext != nil && ![self.contextLookupFilter.lookupContext isEqualToString:@""]) {
            self.contextLookupFilter.isDefaultOn = filterSwitch.on;
            [self reloadData];
        }
    }
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if (tableView.tag == kTableviewTag ) {
    return 2; //context lookup filter + advanced look up filter
    }
    return 1;
}
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (tableView.tag == kTableviewTag ) {
        if (section == 0) {
            if ((self.contextLookupFilter.lookupContext != nil && ![self.contextLookupFilter.lookupContext isEqualToString:@""])) {
                if (self.contextLookupFilter.allowOverride) {
                    return 1;//only one filter at any instance for context lookup
                }
                return 0;//donot show if the allow over ride is false
            }
        }
        else if(section == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.accessoryType = UITableViewCellAccessoryDetailDisclosureButton;
        ...
        SMLog(kLogLevelError,@"Exception Name LookupView :cellForRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason LookupView :cellForRowAtIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    return cell;
}
...
forRowAtIndexPath: (NSIndexPath*)indexPath
{
    if (tableView.tag == kTableviewTag)
    {
        SVMXLookupFilter *lookupfilter;
        //Krishna CONTEXTFILTER
        if (indexPath.section == 0) {
            
            if((self.contextLookupFilter.lookupContext == nil || [self.contextLookupFilter.lookupContext isEqualToString:@""])) {
                return; //no context filter.
            }
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            lookupfilter = self.contextLookupFilter;
        }
            ...
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            lookupfilter = [self.advancedFilters objectAtIndex:indexPath.row];
        ...
        if (filterName == nil)
        {
            ...
            filterName.textAlignment = NSTextAlignmentLeft;
        ...
        if (filterSwitch == nil)
        {
            ...
            if (indexPath.section == 0) {
                ...
                if((self.contextLookupFilter.lookupContext != nil && ![self.contextLookupFilter.lookupContext isEqualToString:@""])) {
                    [filterSwitch addTarget:self action:@selector(switchValueChangedForContextLookUp:) forControlEvents:UIControlEventValueChanged];
                }
            }
            else {
...
- (void)loadLookupFilters {
    ...
    if(self.contextLookupFilter.lookupContext != nil && ![self.contextLookupFilter.lookupContext isEqualToString:@""]) {
        ...
        NSDictionary *dict = [appDelegate.databaseInterface getValueForFieldName:contextLookupFilter.lookupContext andLiteralValue:contextLookupFilter.sourceObjectName];
        
        NSString *contextFilterValue = [dict objectForKey:gFIELD_VALUE_KEY];
        NSString *labelName = [dict objectForKey:gFIELD_LABEL];
        
        
        //if boolean typr and value is "" for the first time set it to NO. else show the value it has - 10693.
        NSString *dataType = [dict objectForKey:gFIELD_DATA_TYPE];
        if ([[dataType lowercaseString] isEqualToString:@"boolean"]) {
            contextFilterValue = [contextFilterValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
            if (([contextFilterValue isEqualToString:@""] || (contextFilterValue == nil) || ([[contextFilterValue class] isEqual:[NSNull class]]))) {
                contextFilterValue = @"False";
            }
            else {
                if([[contextFilterValue lowercaseString] isEqualToString:@"true"])
                    contextFilterValue = @"True";
                else if([[contextFilterValue lowercaseString] isEqualToString:@"false"])
                    contextFilterValue = @"False";
                else {
                    contextFilterValue = ([contextFilterValue boolValue]) ? @"True" : @"False";
                }
            }
        }
        
        NSString *value = [NSString stringWithFormat:@"%@ %@: %@",[appDelegate.wsInterface.tagsDictionary objectForKey:SFM_LOOKUP_CONTEXT_LIMIT_TO],labelName,contextFilterValue];
        
        lookupFilter.name = value;
        //Need to remove this hardcoded value.
        lookupFilter.objectPermission = YES;
        //self.contextLookupFilter.objectPermission;
        lookupFilter.isDefaultOn = YES;
        lookupFilter.allowOverride = self.contextLookupFilter.allowOverride;
        lookupFilter.lookupContext = self.contextLookupFilter.lookupContext;
        lookupFilter.lookupQuery = self.contextLookupFilter.lookupQuery;
        lookupFilter.sourceObjectName = self.contextLookupFilter.sourceObjectName;
        self.contextLookupFilter = lookupFilter;
...
- (BOOL)isCriteriaSupportedOnLookup {
    ...
    if (package <  minVersion) {
        SMLog(kLogLevelWarning,@"Lookup filters is not supported in %@",currentServerPkgVersion);
        return NO;
    }
    SMLog(kLogLevelVerbose,@"Lookup filters is supported %@",currentServerPkgVersion);
    return YES;
}
