...
- (void)didShowLookUp
{
    ...
    SVMXLookupFilter *contextLookup = [[SVMXLookupFilter alloc] init];
    contextLookup.allowOverride = [lookupField.Override_Related_Lookup boolValue];
    contextLookup.lookupContext = lookupField.Field_Lookup_Context;
    contextLookup.lookupQuery = lookupField.Field_Lookup_Query;
    contextLookup.sourceObjectName = lookupField.sourceObject;
    self.lookupView.contextLookupFilter = contextLookup;
    [contextLookup release];
    
    //Shrinivas
    self.searchId = lookupField.searchId; 
    self.relatedObjectName = lookupField.objectName;
    ...
    NSInteger height = [Utility isIOS8] ? 10.0f :parent.heightForPopover; //Defect Fix :- 7447
...
- (void) searchObject:(NSString *)keyword withObjectName:(NSString *)objectName returnTo:(id)caller setting:(BOOL)idAvailable
{
    ...
    [appDelegate.wsInterface getLookUpFieldsWithKeyword:keyword forObject:objectName returnTo:caller setting:idAvailable overrideRelatedLookup:lookupField.Override_Related_Lookup lookupContext:Field_Lookup_Context_Value lookupQuery:lookupField.Field_Lookup_Query];
        ...
        SMLog(kLogLevelError,@"Exception Name LookupFieldPopover :searchObject %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason LookupFieldPopover :searchObject %@",exp.reason);
...
{
    NSDictionary *dict;
    dict = [appDelegate.databaseInterface getLookupDataFromDBWith:self.searchId referenceTo:self.relatedObjectName searchFor:keyword withPreFilters:self.lookupView.preFilters andFilters:self.lookupView.advancedFilters lookupContextFilter:self.lookupView.contextLookupFilter];
