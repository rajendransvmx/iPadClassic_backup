...
@synthesize Override_Related_Lookup, Field_Lookup_Context, Field_Lookup_Query,sourceObject;
...
- (void) didSelectObject:(NSArray *)lookupObject defaultDisplayColumn:(NSString *)defaultdisplayColumn
{
    ...
    [controlDelegate didUpdateLookUp:value fieldApiName:fieldAPIName valueKey:id_value];
        ...
        SMLog(kLogLevelError,@"Exception Name LookupField :didSelectObject %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason LookupField :didSelectObject %@",exp.reason);
...
- (void) addObjectToHistory:(NSArray *)lookupObject withObjectName:(NSString *)value
{
	...
	SMLog(kLogLevelError,@"Exception Name LookupField :addObjectToHistory %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason LookupField :addObjectToHistory %@",exp.reason);
	[appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
 ...
 didFinishPickingMediaWithInfo: (NSDictionary*) info
{
	...
	SMLog(kLogLevelError,@"Exception Name LookupField :imagePickerController %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason LookupField :imagePickerController %@",exp.reason);
	[appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
-(void) DismissBarCodeReader:(NSString *)_text
{
    ...
    SMLog(kLogLevelVerbose,@"Search Text =%@ lenght %d",_searchText,[_searchText length]);
    [delegateHandler LaunchPopover];
    [[self delegateHandler].lookupView updateTxtField:_text];
    SMLog(kLogLevelVerbose,@"Bar Code Scanned Text=%@",_text);
...
{
    SMLog(kLogLevelError,@"Failed to Scan the Barcode");
