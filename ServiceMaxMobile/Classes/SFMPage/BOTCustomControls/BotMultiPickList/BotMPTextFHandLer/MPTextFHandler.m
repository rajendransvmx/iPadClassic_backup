...
{
    [self didShowMultiPickList];
    return NO;
    
}

- (void)didShowMultiPickList
{
    ...
    if (isdependentPicklist && [controllerName length] != 0 && [validFor count] != 0 && isRTDependentPicklist && ![parent.fieldAPIName isEqualToString:@"RecordTypeId"]) {
        NSLog(@"Picklist has both RTDependent and field dependent");
        NSMutableArray *picklistValues = [[NSMutableArray alloc] init];
        NSArray *dependentPickLists = [self  getValuesForDependentPickList];
        NSMutableArray *RTDependentPicklist = [ appDelegate.databaseInterface  getRTPicklistValuesForFieldApiName:parent.fieldAPIName objectApiName:sfmObjectName recordTypeId:recordTypeId];
        NSMutableArray *sortedPicklistContents = [appDelegate.databaseInterface getSortedRTPicklistValues:RTDependentPicklist fieldApiName:parent.fieldAPIName objectApiName:sfmObjectName];
        ...
        contentView.pickListContent = picklistValues;
