...
-(void) tapMultiPicklist:(id)sender
{
    ...
    if([pickerContent count]>0)
    {
        ...
        NSLog(@"Multi-Picklist Max Size %d",Max_Length);
...
//    [picker_Array addObject:@""];
    @try{
        ...
        NSLog(@"Exception Name BotSpinnerTextField :getValuesForDependentPickList %@",exp.name);
        NSLog(@"Exception Reason BotSpinnerTextField :getValuesForDependentPickList %@",exp.reason);
    }
