...
-(void)tapSpinner:(id)sender
{
	@try{
        ...
        for(int i=0;i<[spinnerData count] ;i++)
        {
            ...
            NSLog(@"Size ====== %d",max);
            NSLog(@"Picklist Max Size %d",Max_length);
        }
        ...
        NSLog(@"Exception Name SpinnerTFhandler :textFieldShouldBeginEditing %@",exp.name);
        NSLog(@"Exception Reason SpinnerTFhandler :cetextFieldShouldBeginEditingllForRowAtIndexPath %@",exp.reason);
    }
