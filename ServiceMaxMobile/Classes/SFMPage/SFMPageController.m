...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil mode:(BOOL)viewMode
{
    ...
    if (self)
    {
        ...
        if (![Utility notIOS7]) {
            ...
            [masterView.navigationBar setBackgroundImage:navImage forBarMetrics:UIBarMetricsDefault];
            rootView.extendedLayoutIncludesOpaqueBars = YES;
            rootView.edgesForExtendedLayout = UIRectEdgeNone;
        }
        ...
        if (![Utility notIOS7]) {
            ...
            [detailViewController.navigationBar setBackgroundImage:navImage forBarMetrics:UIBarMetricsDefault];
...
-(void)presentProgressBar:(NSString *)object_name sf_id:(NSString *)sf_id  reocrd_name:(NSString *)record_name
{
    ...
    //[appDelegate invalidateAllTimers];//PB-TIME-FIX
    ...
    if(flag)
    {
        ...
        [monitor monitorSMMessageWithName:@"[SFMPageController presentProgressBar]"
        ...
        while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, NO)) 
        {
            ...
            SMLog(kLogLevelVerbose,@"SFMPageController.m : presentProgressBar: DOD");
#endif
