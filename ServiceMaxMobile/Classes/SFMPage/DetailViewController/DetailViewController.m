...
//            value = [enable integerValue];
    ...
    if (appDelegate.SFMPage != nil)
    {
        ...
        if([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"EDIT"]||[[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"STANDALONECREATE"] || [[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"SOURCETOTARGET"]||[[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"SOURCETOTARGETONLYCHILDROWS"])
        {
            ...
            [popover setPopoverContentSize:actionMenu.contentSize animated:YES];
...
- (void) accessoryTapped:(id)sender
{
    ...
    if(appDelegate.isWorkinginOffline)
    {
        ...
        //settings should be applied only for work order objects
        NSDictionary *hdr_object = [appDelegate.SFMPage objectForKey:@"header"];
        NSString * headerObjName = [hdr_object objectForKey:gHEADER_OBJECT_NAME];//SVMXC__Service_Order__c
        
        if ([headerObjName isEqualToString:@"SVMXC__Service_Order__c"]) {
...
- (NSString *) getHeaderBillingType {
    ...
    for (int i=0;i<[header_sections count];i++)
    {
        ...
        for (int j=0;j<[section_fields count];j++)
        {
            ...
            if([field_api isEqualToString:@"SVMXC__Billing_Type__c"]) {
                billingTypeValue = [section_field objectForKey:gFIELD_VALUE_VALUE];
            }
        }
...
- (void) updateIsBillableToAllTheLinesBasedOnSetting {
    ...
    BOOL isBillable = [self isBillableAfterRespectingTheSettings];
    for (int i=0;i<[details count];i++) //parts, labor, expense for instance
    {
        ...
        //Apply isbillable only to work order detail line.
        NSString *workOrderDetailLine = [detail objectForKey:gDETAIL_OBJECT_NAME];
        if ([workOrderDetailLine isEqualToString:@"SVMXC__Service_Order_Line__c"]) {
            NSString *isBillableStringValue = [NSString stringWithFormat:@"%d",isBillable];
            for (int j=0;j<[details_values count];j++) //parts for instance
            {
                NSMutableArray *child_record_fields = [details_values objectAtIndex:j];
                if([self isBillableFieldHiddenInPageLayout:child_record_fields])
                {
                    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
                    [dict setValue:isBillableStringValue forKey:gVALUE_FIELD_VALUE_VALUE];
                    [dict setValue:isBillableStringValue forKey:gVALUE_FIELD_VALUE_KEY];
                    [dict setValue:@"SVMXC__Is_Billable__c" forKey:gVALUE_FIELD_API_NAME];
                    [child_record_fields addObject:dict];
                    NSLog(@"[SET15-16] is Billable value is ADDED");
                ...
                else
                {
                    for (int k = 0; k < [child_record_fields count];k++) //fields of one part for instance
                    {
                        ...
                        if ([detail_api_name isEqualToString:@"SVMXC__Is_Billable__c"]) {
                            
                            [field setValue:isBillableStringValue forKey:gVALUE_FIELD_VALUE_VALUE];
                            [field setValue:isBillableStringValue forKey:gVALUE_FIELD_VALUE_KEY];
                            NSLog(@"[SET15-16] is Billable value is UPDATED");
                        }
                    }
                }
            }
...
//9794 : is Billable flag respecting setting 15-16
- (BOOL)isBillableFieldHiddenInPageLayout:(NSMutableArray *)array {
    BOOL isBillableHidden = YES;
    for (NSMutableDictionary *dict in array) {
        if ([[dict objectForKey:gVALUE_FIELD_API_NAME] isEqualToString:@"SVMXC__Is_Billable__c"]) {
            isBillableHidden = NO;
        }
    }
...
//9794 : is Billable flag respecting setting 15-16
-(BOOL)isBillableAfterRespectingTheSettings
{
    ...
    NSString *setting15Value = [appDelegate.settingsDict valueForKey:@"WORD005_SET015"];
    NSString *setting16Value = nil;
    BOOL isSetting15True = [setting15Value boolValue];
    NSLog(@"[SET15-16] Setting 15 value is %@",setting15Value);

    if ([setting15Value isEqualToString:@"Entitlement"]) {
        //Apply Setting16
        setting16Value = [appDelegate.settingsDict valueForKey:@"WORD005_SET016"];
        ...
        for (NSString *set16 in setting16ValueArray) {
            if ([set16 isEqualToString:billingTypeValue]) {
                returnValue = false; //return false according to setting 16
            }
        }
    }
    else {
        //change the value for is Billable.
        returnValue = isSetting15True;
    }
    NSLog(@"[SET15-16] Setting applied value is %d",returnValue);
    return returnValue;
}
...
- (void) addMultiChildRows:(NSMutableDictionary *)_array forIndex:(NSInteger)index  
{
    ...
    if(appDelegate.isWorkinginOffline)
    {
        for(int p= 0; p<[_array count];p++)
        {
            ...
            if ([headerObjName isEqualToString:@"SVMXC__Service_Order__c"]) {
                NSLog(@"[SET15-16] Settings applying on Multi add");
                [self updateIsBillableToAllTheLinesBasedOnSetting];
            }
...
//                [customDataDictionary release]; // Damodar - Win14 - MemMgt - revoked to fix crash
    ...
    else
    {
        ...
        //Pushpak 10128
        appDelegate.wsInterface.isCustomWebServiceCallInProgress = YES;
        ...
        while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, FALSE))
        {
            ...
            if (![appDelegate isInternetConnectionAvailable])
            {
                appDelegate.wsInterface.getPrice = TRUE;
                //Pushpak 10128
                appDelegate.wsInterface.isCustomWebServiceCallInProgress = NO;
                [activity stopAnimating];
                [self enableSFMUI];
                return;
            }
                                    ...
                                    withData:(NSString *)dataToValidateString
{
    ...
    NSString *appConfig = [NSString stringWithFormat:@"var appConfig = { \"title\" : \"Mobile business rules application\", \"version\" : \"1.0.0\",\
                           ...
                           ],\"app-config\" : {\"application-id\" : \"application\",\"enable-cache\" : true,\"enable-log\" : true,\"org-name-space\" : \"SVMXC\"},\"platform-config\" : {}};"];
...
-(void) displayAttachmentSharingView:(NSArray *)dataSource viewName:(NSString *)view sender:(UIButton *)button
{
   ...
   /* if ([Utility notIOS7]) {
    ...
    popover = [[UIPopoverController alloc] initWithContentViewController:sharingView];
    sharingView.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
    popover.delegate = self;
    [popover presentPopoverFromRect:button.bounds inView:button permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
