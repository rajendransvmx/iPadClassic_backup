...
- (void) addNavigationButtons:(NSString *)sectionTitle
{
    ...
    [appDelegate.SyncProgress setHidden:YES];
    appDelegate.SyncProgress = nil;
    
    appDelegate.SyncStatus = appDelegate.SyncStatus;
            ...
            for(int j= 0;j<[sectionFileds count]; j++)
            {
                ...
                for(int k=0;k < [all_Keys_values count];k++)
                {
                    ...
                    if([key isEqualToString:filed_api_name])
                    {
                        ...
                        else if([field_data_type isEqualToString:@"currency"]) //10346
                        {
        ...
        for(int j= 0; j < [details count]; j++)
        {
            ...
            for(int l = 0 ;l < [detail_values count]; l++)
            {
                ...
                for(int m = 0 ; m < [eachArray count];m++)
                {
                    ...
                    else if ([[fieldDataType objectForKey:api_name] isEqualToString:@"currency"]) //10346
                    {
                ...
                for(int x = 0 ; x < [object_mapping_keys count]; x++)
                {
                    ...
                    if(field_exist == TRUE)
                    {
                        if([field_key length] != 0)
                        {
                            ...
                            else if([data_type isEqualToString:@"currency"]) //10346
                            {
        ...
        for(int j= 0; j < [details count]; j++)
        {
            ...
            for(int l = 0 ;l < [detail_values count]; l++)
            {
                ...
                {   
                    ...
                    else if ([[fieldDataType objectForKey:api_name] isEqualToString:@"currency"])
                    {
                        NSString * value = [dict objectForKey:gVALUE_FIELD_VALUE_VALUE];
                        NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:api_name  objectName:detailObjectName validationField:MSCALE];
                        if (scale >= 0)
...
- (UITableViewCell *) SFMEditCellForTable:(UITableView *)_tableView AtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (selectedSection == SHOW_HEADER_ROW || selectedSection == SHOWALL_HEADERS)
	{
        ...
        for (int j = 0; j < [field_columns count]; j++)
		{
            ...
            else if ([field_datatype isEqualToString:@"currency"]) //10346
            {
            ...
            if([targetCall isEqualToString:save])
            {
                ...
                for (int i=0;i<[header_sections count];i++)
                {
                    ...
                    for (int j=0;j<[section_fields count];j++)
                    {
                        ...
                        else if ([dataType isEqualToString:@"currency"]) //10346
                        {
                    ...
                    for (int i=0;i<[details count];i++) //parts, labor, expense for instance
                    {
                        ...
                        for (int j=0;j<[details_values count];j++) //parts for instance
                        {
                            ...
                            if([line_record_id isEqualToString:@""])
                            {
                               ...
                               // NSLog(@"*******************************NEWLY CREATED Record LOCAL ID %@ name %@ *******************************",line_local_id,[detail_fields_dict  objectForKey:@"GE_AL_Part_Number__c"]);
...
//                                }
                                 ...
                                 //NSLog(@"*******************************EXISTING  Record LOCAL ID %@ name %@ *******************************",[sfm_detail_field_keyValue  objectForKey:@"local_id"],[sfm_detail_field_keyValue  objectForKey:@"GE_AL_Part_Number__c"]);
                                ...
                                [sfm_detail_field_keyValue  removeObjectForKey:@"details_saved_record"];
                                 [sfm_detail_field_keyValue  removeObjectForKey:@"Id"]; //shr-final-fixGe
                                ...
                                if(check_for_new_id)
                                {
                                   // NSLog(@"**********record from get preice event then insert it  bec it is is SF record******");
											...
											if(!does_exists)
											{
												...
												else
												{
                                                    ...
                                                    if(!does_exists)
                                                    {
                                                        ...
                                                        //shr-final-fixGe
                                                        NSString *sfidNew =  [appDelegate.databaseInterface getSfid_For_LocalId_FROM_SfHeapTable:line_record_id];
                                                        if (sfidNew.length <= 4) {
                                                            sfidNew = [appDelegate.databaseInterface checkforSalesForceIdForlocalId:detail_object_name local_id:line_record_id];
                                                            sfidNew = [sfidNew stringByReplacingOccurrencesOfString:@" " withString:@""];
                                                        }
                                                        if (sfidNew.length > 2) {
                                                            NSLog(@"SFID  found. So reupdate");
                                                            [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:line_record_id SF_id:sfidNew record_type:DETAIL operation:UPDATE object_name:detail_object_name sync_flag:@"false" parentObjectName:headerObjName parent_loacl_id:currentRecordId webserviceName:webserviceName className:className synctype:syncType headerLocalId:appDelegate.sfmPageController.recordId  requestData:customDataDictionary finalEntry:NO];//idvalue
                                                        }
                                                        else{
                                                            NSLog(@"SFID not found. So reinsert");
                                                            [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:line_record_id SF_id:@"" record_type:DETAIL operation:INSERT object_name:detail_object_name sync_flag:@"false" parentObjectName:headerObjName parent_loacl_id:currentRecordId webserviceName:webserviceName className:className synctype:syncType headerLocalId:appDelegate.sfmPageController.recordId requestData:customDataDictionary finalEntry:NO];
                                                        }
            ...
            {
                [activity startAnimating];
                // shr-vipin modified records
                
                //NSDictionary *sfmPageTempp = appDelegate.SFMPage;
                //[self printLineInfo:@"Localids of lines before reloading:"];
                //NSLog (@"SFMPage before reloading quicksave:::  - %@", sfmPageTempp);
                [self fillSFMdictForOfflineforProcess:appDelegate.sfmPageController.processId forRecord:appDelegate.sfmPageController.recordId ];
                [self didReceivePageLayoutOffline];
                
                //[self printLineInfo:@"Localids of lines after reloading:"];
                //sfmPageTempp = appDelegate.SFMPage;
               //NSLog (@"SFMPage before reloading After Quicksave:::  - %@", sfmPageTempp);
                
                //NSLog(@"QUICK SAVE COMPLETED");
            }
            [activity stopAnimating];
        }
...
- (void)eventOccured:(NSString *)eventName andParameter:(NSString *)jsonParameterString {
    ...
    else if ([eventName isEqualToString:@"bizruleresult"])
    {
        ...
        SMLog(kLogLevelVerbose,@"Data  = %@",jsonParameterString);
         ...
         //NSLog(@"jsonParameterString %@",jsonParameterString);
    }
}
...
//                if ([key isEqualToString:@"local_id"]) {
//                     NSLog(@"******************************* Record LOCAL ID AFTER GET PRICE ******************************* %@",value);
//                }
                [allFieldsMutable addObject:keysDictionary];
                [keysDictionary release];
                keysDictionary = nil;
...
- (void)moveTableviewForKeyboardHeight:(NSNotification *)notification
{
	...
	@try {
		if ([[notification name] isEqual:UIKeyboardDidShowNotification])
		{
            ...
            //fixge - keyboard
            CGFloat xxHeight = boundRect.size.height;
            CGFloat xxWidth = boundRect.size.width;
            if (xxWidth > xxHeight) {
                boundRect.size = CGSizeMake(xxHeight,xxWidth);
            }
...
- (void)printLineInfo:(NSString *)string {
    NSLog(@"%@::",string);
    NSDictionary *sfmPageTempp = appDelegate.SFMPage;
    NSMutableArray * details = [sfmPageTempp objectForKey:gDETAILS];
    
    for (int i = 0; i< [details count]; i++) {
        NSMutableDictionary * detail = [details objectAtIndex:i];
        NSString * alias = [detail objectForKey:@"detail_object_alias_name"];
        NSMutableArray * detail_Values_RECID = [detail objectForKey:gDETAIL_VALUES_RECORD_ID];
        
        NSLog(@"%@ ===>  %@",alias,detail_Values_RECID);
    }
