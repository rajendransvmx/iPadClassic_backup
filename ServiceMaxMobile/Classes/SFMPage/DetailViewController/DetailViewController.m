...
@synthesize sourceUpdateConfiguration;
...
- (void) didSwitchProcess:(NSDictionary *)objectDictionary
{
	...
	//8303 - defect fix
        NSString * message = @"";
        if (pinfo.expressionId != nil && [pinfo.expressionId length] > 0)
        {
           message = [appDelegate.dataBase expressionErrorMessageById:pinfo.expressionId];
            
            if (![Util isValidString:message] )
            {
                message = [appDelegate.wsInterface.tagsDictionary objectForKey:NO_VIEW_LAYOUT];
            }
        }
        else
        {
...
- (void) didInternetConnectionChange:(NSNotification *)notification
{
    ...
    else
    {
        ...
        /*      010677 - Defect Fix - Below piece of code is commented since the app is fetching data offline, there is no need to
         refersh the data when there is switching between the connectivity. This piece of code is explicity written for the enterprise app*/
...
- (void) addNavigationButtons:(NSString *)sectionTitle
{
    ...
    [actionButton setAccessibilityIdentifier:kAccBackButton];
...
//        actionButton.accessibilityLabel = @"sfm_signature_capture";
        /*Accessibility Changes*/
        [actionButton setAccessibilityIdentifier:kAccSignatureCapture];
    ...
    if(appDelegate.isWorkinginOffline)
    {
        ...
        /*Accessibility Changes*/
        actionButton.isAccessibilityElement = YES;
        [actionButton setAccessibilityIdentifier:kAccHelpButton];
...
- (void)splitViewController:(UISplitViewController *)svc willHideViewController:(UIViewController *)aViewController withBarButtonItem:(UIBarButtonItem *)barButtonItem forPopoverController: (UIPopoverController *)pc
{
    ...
    if (actionBtn == nil)
    {
        ...
        [actionButton addTarget:self action:@selector(action:) forControlEvents:UIControlEventTouchUpInside];
         /*Accessibility Changes*/
        actionButton.isAccessibilityElement = YES;
        [actionButton setAccessibilityIdentifier:kAccActions];
...
-(void)fillSFMdictForOfflineforProcess:(NSString *) processId forRecord:(NSString *)recordId
{
   ...
   // databaseIntefaceSfm * database = [[databaseIntefaceSfm alloc] init];
    ...
    if(page_layoutInfo == nil)
    {
      ...
      // Clean SUCCESSIVE_SYNC array and make room for next set of details
    //SUCCESSIVE_SYNC
    [appDelegate.databaseInterface.modifiedLineRecords removeAllObjects];
...
//        [details_api_keys release];
    ...
    if([process_type isEqualToString:@"VIEWRECORD"] )
    {
            ...
            for(int j= 0;j<[sectionFileds count]; j++)
            {
                ...
                for(int k=0;k < [all_Keys_values count];k++)
                {
                    ...
                    if([key isEqualToString:filed_api_name])
                    {
                        ...
                        else if([field_data_type isEqualToString:@"currency"] || [field_data_type isEqualToString:@"percent"] || [field_data_type isEqualToString:@"double"]) //10346
                        {
                            NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:filed_api_name objectName:headerObjName validationField:MSCALE];
                            if (scale >= 0)
                            {
                                BOOL iSNaN = [Utility isValueNotANumber:field_key];
                                
                                if ([field_key length] > 0 && (![field_key isEqualToString:@""]) && !iSNaN)
                                {
                                    field_value = field_key;
                                    
                                    NSString *formattedNumber = [Utility  getFormattedString:field_value decimalPoint:scale];
        ...
        for(int j= 0; j < [details count]; j++)
        {
            ...
            //10346
            NSMutableDictionary * fieldDataType = [NSMutableDictionary dictionaryWithCapacity:0];
            ...
            for(int k =0 ;k<[filedsArray count];k++)
            {
                ...
                NSString * api_name = [detailFiled_info objectForKey:gFIELD_API_NAME];
                [details_api_keys addObject:api_name];
                [fieldDataType setObject:[detailFiled_info objectForKey:gFIELD_DATA_TYPE] forKey:api_name]; //10346
            }
            ...
            for(int l = 0 ;l < [detail_values count]; l++)
            {
                ...
                for(int m = 0 ; m < [eachArray count];m++)
                {
                    ...
                    else if ([[fieldDataType objectForKey:api_name] isEqualToString:@"currency"] || [[fieldDataType objectForKey:api_name] isEqualToString:@"percent"] || [[fieldDataType objectForKey:api_name] isEqualToString:@"double"]) //10346
                    {
                        NSString * value = [dict objectForKey:gVALUE_FIELD_VALUE_VALUE];
                        NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:api_name  objectName:detailObjectName validationField:MSCALE];
                        if (scale >= 0)
                        {
                            BOOL iSNaN = [Utility isValueNotANumber:value];
                            
                            if ([value length] > 0 && (![value isEqualToString:@""]) && !iSNaN)
                            {
                                NSString *formattedNumber = [Utility  getFormattedString:value decimalPoint:scale];
                                if ([formattedNumber length] > 0)
                                {
                                    NSMutableString * scalingString = [NSMutableString stringWithFormat:@"%@", formattedNumber];
                                ...
                                [dict setObject:value forKey:gVALUE_FIELD_VALUE_VALUE];
                                [dict setObject:key forKey:gVALUE_FIELD_VALUE_KEY];
                            }
                            else if (iSNaN)
                            {
        ...
        NSString * temp = @"SVMXDEV__Service_Order__c";
        ...
        if([object_api_name isEqualToString:work_order] && [processType isEqualToString:@"VIEWRECORD"])
        {
            ...
            NSString * account_id  = [additionalInfo objectForKey:@"SVMXDEV__Company__c"];
            NSString * toplevelId  = [additionalInfo objectForKey:@"SVMXDEV__Top_Level__c"];
            NSString * componentId = [additionalInfo objectForKey:@"SVMXDEV__Component__c"];
            ...
            if(account_history == TRUE)
            {
                if(account_id != nil && [account_id length] != 0)
                {
                    //shrinivas
                    NSString * Id = @"";
                    NSString *query = [NSString stringWithFormat:@"Select Id From SVMXDEV__Service_Order__c Where local_id = '%@'", appDelegate.sfmPageController.recordId];
                    sqlite3_stmt *stmt;
            ...
            if(product_history == TRUE)
            {
                ...
                {
                    productHistory = [appDelegate.databaseInterface getProductHistoryForanWorkOrder:Id filedName:@"SVMXDEV__Top_Level__c" tableName:headerObjName fieldValue:toplevelId];
                }
                if(componentId != nil && [componentId length] != 0)
                {
                    productHistory = [appDelegate.databaseInterface getProductHistoryForanWorkOrder:Id filedName:@"SVMXDEV__Component__c" tableName:headerObjName fieldValue:componentId];
                }
    ...
    else if ([process_type isEqualToString:@"SOURCETOTARGETONLYCHILDROWS"])
    {
        ...
        //4850
        if (appDelegate.sfmPageController.recordId.length > 0) {
            
            [sourceTargetDictionary setObject:appDelegate.sfmPageController.recordId forKey:HEADER_SOURCE_ID_STRING];
        }
        
        if (headerObjName.length > 0) {
            
            [sourceTargetDictionary setObject:headerObjName forKey:HEADER_SOURCE_OBJECT_NAME];
        }
                    ...
                    if([key isEqualToString:filed_api_name])
                    {
                        ...
                        if([field_key length] != 0)
                        {
                            ...
                            else if ([field_data_type isEqualToString:@"currency"] || [field_data_type isEqualToString:@"percent"] || [field_data_type isEqualToString:@"double"]) //10346
                            {
                                ...
                                if (scale >= 0)
                                {
                                    BOOL iSNaN = [Utility isValueNotANumber:field_value];
                                    
                                    if ([field_value length] > 0 && (![field_value isEqualToString:@""]) && !iSNaN)
                                    {
                                        NSString *formattedNumber = [Utility  getFormattedString:field_value decimalPoint:scale];
                                        if ([formattedNumber length] > 0)
                                        {
        ...
        //4850
        NSMutableDictionary *detailProcessComponentSourceDictionary = [[NSMutableDictionary alloc] init];
        for(int j= 0; j < [details count]; j++)
        {
            NSMutableDictionary * dict = [details objectAtIndex:j];
            NSMutableArray * filedsArray = [dict objectForKey:gDETAILS_FIELDS_ARRAY];
            ...
            NSMutableDictionary * fieldDataType = [NSMutableDictionary dictionaryWithCapacity:0]; //10346
            ...
            NSMutableDictionary * process_components = [appDelegate.databaseInterface getProcessComponentsForComponentType:TARGETCHILD process_id:processId layoutId:detail_layout_id objectName:detailObjectName];
            //4850
			NSString *processSfid = [process_components objectForKey:PROCESS_COMPONENT_SFID];
            NSString  *targetObjectName = [process_components objectForKey:TARGET_OBJECT_NAME];
            ...
            {
                //4850
                NSString *localIdOfDetailRecord = @"";
                [detailValuesArray addObject:[detail_values objectAtIndex:l]];
                NSMutableArray *  eachArray = [detail_values objectAtIndex:l];
                for(int m = 0 ; m < [eachArray count];m++)
                {
                    ...
                    //4580
                    //Before removing the local id , store it as source in Source update dictionary
					if([api_name isEqualToString:@"local_id"])
                    {
                        //4850
                        localIdOfDetailRecord = [NSString stringWithFormat:@"%@",[dict objectForKey:gVALUE_FIELD_VALUE_KEY]];
                    ...
                    else if ([[fieldDataType objectForKey:api_name] isEqualToString:@"currency"] || [[fieldDataType objectForKey:api_name] isEqualToString:@"percent"] || [[fieldDataType objectForKey:api_name] isEqualToString:@"double"]) //10346
                    {
                        ...
                        if (scale >= 0)
                        {
                            ...
                            if ([value length] > 0 && (![value isEqualToString:@""]) && !iSNaN)
                            {
                                ...
                                [dict setObject:value forKey:gVALUE_FIELD_VALUE_VALUE];
                                [dict setObject:value forKey:gVALUE_FIELD_VALUE_KEY];
                            }
                            else if (iSNaN)
                            {
                                [dict setObject:@"" forKey:gVALUE_FIELD_VALUE_VALUE];
                ...
                //4850
                [self addRecords:localIdOfDetailRecord ofObjectName:targetObjectName forComponentId:processSfid ToDictionary:detailProcessComponentSourceDictionary];
                [detail_Values_id addObject:@""];
            }
        }
        //4850
        if ([detailProcessComponentSourceDictionary count] > 0) {
            [sourceTargetDictionary setObject:detailProcessComponentSourceDictionary forKey:DETAIL_OBJECTS_SOURCE];
        }
        [detailProcessComponentSourceDictionary release];
        detailProcessComponentSourceDictionary = nil;
    }
    ...
    else if ([process_type isEqualToString:@"SOURCETOTARGET"] )
    {
        ...
        //4850
        if (appDelegate.sfmPageController.sourceRecordId.length > 0) {
            
            [sourceTargetDictionary setObject:appDelegate.sfmPageController.sourceRecordId forKey:HEADER_SOURCE_ID_STRING];
        }
        
        if (source_parent_object_name.length > 0) {
            
            [sourceTargetDictionary setObject:source_parent_object_name forKey:HEADER_SOURCE_OBJECT_NAME];
        }
            ...
            for(int j= 0;j<[sectionFileds count]; j++)
            {
                ...
                for(int k=0;k < [all_Keys_values count];k++)
                {
                    ...
                    if([key isEqualToString:filed_api_name])
                    {
                        ...
                        else if ([field_data_type isEqualToString:@"currency"] || [field_data_type isEqualToString:@"double"] || [field_data_type isEqualToString:@"percent"]) //10346
                        {
                            NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:filed_api_name  objectName:headerObjName validationField:MSCALE];
...
//            NSString * parent_column_name = [process_components objectForKey:PARENT_COLUMN_NAME];
                ...
                for(int x = 0 ; x < [object_mapping_keys count]; x++)
                {
                    ...
                    if(field_exist == TRUE)
                    {
                        if([field_key length] != 0)
                        {
                            ...
                            else if([data_type isEqualToString:@"currency"] || [data_type isEqualToString:@"percent"] || [data_type isEqualToString:@"double"]) //10346
                            {
                                NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:object_mapping_key  objectName:detailObjectName validationField:MSCALE];
                                if (scale >= 0)
                                {
                                    BOOL iSNaN = [Utility isValueNotANumber:field_value];
            ...
            [detail_Values_id  addObject:@""];
            //4850
            //GET THE local id and source object name and keep it in Dict under process component Id and store it in SFMPage Dictionary
            NSString *sourceRecordLocalId = [object_mapping_dict objectForKey:@"local_id"];//source_child_object_name
            [self addRecords:sourceRecordLocalId ofObjectName:source_child_object_name forComponentId:processSfID ToDictionary:detailProcessComponentSourceDictionary];
          }
    ...
    else if([process_type isEqualToString:@"EDIT"])
    {
        ...
        //SMLog(kLogLevelVerbose," record id %@" ,appDelegate.sfmPageController.recordId);
          ...
          //FieldMerging - headerValueDict
        NSMutableDictionary * headerValueDict = [appDelegate.databaseInterface queryDataFromObjectTable:api_names tableName:headerObjName record_id:appDelegate.sfmPageController.recordId expression:expression_id];
        
        NSLog(@" FieldMerging headerValueDict : %@", headerValueDict);
		//Change of Code for quick save. --> 10/07/2012  -- #4665
		BOOL Entry_criteria = [appDelegate.databaseInterface EntryCriteriaForRecordFortableName:headerObjName record_id:appDelegate.sfmPageController.recordId expression:expression_id];
        ...
        //FieldMerging -   header value_mapping_dict
		
		NSMutableDictionary * value_mapping_dict = [appDelegate.databaseInterface getObjectMappingForMappingId:process_components mappingType:VALUE_MAPPING];
        self.headerValueMappingDict = value_mapping_dict;
        
        NSLog(@" FieldMerging  value_mapping_dict : %@", value_mapping_dict);
        ...
        for(int j= 0; j < [details count]; j++)
        {
			...
			NSMutableDictionary * detail_value_mapping_dict = [appDelegate.databaseInterface getObjectMappingForMappingId:process_components mappingType:VALUE_MAPPING];
             ...
             //FieldMerging -   detail detail_value_mapping_dict
            
            NSLog(@" FieldMerging  detail_value_mapping_dict : %@", detail_value_mapping_dict);
            ...
            NSMutableArray * detail_values = [appDelegate.databaseInterface queryLinesInfo:details_api_keys detailObjectName:detailObjectName headerObjectName:headerObjName detailaliasName:detailaliasName headerRecordId:appDelegate.sfmPageController.recordId expressionId:expressionId parent_column_name:parent_column_name sorting_order:sorting_order];
            //FieldMerging -   detail detail_values
            
            NSLog(@" FieldMerging  detail_values : %@", detail_values);
            ...
            for(int l = 0 ;l < [detail_values count]; l++)
            {
                ...
                {   
                    ...
                    else if ([[fieldDataType objectForKey:api_name] isEqualToString:@"currency"] || [[fieldDataType objectForKey:api_name] isEqualToString:@"percent"] || [[fieldDataType objectForKey:api_name] isEqualToString:@"double"])
                    {
                        NSString * value = [dict objectForKey:gVALUE_FIELD_VALUE_VALUE];
                        NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:api_name  objectName:detailObjectName validationField:MSCALE];
                        if (scale >= 0)
...
//            [details_api_keys release];
    ...
    //4850 - Update the SOURCE UPDATE to appdelegate.sfmpage
    @synchronized(self){
            
        NSMutableDictionary *someDictionary = [[NSMutableDictionary  alloc] initWithDictionary:page_layoutInfo];
            
        if ([sourceTargetDictionary count] > 0) {
                [someDictionary setObject:sourceTargetDictionary forKey:SOURCE_INFO_OF_RECORDS];
        }
        appDelegate.SFMoffline = someDictionary;
        [someDictionary release];
        someDictionary = nil;
        self.sourceUpdateConfiguration = nil;
            
    }
    [sourceTargetDictionary release];
    sourceTargetDictionary = nil;
...
-(void)pushtViewProcessToStack:(NSString *)process_id  record_id:(NSString *)record_id
{
    //9681 24/1/2014 : removed dispatch once
        NSString * temp_process_id = [process_id mutableCopy]; // Damodar - Win14 - MemMgt - revoked to fix crash
            ...
            if( ![temp_process_id length] >0 || temp_process_id == nil)
            {
                temp_process_id = @"";
            }
            
            if( ![temp_record_id length] >0 || temp_record_id == nil)
            {
                temp_record_id = @"";
            }
        
        
        NSDictionary *sfmTempDict=[[NSDictionary alloc] initWithObjects:[NSArray arrayWithObjects:temp_process_id,temp_record_id, nil] forKeys:[NSArray arrayWithObjects:PROCESSID,RECORDID, nil]];
        [appDelegate.sfmPageController.process_stack addObject:sfmTempDict];
        [sfmTempDict release]; sfmTempDict = nil; 
...
- (void) action:(id)sender
{
   ...
   // if ([headerButtons count] > 0)
    ...
    for(int i = 0 ; i< [headerButtons count];i++)   
    {
        ...
        NSString * button_type = ([[array objectAtIndex:0] objectForKey:@"SVMXDEV__Event_Call_Type__c"]!= nil)?[[array objectAtIndex:0] objectForKey:@"SVMXDEV__Event_Call_Type__c"]:@"";
    ...
    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"VIEWRECORD"]) 
    {
        ...
        if([heder_object_name isEqualToString:@"SVMXDEV__Service_Order__c"])
        {
...
//            value = [enable integerValue];
    ...
    if([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"STANDALONECREATE"] || [[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"SOURCETOTARGET"])
    {
        ...
        //10913
        NSArray *newKeys = [NSArray arrayWithObjects:save,quick_save,cancel, nil];
        NSMutableArray *newButtonsArray = [[NSMutableArray alloc] init];
        int indexNumber = 0;
        for(NSString *buttonkey in newKeys)
        {
            for(NSDictionary *buttonDict in buttonsArray_offline)
            {
                NSString *description = [buttonDict objectForKey:@"action_description"];
                if([description caseInsensitiveCompare:buttonkey] == NSOrderedSame)
                {
                    [newButtonsArray insertObject:buttonDict atIndex:indexNumber++];
        ...
        for(NSDictionary *buttonDict in buttonsArray_offline)
        {
            NSString *description = [buttonDict objectForKey:@"action_description"];
            //10913
            if((![description isEqualToString:save]) && (![description isEqualToString:quick_save]) && (![description isEqualToString:cancel]))
            {
                [newButtonsArray addObject:buttonDict];
                
            }
        }
        
        NSLog(@"New Button Dict = %@",newButtonsArray);
        
        
        actionMenu.buttons = newButtonsArray;

        
          [newButtonsArray release];
...
{    
    ...
    else
    {
        ...
        [activity stopAnimating];
        //[appDelegate ScheduleIncrementalDatasyncTimer];//PB-TIME-FIX
...
- (void) startSummaryDataFetch
{
    ...
    NSArray * _keys = [NSArray arrayWithObjects:SVMXDEV__Activity_Type__c, SVMXDEV__Actual_Price2__c, SVMXDEV__Actual_Quantity2__c, SVMXDEV__Billable_Quantity__c,SVMXDEV__Billable_Line_Price__c,nil];
    // Calculate Labor
...
//    NSDictionary * headerData = [headerDataDict objectForKey:gHEADER_DATA];
  ...
  /*  troubleshooting.productId = [self getObjectNameFromHeaderData:headerData forKey:gSVMXDEV__Product__c];
    troubleshooting.productName = [self getObjectNameFromHeaderData:headerData forKey:gSVMXDEV__Product_Name__c];*/
...
- (void) showChatter
{
    ...
    NSDictionary * headerDataDict = [appDelegate.SFMPage objectForKey:gHEADER];
    NSDictionary * headerData = [headerDataDict objectForKey:gHEADER_DATA];
    _chatter.productId = [self getObjectNameFromHeaderData:headerData forKey:gSVMXDEV__Product__c];
...
- (void) getParts:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		SMLog(kLogLevelVerbose,@"SVMXDEV__Product__c = %@", [dict objectForKey:gSVMXDEV__Product__c] );
        NSMutableDictionary *Part = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
        
        ZKSObject * obj2 = [[obj fields] objectForKey:gSVMXDEV__Product__r];
        NSDictionary * SVMXDEV__Product__r = [obj2 isKindOfClass:[NSNull class]]?nil:[obj2 fields];
        if (SVMXDEV__Product__r != nil)
        {
            NSString * partName = [[SVMXDEV__Product__r objectForKey:gName] isKindOfClass:[NSString class]]?[SVMXDEV__Product__r objectForKey:gName]:@"";
            [Part setObject:partName forKey:gName];
        }
        
        NSString * numPartsUsed = [dict objectForKey:gSVMXDEV__Actual_Quantity2__c];
        ...
        NSString * description = [dict objectForKey:gSVMXDEV__Work_Description__c];
        ...
        if ([Part objectForKey:KEY_COSTPERPART] == nil)
            [Part setObject:[[obj fields] objectForKey:gSVMXDEV__Actual_Price2__c] forKey:KEY_COSTPERPART];
        
        if ([Part objectForKey:KEY_PRODUCTID] == nil)
            [Part setObject:[[obj fields] objectForKey:gSVMXDEV__Product__c] forKey:KEY_PRODUCTID];
		
		//pavaman 26th Feb 2011 - adding discount field
        NSString * discount = [dict objectForKey:gSVMXDEV__Discount__c];
...
- (void) getExistingLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int j = 0; j < [array count]; j++)
    {
        ...
        if ([[[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:CALIBRATION])
        {
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_CALIBRATION];
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_CALIBRATION];
        }
        if ([[[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:CLEANUP])
        {
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_CLEANUP];
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_CLEANUP];
        }
        if ([[[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:INSTALLATION])
        {
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_INSTALLATION];
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_INSTALLATION];
        }
        if ([[[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:REPAIR])
        {
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_REPAIR];
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_REPAIR];
        }
        if ([[[obj fields] objectForKey:@"SVMXDEV__Activity_Type__c"] isEqualToString:SERVICE])
        {
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Price2__c"] forKey:RATE_SERVICE];
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SVMXDEV__Actual_Quantity2__c"] forKey:QTY_SERVICE];
        }
    }
    
    NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c	WHERE SVMXDEV__Group_Member__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appTechnicianId];
...
- (void) getPriceForLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if( array != nil && ![array count] )
	{
		...
		NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV__Billable_Cost2__c FROM SVMXDEV__Service_Group_Costs__c WHERE SVMXDEV__Service_Group__c = '%@' AND SVMXDEV__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appServiceTeamId];
	...
	else
	{
        ...
        rate = [[[obj fields] objectForKey:@"SVMXDEV__Billable_Cost2__c"] retain];
...
- (void) getPriceForServiceTeam:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		SMLog(kLogLevelVerbose,@"%@", [[obj fields] objectForKey:@"SVMXDEV__Billable_Cost2__c"]);
		rate = [[[obj fields] objectForKey:@"SVMXDEV__Billable_Cost2__c"] retain];
        if (rate == nil || [rate isKindOfClass:[NSNull class]])
			rate = @"0.0";
	}
...
- (void) requireFieldWarning
{
    ...
    [requiredFields show];
    //krishna 009752 moved release to delegate
...
- (UIView *) tableView:(UITableView *)_tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if( selectedSection == SHOW_ADDITIONALINFO_ROW || selectedSection == SHOW_ALL_ADDITIONALINFO)
    {
        ...
        /*Accessibility changes*/
        seeMoreButton.isAccessibilityElement = YES;
        [seeMoreButton setAccessibilityIdentifier:kAccSeeMore];
...
- (UITableViewCell *) SFMEditCellForTable:(UITableView *)_tableView AtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (selectedSection == SHOW_HEADER_ROW || selectedSection == SHOWALL_HEADERS)
	{
        ...
        for (int j = 0; j < [field_columns count]; j++)
		{
            ...
            NSDictionary * precisionDict = nil; //10346
            
            NSString * object_api_name	=  [[appDelegate.SFMPage objectForKey:@"header"] objectForKey:gHEADER_OBJECT_NAME]; //10346
            ...
            NSString * lookupContext = nil, * lookupQuery = nil, *lookupContextSourceObject = nil;
            if ([field_datatype isEqualToString:@"reference"])
            {
                ...
                lookupContextSourceObject = [dict objectForKey:gCONTEXTSOURCEOBJ];
                
            }
            else if ([field_datatype isEqualToString:@"currency"] || [field_datatype isEqualToString:@"double"] || [field_datatype isEqualToString:@"percent"]) //10346
            {
...
//            [c setAccessibilityIdentifier:@"AddLinesButton"];
            /*Accessibility changes*/
            [c setAccessibilityIdentifier:kAccAddLines];
...
//                [control setAccessibilityIdentifier:@"AddMultiLinesButton"];
                /*Accessibility changes*/
                [control setAccessibilityIdentifier:kAccAddMultiLines];
		...
		else // Display the column values for the row
		{
         ...
         //   [cell setAccessoryType:UITableViewCellAccessoryDetailDisclosureButton];
            ...
            /*Accessibility changes*/
            triangleBtn.isAccessibilityElement = YES;
            [triangleBtn setAccessibilityIdentifier:kAccDisclosure];
        ...
        {
            {
                ...
                /*Accessibility changes*/
                switchButton.isAccessibilityElement = YES;
                [switchButton setAccessibilityIdentifier:kAccChildSfmBtn];
...
//                [viewPadding release]; // Damodar - Win14 - MemMgt - revoked to fix crash
                ...
                /*Accessibility changes*/
                triangleBtn.isAccessibilityElement = YES;
                [triangleBtn setAccessibilityIdentifier:kAccDisclosureDown];
                ...
                /*Accessibility changes*/
                saveBtn.isAccessibilityElement = YES;
                [saveBtn setAccessibilityIdentifier:kAccLineEditingDone];
                ...
                /*Accessibility changes*/
                removeBtn.isAccessibilityElement = YES;
                [removeBtn setAccessibilityIdentifier:kAccDiscardLine];
        ...
        if(SLA_FLAG)
        {
            ...
            else
            {
                ...
                if(appDelegate.isWorkinginOffline)
                {
                    ...
                    sla_clock_paused = [SLADict objectForKey:@"SVMXDEV__Sla_Clock_Paused__C"];
                    actual_resolution = [SLADict objectForKey:@"SVMXDEV__Actual_Resolution__C"];
                    actual_restoration = [SLADict objectForKey:@"SVMXDEV__Actual_Restoration__C"];
                    restorationCustomerBy = [SLADict objectForKey:@"SVMXDEV__Restoration_Customer_By__C"];
                    resolutionCustomerBy = [SLADict objectForKey:@"SVMXDEV__Resolution_Customer_By__C"];
                    pausedTime = [SLADict objectForKey:@"SVMXDEV__Sla_Clock_Pause_Time__C"];
            ...
            if ([field_data_type isEqualToString:@"reference"])
            {
                ...
                if (recordExists)
                {
                    for (int j = 0; j < [appDelegate.view_layout_array count]; j++)
                    {
                        ...
                        NSString * objName = [viewLayoutDict objectForKey:@"SVMXDEV__Source_Object_Name__c"];
...
//                        custLabel.accessibilityValue = @"{text_color: blue}";
                        /*Accessibility Changes*/
                        [custLabel setAccessibilityValue:kAccReferencevalue];
                    }
                    else
                    {              
                    }
    ...
    if (selectedSection == SHOW_LINES_ROW || selectedSection == SHOWALL_LINES)
	{
		...
		else //display the column values for the row
		{
            ...
            for (int j = 0; j < [appDelegate.view_layout_array count]; j++)
            {
                ...
                if ([objName isEqualToString:objectName_])
                {
                    ...
                    newProcessId = [viewLayoutDict objectForKey:@"SVMXDEV__ProcessID__c"];
                    break;
                }
            }
		...
		//	UIImage * switchImage = [UIImage imageNamed:@"SFM-Screen-Switch-Views-button_transparent.png"];
            ...
            UIImage * switchImage = [UIImage imageNamed:@"more.png"];
        ...
        else
        {
            ...
            if(info_dict != nil)
            {
                ...
                //    UILabel * lbl = [[[UILabel alloc]init] autorelease];
                    ...
                    if ([str isEqualToString:gSVMXDEV__Problem_Description__c])
                    {
                        value = [info_dict objectForKey:gSVMXDEV__Problem_Description__c];
                        lbl.backgroundColor = [UIColor clearColor];
                        lbl.text = value;
                        [background addSubview:lbl];
                    }
...
-(void)celltapRecognizer:(id)sender
{
    ...
    {       
        ...
        if ([tap.view isKindOfClass:[UITableViewCell class]] || [tableViewCell isKindOfClass:[UITableViewCell class]] )
        {
            ...
            if (detailArray != nil && [detailArray count] > 0)  //Fix for the crash - 10079
            {
...
//    attachmentview = nil;
    ...
    [sourceUpdateConfiguration release];
    sourceUpdateConfiguration = nil;
...
//Defect 10775:Shubha To resign all other responder
    ...
    for ( UIView * background in allSubViews) {
        ...
        for (UIView *eachView in backgroundSubViews) {
            if ([eachView isKindOfClass:[UITextField class]] || [eachView isKindOfClass:[UITextView class]]) {
                ...
                if(eachView.isFirstResponder)
                [eachView performSelectorOnMainThread:@selector(resignFirstResponder) withObject:nil waitUntilDone:YES];
                
                NSLog(@"%@",eachView.window);
            }
...
-(id)getControl:(NSString *)controlType withRect:(CGRect)frame withData:(NSArray *)datasource withValue:(NSString *)value fieldType:(NSString *)fieldType labelValue:(NSString *)labelValue enabled:(BOOL)readOnly refObjName:(NSString *)refObjName referenceView:(UIView *)POView indexPath:(NSIndexPath *)indexPath required:(BOOL)required valueKeyValue:(NSString *)valueKeyValue lookUpSearchId:(NSString *)searchid overrideRelatedLookup:(NSNumber *)Override_Related_Lookup fieldLookupContext:(NSString *)Field_Lookup_Context fieldLookupQuery:(NSString *)Field_Lookup_Query dependentPicklistControllerName:(NSString *)dependPick_controllerName picklistValidFor:(NSMutableArray *)validFor picklistIsdependent:(BOOL)isdependentPicklist objectAPIName:(NSString *)object_api_name forSourceObject:(NSString *)lookupContextSourceObject percisionValue:(NSDictionary *)numberValidationDict
{
    if([controlType isEqualToString:@"picklist"])
    {
        ...
        else
        {
            ...
            SMLog(kLogLevelVerbose,@"PickList Value = %@", botSpinner.text);
    ...
    if([controlType isEqualToString:@"percent"])
    {
        ...
        PercentType.precision = [[numberValidationDict objectForKey:MPRECISION] integerValue]; //10346
    ...
    if([controlType isEqualToString:@"currency"])
    {
        ...
        else 
        {
            ...
            currency.precision = [[numberValidationDict objectForKey:MPRECISION] integerValue]; //10346
    ...
    if([controlType isEqualToString:@"double"])
    {
        ...
        else
		{
            ...
            doubleType.precision = [[numberValidationDict objectForKey:MPRECISION] integerValue]; //009746
            doubleType.length = [[numberValidationDict objectForKey:@"lenght"] integerValue]; //009746
...
- (void) tableView:(UITableView *)_tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        NSString *layoutId = [detail objectForKey:gDETAILS_LAYOUT_ID];
        //code for delete the records
        
        NSString * deleted_record = @"";
        //Crash fix
        if ([detail_record_id count] > 0 && _indexPath.row <= [detail_record_id count])
        {
...
//            [detail_values removeObjectAtIndex:_indexPath.row-1];
        ...
        if ([detail_record_id count] > 0)
        {
            ...
            [self removeSourceRecordAsDetalRecordDeletedAtIndex:_indexPath.row-1 forProcessLayoutId:layoutId];
        }
        ...
        //  Removed :  [self hideExpandedChildViews];
        //  hideExpandedChildViews will reload the tableview internally. But since we are using [tableView beginUpdates] According to apple documentation we should not call reloadData within the group. if we call reloadData method within the group, we will need to perform any animations ourself. Since we are not allowing any row to be deleted when the table view disclosure is open, removing this method will have no impact over here.
...
- (BOOL)shouldInsertRecordIntoTrailorTableInCaseDataModified:(BOOL)dataModified
{
            ...
            if([targetCall isEqualToString:save])
            {
                ...
                for (int i=0;i<[header_sections count];i++)
                {
                    ...
                    for (int j=0;j<[section_fields count];j++)
                    {
                        ...
                        else if ([dataType isEqualToString:@"currency"] || [dataType isEqualToString:@"percent"] || [dataType isEqualToString:@"double"]) //10346
                        {
                            NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:field_api objectName:headerObjName validationField:MSCALE];
                            ...
                            if ([value length] > 0 && (![value isEqualToString:@""]) && !iSNaN)
                            {
                                ...
                                if ([formattedNumber length] > 0)
                                {
                                    ...
                                    [section_field setValue:value forKey:gFIELD_VALUE_KEY];
                                    [SFM_header_fields setObject:value forKey:field_api];
                                }
                            }
                            else if (iSNaN)
                            {
                                [section_field setValue:@"" forKey:gFIELD_VALUE_VALUE];
                                [section_field setValue:@"" forKey:gFIELD_VALUE_KEY];
                                [SFM_header_fields setObject:@"" forKey:field_api];
                            }
                    ...
                    for (int j=0;j<[details_values count];j++) //parts for instance
                    {
                        ...
                        for (int k = 0; k < [child_record_fields count];k++) //fields of one part for instance
                        {
                            ...
                            for(int l = 0 ;l < [fields_array count]; l++)
                            {
                                ...
                                if ([detail_api_name isEqualToString:@"SVMXDEV__Email__c"] )  //Shrinivas Fix for Email Validation 03/04/2012
                                {
                                    BOOL result;
                                    NSString * emailRegEx = @"[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
                                    NSPredicate * emailTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", emailRegEx];
                ...
                NSString * header_record_local_id = [[AppDelegate GetUUID] retain];
                    ...
                    for (int i=0;i<[details count];i++) //parts, labor, expense for instance
                    {
                       ...
                       // NSArray * fields_array = [detail  objectForKey:gDETAILS_FIELDS_ARRAY];
                        ...
                        for (int j=0;j<[details_values count];j++) //parts for instance
                        {
                            ...
                            // Mem_leak_fix - Vipindas 9493 Jan 18
                            //get the GUID
                            NSString * detail_record_local_id = [[AppDelegate GetUUID] retain];
                            ...
                            [detail_record_local_id release];
                        }
                    }
                ...
                [header_record_local_id release];
//                [SFM_header_fields release]; // Damodar - Win14 - MemMgt - revoked to fix crash
        ...
        if([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"EDIT"] || [[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"SOURCETOTARGETONLYCHILDROWS"])
        {
            ...
            /**
             *  Feature : Field Merge
             *  Skipping request call if there is no change in the record(s).
             *  This will apply only if field merge is enabled on the application. Default value is "YES".
             *
             */
            
            int  numberOfRecordChanged = 0;
            BOOL shouldMakeRequest = YES;
            BOOL foundNonFieldMergeChange = NO;
            ...
            if([targetCall isEqualToString:save] || [targetCall isEqualToString:quick_save])
            {
                ...
                for (int i=0;i<[header_sections count];i++)
                {
                    ...
                    for (int j=0;j<[section_fields count];j++)
                    {
                        ...
                        {
                            NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:[section_field objectForKey:gFIELD_API_NAME] objectName:headerObjName validationField:MSCALE];
		    ...
		    [self requireFieldWarning];
                ...
                [appDelegate.databaseInterface replaceCurrentRecordOrheaderLiteral:all_header_fields
                                                                    headerRecordId:@""
                                                                  headerObjectName:@""
                                                                   currentRecordId:currentRecordId
                                                                 currentObjectName:headerObjName];
                
                //FieldMerging -  heADER ON SAVE  all_header_fields

                //FieldMerging implementation
                PageModificationObserver *headerModificationObserver = [[PageModificationObserver alloc] initWithModifiedDataDictionary:all_header_fields
                                                                                                                               recordId:currentRecordId
                                                                                                                     andObjectOrApiName:headerObjName];
                
                
                BOOL success_flag = [appDelegate.databaseInterface  UpdateTableforId:currentRecordId
                                                                           forObject:headerObjName
                                                                                data:all_header_fields];
                
                
                
                [headerModificationObserver doDataComparison];
                
                if ((success_flag) && ([headerModificationObserver hasFoundNonAdvancedFieldMergeChanges]))
                {
                    foundNonFieldMergeChange = YES;
                }
                
                [headerModificationObserver autorelease];
                
                
                 NSLog(@" FieldMerging ON SAVE all_header_fields : %@", all_header_fields);
                ...
                {
                    //SUCCESSIVE_SYNC
                    NSMutableDictionary * successiveSyncDict = [self getSuccessiveSyncDictionaryFor:MASTER
                                                                                      visibleFields:header_sections
                                                                                       hiddenFields:object_mapping_dict
                                                                                       detailFields:nil];
                    
                    [appDelegate.databaseInterface replaceCurrentRecordOrheaderLiteral:successiveSyncDict
                                                                        headerRecordId:@""
                                                                      headerObjectName:@""
                                                                       currentRecordId:currentRecordId
                                                                     currentObjectName:headerObjName];

                    [appDelegate.databaseInterface  successiveSyncEntryForLocalId:currentRecordId
                                                                             sfId:id_value
                                                                        operation:UPDATE
                                                                       recordType:MASTER
                                                                          objName:headerObjName
                                                                    parentObjName:@""
                                                                    parentLocalId:@""
                                                                    headerLocalId:appDelegate.sfmPageController.recordId
                                                                         syncType:AGRESSIVESYNC
                                                                         dataDict:successiveSyncDict
                                                                         syncFlag:@"false"
                                                                     valuemapping: YES ];
                    
					if ([syncType isEqualToString:AGRESSIVESYNC])
					{
						BOOL does_exists = [appDelegate.databaseInterface DoesTrailerContainTheRecord:currentRecordId
                                                                                       operation_type:UPDATE
                                                                                          object_name:headerObjName];
                        if(!does_exists || does_exists) //11698
						{
                            // FM_TRLR

                            if ([self shouldInsertRecordIntoTrailorTableInCaseDataModified:[headerModificationObserver hasDataModified]])
                            {
                                [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:currentRecordId
                                                                                              SF_id:id_value
                                                                                        record_type:MASTER
                                                                                          operation:UPDATE
                                                                                        object_name:headerObjName
                                                                                    ...
                                                                                    parent_loacl_id:@""
                                                                                     webserviceName:webserviceName
                                                                                          className:className
                                                                                           synctype:syncType
                                                                                      headerLocalId:appDelegate.sfmPageController.recordId
                                                                                        requestData:customDataDictionary
                                                                                         finalEntry:NO];
                                [headerModificationObserver updateModifiedFieldsData];
                                ++numberOfRecordChanged;
					...
					else
					{
                        //RAdha
                        // FM_TRLR
                        //Commenting the below code since for "Override Sync" we have dependency that for ant line added, header details has to sent as refernce in the response
                    //    if ([self shouldInsertRecordIntoTrailorTableInCaseDataModified:[headerModificationObserver hasDataModified]]) //11698
                        {
                ...
                {
                    //11698
                    if (!isSalesForceRecord && [syncType isEqualToString:AGRESSIVESYNC])
                    {
                        BOOL does_exists = [appDelegate.databaseInterface DoesTrailerContainTheRecord:currentRecordId
                    ...
                    if([syncType isEqualToString:CUSTOMSYNC])
					{
						...
						[appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:currentRecordId
                                                                           ...
                                                                           parentObjectName:@""
                                                                            parent_loacl_id:@""
                                                                             webserviceName:webserviceName className:className
                                                                                   synctype:syncType
																			  headerLocalId:appDelegate.sfmPageController.recordId
                                                                                requestData:customDataDictionary
                                                                                 finalEntry:NO];
                ...
                if(success_flag)
                {
                    ...
                    for (int i=0;i<[details count];i++) //parts, labor, expense for instance
                    {
                        ...
                        NSMutableDictionary * process_components = [appDelegate.databaseInterface  getProcessComponentsForComponentType:TARGETCHILD process_id:processId layoutId:detail_layout_id objectName:detail_object_name];
                         ...
                         NSString *childProcessComponentId = [process_components objectForKey:PROCESS_COMPONENT_SFID];//4850
                        ...
                        for (int j=0;j<[details_values count];j++) //parts for instance
                        {
                            ...
                            if([line_record_id isEqualToString:@""])
                            {
                                ...
                                // Mem_leak_fix - Vipindas 9493 Jan 18
                                NSString * line_local_id = [[AppDelegate GetUUID] retain];
                                ...
                                if ((childCurrencyCode == nil) || ([childCurrencyCode length] == 0))
                                {
                                    BOOL isFieldExists = [appDelegate.databaseInterface ColumnExists:gCurrencyIsoCode
                                                                                           tableName:headerObjName];
                                    if (isFieldExists)
                                    {
                                        NSString *currencyISOCode = [appDelegate.databaseInterface getValueForField:gCurrencyIsoCode
                                                                                                         objectName:headerObjName
                                                                                                           recordId:appDelegate.sfmPageController.recordId];
                                        if ([currencyISOCode length]>0)
                                        {
                                            [detail_fields_dict setObject:currencyISOCode forKey:gCurrencyIsoCode];
                                        }
                                ...
                                if(data_inserted )
                                {
                                    ...
                                    //4850
                                    //Update source of above record here
                                    //4850
                                    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"SOURCETOTARGETONLYCHILDROWS"]) {
                                        
                                        //update the source of child record and insert it into data trailer as update
                                        [self applySourceUpdateConfigurationFor:appDelegate.sfmPageController.processId withComponentId:childProcessComponentId componentType:TARGETCHILD andRecord:detail_fields_dict andIndex:j];
                                    ...
                                    if(isSalesForceRecord)
                                    {
                                       ...
                                       // NSString * parent_SFId = [NSString stringWithFormat:@"SFID%@", id_value];
                                        ...
                                        [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:line_local_id
                                                                                                      SF_id:@""
                                                                                                record_type:DETAIL
                                                                                                  operation:INSERT
                                                                                                object_name:detail_object_name
                                                                                                  sync_flag:@"false"
                                                                                           parentObjectName:headerObjName
                                                                                            parent_loacl_id:currentRecordId
                                                                                             webserviceName:webserviceName
                                                                                                  className:className
                                                                                                   synctype:syncType
                                                                                              headerLocalId:appDelegate.sfmPageController.recordId
...
//                                }
                                ...
                                if(check_for_new_id)
                                {
                                    ...
                                    if(data_inserted )
                                    {
                                        ...
                                        if(isSalesForceRecord)
                                        {
                                            foundNonFieldMergeChange =  YES;
                                            //update  String SF_ID 
                                            [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:line_record_id
                                                                                                          SF_id:@""
                                                                                                    record_type:DETAIL
                                                                                                      operation:INSERT
                                                                                                    object_name:detail_object_name
                                ...
                                else
                                {
                                    ...
                                    NSLog(@" FieldMerging ON SAVE sfm_detail_field_keyValue : %@", sfm_detail_field_keyValue);

                                    PageModificationObserver *detailModificationObserver = nil;
                                    
                                    detailModificationObserver = [[PageModificationObserver alloc] initWithModifiedDataDictionary:sfm_detail_field_keyValue
                                                                                                                         recordId:line_record_id
                                                                                                               andObjectOrApiName:detail_object_name];
                                    
                                    BOOL detail_success_flag = [appDelegate.databaseInterface  UpdateTableforId:line_record_id
                                                                                                      forObject:detail_object_name
                                                                                                           data:sfm_detail_field_keyValue];
                                    
                                    [detailModificationObserver doDataComparison];
                                    
                                    if ((success_flag) && ([detailModificationObserver hasFoundNonAdvancedFieldMergeChanges]))
                                    {
                                        foundNonFieldMergeChange = YES;
                                    }
                                    
                                    [detailModificationObserver autorelease];
											...
											if(!does_exists)
											{
												...
												if(child_isSalesForceRecord)
												{
													if(isSalesForceRecord)
													{
                                                        ...
                                                        {
                                                            // FM_TRLR
                                                            if ([self shouldInsertRecordIntoTrailorTableInCaseDataModified:[detailModificationObserver hasDataModified]])
                                                            {
                                                                                                               ...
                                                                                                               parentObjectName:headerObjName
                                                                                                                parent_loacl_id:currentRecordId
                                                                                                                 webserviceName:webserviceName
                                                                                                                  ...
                                                                                                                  headerLocalId:appDelegate.sfmPageController.recordId
                                                                                                                    requestData:customDataDictionary
                                                                                                                     finalEntry:NO];//idvalue
                                                                
                                                                [detailModificationObserver updateModifiedFieldsData];
                                                                ++numberOfRecordChanged;
												...
												else
												{
                                                    ...
                                                    //11698
                                                    NSString * sfid = [appDelegate.databaseInterface getSfid_For_LocalId_From_Object_table:detail_object_name local_id:line_record_id];
                                                    ...
                                                    //11698
                                                    else if (([sfid length] == 0 || [sfid isEqualToString:@""]) && does_exists)
                                                    {
                                                        ++numberOfRecordChanged;
                                                    }
                                                    else
                        ...
                        // Lets make count of deletion of record as well.
                        if (detail_deleted_records!= nil && ([detail_deleted_records count] > 0))
                        {
                            foundNonFieldMergeChange = YES;
                        }
                     
                        [self UpdateAlldeletedRecordsIntoSFTrailerTable:detail_deleted_records
                                                            object_name:detail_object_name
                                                         webserviceName:webserviceName
                                                              className:className
                                                               synctype:syncType
                  ...
                  //  [appDelegate.attachmentDataBase saveAttachmentRecords];
                    ...
                    if ([appDelegate isFieldMergeEnabled])
                    {
                        if (numberOfRecordChanged < 1)
                        {
                            shouldMakeRequest = NO;
                        }
                        else
                        {
                            shouldMakeRequest = YES;
                        }
                    }
                    else
                    {
                    ...
                    // Vipin-impl
                    if (shouldMakeRequest || foundNonFieldMergeChange)
                    {
                        [appDelegate callDataSync];
                    }
                }
            ...
            if([targetCall isEqualToString:save] && ![self isInvokedFromChildSfm:buttonDict])
            {
               ...
               //9681 24/1/2014
                [self navigateToViewProcessForRecord:appDelegate.sfmPageController.recordId objectName:headerObjName];
            }
        ...
        if([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"SOURCETOTARGET"])
        {
            ...
            if([targetCall isEqualToString:save])
            {
                ...
                NSMutableDictionary * process_components = [appDelegate.databaseInterface getProcessComponentsForComponentType:TARGET process_id:processId layoutId:layout_id objectName:headerObjName];
                 NSString *headerProcessCompoId = [process_components objectForKey:PROCESS_COMPONENT_SFID];////4850
                //[appDelegate.databaseInterface  getValueMappingForlayoutId:layout_id process_id:processId objectName:headerObjName];
               ...
               // NSString * source_parent_api_name = [process_components objectForKey:]
                ...
                {
                    //4850
                    //Update the source here in source to target process
                    [self applySourceUpdateConfigurationFor:processId withComponentId:headerProcessCompoId componentType:TARGET andRecord:header_fields_dict andIndex:0];
                   ...
                   // NSString * header_record_local_id = [appDelegate.databaseInterface getTheRecordIdOfnewlyInsertedRecord:headerObjName];
                    ...
                    for (int i=0;i<[details count];i++) //parts, labor, expense for instance
                    {
                        ...
                        ////4850
                        //get the source update information here
                        NSString *childProcessComponentId = [processComponents objectForKey:PROCESS_COMPONENT_SFID];
...
//                [customDataDictionary release]; // Damodar - Win14 - MemMgt - revoked to fix crash
    ...
    else
    {
        ...
        if (![appDelegate isInternetConnectionAvailable])
        {
            ...
            appDelegate.shouldShowConnectivityStatus = TRUE;
            [appDelegate displayNoInternetAvailable];
            [self enableSFMUI]; //Defect Fix 8740
            return;
        } 
        ...
        if(performSync && !(appDelegate.connection_error))
        {
			...
			//[[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_TIMER_INVALIDATE object:appDelegate.datasync_timer];//PB-TIME-FIX
			//RADHA Defect Fix 5542
			appDelegate.shouldScheduleTimer = YES;
            [appDelegate callDataSync];
            while (CFRunLoopRunInMode( kCFRunLoopDefaultMode, kRunLoopTimeInterval, FALSE))
...
//Added for Successive Sync Implementation
-(NSMutableDictionary *)getSuccessiveSyncDictionaryFor:(NSString *)recordType visibleFields:(NSArray *)visibleFields  hiddenFields:(NSDictionary *)hiddenFieldDict detailFields:(NSArray *)detailFields
{
    NSMutableDictionary * successiveSyncDict = [[NSMutableDictionary alloc] initWithCapacity:0];
    
    if([recordType isEqualToString:MASTER])
    {
        for(int i=0; i <[visibleFields count] ;i++)
        {
            NSDictionary * section_info = [visibleFields objectAtIndex:i];
            NSMutableArray * sectionFileds= [section_info objectForKey:@"section_Fields"];
            ...
            for(int j= 0;j<[sectionFileds count]; j++)
            {
                ...
                [successiveSyncDict setObject:field_value forKey:filed_api_name]; //SUCCESSIVE_SYNC
            }
        }
    }
   else  if([recordType isEqualToString:DETAIL])
   {
       for (int k = 0; k < [visibleFields count];k++) //fields of one part for instance
       {
           NSDictionary * Visiblefield = [visibleFields objectAtIndex:k];
           NSString * detail_api_name = [Visiblefield objectForKey:gVALUE_FIELD_API_NAME];
           for(int l = 0 ;l < [detailFields count]; l++)
           {
               NSDictionary *fieldDict = [detailFields  objectAtIndex:l];
               NSString * api_name = [fieldDict objectForKey:gFIELD_API_NAME];
               if([api_name isEqualToString:detail_api_name])
               {
                   NSString * detailValue = [Visiblefield objectForKey:gVALUE_FIELD_VALUE_KEY];
                   [successiveSyncDict setObject:(detailValue != nil)?detailValue:@"" forKey:detail_api_name];
               }
           }
       }
    ...
    NSArray * object_mapping_keys = [ hiddenFieldDict allKeys];
    NSArray * all_keys = [successiveSyncDict allKeys];
    for(NSString * mapping_key in object_mapping_keys)
    {
        NSString * mapping_value = [ hiddenFieldDict objectForKey:mapping_key];
        BOOL mapping_flag = FALSE;
        for(NSString * key in all_keys)
        {
            if([key isEqualToString:mapping_key])
            {
                NSString * value = [successiveSyncDict objectForKey:mapping_key];
                NSString * temp_value = [value stringByReplacingOccurrencesOfString:@" " withString:@""];
                if([temp_value length] == 0 || temp_value == nil )
                {
                    [successiveSyncDict setObject:mapping_value forKey:mapping_key]; //SUCCESSIVE_SYNC
                }
                mapping_flag = TRUE;
            }
        }
    ...
    return [successiveSyncDict autorelease];
}
                                          ...
                                          childObjects:(NSArray *)childObjectsArray
{
    ...
    //009652
    NSArray *processIDArray = [appDelegate.dataBase getAllRecordsFromTable:SFPROCESS
                                                                 forColumns:processColumn
                                                             filterCriteria:filterCriteria
                                                                      limit:@"1"];
    if([processIDArray count] == 0)
        return nil;
    NSString *processID = [[processIDArray objectAtIndex:0] objectForKey:@"sfID"];
    ...
    for(NSDictionary *ruleDict in headerRulesArray)
    {
        ...
        if(ruleInfo)
        {
            ...
            {
                NSString *parameterType = [rule objectForKey:@"SVMXDEV__Parameter_Type__c"];
                NSString *fieldName = [rule objectForKey:@"SVMXDEV__Field_Name__c"];
                if(![headerFields containsObject:fieldName])
                    [headerFields addObject:fieldName];
                if([parameterType isEqualToString:@"Field Value"])
                {
                    NSString *operandFieldName = [rule objectForKey:@"SVMXDEV__Operand__c"];
        ...
        if([valuesArray count])
        {
            ...
            for(NSString *fieldName in fields)
            {
                ...
                if(fieldValue)
                {
                    ...
                    {
                        //8945
                        NSString *dateTimeFieldValue = [iOSInterfaceObject localTimeFromGMT:(NSString *)fieldValue];
    ...
    for(NSDictionary *sectionDict in headerArray)
    {
        ...
        for(NSDictionary *section in sectionFieldsArray)
        {
            ...
            if([fieldsForHeader containsObject:sectionFieldName])
            {
                ...
                {
                    //8945
                    NSString *dateTimeFieldValue = [iOSInterfaceObject localTimeFromGMT:(NSString *)sectionFieldValue];
                    if(dateTimeFieldValue && ( [dateTimeFieldValue length] > 16))
...
//                if(sectionFieldValue != nil) // Damodar - Win14 - MemMgt - revoked to fix crash
    ...
    for(NSDictionary *childDict in childObjectsArray)
    {
        ...
        if(![fieldApiNameArray count])
        {
            ...
            //object_api_name ='SVMXDEV__RMA_Shipment_Order__c' and reference_to= 'SVMXDEV__Service_Order__c'
                ...
                for(NSArray *detailFieldsArray in detailsArray)
                {
                    ...
                    for(NSDictionary *detail in detailFieldsArray)
                    {
                        ...
                        if([fieldsForChild containsObject:detailFieldName])
                        {
                            ...
                            {
                                NSString *dateTimeFieldValue = [iOSInterfaceObject localTimeFromGMT:(NSString *)detailFieldValue];
                                if(dateTimeFieldValue && ( [dateTimeFieldValue length] > 16))
                                    ...
                                    withData:(NSString *)dataToValidateString
{
    ...
    NSString *appConfig = [NSString stringWithFormat:@"var appConfig = { \"title\" : \"Mobile business rules application\", \"version\" : \"1.0.0\",\
                           ...
                           ],\"app-config\" : {\"application-id\" : \"application\",\"enable-cache\" : true,\"enable-log\" : true,\"org-name-space\" : \"SVMXDEV\"},\"platform-config\" : {}};"];
...
- (NSArray *) getBusinessRulesDict:(NSArray *)businessRulesArray
{
    ...
    for(NSDictionary *dict in businessRulesArray)
    {
        ...
        for(NSDictionary *detailsDict in bizRuleDetailsArray)
        {
            ...
            [details setObject:operand forKey:@"SVMXDEV__Operand__c"];
            [details setObject:expressionID forKey:@"SVMXDEV__Expression_Rule__c"];
            [details setObject:parameterType forKey:@"SVMXDEV__Parameter_Type__c"];
            [details setObject:operator forKey:@"SVMXDEV__Operator__c"];
            [details setObject:fieldName forKey:@"SVMXDEV__Field_Name__c"];
        ...
        [ruleDict setObject:sourceObjectName forKey:@"SVMXDEV__Source_Object_Name__c"];
        [ruleDict setObject:advancedExpression forKey:@"SVMXDEV__Advance_Expression__c"];
        [ruleDict setObject:messageType forKey:@"SVMXDEV__Message_Type__c"];
...
//9681 24/1/2014
-(void)navigateToViewProcessForRecord:(NSString *)recordId objectName:(NSString *)objectname
{
    
    NSString * lastViewdProcessId =  [appDelegate.switchViewLayouts objectForKey:objectname];
    lastViewdProcessId = (lastViewdProcessId != nil)?lastViewdProcessId:@"";
    
    processInfo * pinfo =  [appDelegate getViewProcessForObject:objectname record_id:recordId processId:lastViewdProcessId isswitchProcess:FALSE];
    BOOL process_exist = pinfo.process_exists;
    NSString * processId = pinfo.process_id;
    
    if(process_exist)
    {
        [self initAllrequriredDetailsForProcessId:processId recordId:recordId object_name:objectname];
        //check For view process
        [self fillSFMdictForOfflineforProcess:processId forRecord:recordId ];
        [self didReceivePageLayoutOffline];
    }
    else
    {
...
-(void)SaveRecordIntoPlist:(NSString *)record_id objectName:(NSString *) object_name;
{
   ...
   /* NSDateFormatter * frm = [[[NSDateFormatter alloc] init] autorelease];
    ...
    NSString * lastViewdProcessId =  [appDelegate.switchViewLayouts objectForKey:object_name];
    lastViewdProcessId = (lastViewdProcessId != nil)?lastViewdProcessId:@"";
    
    processInfo * pinfo =  [appDelegate getViewProcessForObject:object_name record_id:record_id processId:lastViewdProcessId isswitchProcess:FALSE];
    BOOL process_exist = pinfo.process_exists;
    process_id = pinfo.process_id;
    
    
    [created_object_info setObject:process_id forKey:gPROCESS_ID];
    [created_object_info setObject:date forKey:gDATE_TODAY]; //10312
...
{
    NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV__Product__c FROM SVMXDEV__Service_Order__c WHERE local_id = '%@'", recordId];
...
-(void)UpdateAlldeletedRecordsIntoSFTrailerTable:(NSArray *)deleted_record_array   object_name:(NSString *)object_name webserviceName:(NSString *)webservice_name className:(NSString *)class_name synctype:(NSString *)sync_type headerLocalId:(NSString *)header_localId requestData:(NSMutableDictionary *)request_data
{
    ...
    for(int i = 0; i < [deleted_record_array count]; i++)
    {
        ...
        {
            //9757
            if([deleted_record_id length])
            {
                [appDelegate.databaseInterface DeleterecordFromTable:object_name Forlocal_id:deleted_record_id];
                [appDelegate.databaseInterface DeleterecordFromTable:SFDATATRAILER Forlocal_id:deleted_record_id];
            }
        }
    }
...
-(void)doubleTapOnCusLabel:(id)cusLabel
{
    if ([cusLabel isKindOfClass:[UILabel  class]])    
    {
       ...
       /* NSString * newProcessId = @"";
        ...
        NSString * lastViewdProcessId =  [appDelegate.switchViewLayouts objectForKey:reffered_to_table_name];
        lastViewdProcessId = (lastViewdProcessId != nil)?lastViewdProcessId:@"";
        
        processInfo * pinfo =  [appDelegate getViewProcessForObject:reffered_to_table_name record_id:record_id processId:lastViewdProcessId isswitchProcess:FALSE];
        NSString * newProcessId = pinfo.process_id;
...
//    view.accessibilityIdentifier = @"DetailViewError";
    /*Accessibility Changes*/
    [view setAccessibilityIdentifier:kAccValidationError];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex;
{
    ...
    {
        //krishna 009752 : isEqual was accessing the dealloced requiredFields.
        if((requiredFields != nil) && [requiredFields isEqual:alertView])
        {
            if (buttonIndex == 0)
            {
...
- (void)updateWorkOrderWithPrice:(NSDictionary *)updatedWo  {
    ...
    NSString *tableNameTwo = @"SVMXDEV__Service_Order_Line__c";
...
- (NSDictionary *)getServiceBooleanTypeDictionary:(NSString *)tableName {
    ...
    if ([tableName isEqualToString:@"SVMXDEV__Service_Order__c"]) {
        [someDictionary setObject:@"invalid_address_c" forKey:@"invalid_address_c"];
        [someDictionary setObject:@"SVMXDEV_invoice_created_c" forKey:@"SVMXDEV_invoice_created_c"];
        [someDictionary setObject:@"auto_calc_drive_time_c" forKey:@"auto_calc_drive_time_c"];
        [someDictionary setObject:@"SVMXDEV_clock_paused_forever_c" forKey:@"SVMXDEV_clock_paused_forever_c"];
        [someDictionary setObject:@"SVMXDEV_optimax_error_occured_c" forKey:@"SVMXDEV_optimax_error_occured_c"];
        [someDictionary setObject:@"SVMXDEV_is_pm_work_order_c" forKey:@"SVMXDEV_is_pm_work_order_c"];
        [someDictionary setObject:@"SVMXDEV_is_sla_calculated_c" forKey:@"SVMXDEV_is_sla_calculated_c"];
        [someDictionary setObject:@"SVMXDEV_apply_business_hours_for_optimax_c" forKey:@"SVMXDEV_apply_business_hours_for_optimax_c"];
        [someDictionary setObject:@"regulatory_4_c" forKey:@"regulatory_4_c"];
        [someDictionary setObject:@"SVMXDEV_locked_by_dc_c" forKey:@"SVMXDEV_locked_by_dc_c"];
        [someDictionary setObject:@"SVMXDEV_sla_clock_paused_c" forKey:@"SVMXDEV_sla_clock_paused_c"];
        [someDictionary setObject:@"geoviatrigger_c" forKey:@"geoviatrigger_c"];
        [someDictionary setObject:@"SVMXDEV_is_service_covered_c" forKey:@"SVMXDEV_is_service_covered_c"];
        [someDictionary setObject:@"SVMXDEV_pm_tasks_created_c" forKey:@"SVMXDEV_pm_tasks_created_c"];
        [someDictionary setObject:@"SVMXDEV_customer_down_c" forKey:@"SVMXDEV_customer_down_c"];
        [someDictionary setObject:@"SVMXDEV_perform_auto_entitlement_c" forKey:@"SVMXDEV_perform_auto_entitlement_c"];
        [someDictionary setObject:@"cust_bool_c" forKey:@"cust_bool_c"];
        [someDictionary setObject:@"SVMXDEV_is_entitlement_performed_c" forKey:@"SVMXDEV_is_entitlement_performed_c"];
        [someDictionary setObject:@"SVMXDEV_isPartnerRecord_c" forKey:@"SVMXDEV_isPartnerRecord_c"];
        [someDictionary setObject:@"parts_required_c" forKey:@"parts_required_c"];
        
    }
    else {
        [someDictionary setObject:@"SVMXDEV_service_order_line_c" forKey:@"SVMXDEV_service_order_line_c"];
        [someDictionary setObject:@"SVMXDEV_use_price_fromPricebook_c" forKey:@"SVMXDEV_use_price_fromPricebook_c"];
        [someDictionary setObject:@"isDeleted" forKey:@"isDeleted"];
        [someDictionary setObject:@"SVMXDEV_posted_to_inventory_c" forKey:@"SVMXDEV_posted_to_inventory_c"];
        [someDictionary setObject:@"SVMXDEV_is_billable_c" forKey:@"SVMXDEV_is_billable_c"];
        [someDictionary setObject:@"SVMXDEV_include_in_quote_c" forKey:@"SVMXDEV_include_in_quote_c"];
        [someDictionary setObject:@"SVMXDEV_select_c" forKey:@"SVMXDEV_select_c"];
    }
...
- (BOOL)getPriceDataFromOnline:(NSString *)someIdentifier {
    ...
    if ([appDelegate isInternetConnectionAvailable]) {
        ...
        //[self disableAllRunningNetworkOperations];//PB-TIME-FIX
        ...
        /*  Enable all timers  */
        //[self enableAllNetworkOpertaions];//PB-TIME-FIX
...
- (BOOL)shouldShowBillableAmountInServiceReport {
    return [appDelegate.calDataBase checkIfBillablePriceExistForWorkOrderId:appDelegate.sfmPageController.recordId andFieldName:@"SVMXDEV__Billable_Line_Price__c"];
}

- (BOOL)shouldShowBillableQuantityInServiceReport {
    
    
    return [appDelegate.calDataBase checkIfBillablePriceExistForWorkOrderId:appDelegate.sfmPageController.recordId andFieldName:@"SVMXDEV__Billable_Quantity__c"];
}
#pragma mark -
...
//D-00003728
-(void) displayAttachmentSharingView:(NSArray *)dataSource viewName:(NSString *)view sender:(UIButton *)button
{
    UIActivityViewController * sharingView = [[[UIActivityViewController alloc] initWithActivityItems:dataSource applicationActivities:nil] autorelease];
    ...
    if ([Utility notIOS7]) {
        ...
        [(SFMPageController *)delegate presentViewController:sharingView animated:YES completion:nil];
    }
    ...
    [sharingView setCompletionHandler:^(NSString *activityType, BOOL completed) {
        ...
        if (activityType == UIActivityTypeMail)
        {
            ...
            {
                UIAlertView *mailErrorAlert = [[UIAlertView alloc]initWithTitle:[appDelegate.wsInterface.tagsDictionary objectForKey:alert_application_error] message:[appDelegate.wsInterface.tagsDictionary objectForKeyedSubscript:CONFIGUREMAIL] delegate:nil cancelButtonTitle:[appDelegate.wsInterface.tagsDictionary objectForKey:ALERT_ERROR_OK] otherButtonTitles:nil];
                [mailErrorAlert show];
                [mailErrorAlert release];
                // device is configured to send mail
            }
        }
        [self reloadAttachmentView:view];
        if (popover != nil)
            [popover release];
    }];
...
//D-00003728
- (void) reloadAttachmentView:(NSString *)view
{
    if ([view isEqualToString:@"CollectionView"])
    {
        [self.imageCollectionView refreshSelectitems]; //11429
        [self.imageCollectionView.CollectionView reloadData];
        [self.imageCollectionView deleteAttachmetFiles]; //Defect 11338
    }
    else
    {
        [self.documentView refreshSelectitems]; //11429
        [self.documentView.mainTableView reloadData];
        [self.documentView deleteAttachmetFiles]; //Defect 11338
    }
}
...
-(void)ButtonClick:(int)selectedIndex{
    ...
    else
    {
        ...
        if (status != ALAuthorizationStatusAuthorized) {
            ...
            if (selectedIndex == CAPTURE_IMAGES)
                [self startCameraControllerFromViewControllerisImageFromCamera:NO isVideoMode:NO];
...
- (BOOL) startCameraControllerFromViewControllerisImageFromCamera:(BOOL)isCameraCapture isVideoMode:(BOOL)isVideoMode {
    ...
    if(isCameraCapture) {
        ...
        cameraViewController.videoMaximumDuration=120;
...
#pragma mark -
#pragma mark Source update - 4850
- (void)addRecords:(NSString *)recordId ofObjectName:(NSString *)objectName forComponentId:(NSString *)processCompId ToDictionary:(NSMutableDictionary*)sourceToTargetDetailDictionary {
    ...
    if (processCompId != nil) {
        
        NSMutableArray *sourceArray = [sourceToTargetDetailDictionary objectForKey:processCompId];
        if (sourceArray == nil ) {
            NSMutableArray *tempArray = [[NSMutableArray alloc] init];
            [sourceToTargetDetailDictionary setObject:tempArray forKey:processCompId];
            [tempArray release];
            sourceArray = [sourceToTargetDetailDictionary objectForKey:processCompId];
        }
        
        if (recordId != nil) {
            [sourceArray addObject:recordId];
        }
    }
}
- (void)removeSourceRecordAsDetalRecordDeletedAtIndex:(NSInteger)index forProcessLayoutId:(NSString *)processLayoutId {
    ...
    NSString *processId = appDelegate.sfmPageController.processId;
    NSString *processComponentId = [appDelegate.dataBase getSFIDFromProcessId:processId andLayoutId:processLayoutId];
    if (processComponentId == nil) {
        return;
    }
    
    @synchronized(self) {
        NSMutableDictionary *sourceDictionary = [appDelegate.SFMPage objectForKey:SOURCE_INFO_OF_RECORDS];
        if ([sourceDictionary count] > 0) {
            NSMutableDictionary *procCompIdAsKeyRecIdValueDict = [sourceDictionary objectForKey:DETAIL_OBJECTS_SOURCE];
            NSMutableArray *recordArray = [procCompIdAsKeyRecIdValueDict objectForKey:processComponentId];
            if ([recordArray count] > index && index >=0 ) {
...
- (void)applySourceUpdateConfigurationFor:(NSString *)processUniqueId
                          withComponentId:(NSString *)componentId
                            componentType:(NSString *)type
                                andRecord:(NSDictionary *)record andIndex:(NSInteger)index {
    
    NSMutableDictionary *sourceDictionary = [appDelegate.SFMPage objectForKey:SOURCE_INFO_OF_RECORDS];
    NSString *processId =[appDelegate.databaseInterface getProcessSfIdForProcess_uniqueName:processUniqueId];
    
    if ([type isEqualToString:TARGET]) {
        
        NSString *targetId =  [appDelegate.dataBase getTargetNodeNameForProcessId:processUniqueId];
        NSArray *sourceUpdateConfigArr = [appDelegate.databaseInterface sourceUpdatesConfigurationForProcessId:processId andSettingId:targetId];
        
        NSString *sourceId = [sourceDictionary objectForKey:HEADER_SOURCE_ID_STRING];
        if ([sourceUpdateConfigArr count] > 0) {
            [self updateSourceRecord:sourceId withConfigurationArray:sourceUpdateConfigArr andTargetDictionary:record];
            SourceUpdateConfig *aConfig = [sourceUpdateConfigArr objectAtIndex:0];
            NSString * sfId = [appDelegate.databaseInterface checkforSalesForceIdForlocalId:aConfig.sourceObjectName local_id:sourceId];
            if (![Utility isStringEmpty:sfId]) {
                
                //Successive_Sync - as requested by Sahana , we are calling the below function
                NSMutableDictionary *mutableDict = [[NSMutableDictionary alloc] initWithDictionary:record];
                 [appDelegate.databaseInterface successiveSyncEntryForLocalId:sourceId sfId:sfId operation:UPDATE recordType:MASTER objName:aConfig.sourceObjectName parentObjName:@"" parentLocalId:@"" headerLocalId:@"" syncType:AGRESSIVESYNC dataDict:mutableDict syncFlag:@"false" valuemapping:YES];
                [mutableDict release];
                mutableDict = nil;
                
                NSMutableDictionary * customDataDictionary = [[NSMutableDictionary alloc] initWithCapacity:0];
                [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:sourceId SF_id:sfId record_type:MASTER operation:UPDATE object_name:aConfig.sourceObjectName sync_flag:@"false" parentObjectName:@"" parent_loacl_id:@"" webserviceName:webserviceName className:className synctype:AGRESSIVESYNC
                                                                      headerLocalId:@"" requestData:customDataDictionary finalEntry:NO];
                [customDataDictionary release];
                customDataDictionary = nil;
            }
            
        }
    }
    else{
        if (sourceUpdateConfiguration == nil) {
            NSMutableDictionary *someDictionary = [[NSMutableDictionary alloc] init];
            self.sourceUpdateConfiguration = someDictionary;
            [someDictionary release];
            someDictionary = nil;
        }
        
        //NSDictionary *srcTarDict = [appDelegate.dataBase sourceObjectNameForSettingId:componentId];
        
        NSArray *sourceConfigArray =  [sourceUpdateConfiguration objectForKey:componentId];
        if (sourceConfigArray == nil) {
            NSArray *sourceUpdateConfigArr = [appDelegate.databaseInterface sourceUpdatesConfigurationForProcessId:processId andSettingId:componentId];
            
            //            NSArray *sourceUpdateConfigArr = [appDelegate.databaseInterface sourceUpdatesConfigurationForProcessId:processId andDictionary:srcTarDict];
            
            if (sourceUpdateConfigArr != nil) {
                [sourceUpdateConfiguration setObject:sourceUpdateConfigArr forKey:componentId];
                sourceConfigArray = sourceUpdateConfigArr;
            }
        }
        NSString *sourceRecordId = nil;
        if ([sourceDictionary count] > 0) {
            NSMutableDictionary *procCompIdAsKeyRecIdValueDict = [sourceDictionary objectForKey:DETAIL_OBJECTS_SOURCE];
            NSMutableArray *recordArray = [procCompIdAsKeyRecIdValueDict objectForKey:componentId];
            if ([recordArray count] > index && index >=0 ) {
                sourceRecordId = [recordArray objectAtIndex:index];
            }
        }
        
        if ([sourceConfigArray count] > 0 && sourceRecordId != nil) {
            [self updateSourceRecord:sourceRecordId withConfigurationArray:sourceConfigArray andTargetDictionary:record];
            
            SourceUpdateConfig *aConfig = [sourceConfigArray objectAtIndex:0];
            NSString * childSfId = [appDelegate.databaseInterface checkforSalesForceIdForlocalId:aConfig.sourceObjectName local_id:sourceRecordId];
            
            if (![Utility isStringEmpty:childSfId]) {
                
                NSString *sourceHeaderId = [sourceDictionary objectForKey:HEADER_SOURCE_ID_STRING];
                NSString *sourceHeaderObjName = [sourceDictionary objectForKey:HEADER_SOURCE_OBJECT_NAME];
                
                //Successive_Sync - as requested by Sahana , we are calling the below function
                NSMutableDictionary *mutableDict = [[NSMutableDictionary alloc] initWithDictionary:record];
                [appDelegate.databaseInterface successiveSyncEntryForLocalId:sourceRecordId sfId:childSfId operation:UPDATE recordType:DETAIL objName:aConfig.sourceObjectName parentObjName:sourceHeaderObjName parentLocalId:sourceHeaderId headerLocalId:sourceHeaderId syncType:AGRESSIVESYNC dataDict:mutableDict syncFlag:@"false" valuemapping:YES];
                [mutableDict release];
                mutableDict = nil;
                
                NSMutableDictionary * customDataDictionary = [[NSMutableDictionary alloc] initWithCapacity:0];
                
                [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:sourceRecordId SF_id:childSfId record_type:DETAIL operation:UPDATE object_name:aConfig.sourceObjectName sync_flag:@"false" parentObjectName:sourceHeaderObjName parent_loacl_id:sourceHeaderId webserviceName:webserviceName className:className synctype:AGRESSIVESYNC headerLocalId:sourceHeaderId  requestData:customDataDictionary finalEntry:NO];
                [customDataDictionary release];
                customDataDictionary = nil;
            }
        }
...
- (void)updateSourceRecord:(NSString *)sourceRecordId withConfigurationArray:(NSArray *)suConfigurationArray andTargetDictionary:(NSDictionary *)targetDictionary {
    NSString *sourceObjectName = nil;
    NSMutableDictionary *keysToBeUpdated = [[NSMutableDictionary alloc] init];
    for (int counter = 0; counter < [suConfigurationArray count]; counter++) {
        SourceUpdateConfig *aConfig = [suConfigurationArray objectAtIndex:counter];
        if (aConfig.sourceFieldName != nil) {
            NSString *rhsValue = @"";
            if ([Utility isStringEmpty:aConfig.targetFieldname]) {
                rhsValue = aConfig.displayValue;
            }
            else{
                rhsValue = [targetDictionary objectForKey:aConfig.targetFieldname];
            }
            sourceObjectName = aConfig.sourceObjectName;
            NSString * dataType = [[appDelegate.databaseInterface getFieldDataType:aConfig.sourceObjectName filedName:aConfig.sourceFieldName] lowercaseString];
            if ([Utility isStringEmpty:dataType]) {
                continue;
            }
            
            if ([aConfig.actionType isEqualToString:@"Set"]) {
                
                NSString *literalvalue = [appDelegate.dataBase evaluateLiteral:rhsValue forControlType:dataType];
                if (![Utility isStringEmpty:literalvalue]) {
                    rhsValue = literalvalue;
                }
                //9636
                if ([dataType isEqualToString:@"date"]){
                    if ([rhsValue length] >= 10 ) {
                        rhsValue = [rhsValue substringToIndex:10];
                    }
                }
            }
            else if ([aConfig.actionType isEqualToString:@"Increase"]){
                
                NSString *someValue = [appDelegate.databaseInterface getValueForField:aConfig.sourceFieldName objectName:aConfig.sourceObjectName recordId:sourceRecordId];
                float newValue = [someValue floatValue]+[rhsValue floatValue];
                rhsValue = [NSString stringWithFormat:@"%f",newValue];
                
            }
            else if ([aConfig.actionType isEqualToString:@"Decrease"]){
                NSString *someValue = [appDelegate.databaseInterface getValueForField:aConfig.sourceFieldName objectName:aConfig.sourceObjectName recordId:sourceRecordId];
                float newValue = [someValue floatValue] - [rhsValue floatValue];
                rhsValue = [NSString stringWithFormat:@"%f",newValue];
            }
            ...
            if (rhsValue != nil) {
                [keysToBeUpdated setObject:rhsValue forKey:aConfig.sourceFieldName];
            }
        }
    }
    
    if ([keysToBeUpdated count] > 0) {
        [appDelegate.databaseInterface updateRecord:sourceRecordId ofObject:sourceObjectName andFieldDictionary:keysToBeUpdated];
    }
    [keysToBeUpdated release];
    keysToBeUpdated = nil;
