...
//        [details_api_keys release];
    ...
    if([process_type isEqualToString:@"VIEWRECORD"] )
    {
        ...
        NSString * temp = @"SPR14__Service_Order__c";
        ...
        if([object_api_name isEqualToString:work_order] && [processType isEqualToString:@"VIEWRECORD"])
        {
            ...
            NSString * account_id  = [additionalInfo objectForKey:@"SPR14__Company__c"];
            NSString * toplevelId  = [additionalInfo objectForKey:@"SPR14__Top_Level__c"];
            NSString * componentId = [additionalInfo objectForKey:@"SPR14__Component__c"];
            ...
            if(account_history == TRUE)
            {
                if(account_id != nil && [account_id length] != 0)
                {
                    //shrinivas
                    NSString * Id = @"";
                    NSString *query = [NSString stringWithFormat:@"Select Id From SPR14__Service_Order__c Where local_id = '%@'", appDelegate.sfmPageController.recordId];
                    sqlite3_stmt *stmt;
            ...
            if(product_history == TRUE)
            {
                ...
                {
                    productHistory = [appDelegate.databaseInterface getProductHistoryForanWorkOrder:Id filedName:@"SPR14__Top_Level__c" tableName:headerObjName fieldValue:toplevelId];
                }
                if(componentId != nil && [componentId length] != 0)
                {
                    productHistory = [appDelegate.databaseInterface getProductHistoryForanWorkOrder:Id filedName:@"SPR14__Component__c" tableName:headerObjName fieldValue:componentId];
                }
...
- (void) action:(id)sender
{
   ...
   // if ([headerButtons count] > 0)
    ...
    for(int i = 0 ; i< [headerButtons count];i++)   
    {
        ...
        NSString * button_type = ([[array objectAtIndex:0] objectForKey:@"SPR14__Event_Call_Type__c"]!= nil)?[[array objectAtIndex:0] objectForKey:@"SPR14__Event_Call_Type__c"]:@"";
    ...
    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"VIEWRECORD"]) 
    {
        ...
        if([heder_object_name isEqualToString:@"SPR14__Service_Order__c"])
        {
...
- (void) startSummaryDataFetch
{
    ...
    NSArray * _keys = [NSArray arrayWithObjects:SPR14__Activity_Type__c, SPR14__Actual_Price2__c, SPR14__Actual_Quantity2__c, SPR14__Billable_Quantity__c,SPR14__Billable_Line_Price__c,nil];
    // Calculate Labor
...
//    NSDictionary * headerData = [headerDataDict objectForKey:gHEADER_DATA];
  ...
  /*  troubleshooting.productId = [self getObjectNameFromHeaderData:headerData forKey:gSPR14__Product__c];
    troubleshooting.productName = [self getObjectNameFromHeaderData:headerData forKey:gSPR14__Product_Name__c];*/
...
- (void) showChatter
{
    ...
    NSDictionary * headerDataDict = [appDelegate.SFMPage objectForKey:gHEADER];
    NSDictionary * headerData = [headerDataDict objectForKey:gHEADER_DATA];
    _chatter.productId = [self getObjectNameFromHeaderData:headerData forKey:gSPR14__Product__c];
...
- (void) getParts:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		SMLog(kLogLevelVerbose,@"SPR14__Product__c = %@", [dict objectForKey:gSPR14__Product__c] );
        NSMutableDictionary *Part = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
        
        ZKSObject * obj2 = [[obj fields] objectForKey:gSPR14__Product__r];
        NSDictionary * SPR14__Product__r = [obj2 isKindOfClass:[NSNull class]]?nil:[obj2 fields];
        if (SPR14__Product__r != nil)
        {
            NSString * partName = [[SPR14__Product__r objectForKey:gName] isKindOfClass:[NSString class]]?[SPR14__Product__r objectForKey:gName]:@"";
            [Part setObject:partName forKey:gName];
        }
        
        NSString * numPartsUsed = [dict objectForKey:gSPR14__Actual_Quantity2__c];
        ...
        NSString * description = [dict objectForKey:gSPR14__Work_Description__c];
        ...
        if ([Part objectForKey:KEY_COSTPERPART] == nil)
            [Part setObject:[[obj fields] objectForKey:gSPR14__Actual_Price2__c] forKey:KEY_COSTPERPART];
        
        if ([Part objectForKey:KEY_PRODUCTID] == nil)
            [Part setObject:[[obj fields] objectForKey:gSPR14__Product__c] forKey:KEY_PRODUCTID];
		
		//pavaman 26th Feb 2011 - adding discount field
        NSString * discount = [dict objectForKey:gSPR14__Discount__c];
...
- (void) getExistingLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    for (int j = 0; j < [array count]; j++)
    {
        ...
        if ([[[obj fields] objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:CALIBRATION])
        {
            ...
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_CALIBRATION];
        }
        if ([[[obj fields] objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:CLEANUP])
        {
            ...
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_CLEANUP];
        }
        if ([[[obj fields] objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:INSTALLATION])
        {
            ...
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_INSTALLATION];
        }
        if ([[[obj fields] objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:REPAIR])
        {
            ...
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_REPAIR];
        }
        if ([[[obj fields] objectForKey:@"SPR14__Activity_Type__c"] isEqualToString:SERVICE])
        {
            ...
            [LabourValuesDictionary setValue:[[obj fields] objectForKey:@"SPR14__Actual_Quantity2__c"] forKey:QTY_SERVICE];
        }
    }
    
    NSString * query = [NSString stringWithFormat:@"SELECT SPR14__Billable_Cost2__c FROM SPR14__Service_Group_Costs__c	WHERE SPR14__Group_Member__c = '%@' AND SPR14__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appTechnicianId];
...
- (void) getPriceForLabor:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    ...
    if( array != nil && ![array count] )
	{
		...
		NSString * query = [NSString stringWithFormat:@"SELECT SPR14__Billable_Cost2__c FROM SPR14__Service_Group_Costs__c WHERE SPR14__Service_Group__c = '%@' AND SPR14__Cost_Category__c = 'Straight' LIMIT 1", appDelegate.appServiceTeamId];
        ...
        if(SLA_FLAG)
        {
            ...
            else
            {
                ...
                if(appDelegate.isWorkinginOffline)
                {
                    ...
                    sla_clock_paused = [SLADict objectForKey:@"SPR14__Sla_Clock_Paused__C"];
                    actual_resolution = [SLADict objectForKey:@"SPR14__Actual_Resolution__C"];
                    actual_restoration = [SLADict objectForKey:@"SPR14__Actual_Restoration__C"];
                    restorationCustomerBy = [SLADict objectForKey:@"SPR14__Restoration_Customer_By__C"];
                    resolutionCustomerBy = [SLADict objectForKey:@"SPR14__Resolution_Customer_By__C"];
                    pausedTime = [SLADict objectForKey:@"SPR14__Sla_Clock_Pause_Time__C"];
        ...
        else
        {
            ...
            if(info_dict != nil)
            {
                ...
                //    UILabel * lbl = [[[UILabel alloc]init] autorelease];
                    ...
                    if ([str isEqualToString:gSPR14__Problem_Description__c])
                    {
                        value = [info_dict objectForKey:gSPR14__Problem_Description__c];
                        lbl.backgroundColor = [UIColor clearColor];
                        lbl.text = value;
                        [background addSubview:lbl];
                    }
                    ...
                    for (int j=0;j<[details_values count];j++) //parts for instance
                    {
                        ...
                        for (int k = 0; k < [child_record_fields count];k++) //fields of one part for instance
                        {
                            ...
                            for(int l = 0 ;l < [fields_array count]; l++)
                            {
                                ...
                                if ([detail_api_name isEqualToString:@"SPR14__Email__c"] )  //Shrinivas Fix for Email Validation 03/04/2012
                                {
                                    BOOL result;
                                    NSString * emailRegEx = @"[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
                                    NSPredicate * emailTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", emailRegEx];
                                          ...
                                          childObjects:(NSArray *)childObjectsArray
{
    ...
    for(NSDictionary *ruleDict in headerRulesArray)
    {
        ...
        if(ruleInfo)
        {
            ...
            {
                NSString *parameterType = [rule objectForKey:@"SPR14__Parameter_Type__c"];
                NSString *fieldName = [rule objectForKey:@"SPR14__Field_Name__c"];
                if(![headerFields containsObject:fieldName])
                    [headerFields addObject:fieldName];
                if([parameterType isEqualToString:@"Field Value"])
                {
                    NSString *operandFieldName = [rule objectForKey:@"SPR14__Operand__c"];
...
//                if(sectionFieldValue != nil) // Damodar - Win14 - MemMgt - revoked to fix crash
    ...
    for(NSDictionary *childDict in childObjectsArray)
    {
        ...
        if(![fieldApiNameArray count])
        {
            ...
            //object_api_name ='SPR14__RMA_Shipment_Order__c' and reference_to= 'SPR14__Service_Order__c'
                                    ...
                                    withData:(NSString *)dataToValidateString
{
    ...
    NSString *appConfig = [NSString stringWithFormat:@"var appConfig = { \"title\" : \"Mobile business rules application\", \"version\" : \"1.0.0\",\
                           ...
                           ],\"app-config\" : {\"application-id\" : \"application\",\"enable-cache\" : true,\"enable-log\" : true,\"org-name-space\" : \"SPR14\"},\"platform-config\" : {}};"];
...
- (NSArray *) getBusinessRulesDict:(NSArray *)businessRulesArray
{
    ...
    for(NSDictionary *dict in businessRulesArray)
    {
        ...
        for(NSDictionary *detailsDict in bizRuleDetailsArray)
        {
            ...
            [details setObject:operand forKey:@"SPR14__Operand__c"];
            [details setObject:expressionID forKey:@"SPR14__Expression_Rule__c"];
            [details setObject:parameterType forKey:@"SPR14__Parameter_Type__c"];
            [details setObject:operator forKey:@"SPR14__Operator__c"];
            [details setObject:fieldName forKey:@"SPR14__Field_Name__c"];
        ...
        [ruleDict setObject:sourceObjectName forKey:@"SPR14__Source_Object_Name__c"];
        [ruleDict setObject:advancedExpression forKey:@"SPR14__Advance_Expression__c"];
        [ruleDict setObject:messageType forKey:@"SPR14__Message_Type__c"];
...
{
    NSString * query = [NSString stringWithFormat:@"SELECT SPR14__Product__c FROM SPR14__Service_Order__c WHERE local_id = '%@'", recordId];
...
- (void)updateWorkOrderWithPrice:(NSDictionary *)updatedWo  {
    ...
    NSString *tableNameTwo = @"SPR14__Service_Order_Line__c";
...
- (NSDictionary *)getServiceBooleanTypeDictionary:(NSString *)tableName {
    ...
    if ([tableName isEqualToString:@"SPR14__Service_Order__c"]) {
        [someDictionary setObject:@"invalid_address_c" forKey:@"invalid_address_c"];
        [someDictionary setObject:@"SPR14_invoice_created_c" forKey:@"SPR14_invoice_created_c"];
        [someDictionary setObject:@"auto_calc_drive_time_c" forKey:@"auto_calc_drive_time_c"];
        [someDictionary setObject:@"SPR14_clock_paused_forever_c" forKey:@"SPR14_clock_paused_forever_c"];
        [someDictionary setObject:@"SPR14_optimax_error_occured_c" forKey:@"SPR14_optimax_error_occured_c"];
        [someDictionary setObject:@"SPR14_is_pm_work_order_c" forKey:@"SPR14_is_pm_work_order_c"];
        [someDictionary setObject:@"SPR14_is_sla_calculated_c" forKey:@"SPR14_is_sla_calculated_c"];
        [someDictionary setObject:@"SPR14_apply_business_hours_for_optimax_c" forKey:@"SPR14_apply_business_hours_for_optimax_c"];
        [someDictionary setObject:@"regulatory_4_c" forKey:@"regulatory_4_c"];
        [someDictionary setObject:@"SPR14_locked_by_dc_c" forKey:@"SPR14_locked_by_dc_c"];
        [someDictionary setObject:@"SPR14_sla_clock_paused_c" forKey:@"SPR14_sla_clock_paused_c"];
        [someDictionary setObject:@"geoviatrigger_c" forKey:@"geoviatrigger_c"];
        [someDictionary setObject:@"SPR14_is_service_covered_c" forKey:@"SPR14_is_service_covered_c"];
        [someDictionary setObject:@"SPR14_pm_tasks_created_c" forKey:@"SPR14_pm_tasks_created_c"];
        [someDictionary setObject:@"SPR14_customer_down_c" forKey:@"SPR14_customer_down_c"];
        [someDictionary setObject:@"SPR14_perform_auto_entitlement_c" forKey:@"SPR14_perform_auto_entitlement_c"];
        [someDictionary setObject:@"cust_bool_c" forKey:@"cust_bool_c"];
        [someDictionary setObject:@"SPR14_is_entitlement_performed_c" forKey:@"SPR14_is_entitlement_performed_c"];
        [someDictionary setObject:@"SPR14_isPartnerRecord_c" forKey:@"SPR14_isPartnerRecord_c"];
        [someDictionary setObject:@"parts_required_c" forKey:@"parts_required_c"];
        
    }
    else {
        [someDictionary setObject:@"SPR14_service_order_line_c" forKey:@"SPR14_service_order_line_c"];
        [someDictionary setObject:@"SPR14_use_price_fromPricebook_c" forKey:@"SPR14_use_price_fromPricebook_c"];
        [someDictionary setObject:@"isDeleted" forKey:@"isDeleted"];
        [someDictionary setObject:@"SPR14_posted_to_inventory_c" forKey:@"SPR14_posted_to_inventory_c"];
        [someDictionary setObject:@"SPR14_is_billable_c" forKey:@"SPR14_is_billable_c"];
        [someDictionary setObject:@"SPR14_include_in_quote_c" forKey:@"SPR14_include_in_quote_c"];
        [someDictionary setObject:@"SPR14_select_c" forKey:@"SPR14_select_c"];
    }
...
- (BOOL)shouldShowBillableAmountInServiceReport {
    return [appDelegate.calDataBase checkIfBillablePriceExistForWorkOrderId:appDelegate.sfmPageController.recordId andFieldName:@"SPR14__Billable_Line_Price__c"];
}

- (BOOL)shouldShowBillableQuantityInServiceReport {
    
    
    return [appDelegate.calDataBase checkIfBillablePriceExistForWorkOrderId:appDelegate.sfmPageController.recordId andFieldName:@"SPR14__Billable_Quantity__c"];
}
#pragma mark -
