...
#import "AccessibilitySFMTollbarActionConstants.h"
...
- (void)viewDidLoad
{
	...
	SMLog(kLogLevelError,@"Exception Name SFWToolBar :viewDidLoad %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFWToolBar :viewDidLoad %@",exp.reason);
    }
...
{
    SMLog(kLogLevelVerbose,@"%@", ipad_only_array);                      
                                 
                                 
    SMLog(kLogLevelVerbose,@"%f, %f", ipad_only_view.frame.size.width, ipad_only_view.frame.size.height);
    ...
    for (int i = 0 ; i <[ipad_only_array count] ; i++) 
    {
        ...
        ipad_only_view.backgroundColor=[UIColor clearColor];
    }
     }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name SFWToolBar :showIpadOnlyButtons %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason SFWToolBar :showIpadOnlyButtons %@",exp.reason);
    }
}
...
{
    SMLog(kLogLevelVerbose,@" buttonsArray_offline %@ ", buttonsArray_offline);
    @try{
    NSMutableArray * array = [wizard_info objectForKey:SFW_WIZARD_INFO];
	...
	SMLog(kLogLevelError,@"Exception Name SFWToolBar :numberOfRowsInSection %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFWToolBar :numberOfRowsInSection %@",exp.reason);
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
        ...
        if([key isEqualToString:wizard_id])
        {
            ...
            for(int j = x, index =0  ; j < (x+6) ;j++, index++)
            {
                ...
                if ([action_type isEqualToString:@"SFW_Custom_Actions"])
                {
                    ...
                    btn.accessibilityValue = kAccWizardBorderNone; /*Accessibility changes*/
                }
                else
                {
                    btn.accessibilityValue = kAccWizardBorderGreen; /*Accessibility changes*/
                }
                ...
                [btn setAccessibilityIdentifier:title];
	...
	SMLog(kLogLevelError,@"Exception Name SFWToolBar :cellForRowAtIndexPath %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFWToolBar :cellForRowAtIndexPath %@",exp.reason);
    }
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section;
{
   ...
   // NSString * wizard_description = [dict objectForKey:WIZARD_DESCRIPTION];
	...
	SMLog(kLogLevelError,@"Exception Name SFWToolBar :titleForHeaderInSection %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason SFWToolBar :titleForHeaderInSection %@",exp.reason);
    }
