...
- (void) buttonClicked:(id)sender
{
    ...
    {
        NSLog(@"Exception Name Action Menu :buttonClicked %@",e.name);
        NSLog(@"Exception Reason Action Menu :buttonClicked %@",e.reason);
...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i < [buttons count]; i++)
    {
        ...
        if (size.width > maxSize)
            ...
            NSLog(@"Exception Name Action Menu :viewDidLoad %@",exp.name);
            NSLog(@"Exception Reason Action Menu :viewDidLoad %@",exp.reason);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        NSLog(@"Exception Name Action Menu :cellForRowAtIndexPath %@",exp.name);
        NSLog(@"Exception Reason Action Menu :cellForRowAtIndexPath %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
