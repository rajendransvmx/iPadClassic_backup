...
#import "AccessibilitySFMRootViewConstants.h"
...
- (void) displaySwitchViews
{
      ...
      if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"VIEWRECORD"])
    {
        ...
        [selectProcessButton setAccessibilityIdentifier:kAccViews];
...
- (void) showLastModifiedTimeForSFMRecord
{
    ...
    if(lastModifiedTime == nil)
    {
        ...
        /*Accessibility changes*/
        lastModifiedTime.isAccessibilityElement = YES;
        [lastModifiedTime setAccessibilityIdentifier:kAccLastUpdatedOn];
    ...
    if(check_On_demand)
    {
        ...
        {
            [formatter1 setDateFormat:DATETIMEFORMAT24HR];
        }
        else
        {
            [formatter1 setDateFormat:DATETIMEFORMAT];
        }
            
        [formatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease]];
        NSCalendar *cal = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
        [formatter setCalendar:cal];
        [formatter1 setCalendar:cal];
        [formatter1 setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease]];
        ...
        if ([Utility isString24HrFormat:recordTimeStamp])
        {
            [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        }
        else
        ...
        /*Accessibility changes*/
        timeStamp.isAccessibilityElement = YES;
        [timeStamp setAccessibilityIdentifier:kAccDODTimeStamp];
        [formatter release];
        [formatter1 release];
	}
    else
...
//8799 : Pop overs not getting dismissed from switch view resulting in black out of the page
- (void) selectProcess:(id)sender
{
    if (switchProcess.popOver.isPopoverVisible)
    {
        [switchProcess.popOver dismissPopoverAnimated:YES];
        //release it once dismissed else release it in dealloc
        [switchProcess release];
        switchProcess = nil;
        return;
    }
    ...
    {
        switchProcess = [[SelectProcessController alloc] initWithNibName:@"SelectProcessController" bundle:nil];
        ...
        @try{
            switch (section)
            {
                case 0:
                {
                    ...
                    [header_button setAccessibilityIdentifier:kAccShowAllHeader];
                ...
                case 1:
                {
                    ...
                    /*Accessibility changes*/
                    lines_button.isAccessibilityElement = YES;
                    [lines_button setAccessibilityIdentifier:kAccShowAllLines];
                ...
                case 2:
                {
                    ...
                    /*Accessibility changes*/
                    lines_button.isAccessibilityElement = YES;
                    [lines_button setAccessibilityIdentifier:kAccShowAllAdditionalInfo];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView == self.errorTableView)
    {
        ...
        self.errorTableView.accessibilityIdentifier = kAccBizRuleError; //Accessibility Changes
        ...
        if(errorDictonary != nil || errorDictonary != NULL)
        {
            ...
            if([[SFMPageDetailDict allKeys] count]>0 && SFMPageDetailDict != nil && SFMPageDetailDict != NULL)
            {
                ...
                NSString *str= [[[[self.conflictsArray objectAtIndex:indexPath.row] objectForKey:@"ruleInfo"] objectForKey:@"bizRule"] objectForKey:@"SVMXDEV__Source_Object_Name__c"];
