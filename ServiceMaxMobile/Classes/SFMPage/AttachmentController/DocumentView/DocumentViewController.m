...
//                        cellView.editIconImageView.hidden = YES; //D-00003728
                    ...
                    if ([self.sharingAttachmentList objectForKey:attachmentId] != nil) {
                        ...
                        //Defect 11338
                        NSString * fileName = [self.duplicateSharingFiles objectForKey:attachmentId];
                        ...
                        [self.duplicateSharingFiles removeObjectForKey:attachmentId];
                    }
                    //D-00003728
                    else {
                        ...
                        if (self.duplicateSharingFiles == nil) {
                            NSMutableDictionary *tempDictionary  = [[NSMutableDictionary alloc] init];
                            self.duplicateSharingFiles  = tempDictionary;
                            [tempDictionary release];
                            tempDictionary = nil;
                        }
                       ...
                       {
                            NSData * data =  [AttachmentUtility getEncodedDataForExistingFile:attachmentId attachmentName:fileName];
                            
                            if (data != nil)
                            {
                                [AttachmentUtility saveDuplicateAttachmentData:data inFileName:fileName];
                                NSURL * url = [AttachmentUtility getUrlForFilename:fileName attchmentId:attachmentId];
                                if (url != nil)
                                {
                                    [self.sharingAttachmentList setObject:url forKey:attachmentId];
                                    [self.duplicateSharingFiles setObject:fileName forKey:attachmentId]; //Defect 11338
                                }
                            }
                            [self enableEditButtons:YES];
                        }
...
- (void)clearSelectedData
{
    ...
    //Defect 11338
    if ((self.duplicateSharingFiles != nil) && ([self.duplicateSharingFiles count] > 0))
    {
