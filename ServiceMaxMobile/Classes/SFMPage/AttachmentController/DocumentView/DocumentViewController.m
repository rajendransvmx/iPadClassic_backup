...
- (UIImage *) imageForDocument:(NSDictionary *)documentDict andStatus:(ATTACHMENT_STATUS)status
{
    ...
    {
        image = [UIImage imageNamed:[NSString stringWithFormat:@"FileNotFound.png"]];
    }
    return image;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (docSize != nil)
    {
        cell.leftLabel.text = [NSString stringWithFormat:@"%@MB",[self getFormattedSize:docSize]];
    }
    else
    {
        cell.leftLabel.text = @"";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.documentsArray count] > indexPath.row) {
        if(!isInEditMode)
        {
            ...
            else if(attachmentStatus ==  ATTACHMENT_STATUS_YET_TO_DOWNLOAD || attachmentStatus ==  ATTACHMENT_STATUS_ERROR_IN_DOWNLOAD)
            {
...
#warning The existing tag IPAD024_TAG007 was changed to "Delete Permanently" for use in delete alert confirmation. Hence this tag cannot be used as title for the button.
    NSString *deleteButtonLabel = @"Delete";//[appDelegate.wsInterface.tagsDictionary objectForKey:DELETE_ACTION];
...
{
    NSLog(@"Handle cancel button tap");
    NSString *editButtonLabel = [appDelegate.wsInterface.tagsDictionary objectForKey:EDIT_LIST];
    self.isInEditMode = NO;
    [self handleCancelButtonDisplay];
    [self.selectedIdDictionary removeAllObjects];
