...
- (void)updateTrailerTableWithDeletedIds:(NSArray*)deletedIds
{
    for (NSString *deletedId in deletedIds)
    {
        ...
        // Fix For Defect 9219
        [self deleteFromSFAttachmentTrailer:deletedId];
    }
    // Fix For Defect 9219
    [[SMAttachmentRequestManager sharedInstance] cancelRequestForIds:deletedIds];
}
            ...
            if ( _someString != nil ){
                ...
                if (objString != nil)
                {
                    ...
                    if (objString.length <= 2) {
                        NSString *fileName = [self getColoumnFromAttachment:@"Name" forLocalId:localId];
                        if ([action isEqualToString:kAttachmentActionTypeDownload]) {
                            /*NSString - get action */
                            ...
                            fileName = [NSString stringWithFormat:@"%@.%@",localId,pathExtension];
                        }
...
//9212
- (int)getErrorCodeForAttachmentId:(NSString *)attachmentId {
    
    sqlite3_stmt * stmt = nil;
    NSInteger errorCode = 0;
    NSString * query = [NSString stringWithFormat:@"SELECT error_code FROM 'AttachmentError' where attachment_id='%@'", attachmentId];
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
    {
        while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
        {
            char * tempStr = (char *)synchronized_sqlite3_column_text(stmt, 0);
            if ((tempStr != nil) && strlen(tempStr))
            {
                NSString *counterStr = [NSString stringWithUTF8String:tempStr];
                errorCode = [counterStr intValue];
            }
...
//Data Purge -10339 Defect Fix
- (void)deleteEntriesFromAttachmentErrorAndTrailerTable:(NSString *)localIDs Name:(NSString *)tableName
{
	NSString * query = [[NSString alloc] initWithFormat:@"DELETE FROM '%@' WHERE attachment_id IN (%@)",tableName,localIDs]; //10289
