         ...
         if(attachmentStatus != ATTACHMENT_STATUS_YET_TO_DOWNLOAD && attachmentStatus != ATTACHMENT_STATUS_DOWNLOAD_IN_PROGRESS && attachmentStatus != ATTACHMENT_STATUS_DOWNLOAD_IN_QUEUE && attachmentStatus != ATTACHMENT_STATUS_ERROR_IN_DOWNLOAD  &&  attachmentStatus != ATTACHMENT_STATUS_UPLOAD_IN_PROGRESS)//9212 
         {
             ...
             if(deleted)
             {
                 ...
                 if (fileName != nil && [fileName length] > 0)
                 {
                     ...
                     NSData * data =  [AttachmentUtility getEncodedDataForExistingFile:cell.localId attachmentName:fileName];
                     
                     if (data != nil)
                     {
                         NSString * attachmentName = [self getFileName:indexPath];
                         
                         if (attachmentName != nil && [fileName length] > 0)
                         {
                             [AttachmentUtility saveDuplicateAttachmentData:data inFileName:attachmentName];
                         }
                         NSURL * url = [AttachmentUtility getUrlForFilename:attachmentName attchmentId:cell.localId];
                         
                         if (url != nil)
                         {
                             [self.sharingAttachmentList setValue:url forKey:cell.localId];
                             [self.duplicateSharingFiles setValue:attachmentName forKey:cell.localId]; //Defect 11338
                         }
                     }
                 }
             }
             else
             {
                 ...
                 if ([self.sharingAttachmentList count] > 0) //D-00003728
                 {
                     ...
                     //Defect 11338
                     NSString * fileName = [self.duplicateSharingFiles objectForKey:cell.localId];
                     BOOL success =[AttachmentUtility deleteDuplicateFileCreated:fileName];
                     SMLog(kLogLevelVerbose, @"Duplicate file deleted %@ , success = %d",fileName, success);
                     [self.duplicateSharingFiles removeObjectForKey:cell.localId];
                 }
             }
             [cell UnhighlightcellFromDeletion];
         }
...
- (void)dealloc {
    ...
    [_duplicateSharingFiles release];
    [_addIcon release];
    [super dealloc];
}
...
- (IBAction)shareAttachment:(id)sender
{
    ...
    if ([self.sharingAttachmentList count] > 0)
    {
        NSArray * urls = [self.sharingAttachmentList allValues];
        if ([imageViewDelegate conformsToProtocol:@protocol(ImageViewControllerDelegate)])
        {
            [imageViewDelegate displayAttachmentSharingView:urls viewName:COLLECTIONVIEW];
        }
    }
    [self.deletedList removeAllObjects];
    [self.sharingAttachmentList removeAllObjects];
...
{
    if ([self.duplicateSharingFiles count] > 0)
    {
        NSArray * array = [self.duplicateSharingFiles allValues];
        [self deleteDuplicateAttachmentsCreated:array];
    }
    [self.duplicateSharingFiles removeAllObjects];
}
...
- (void)clearSelectedData
{
    ...
    //Defect 11338
    if (self.duplicateSharingFiles == nil)
    {
        self.duplicateSharingFiles = [[NSMutableDictionary alloc] initWithCapacity:0];
    }
    else
    {
        [self deleteAttachmetFiles];
