...
#warning The existing tag IPAD024_TAG007 was changed to "Delete Permanently" for use in delete alert confirmation. Hence this tag cannot be used as title for the button.
    NSString *delete=@"Delete";//[appDelegate.wsInterface.tagsDictionary objectForKey:DELETE_ACTION];
...
-(void)fillImageDataAtIndexPath:(NSIndexPath *)indexPath collectionViewcell:(imageCollectionViewCell *)cell initiateViews:(BOOL)initiateViews
{
    ...
    if(initiateViews)
    {
        ...
        {
           size_text = [NSString stringWithFormat:@"%@MB",[AttachmentUtility getFormattedSize:attachmentSize]];;
        }
