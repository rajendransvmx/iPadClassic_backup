...
-(NSMutableArray *)getObjectMappingForMappingId:(NSMutableDictionary *)process_components  source_record_id:(NSString *)source_record_id field_name:(NSString *)field_name
{
    ...
    if([source_record_id length ] != 0 && source_record_id != nil )
    {
     ...
     /*   NSMutableDictionary * dataTypeForFeilds = [NSMutableDictionary dictionary];
        ...
        /**
         * Pushpak defect 014051
         */
        NSString *SFId = [self getSfid_For_LocalId_From_Object_table:field_name local_id:source_record_id];
        ...
        if(expression_ != nil && [expression_ length] != 0)
        {
            ...
            if (SFId.length) {
                ...
                query = [NSString stringWithFormat:@"SELECT %@ FROM '%@' where (%@ = '%@' or %@ = '%@')  and %@",query_field_names, source_object_name,field_name, source_record_id,field_name, SFId,expression_];
            } else
            {
        ...
        else
        {
            ...
            if (SFId.length) {
                 ...
                 query = [NSString stringWithFormat:@"SELECT %@ FROM '%@' where (%@ = '%@' or %@ = '%@') ",query_field_names, source_object_name,field_name,source_record_id,field_name,SFId];
            } else
            {
...
//011782
- (void)clearObjectMetaDataRelatedCache
{
    if (self.objectFieldDictionary != nil) {
        self.objectFieldDictionary = nil;
    }
    if (self.apiNameToInsertionQueryDictionary != nil) {
        self.apiNameToInsertionQueryDictionary = nil;
    }
    if (self.fieldDataTypeDictionary != nil) {
        self.fieldDataTypeDictionary   = nil;
    }
    if (self.childInfoDictionary != nil) {
        self.childInfoDictionary = nil;
    }
    if (self.childInfoCacheDictionary != nil) {
        self.childInfoCacheDictionary = nil;
    }
}
