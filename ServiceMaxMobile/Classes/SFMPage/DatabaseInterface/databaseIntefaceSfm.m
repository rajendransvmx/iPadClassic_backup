...
-(NSMutableDictionary *)getRestorationAndResolutionTimeForWorkOrder:(NSString *)record_id  tableName:(NSString *)tableName;
{
    ...
    if(record_id != nil)
    {
        NSString * query = [NSString stringWithFormat:@"SELECT SPR14__Restoration_Customer_By__c, SPR14__Resolution_Customer_By__c,SPR14__Sla_Clock_Paused__C,SPR14__Actual_Resolution__C,SPR14__Actual_Restoration__C,SPR14__Restoration_Customer_By__C,SPR14__Resolution_Customer_By__C,SPR14__Sla_Clock_Pause_Time__C FROM '%@' where local_id = '%@'", tableName ,record_id];
    ...
    [SLADict setObject:clockedPaused forKey:@"SPR14__Sla_Clock_Paused__C"];
    [SLADict setObject:actualRestoration forKey:@"SPR14__Actual_Resolution__C"];
    [SLADict setObject:actualResolution forKey:@"SPR14__Actual_Restoration__C"];
    [SLADict setObject:restorationCustomerBy forKey:@"SPR14__Restoration_Customer_By__C"];
    [SLADict setObject:resolutionCustomerBy forKey:@"SPR14__Resolution_Customer_By__C"];
    [SLADict setObject:pausedTime forKey:@"SPR14__Sla_Clock_Pause_Time__C"];
    return SLADict;
}
...
-(NSMutableDictionary *)gettheAdditionalInfoForForaWorkOrder:(NSString *)record_id tableName:(NSString *)tablename
{
    ...
    if(record_id != nil)
    {
        NSString * query = [NSString stringWithFormat:@"SELECT  SPR14__Top_Level__c,SPR14__Component__c,SPR14__Company__c  FROM '%@' where local_id = '%@'", tablename ,record_id];
        ...
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
        {
            while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                [additional_info setObject:topLevel_Id forKey:@"SPR14__Top_Level__c"];
                [additional_info setObject:component forKey:@"SPR14__Component__c"];
                [additional_info setObject:account_Id forKey:@"SPR14__Company__c"];
            }
        }
        synchronized_sqlite3_finalize(stmt);
    }
...
-(NSMutableArray *)getAccountHistoryForanWorkOrder:(NSString *)record_id  account_id:(NSString *)account_id tableName:(NSString *)tablename 
{
    ...
    NSArray * keys = [NSArray arrayWithObjects:@"CreatedDate",@"SPR14__Problem_Description__c", nil];
    ...
    {
        NSString * query =  [NSString stringWithFormat:@"SELECT  SPR14__Problem_Description__c, CreatedDate FROM '%@' WHERE CreatedDate <= '%@' AND SPR14__Order_Status__c = 'Closed' AND Id != '%@' AND SPR14__Company__c = '%@'",tablename, dateString, record_id, account_id];
...
-(NSMutableArray *)getProductHistoryForanWorkOrder:(NSString *)record_id  filedName:(NSString *)fieldName tableName:(NSString *)tablename  fieldValue:(NSString *)fieldValue
{
    ...
    {
        NSString * query =  [NSString stringWithFormat:@"SELECT  SPR14__Problem_Description__c, CreatedDate FROM '%@' WHERE CreatedDate <= '%@' AND SPR14__Order_Status__c = 'Closed' AND Id != '%@' AND %@ = '%@'",tablename, dateString, record_id,fieldName, fieldValue];
        sqlite3_stmt * stmt ;
...
-(NSMutableDictionary *)queryForMapWorkOrderInfo:(NSString *)record_id tableName:(NSString *)tableName;
{
    ...
    if(record_id != nil)
    {
        ...
        NSString * query = [NSString stringWithFormat:@"select c.phone ,c.Email, c.Name, w.Name ,w.SPR14__Problem_Description__c ,w.SPR14__Order_Type__c, w.SPR14__Contact__c from SPR14__Service_Order__c  as w  LEFT OUTER join  Contact  as c on  w.SPR14__Contact__c = c. id   where  w.local_id = '%@'" ,record_id];
        sqlite3_stmt * stmt ;
        ...
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
        {
            ...
            {   
                ...
                [dict  setObject:Phone forKey:@"SPR14__Contact__r.Phone"];
                [dict  setObject:email forKey:@"SPR14__Contact__r.Email"];
                [dict  setObject:caontactName forKey:@"SPR14__Contact__r.Name"];
                [dict  setObject:Work_orderName forKey:@"Name"];
                [dict  setObject:workOrderProblemDescription forKey:@"SPR14__Problem_Description__c"];
                [dict  setObject:workOrderOrderType forKey:@"SPR14__Order_Type__c"];
                [dict  setObject:contact_Id forKey:@"SPR14__Contact__c"]; //shrinivas
                        ...
                        
{
    ...
    NSMutableArray *_dictKeys = [NSMutableArray arrayWithObjects:@"DATA", @"SEQUENCE", @"SPR14__Default_Lookup_Column__c", nil];
...
//{
//    NSString * query = [NSString stringWithFormat:@"SELECT SPR14_Field_Name_c FROM '%@' where Id = '%@' and default_desplay_column = '%@' ",SFCONFIG_DATA_TABLE ,lookup_id,@"true" ];
//    sqlite3_stmt * stmt ;
//    NSString * field_name = @"";
//    
//    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
            ...
            if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sql UTF8String], -1, &statement, nil) == SQLITE_OK)
            {
                ...
                {   
                    ...
                    else if ([local_id length] != 0 && [json_record length] != 0 )
                    {
                        ...
                        if ([object_Name isEqualToString:@"SPR14__User_GPS_Log__c"]) {
                            BOOL isUser_GPS_LogTableDelete=[self DeleterecordFromTable:@"SPR14__User_GPS_Log__c" Forlocal_id:local_id];
                            [appDelegate.dataBase deleteSequenceofTable:@"SPR14__User_GPS_Log__c"];
