            ...
            if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
            {
                while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW) 
                {
                    ...
                    if(target_field_name != nil || [target_field_name length] != 0)
                    {
                        ...
                        else if ([data_type isEqualToString:@"currency"])
                        {
...
-(NSString *) queryForExpressionComponent:(NSString *)expression expression_id:(NSString *)expression_id object_name:(NSString *)object_name;
{
    ...
    NSString *collateNoCase = @"COLLATE NOCASE";
    ...
    for(int p = 0 ; p < [final_Comonent_array count];p++)
    {
        ...
        for(int q = 0; q <[keys count]; q++)
        {
            ...
            if ([data_type isEqualToString:@"date"] || [data_type isEqualToString:@"datetime"]) {
                ...
                if(![rhs caseInsensitiveCompare:MACRO_NOW]== NSOrderedSame) {
                    if ([newRhsValue length] > 10) {
                        NSString *newValue = [newRhsValue substringToIndex:10];
                        newRhsValue = [NSString stringWithFormat:@"%%%@%%", newValue];
                    }
            ...
            if([temp_lhs isEqualToString:@"RecordTypeId"])
            {
				...
				else
				{
                    ...
                    {
                        component_expression = [NSString stringWithFormat:@"( %@   in   (select  record_type_id  from SFRecordType where (record_type %@ '%@' %@ or trim(record_type) = '' or   record_type is null )))" ,lhs,operator, rhs, collateNoCase];
                    ...
                    else if([operator isEqualToString:@" IN "] || [operator isEqualToString:@" NOT IN "])
                    {
                        ...
                        if([operator isEqualToString:@" NOT IN "]) {
                            ...
                            component_expression = [NSString stringWithFormat:@"( %@   in   (select  record_type_id  from SFRecordType where (record_type  %@ %@ ( %@ ) or trim(record_type) = '' or   record_type is null )))" ,lhs, collateNoCase, operator, stringArray];
                            component_expression = [NSString stringWithFormat:@" ( %@ OR  trim(%@) = '' OR %@ is null )",component_expression,lhs,lhs];
                        }
                        ...
                        component_expression = [NSString stringWithFormat:@"( %@ in   (select  record_type_id  from SFRecordType where record_type %@ %@ ( %@ ) ) )",lhs,collateNoCase, operator,stringArray];
                        }
                        [stringArray release]; stringArray = nil;
                    }

                    else {
                        component_expression = [NSString stringWithFormat:@" %@   in   (select  record_type_id  from SFRecordType where record_type %@ '%@' %@ )" ,lhs,operator, rhs, collateNoCase];
                    }
				}
                //5755
            }
            else if([data_type isEqualToString:@"reference"])
            {
                ...
                else if([operator isEqualToString:@" IN "] || [operator isEqualToString:@" NOT IN "])
                {
                    ...
                    if ( [operator isEqualToString:@" NOT IN "] ) {
                        component_expression = [NSString stringWithFormat:@" (%@ in   (select  Id  from '%@' where %@ %@ %@  ( %@ )) OR %@ = '' OR %@ isnull)",lhs,referenceToTable, nameField, collateNoCase, operator,stringArray, lhs, lhs];
                    }
                    else {
                        component_expression = [NSString stringWithFormat:@" (%@ in   (select  Id  from '%@' where %@ %@ %@  ( %@ )))",lhs,referenceToTable, nameField, collateNoCase, operator,stringArray];
                    }
    
                    [stringArray release]; stringArray = nil;
                }
                else if([operator isEqualToString:@" NOT LIKE "] ) {
                    component_expression = [NSString stringWithFormat:@" (%@   in   (select  Id  from '%@' where %@ %@ %@ '%@' ) OR %@ = '' OR %@ isnull)" , lhs,referenceToTable, nameField, collateNoCase, operator ,rhs, lhs, lhs];
                }
				else
				{
					component_expression = [NSString stringWithFormat:@" (%@   in   (select  Id  from '%@' where %@ %@ %@ '%@' ) )" , lhs,referenceToTable , nameField, collateNoCase, operator ,rhs];
				}
            ...
            else if([operator isEqualToString:@" IN "] || [operator isEqualToString:@" NOT IN "])
            {
               ...
               // rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                ...
                if( [operator isEqualToString:@" NOT IN "] ) {
                    //handling null for "!="
                     component_expression = [NSString stringWithFormat:@" ( %@ isnull or ( %@ %@ %@ ( %@ ) )) ",lhs,lhs,  collateNoCase, operator,   stringArray];
                }
                else {
                    component_expression = [NSString stringWithFormat:@" (%@ %@ %@ ( %@ ) )",lhs, collateNoCase, operator,stringArray];
                }
                [stringArray release]; stringArray = nil;
            }
            else if ([operator isEqualToString: @"!="])
            {
                component_expression = [NSString stringWithFormat:@" ( %@ isnull or (%@ %@ '%@' %@)) ",lhs,lhs,operator,rhs, collateNoCase];
            }
            ...
            else
            {
                ...
                //011250
             //   rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                rhs = [NSString stringWithFormat:@"'%@' %@",rhs, collateNoCase];
                NSString *newString = [NSString stringWithFormat:@"(%@ %@)", component_expression, rhs];
                component_expression = newString;
                ...
                for(int q = 0; q <[keys_test count]; q++)
                {
                    ...
                    if([key isEqualToString:key_test])
                    {
                        ...
                        if(sequence_no != sequencetest_no)
                        {
                            ...
                            if ([data_type isEqualToString:@"reference"] && !([lhs_ isEqualToString:@"RecordTypeId"])) //9583
                            {
                                ...
                                newExpression = [NSString stringWithFormat:@"('%@'.%@  in  (select  Id  from '%@' where %@ %@ %@ %@ ))" , object_name,lhs_,referenceToTable , nameField, operator_ ,rhs_ , collateNoCase];
                                component_expression = [component_expression stringByAppendingFormat:@" %@ ",newExpression];
                            }
                            
                            else
                            {
                                NSString *newString = [NSString stringWithFormat:@"('%@'.%@",object_name, lhs_];
                                newString = [newString stringByAppendingString:operator_];
                                
                                rhs = [NSString stringWithFormat:@"%@ %@)",rhs_, collateNoCase];
                                newString = [newString stringByAppendingString:rhs];
                                component_expression = [component_expression stringByAppendingString:newString];
                            }
            ...
            NSString *newString = [NSString stringWithFormat:@"(%@)", component_expression];
            component_expression = newString;
        ...
        if(prepare_ == SQLITE_OK)
        {
            if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sql UTF8String], -1, &statement, nil) == SQLITE_OK)
            {
                ...
                {   
                    ...
                    {
                        /////11366////////
                       // - 11366 - Since we are supporting both localId or sfid in the "parent column name", it is same to comment the below code
//                        if([record_type isEqualToString:DETAIL])
//                        {
//                            [final_dict removeObjectForKey:parent_column_name];
 //                       }
