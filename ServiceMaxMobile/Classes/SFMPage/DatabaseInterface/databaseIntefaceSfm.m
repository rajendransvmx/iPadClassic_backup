...
-(NSMutableDictionary *)getRestorationAndResolutionTimeForWorkOrder:(NSString *)record_id  tableName:(NSString *)tableName;
{
    ...
    if(record_id != nil)
    {
        NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV__Restoration_Customer_By__c, SVMXDEV__Resolution_Customer_By__c,SVMXDEV__Sla_Clock_Paused__C,SVMXDEV__Actual_Resolution__C,SVMXDEV__Actual_Restoration__C,SVMXDEV__Restoration_Customer_By__C,SVMXDEV__Resolution_Customer_By__C,SVMXDEV__Sla_Clock_Pause_Time__C FROM '%@' where local_id = '%@'", tableName ,record_id];
    ...
    [SLADict setObject:clockedPaused forKey:@"SVMXDEV__Sla_Clock_Paused__C"];
    [SLADict setObject:actualRestoration forKey:@"SVMXDEV__Actual_Resolution__C"];
    [SLADict setObject:actualResolution forKey:@"SVMXDEV__Actual_Restoration__C"];
    [SLADict setObject:restorationCustomerBy forKey:@"SVMXDEV__Restoration_Customer_By__C"];
    [SLADict setObject:resolutionCustomerBy forKey:@"SVMXDEV__Resolution_Customer_By__C"];
    [SLADict setObject:pausedTime forKey:@"SVMXDEV__Sla_Clock_Pause_Time__C"];
    return SLADict;
}
...
-(NSMutableDictionary *)gettheAdditionalInfoForForaWorkOrder:(NSString *)record_id tableName:(NSString *)tablename
{
    ...
    if(record_id != nil)
    {
        NSString * query = [NSString stringWithFormat:@"SELECT  SVMXDEV__Top_Level__c,SVMXDEV__Component__c,SVMXDEV__Company__c  FROM '%@' where local_id = '%@'", tablename ,record_id];
        ...
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
        {
            while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                [additional_info setObject:topLevel_Id forKey:@"SVMXDEV__Top_Level__c"];
                [additional_info setObject:component forKey:@"SVMXDEV__Component__c"];
                [additional_info setObject:account_Id forKey:@"SVMXDEV__Company__c"];
            }
        }
        synchronized_sqlite3_finalize(stmt);
    }
...
-(NSMutableArray *)getAccountHistoryForanWorkOrder:(NSString *)record_id  account_id:(NSString *)account_id tableName:(NSString *)tablename 
{
    ...
    NSArray * keys = [NSArray arrayWithObjects:@"CreatedDate",@"SVMXDEV__Problem_Description__c", nil];
    
    NSDate * _date = [NSDate date];
    NSDateFormatter * dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSCalendar * cal = [[NSCalendar alloc]  initWithCalendarIdentifier:NSGregorianCalendar]; //10923
    ...
    [cal release];
    [datetimeFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease]];

    if(record_id != nil && account_id != nil && dateString != nil)
    {
        NSString * query =  [NSString stringWithFormat:@"SELECT  SVMXDEV__Problem_Description__c, CreatedDate FROM '%@' WHERE CreatedDate <= '%@' AND SVMXDEV__Order_Status__c = 'Closed' AND Id != '%@' AND SVMXDEV__Company__c = '%@'",tablename, dateString, record_id, account_id];
        ...
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
        {
            while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                created_date = [datetimeFormatter dateFromString:_createdBy]; //10923
...
-(NSMutableArray *)getProductHistoryForanWorkOrder:(NSString *)record_id  filedName:(NSString *)fieldName tableName:(NSString *)tablename  fieldValue:(NSString *)fieldValue
{
    ...
    {
        NSString * query =  [NSString stringWithFormat:@"SELECT  SVMXDEV__Problem_Description__c, CreatedDate FROM '%@' WHERE CreatedDate <= '%@' AND SVMXDEV__Order_Status__c = 'Closed' AND Id != '%@' AND %@ = '%@'",tablename, dateString, record_id,fieldName, fieldValue];
        sqlite3_stmt * stmt ;
...
-(NSMutableDictionary *)queryForMapWorkOrderInfo:(NSString *)record_id tableName:(NSString *)tableName;
{
    ...
    if(record_id != nil)
    {
        ...
        NSString * query = [NSString stringWithFormat:@"select c.phone ,c.Email, c.Name, w.Name ,w.SVMXDEV__Problem_Description__c ,w.SVMXDEV__Order_Type__c, w.SVMXDEV__Contact__c from SVMXDEV__Service_Order__c  as w  LEFT OUTER join  Contact  as c on  w.SVMXDEV__Contact__c = c. id   where  w.local_id = '%@'" ,record_id];
        sqlite3_stmt * stmt ;
        ...
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
        {
            ...
            {   
                ...
                [dict  setObject:Phone forKey:@"SVMXDEV__Contact__r.Phone"];
                [dict  setObject:email forKey:@"SVMXDEV__Contact__r.Email"];
                [dict  setObject:caontactName forKey:@"SVMXDEV__Contact__r.Name"];
                [dict  setObject:Work_orderName forKey:@"Name"];
                [dict  setObject:workOrderProblemDescription forKey:@"SVMXDEV__Problem_Description__c"];
                [dict  setObject:workOrderOrderType forKey:@"SVMXDEV__Order_Type__c"];
                [dict  setObject:contact_Id forKey:@"SVMXDEV__Contact__c"]; //shrinivas
            ...
            if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
            {
                while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW) 
                {
                    ...
                    if(target_field_name != nil || [target_field_name length] != 0)
                    {
                        ...
                        else if ([data_type isEqualToString:@"currency"] || [data_type isEqualToString:@"percent"] || [data_type isEqualToString:@"double"])
                        {
                            
                            NSMutableDictionary * precisinDict = [appDelegate.dataBase getNumberFieldVadlidationData:target_field_name objectName:target_object_name validationField:MPRECISION];
                            
                            NSInteger precision = [[precisinDict objectForKey:MPRECISION] integerValue];
                            
                            if ([Utility containsString:@"." inString:mapping_value])
                            {
                                precision = precision + 1;
                            }
                            
                            if (([mapping_value length] > precision) && precision != 0 )
                            {
                                mapping_value = [mapping_value substringToIndex:precision];
                            }
                            else
                            {
                                NSInteger scale = [appDelegate.dataBase getScaleValueForNumberField:target_field_name  objectName:target_object_name validationField:MSCALE];
                                
                                if (scale >= 0)
                                {
                                    BOOL iSNaN = [Utility isValueNotANumber:mapping_value];
                                    
                                    if ([mapping_value length] > 0 && (![mapping_value isEqualToString:@""]) && !iSNaN)
                                    {
                                        NSString *formattedNumber = [Utility  getFormattedString:mapping_value decimalPoint:scale];
                                        if ([formattedNumber length] > 0)
                                        {
                                            ...
                                            {
                                                mapping_value = newValue;
                                                [dict setObject:mapping_value forKey:target_field_name];
                                            }
                                            else
                                            {
                                                mapping_value = formattedNumber;
                                                [dict setObject:mapping_value forKey:target_field_name];
                                            }
                                        }
                        ...
                        
{
    ...
    NSDictionary *dict = [self getValueForFieldName:contextLookupFilter.lookupContext andLiteralValue:contextLookupFilter.sourceObjectName];
    
    NSString *contextFilterValue = [dict objectForKey:gFIELD_VALUE_VALUE];
    NSString *relatedObject = [dict objectForKey:gFIELD_RELATED_OBJECT_NAME];
    NSString *fieldDataType = [dict objectForKey:gFIELD_DATA_TYPE];
    NSString *fieldType = nil;
    NSString *contextFilterCriteria = @"";
    
    //condition for context to apply 1) query and context should not be nil and 2) the default switch should be ON.
    if([contextLookupFilter.lookupContext length] > 0 && contextLookupFilter.isDefaultOn && [contextLookupFilter.lookupQuery length] > 0)
    {
        SMLog(kLogLevelVerbose,@"[Context] : filter applied");
        NSString *local_id = @"";
        NSString *contextFilterWithoutBlankSpace = [contextFilterValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        //10691
        //check if the value is blank so that return all the results having blank values. But if boolean check for NO.
        if (([contextFilterWithoutBlankSpace length] == 0 || [contextFilterValue isKindOfClass:[NSNull class]])  && ![[fieldDataType lowercaseString] isEqualToString:@"boolean"]) {
            
            contextFilterCriteria = [NSString stringWithFormat:@"( %@ is NULL OR %@ = ' ' OR %@ = '' ) ", contextLookupFilter.lookupQuery,contextLookupFilter.lookupQuery,contextLookupFilter.lookupQuery];
        }
        else {
            if (![relatedObject isEqualToString:@""]) {
                
                //get local id
                local_id = [self getValueForField:@"local_id" objectName:relatedObject recordId:contextFilterValue andWhereField:@"Id"];
                if([local_id length] > 0 && ![local_id isEqualToString:@""]) {
                
                    //criteria if id is nil, please check for local_id
                    contextFilterCriteria = [NSString stringWithFormat:@"( %@ = '%@' OR %@ = '%@' ) ", contextLookupFilter.lookupQuery,contextFilterValue,contextLookupFilter.lookupQuery,local_id];
                }
                else {
                    contextFilterCriteria = [NSString stringWithFormat:@"( %@ = '%@' ) ", contextLookupFilter.lookupQuery,contextFilterValue];
                }
            }
            else {
                //Defect 10693
                if ([[fieldDataType lowercaseString] isEqualToString:@"boolean"]) {
                    
                    fieldType = [NSString stringWithFormat:@"%d",[contextFilterValue boolValue]];

                    //check for boolean value and also the value got from the page data.
                    contextFilterCriteria = [NSString stringWithFormat:@"( %@ = '%@' OR  %@ = '%@')", contextLookupFilter.lookupQuery,contextFilterValue,contextLookupFilter.lookupQuery,fieldType];
                }
                else if([[fieldDataType lowercaseString] isEqualToString:@"url"]) {
                    //10691 : prefix http:// needs to be removed.
                    if ([contextFilterValue length] > 0)
                    {
                        if ([contextFilterValue hasPrefix:@"http://"]) {
                            contextFilterValue = [contextFilterValue stringByReplacingOccurrencesOfString:@"http://" withString:@""];
                        }
                    }
                    contextFilterCriteria = [NSString stringWithFormat:@"( %@ = '%@' COLLATE NOCASE)", contextLookupFilter.lookupQuery,contextFilterValue];
                }
                else if ([[fieldDataType lowercaseString] isEqualToString:@"datetime"]) {
                   //10692
                    //date time format stored in db is not same as it is in page layout
                    ...
                    //path extention (.000Z) is not been appended, if the datetime field is populated from server.
                    //if datetime is updated from iPad the path extension remains which needs to be removed to make consistent.
                    
                    if (![[contextFilterValue pathExtension] isEqualToString:@""]) {
                        contextFilterValue = [contextFilterValue stringByDeletingPathExtension];
                    }
                    [dateformatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss"];

                    [dateformatter setTimeZone:[NSTimeZone timeZoneWithName:@"GMT"]];
                    NSDate *localeDate = [dateformatter dateFromString:contextFilterValue];
                    
                    [dateformatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss.SSSZ"];
                    
                    NSString *dateTimeValueForContext = [dateformatter stringFromDate:localeDate];
                    
                    [dateformatter release]; dateformatter = nil;
                    
                    contextFilterCriteria = [NSString stringWithFormat:@"( %@ = '%@' )", contextLookupFilter.lookupQuery,dateTimeValueForContext];
                }
                else {
                    //only one literal supported for Context lookup i.e " = "
                    contextFilterCriteria = [NSString stringWithFormat:@"( %@ = '%@' COLLATE NOCASE)", contextLookupFilter.lookupQuery,contextFilterValue];
                }
            }
        }
    ...
    NSMutableArray *_dictKeys = [NSMutableArray arrayWithObjects:@"DATA", @"SEQUENCE", @"SVMXDEV__Default_Lookup_Column__c", nil];
    ...
    //10651
    //if default_lookup_column is nil or empty use the Name field for the object.
        NSString *nameField = @"";
        NSString *isdefaultV = @"";
        NSString *isStandardV = @"";
        
        if ( [lookup_object_info count] == 0) {
            
            nameField = [self getNameFieldForObject:object];
            isStandardV = @"false"; //Since the default_lookup_column will be nil only for custom objects
            isdefault = @"true";    //By default its true
             NSMutableDictionary * dict  = [NSMutableDictionary dictionaryWithObjects:[NSArray arrayWithObjects:nameField,object,isStandardV,isdefaultV, nil] forKeys:lookUp_info_object_keys];
            [lookup_object_info addObject:dict];
        }
    ...
    if(lookupID == nil || [lookupID isEqualToString:@""])
    {
        ...
        {
            //krishna CONTEXTFILTER
            if([contextFilterCriteria length] > 0) {
            querystring2 = [NSString stringWithFormat:@"Select %@ , Id from '%@'  WHERE  Id  NOT NULL   AND Id != '' and %@ LIKE '%%%@%%' and %@ LIMIT %d", default_column_name, object, default_column_name, _searchForString,contextFilterCriteria, records];
                
                SMLog(kLogLevelVerbose,@"[Context] : %@",querystring2);
            }
            else {
		...
		{
            if([contextFilterCriteria length] > 0) {
                querystring2 = [NSString stringWithFormat:@"Select %@ , Id from '%@'  WHERE  Id  NOT NULL   AND Id != '' and %@ LIMIT %d", default_column_name, object,contextFilterCriteria, records];
                SMLog(kLogLevelVerbose,@"[Context] : %@",querystring2);
            }
            else {
        ...
        finalDict = [NSMutableDictionary dictionaryWithObjects:[NSArray arrayWithObjects:each_record,sequenceArray,default_column_name, nil] forKeys:_dictKeys];
    }
    else
    {
		//Sahana Fixed
        ...
        //10647 - Defect fix - By default name should be diaplyed in the lookup. Is name field is not configured in the lookup add name to thr results_array
        NSString * default_column_name = @"";
        for(NSDictionary * dict in lookup_object_info)
        {
			default_column_name = [dict objectForKey:LOOKUP_DEFAULT_LOOK_UP_CLMN];
			if ([default_column_name length] > 0)
            {
                if (![results_array containsObject:default_column_name])
                {
                    [results_array addObject:default_column_name];
                }
                break;
            }
        }
        ...
        if ([Utility isStringEmpty:advancedFilterString]) {
            if([contextFilterCriteria length] > 0)
            {
                querystring2 = [NSString stringWithFormat:@"Select %@ from '%@'  where %@ AND %@ LIMIT %d ", result_fieldNames, object, searchFieldNames, contextFilterCriteria, records];
                SMLog(kLogLevelVerbose,@"[Context] : %@",querystring2);
            }
            else
            {
            ...
            if([contextFilterCriteria length] > 0) 
            {
                 querystring2 = [NSString stringWithFormat:@"Select %@ from '%@'  where %@ AND %@ AND %@ LIMIT %d ", result_fieldNames, object, searchFieldNames,advancedFilterString, contextFilterCriteria, records];
                SMLog(kLogLevelVerbose,@"[Context] : %@",querystring2);
            }
            else
            {
        ...
        finalDict = [NSMutableDictionary dictionaryWithObjects:[NSArray arrayWithObjects:each_record, sequenceArray,default_display_column, nil] forKeys:_dictKeys];
    }
	[each_record release];
    }@catch (NSException *exp) {
...
//{
//    NSString * query = [NSString stringWithFormat:@"SELECT SVMXDEV_Field_Name_c FROM '%@' where Id = '%@' and default_desplay_column = '%@' ",SFCONFIG_DATA_TABLE ,lookup_id,@"true" ];
//    sqlite3_stmt * stmt ;
//    NSString * field_name = @"";
//    
//    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
...
-(NSString *)queryForExpression:(NSString *)expression_id forObject:(NSString *)object_name;
{
    ...
    //9703
    if (([expression isEqualToString:@""] || [expression length] == 0) && [expression_id length] > 0)
    {
        NSString * defaultExpression = [self getDefaultExpression:expression_id] ;
        
        if ([defaultExpression length] > 0)
        {
            expression = (NSString *)defaultExpression;
        }
    }
...
//9703 - Query sequence number
- (NSMutableString *)getDefaultExpression:(NSString *)expressionId
{
    NSString * query = [[NSString alloc] initWithFormat:@"SELECT DISTINCT component_sequence_number FROM 'SFExpressionComponent' where expression_id = '%@'", expressionId];
    
    NSMutableString * defaultExpression = [NSMutableString string];
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
    {
        while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
        {
            ...
            if(value != nil)
            {
                NSString * sequecnce  = [NSString stringWithUTF8String:value];
                if (sequecnce != nil && [sequecnce length] > 0)
                {
                    int sequenceNumber = [sequecnce intValue];
                    
                    if ([defaultExpression length] > 0)
                    {
                        [defaultExpression appendFormat:@" AND %d", sequenceNumber];
                    }
                    else
                    {
                        [defaultExpression appendFormat:@"%d", sequenceNumber];
                    }
                }
            }
        }
        ...
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
        {
            while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW) 
            {
                ...
                if([component_lhs length] != 0 && [component_operator length] != 0)
                {
                    ...
                    {
                        //krishna supporting multiple value for "=" 011186
                        operator_ = @" IN ";
                    }
    ...
    for(int p = 0 ; p < [final_Comonent_array count];p++)
    {
        ...
        for(int q = 0; q <[keys count]; q++)
        {
            ...
            if([temp_lhs isEqualToString:@"RecordTypeId"])
            {
				...
				{
                    //krishna : handling quotes in the rhs. 011250
                    rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                    ...
                    else if([operator isEqualToString:@" IN "] || [operator isEqualToString:@" NOT IN "])
                    {
                        //011186
                        NSArray *strArray = [rhs componentsSeparatedByString:@","];
                        NSMutableString *stringArray = [[NSMutableString alloc] init];
                        int count = 0;
                        for (NSString *str in strArray) {
                        
                            if (count == 0) {
                                [stringArray appendFormat:@"'%@'",str];
                            }
                            else {
                                [stringArray appendFormat:@",'%@'",str];
                            }
                        ...
                        if([operator isEqualToString:@" NOT IN "]) {
                            
                            //check for null and emplty string.
                            component_expression = [NSString stringWithFormat:@"( %@   in   (select  record_type_id  from SFRecordType where (record_type %@ ( %@ ) or trim(record_type) = '' or   record_type is null )))" ,lhs,operator, stringArray];
                            component_expression = [NSString stringWithFormat:@" ( %@ OR  trim(%@) = '' OR %@ is null )",component_expression,lhs,lhs];
                        }
                        else {
                            //support for multiple values in rhs. for equals.
                        component_expression = [NSString stringWithFormat:@" %@ in   (select  record_type_id  from SFRecordType where record_type %@ ( %@ ) )",lhs,operator,stringArray];
                        }
            ...
            {
                //krishna handle single quotes 011250
                rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                ...
                else if([operator isEqualToString:@" IN "] || [operator isEqualToString:@" NOT IN "])
                {
                    ...
                    component_expression = [NSString stringWithFormat:@" %@ in   (select  Id  from '%@' where Name %@  ( %@ ))",lhs,referenceToTable,operator,stringArray];
                    [stringArray release]; stringArray = nil;
                }
            ...
            {
                //krishna : handling quotes in the rhs. 011186
                rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                ...
                if( [operator isEqualToString:@" NOT IN "] ) {
                    //handling null for "!="
                     component_expression = [NSString stringWithFormat:@" ( %@ isnull or %@ %@ ( %@ )) ",lhs,lhs,operator,stringArray];
                }
                else {
                component_expression = [NSString stringWithFormat:@" %@ %@ ( %@ )",lhs,operator,stringArray];
                }
                [stringArray release]; stringArray = nil;
            }
                ...
                for(int q = 0; q <[keys_test count]; q++)
                {
                    ...
                    if([key isEqualToString:key_test])
                    {
                        ...
                        if(sequence_no != sequencetest_no)
                        {
                            ...
                            if ([data_type isEqualToString:@"reference"]) //9583
                            {
                                NSString * referenceToTable = [appDelegate.dataBase getReferencetoFiledForObject:object_name api_Name:lhs_];
                                
                                //krishna : handling quotes in the rhs.011250
                                rhs_ = [rhs_ stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                                
                                newExpression = [NSString stringWithFormat:@"('%@'.%@  in  (select  Id  from '%@' where Name %@ %@ ))" , object_name,lhs_,referenceToTable , operator_ ,rhs_];
...
-(NSMutableDictionary *)getWizardInformationForObjectname:(NSString *) objectName  record_id:(NSString *)record_id
{
    ...
    BOOL columnExists = [self ColumnExists:@"wizard_layout_row" tableName:SFWIZARD];
    
    if (columnExists)
    {
        query = [[NSString alloc] initWithFormat:@"SELECT wizard_id , expression_id , wizard_description, wizard_name, wizard_layout_row, wizard_layout_column  FROM '%@' where object_name = '%@'" ,SFWIZARD ,objectName];
    }
    else
    {
        query = [[NSString alloc] initWithFormat:@"SELECT wizard_id , expression_id , wizard_description, wizard_name  FROM '%@' where object_name = '%@'" ,SFWIZARD ,objectName];
    }
    ...
    NSString * sequence = nil; //9366 - Defect Fix
    ...
    if(columnExists)
    {
        keys = [NSArray arrayWithObjects:WIZARD_ID,WIZARD_DESCRIPTION, WIZARD_TITLE, WIZARD_SEQUENCE, nil];
    }
    else
    {
        keys = [NSArray arrayWithObjects:WIZARD_ID,WIZARD_DESCRIPTION, WIZARD_TITLE, nil];
    }
    ...
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
    {
        while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (columnExists)
            {
                int row = synchronized_sqlite3_column_double(stmt, 4);
                
                int column = synchronized_sqlite3_column_double(stmt, 5);
                ...
                sequence = [[NSString alloc] initWithFormat:@"%d%d", row, column];
            }
            ...
            if([expression_id length] != 0)
            {
                ...
                if(flag)
                {
                    if (columnExists)
                    {
                        NSMutableDictionary * dict = [NSMutableDictionary dictionaryWithObjects:[NSArray arrayWithObjects:wizard_id,wizard_description, wizard_title, [NSNumber numberWithInt:[sequence intValue]], nil] forKeys:keys];
    ...
    if (columnExists)
    {
        NSSortDescriptor * sortDescriptor = [[NSSortDescriptor alloc] initWithKey:WIZARD_SEQUENCE ascending:YES];
        NSArray * array = [wizard_array sortedArrayUsingDescriptors:[NSArray arrayWithObject:sortDescriptor]];
        if (array != nil &&  [array count] > 0)
        {
            [wizard_array removeAllObjects];
            [wizard_array addObjectsFromArray:array];
        }
    }
...
//    NSString *  parent_column_name = [self getchildInfoFromChildRelationShip:SFCHILDRELATIONSHIP ForChild:objectName field_name:@"parent_column_name"];
    ...
    /*10615 - Defct Fix
    Commenting the below piece of code, because the current implementation supports both local id and sf id reference for parent column name in the child record*/
    ...
    for (int f = 0; f < [json_allkeys count]; f++)
    {
        ...
        if ([id_type isKindOfClass:[NSDictionary class]])
        {
            ...
            {  // SMLog(kLogLevelVerbose,@"Querying");
                ...
                // Vipind-db-optmz
                if (objectName == nil || ([objectName length] == 0)) //10408 Defect Fix -
                {
                 ...
                 /* PLEASE NOTE - HERE objectName REFERES TO NOT THE EXACT NAME OF THE OBJECT. IT IS THE FIELD NAME OF THE OBJECT*/
                
                //Only update the dictonary if value is not empty - As discussed with Ranga


                NSString * referenceValue = [dict objectForKey:objectName];
                
                //10408 Defect Fix
                if (referenceValue != nil && [referenceValue length] > 0)
                {
                    ...
                    [lookUpDict setValue:referenceValue forKey:@"Name"];
                    // defect 8460
...
-(NSMutableArray *) getAllInsertRecords:(NSString *)operation_type
{
    ...
    NSMutableArray * array = [[NSMutableArray  alloc] initWithCapacity:0];
    // Vipindas field merge feature
    NSArray * keys = [NSArray arrayWithObjects:@"local_id", @"object_name", @"parent_object_name", @"parent_local_id" , @"time_stamp" , @"record_type", @"fields_modified", nil];
    
    sqlite3_stmt * statement;
    NSString * local_id = @"", * object_name = @"" , * parent_object_name = @"" , *parent_local_id = @"" , * time_stamp= @"" , * record_type = @"", *fields_modified = @"";
    ...
    // Vipindas field merge feature
    NSString * query  = [NSString stringWithFormat:@"SELECT  local_id , object_name  , parent_object_name, parent_local_id , timestamp , record_type, fields_modified  FROM '%@' WHERE operation = '%@' and sync_flag = 'false' and timestamp <= '%@' and record_sent = 'false'" , SFDATATRAILER_TEMP, operation_type,request_time];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String],-1 , &statement , nil)  ==  SQLITE_OK)
    {
        ...
        {
            // Vipindas field merge feature
            local_id = @"", object_name = @"" , parent_object_name = @"" , parent_local_id = @"" , time_stamp = @"" ,record_type = @"", fields_modified = @"";
            ...
            if(temp_record_type != nil)
                record_type = [NSString stringWithUTF8String:temp_record_type];
            // Vipindas field merge feature
            // fields_modified added as part of field merging
            char * temp_fields_modified = (char *) synchronized_sqlite3_column_text(statement, 6);
            if(temp_fields_modified != nil)
                fields_modified = [NSString stringWithUTF8String:temp_fields_modified];
            
            NSMutableDictionary * dict = [NSMutableDictionary dictionaryWithObjects:[NSArray arrayWithObjects:local_id, object_name , parent_object_name, parent_local_id , time_stamp,record_type, fields_modified,nil] forKeys:keys];
            [array addObject:dict];
        }
    }
...
-(void) insertdataIntoTrailerTableForRecord:(NSString *)local_id
                                      SF_id:(NSString *)sf_id
                                record_type:(NSString *)record_type
                                  operation:(NSString *)operation
                                object_name:(NSString *)object_name
                                  sync_flag:(NSString *)sync
                           parentObjectName:(NSString *)parentObjectName
                            parent_loacl_id:(NSString *)parent_local_id
                             webserviceName:(NSString *)webservice_name
                                  className:(NSString *)class_name
                                   synctype:(NSString *)sync_type
                              headerLocalId:(NSString *)header_localId
                                requestData:(NSMutableDictionary *)request_data
                                 finalEntry:(BOOL)isFinalCustomEntry

{
	...
	if (columnExists)
	{
		...
		{
			[self fillSyncRecordDictForRecordType:record_type
                                            SF_Id:sf_id
                                         local_id:local_id
                                   operation_type:operation
                                 final_dictionary:request_data
                                      object_naem:object_name
                               parent_object_name:parentObjectName
                                  parent_local_id:header_localId];
			return;
		}
...
-(void)copyTrailerTableToTempTrailerForOperationType:(NSString *)operation_type
{
    ...
    [[PerformanceAnalytics sharedInstance] observePerformanceForContext:[NSString stringWithFormat:@"copyTrailerTableToTempTrailerForOperationType : %@", operation_type]
                                                         andRecordCount:0];
    // Vipindas field merge feature
    NSString * query = [NSString stringWithFormat:@"INSERT INTO '%@' (timestamp ,  local_id , sf_id ,  record_type ,  operation ,  object_name ,  sync_flag  ,  parent_object_name   , parent_local_id   ,  record_sent,  fields_modified)   SELECT timestamp ,  local_id , sf_id ,  record_type ,  operation ,  object_name ,  sync_flag  ,  parent_object_name   , parent_local_id,  record_sent, fields_modified  from  '%@' WHERE operation = '%@'", SFDATATRAILER_TEMP, SFDATATRAILER, operation_type];
        ...
        if(prepare_ == SQLITE_OK)
        {
            if(synchronized_sqlite3_prepare_v2(appDelegate.db, [sql UTF8String], -1, &statement, nil) == SQLITE_OK)
            {
                ...
                {   
                    ...
                    {
                        //Radha - 11366 - Since we are supporting both localId or sfid in the "parent column name", it is same to comment the below code
                        /*if([record_type isEqualToString:DETAIL])
                        {
                            [final_dict removeObjectForKey:parent_column_name];
                        }*/
                        if ([object_Name isEqualToString:@"SVMXDEV__User_GPS_Log__c"]) {
                            BOOL isUser_GPS_LogTableDelete=[self DeleterecordFromTable:@"SVMXDEV__User_GPS_Log__c" Forlocal_id:local_id];
                            [appDelegate.dataBase deleteSequenceofTable:@"SVMXDEV__User_GPS_Log__c"];
                        ...
                        {
                            //Defect 11034,11325
                            final_dict = [[self updateEmptyFieldValuesForDict:final_dict objectName:Object_name_temp] retain];
...
// Vipindas field merge feature
- (BOOL)updateDataTrailerTableFieldsModifiedData:(NSString *)dataJsonString byLocalId:(NSString *)localId
{
    BOOL isSaved = NO;
    [appDelegate.dataBase beginTransaction];
...
//  NSString * updateStatement = [NSString stringWithFormat:@"UPDATE '%@' SET %@ = '%@'  WHERE Id = ?1", child_objectName,parent_column_name , parent_local_id];
     ...
     NSString * updateStatement = [NSString stringWithFormat:@" UPDATE '%@' SET fields_modified = ?1  WHERE local_id = ?2 AND operation = ?3", SFDATATRAILER];
     ...
     @try {
        ...
        int ret_val = synchronized_sqlite3_prepare_v2(appDelegate.db, [updateStatement UTF8String], strlen([updateStatement UTF8String]),  &bulkStmt, NULL);
         ...
         if (ret_val == SQLITE_OK)
         {
//            char * _tableName= [appDelegate convertStringIntoChar:SFDATATRAILER];
//            sqlite3_bind_text(bulkStmt, 1, _tableName, strlen(_tableName), SQLITE_TRANSIENT);
//         
            char * _dataJsonString = [appDelegate convertStringIntoChar:dataJsonString];
            sqlite3_bind_text(bulkStmt, 1, _dataJsonString, strlen(_dataJsonString), SQLITE_TRANSIENT);
         
            char * _localId = [appDelegate convertStringIntoChar:localId];
            sqlite3_bind_text(bulkStmt, 2, _localId, strlen(_localId), SQLITE_TRANSIENT);
         
            char * _operationType= [appDelegate convertStringIntoChar:UPDATE];
            sqlite3_bind_text(bulkStmt, 3, _operationType, strlen(_operationType), SQLITE_TRANSIENT);
         
            if (synchronized_sqlite3_step(bulkStmt) != SQLITE_DONE)
            {
                SMLog(kLogLevelError,@"Failure updateDataTrailerTableFieldsModifiedData - updateStatement => %@", updateStatement);
            }
            else
            {
                 isSaved = YES;
                //SMLog(kLogLevelVerbose,@" Success updateDataTrailerTableFieldsModifiedData - updateStatement => %@", updateStatement);
            }
         ...
         {
              SMLog(kLogLevelError,@"Failure prepare statement :%s, updateDataTrailerTableFieldsModifiedData - updateStatement => %@", sqlite3_errmsg(appDelegate.db), updateStatement);
         }
    ...
    char * err ;
    NSString * updateStatement = [NSString stringWithFormat:@"UPDATE '%@' SET fields_modified = '%@'  WHERE local_id = '%@' AND operation = '%@'",
                                  SFDATATRAILER,
                                  dataJsonString,
                                  localId,
                                  UPDATE];
    ...
    {
        SMLog(kLogLevelError,@"%@", updateStatement);
		SMLog(kLogLevelError,@"METHOD:updateDataTrailerTableFieldsModifiedData" );
		SMLog(kLogLevelError,@"ERROR IN UPDATING %s", err);
    }
    else
    {
...
- (BOOL)updateDataTrailerTempTableFieldsModifiedData:(NSString *)dataJsonString byLocalId:(NSString *)localId
{
    BOOL isSaved = NO;
    [appDelegate.dataBase beginTransaction];
    
    NSString * updateStatement = [NSString stringWithFormat:@" UPDATE '%@' SET fields_modified = ?1  WHERE local_id = ?2 AND operation = ?3", SFDATATRAILER_TEMP];
    ...
    @try {
        ...
        if (ret_val == SQLITE_OK)
        {
            ...
            if (synchronized_sqlite3_step(bulkStmt) != SQLITE_DONE)
            {
                SMLog(kLogLevelError,@"Failure updateDataTrailerTempTableFieldsModifiedData - updateStatement => %@", updateStatement);
            }
            else
            {
                isSaved = YES;
        ...
        {
            SMLog(kLogLevelError,@"Failure prepare statement :%s, updateDataTrailerTempTableFieldsModifiedData - updateStatement => %@", sqlite3_errmsg(appDelegate.db), updateStatement);
        }
...
- (NSString *)fieldsModifiedDataFromTrailorTableByLocalId:(NSString *)localId
{
    NSString * query = [NSString stringWithFormat:@"SELECT fields_modified FROM '%@' WHERE local_id = '%@' and operation = '%@'",
                        SFDATATRAILER,
                        localId,
                        UPDATE];
    
    sqlite3_stmt * stmt;
    NSString * storedModifiedFieldData = @"";
    
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK )
    {
        while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
        {
            char * tempData = (char *)synchronized_sqlite3_column_text(stmt, 0);
            
            if (tempData != nil)
            {
                storedModifiedFieldData= [NSString stringWithUTF8String:tempData];
            }
        }
    }
    
    synchronized_sqlite3_finalize(stmt);
    return storedModifiedFieldData;
}

- (NSString *)fieldsModifiedDataFromTrailorTableById:(NSString *)salesForceId
{
    NSString * query = [NSString stringWithFormat:@"SELECT fields_modified FROM '%@' WHERE sf_id = '%@' and operation = '%@'",
                        SFDATATRAILER,
                        salesForceId,
                        UPDATE];
    ...
    NSLog(@" TrailorTable query : %@", query);
...
// Vipindas field merge feature
- (NSString *)fieldsModifiedDataFromTrailorTempTableById:(NSString *)salesForceId
{
    NSString * query = [NSString stringWithFormat:@"SELECT fields_modified FROM '%@' WHERE sf_id = '%@' and operation = '%@'",
                        SFDATATRAILER_TEMP,
                        salesForceId,
    ...
    NSLog(@" TrailorTempTable query : %@", query);
...
- (NSString *)fieldsModifiedDataFromTrailorTempTableByLocalId:(NSString *)localId
{
    NSString * query = [NSString stringWithFormat:@"SELECT fields_modified FROM '%@' WHERE local_id = '%@' and operation = '%@'",
                        SFDATATRAILER_TEMP,
                        localId,
...
-(BOOL)DeleteUpdateRecordFromTable:(NSString *)object_name Forlocal_id:(NSString *)local_id
{
    ...
    NSString * delete_query = [NSString stringWithFormat:@"DELETE FROM '%@' WHERE local_id = '%@' AND operation = 'UPDATE'" ,object_name , local_id ];
...
-(NSMutableArray *)getAllobjectsApiNameFromSFObjectField
{
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1 , &statement , nil)  ==  SQLITE_OK)
    {
     ...
     NSLog(@"  objects_list : %@", [objects_list description]);
    [objects_list removeObject:@"User"];
     NSLog(@" After  objects_list : %@", [objects_list description]);
    synchronized_sqlite3_finalize(statement);
    ...
    @try
    {
                ...
                while (synchronized_sqlite3_step(stmt)  == SQLITE_ROW)
                {
                   ...
                   if ([component_rhs isEqualToString:SVMX_USER_TRUNK]) {
                    ...
                    if([component_lhs length] != 0 && [component_operator length] != 0)
                    {
                        ...
                        if([component_operator isEqualToString:@"eq"])
                        {
                            operator_  = @" IN ";//supporting multiple values 011186
                        }
                        else if([component_operator isEqualToString:@"gt"])
                        {
                            operator_  = @">";
                        ...
                        {
                            operator_  = @" NOT IN ";//supporting multiple values 011186
                        }
                        else if ([component_operator isEqualToString:@"ge"])
                        {
                            operator_  = @">=";
                        ...
                        {
                            operator_ = @" LIKE ";
                            //krishna handling single quotes 011250
                            component_rhs = [component_rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                            NSString * temp = [NSString stringWithFormat:@"%%%@%%",component_rhs];
                            component_rhs = [temp retain];
                        }
        ...
        for(int p = 0 ; p < [final_Comonent_array count];p++)
        {
            ...
            for(int q = 0; q <[keys count]; q++)
            {
                ...
                else if([data_type isEqualToString:@"reference"])
                {
                    ...
                    if (fieldCount > 1) {
                        ...
                        else if([operator isEqualToString:@" IN "] || [operator isEqualToString:@" NOT IN "])
                        {
                            ...
                            component_expression = [NSString stringWithFormat:@"  ( %@   in   (select  Id  from '%@' where ( Name %@ ( %@ )  OR Id %@  ( %@ ) )) OR %@   in   (select  local_id  from '%@' where (Name %@ ( %@ )  OR Id %@ ( %@ ))) )",lhs,referenceToTable,operator,stringArray,operator ,stringArray,lhs,referenceToTable , operator ,stringArray,operator ,stringArray];
                            [stringArray release]; stringArray = nil;
                        }
                        else
                        {
                ...
                {
                    //011186
                    //krishna : handling quotes in the rhs.
                    rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                ...
                {
                    //krishna handling single quote operator. 011250
                    rhs = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
                        ...
                        if([key isEqualToString:key_test])
                        {
                            ...
                            if(sequence_no != sequencetest_no)
                            {
                                ...
                                if ([Utility containsString:SVMX_USER_TRUNK inString:rhs_]) {
                                    ...
                                    component_expression = [component_expression stringByAppendingString:operator_];
                                    //krishna handling single quote operator. 011250
                                    rhs_ = [rhs stringByReplacingOccurrencesOfString:@"'" withString:@"''"];
...
- (NSDictionary *)getValueForFieldName:(NSString *)fieldName andLiteralValue:(NSString *)headerValue {
    ...
    if (isHeader == 1) {
        ...
        criteriaDictionary = [self filterCriteriaForContextFilter:fieldName fromHeaderSections:headerDictionary  andRecordId:recordId];
    }
    else if (isHeader == 2)
    {
        //Fix : header value (Context Source Object) was blank instead of CURRENT_RECORD
        if([headerValue isEqualToString:CURRENTRECORD_CONTEXTFILTER ] || [headerValue length] == 0 || [headerValue isEqualToString:@""])
        {
            NSArray *details = [currentPageDictionary objectForKey:gDETAILS];
            if ([details count] > detailIndexPath.section) {
                NSDictionary *detailDictionary = [details objectAtIndex:detailIndexPath.section];
                
                criteriaDictionary = [self filterCriteriaForDetailContextFilter:fieldName fromDetailsDictionary:detailDictionary andRecordId:detailIndexPath.row];
            }
...
//krishna CONTEXTFILTER
- (NSDictionary *) filterCriteriaForContextFilter:(NSString *)fieldName fromHeaderSections:(NSDictionary *)headerDictionary andRecordId:(NSString *)recordId
{
    NSMutableDictionary *criteriaDictionary = [NSMutableDictionary dictionary];
    ...
    //Display filter label in look up
    NSString *displayKey = @"";
    NSString *fieldLabel = @"";
    NSString *fieldType = @"";
    for (int counter = 0; counter < totalCount; counter++) {
        ...
        for (int innerCounter = 0; innerCounter < fieldLength; innerCounter++) {
            ...
            if ([fieldApiName isEqualToString:fieldName]) {
                ...
                fieldValue = ([fieldDictionary objectForKey:gFIELD_VALUE_KEY]!=nil) ? [fieldDictionary objectForKey:gFIELD_VALUE_KEY] : @"";
                relatedObjectName = ([fieldDictionary objectForKey:gFIELD_RELATED_OBJECT_NAME] != nil) ? [fieldDictionary objectForKey:gFIELD_RELATED_OBJECT_NAME] : @"";
                displayKey = ([fieldDictionary objectForKey:gFIELD_VALUE_VALUE] != nil) ? [fieldDictionary objectForKey:gFIELD_VALUE_VALUE]: @"";
                fieldLabel = ([fieldDictionary objectForKey:gFIELD_LABEL] != nil) ? [fieldDictionary objectForKey:gFIELD_LABEL] : @"" ;
                fieldType =  ([fieldDictionary objectForKey:gFIELD_DATA_TYPE] != nil) ? ([fieldDictionary objectForKey:gFIELD_DATA_TYPE]):@"";
                
                [criteriaDictionary setObject:fieldValue forKey:gFIELD_VALUE_VALUE];
                [criteriaDictionary setObject:relatedObjectName forKey:gFIELD_RELATED_OBJECT_NAME];
                [criteriaDictionary setObject:displayKey forKey:gFIELD_VALUE_KEY];
                [criteriaDictionary setObject:fieldLabel forKey:gFIELD_LABEL];
                [criteriaDictionary setObject:fieldType forKey:gFIELD_DATA_TYPE];
            }
        }
    }
    return criteriaDictionary;
}
-(NSDictionary *)filterCriteriaForDetailContextFilter:(NSString *)fieldName fromDetailsDictionary:(NSDictionary *)detailDictionary andRecordId:(NSInteger )recordIndex{
    ...
    if ([valueArray count] > recordIndex) {
        ...
        for (int counter = 0; counter < [fieldsValueArray count]; counter++) {
            ...
            if ([fieldApiName isEqualToString:fieldName]) {
                fieldValue = ([fieldDictionary objectForKey:gVALUE_FIELD_VALUE_KEY] != nil) ? [fieldDictionary objectForKey:gVALUE_FIELD_VALUE_KEY] : @"";
                displayKey = ([fieldDictionary objectForKey:gVALUE_FIELD_VALUE_VALUE] != nil) ? [fieldDictionary objectForKey:gVALUE_FIELD_VALUE_VALUE] : @"";
