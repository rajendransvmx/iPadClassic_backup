...
- (void)viewDidLoad
{
    ...
    [multiAddButton setTitle:[Utility getValueForTagFromTagDict:DONE_BUTTON_TITLE] forState:UIControlStateNormal];
    if ([Utility isIOS8]) {
        _tableView.frame = CGRectMake(_tableView.frame.origin.x, CGRectGetHeight(multiAddButton.frame), CGRectGetWidth(_tableView.frame), CGRectGetHeight(_tableView.frame ));
        [searchBar sizeToFit];
...
-(void) searchBarSearchButtonClicked:(UISearchBar *)_searchBar
{
	@try{
        ...
        if(appDelegate.isWorkinginOffline)
        {
            ...
            //Shubha :Defect 11351
            NSDictionary *dict = [appDelegate.databaseInterface getLookupDataFromDBWith:self.searchId referenceTo:objectName searchFor:_searchBar.text withPreFilters:nil andFilters:nil lookupContextFilter:nil];
        ...
        SMLog(kLogLevelError,@"Exception Name MultiAddLookupView :searchBarSearchButtonClicked %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason MultiAddLookupView :searchBarSearchButtonClicked %@",exp.reason);
    }
}
...
-(void) searchBarcodeResult:(NSString *) searchText
{
    if(appDelegate.isWorkinginOffline)
    {
        ...
        //Shubha - defect 11351
        NSDictionary *dict = [appDelegate.databaseInterface getLookupDataFromDBWith:self.searchId referenceTo:objectName searchFor:searchText withPreFilters:nil andFilters:nil lookupContextFilter:nil];
...
- (void) setLookupData:(NSDictionary *)lookupDictionary
{
    ...
    if(appDelegate.isWorkinginOffline)
    {
        ...
        for (int i = 0; i < [[lookupData objectForKey:@"DATA"] count]; i++)
        {
            ...
            for(NSDictionary *dict in eachLookUp)
            {
                ...
                else if([[dict objectForKey:@"key"] isEqualToString:defaultLookupColumn])         //Defect 11351 :Shubha
                {
    ...
    else
    {
        ...
        SMLog(kLogLevelError,@"Exception Name MultiAddLookupView :setLookupData %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason MultiAddLookupView :setLookupData %@",exp.reason);
    }
    @finally {
        [self reloadData];
...
{
    SMLog(kLogLevelVerbose,@"Mutable Array = %@",mappingArray);
    @try{
    ...
    SMLog(kLogLevelVerbose,@"Objects Selected  = %@",objectSelected);
    [delegate addMultiChildRows:objectSelected forIndex:index];
	}@catch (NSException *exp) {
	SMLog(kLogLevelError,@"Exception Name MultiAddLookupView :doneButtonClicked %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason MultiAddLookupView :doneButtonClicked %@",exp.reason);
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	SMLog(kLogLevelError,@"Exception Name MultiAddLookupView :cellForRowAtIndexPath %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason MultiAddLookupView :cellForRowAtIndexPath %@",exp.reason);
    }
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SMLog(kLogLevelVerbose,@"Mapping Array = %@",mappingArray);
	[tableView deselectRowAtIndexPath:indexPath animated:YES];
    }@catch (NSException *exp) {
        SMLog(kLogLevelError,@"Exception Name MultiAddLookupView :didSelectRowAtIndexPath %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason MultiAddLookupView :didSelectRowAtIndexPath %@",exp.reason);
    }
}
...
- (void) selectObjectValue:(NSArray *)objectHistory
{
	...
	SMLog(kLogLevelError,@"Exception Name MultiAddLookupView :selectObjectValue %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason MultiAddLookupView :selectObjectValue %@",exp.reason);
    }
