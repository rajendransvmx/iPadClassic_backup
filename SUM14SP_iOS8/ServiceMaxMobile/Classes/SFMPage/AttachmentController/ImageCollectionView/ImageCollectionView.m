...
- (void) dataFromCapturedImage:(NSData *)capturedImageData withExtension:(NSString *)extension{
    ...
    if(canAttach)
    {
        ...
        [attachmentInfo setObject:extension forKey:@"Extension"];
...
-(void)videoCaptured:(NSURL*)inputPathURL isCaptured:(BOOL)isCaptured picker:(UIImagePickerController *)picker
{
    ...
    if (sizeinMB > 25) {
        ...
        if(canAttach)
        {
            [self saveAttachmentInfoDictWithId:local_id];
        }
    }
    else {
...
-(void)saveAttachmentInfoDictWithId:(NSString*)localId
{
    NSMutableDictionary *attachmentInfo=[[NSMutableDictionary alloc]init];
    [attachmentInfo setObject:IMAGES_DICT forKey:@"AttachmentType"];
    [attachmentInfo setObject:NEW_ATTACHMENT forKey:@"Operation"];
    [attachmentInfo setObject:localId forKey:@"local_id"];
...
- (void) imagePickerController: (UIImagePickerController *) picker didFinishPickingMediaWithInfo:(NSDictionary *) info {
    ...
    else if([mediaType isEqualToString:@"public.image"])
    {
        if([info objectForKey:UIImagePickerControllerReferenceURL] == nil)
        {
             ...
             * Pushpak defect 014109
             */
            if(self && [self respondsToSelector:@selector(dataFromCapturedImage:withExtension:)]) {
                [self performSelector:@selector(dataFromCapturedImage:withExtension:) withObject:dataToSaveFromImage withObject:nil];
            }
            ...
            [library assetForURL:[info valueForKey:UIImagePickerControllerReferenceURL] resultBlock:^(ALAsset *asset) {
                ...
                if(self && [self respondsToSelector:@selector(dataFromCapturedImage:withExtension:)]) {
                    [self performSelector:@selector(dataFromCapturedImage:withExtension:) withObject:dataToSaveFromImage withObject:extension];
                }
