...
#import "PageModificationObserver.h" //// Vipindas field merge feature

/*Accessibility changes*/
#import "AccessibilityModalViewCalConstants.h"
...
- (void)viewWillLayoutSubviews
{
    ...
    if( isShowingDailyView )
    {
        ...
        if(_stringNumber >=  (KMinPkgForScheduleEvents * 100000))
        {
            ...
            {
                appDelegate.SyncProgress.frame = CGRectMake(776, 25, 35, 35);
            }
            else
            {
        ...
        else
        {
            if ([[[UIDevice currentDevice] systemVersion] integerValue] >= 8)
            {
                appDelegate.SyncProgress.frame = CGRectMake(834, 25, 35, 35);
            }
            else
            {
    ...
    else
    {
        ...
        if(_stringNumber >=  (KMinPkgForScheduleEvents * 100000))
        {
            if ([[[UIDevice currentDevice] systemVersion] integerValue] >= 8)
            {
                appDelegate.SyncProgress.frame = CGRectMake(826, 25, 35, 35);
            }
            else
            {
        ...
        else
        {
            if ([[[UIDevice currentDevice] systemVersion] integerValue] >= 8)
            {
                appDelegate.SyncProgress.frame = CGRectMake(876, 25, 35, 35);
            }
            else
            {
    ...
    if (![Utility notIOS7]) {
        CGRect syncFrmae = appDelegate.SyncProgress.frame;
        syncFrmae.origin.y = 5 + 20;
        appDelegate.SyncProgress.frame = syncFrmae;
    }
...
- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
     SMLog(kLogLevelVerbose,@"MODAL VIEW WILL APPEAR");
    if ([Utility getRefreshCalendarView]) {
        SMLog(kLogLevelVerbose,@"Reloading the events on save");
    ...
    [self syncButtonframechnage];
}
// IOS 8 changes for UI
-(void)syncButtonframechnage{
    if( isShowingDailyView )
    {
        //sahana  dec 14th
        NSString * version = [appDelegate serverPackageVersion];
...
- (void)viewDidLoad
{
    ...
    //Changed by shubha :010703 defect 14th may 2014
    
    dayString =  [appDelegate.wsInterface.tagsDictionary objectForKey:SEGMTDAYBTN];
    
    //it will give truncated string if string length is too large
    
    dayString = [[self getTruncatedStringFromString:dayString] retain];
    
    weekString = [appDelegate.wsInterface.tagsDictionary objectForKey:SEGMTWEEKBTN];
    
    weekString = [[self getTruncatedStringFromString:weekString] retain];
    
    //to get width of the text
    
    UIFont *font = [UIFont boldSystemFontOfSize:14];
    CGSize textSize = [dayString sizeWithFont:font];
    
    //Calculating xPosition for text inside image.(111 is image length).
    
    xValueForDay = (111 - textSize.width) / 2;
    
    textSize = [weekString sizeWithFont:font];
    xValueForWeek = (111 - textSize.width) / 2;
    ...
    if([Utility notIOS7])
    {
        ...
        //Changed by shubha :010703 defect.Draw text inside image.
        
        [segmentButton setImage:[UIImage drawText:dayString inImage:[UIImage imageNamed:@"iService-Day-Button-Down-State_New.png"] atPoint:CGPointMake(xValueForDay, 5)] forSegmentAtIndex:0];
        [segmentButton setImage:[UIImage drawText:weekString inImage:[UIImage imageNamed:@"iService-Week-Button-Up-State_New.png"] atPoint:CGPointMake(xValueForWeek, 5)] forSegmentAtIndex:1];
...
{
    /*Accessibility changes*/
    userInfoButton.isAccessibilityElement = YES;
    [userInfoButton setAccessibilityIdentifier:kAccDisplayUser];
    
    HomeButton.isAccessibilityElement = YES;
    [HomeButton setAccessibilityIdentifier:kAccHomeButton];
    
    showMapButton.isAccessibilityElement = YES;
    [showMapButton setAccessibilityIdentifier:kAccMapButton];
    
    todayBtn.isAccessibilityElement = YES;
    [todayBtn setAccessibilityIdentifier:kAccTodayButton];
    
    slider.isAccessibilityElement = YES;
    [slider setAccessibilityIdentifier:kAccSliderView];
    
    incrDateBtn.isAccessibilityElement = YES;
    [incrDateBtn setAccessibilityIdentifier:kAccIncDateBtn];
    
    decrDateBtn.isAccessibilityElement = YES;
    [decrDateBtn setAccessibilityIdentifier:kAccDecDateBtn];
    
    helpButton.isAccessibilityElement = YES;
    [helpButton setAccessibilityIdentifier:kAccHelpButton];
    
    Add_event_Button.isAccessibilityElement = YES;
    [Add_event_Button setAccessibilityIdentifier:kAccAddEvent];
    
    taskButton.isAccessibilityElement = YES;
    [taskButton setAccessibilityIdentifier:kAccTask];
}
...
- (void) didInternetConnectionChange:(NSNotification *)notification
{
    ...
    if (isReachable)
    {
        SMLog(kLogLevelVerbose,@"ModalViewController Internet Reachable");
        // Special Handling
        // Enable slider
    ...
    {
        SMLog(kLogLevelWarning,@"ModalViewController Internet Not Reachable");
        // Special Handling
        // Disable slider
...
{
    SMLog(kLogLevelVerbose,@"Removed Crash Protector");
...
{
    SMLog(kLogLevelVerbose,@"SetupTask");
...
{
    SMLog(kLogLevelVerbose,@"didQueryTasks");
    @try{
    ...
    [self performSelector:@selector(didGetAllTasks) withObject:nil afterDelay:0.3];
        ...
        SMLog(kLogLevelError,@"Exception Name ModalViewController :didQueryTasksForDate %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason ModalViewController :didQueryTasksForDate %@",exp.reason);
...
- (void) movedEvent:(EventViewController *)event
{
    ...
    SMLog(kLogLevelVerbose,@"%@", startEndTime);
    ...
    updatestartDateTime = [updatestartDateTime stringByReplacingOccurrencesOfString:@"Z" withString:@""];
        ...
        SMLog(kLogLevelError,@"Exception Name ModalViewController :movedEvent %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason ModalViewController :movedEvent %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];;
    }
} 
...
- (void) refresh
{
    ...
    /* Shravya - Fixing the crash on reload of calendar view */
    [self performSelectorOnMainThread:@selector(disableUI) withObject:Nil waitUntilDone:YES];
...
- (void) reloadCalendar
{
    ...
    [self enableUI];
    SMLog(kLogLevelVerbose,@"currentdate =  %@", currentDate);
...
{
    SMLog(kLogLevelVerbose,@"Updated objects");
...
- (IBAction) ShowMap
{
    ...
    SMLog(kLogLevelVerbose,@"%@", appDelegate.workOrderEventArray);
    ...
    else
    {
        ...
        FirstDetailViewController * mapView = nil;
        if ([Utility notIOS7]) {
            mapView = [[FirstDetailViewController alloc] initWithNibName:@"FirstDetailView" bundle:nil];
        }
        else{
            mapView = [[FirstDetailViewController alloc] initWithNibName:@"FirstDetailViewios7" bundle:nil];
        }
	...
	SMLog(kLogLevelError,@"Exception Name ModalViewController :ShowMap %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason ModalViewController :ShowMap %@",exp.reason);
	 [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (IBAction) ToggleLandscapeView
{
    ...
    if ((weekView != nil) && !isShowingDailyView)
    {
        ...
        if([Utility notIOS7])
        {
            //Changed by shubha :010703 defect
            
            [segmentButton setImage:[UIImage drawText:dayString inImage:[UIImage imageNamed:@"iService-Day-Button-Up-State_New.png"] atPoint:CGPointMake(xValueForDay, 5)] forSegmentAtIndex:0];
            [segmentButton setImage:[UIImage drawText:weekString inImage:[UIImage imageNamed:@"iService-Week-Button-Down-State_New.png"] atPoint:CGPointMake(xValueForWeek, 5)] forSegmentAtIndex:1];
    ...
    if (isShowingDailyView)
    {
        ...
        if (weekView == nil)
        {
            ...
            if ([Utility notIOS7]) {
                ...
                weekView.view.frame = CGRectMake(0, 64, weekView.view.frame.size.width, weekView.view.frame.size.height);
            }
        ...
        if([Utility notIOS7])
        {
            //Shubha :010703 defect 14th may 2014
...
//            [segmentButton setImage:[UIImage imageNamed:@"iService-Week-Button-Up-State.png"] forSegmentAtIndex:1];
    ...
    /*ios7_support shravya-navbar*/
    if (![Utility notIOS7]) {
        CGRect syncFrame = appDelegate.SyncProgress.frame;
        syncFrame.origin.y = 5 + 20;
        appDelegate.SyncProgress.frame = syncFrame;
    }
}
...
{
    // SMLog(kLogLevelVerbose,@"willRotateToInterfaceOrientation");
}

- (void)willAnimateFirstHalfOfRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    // SMLog(kLogLevelVerbose,@"willAnimateFirstHalfOfRotationToInterfaceOrientation");
}

- (void)didAnimateFirstHalfOfRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation
{
    // SMLog(kLogLevelVerbose,@"didAnimateFirstHalfOfRotationToInterfaceOrientation");
}

- (void)willAnimateSecondHalfOfRotationFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation duration:(NSTimeInterval)duration
{
    // SMLog(kLogLevelVerbose,@"willAnimateSecondHalfOfRotationFromInterfaceOrientation");
}

- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation duration:(NSTimeInterval)duration
{
    // SMLog(kLogLevelVerbose,@"willAnimateRotationToInterfaceOrientation");
    /*ios7_support shravya-navbar*/
    if (![Utility notIOS7]) {
...
- (void) RefreshLandscapeEventPane
{
    ...
    [leftPane addSubview:calendar.view];
	//  SMLog(kLogLevelVerbose,@"%@", eventViewArray);
}
...
- (void) setDate:(NSUInteger)date
{
	...
	SMLog(kLogLevelVerbose,@" SetDate - %@",currentDate);
    SMLog(kLogLevelVerbose,@"%d", date);
	...
	if (didFirstTimeLoad == TRUE)
	{
        ...
        SMLog(kLogLevelVerbose,@"%@", week_bounds);
	}
    ...
    appDelegate.wsInterface.eventArray = [appDelegate.calDataBase GetEventsFromDBWithStartDate:[currentDateRange objectAtIndex:0] endDate:[currentDateRange objectAtIndex:1]];
        ...
        [currentDateRange release];
    SMLog(kLogLevelVerbose,@"app = %@", appDelegate.wsInterface.eventArray);
    ...
    if ( (appDelegate.wsInterface.eventArray != nil ) && [appDelegate.wsInterface.eventArray count] > 0 )
    {
        ...
        if ([self isDate:currentDate inRange:appDelegate.wsInterface.currentDateRange])
        {
            ...
            if ([appDelegate.wsInterface.rescheduleEvent isEqualToString:@"SUCCESS"])
            {
                ...
                SMLog(kLogLevelVerbose,@"%@ %@", appDelegate.wsInterface.startDate, appDelegate.wsInterface.endDate);
            }
    ...
    else
    {
        ...
        SMLog(kLogLevelError,@"Exception Name ModalViewController :setDate %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason ModalViewController :setDate %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    }
}
...
{
    SMLog(kLogLevelVerbose,@"%@", appDelegate.wsInterface.eventArray);
    ...
    for ( int i = 0; i < [appDelegate.wsInterface.eventArray count]; i++ )
    {
        ...
        SMLog(kLogLevelVerbose,@"%@",activtyDate);
        ...
        SMLog(kLogLevelVerbose,@"Activitydatetime = %@", activityDateTime);
        
		//    NSString * eventDate = [activityDateTime substringToIndex:10];
		//    SMLog(kLogLevelVerbose,@"eventDate = %@", eventDate);
        ...
        SMLog(kLogLevelVerbose,@"Enddatetime = %@", endDateTime);
        ...
        SMLog(kLogLevelVerbose,@"%@", [dict objectForKey:WHATID]);
        ...
        if ( [_date isEqualToString:eventDate] == TRUE )
        {
            ...
            SMLog(kLogLevelVerbose,@"%@ %@", [rightPane description], events.processName);
        ...
        SMLog(kLogLevelError,@"Exception Name ModalViewController :setEventsView %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason ModalViewController :setEventsView %@",exp.reason);
         [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    @finally
    {
...
{
    SMLog(kLogLevelVerbose,@"swiped on Event");
...
//        [self SFMeditEvent:gesture_view.recordId];
        ...
        SMLog(kLogLevelVerbose,@"its a view controller");
...
- (void) showSFMForWeek:(NSDictionary *)event
{
	...
	SMLog(kLogLevelError,@"Exception Name ModalViewController :showSFMForWeek %@",exp.name);
	SMLog(kLogLevelError,@"Exception Reason ModalViewController :showSFMForWeek %@",exp.reason);
	[appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    @try
    {
        ...
        // if ([touch view] == eventView.view)
        {
            for (int i = 0; i < [eventViewArray count]; i++)
            {
                self.eventView = [eventViewArray objectAtIndex:i]; ////8835
                if ([touch view] == eventView.view)
                {
    ...
    {
        SMLog(kLogLevelVerbose,@"touchesBegan: Caught & Handled Exception.");
        [activity stopAnimating];
    }
    @finally
    {
        SMLog(kLogLevelVerbose,@"touchesBegan: Finally Handled Exception.");
        [activity stopAnimating];
    }
}
...
- (void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([touch view] == rightPane)
    {
        ...
        // // SMLog(kLogLevelVerbose,@"Right pane y = %f", rightPane.frame.origin.y);
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event 
{
        ...
        if ([touch view] == calEventView.view)
        {
            ...
            else
            {
                ...
                if(Continue_rescheduling)
                {
                    ...
                    {
                        SMLog(kLogLevelVerbose,@"%@", updatestartDateTime);
                        SMLog(kLogLevelVerbose,@"%@", calEventView.eventId);
                        
                        // Vipindas field merge feature
                        PageModificationObserver *eventModificationObserver = nil;
                        
                        BOOL shouldMakeServiceCall = NO;
                        eventModificationObserver = [[PageModificationObserver alloc] initWithModifiedEventStartDate:updatestartDateTime
                                                                                                             endDate:updateendDateTime
                                                                                                            recordId:calEventView.local_id
                                                                                                             andSfId:calEventView.eventId];
                        
                        
                        [appDelegate.calDataBase updateMovedEventWithStartTime:updatestartDateTime
                                                                       EndDate:updateendDateTime
                                                                      RecordID:calEventView.eventId
                                                                 event_localId:calEventView.local_id];
                        
                        [eventModificationObserver doDataComparison];
                        [eventModificationObserver autorelease];
                        ...
                        NSMutableDictionary * eventDict = [NSMutableDictionary dictionaryWithObjects:[NSArray arrayWithObjects:updatestartDateTime,updateendDateTime, updatestartDateTime,nil] forKeys:[NSArray arrayWithObjects:@"StartDateTime", @"EndDateTime",@"ActivityDateTime",nil]];
                        [appDelegate.databaseInterface successiveSyncEntryForLocalId:local_id sfId:calEventView.eventId operation:UPDATE recordType:MASTER objName:@"Event" parentObjName:@"" parentLocalId:@"" headerLocalId:@"" syncType:AGRESSIVESYNC dataDict:eventDict  syncFlag:@"false" valuemapping:YES];
						...
						//  if(!does_exists)
                        {
							//Sync_Override
                            // Vipindas field merge feature
                           if ([appDelegate shouldSaveChanges:[eventModificationObserver hasDataModified]])
                           {
                            shouldMakeServiceCall = YES;
                            [appDelegate.databaseInterface  insertdataIntoTrailerTableForRecord:local_id
                                                                                          SF_id:calEventView.eventId
                                                                                    record_type:MASTER
                                                                                      operation:UPDATE
                                                                                    object_name:@"Event"
                                                                                      sync_flag:@"false"
                                                                                ...
                                                                                parent_loacl_id:@""
                                                                                 webserviceName:@""
                                                                                  ...
                                                                                  headerLocalId:local_id
                                                                                    requestData:nil
                                                                                     finalEntry:NO];
                               [eventModificationObserver updateModifiedFieldsData];
                           }
                        }
                        
                       // if (shouldMakeServiceCall)
                        {
    ...
    @catch (NSException *exception)
    {
        SMLog(kLogLevelVerbose,@"touchesEnded: Caught & Handled Exception.");
    }
    @finally
    {
        SMLog(kLogLevelVerbose,@"touchesEnded: Finally Handled Exception.");
    }
	[self enableUI];
}
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    @catch (NSException *exception)
    {
        SMLog(kLogLevelVerbose,@"touchesCancelled: Caught & Handled Exception.");
    }
    @finally
    {
        SMLog(kLogLevelVerbose,@"touchesCancelled: Finally Handled Exception.");
    }
}
...
- (void) showSFMWithDayEvent:(NSDictionary *)event
{
    ...
    else
    {
        ...
        SMLog(kLogLevelError,@"Exception Name ModalViewController :showSFMWithDayEvent %@",exp.name);
        SMLog(kLogLevelError,@"Exception Reason ModalViewController :showSFMWithDayEvent %@",exp.reason);
        [appDelegate CustomizeAletView:nil alertType:APPLICATION_ERROR Dict:nil exception:exp];
    }
    @finally {
    [activity stopAnimating];
...
- (NSString*)getTruncatedStringFromString:(NSString*)string {
    
    NSString *truncatedString = [[[NSString alloc] init]autorelease];
    ...
    if (len > 13) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if(i > 10){
                ...
                truncatedString = [truncatedString stringByAppendingString:[NSString stringWithFormat: @"%C", character]];
            }
        }
    } else {
