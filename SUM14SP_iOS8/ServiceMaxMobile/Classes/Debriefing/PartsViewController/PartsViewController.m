...
- (void) getParts:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
	...
	for (int i = 0; i < [array count]; i++)
	{
		...
		//SMLog(kLogLevelVerbose,@"SVMXC__Product__r.Name = %@", [[[[obj fields] objectForKey:@"SVMXC__Product__r"] fields] objectForKey:@"Name"] );
		SMLog(kLogLevelVerbose,@"SVMXC__Product__c = %@", [[obj fields] objectForKey:@"SVMXC__Product__c"] );
