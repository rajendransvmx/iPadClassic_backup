...
- (void)main {
    @synchronized([self class]) {
        @autoreleasepool {
            ...
            NSDate *requestedTime = [NSDate date];
            ...
            [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
                //SXLogDebug(@"Successfully downloaded file to %@", filePathLocal);
                SXLogDebug(@"Successfully  file downloaded");
                ...
                SXLogInfo(@"%@ req-f latency : %f sec", self.eventName,[[NSDate date] timeIntervalSinceDate:requestedTime]);

                SXLogError(@"Error: %@", error);
...
- (NSString*)urlByType:(RequestType)type andFileModel:(FileModel *)fileModel
{
    ...
    if (type == RequestTypeChatterUserImage) {
        return url = fileModel.suffixUrl;
    }
    else {
        switch (type) {
