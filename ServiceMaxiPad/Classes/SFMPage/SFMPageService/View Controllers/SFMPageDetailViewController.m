...
@property (strong, nonatomic) IBOutlet UIButton *selectedSectionButton;
...
- (void)setPageDetailChildViewController:(UIViewController *)childViewController
{
    ...
    if (newViewController != nil) {
        ...
        [self transitionFromViewController:currentViewController toViewController:newViewController duration:0 options:UIViewAnimationOptionTransitionCrossDissolve animations:nil completion:^(BOOL finished) {
...
-(void)registerForPopOverDismissNotification
{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(dismissPopoverPagedetailView)
...
- (void)dismissPopoverPagedetailView
{
...
{
    if ([self.descPopOver isPopoverVisible] &&
        self.descPopOver) {
        
        [self.descPopOver dismissPopoverAnimated:YES];
        self.descPopOver = nil;
    }
}
