...
@property (nonatomic, strong)NSMutableArray *rtPicklistFields;
...
- (void)showMultiSelectPopover:(NSString *)objectName
{
    ...
    multiSelectController.dataSource =[NSMutableArray arrayWithArray:[self getPickListArray:objectName andFiledName:self.pageFieldData.fieldName]] ;
...
{
    NSArray *dependentPicklistFields = nil;
    NSDictionary *defaultValueDict = nil;
    
    if ([self.pageFieldData.fieldName isEqualToString:kSfDTRecordTypeId] && [self.pageFieldData.dataType isEqualToString:kSfDTReference]) {
        dependentPicklistFields = [self dependentPicklistsForRecordTypeFields:indexPath recordTypeID:recordTypeId];
        
        defaultValueDict = [self getDeFaultValuesForRTDependentFields:recordTypeId];
    }
    else {
        dependentPicklistFields = [self dependentPicklistsForField:self.pageFieldData.fieldName indexPath:indexPath];
    }
    
    if ([dependentPicklistFields count] > 0) {
        if([self.viewControllerDelegate respondsToSelector:@selector(clearDependentFields:dataDict:)]) {
            [self.viewControllerDelegate clearDependentFields:dependentPicklistFields dataDict:defaultValueDict];
        }
    }
    
    self.rtPicklistFields = nil;
...
- (NSArray *)dependentPicklistsForRecordTypeFields:(NSIndexPath *)indexPath recordTypeID:(NSString *)recordTypeId
{
    NSMutableArray *fieldArray = [NSMutableArray new];
    
    if (self.rtPicklistFields == nil) {
        self.rtPicklistFields = [NSMutableArray new];
    }
    
    NSArray *dependentPicklist = [self.pageManager getRTDependentPicklistInfoForobject:self.objectName
    ...
    if (self.isChild) {
        ...
        for (SFMPageField *pageField in  detailLayout.detailSectionFields)
        {
            if ([dependentPicklist containsObject:pageField.fieldName])
            {
                [fieldArray addObject:pageField];
                [self.rtPicklistFields addObject:pageField.fieldName];
            }
        }
    }
    else {
...
- (NSDictionary *)getDeFaultValuesForRTDependentFields:(NSString *)recordTypeId
{
    if ([self.rtPicklistFields count] > 0) {
        return [self.pageManager getDefautValueForRTDepFields:self.rtPicklistFields
...
-(void)registerForPopOverDismissNotification
{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(dismissPopoverEditHandler)
...
- (void)dismissPopoverEditHandler
{
