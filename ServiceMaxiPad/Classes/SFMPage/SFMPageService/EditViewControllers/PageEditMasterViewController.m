        ...
        case SFMEditPageMasterSectionTypeAttachment:
            if ([self isAttachmentEnabled]) {
                if (indexPath.row == 0 ) {
                    ...
                    title = [tagManager tagByName:kTag_ImagesAndVideo];//kAttachmentImagesAndVideos;
                }
            }
            break;
        default:
...
- (id)attachmentViewControllerForIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([[self rowTitleForIndexPath:indexPath] isEqualToString:[[TagManager sharedInstance] tagByName:kTag_ImagesAndVideo]])
    {
        ...
        if ([StringUtil isStringEmpty:parentId]) {
            parentId = self.sfmPage.recordId;
            AttachmentLocalModel *localModel = [[AttachmentLocalModel alloc] init];
            localModel.parentLocalId = self.sfmPage.recordId;
            localModel.parentObjectName = self.sfmPage.objectName;
            [AttachmentHelper saveAttachmentLocalModelToDB:localModel];
        }
...
- (void) tappedOnButton:(id)sender withIndex:(NSUInteger)index {
    ...
    [self setShowAllDetailChildVCsForIndexPath:(int)self.selectedSection];
}
