...
- (void)viewDidLoad
{
    ...
    self.navigationItem.titleView = [UILabel navBarTitleLabel:[[TagManager sharedInstance]tagByName:kTagRecents]];
    ...
    detailNavController.navigationBar.tintColor = [UIColor whiteColor];
...
{
    //[self refreshRecent];
    [self performSelectorInBackground:@selector(refreshRecent) withObject:nil];
}
...
- (void) refreshUI
{
    ...
    if ([recentMasterViewController.recentObjects count]) {
        ...
        recentModelArray = [recentMasterViewController.recentRecordDictionary objectForKey:selectedObject];
    }
