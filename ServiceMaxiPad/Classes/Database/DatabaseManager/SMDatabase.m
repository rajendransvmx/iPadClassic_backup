...
NSString *const kDatabaseEncryptionKey         = @"svmxipad";
...
- (BOOL)open
{
    ...
    {
        /*
         * Don't change to SXLog ....
         * Since we are calling this within one transaction block.
         */
...
- (BOOL)close
{
    ...
    do {
        ...
        else if (SQLITE_OK != rc)
        {
             ...
             * Since we are calling this within one transaction block.
             */
            NSLog(@"error while closing db!: %d", rc);
        }
    }
    while (retry);
...
- (void)setCachedStatement:(SQLStatement*)statement forQuery:(NSString*)query
{
    ...
    NSString *copiedQuery = [query copy]; // in case we got handed in a mutable string...
    
    [statement setQuery:copiedQuery];
    
    NSMutableSet* statements = [cachedStatements objectForKey:copiedQuery];
    ...
    [cachedStatements setObject:statements forKey:copiedQuery];
