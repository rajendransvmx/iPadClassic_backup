        ...
        @autoreleasepool {
            ...
            if ([responseData isKindOfClass:[NSDictionary class]]) {
                ...
                for (NSDictionary *valueDict in valueMap) {
                    ...
                    if ([key isEqualToString:ORG_NAME_SPACE@"__Service_Group_Members__c"]) {
                        ...
                        if ((![jsonValue isKindOfClass:[NSNull class]]) && (![StringUtil isStringEmpty:jsonValue])) {
                            ...
                            NSArray *jsonArray = [NSJSONSerialization JSONObjectWithData:objectData
                                                                                 options:NSJSONReadingMutableContainers
                                                                                   error:&jsonError];
                            if ([jsonArray count] > 0) {
                                NSDictionary *serviceGroupMemebersDict = [jsonArray objectAtIndex:0];
                                NSString *technicianId = [serviceGroupMemebersDict objectForKey:kId];
                                if (![StringUtil isStringEmpty:technicianId]) {
                                    [PlistManager storeTechnicianId:technicianId];
                                }
                            }
