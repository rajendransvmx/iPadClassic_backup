        ...
        @autoreleasepool {
            ...
            if ([objectsArray isKindOfClass:[NSArray class]] && [objectsArray count] > 0) {
                ...
                for (int counter = 0; counter < [objectsArray count]; counter++) {
                    ...
                    NSString *key = [objectDict objectForKey:kSVMXRequestKey];
                    
                    if ([key isEqualToString:ORG_NAME_SPACE@"__SVMX_Event__c"]) {
                        [self parseServiceMaxEventFromObjectDict:objectDict andInsertIntoDictionay:heapDictionary];
                    } else {
...
- (void)parseServiceMaxEventFromObjectDict:(NSDictionary*)objectDict andInsertIntoDictionay:(NSMutableDictionary*)heapDictionary
{
    NSString *objectName  =  [objectDict objectForKey:kSVMXRequestKey];
    NSString *jsonString = [objectDict objectForKey:kSVMXRequestValue];
    NSData *jsonData = nil;
    if (jsonString != nil && ![jsonString isKindOfClass:[NSNull class]]) {
