//
//  SMXAddEventPopoverController.m
/**
 *  @file   FILE_NAME.m
 *  @class  CLASS_NAME
 *
...
#import "SMXSearchBarWithAutoComplete.h"
...
@interface SMXAddEventPopoverController () <UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) SMXSearchBarWithAutoComplete *searchBarCustom;
@property (nonatomic, strong) SMXButtonWithDatePopover *buttonDate;
@property (nonatomic, strong) SMXButtonWithHourPopover *buttonTimeBegin;
@property (nonatomic, strong) SMXButtonWithHourPopover *buttonTimeEnd;
@property (nonatomic, strong) SMXGuestsTableView *tableViewGuests;
@end

@implementation SMXAddEventPopoverController
...
- (IBAction)buttonDoneAction:(id)sender {
    ...
    if (!eventNew.numCustomerID) {
        stringError = [[TagManager sharedInstance]tagByName:kTag_PleaseSelectCustomer];
    } else if (![self isTimeBeginEarlier:eventNew.dateTimeBegin timeEnd:eventNew.dateTimeEnd]) {
        stringError = [[TagManager sharedInstance]tagByName:kTag_StartTimeMustEarlierEndTime];
    } else if (eventNew.arrayWithGuests.count == 0) {
        stringError = [[TagManager sharedInstance]tagByName:kTag_PleaseSelectGuest];
    }
    
    if (stringError) {
        [[[UIAlertView alloc] initWithTitle:nil message:stringError delegate:self cancelButtonTitle:[[TagManager sharedInstance]tagByName:kTagAlertErrorOk] otherButtonTitles: nil] show];
...
- (void)addButtonCancelWithCustomView:(UIView *)customView {
    ...
    [self customLayoutOfButton:buttonCancel withTitle:[[TagManager sharedInstance]tagByName:kTagCancelButton] action:@selector(buttonCancelAction:) frame:CGRectMake(20, 0, 80, BUTTON_HEIGHT+30)];
...
- (void)addButtonDoneWithCustomView:(UIView *)customView {
    ...
    [self customLayoutOfButton:buttonDone withTitle:[[TagManager sharedInstance]tagByName:kTagAddTaskDoneButton] action:@selector(buttonDoneAction:) frame:CGRectMake(buttonCancel.superview.frame.size.width-80-10, buttonCancel.frame.origin.y, 80, buttonCancel.frame.size.height)];
    [buttonCancel.superview addSubview:buttonDone];
}

- (void)addSearchBarWithCustomView:(UIView *)customView {
    
    searchBarCustom = [[SMXSearchBarWithAutoComplete alloc] initWithFrame:CGRectMake(0,buttonCancel.superview.frame.origin.y+buttonCancel.superview.frame.size.height+ BUTTON_HEIGHT, customView.frame.size.width, BUTTON_HEIGHT)];
    [customView addSubview:searchBarCustom];
}

- (void)addButtonDateWithCustomView:(UIView *)customView {
    
    buttonDate = [[SMXButtonWithDatePopover alloc] initWithFrame:CGRectMake(0, searchBarCustom.frame.origin.y+searchBarCustom.frame.size.height+2, customView.frame.size.width, BUTTON_HEIGHT) date:event.ActivityDateDay];
    [customView addSubview:buttonDate];
}

- (void)addButtonTimeBeginWithCustomView:(UIView *)customView {
    
    buttonTimeBegin = [[SMXButtonWithHourPopover alloc] initWithFrame:CGRectMake(0, buttonDate.frame.origin.y+buttonDate.frame.size.height+BUTTON_HEIGHT, customView.frame.size.width, BUTTON_HEIGHT) date:event.dateTimeBegin];
    [customView addSubview:buttonTimeBegin];
}

- (void)addButtonTimeEndWithCustomView:(UIView *)customView {
    
    buttonTimeEnd = [[SMXButtonWithHourPopover alloc] initWithFrame:CGRectMake(0, buttonTimeBegin.frame.origin.y+buttonTimeBegin.frame.size.height+2, customView.frame.size.width, BUTTON_HEIGHT) date:event.dateTimeEnd];
...
- (void)addtableViewGuestsWithCustomView:(UIView *)customView {
    ...
    tableViewGuests = [[SMXGuestsTableView alloc] initWithFrame:CGRectMake(0, y, customView.frame.size.width,customView.frame.size.height-y)];
