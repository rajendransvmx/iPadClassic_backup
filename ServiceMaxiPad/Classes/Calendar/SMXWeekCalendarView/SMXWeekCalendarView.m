...
- (void)setDictEvents:(NSMutableDictionary *)_dictEvents {
    ...
    if (!scrollViewHeaderWeek) {
        ...
        [self scrollToPage:((int)[NSDate componentsOfDate:[[SMXDateManager sharedManager] currentDate]].weekOfMonth)+1];
        ...
        weekContainerScroll.weekCalendarDelegate=self;
        weekContainerScroll.contentInset=UIEdgeInsetsMake(20, 0, 20, 0);
//        if ([Utility isDeviceIOS8]) commented this on 25/nov/2014 BSP. The issue of re-setting the contentsize started appearing in ios7 once day/week/month started to get initalized only after the segementcontrol is pressed.
        {
            [weekContainerScroll setDelegate:self];
        }
...
- (void)showHourLine:(BOOL)show {
    ...
    [weekContainerScroll.currentPatch setAlpha:show];
