...
-(void)setUpUI
{
    ...
    self.TapSelectLabel.text = [[TagManager sharedInstance]tagByName:kTag_Tap_ItemToSelect];
    [self defaultSetUp];
    [self setLabelValues];
...
-(void)defaultSetUp
{
    ...
    self.selectedItemlabel.textColor = [UIColor getUIColorFromHexValue:@"#434343"];
...
//        NSArray *newArray = [self.selectedItemArray filteredArrayUsingPredicate:resultPredicate];
        ...
        [self removeEntryFromDataSource:data];
        [self reloadTableOnSearch];
    }
}
...
- (void)removeSelecteditem:(id)sender
{
    ...
    if ([self.selectedItemArray count] > 0) {
        ...
        SFMPickerData * removedItem =  [self.selectedItemArray objectAtIndex:imgView.tag];
        [self addPicklistEntryToDataSource:removedItem];
...
-(void)addPicklistEntryToDataSource:(SFMPickerData *)addItem
{
    //[self.dataSource addObject:addItem];
    [self.dataSource insertObject:addItem atIndex:0];
    
    if(self.searchOn){
        
        NSRange range = [addItem.pickerValue rangeOfString:self.searchBar.text options:NSCaseInsensitiveSearch];
        ...
        if(range.length > 0)
        {
            ...
            //[self.searchResults addObject:addItem];
            [self.searchResults insertObject:addItem atIndex:0];
        }
    }
}
-(void)removeEntryFromDataSource:(SFMPickerData *)addItem
{
    [self removeSectedItemFromDataSource:addItem];
    [self removeSectedItemFromSearchResults:addItem];
}

-(void)setUpdataSource
{
    for(SFMPickerData * selectedData in self.selectedItemArray)
    {
        //pickerData.pickerValue
        int index = -1;
        for (SFMPickerData * pickerData in self.dataSource) {
            index++;
            if([selectedData.pickerValue isEqualToString:pickerData.pickerValue]){
                break;
            }
        }
...
-(void)removeSectedItemFromDataSource:(SFMPickerData *)selectedData
{
        //pickerData.pickerValue
        int index = -1;
        for (SFMPickerData * pickerData in self.dataSource) {
...
-(void)removeSectedItemFromSearchResults:(SFMPickerData *)selectedData
{
    //pickerData.pickerValue
    int index = -1;
    for (SFMPickerData * pickerData in self.searchResults) {
        index++;
        if([selectedData.pickerValue isEqualToString:pickerData.pickerValue]){
            break;
        }
    ...
    {
        [self.searchResults removeObjectAtIndex:index];
    }
