...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<SSZipArchiveDelegate>)delegate {
	...
	do {
        ...
        if (nil != err) {
            //undochangespushpak
            //
            //SMLog(kLogLevelError,@"[SSZipArchive] Error: %@", err.localizedDescription);
        }
                ...
                if (fileInfo.dosDate != 0) {
                    ...
                    if (attr) {
                        if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            //undochangespushpak
                            //
                            //SMLog(kLogLevelError,@"[SSZipArchive] Failed to set attributes");
                        }
                    }
                }
            }
        ...
        else
        {
            ...
            //SMLog(kLogLevelVerbose,@"Symlinking to: %@", destinationPath);
            ...
            if(symlinkError != nil)
            {
                //undochangespushpak
                //
                //SMLog(kLogLevelError,@"Failed to create symbolic link at \"%@\" to \"%@\". Error: %@", symlinkURL.absoluteString, destinationURL.absoluteString, symlinkError.localizedDescription);
            }
        }
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[d objectForKey:@"modDate"], NSFileModificationDate, nil] ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
            //undochangespushpak
            //
            //SMLog(kLogLevelError,@"[SSZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
        }
        if (err) {
            //undochangespushpak
            //
            //SMLog(kLogLevelError,@"[SSZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
