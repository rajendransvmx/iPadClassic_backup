...
- (NSString*)getDatatypeOfField:(NSString*)fieldName andObject:(NSString*)objectName
{
    DBCriteria *criteriaOne = [[DBCriteria alloc]initWithFieldName:@"objectName" operatorType:SQLOperatorEqual andFieldValue:objectName];
      DBCriteria *criteriaTwo = [[DBCriteria alloc]initWithFieldName:@"fieldName" operatorType:SQLOperatorEqual andFieldValue:fieldName];
   NSArray *sfObjectFields = [self fetchSFObjectFieldsInfoByFields:@[@"type"] andCriteriaArray:@[criteriaOne,criteriaTwo] advanceExpression:nil];
    SFObjectFieldModel *objectFieldModel;
    if ([sfObjectFields count] > 0) {
        objectFieldModel = [sfObjectFields objectAtIndex:0];
    }
