...
- (NSArray *)getRequestParametersForAccHistory
{
    ...
    if (model != nil) {
        ...
        NSString *query  = [NSString stringWithFormat:@"SELECT Id, %@, CreatedDate  FROM %@  WHERE ( (  ( CreatedDate <= %@ )  AND   ( %@ = 'Closed' )  AND   ( Id != '%@' )  AND   ( %@ = '%@' ) ) )", kWorkOrderProblemDescription, kWorkOrderTableName, createdDate, kOrderStatus, [model valueForField:kId], kWorkOrderCompanyId, [model valueForField:kWorkOrderCompanyId] ];
...
- (NSArray *)getRequestParametersForProHistory
{
    ...
    if (model != nil) {
        ...
        if ([topLevelId length] > 0) {
            query  = [NSString stringWithFormat:@"SELECT Id, %@, CreatedDate  FROM %@  WHERE ( (  ( CreatedDate <= %@ )  AND   ( %@ = 'Closed' )  AND   ( Id != '%@' )  AND   ( %@ = '%@' ) ) )", kWorkOrderProblemDescription, kWorkOrderTableName, createdDate, kOrderStatus, [model valueForField:kId], kTopLevelId, [model valueForField:kTopLevelId] ];
        }
        else if ([productId length] > 0) {
            query  = [NSString stringWithFormat:@"SELECT Id, %@, CreatedDate  FROM %@  WHERE ( (  ( CreatedDate <= %@ )  AND   ( %@ = 'Closed' )  AND   ( Id != '%@' )  AND   ( %@ = '%@' ) ) )", kWorkOrderProblemDescription, kWorkOrderTableName, createdDate, kOrderStatus, [model valueForField:kId], kComponentId, [model valueForField:kComponentId] ];
        }
