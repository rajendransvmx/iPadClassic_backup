...
static NSString *const kChatterDirectoryName = @"Chatter";
static NSString *const kAttachmentsDirectoryName = @"Attachments";
...
+ (BOOL)moveFileAtPath:(NSString *)sourcePath toPath:(NSString *)targetPath
{
    ...
    if ([fileManager fileExistsAtPath:sourcePath])
    {
        ...
        if (!isFileMoved)
        {
            ...
            {
                NSLog(@"Error while moving the file.");
            }
        }
    }
    else
...
+ (void)installJavascriptFiles
{
    ...
    for (NSInteger i = 0; i < [javascriptFiles count]; i++)
    {
        ...
        if ([javascriptFiles objectAtIndex:i] != nil)
        {
    ...
    if(array.count)
    {
        ...
        else
        {
            ...
            if ([StringUtil isStringEmpty:prevAppVersion] || ![prevAppVersion isEqualToString:currentAppVersion]) {
                ...
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"kCoreLibUpgraded"];
...
+ (NSString*)getPerformanceLogDirectoryPath
{
    NSFileManager *fm = [NSFileManager defaultManager];
    NSString * rootPath = [self getRootPath];
    rootPath = [rootPath stringByAppendingPathComponent:kPerforamanceLogDirectoryName];
    if(![fm fileExistsAtPath:rootPath])
    {
        [fm createDirectoryAtPath:rootPath
      withIntermediateDirectories:YES
...
+ (NSString*)getAttachmentsSubDirectoryPath
{
    ...
    NSString * rootPath = [self getRootPath];
    BOOL isSuccess = YES;
    rootPath = [rootPath stringByAppendingPathComponent:kAttachmentsDirectoryName];
    if (![fm fileExistsAtPath:rootPath]) {
...
+ (NSString *)getChatterSubDirectoryPath
{
    NSFileManager *fm = [NSFileManager defaultManager];
    NSString * rootPath = [self getRootPath];
    rootPath = [rootPath stringByAppendingPathComponent:kChatterDirectoryName];
    if(![fm fileExistsAtPath:rootPath])
    {
        [fm createDirectoryAtPath:rootPath
      withIntermediateDirectories:YES
...
{
    NSFileManager *fm = [NSFileManager defaultManager];
    NSString * rootPath = [self getChatterSubDirectoryPath];
    rootPath = [rootPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.png", name]];
