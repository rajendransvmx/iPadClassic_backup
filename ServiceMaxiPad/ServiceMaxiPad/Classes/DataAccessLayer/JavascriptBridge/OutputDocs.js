...
function OPDGetDocumentMetaData(processIdObj, callbackFunction, context)
{
  ...
  var objectName = "SFProcess";
  var fieldNames = [{fieldName:'process_unique_id',fieldType:'TEXT'},{fieldName:'doc_template_id',fieldType:'TEXT'}];
  var criteria = [{fieldName:'process_unique_id',fieldValue:processId,operator:'='}];
  var templateId = 0;
  $DAL.executeQuery(objectName,fieldNames,criteria,null,null,function(request)
  {
    if(request.response.statusCode != '1')
...
var GetDataForTemplateDetailsRecord = function(inputObj, templateDetailRecords, rec_Local_sf_Id, index, callbackFunction)
{
   // 8980 : need local_id and also sfid with conditional OR
    ...
    var criteria = [{fieldName:'process_id',fieldValue:processSfid,operator:'='},{fieldName:'doc_template_Detail_id',fieldValue:dtd_Id,operator:'='}];
    var expression_id = 0;
...
function OPDGetDocumentData(inputObj, callbackFunction, context)
{
    ...
    var profieldNames = [{fieldName:'doc_template_id',fieldType:'TEXT'},{fieldName:'process_sfid',fieldType:'TEXT'}];
    var procriteria = [{fieldName:'process_unique_id',fieldValue:processId,operator:'='}];
    var templateId = 0;
