...
- (NSString *)getExpressionForCriteriaObject:(NSDictionary *)criteriaDict andObjectName:(NSString *)objectName {
    ...
    //8906
    NSDictionary *fieldInfo = [NSDictionary dictionary]; //[[SVMXDatabaseMaster sharedDataBaseMaterObject] getFieldTypeForFieldName:fieldName Object:objectName];
    ...
    if (![Utility isStringEmpty:fieldName] && ![Utility isStringEmpty:operatorValue]) {
        ...
        //8906
        else if ([operatorValue isEqualToString:@"is null"])
        {
        ...
        NSLog(@"fieldtype =%@ expression =%@",fieldType,expression);
        return expression  ;
    }
    return nil;
}
- (NSString *)evaluateLiteralForFieldvalue:(NSString *)value andFieldType:(NSString *)type {
    ...
    NSString *literalvalue = [SFMPageHelper valueOfLiteral:value dataType:type];
...
- (NSDictionary *)getOperatorForString:(NSDictionary *)criterion  {
    ...
    //Handling quotes for OPDOc qualifying criteria - 11250, 10790
