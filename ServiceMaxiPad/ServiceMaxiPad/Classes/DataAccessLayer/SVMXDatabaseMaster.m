    ...
    @synchronized(self) {
        ...
        @autoreleasepool
        {
            ...
            [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
                ...
                while([resultSet next])
                {
                    [objectsArray addObject:[resultSet resultDictionary]];
                }
            }];
        }
        [self showLog:queryStatement];
...
//        synchronized_sqlite3_finalize(selectStmt);
        ...
        NSArray *resultObjFieldsArray = [objectFieldDao fetchSFObjectFieldsInfoByFields:@[@"fieldName",@"label",@"type",@"referenceTo",@"isNillable",@"dependentPicklist",@"controlerField"] andCriteriaArray:@[objectFieldcriteria] advanceExpression:nil];
...
- (NSString *)getNameValueForId:(NSString *)recordSfid {
    ...
    NSArray *nameFieldValues = [service fetchObjectNameFieldValueByFields:@[recordSfid] andCriteria:nil];
    
    if(nameFieldValues.count)
        nameValue = nameFieldValues[0];
...
- (NSString*)getNameFieldForObject:(NSString*)objectName
{
    ...
    NSArray *fieldNames = [service fetchSFObjectFieldsInfoByFields:@[kfieldname] andCriteria:dbcrit];
