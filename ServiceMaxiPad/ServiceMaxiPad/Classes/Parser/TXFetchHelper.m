            ...
            @autoreleasepool {
                ...
                for (SyncRecordHeapModel *objectModel in objectList) {
                    ...
                    if (idsCount > 0) {
                        ...
                        NSLog(@" Request: ObjectName %@ Count %d",objectName,[idsDictionary count]);
                    }
...
- (void)fillUpObjectNameFieldFrom:(TransactionObjectModel *)transModel
    ...
    for (NSString *eachKey in allKeys) {
        ...
        if ([referenceDictionary isKindOfClass:[NSDictionary class]]) {
            ...
            NSString *value = [referenceDictionary objectForKey:@"Name"];
            
            //NSString *objectName = [referenceDictionary objectForKey:kType];
