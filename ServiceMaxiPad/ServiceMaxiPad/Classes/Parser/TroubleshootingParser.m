    ...
    if ([daoService conformsToProtocol:@protocol(DocumentDAO)])
    {
        for (DocumentModel *model in detailsArray)
        {
            ...
            if ([settingsService conformsToProtocol:@protocol(DocumentDAO)])
            {
                ...
                {
                    BOOL resultStatus =   [daoService insertIntoDocumentsWithTheProductDetails:@[model]];
                  //  Below commented code is proper one to insert new record. Change in the Documnet model scema
