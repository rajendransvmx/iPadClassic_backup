...
- (void)parsePutResponse:(NSArray *)internalResponse  withOperationType:(NSString *)operationType{
    ...
    @autoreleasepool {
        ...
        for(NSDictionary * eachValueMap in valueMap)
        {
           ...
           /*else if(([responseKey isEqualToString:SYNC_TIME_STAMP] && [operationType isEqualToString:kModificationTypeUpdate]) || ([responseKey isEqualToString:@"PUT_INSERT_TIME"] &&[operationType isEqualToString:kModificationTypeInsert]))
            {
                NSString * syncTimeStamp = [eachValueMap objectForKey:kValue];
                if (![SMXiPhone_Utility isStringEmpty:syncTimeStamp]) {
                    [SMXiPhone_Utility storeLastUpdateTimeInDefaults:syncTimeStamp];
                }
            }*/
        ...
        {
            [self.oneCallSyncHelper  deleteSyncRecordsFromSyncModificationTableWithIndex:deleteObjectMapIds withModificationType:operationType];
        }
...
- (void)handleConflicts:(NSArray *)internalValueMapArray  withOperationType:(NSString *)operationType withErrorType:(NSString *)errorType
{
    ...
    for (NSDictionary * eachDict in internalValueMapArray ) {
        ...
        NSLog(@"message = %@",errorMsg);
        NSLog(@"errorType = %@",errorType);
        NSLog(@"operationType = %@",operationType);
        NSLog(@"================================");
    }
    ...
    if(syncConflictsRecords != nil)
    {
        ...
        if ([daoService conformsToProtocol:@protocol(SyncErrorConflictDAO)]) {
            ...
            if (resultStatus) {
                NSLog(@"SyncErrorConflict models inserted successfully!");
            }
        }
...
- (void)handleGetUpdateResponse:(NSArray *)valueMap responseType:(OneCallSyncResponseType)responseType {
    NSLog(@"Number of objects updated: %d",[valueMap count]);
    ...
    if (allEventsExist) {
        if ([allEvenstDict count]>0) {
            ...
            [self.oneCallSyncHelper deleteAllEventsOfTheLoggedInUser];
        }
