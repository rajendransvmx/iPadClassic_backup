...
{
    [super viewDidLoad];
    self.navigationItem.titleView = [UILabel navBarTitleLabel:@"Tasks"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"summary-icon.png"]];
    }
    else if ([model.priority isEqualToString:[[TagManager sharedInstance] tagByName:kTagAddTaskPriorityLow]])
    {
        cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"Section_down_arrow.png"]];
    }
...
//Commented due to inconsistency in UI as per design.
...
//    
//    [lblTitle setFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:13]];
//    [lblTitle setTextColor:[UIColor blackColor]];
//    [lblTitle setTextAlignment:NSTextAlignmentLeft];
//    [lblTitle setBackgroundColor:[UIColor clearColor]];
//    [viewHeader addSubview:lblTitle];
//    lblTitle.text = [self sectionTitleForSection:section];
//    viewHeader.backgroundColor = [UIColor colorWithHexString:kPageViewMasterBGColor];
//
//    return viewHeader;
//    return viewHeader;
//}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [[self.dataSource objectForKey:[self sectionTitleForSection:section]] count];
}
...
{
    //fetch data from database and update data source.
    self.addOrEditTaskVC.delegate = nil;
...
- (void)addOrEditTaskVC:(AddOrEditTaskVC *)addOrEditTaskVC userPressedSaveForTask:(SFMTaskModel *)task {
    [addOrEditTaskVC dismissViewControllerAnimated:YES completion:nil];
