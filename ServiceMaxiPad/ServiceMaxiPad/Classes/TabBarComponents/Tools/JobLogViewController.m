...
- (void)viewDidLoad
{
    ...
    self.smSplitViewController.navigationItem.titleView = [UILabel navBarTitleLabel:@"Push Logs"];
...
- (IBAction)pushLogClicked:(UIButton *)sender {
    ...
    if (![self isLogSettingsON]) {
        ...
        [[AlertMessageHandler sharedInstance] showCustomMessage:@"Application Level Log Setting is OFF." withDelegate:nil tag:89 title:nil cancelButtonTitle:@"OK" andOtherButtonTitles:nil];
        return;
    }
    ...
    else
    {
        ...
        [[AlertMessageHandler sharedInstance] showCustomMessage:@"No logs to push." withDelegate:nil tag:89 title:nil cancelButtonTitle:@"OK" andOtherButtonTitles:nil];
        SXLogError(@"rertwwetw");
    }
...
- (void)flowStatus:(id)status
{
    if([status isKindOfClass:[WebserviceResponseStatus class]])
    {
        ...
        if (st.syncStatus == SyncStatusSuccess) {
            [PlistManager storeLastPushLogStatus:[[TagManager sharedInstance] tagByName:kTagPushLogStausSuccess]];
