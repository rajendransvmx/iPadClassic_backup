...
- (IBAction)syncDataClciked:(id)sender
{
...
- (void)updateUIForDataSyncLastSyncTimeAndStatus{
    ...
    if (([status isEqualToString:@"In Progress" ] | [status isEqualToString:@"Conflict"]) && dateString)
    {
...
- (void)showDataSyncConfirmMessage
{
    ...
    NSString *otherButtonTittle = @"Start Sync";
...
-(void)startConfigSync
{
    if ( [[SNetworkReachabilityManager sharedInstance] isNetworkReachable])
    {
        ...
        NSString *message2 = @"doing so will cancel or intererupt the synchronization.";
        NSArray *messages = [NSArray arrayWithObjects:message1, message2, nil];
...
- (void)updateUserInterface:(SyncProgressDetailModel*)progressObject
{
    ...
    switch (progressObject.syncStatus)
    {
        case SyncStatusInProgress:
        {
            ...
            {
                message = [NSString stringWithFormat:@"Step %@ of %@ - %@", progressObject.currentStep, progressObject.numberOfSteps, progressObject.message];
            }
            ...
            if (message != nil)
            {
                [self.configSyncProgressAlertView updateProgressBarWithValue:progress/100.0f andMessage:nil];
            }
