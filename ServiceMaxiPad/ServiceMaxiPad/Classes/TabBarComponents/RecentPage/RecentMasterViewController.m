...
- (void)refreshData
{
    ...
    NSMutableArray *listOfRecentRecord;
    if ([recentService conformsToProtocol:@protocol(RecentsDao)]) {
        listOfRecentRecord = [NSMutableArray arrayWithArray:[recentService getRecentRecordInfo]];
    }
    if([listOfRecentRecord count] > kLimit) {
        
        [recentService deleteOldestRecentObject];
        [listOfRecentRecord removeLastObject];
    }
