...
- (void)main {
    ...
    @synchronized([self class]) {
        @autoreleasepool {
            ...
            [[ZKServerSwitchboard switchboard] query:self.requestParameter.value target:self selector:@selector(didGetResponceWithResult:error:context:) context:nil];
...
- (void) didGetResponceWithResult:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    @synchronized([self class]) {
