...
- (void)displayRequest:(AFHTTPRequestOperation *)operation
{
    ...
    if (operation != nil)
    {
        SXLogError(@"Request url : %@\nmethod :%@\nheaders : %@\nbody : %@", [[[operation request] URL] absoluteString], [[operation request] HTTPMethod], [(NSMutableURLRequest*)[operation request] allHTTPHeaderFields], [[NSString alloc] initWithData:[[operation request] HTTPBody] encoding:NSUTF8StringEncoding]);
       
       SXLogError(@"Response       : %@", operation.responseString);
