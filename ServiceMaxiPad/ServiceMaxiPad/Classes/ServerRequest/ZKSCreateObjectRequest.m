...
- (void)didAttachDocument:(NSArray *)result error:(NSError *)error context:(id)context
{
    ...
    @synchronized([self class]) {
        ...
        if(error) {
            ...
            NSLog(@"%lu req-f latency : %f sec", (unsigned long)(self.requestType), [[NSDate date] timeIntervalSinceDate:self.startTime]);
            ...
            {
                NSLog(@"Invalid response! - Either result or context is nil.");
                [self.serverRequestdelegate didRequestFailedWithError:error
                                                             Response:result
                                                     andRequestObject:self];
                return;
            ...
            NSLog(@"%lu req-s latency : %f sec", (unsigned long)(self.requestType), [[NSDate date] timeIntervalSinceDate:self.startTime]);
