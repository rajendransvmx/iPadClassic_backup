...
- (RequestType )getNextRequestForCategoryType:(CategoryType)categoryType
    ...
    switch (categoryType) {
        case CategoryTypeOneCallRestIntialSync:
            requestType = [self getNextRequestForOneCallMetaInitialSync:currentRequest andPreviousRequest:previousRequest];
            break;
        case CategoryTypeValidateProfile:
            requestType = [self getNextRequestForValidateProfile:currentRequest andPreviousRequest:previousRequest];
            break;
        case CategoryTypeResetApp:
              //requestType = [self getNextRequestForResetInitialSync:currentRequest andPreviousRequest:previousRequest];
            break;
        case CategoryTypeOneCallDataSync:
            requestType = [self getNextRequestForOneCallDataSync:currentRequest andPreviousRequest:previousRequest];
            break;
            case CategoryTypeIncrementalOneCallMetaSync:
            requestType = [self getNextRequestForIncrementalOneCallConfigSync:currentRequest andPreviousRequest:previousRequest];
            break;
        case CategoryTypeTroubleShooting:
            requestType = [self getNextRequestForTroubleShooting:currentRequest andPreviousRequest:previousRequest];
            break;
        case CategoryTypeTroubleShootingDataDownload:
            requestType = [self getNextRequestForTroubleShootingDataDownload:currentRequest andPreviousRequest:previousRequest];
            break;
        case CategoryTypeJobLog:
...
- (RequestType)getNextRequestForOneCallMetaInitialSync:(SVMXServerRequest *)currentRequest
    ...
    /* HS 17 Oct commented to test EE org with group prifle issue
...
{
    BOOL shouldSkip = NO;
    if (type == RequestStaticResourceDownload || type == RequestAttachmentDownload ||type == RequestDocumentDownload || type == RequestDocumentInfoFetch || type == RequestTroubleShootDocInfoFetch || type == RequestDependantPickListRest || type == RequestRTDependentPicklist)
    {
        shouldSkip = YES;
    }
    return shouldSkip;
...
- (RequestType)getNextRequestForDataPurge:(SVMXServerRequest *)currentRequest
    ...
    if (currentRequest == nil) {
        nextRequestType = RequestDataPurge;
    }
    if (currentRequest.requestType == RequestDataPurgeFrequency) {
        nextRequestType = RequestTypeNone;
    }
