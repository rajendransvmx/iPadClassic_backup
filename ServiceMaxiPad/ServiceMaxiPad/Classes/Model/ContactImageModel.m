...
- (id)initWithTransactionModel:(TransactionObjectModel*)transactionModel {
    ...
    if (self)
    {
        ...
        _contactImage = @"";
        _localId = [transactionModel valueForField:kLocalId];
...
- (void)explainMe
{
    SXLogInfo(@"ContactImage contactId : %@ \n contactName : %@ \n contactImage : %@ \n emailString : %@ \n phoneString : %@, localId : %@",_contactId, _contactName, _contactImage, _emailString, _phoneString, _localId);
}
