...
- (id)initWithTransactionModel:(TransactionObjectModel*)transactionModel andIdEventStartValueMap:(NSMutableDictionary*)idEventStartValueMap
{
    ...
    if (self)
    {
        ...
        if ([self.gmtEventStartDateTime length]) {
            self.localEventStartDateTime = [DateUtil getDateStringForDBDateTime:self.gmtEventStartDateTime  inFormat:kDateFormatType3];
        }
        if ([self.gmtScheduledDateTime length]) {
            self.localScheduleDateTime = [DateUtil getDateStringForDBDateTime:self.gmtScheduledDateTime  inFormat:kDateFormatType3];
        }
