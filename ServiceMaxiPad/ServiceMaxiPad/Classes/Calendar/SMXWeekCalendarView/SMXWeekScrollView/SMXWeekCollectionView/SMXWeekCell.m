        ...
        for (SMXEvent *event in array) {
            ...
            for (SMXHourAndMinLabel *label in arrayLabelsHourAndMin) {
                ...
                if (compLabel.hour == compEventBegin.hour && compLabel.minute <= compEventBegin.minute && compEventBegin.minute < compLabel.minute+MINUTES_PER_LABEL) {
                   ...
                   // yTimeBegin = label.frame.origin.y+label.frame.size.height/2. + additonalheight * WEEKVIEW_HEIGHT_CELL_HOUR/60;  // 70 pixels = 60 mins. Hence 1 min = 70/60
                }
    ...
    if ((lOldButton.frame.origin.y + lOldButton.frame.size.height) >= (_button.frame.origin.y + _button.frame.size.height))
    {
        ...
        else
        {
            ...
            if(![cEventInsideALargerEvent containsObject:_button])
            {
                ...
                //commented due to crash :Radha and Pushpak
                //[cEventsWhichAccomodateSmallerEvents setObject:lOldButton forKey:_button.eventName.text];
    ...
    if (gesture.state == UIGestureRecognizerStateEnded) {
       ...
       // [cSelectedEventButton superview].layer.zPosition=zPositionOfSelectedView;
        if (cSelectedEventButton) {
            ...
            UIAlertView *lAlertView = [[UIAlertView alloc] initWithTitle:[[TagManager sharedInstance] tagByName:kTagEventReschedulePrompt] message:nil delegate:self cancelButtonTitle:[[TagManager sharedInstance]tagByName: kTagRescheduleYes] otherButtonTitles:[[TagManager sharedInstance]tagByName: kTagRescheduleNo], nil];
            [lAlertView show];
            lAlertView = nil;
        }else{
