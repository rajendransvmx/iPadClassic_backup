...
- (void)initLayout {
    ...
    if (!imageViewCircle) {
        ...
        //labelDay.font=[UIFont fontWithName:@"HelveticaNeue-Light" size:18.0f];
...
-(void)setButton{
    cellButton=[[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    ...
    if ([arrayEvents count] > 0) {
        ...
        for (int i = 0; i < [arrayEvents count] ; i++) {
            ...
            if ((buttonOfNumber == maxNumOfButtons) && ([arrayEvents count] - maxNumOfButtons > 0)) {
                [button setTitle:[NSString stringWithFormat:@"(and %i more)", [arrayEvents count] - maxNumOfButtons + 1] forState:UIControlStateNormal];
                ...
                [button setTitle:[NSString stringWithFormat:@"• %@",event.stringCustomerName] forState:UIControlStateNormal];
