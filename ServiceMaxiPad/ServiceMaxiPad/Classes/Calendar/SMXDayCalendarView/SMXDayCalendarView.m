...
-(void)dayCalendarPopup{
    if (grayCalenderBkView==nil) {
        ...
        [CalendarPopupContent setTileWidth:46.0f];
        ...
        monthCalender.view.frame=CGRectMake(0, -366, 302, 366);
        ...
        monthCalender.view.layer.shadowOffset = CGSizeMake(0.0f,7.5f);
        ...
        [UIView animateWithDuration:0.30
                         ...
                         animations:^{
                             monthCalender.view.frame=CGRectMake(0, 0, 302, 366);
                         }
                         completion:nil];
    }else{
...
-(void)rescheduleEvent:(SMXEvent *) _event;
{
    ...
    [lCancelbutton setTitle:@"Cancel" forState:UIControlStateNormal];
    ...
    [lSaveButton setTitle:@"Save" forState:UIControlStateNormal];
    ...
    UILabel *lReschedulePopoverTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(lCancelbutton.frame.origin.x + lCancelbutton.frame.size.width, 0, cPopOverView.frame.size.width - lSaveButton.frame.size.width - lCancelbutton.frame.size.width, 45)];
    lReschedulePopoverTitleLabel.text = @"Select Date and Time";
    lReschedulePopoverTitleLabel.textAlignment = NSTextAlignmentCenter;
    lReschedulePopoverTitleLabel.backgroundColor = [UIColor clearColor];
    ...
    UILabel *lEventClashingWithOtherEventLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, lHorizontalLine.frame.origin.y + lHorizontalLine.frame.size.height + 2.5, cPopOverView.frame.size.width, 40)];
    lEventClashingWithOtherEventLabel.backgroundColor = [UIColor yellowColor];
    lEventClashingWithOtherEventLabel.text = @"This time overlaps with another appointment";
    lEventClashingWithOtherEventLabel.textAlignment = NSTextAlignmentCenter;
    
    lEventClashingWithOtherEventLabel.font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:18];
    
    [cPopOverView addSubview:lEventClashingWithOtherEventLabel];
    
    UIColor *customLightGrayColor = [UIColor colorWithRed:121.0/255.0 green:121.0/255.0 blue:121.0/255.0 alpha:1.0];
    
    
    UILabel *lStartTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(60, lEventClashingWithOtherEventLabel.frame.origin.y + lEventClashingWithOtherEventLabel.frame.size.height, cPopOverView.frame.size.width - 120, 30)];
...
-(void)saveRescheduledInfo:(id)sender
{
    ...
    if ([lStartDate compare:lEndDate] == NSOrderedDescending) {
        ...
        UIAlertView *lAlert = [[UIAlertView alloc] initWithTitle:@"Warning" message:@"Implementation Pending but still, How can start date be later than the end date? No Common Sense?" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles: nil];
        [lAlert show];
        lAlert = nil;
...
-(void)checkOrientationAndReset
{
    ...
    else
    {
        ...
        frame.origin.y = 0.0;
        frame.size.width = self.frame.size.width - dayContainerScroll.frame.size.width;
        frame.size.height = self.frame.size.height;
        cNoEventSelectedLabel.frame = frame;
