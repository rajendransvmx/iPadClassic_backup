...
{
    lTempEvent = _event;
    if (_event.isAlldayEvent || _event.isMultiDayEvent) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[[TagManager sharedInstance] tagByName:kTagAlertWarningError] message:@"All Day Event or Multi Day Event cannot be Rescheduled." delegate:nil cancelButtonTitle:[[TagManager sharedInstance]tagByName:kTagAlertErrorOk] otherButtonTitles:nil];
        [alert show];
        return;
    }
...
//    NSString *lDate = [lDF stringFromDate:[NSDate combineDate:_event.ActivityDateDay withTime:_event.dateTimeBegin] ];
    ...
    NSDateComponents *comp = [cal components:NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay|NSCalendarUnitHour|NSCalendarUnitMinute fromDate:_event.dateTimeBegin];
    NSString *dateTimeString = [CalenderHelper getTagValueForMonth:1]
    
    NSString *lDate = [lDF stringFromDate:_event.dateTimeBegin]; //12-Jan-2015 BSP
    ...
    NSString *lDate = [CalenderHelper getStringValueForTheDate:_event.dateTimeBegin];
...
//    lDate = [lDF stringFromDate:_event.dateTimeEnd];
    ...
    lDate = [CalenderHelper getStringValueForTheDate:_event.dateTimeEnd];
...
//    NSDate *lEndDateTime = [lDF dateFromString:[lDF stringFromDate:lEndDate]];
    ...
    if ([lTempStartDateTime compare:lTempEndDateTime] == NSOrderedSame) {
        UIAlertView *lAlert = [[UIAlertView alloc] initWithTitle:@"" message:@"Start Date/Time must be before End Date/Time." delegate:nil cancelButtonTitle:[[TagManager sharedInstance]tagByName:kTagAlertErrorOk] otherButtonTitles: nil];
...
-(void)updateEvent:(SMXEvent *)event fromActivityDate:(NSDate *)fromActivityDate toActivityDate:(NSDate *)activityDate andStartTime:(NSDate *)startTime withEndTime:(NSDate *)endTim fromIndex:(int)fromIndex toIndex:(int)toIndex{
    ...
    [CalenderHelper updateEvent:event toActivityDate:fromActivityDate andStartTime:startTime withEndTime:endTim];
}
...
//            if (([eventStartTime compare:lTempStartDateTime] == NSOrderedAscending || [eventStartTime compare:lTempStartDateTime] == NSOrderedSame)&& ([lTempEndDateTime compare:eventEndTime] == NSOrderedAscending || [lTempEndDateTime compare:eventEndTime] == NSOrderedSame)) {
            if (([event.dateTimeBegin compare:lTempStartDateTime] == NSOrderedAscending || [event.dateTimeBegin compare:lTempStartDateTime] == NSOrderedSame)&& ([lTempEndDateTime compare:event.dateTimeEnd] == NSOrderedAscending || [lTempEndDateTime compare:event.dateTimeEnd] == NSOrderedSame)) {
    ...
    if (viewDetail) {
        ...
        if (lTempEvent.whatId.length) {
           ...
           
            {
                ...
                NSMutableArray *lEventArray = [lDictionary objectForKey:lDate];
