...
-(void)setMonthAndYear
{
    ...
    [attString appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %d",year] attributes:yearDict]];
...
-(void)saveRescheduledInfo:(id)sender
{
    ...
    [self updateEvent:lTempEvent fromActivityDate:lTempEvent.ActivityDateDay toActivityDate:[lDF dateFromString:[lDF stringFromDate:lStartDateTime]] andStartTime:lStartDateTime withEndTime:lEndDateTime fromIndex:1 toIndex:0];
    
    NSDateComponents *componentsActivitydate = [[NSCalendar currentCalendar] components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear fromDate:oldActivityDate];
...
-(void)refreshDetailView;
{
    ...
    if (viewDetail && lTempEvent.whatId) {
        NSMutableDictionary *dict = [NSMutableDictionary new];
        [dict setObject:[NSString stringWithFormat:@"%@",lTempEvent.dateTimeBegin] forKey:lTempEvent.whatId];
        NSArray *lArray =  [CalenderHelper getWorkOrdersForWhatIds:dict];
        if ([lArray count])
        {
            NSDictionary *slaPriorityDictionary = [CalenderHelper SLAAndPriorityStatusForwhatID:lTempEvent.whatId];
            
            lTempEvent.cWorkOrderSummaryModel = [lArray objectAtIndex:0];
