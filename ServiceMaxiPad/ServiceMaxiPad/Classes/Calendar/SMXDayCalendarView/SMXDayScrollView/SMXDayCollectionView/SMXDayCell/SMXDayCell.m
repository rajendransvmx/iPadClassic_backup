    ...
    if (arrayEvents) {
        ...
        for (SMXEvent *event in arrayEvents) {
            ...
            if ( event.cWorkOrderSummaryModel) {
                ...
                NSDictionary *lDataDict = [CalenderHelper SLAAndPriorityStatusForwhatID:event.whatId];

                NSNumber *slaMetric = [lDataDict objectForKey:@"slaStatus"];
                slaInfo = [slaMetric boolValue];
                
                NSString *priorityStatus = [lDataDict objectForKey:@"priorityStatus"];
            ...
            if(event.whatId)
            {
                conflictInfo = [CalenderHelper isConflicted:event.whatId];
            }
           ...
           float lWidth = (self.frame.size.width - EVENT_X_POSITION)/cSamePositionAndDurationEvents.count;
            ...
            for (int i = 0; i < cEventInsideALargerEvent.count; i++) {
                ...
                lFirstButtonFrame.size.width = lbutton.frame.size.width - 5;
                lbutton.eventName.frame = lFirstButtonFrame;
...
-(void)moveEvent:(UILongPressGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateEnded) {
        ...
        if (cSelectedEventButton) {
            ...
            NSString * buttonLOC = [[TagManager sharedInstance] tagByName:kTagRescheduleNo];
            
            [alertHandler showCustomMessage:nil withDelegate:self title:[[TagManager sharedInstance] tagByName:kTagEventReschedulePrompt] cancelButtonTitle:[[TagManager sharedInstance] tagByName:kTagRescheduleYes] andOtherButtonTitles:[[NSArray alloc] initWithObjects:buttonLOC, nil]];
