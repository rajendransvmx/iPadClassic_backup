...
//    [labelWithSameYOfCurrentHour setAlpha:boolIsToday];  //TODO: if the hour/half-hour line is need to be displayed, set the alpha to 1. or else hide the line near to the current time line by setting alpha as 0.
    ...
    if (arrayEvents) {
        ...
        for (SMXEvent *event in arrayEvents) {
            ...
            NSLog(@"conflict image frame: %@", NSStringFromCGRect(_button.firstImageView.frame));
            NSLog(@"sla image frame: %@", NSStringFromCGRect(_button.secondImageView.frame));
            NSLog(@"priority image frame: %@", NSStringFromCGRect(_button.thirdImageView.frame));
