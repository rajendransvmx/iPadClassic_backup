...
//           cAllEventDictionary = [self compareAndSaveEvents:cAllEventDictionary andButton:_button];
    ...
    if(cSelectedEventButton)  //For the case when the event is re-scheduled and the entire left panel is re-rendered.
    {
        for (SMXBlueButton *button in arrayButtonsEvents) {
            if ([button.event.localID isEqualToString:cSelectedEventButton.event.localID]) {
...
-(void)compareWithSelectedButtonAndChangeColorForDraggedButton
{
    if (cSelectedEventButton) {
        if (cSelectedEventButton == cDraggedForReschedulingButton) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex;
{
    if (buttonIndex == 0) {
        ...
        if (lDifferenceInPosition > 0)
        {
            ...
            [CalenderHelper updateEvent:cDraggedForReschedulingButton.event toActivityDate:cDraggedForReschedulingButton.event.ActivityDateDay andStartTime:[cDraggedForReschedulingButton.event.dateTimeBegin dateByAddingTimeInterval:-60*min] withEndTime:[cDraggedForReschedulingButton.event.dateTimeEnd dateByAddingTimeInterval:-60*min]];
        }
        else
        {
