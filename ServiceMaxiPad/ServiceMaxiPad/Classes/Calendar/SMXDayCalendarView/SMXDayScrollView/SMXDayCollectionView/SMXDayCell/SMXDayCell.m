...
#define kEventStartTimeDate             @"startTime"
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if (!cAllEventDictionary) {
            ...
            [cAllEventDictionary setObject:[NSNumber numberWithLong:24*60] forKey:kEventDurationLong];
...
//    [labelWithSameYOfCurrentHour setAlpha:boolIsToday];  //TODO: if the hour/half-hour line is need to be displayed, set the alpha to 1. or else hide the line near to the current time line by setting alpha as 0.
    ...
    if (arrayEvents) {
        ...
        for (SMXEvent *event in arrayEvents) {
            ...
            _button.eventName.text = (event.cWorkOrderSummaryModel ? event.cWorkOrderSummaryModel.companyName : event.stringCustomerName);
            NSLog(@"_button.eventName.text: %@", _button.eventName.text);
...
//            [self compareAndSaveEvents:cAllEventDictionary andButton:_button];
...
-(void)compareAndSaveEvents:(NSDictionary *)eventDictionary andButton:(SMXBlueButton *)theButton
{
//TODO: Work on this. The DIctionary striucture is in the note book.
    ...
    long durationinMins = [[eventDictionary objectForKey:kEventDurationLong] longValue];
    NSMutableDictionary *SmallerEventsDictionary = [eventDictionary objectForKey:kSmallerEventsDictionary];
    NSMutableArray *lEventArray = [eventDictionary objectForKey:kEventsArray];
    ...
    NSDateComponents *startDateComponent = [[NSCalendar currentCalendar] components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit|NSHourCalendarUnit|NSMinuteCalendarUnit|NSSecondCalendarUnit fromDate:lStartDateTime];
