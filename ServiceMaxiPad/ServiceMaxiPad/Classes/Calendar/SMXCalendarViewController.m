...
-(void)eventSelected:(NSNotification *)pNotification
{
    ...
    else
    {
        ...
        NSString *whatId = eventData.whatId;
        
        if ([whatId length] < 1) {
            whatId = eventData.localID;
        }
        
        TransactionObjectModel *model = [CalenderHelper getRecordForlocalID:whatId];
...
-(void)eventSelectedShare:(SMXEvent *)eventData userInfor:(NSDictionary *)contactId
{
    ...
    if(contactId)
    {
        pageManager = [[SFMViewPageManager alloc] initWithObjectName:[contactId objectForKey:@"objectName"] recordId:[contactId objectForKey:@"contactID"]];
    }
    else
    {
...
-(void)setEvents:(NSArray *) lEventArray
{
    ...
    NSSortDescriptor *eventStartDateSortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"dateTimeBegin"
                                                                                   ascending:YES];
    NSArray *sortedArray = [lEventCollectionArray sortedArrayUsingDescriptors:[NSArray arrayWithObjects:eventStartDateSortDescriptor, nil]];
    self.arrayWithEvents = [NSMutableArray arrayWithArray:sortedArray];
}
