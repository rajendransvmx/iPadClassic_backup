...
@interface SMXCalendarViewController () <SMXButtonAddEventWithPopoverProtocol, SMXMonthCalendarViewProtocol, SMXWeekCalendarViewProtocol, SMXDayCalendarViewProtocol,SMActionSideBarViewControllerDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    cTimeLineTimer = [NSTimer scheduledTimerWithTimeInterval:tillNextMinute
                                                      target:self
...
- (void)customNavigationBarLayout
{
    ...
    if (cSegmentedControl == nil) {
       // NSArray *itemArray = [NSArray arrayWithObjects: [[TagManager sharedInstance]tagByName:kTagDay], [[TagManager sharedInstance]tagByName:kTagWeek], [[TagManager sharedInstance]tagByName:kTagMonth],[[TagManager sharedInstance]tagByName:kTagMap], nil];
          ...
          NSArray *itemArray = [NSArray arrayWithObjects: [[TagManager sharedInstance]tagByName:kTagDay], [[TagManager sharedInstance]tagByName:kTagWeek], @"Month",[[TagManager sharedInstance]tagByName:kTagMap], nil];
...
- (IBAction)buttonYearMonthWeekDayAction:(id)sender {
    ...
    if (index == 0) {
        ...
        rightButton.tag=1;
        leftButton.tag=1;
        [rightButton setTitle:kTagAddEventTitle forState:UIControlStateNormal];
        [rightButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [rightButton setTitleColor:[UIColor grayColor] forState:UIControlStateHighlighted];
        self.navigationItem.rightBarButtonItem.enabled = YES;
...
-(void)createNewEvent
{
    ...
    DBCriteria * criteria2 = [[DBCriteria alloc] initWithFieldName:ktype operatorType:SQLOperatorEqual andFieldValue:kProcessTypeStandAloneEdit];
