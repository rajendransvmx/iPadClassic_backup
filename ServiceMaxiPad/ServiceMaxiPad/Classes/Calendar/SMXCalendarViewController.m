...
-(void)checkFOrTimeZoneChange
{
    ...
    if (![currentTimeZone isEqualToTimeZone:[[SMXDateManager sharedManager] timeZone]]) {
        
        // If the timezone is chaged, then the events have to be feteched again from the db.
        [[SMXDateManager sharedManager] setTimeZone:currentTimeZone];
        [self getEventsFromDBAgainAndRender];
    }
}

-(void)getEventsFromDBAgainAndRender
{
...
- (IBAction)buttonYearMonthWeekDayAction:(id)sender {
    ...
    int index = (int)[sender tag];//[arrayButtons indexOfObject:sender];
