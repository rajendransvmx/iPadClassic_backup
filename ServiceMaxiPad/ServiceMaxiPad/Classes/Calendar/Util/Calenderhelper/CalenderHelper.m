...
-(NSArray *) getEventDetailsForTheDay
{
    ...
    for (CalenderEventObjectModel *lModel in lEventArray) {
        ...
        {
            // no SFID, so assuming nothing in server. SO no conflict.
            lModel.conflict = NO;
        }
...
//    NSDateComponents *lTodayComponent = [[NSCalendar currentCalendar] components:NSDayCalendarUnit | NSMonthCalendarUnit | NSYearCalendarUnit fromDate:[NSDate date]];
    ...
    for (CalenderEventObjectModel *lModel in eventArray) {
        ...
        NSDateComponents *conversionInfo = [[NSCalendar currentCalendar] components:unitFlags fromDate:[NSDate date]   toDate:lEventStartDate  options:0];
...
//    lRangeDayInt = 150; //TODO:Remove it. for testing only
    ...
    NSArray *fieldsArray = [[NSArray alloc] initWithObjects:kActivityDate,kActivityDateTime,kDurationInMinutes,kEndDateTime, kStartDateTime, kSubject, kWhatId, kId, klocalId, kEventDescription, nil];
