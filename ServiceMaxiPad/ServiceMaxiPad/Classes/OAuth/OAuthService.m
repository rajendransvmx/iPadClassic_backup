...
+ (NSMutableURLRequest *)getRequestForService:(NSString *)serviceName
{
    ...
    else if ([kOAuthServiceAuthorization isEqualToString:serviceName])
    {
        ...
        NSLog(@" %@ post Data - %@", serviceName, post);
        postData = [post dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
        orgURL   = [[CustomerOrgInfo sharedInstance] identityURL];
        
        NSLog(@" %@ identityURL  - %@", serviceName, orgURL);
    }
    else
    {
    ...
    NSLog(@"%@ body : %@  ", [[request URL] absoluteString], serviceName);
...
+ (BOOL)refreshAccessToken
{
    ...
    else
    {
        ...
        responseString = nil;
         ...
         NSLog(@" Refresh Token reponse : %@ ", [responseDictionary description]);
...
{
    NSLog(@" parseAndSaveCustomerOrgInfoFromResponse ----- %@", responseDictionary);
	//SBJsonParser *parser = [[SBJsonParser alloc] init];
...
+ (void)verifyAuthorization
{
    ...
    else
    {
        ...
        NSLog(@" verifyAuthorization responseData - %@", responseData);

        if (responseData != nil)
        {
            [[self class] explainRequest:request andResponseData:responseData];
            NSLog(@" parseAndSaveCustomerOrgInfoFromResponse ----- %@", responseData);
            SBJsonParser *parser = [[SBJsonParser alloc] init];
