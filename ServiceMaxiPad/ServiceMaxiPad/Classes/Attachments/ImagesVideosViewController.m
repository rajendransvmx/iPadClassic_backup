...
- (void)refreshPage
{
    ...
    if (imageAndVideoView.hidden)
    {
        ...
        {
            if ([_selectedImagesAndVideosArray containsObject:attachmentModel]) {
                attachmentModel.isSelected = YES;
            }
        }
    }
...
- (void)displaySharingView:(NSArray*)urlItems sender:(UIButton *)button
{
    ...
    [sharingView setCompletionHandler:^(NSString *activityType, BOOL completed) {
        SXLogInfo(@"Activity Type = %@ Completed = %d", activityType, completed);
...
- (void) dataFromCapturedImage:(NSData *)capturedImageData {
    ...
    bool canAttach = [AttachmentUtility canAttachthisfile:capturedImageData type:kPngExtension];
    if(canAttach)
    {
        [AttachmentUtility writeAttachmentToDocumentDirectory:capturedImageData localId:local_id withExt:kPngExtension];

        //TODO: update model and save it to database
        [self saveAttachmentInfoDictWithId:local_id andExtension:kPngExtension];
    }
    [self hideImagePickerPopover];
...
{
    NSString *local_id = [AppManager generateUniqueId];
    NSString *outputPath = [AttachmentUtility pathToAttachmentfile:local_id withExt:kMovExtension];
    ...
    if (sizeinMB > 25) {
        ...
        BOOL canAttach= [AttachmentUtility canAttachthisfile:dataAfterCompres type:kMovExtension];
        // Adding details to dict
        if(canAttach)
        {
            [self saveAttachmentInfoDictWithId:local_id andExtension:kMovExtension];
        }
    }
    else {
...
-(void)dealloc
{
    ...
    _sharePopOver = nil;
    [imagesAndVideosArray removeAllObjects];
    imagesAndVideosArray = nil;
    [_selectedImagesAndVideosArray removeAllObjects];
