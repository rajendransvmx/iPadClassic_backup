...
@property(nonatomic, strong) NSMutableDictionary *confirmationDict;
...
- (BOOL) executeBusinessRules{
    ...
    if (![self isBizRuleInfoAvailable]) {
        NSLog(@"Required core library is not available.");
        ...
        if ([bizRuleProcessArray count] == 0) {
            NSLog(@"No biz rule process available");
...
- (BOOL)allWarningsAreConfirmed{
    ...
    if ([self numberOfWarningsInResult] == ([[self.confirmationDict allKeys]count])) {
        return YES;
    }
    return NO;
}
...
- (NSString *) getBizRuleHtmlStringForProcesses:(NSArray *)bizRuleProcessArray{
    ...
    NSString *metaDataStr = [jsonWriter stringWithObject:[bizRuleDict valueForKey:kBizRuleMetaData]];
    NSString *fieldStr = [jsonWriter stringWithObject:[bizRuleDict valueForKey:kBizRuleFields]];
    NSString *dataStr = [jsonWriter stringWithObject:[bizRuleDict valueForKey:KBizRuleData]];
...
- (void)eventOccured:(NSString *)eventName andParameter:(NSString *)jsonParameterString
{
    ...
    if ([eventName isEqualToString:kBizRuleResult])
    {
        ...
        {
            [self.delegate businessRuleFinishedWithResults:bizRuleResultArray];
        }
    }
}
...
#pragma mark -
#pragma mark BizRuleErrorHandler Delegate Method
//Yes indicates user has tapped on Ok to confirm the warning..
- (void) didConfirmWarning:(BOOL)isConfirmed forBizRuleId:(NSString *)ruleId{
    if (nil == self.confirmationDict) {
        self.confirmationDict = [[NSMutableDictionary alloc] init];
    }
    if (nil == self.warningConfirmationDict) {
        self.warningConfirmationDict = [[NSMutableDictionary alloc] init]; //For retaining the State of Button ClickedYes/No
    }
    if (isConfirmed) {
        [self.confirmationDict setValue:[NSNumber numberWithBool:isConfirmed] forKeyPath:ruleId];
    }
    else{
        if ([self.confirmationDict valueForKey:ruleId]) {
            [self.confirmationDict removeObjectForKey:ruleId];
        }
    }
    
    [self.warningConfirmationDict setValue:[NSNumber numberWithBool:isConfirmed] forKeyPath:ruleId];
    [self.delegate handleSaveOnWarningStatus:isConfirmed];
