...
- (void) fillHeaderDataInfo:(NSMutableDictionary *)mainDataDict forFields:(NSDictionary *)fieldsDict{
    ...
    @autoreleasepool {
        ...
        NSDictionary *dict  =  [SFMPageHelper getDataForObject:self.sfmPage.objectName fields:allFields recordId:self.sfmPage.recordId];
        ...
        for (NSString *fieldName in allFields) {
            ...
            if (recField == nil) {
                recField = [[SFMRecordFieldData alloc] initWithFieldName:fieldName value:[dict valueForKey:fieldName] andDisplayValue:[dict valueForKey:fieldName]];
            }
            //Fix for 011535
            NSString *value = @"";
            if([fieldName isEqualToString:kSfDTRecordTypeId])
            {
                ...
                NSString *recordType = [dbService getRecordTypeForId:recField.internalValue objectName:objectName];
                value = recordType;


            }
            else if ([[allFieldDict valueForKey:fieldName] isEqualToString:kSfDTReference]) {
                value = recField.displayValue;
            }
            else if([[allFieldDict valueForKey:fieldName] isEqualToString:kSfDTDateTime]){
                if (![StringUtil isStringEmpty:recField.internalValue]) {
                    value = recField.internalValue;
                }
            }
            else{
                value = recField.internalValue;
        ...
        for (SFMDetailLayout *detailLayout in detailLayoutArray)
        {
            for (ProcessBusinessRuleModel *bizruleProcess in self.bizRuleProcesses) {
                ...
                if ([bizRule.sourceObjectName isEqualToString:detailLayout.objectName] && [bizruleProcess.processNodeObject isEqualToString:detailLayout.processComponentId]) {
                    ...
                    NSMutableArray *pageFields = [[NSMutableArray alloc] init];
                    ...
                    for (NSString *fieldApiName in allFieldsKeys) {
                        SFMPageField *aField = [[SFMPageField alloc] init];
                        aField.fieldName = fieldApiName;
                        [pageFields addObject:aField];
                    }
                    ...
                    for (NSDictionary *detalDict in detailRecords) {
                        ...
                        NSDictionary *dict  = [SFMPageHelper getDataForObject:detailLayout.objectName  fields:allFieldsKeys recordId:localId];
                        ...
                        for (NSString *fieldApiName in allFieldsKeys) {
                            ...
                            if (nil == recordField) {
                                recordField = [[SFMRecordFieldData alloc] initWithFieldName:fieldApiName value:[dict valueForKey:fieldApiName] andDisplayValue:[dict valueForKey:fieldApiName]];
                            }
                            ...
                            {
                                BusinessRuleDatabaseService *dbService = [[BusinessRuleDatabaseService alloc] init];
                                NSString *recordType = [dbService getRecordTypeForId:recordField.internalValue objectName:detailLayout.objectName];
                                value = recordType;
                            }
                            else if ([[allFields valueForKey:fieldApiName] isEqualToString:@"reference"]) {
                                value = recordField.displayValue;
                            }
                            else if([[allFields valueForKey:fieldApiName] isEqualToString:@"datetime"]){
    ...
    if ([[dict allKeys] count]>0) {
        ...
        for (ProcessBusinessRuleModel *process in self.bizRuleProcesses) {
            ...
            if ([bizRule.Id isEqualToString:ruleId]) {
                result.fieldLabel = bizRule.name;
