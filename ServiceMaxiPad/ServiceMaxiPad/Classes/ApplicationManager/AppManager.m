...
- (void)setApplicationStatus:(ApplicationStatus)status
{
    ...
    if (  ( (status == ApplicationStatusAccessTokenExpired) || (status == ApplicationStatusTokenRevoked)))
    {
...
{
    return (applicationStatus == ApplicationStatusTokenRevoked || applicationStatus == ApplicationStatusAccessTokenExpired);
}
...
{
    SXLogDebug(@" CompletedLoginProcessWithStatus --- %d ", status);
...
- (void)serializeDatabaseConnection
{
    NSLog(@"Seerialize db connection");
