...
- (SyncProgressDetailModel *)getDetailsForOneCallRestIntialSync:(WebserviceResponseStatus *)responseStatus
{
    @synchronized([self class])
    {
        ...
        @autoreleasepool
        {
            switch (responseStatus.syncProgressState)
            {
                ...
                case SyncStatusCompleted:
                    message =   @"Donwload complete.";//[[SMXTagsManager sharedTagManager]getTagForId:Tag_Downloading_Data];
                    currentStep = @"6";
                    progress = @"100";
                    break;
                    
                case SyncStatusvalidateProfile:
                    message =  @"Validating Group Profile";//[[SMXTagsManager sharedTagManager]getTagForId:Tag_Downloading_translations];
                    currentStep = @"1";
                    progress = @"5";
                    break;
        ...
        SyncProgressDetailModel * progressModel = [[SyncProgressDetailModel alloc] initWithProgress:progress currentStep:currentStep message:message totalSteps:numOfSteps syncStatus:responseStatus.syncStatus];
        return progressModel;
    }
}
...
- (SyncProgressDetailModel *)getDetailsForValidationProfile:(WebserviceResponseStatus *)responseStatus
{
    @synchronized([self class])
    {
    SyncProgressDetailModel * progressModel = [[SyncProgressDetailModel alloc] initWithProgress:nil currentStep:nil message:@"Validation profile in progress" totalSteps:nil syncStatus:responseStatus.syncStatus];
    return progressModel;
    }
}
@end
