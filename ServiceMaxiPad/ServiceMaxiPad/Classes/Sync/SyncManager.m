...
//    NSString *appDirPath = [(AppDelegate*)[[UIApplication sharedApplication] delegate] getAppCustomSubDirectory];
    ...
    return [[FileManager getRootPath]stringByAppendingPathComponent:syncMetaDataFile];
}
...
- (void)performInitialSync
{
    ...
    TaskModel *taskModel = [TaskGenerator generateTaskFor:CategoryTypeOneCallRestIntialSync requestParam:nil callerDelegate:self];
...
- (void)performConfigSync
{
    TaskModel *taskModel = [TaskGenerator generateTaskFor:CategoryTypeIncrementalOneCallMetaSync requestParam:nil callerDelegate:self];
    [[TaskManager sharedInstance] addTask:taskModel];
}
...
{
    TaskModel *taskModel = [TaskGenerator generateTaskFor:CategoryTypeValidateProfile requestParam:nil callerDelegate:self];
    [[TaskManager sharedInstance] addTask:taskModel];
}
...
- (BOOL)initiateDataSync {
    @synchronized([self class]) {
        ...
        if (self.isDataSyncRunning) {
            NSLog(@" Data sync is already running ");
            return NO;
        }
...
- (void)currentDataSyncfinished {
    @synchronized([self class]) {
        ...
        if (!didRestart) {
            ...
            [self updatePlistWithLastDataSyncTimeAndStatus:[[TagManager sharedInstance] tagByName:kTagPushLogStausSuccess]];
            ...
            //[SMXiPhone_UserDefaultsUtility clearAllWhatIdObjectInformation];
...
- (void)currentDataSyncFailedWithError:(NSError *)error {
    ...
    @synchronized([self class]) {
        ...
        [[AlertMessageHandler sharedInstance] showCustomMessage:[error errorEndUserMessage] withDelegate:nil tag:alertViewTagForConfigSync title:@"Error" cancelButtonTitle:@"OK" andOtherButtonTitles:nil];
    }
...
{
    [self updatePlistWithLastConfigSyncTimeAndStatus:[[TagManager sharedInstance] tagByName:kTagPushLogStausSuccess]];
}
...
- (void)currentConfigSyncFailedWithError:(NSError *)error
{
    ...
    if (error) {
        ...
        //}
        [[AlertMessageHandler sharedInstance] showCustomMessage:[error errorEndUserMessage] withDelegate:self tag:alertViewTagForConfigSync title:@"Error" cancelButtonTitle:nil andOtherButtonTitles:@[@"Ok"]];
    }
}
...
- (void)profileValidationFailedWithError:(NSError *)error
{
    ...
    if (error) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"Error" message:[error errorEndUserMessage] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alertView show];
