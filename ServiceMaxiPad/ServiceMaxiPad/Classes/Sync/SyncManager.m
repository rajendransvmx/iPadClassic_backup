...
- (void)currentInitialSyncFailedWithError:(NSError *)error
{
    ...
    if (error) {
        ...
        [[AlertMessageHandler sharedInstance] showCustomMessage:[error errorEndUserMessage] withDelegate:self tag:alertViewTagForInitialSync title:[[TagManager sharedInstance]tagByName:kTagSyncErrorMessage] cancelButtonTitle:nil andOtherButtonTitles:@[[[TagManager sharedInstance]tagByName:kTagAlertErrorOk]]];//HS added 23 oct
    }
}
...
{
    if (self.dataSyncStatus == SyncStatusInProgress)
    {
        return YES;
    }
    else
