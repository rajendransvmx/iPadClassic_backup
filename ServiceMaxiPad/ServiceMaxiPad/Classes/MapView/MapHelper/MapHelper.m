...
+ (NSArray*) eventsOfCurrentDay {
    ...
    NSDate *startDate = [DateUtil dateWithOutTimeInGMT:[NSDate date]];
    NSDate *endDate = [DateUtil dateWithOutTimeInGMT:[NSDate tomorrow]];
...
+ (ServiceLocationModel*)getTechnicianAddress {
    ...
    NSArray *fieldsArray = [[NSArray alloc] initWithObjects:kStreet,kCity,kState,kCountry, kPostalCode, kLongitude, kLatitude, nil];
    DBCriteria *criteria = [[DBCriteria alloc] initWithFieldName:kId operatorType:SQLOperatorEqual andFieldValue:[self ownerId]];
    DBRequestSelect * requestSelect = [[DBRequestSelect alloc] initWithTableName:@"User" andFieldNames:fieldsArray whereCriteria:criteria];
    __block NSDictionary *addressDictionary = nil;
    @autoreleasepool {
    ...
    [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
        ...
        while ([resultSet next]) {
            addressDictionary = [resultSet resultDictionary];
        }
        [resultSet close];
    }];
    }
    ServiceLocationModel *serviceLocationModel = [[ServiceLocationModel alloc] initWithAddressDictionary:addressDictionary];
