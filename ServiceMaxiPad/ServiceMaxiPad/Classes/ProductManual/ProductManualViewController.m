...
- (void)viewDidLoad {
    ...
    NSLog(@" product %@",self.productName);
...
{
    if (![self.productId isKindOfClass:[NSNull class]]
        && ![self.productId isEqualToString:@""])
    {
        ...
        else
        {
           ...
           self.masterViewController .productDetailsArray = self.productDetailsArray;
            ...
            else
            {
                ...
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:title message:@"NO Product Manuals Availabel For The Product" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                [alertView show];
            }
...
-(NSArray *)getPdfFileNamesOnlyFromTheArray:(NSArray *)modelArray
{
    ...
    for (ProductManualModel *model in modelArray)
    {
       ...
       if( [[fileName substringFromIndex:[fileName length]-4]  isEqual:@".pdf"] )
       {
...
- (void)loadTableViewAndWebView
{
    ...
    [self.masterViewController.tableView selectRowAtIndexPath:firstRowPath animated:NO scrollPosition: UITableViewScrollPositionNone];
    [self.masterViewController tableView:self.masterViewController.tableView didSelectRowAtIndexPath:firstRowPath];
    [self.detailViewController setContentWithItem:title];
    [self.detailViewController loadWebViewForTheProductName:model.prod_manual_name AndProductManualID:model.prod_manual_Id];
}
