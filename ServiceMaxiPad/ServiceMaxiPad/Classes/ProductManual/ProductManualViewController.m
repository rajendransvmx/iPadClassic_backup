        ...
        switch (st.category)
        {
            case CategoryTypeProductManual:
            {
                ...
                if (st.syncStatus == SyncStatusSuccess)
                {
                    ...
                    if([self.productDetailsArray  count] >0)
                    {
                       // self.masterViewController.productDetailsArray = [self getPdfFileNamesOnlyFromTheArray:self.productDetailsArray];[self.masterViewController .tableView reloadData];
