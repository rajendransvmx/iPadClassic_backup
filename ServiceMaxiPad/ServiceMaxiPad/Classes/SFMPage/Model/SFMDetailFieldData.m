    ...
    if (![StringUtil isStringEmpty:self.expression]) {
        if ([criteriaObjects count] == 2) {
            if ([self.criteriaObjects count] > 0) {
                newExpression = [[NSString alloc] initWithFormat:@"(%@) AND (%d OR %d)",self.expression,([self.criteriaObjects count]+1), ([self.criteriaObjects count]+2)];
            }
            else {
            ...
            if ([self.criteriaObjects count] > 0) {
                 newExpression = [[NSString alloc] initWithFormat:@"(%@) AND %d",self.expression,([self.criteriaObjects count]+1)];
            }
        }
    }
    else {
