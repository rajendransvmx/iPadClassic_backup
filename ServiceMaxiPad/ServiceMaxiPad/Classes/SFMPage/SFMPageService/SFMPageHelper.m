...
+ (NSArray *)getDetialsRecord:(SFMDetailFieldData *)detailData
{
    ...
    if ([detailData shouldApplySortingOrder]) {
        ...
        resultSet = [transactionObject fetchDataForObject:detailData.objectName fields:fieldArray
                                               expression:detailData.expression
...
+ (NSArray *)getAccountHistoryInfo:(NSString *)objectName localId:(NSString *)recordId;
{
    ...
    if ((![accounId isKindOfClass:[NSNull class]] && [accounId length] > 0) && [Id length] > 0) {
        ...
        array = [self getPageHistortInfo:objectName sfId:Id requiredCriteria:criteria];
...
+ (NSArray *)getProductHistoryInfo:(NSString *)objectName localId:(NSString *)recordId
{
    ...
    if (![Id isKindOfClass:[NSNull class]] && [Id length] > 0) {
        ...
        if (![productId isKindOfClass:[NSNull class]] && [productId length] > 0) {
    ...
    if ([criteriaArray count] == 2) {
        array =  [self getPageHistortInfo:objectName sfId:Id requiredCriteria:criteriaArray];
    }
    else if ([criteriaArray count] == 1) {
        array = [self getPageHistortInfo:objectName sfId:Id requiredCriteria:[criteriaArray objectAtIndex:0]];
    }
...
+ (NSArray *)processHistoryData:(NSArray *)array
{
    ...
    NSArray *fields = [NSArray arrayWithObjects:kWorkOrderProblemDescription, @"CreatedDate", nil];
...
{
    return [self getRequiredInfoForPageHistory:objectName localId:recordId fields:[NSArray arrayWithObjects:kWorkOrderCompanyId, kId, nil]];
}

+(TransactionObjectModel *)getRequiredInfoForProductHistory:(NSString *)objectName localId:(NSString *)recordId
{
    
    return [self getRequiredInfoForPageHistory:objectName localId:recordId fields:[NSArray arrayWithObjects:kComponentId, kTopLevelId, kId, nil]];
}
...
+ (NSArray *)getPageHistortInfo:(NSString *)objectName sfId:(NSString *)sfId requiredCriteria:(id)criteria
{
    id <TransactionObjectDAO> transactionObject = [FactoryDAO serviceByServiceType:ServiceTypeTransactionObject];
    ...
    if ([todaysDate length] > 0) {
        ...
        DBCriteria *criteria1 = [[DBCriteria alloc] initWithFieldName:@"CreatedDate" operatorType:SQLOperatorLessThanEqualTo andFieldValue:todaysDate];
        ...
        dataArray = [transactionObject fetchDataForObject:objectName fields:[NSArray arrayWithObjects:kWorkOrderProblemDescription, @"CreatedDate", nil] expression:advanceExpression criteria:criteriaArray];
    }
