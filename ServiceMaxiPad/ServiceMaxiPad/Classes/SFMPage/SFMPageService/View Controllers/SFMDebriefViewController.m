...
//    self.debriefTableView.layer.borderWidth = 1;
    NSLog(@"%@",NSStringFromCGRect(self.debriefTableView.frame));
...
-(void)addDbriefViewForSection:(NSInteger)section
{
   ...
   // debriefView
    ...
    [debriefView setSelectedSection:section];
...
-(CGFloat)getHeightForSection:(NSInteger)section
{
    ...
    NSLog(@"cVHeight %f ection %d",height,section);
    return height;
}
...
-(NSString *)titleForSection:(NSInteger)section
{
    ...
    {
        NSDictionary * recordDict = [detailRecords objectAtIndex:0];
...
-(void)setBorder
{
    ...
    CGRect  layerFrame = CGRectMake(self.debriefTableView.bounds.origin.x
                             , self.debriefTableView.bounds.origin.y,  self.debriefTableView.contentSize.width , contentViewRect);
    ...
    NSLog(@"Layerframe%@",NSStringFromCGRect(layerFrame));
