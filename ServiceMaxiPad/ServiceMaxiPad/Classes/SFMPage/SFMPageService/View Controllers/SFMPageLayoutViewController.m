...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([pageField.dataType isEqualToString:kSfDTReference]) {
        ...
        [((SFMPageReferenceFieldCollectionViewCell *)cell) isRefernceRecordExist:recordData.isReferenceRecordExist];//recordData.isReferenceRecordExist
        [((SFMPageReferenceFieldCollectionViewCell *)cell) setIndex:indexPath.item];
        
        if ([pageField.fieldName isEqualToString:kWorkOrderContactId]) {
    ...
    if (textSize.width > cell.frame.size.width) {
...
-(void)showSFMPageViewForRerenceField:(NSInteger)index
{
    ...
    if ([recordData.internalValue length] > 0) {
        ...
        SFMViewPageManager *pageManager = [[SFMViewPageManager alloc] initWithObjectName:pageField.relatedObjectName recordId:recordData.internalValue];
...
- (void)openContactMeaageOrMail:(id)sender
{
    UIButton *button = (UIButton *)sender;
    
    if (button.tag == 888) {
        NSString *contactNumber  = self.sfmPageView.contactNUmber;
        ...
        NSString *mail  = self.sfmPageView.contactMail;
