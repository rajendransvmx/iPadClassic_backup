    ...
    switch (indexPath.section) {
        ...
        case SFMPageMasterSectionTypeHistory:/*Additional Info*/
            if (headerLayout.enableProductHistory &&  headerLayout.enableAccountHistory) {
                if (indexPath.row == 0) {
                    title = kTag_AccountHistory_Records;//[tagManager tagByName:kTag_AccountHistory];
                }
                else if (indexPath.row == 1) {
                    title = kTag_ProductHistory_Records;//[[TagManager sharedInstance]tagByName:kTag_ProductHistory_Records];
                }
            }
            else if(headerLayout.enableProductHistory)
            {
                title = kTag_ProductHistory_Records;//[tagManager tagByName:kTag_ProductHistory_Records];
            }
...
- (BOOL)isAccountHistory:(NSIndexPath *)indexPath
{
    ...
    NSString *title = kTag_AccountHistory_Records;//[[TagManager sharedInstance] tagByName:kTag_AccountHistory];
...
- (BOOL)isProductHistory:(NSIndexPath *)indexPath
{
    ...
    if([[self rowTitleForIndexPath:indexPath] isEqualToString:kTag_ProductHistory_Records])
    {
        result = YES;
    }
    return result;
