...
- (void)setTableViewFrame
{
    ...
    int numberOfRows = [self.historyInfo count] * 2;
...
- (UIView *) viewForTableFooter
{
    ...
    [seeMore setTitle:@"Include Online Items" forState:UIControlStateNormal];
...
- (NSString *)titleForHeaderInSection
{
    ...
    if (self.historyInfoType == HistoryTypeAccount) {
        title = [[TagManager sharedInstance] tagByName:kTagSfmLeftPaneAccountHistory];
    }
    else if (self.historyInfoType == HistoryTypeProduct){
        title =  @"Product History and Records";//[[TagManager sharedInstance] tagByName:kTagSfmLeftPaneProductHistory];
    }
    return title;
}
