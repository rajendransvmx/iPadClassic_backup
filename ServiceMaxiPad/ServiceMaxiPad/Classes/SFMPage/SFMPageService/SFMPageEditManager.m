    ...
    if (recordUpdatedSuccessFully) {
        ...
        BOOL doesExist =   [modifiedRecordService doesRecordExistForId:page.recordId];
         if (!doesExist) {
             ...
             if (([syncRecord.operation isEqualToString:kModificationTypeUpdate]) && ([StringUtil isStringEmpty:syncRecord.sfId])) {
...
//        
//        [self applyFieldMapping:mapppingArray forRecord:headerDictionary andSourceDictionary:sourceDataDict objectname:sfmPage.objectName];
//        
//        
//        /* Set preferences */
//        
...
//         NSDictionary *fieldinfo = [self getFieldInForObject:sfmPage.objectName];
...
- (void)applyFormFillOnChildRecordOfIndexPath:(NSIndexPath *)selectedIndexPath
                             recordDictionary:(NSMutableDictionary *)recordDictionary
    ...
    if ([mapppingArray count] > 0) {
         NSDictionary *sourceDataDict = [SFMPageEditHelper getAllDataForObject:pageField.relatedObjectName andRecordId:selctedRecordField.internalValue];
         [self applyFieldMapping:mapppingArray forRecord:recordDictionary andSourceDictionary:sourceDataDict objectname:objectName];
