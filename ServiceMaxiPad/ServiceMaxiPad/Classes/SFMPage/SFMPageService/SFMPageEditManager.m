...
- (void)saveHeaderRecord:(SFMPage *)page {
    ...
    //[self performSuccessiveSync:syncRecord withData:page.headerRecord];
...
- (void)saveDetailRecords:(SFMPage *)sfmPage {
    ...
    for(NSString * processCompId in allDetailProcessComponents)
    {
        ...
        for (NSMutableDictionary * eachDetailDict in detailRecordsArray)
        {
            ...
            //[self performSuccessiveSync:syncRecord withData:eachDetailDict];
                              ...
                              sfmPage:(SFMPage *)sfmPage
{
    ...
    if(dict == nil){
        ...
        [sfmPage.sourceTargetRecordMap setObject:dict forKey:compId];
    }
    ...
    for (NSString * targetRecordId in [targetRecordsDict allKeys]) {
        ...
        for (SFSourceUpdateModel * model in sourceUpdateConfig) {
            ...
            if(![StringUtil isStringEmpty:model.targetObjectName]){
               ...
               SFMRecordFieldData * recordField = [eachTargetDict objectForKey:model.targetObjectName];
            ...
            if([model.action isEqualToString:@"Set"]){
                ...
                NSString * fieldType =  [fieldDataType objectForKey: model.sourceObjectName];
               ...
               NSString * sourceFieldValue  = [eachSourcedDict objectForKey: model.sourceObjectName];
               float finalFloatValue = [finalValue floatValue] + [sourceFieldValue floatValue];
                finalValue = [NSString stringWithFormat:@"%f",finalFloatValue];
            }
            else if([model.action isEqualToString:@"decrease"]){
                NSString * sourceFieldValue  = [eachSourcedDict objectForKey: model.sourceObjectName];
                float finalFloatValue = [sourceFieldValue floatValue] + [finalValue floatValue];
                finalValue = [[NSString alloc] stringByAppendingFormat:@"%f",finalFloatValue];
            }
            
            if(![StringUtil isStringEmpty:finalValue]){
                [eachSourcedDict setObject:finalValue forKey:model.sourceObjectName];
            }
        }
    }
...
-(NSMutableDictionary *)getSourceRecordsForObjectName:(NSString *)sourceObjName fieldsArray:(NSArray *)fieldsArray criteriaRecords:(NSArray *)criteriaRecords
{
    ...
    NSArray * sourceRecords = [transactionObject fetchDataForObject:sourceObjName fields:fieldsArray expression:@" 1 " criteria:[NSArray arrayWithObject:criteria]];
...
-(NSArray *)getFieldsArrayForSourceUpdate:(NSArray *)sourceUpdateConfig
{
   NSMutableArray * fieldsArray = [[NSMutableArray alloc] init];
    for (SFSourceUpdateModel * sourceUpdateModel in sourceUpdateConfig) {
        NSString *sourcefieldName =  sourceUpdateModel.sourceObjectName;
...
#pragma mark -
#pragma mark Successive Sync Methods
- (void) performSuccessiveSync:(ModifiedRecordModel *)syncRecord withData:(NSDictionary *)recordDict
{
    SuccessiveSyncManager *succSyncManager = [SuccessiveSyncManager sharedSuccessiveSyncManager];
    if ([succSyncManager shouldPerformSyccessiveSync:syncRecord.recordLocalId]) {
        NSLog(@"Successive sync call");
        syncRecord.recordDictionary  = [[NSMutableDictionary alloc]initWithDictionary:recordDict];
        [succSyncManager updateSuccessiveSyncRecord:syncRecord];
    }
