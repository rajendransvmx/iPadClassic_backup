                                 ...
                                 sfmpage:(SFMPage *)newSfmPage
{
    ...
    if ([pageField.fieldName isEqualToString:kSfDTRecordTypeId] && [pageField.dataType isEqualToString:kSfDTReference])
    {
        ...
        for (SFRecordTypeModel *model in picklistData) {
            if (model != nil) {
                [array addObject:model.recordtypeLabel];
            }
        }
    }
    else
    {
        ...
        for (SFPicklistModel *model in picklistData) {
            if (model != nil) {
...
- (void)deleteRecordIds:(NSArray *)deletedRecordIds
    ...
    {
       BOOL isRecordExist =  [transObjectService isRecordExistsForObject:processComponent.objectName forRecordLocalId:deletedId];
        if(!isRecordExist)
        {   continue;
        }
