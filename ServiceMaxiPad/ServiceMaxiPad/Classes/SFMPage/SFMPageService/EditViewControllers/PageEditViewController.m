...
- (void)businessRuleFinishedWithResults:(NSMutableArray *)resultArray
{
    ...
    if (([resultArray count]== 0) || (resultArray == nil) || ([self.ruleManager allWarningsAreConfirmedForBusinessRuleResult:resultArray] && ([self.ruleManager numberOfErrorsInResult] == 0))) {
        [self performSelectorInBackground:@selector(saveRecord) withObject:nil];
        return;
    }
    else
    {
        /*Display the biz rule result in UI*/
    }

   // NSMutableArray * dataarray = [[NSMutableArray alloc] initWithArray:resultArray];
