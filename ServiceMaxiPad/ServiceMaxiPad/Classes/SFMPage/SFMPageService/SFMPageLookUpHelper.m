...
-(NSArray *)getWhereclause:(SFMLookUp *)lookUpObj 
{
   ...
   /* if ([lookUpObj.preFilters count] > 0) {
    ...
    if ([lookUpObj.advanceFilters count] > 0) {
        NSArray *filerCriteria = [self getCriteriaArrayForPreFilters:lookUpObj.advanceFilters];
        if ([filerCriteria count] > 0) {
            [criteriaArray addObjectsFromArray:filerCriteria];
        }
    }*/
...
- (NSArray *)getCriteriaObjectForfilter:(SFMLookUpFilter *)filter {
    ...
    return [expressionParser expressionCriteriaObjectsForFilters];
}
...
{
    int value = 0;
    NSMutableString *modifiedExpression = [[NSMutableString alloc] init];
