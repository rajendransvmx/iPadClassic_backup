...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
			// SMLog(kLogLevelVerbose,@"%@", [NSString stringWithFormat:@"%d entries in the zip file",globalInfo.number_entry]);
		}
	}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		...
		if( isDirectory )
            ...
            [fman createDirectoryAtPath:fullPath attributes:nil];
		else
        ...
        // SMLog(kLogLevelVerbose,@"%@", [fullPath stringByDeletingLastPathComponent]);
        if (error != nil)
		...
		if( fp )
		{
			...
			if( attr )
			{
				...
				if( ![[NSFileManager defaultManager] setAttributes:attr ofItemAtPath:fullPath error:nil] )
				{
					...
					// SMLog(kLogLevelVerbose,@"Failed to set attributes");
				}
