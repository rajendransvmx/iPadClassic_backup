...
- (BOOL)closeDatabase
{
    ...
    [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
        ...
        if (!sucessFull)
        {
            if ([db hadError])
            {
                SXLogError(@"Database closing failed with error : %@ ", [db lastErrorMessage]);
            }
        }
    }];
    }
...
@autoreleasepool {
    ...
    [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
        ...
        SQLResultSet * resultSet = [db executeQuery:@"PRAGMA page_size = 64", nil];
        
        if (!pageSizeResult)
        {
            if ([db hadError])
            {
                SXLogError(@"page_size Error: %@ ", [db lastErrorMessage]);
            }
        }
        [resultSet close];
        ...
        resultSet = [db executeQuery:@"PRAGMA synchronous = OFF"];
        
        if (!syncResult)
        {
            if ([db hadError])
            {
                SXLogError(@"synchronous Error : %@ ", [db lastErrorMessage]);
            }
        }
        ...
        if (!journalResult)
        {
            if ([db hadError])
            {
                SXLogError(@"journal_mode Error: %@ ", [db lastErrorMessage]);
            }
        }
        [resultSet close];
        
        BOOL pageCacheSizeResult = YES;
        resultSet = [db executeQuery:@"PRAGMA cache_size = 50"];
        if  (!pageCacheSizeResult)
        {
            if ([db hadError]) {
                SXLogError(@"pageCacheSize Error: %@ ", [db lastErrorMessage]);
            }
        }
    }];
}
    ...
    for (int i = 0; i < [tableSchemaArray count]; i++)
    {
        ...
        [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
            ...
            if (!sucessFull)
            {
                if ([db hadError])
                {
                    SXLogError(@"Create table failed with error : %@ ", [db lastErrorMessage]);
                }
            }
        }];
        }
    ...
    for (int i = 0; i < [createTableIndexSchemaArray count]; i++)
    {
        ...
        [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
            ...
            if (!sucessFull)
            {
                if ([db hadError])
                {
                    SXLogError(@"Create table index failed with error : %@ ", [db lastErrorMessage]);
                }
            }
        }];
        }
    ...
    for (int i = 0; i < [dropTableIndexSchemaArray count]; i++)
    {
        ...
        [queue inTransaction:^(SMDatabase *db, BOOL *rollback) {
            ...
            if (!sucessFull)
            {
                if ([db hadError])
                {
                    SXLogError(@"Drop Table Index failed : %@ ", [db lastErrorMessage]);
                }
            }
        }];
        }
