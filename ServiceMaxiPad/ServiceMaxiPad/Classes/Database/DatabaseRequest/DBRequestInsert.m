...
@property(nonatomic)BOOL shouldAddReplaceToQuery;
...
- (NSString *)query {
    @synchronized([self class]){
        @autoreleasepool {
            ...
            if (self.shouldAddReplaceToQuery) {
                query = [[NSMutableString alloc] initWithFormat:@"INSERT OR REPLACE INTO '%@' ",self.tableName];
            }
            else{
                query = [[NSMutableString alloc] initWithFormat:@"INSERT INTO '%@' ",self.tableName];
            }
...
- (void)setInsertOrReplaceOption {
    self.shouldAddReplaceToQuery = YES;
}
