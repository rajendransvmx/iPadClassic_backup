...
static NSString *const kMainDatabaseFileName          = @"sfm";
static NSString *const kMainDatabaseFileExtension     = @"sqlite";
static NSString *const kSecondaryDatabaseFileName     = @"temporary";
...
- (BOOL)createEditableDatabaseIfNeeded
{
    ...
    self.databasePath = [documentFolderPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@", kMainDatabaseFileName, kMainDatabaseFileExtension]];
...
{
    [self close];
    DBRelease(openResultSets);
    DBRelease(cachedStatements);
    DBRelease(openFunctions);
...
{
    DatabaseManager *self = (__bridge DatabaseManager*)f;
...
{
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"DB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets
{
    ...
    NSSet *openSetCopy = (NSSet *) DBReturnAutoreleased([openResultSets copy]);
...
- (void)bindObject:(id)obj toColumn:(int)idx inStatement:(sqlite3_stmt*)pStmt
{
    ...
    else if ([obj isKindOfClass:[NSNumber class]])
    {
        ...
        {
            sqlite3_bind_text(pStmt, idx,([obj boolValue] ? [kTrue UTF8String] :[kFalse UTF8String]), -1, SQLITE_STATIC);
        }
                  ...
                  orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args
{
    ...
    DBRetain(statement); // to balance the release below
