           ...
           advanceExpression:(NSString *)expression
{
    ...
    {
        newAdvExpr = [[NSString alloc] initWithFormat:@"(%@) AND %d",expression,(int)([criteriaArray count]+1)];
    }
    else if([StringUtil isStringEmpty:expression])
    {
        newAdvExpr = [[NSString alloc] initWithFormat:@"%d",(int)([criteriaArray count]+1)];
    }
