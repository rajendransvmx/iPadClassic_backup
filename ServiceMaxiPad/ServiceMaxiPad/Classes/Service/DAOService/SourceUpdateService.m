...
- (void)updateTargetObjectsForSmartDocProcess:(NSString *)processId forObject:(NSString*)objectName andLocalId:(NSString*)localId
{
    ...
    for (SFSourceUpdateModel *model in srcUpdtList)
    {
        ...
        [model setValue:escapedValue forField:srcFieldName];
        
        DBCriteria *crit = [[DBCriteria alloc] initWithFieldName:@"localId" operatorType:SQLOperatorEqual andFieldValue:localId];
        DBRequestUpdate *reqUpdate = [[DBRequestUpdate alloc] initWithTableName:objectName andFieldNames:@[srcFieldName] whereCriteria:@[crit] andAdvanceExpression:nil];
        recordUpdatedSuccessFully = [self updateEachRecord:model withQuery:[reqUpdate query]];
