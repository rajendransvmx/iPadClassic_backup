...
{
    NSString *query = [NSString stringWithFormat:@"CREATE TABLE SFCameraAttachmentData ('record_Id' VARCHAR,'object_api_name' VARCHAR,'attachment_data' TEXT,'attachment_Id' TEXT,'WorkOrderNumber' VARCHAR,'fileName' VARCHAR,'fileType' VARCHAR,'size' VARCHAR,'local_id' INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL  DEFAULT 0 )"];
...
- (NSData *) getAttachmentDataForFile:(NSString *)fileName 
{
    ...
    {
        NSLog(@"Unable to execute the Statement = %@",query);
    }
...
- (BOOL) deleteAttachmentFromTableWithName:(NSString *)fileName withRecordId:(NSString *)recordId
{
    ...
    { 
        ...
        {
            status = FALSE;
            NSLog(@"Failed to delete the attachment");
        }
    }
    synchronized_sqlite3_finalize(statement);
    return status;
