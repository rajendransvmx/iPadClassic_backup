...
@synthesize attachmentEditView;
@synthesize attachmentEditImageView;
@synthesize attachmentEditTextField;
@synthesize attachmentEditSize;
    ...
    switch (section) {
        ...
        */
        ...
        case SHOW_ADDITIONALINFO_ROW:
            ...
            [tableView reloadData];
            break;
        case SHOW_CAMERA_ROW:
            isDefault = NO;
            selectedSection = SHOW_CAMERA_ROW;
            selectedRow = row;
            [tableView reloadData];
            break;
        default:
...
-(void) didselectSection:(NSInteger) section;
{
    ...
    switch (section) 
    {
        ...
        case 2:
            ...
            break;
        case 3:
            selectedSection = SHOW_ALL_CAMERA ;
            isDefault= YES;
            [tableView  reloadData];
            break;
        default:
...
- (void)viewDidLoad
{
    ...
    camera = [appDelegate.wsInterface.tagsDictionary objectForKey:CAM_CameraModule_Name];
    video = [appDelegate.wsInterface.tagsDictionary objectForKey:CAM_VideoModule_Name];
...
- (void) action:(id)sender
{
    ...
    NSMutableDictionary * dict_events_camera;
    NSMutableDictionary * dict_events_video;
    if([appDelegate isCameraAvailable])
    {
        objects_event = [NSArray arrayWithObjects:@"",camera,@"",@"",gBUTTON_TYPE_TDM_IPAD_ONLY ,@"",@"true",nil];
        keys_event = [NSArray arrayWithObjects:SFW_ACTION_ID,SFW_ACTION_DESCRIPTION,SFW_EXPRESSION_ID,SFW_PROCESS_ID,SFW_ACTION_TYPE ,SFW_WIZARD_ID,SFW_ENABLE_ACTION_BUTTON,nil]; 
        dict_events_camera = [NSMutableDictionary dictionaryWithObjects:objects_event forKeys:keys_event];

        objects_event = [NSArray arrayWithObjects:@"",video,@"",@"",gBUTTON_TYPE_TDM_IPAD_ONLY ,@"",@"true",nil];
        keys_event = [NSArray arrayWithObjects:SFW_ACTION_ID,SFW_ACTION_DESCRIPTION,SFW_EXPRESSION_ID,SFW_PROCESS_ID,SFW_ACTION_TYPE ,SFW_WIZARD_ID,SFW_ENABLE_ACTION_BUTTON,nil]; 
        dict_events_video = [NSMutableDictionary dictionaryWithObjects:objects_event forKeys:keys_event];
    }
   ...
   // if ([headerButtons count] > 0)
    ...
    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"VIEWRECORD"]) 
    {
        ...
        if([heder_object_name isEqualToString:@"SVMXC__Service_Order__c"])
        {
            ...
            if([appDelegate isCameraAvailable])
            {
                [ipad_only_array addObject:dict_events_camera];
                [ipad_only_array addObject:dict_events_video];
            }            
        }
    }
    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"EDIT"]) 
...
- (void) startSummaryDataFetch
{
    ...
    [(SFMPageController *)delegate presentModalViewController:Summary animated:YES];
...
//    NSDictionary * headerData = [headerDataDict objectForKey:gHEADER_DATA];
  ...
  /*  troubleshooting.productId = [self getObjectNameFromHeaderData:headerData forKey:gSVMXC__Product__c];
    ...
    [(SFMPageController *)delegate presentModalViewController:troubleshooting animated:YES];
...
- (void) showChatter
{
    ...
    [(SFMPageController *)delegate presentModalViewController:_chatter animated:YES];
    [_chatter release];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath 
{
    ...
    if (!isInEditDetail)
    {
        ...
        {
            if (selectedSection == SHOW_ALL_CAMERA || selectedSection == SHOW_CAMERA_ROW)
            {
                return 100;
            }
...
//                count_info = [[appDelegate.SFMPage objectForKey:ACCOUNTHISTORY] count];
        ...
        else  if( selectedSection == SHOW_ALL_CAMERA || selectedSection == SHOW_CAMERA_ROW)
        {
            if(index == 0)
            {
                return @"Images";
...
- (UIView *) tableView:(UITableView *)_tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if(selectedSection == SHOW_ALL_CAMERA || selectedSection == SHOW_CAMERA_ROW)
    {
        ...
        assetsButton.frame = CGRectMake(650, 0, 31, 31);
        [assetsButton addTarget:self action:@selector(addImages:) forControlEvents:UIControlEventTouchUpInside];
        UIImageView *buttonImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"addAttachment.png"]];
        [buttonImage setFrame:CGRectMake(0, 0, 31, 31)];
        [assetsButton addSubview:buttonImage];
        if(selectedSection == SHOW_ALL_CAMERA)
        {
            if(section == 0)
                [assetsButton setTag:0];
            else
                [assetsButton setTag:1];
        }
        else
        {
            if(selectedRow == 0)
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    if (!isInEditDetail)
    {
        if (isDefault)
        {
            ...
            else if (selectedSection == SHOW_ALL_CAMERA)
            {
                return 2;
            }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (!isInEditDetail)
    {
        if (isDefault)
        {
            ...
            {
                //get all the images and videos count and return.
                NSString *recordId = appDelegate.sfmPageController.recordId;
                if(section == 0)
                {
                    imagesArray = [[appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Image"] retain]; 
                    return [imagesArray count];
                }
                else if(section == 1)
                {
                    videosArray = [[appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Video"] retain]; 
        ...
        else
        {
            ...
            else if(selectedSection == SHOW_CAMERA_ROW)
            {
                // get the count for selectedRow and return 
                
                NSString *recordId = appDelegate.sfmPageController.recordId;
                if(selectedRow == 0)
                {
                    NSArray *images = [appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Image"]; 
                    NSLog(@"Return Row %d count = %d",selectedRow,[images count]);
                    return [images count];
                }
                else if(selectedRow == 1)
                {
                    NSArray *videos = [appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Video"]; 
                    NSLog(@"Return Row %d count = %d",selectedRow,[videos count]);
                    return [videos count];
                }                
...
//        isCellNew = NO;
    ...
    else if(selectedSection == SHOW_ADDITIONALINFO_ROW || selectedSection == SHOW_ALL_ADDITIONALINFO)
    {
     ...
     if (selectedSection == SHOW_CAMERA_ROW || selectedSection == SHOW_ALL_CAMERA)
     {
         NSString *recordId = appDelegate.sfmPageController.recordId;
         imagesArray = [[appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Image"] retain]; 
         videosArray = [[appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Video"] retain]; 
         ...
         [cell setAccessoryType:UITableViewCellAccessoryNone];
         [background setTag:4];
        if(((selectedSection == SHOW_ALL_CAMERA) && (section == 0)) ||
           ((selectedSection == SHOW_CAMERA_ROW) && (index == 0))
           )
        {
            UIImageView *attachmentImage = [[[UIImageView alloc] initWithFrame:CGRectMake(10, 0, 90, 90)] autorelease];
            NSString *name = [[imagesArray objectAtIndex:indexPath.row] objectForKey:@"Name"];
            UIImage *image = [[UIImage alloc] initWithData:[appDelegate.dataBase getAttachmentDataForFile:name]];
            image = [self scaleImage:image toSize:CGSizeMake(90, 90)];
            UITapGestureRecognizer * tapMe = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(attachmentImageTapped:)];
            [attachmentImage addGestureRecognizer:tapMe];
            [tapMe release];

            [attachmentImage setImage:image];
            [background addSubview:attachmentImage];

            UITextField *nameTxtField = [[UITextField alloc] initWithFrame:CGRectMake(110, 10, 60, control_height)];
            [nameTxtField setText:[NSString stringWithFormat:@"Name :"]];
            [nameTxtField setTextColor:[UIColor blackColor]];
            [nameTxtField setFont:[UIFont fontWithName:@"HelveticaBold" size:19]];
            [nameTxtField setFont:[UIFont boldSystemFontOfSize:nameTxtField.font.pointSize]];
            [nameTxtField setEnabled:NO];
            [nameTxtField setBackgroundColor:[UIColor clearColor]];
            [background addSubview:nameTxtField];
            [nameTxtField release];

            UITextField *nameField = [[UITextField alloc] initWithFrame:CGRectMake(170, 10, 430, control_height)];
            [nameField setText:name];
            [nameField setTextColor:[UIColor blackColor]];
            [nameField setFont:[UIFont fontWithName:@"HelveticaBold" size:19]];
            [nameField setFont:[UIFont boldSystemFontOfSize:nameField.font.pointSize]];
            ...
            UILabel * sizeLabel = [[[UILabel alloc] initWithFrame:CGRectMake(110, control_height+20, 430,control_height)] autorelease];
            NSString *size = [[imagesArray objectAtIndex:indexPath.row] objectForKey:@"Size"];
            sizeLabel.text = [NSString stringWithFormat:@"Size : %@",size];
            sizeLabel.font = [UIFont fontWithName:@"HelveticaBold" size:19];
            sizeLabel.font = [UIFont boldSystemFontOfSize:sizeLabel.font.pointSize];
            ...
            [editButton setFrame:CGRectMake(550, 50, 60, 40)];
            UIImage *editImage = [UIImage imageNamed:@"SFM-Screen-Done-Back-Button"];
            [editImage stretchableImageWithLeftCapWidth:9 topCapHeight:9];
            ...
            [editButton addTarget:self action:@selector(editAttachmentDetails:) forControlEvents:UIControlEventTouchUpInside];
            [background addSubview:editButton];
            

        }
        else if(((selectedSection == SHOW_ALL_CAMERA) &&(section ==1)) ||
                 ((selectedSection == SHOW_CAMERA_ROW) && (index == 1))
                )
        {
            ...
            NSString *name = [[videosArray objectAtIndex:indexPath.row] objectForKey:@"Name"];
            ...
            UIImageView *attachmentImage = [[[UIImageView alloc] initWithFrame:CGRectMake(10, 0, 90, 90)] autorelease];
            [background addSubview:attachmentImage];
            UIImage *image = [[self getThumbnailImageForFile:name] retain];
            [attachmentImage setBackgroundColor:[UIColor blackColor]];
            [attachmentImage setImage:image];
            [image release];
            
            
            UIImageView *playSubView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
            CGRect imageBounds = [attachmentImage bounds];
            playSubView.center = CGPointMake(imageBounds.size.width/2, imageBounds.size.height/2);
            [playSubView setImage:[UIImage imageNamed:@"play.png"]];            
            [attachmentImage addSubview:playSubView];
            [playSubView release];
            
             
            
            UILabel * sizeLabel = [[[UILabel alloc] initWithFrame:CGRectMake(110, control_height+20, 430,control_height)] autorelease];
            NSString *size = [[videosArray objectAtIndex:indexPath.row] objectForKey:@"Size"];
            sizeLabel.text = [NSString stringWithFormat:@"Size : %@",size];
            sizeLabel.font = [UIFont fontWithName:@"HelveticaBold" size:19];
            sizeLabel.font = [UIFont boldSystemFontOfSize:sizeLabel.font.pointSize];
            sizeLabel.textColor = [UIColor blackColor];
...
- (void) editAttachmentDetails:(id)sender
{
    NSLog(@"Edit Attachment Details");
    ...
    {
        oldFileName = [[imagesArray objectAtIndex:indexPath.row] objectForKey:@"Name"];
        extension = @".png";
    }
    else if(indexPath.section == 1)
    {
        isVideo = YES;
        oldFileName = [[videosArray objectAtIndex:indexPath.row] objectForKey:@"Name"]; 
        extension = @".mov";
    }
    NSString *localID  = [appDelegate.calDataBase  getLocalIDForAttachmentName:oldFileName];
    UITableViewCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
    NSArray *cellSubviews = [cell subviews];
    NSArray *backgroundSubViews;
    for(UIView *cellSubView in cellSubviews)
    {
        if(![[[cellSubView class] description] isEqualToString:[[UIImageView class] description]])
        {
            NSArray *cellViews = [cellSubView subviews];
            for(UIView *csView in cellViews)
            {
                if([csView tag] == 4)
                {
                    backgroundSubViews = [csView subviews];
                }
            }
        }
    }
    if(!backgroundSubViews)
    {
        NSLog(@"NO Sub Views ");
        return;
    }
    for(UIView *subView in backgroundSubViews)
    {
        if([subView tag] == 5)
        {
            ...
            {
                NSString *newFileName = [(UITextField *)subView  text];
                if(!newFileName || [newFileName length] == 0)
                    newFileName = oldFileName;
                if(![newFileName Contains:extension])
                {
                    newFileName = [newFileName stringByAppendingFormat:@"%@%@",newFileName,extension];
                }
                NSLog(@"New File Name = %@",newFileName);
                [appDelegate.calDataBase updateFileName:newFileName forLocalID:localID];
                if(isVideo)
                {
                    NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
                    NSString *destinationVideoPath = [documentsDirectory stringByAppendingFormat:@"/videos/%@",oldFileName];
                    NSString *newDestinationVideoPath = [documentsDirectory stringByAppendingFormat:@"/videos/%@",newFileName];
                    
                    NSFileManager *fileManager = [NSFileManager defaultManager];
                    NSError *error = nil;
                    if([fileManager fileExistsAtPath:destinationVideoPath])
                    {
                        [fileManager moveItemAtPath:destinationVideoPath toPath:newDestinationVideoPath error:&error];
                        if(error != nil)
                        {
                            NSLog(@"Unable to move the file from documents folder");
                        }
                    }
            ...
            {
                [(UITextField *)subView setEnabled:YES];
                [(UITextField *)subView setBackgroundColor:[UIColor whiteColor]];
...
{
    NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
	NSString *destinationPath = [documentsDirectory stringByAppendingFormat:@"/videos/%@",videoFile];
    UIImage *theImage = nil;
    NSURL *url = [[NSURL alloc] initFileURLWithPath:destinationPath];
...
- (void)addImages:(id)sender
{
    ...
    {
        self.imageAsset = [[[AssetsImageController alloc] init] autorelease];
        imageAsset.delegate = self;
        self.imagePickerPopover = [[[UIPopoverController alloc] 
                                    initWithContentViewController:imageAsset] autorelease];               
    ...
    if(appDelegate == nil)
        appDelegate = (iServiceAppDelegate *)[[UIApplication sharedApplication] delegate];
    NSDictionary * headerDict = [appDelegate.SFMPage objectForKey:gHEADER];
    NSString * apiName = [headerDict objectForKey:@"hdr_Object_Name"];
    NSString * WoNumber = @"";
    NSString *record_Id = appDelegate.sfmPageController.recordId;
    if ([detailTitle length] > 10)
        WoNumber = [detailTitle substringFromIndex:12];
    
    if ([WoNumber isEqualToString:nil] || [WoNumber isEqualToString:@""] || [WoNumber isEqualToString:@" "])
        WoNumber = @"";

    [dict setObject:WoNumber forKey:@"attachment_Id"];
    [dict setObject:apiName forKey:@"object_api_name"];
    [dict setObject:WoNumber forKey:@"WorkOrderNumber"];
    [dict setObject:record_Id forKey:@"record_Id"];
    if([sender tag] == 0)
        [dict setObject:ALAssetTypePhoto forKey:@"AttachmentType"];
    else
        [dict setObject:ALAssetTypeVideo forKey:@"AttachmentType"];
    self.imageAsset.attachmentDataDict = dict;
    [dict release];
    [self.imagePickerPopover setPopoverContentSize:size animated:YES];
    
    [self.imagePickerPopover presentPopoverFromRect:[sender bounds] inView:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
    
}
- (IBAction)editAttachmentDetailsDone:(id)sender
{
...
- (void)attachmentImageTapped:(id)sender
{
    if(isInViewMode)
    {
...
//    NSString *saveDirectory = [paths objectAtIndex:0];	
    ...
    if(((selectedSection == SHOW_ALL_CAMERA)  && (indexPath.section == 0)) ||
        ((selectedSection == SHOW_CAMERA_ROW) && (selectedRow == 0)))
    {
        //Image
        NSString *imageName = [[imagesArray objectAtIndex:indexPath.row] objectForKey:@"Name"];
        ImageViewController *imageController = [[ImageViewController alloc] init];
        [imageController setImageName:imageName];
        [appDelegate.sfmPageController presentModalViewController:imageController animated:YES];
        [imageController release];
    }
    else if(((selectedSection == SHOW_ALL_CAMERA) && (indexPath.section == 1)) ||
        ((selectedSection == SHOW_CAMERA_ROW) && (selectedRow == 1)))
    {
        //Video
        
        NSString *videoName = [[videosArray objectAtIndex:indexPath.row] objectForKey:@"Name"];
        /*
        VideoPlayerViewController *videoPlayer = [[VideoPlayerViewController alloc] init];
        [videoPlayer setFileName:videoName];
        [appDelegate.sfmPageController presentModalViewController:videoPlayer animated:YES];
        [videoPlayer release];
        */
        NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
        NSString *destinationVideoPath = [documentsDirectory stringByAppendingFormat:@"/videos/%@",videoName];
        NSFileManager *fileManager = [NSFileManager defaultManager];
        if(![fileManager fileExistsAtPath:destinationVideoPath])
        {
...
- (void)dealloc
{
    ...
    [backBtn release];
    [attachmentEditView release];
    [attachmentEditImageView release];
    [attachmentEditTextField release];
    [attachmentEditSize release];
...
- (void) tableView:(UITableView *)_tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
        if(selectedSection == SHOW_ALL_CAMERA || selectedSection == SHOW_CAMERA_ROW)
        {
            ...
            if (selectedSection == SHOW_ALL_CAMERA)
            {
                ...
                {
                    NSArray *images = [appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Image"] ; 
                    fileName = [[images objectAtIndex:indexPath.row] objectForKey:@"Name"];
                    
                }
                else if(indexPath.section == 1)
                {
                    NSArray *videos = [appDelegate.dataBase getImagesForRecordId:recordId withFileType:@"Video"] ; 
                    fileName = [[videos objectAtIndex:indexPath.row] objectForKey:@"Name"];
                    isVideo = YES;
                }                
            }
            else if(selectedSection == SHOW_CAMERA_ROW)
...
- (void) showHelp
{
    ...
    [(SFMPageController *)delegate presentModalViewController:help animated:YES];
    [help release];
    appDelegate.isDetailActive = NO;
}
...
{  
    ...
    if ([[appDelegate.SFMPage objectForKey:gPROCESSTYPE] isEqualToString:@"VIEWRECORD"]) 
    {
        ...
        else if([targetCall isEqualToString:camera])
        {
            NSLog(@"Launch Camera Module");
            if(appDelegate == nil)
                appDelegate = (iServiceAppDelegate *)[[UIApplication sharedApplication] delegate];
            NSDictionary * dict = [appDelegate.SFMPage objectForKey:gHEADER];
            NSString * apiName = [dict objectForKey:@"hdr_Object_Name"];
            NSString * WoNumber = @"";
            NSString *record_Id = appDelegate.sfmPageController.recordId;
            if ([detailTitle length] > 10)
                WoNumber = [detailTitle substringFromIndex:12];
            ...
            if ([WoNumber isEqualToString:nil] || [WoNumber isEqualToString:@""] || [WoNumber isEqualToString:@" "])
                WoNumber = @"";
            NSMutableDictionary *attachmentDataDict = [[NSMutableDictionary alloc] init];
            
            [attachmentDataDict setObject:WoNumber forKey:@"attachment_Id"];
            [attachmentDataDict setObject:apiName forKey:@"object_api_name"];
            [attachmentDataDict setObject:WoNumber forKey:@"WorkOrderNumber"];
            [attachmentDataDict setObject:record_Id forKey:@"record_Id"];
            
            
            CameraViewController *cameraController = [[CameraViewController alloc] init];
            [cameraController setAttachmentDataDict:attachmentDataDict];
            [attachmentDataDict release];
            cameraController.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
            [appDelegate.sfmPageController presentModalViewController:cameraController animated:YES];

        }
        else if([targetCall isEqualToString:video])
        {
            NSLog(@"Launch Video Module");
            if(appDelegate == nil)
                appDelegate = (iServiceAppDelegate *)[[UIApplication sharedApplication] delegate];
            NSDictionary * dict = [appDelegate.SFMPage objectForKey:gHEADER];
            NSString * apiName = [dict objectForKey:@"hdr_Object_Name"];
            ...
            [attachmentDataDict setObject:WoNumber forKey:@"WorkOrderNumber"];
            [attachmentDataDict setObject:record_Id forKey:@"record_Id"];
            [videoController setAttachmentDataDict:attachmentDataDict];
            NSLog(@"Attachment Data Dict = %@",attachmentDataDict);
            [attachmentDataDict release];
            
            videoController.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
            [appDelegate.sfmPageController presentModalViewController:videoController animated:YES];
        }
...
- (void) imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:( NSString *)kUTTypeMovie])
    {
        ...
        NSLog(@"Video URL = %@", urlOfVideo); NSError *dataReadingError = nil;
        NSData *videoData =
        ...
        if (videoData != nil)
        {
            ...
            imageName = [NSString stringWithFormat:@"video%@.mov",[dateFormatter stringFromDate:[NSDate date]]];
            [dateFormatter release];
            filePath = [saveDirectory stringByAppendingPathComponent:imageName];
    ...
    else if ([mediaType isEqualToString:( NSString *)kUTTypeImage])
    {
        ...
        NSLog(@"Metda Data = %@",metadata);
        UIImage *theImage = [info objectForKey:
                             UIImagePickerControllerOriginalImage];
        NSLog(@"Image Orientation = %d", [theImage imageOrientation]); 
        NSData *screenshotImageData = UIImagePNGRepresentation(theImage);
        NSFileManager * fileManager = [NSFileManager defaultManager];
        ...
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd_HH-mm-ss"];
        imageName = [NSString stringWithFormat:@"image%@.png",[dateFormatter stringFromDate:[NSDate date]]];
        [dateFormatter release];
        filePath = [saveDirectory stringByAppendingPathComponent:imageName];
        [fileManager createFileAtPath:filePath contents:screenshotImageData attributes:nil];
    }
    [picker dismissModalViewControllerAnimated:YES]; 
    // NSLog(@"File Name = %@ and File Path = %@",fileName,filePath);
    //[self moveFile:imageName atPath:filePath];
}
- (void) moveFile:(NSString *)oldFileName atPath:(NSString *)filePath
{
    /*
     fileName = [filePath retain];
     CGRect alertViewFrame = [[self view] frame];
     UIAlertView *imageNameAlert = [[UIAlertView alloc] initWithFrame:CGRectMake(alertViewFrame.size.width/2, alertViewFrame.size.height/2, 400, 400)];
     [imageNameAlert setTitle:@"Enter Name of the File"];
     imageNameAlert.alertViewStyle = UIAlertViewStylePlainTextInput;
     [imageNameAlert addButtonWithTitle:@"OK"];
     [imageNameAlert setMessage:[NSString stringWithFormat:@"Default Name of the File is %@",oldFileName]];
     [imageNameAlert setDelegate:self];
     [imageNameAlert show];
     [imageNameAlert release];
     */
}
...
{
    /*
     NSString  *newFileName = [alertView textFieldAtIndex:buttonIndex].text;
     newFileName = [newFileName stringByReplacingOccurrencesOfString:@"." withString:@""];
     newFileName = [newFileName stringByReplacingOccurrencesOfString:@"/" withString:@""];
     NSArray *fileExt = [fileName componentsSeparatedByString:@"."];
     NSLog(@"Ext = %@",fileExt);
     newFileName = [newFileName stringByAppendingString:[NSString stringWithFormat:@".%@", [fileExt objectAtIndex:1]]];
     ...
     NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
     NSString *saveDirectory = [paths objectAtIndex:0];	
     if([pickerData objectAtIndex:[pickerView selectedRowInComponent:0]])
     saveDirectory = [saveDirectory stringByAppendingFormat:@"/%@",[pickerData objectAtIndex:[pickerView selectedRowInComponent:0]]];
     
     //fileName = [saveDirectory stringByAppendingPathComponent:fileName];
     NSLog(@"Old File Path = %@",fileName);
     NSError *error = nil;
     ...
     NSString *oldFileName = [[newFileName componentsSeparatedByString:@"."] objectAtIndex:0];
     if(!oldFileName || [oldFileName length] == 0)
     {
     NSArray *newFileArray = [fileName componentsSeparatedByString:@"/"];
     NSString *newFile = [newFileArray objectAtIndex:([newFileArray count] -1)];
     newFilePath = [saveDirectory stringByAppendingPathComponent:newFile];
     }
     else
     newFilePath = [saveDirectory stringByAppendingPathComponent:newFileName];
     
     [fileManager moveItemAtPath:fileName toPath:newFilePath error:&error];
     if(error == nil)
     {
     ...
     {
     NSLog(@"File Moved Failed");
     }
     }
     */
}
#pragma mark - Camera Attachment
- (void) launchImageController
{
    NSLog(@"Launch Camera Module");
    if(appDelegate == nil)
        appDelegate = (iServiceAppDelegate *)[[UIApplication sharedApplication] delegate];
