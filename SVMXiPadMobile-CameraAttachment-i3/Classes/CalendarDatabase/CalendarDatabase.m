...
#pragma mark - Camera 
- (void) insertCameraData:(NSData *)cameraData 
                 withInfo:(NSDictionary *)dict
{
    NSString * queryStatement = @"";
    NSString * stringData;
    if(cameraData!=nil)
        stringData = [Base64 encode:cameraData];
    NSString *cameraId = [dict objectForKey:@"attachment_Id"];
    NSString *oApiName = [dict objectForKey:@"object_api_name"];
    NSString *WONumber = [dict objectForKey:@"WorkOrderNumber"];
    NSString *recordId = [dict objectForKey:@"record_Id"];
    NSString *fileName = [dict objectForKey:@"fileName"];
    NSString *fileType = [dict objectForKey:@"fileType"];
    NSString *fileSize = [dict objectForKey:@"size"];
    if(cameraData != nil)
        queryStatement = [NSString stringWithFormat:@"INSERT INTO SFCameraAttachmentData ('attachment_Id', 'object_api_name', 'attachment_data', 'WorkOrderNumber' ,'record_Id','fileName','fileType','size') VALUES ('%@','%@', '%@', '%@', '%@', '%@','%@','%@')", cameraId, oApiName, stringData, WONumber, recordId,fileName,fileType,fileSize];
    else
        queryStatement = [NSString stringWithFormat:@"INSERT INTO SFCameraAttachmentData ('attachment_Id', 'object_api_name', 'WorkOrderNumber' ,'record_Id','fileName','fileType','size') VALUES ('%@','%@', '%@', '%@', '%@', '%@','%@')", cameraId, oApiName, WONumber, recordId,fileName,fileType,fileSize];
    ...
    if (ret != SQLITE_OK)
    {
        NSLog(@"Failed to insert in to table SFCameraAttachmentData.");
    }                                                      
}
- (NSString *) getSFIdForCameraAttachmentForData:(NSDictionary *)dict
{
    NSString *recordId = [dict objectForKey:@"RecordId"];
    NSString *objectAPIName = [dict objectForKey:@"ObjectApiName"];
    
    NSString *selectQuery = [NSString stringWithFormat:@"Select Id From '%@' Where local_id = '%@'", objectAPIName, recordId];
    sqlite3_stmt * stmt;
    NSString *SFID = nil;
...
- (void) sendAttachmentToSFDC:(NSString *)SFId 
                 withFileName:(NSString *)fileName
                     fileType:(NSString *)fileType
{
    //Write the attachments to SFDC
    NSString * imageData = @"";
    NSString * recordId = @"";
    NSString *selectQuery = [NSString stringWithFormat:@"Select attachment_data, record_Id from SFCameraAttachmentData Where  fileName = '%@'",fileName];
    ...
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [selectQuery UTF8String], -1, &stmt, NULL) == SQLITE_OK)
    {
        if (synchronized_sqlite3_step(stmt) == SQLITE_ROW) 
        {
            char *_imageData = (char *) synchronized_sqlite3_column_text(stmt, 0);
            if ( _imageData != nil )
                imageData = [NSString stringWithUTF8String:_imageData];            
            char *_recordId = (char *) synchronized_sqlite3_column_text(stmt, 1);
            if ( _recordId != nil )
    ...
    if([fileType isEqualToString:@"Video"])
    {
        ...
        {
            NSData *myData = [NSData dataWithContentsOfFile:destinationVideoPath];
            if(myData!=nil)
                imageData = [Base64 encode:myData];
    ...
    {  
        ...
        NSLog(@"Image Data is Successful");
    }
    // Upload attachments to SFDC
    ZKSObject * obj = [[ZKSObject alloc] initWithType:@"Attachment"];
    [obj setFieldValue:fileName field:@"Name"];
    [obj setFieldValue:imageData field:@"Body"];
    ...
    NSLog(@"File Name  = %@ SFId = %@",fileName,SFId);
    [[ZKServerSwitchboard switchboard] create:array 
                                       target:self 
                                     selector:@selector(didAttachImage:error:context:) 
                                      context:nil];
    
    didAttachmentSuccessful = NO;
    didAttachmentSuccessfulSent = NO;
    while (CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0, YES))
    {
        ...
        if(didAttachmentSuccessful == YES)
        {
            break;
        }
    }
    [[ZKServerSwitchboard switchboard] setLogXMLInOut:NO];
    if (didAttachmentSuccessfulSent)
    {
        [self deleteAttachmentWithName:fileName];
    }
    [array release];
    [obj release];
...
- (void) deleteAttachmentWithName:(NSString *)attachmentName
{
    NSString * query = [NSString stringWithFormat:@"DELETE FROM SFCameraAttachmentData WHERE fileName = '%@'", attachmentName];
...
- (void) didAttachImage:(ZKQueryResult *)result error:(NSError *)error context:(id)context
{
    didAttachmentSuccessful = YES; 
    if(error == nil)
    {
        didAttachmentSuccessfulSent = YES;
    }
}
- (void) sendImagesToSFDC
{
    NSString *selectQuery = [NSString stringWithFormat:@"Select record_Id, object_api_name, fileName From SFCameraAttachmentData where fileType = 'Image'"]; 
    sqlite3_stmt * stmt;
    ...
    NSMutableArray  *attachmentDataArray = [[NSMutableArray alloc] init];
    
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [selectQuery UTF8String], -1, &stmt, NULL) == SQLITE_OK)
    {
        while(synchronized_sqlite3_step(stmt) == SQLITE_ROW) 
        {
            NSMutableDictionary * imageDataDict = [[NSMutableDictionary alloc] initWithCapacity:0];
            char *_recordId = (char *) synchronized_sqlite3_column_text(stmt, COLUMN_1);
            if ( _recordId != nil )
                recordId = [NSString stringWithUTF8String:_recordId];
            
            char *_objectApiName = (char *) synchronized_sqlite3_column_text(stmt, COLUMN_2);
            if ( _objectApiName != nil )
                objApiName = [NSString stringWithUTF8String:_objectApiName];
            
            char *_attachmentName = (char *) synchronized_sqlite3_column_text(stmt, COLUMN_3);
            if ( _attachmentName != nil )
                attachmentName = [NSString stringWithUTF8String:_attachmentName];
            
            [imageDataDict setValue:recordId forKey:@"RecordId"];
            [imageDataDict setValue:objApiName forKey:@"ObjectApiName"];
            [imageDataDict setValue:attachmentName forKey:@"FileName"];
            
            [attachmentDataArray addObject:imageDataDict];
            [imageDataDict release];
        }
    }
    synchronized_sqlite3_finalize(stmt);
    for (int i = 0; i < [attachmentDataArray count]; i++)
    {
        NSDictionary *dataDict = [attachmentDataArray objectAtIndex:i];
        NSString *sfId = [self getSFIdForCameraAttachmentForData:dataDict];
        if(sfId != nil)
            [self sendAttachmentToSFDC:sfId withFileName:[dataDict objectForKey:@"FileName"] fileType:@"Image"];
        else
        {
            NSLog(@"SFId is null deleting the attachment");
            [self deleteAttachmentWithName:[dataDict objectForKey:@"FileName"]];
        }
    }
    [attachmentDataArray release];
}
- (void) updateFileName:(NSString *) fileName forLocalID:(NSString *)localID
{

    NSString *queryString = [NSString stringWithFormat:@"Update SFCameraAttachmentData Set fileName = '%@' Where local_id = '%@'", fileName,localID]; 
    char *err;
    ...
    if (retVal != SQLITE_OK)
    {
        NSLog(@"Failed to Update the table");
    }
    
}
- (NSString *) getLocalIDForAttachmentName:(NSString *) fileName
{
    NSString *selectQuery = [NSString stringWithFormat:@"Select local_id From SFCameraAttachmentData where fileName = '%@'",fileName]; 
    sqlite3_stmt * stmt;
    NSString *localId;
    if (synchronized_sqlite3_prepare_v2(appDelegate.db, [selectQuery UTF8String], -1, &stmt, NULL) == SQLITE_OK)
    {
...
- (void) sendVideosToSFDC
{
    NSString *selectQuery = [NSString stringWithFormat:@"Select record_Id, object_api_name, fileName From SFCameraAttachmentData where fileType = 'Video'"]; 
    sqlite3_stmt * stmt;
    ...
    for (int i = 0; i < [attachmentDataArray count]; i++)
    {
        ...
        NSString *sfId = [self getSFIdForCameraAttachmentForData:dataDict];
        if(sfId != nil)
            [self sendAttachmentToSFDC:sfId withFileName:[dataDict objectForKey:@"FileName"] fileType:@"Video"];
        else
        {
            NSLog(@"SFId is null deleting the attachment");
            [self deleteAttachmentWithName:[dataDict objectForKey:@"FileName"]];
...
- (void) deleteFromDocumentsWithVideoFileName:(NSString *)videoName
{
    NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *destinationPath = [documentsDirectory stringByAppendingFormat:@"/videos/%@",videoName];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    [fileManager removeItemAtPath:destinationPath error:&error];
    if(error!= nil)
    {
        NSLog(@"Unable to delete the file %@ From Documents/videos Folder",videoName);
    }
}
#pragma mark - Summary
