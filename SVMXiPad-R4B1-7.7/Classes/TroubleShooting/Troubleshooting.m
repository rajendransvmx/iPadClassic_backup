...
- (NSString *) showTroubleshootingForIndex:(NSUInteger)index
{
    ...
    folderNameToCreate = fileId;
    
    NSArray * keys = [[NSArray alloc] initWithObjects:FILEID, FILENAME, FOLDERNAMETOCREATE, CELLINDEX, nil];
    NSArray * objects = [[NSArray alloc] initWithObjects:fileId, fileName, folderNameToCreate, [NSNumber numberWithInt:index], nil];
    ...
    BOOL isDirectory = NO;
    NSString * folderPath = [documentsDirectoryPath stringByAppendingPathComponent:folderNameToCreate];
    BOOL retVal = [fileManager fileExistsAtPath:folderPath isDirectory:&isDirectory];
    BOOL doesFileExist = [fileManager fileExistsAtPath:[NSString stringWithFormat:@"%@%@", folderPath, @".zip"]];
    // check if folderNameToCreate already exists
    if (!((retVal == YES) && (isDirectory == YES)) && !doesFileExist)
    {
        if (!appDelegate.isInternetConnectionAvailable)
        {
            [activity stopAnimating];
    ...
    else
    {
        ...
        NSString * actualFilePath = [folderPath stringByAppendingPathComponent:fileName];
        actualFilePath = [actualFilePath stringByAppendingPathComponent:@"index.html"];
        
        NSURL * baseURL = [NSURL fileURLWithPath:[folderPath stringByAppendingPathComponent:fileName]];
        NSError * error;
        NSString * fileContents = [NSString stringWithContentsOfFile:actualFilePath encoding:NSUTF8StringEncoding error:&error];
        if (fileContents == nil)
        {
            NSString * filePath = [documentsDirectoryPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@%@", fileId, @".zip"]];
            ...
            // GIVE UP AFTER THIS LAST ATTEMPT
        }
