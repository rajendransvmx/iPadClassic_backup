...
-(NSMutableDictionary *) getAllRecordsFromRecordsHeap
{
    ...
    NSString * query = [NSString  stringWithFormat:@"SELECT sf_id, object_name from sync_Records_Heap where sync_flag = 'false'"];
    sqlite3_stmt * stmt;
    NSString * object_name = @"";
    NSString * sf_id = @"";
    if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK)
...
{
    NSLog(@"SAMMAN updateAllRecordsToSyncRecordsHeap Processing starts: %@", [NSDate date]);
...
-(NSString *) getchildInfoFromChildRelationShip:(NSString * )tableName  ForChild:(NSString *)child_table  field_name:(NSString *)field_name
{
    ...
    if(child_table != nil )
    {
        NSString * query = [NSString stringWithFormat:@"SELECT %@ FROM '%@' where  object_api_name_child = '%@' ", field_name,tableName ,child_table];
        sqlite3_stmt * stmt ;
        if(synchronized_sqlite3_prepare_v2(appDelegate.db, [query UTF8String], -1, &stmt, nil) == SQLITE_OK  )
        {
            while(synchronized_sqlite3_step(stmt) == SQLITE_ROW)
...
-(NSMutableDictionary *)getDictForJsonString:(NSString *)json_record
{
    ...
    NSLog(@"%d, %d", [json_record retainCount], [json_record length]);

    NSMutableDictionary * lookUpDict = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
    NSMutableDictionary * final_dict = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
