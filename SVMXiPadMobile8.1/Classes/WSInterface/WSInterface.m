...
-(NSString *)get_SYNCHISTORYTime_ForKey:(NSString *)forkey
{
    ...
    for(NSString * str in allkeys)
    {
        ...
        {
            value =  [dict objectForKey:forkey];
        }
    }
...
//sahana 26/feb
-(void)setSyncStatus:(NSString *)success_flag
{
    NSString * rootpath_SYNHIST = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString * plistPath_SYNHIST = [rootpath_SYNHIST stringByAppendingPathComponent:SYNC_HISTORY];
...
last_sync_time:(NSString *)last_sync_time
{
    ...
    if([sync_type isEqualToString:INSERT] && [operation_type isEqualToString:RESPONSE])
    {
        for(NSString *  str in allkeys)
        {
            ...
            /*else if ([str isEqualToString:INSERT_SUCCESS])
            {
                [dict setObject:@"true" forKey:INSERT_SUCCESS];
            }*/
        }
    }
    if([sync_type isEqualToString:UPDATE] && [operation_type isEqualToString:REQUEST])
    {
        for(NSString *  str in allkeys)
        {
    ...
    if([sync_type isEqualToString:UPDATE] && [operation_type isEqualToString:RESPONSE])
    {
        for(NSString *  str in allkeys)
        {
           ...
           /* else if ([str isEqualToString:UPDATE_SUCCESS])
            {
                [dict setObject:@"true" forKey:UPDATE_SUCCESS];
            }*/
        }
    }
    if([sync_type isEqualToString:DELETE] && [operation_type isEqualToString:REQUEST])
    {
        for(NSString *  str in allkeys)
        {
...
-(void)copyTrailertoTempTrailer:(NSString *)operation_type
{
    ...
    if([operation_type isEqualToString:INSERT])
    {
        ...
        BOOL last_sync_status = [self getSyncStatusForRequestId];
        if(last_sync_status)
        {
...
//        if (isLoggedIn)
    ...
    if ([operation isKindOfClass:[INTF_WebServicesDefBinding_INTF_DataSync_WS class]])
    {
        ...
        if ([wsResponse.result.eventName isEqualToString:@"DATA_SYNC"])
        {
                ...
                else if ([key isEqualToString:@"Child_Object"] || [key isEqualToString:@"Parent_Object"])
                {
                    ...
                    for (int j = 0; j < [valueMap count]; j++)
                    {
                        ...
                        for (int k = 0; k < [values count]; k++)
                        {
                            ...
                            if(flag)
                            {
                                NSDictionary * dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:@"", @"",sf_id,event_name,record_type,nil] forKeys:[NSArray arrayWithObjects:@"LOCAL_ID",@"JSON_RECORD",@"SF_ID",@"SYNC_TYPE", @"RECORD_TYPE",nil]];
                                NSMutableArray * array = [record_dict objectForKey:objectApiName];
                                [array addObject:dict];
                            }
                            else
                            {
                                ...
                                NSMutableArray * array = [[NSMutableArray alloc] initWithCapacity:0];
                                [array addObject:dict];
                                [record_dict setObject:array forKey:objectApiName];
                            }
                        }
                    }
                }
            ...
            NSLog(@"SAMMAN DATA_SYNC Processing ends: %@", [NSDate date]);
        ...
        else if ([wsResponse.result.eventName isEqualToString:@"GET_INSERT"] || [wsResponse.result.eventName isEqualToString:@"PUT_INSERT"] )//INSERT
        {
        ...
        else if ([wsResponse.result.eventName isEqualToString:@"GET_UPDATE"] || [wsResponse.result.eventName isEqualToString:@"PUT_UPDATE"] )//INSERT
        {
        ...
        else if ([wsResponse.result.eventName isEqualToString:@"GET_DELETE"] || [wsResponse.result.eventName isEqualToString:@"PUT_DELETE"] )
        {
            NSString * event_name = wsResponse.result.eventName;
            NSMutableDictionary * record_dict = [[NSMutableDictionary alloc] initWithCapacity:0];
            NSMutableDictionary * conflict_dict = [[[NSMutableDictionary alloc] initWithCapacity:0] autorelease];
...
-(NSMutableArray *)getIdsFromJsonString:(NSString *)jsonstrings
{
    ...
    jsonstrings = [jsonstrings stringByReplacingOccurrencesOfString:@"},{" withString:@"}$,${"];
    NSArray * records_array = [jsonstrings componentsSeparatedByString:@"$,$"];
    NSMutableArray * records_list = [[[NSMutableArray alloc] initWithCapacity:0] autorelease];
    for(NSString * jsonString in records_array)
    {
        NSDictionary * jsonDict = [jsonParser objectWithString:jsonString];
        NSLog(@"json Dict  ");
